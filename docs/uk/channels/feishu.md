...
Feishu/Lark — це універсальна платформа для співпраці, де команди спілкуються в чаті, обмінюються документами, керують календарями та працюють разом.

**Стан:** готово до використання в продакшені для особистих повідомлень боту та групових чатів. WebSocket — режим за замовчуванням; режим Webhook — необов’язковий.
...
Відкрийте групу в Feishu/Lark, натисніть значок меню у верхньому правому куті та перейдіть у **Settings**. Ідентифікатор групи (`chat_id`) указано на сторінці налаштувань.
...
Знайдіть `open_id` у виводі журналу. Ви також можете перевірити незавершені запити на сполучення:
...
глобальної конфігурації TTS, тому конфігурації Feishu з кількома ботами можуть зберігати спільні
облікові дані провайдерів глобально, перевизначаючи для кожного облікового запису лише voice, model, persona або auto mode.
...
Feishu/Lark підтримує потокові відповіді через інтерактивні картки. Якщо цю функцію ввімкнено, бот оновлює картку в реальному часі під час генерування тексту.
...
`--thread here` працює для особистих повідомлень і повідомлень у гілках Feishu/Lark. Подальші повідомлення в прив’язаній розмові напряму маршрутизуються до цієї сесії ACP.
...
Поради щодо пошуку див. в [Отримання ідентифікаторів груп/користувачів](#get-groupuser-ids).
...
| `channels.feishu.allowFrom`                       | Дозволений список для особистих повідомлень (`open_id`) | [BotOwnerId]     |
...
завантажує ресурс голосового повідомлення та запускає спільну транскрипцію аудіо перед
ходом агента, щоб агент отримав розшифрований текст мовлення. Якщо Feishu уже містить
текст транскрипції безпосередньо в аудіонавантаженні, цей текст використовується без додаткового
виклику ASR. Без провайдера транскрипції аудіо агент усе одно отримує
заповнювач `<media:audio>` разом зі збереженим вкладенням, а не сире
навантаження ресурсу Feishu.
...
транскодуються в 48 кГц Ogg/Opus за допомогою `ffmpeg` лише тоді, коли відповідь запитує
голосову доставку (`audioAsVoice` / інструмент повідомлень `asVoice`, включно з голосовими відповідями
TTS у вигляді голосових повідомлень). Звичайні вкладення MP3 залишаються звичайними файлами. Якщо `ffmpeg` відсутній або
конвертація не вдається, OpenClaw повертається до вкладення файлу та записує причину в журнал.
...
Звичайні групові відповіді, які OpenClaw перетворює на гілки, і далі використовують
ID кореневого повідомлення відповіді (`om_*`), щоб перший хід і наступний хід
залишалися в межах однієї сесії.
