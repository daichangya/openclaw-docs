...
Керування ізольованими агентами (робочі простори + автентифікація + маршрутизація).
...
Якщо ви не вкажете `--agent` для `bind` або `unbind`, OpenClaw використовує поточного типового агента.
...
- Передавання будь-яких явних прапорців add перемикає команду в неінтерактивний режим.
- Неінтерактивний режим вимагає як ім’я агента, так і `--workspace`.
- `main` зарезервовано і не може використовуватися як новий id агента.
...
- Каталоги робочого простору, стану агента та транскриптів сесій переміщуються до Trash, а не видаляються безповоротно.
