...
x-i18n:
    generated_at: "2026-04-25T22:53:52Z"
    model: gpt-5.4
    provider: openai
...
OpenClaw може перетворювати вихідні відповіді на аудіо за допомогою ElevenLabs, Google Gemini, Gradium, Inworld, Local CLI, Microsoft, MiniMax, OpenAI, Volcengine, Vydra, xAI або Xiaomi MiMo.
...
хостинговий сервіс (не локальний), він використовує кінцеві точки Microsoft
і не потребує API-ключа.
`node-edge-tts` надає параметри конфігурації мовлення та формати виводу, але
не всі параметри підтримуються сервісом. Застарілі конфігурації та вхідні
директиви з `edge` усе ще працюють і нормалізуються до `microsoft`.

Оскільки цей шлях використовує публічний вебсервіс без опублікованої SLA або квоти,
вважайте його найкращою спробою. Якщо вам потрібні гарантовані ліміти та підтримка,
використовуйте OpenAI або ElevenLabs.

## Необов’язкові ключі

Якщо ви хочете використовувати ElevenLabs, Google Gemini, Gradium, Inworld, MiniMax, OpenAI, Volcengine, Vydra, xAI або Xiaomi MiMo:
...
`MINIMAX_CODING_API_KEY`), потім `MINIMAX_API_KEY`. Якщо явний `baseUrl` для TTS
не задано, OpenClaw може повторно використати налаштований OAuth-хост `minimax-portal`
...
кодування, тому не передавайте сирий bearer token і не виконуйте
Base64-кодування самостійно. Ключ має резервний варіант у змінній середовища
`INWORLD_API_KEY`. Див. [Провайдер Inworld](/uk/providers/inworld) для повного налаштування.
...
провайдерів моделей Doubao. Порядок визначення:
`messages.tts.providers.volcengine.apiKey` ->
...
рідний для провайдера формат `ogg_opus`; звичайні цілі аудіофайлів запитують `mp3`.
...
### Лише відповідати аудіо після вхідного голосового повідомлення
...
- Застарілу конфігурацію `provider: "edge"` виправляє `openclaw doctor --fix`, і
  її буде переписано на `provider: "microsoft"`.
...
- Значення `apiKey` мають резервні варіанти у змінних середовища (`ELEVENLABS_API_KEY`/`XI_API_KEY`, `GEMINI_API_KEY`/`GOOGLE_API_KEY`, `GRADIUM_API_KEY`, `INWORLD_API_KEY`, `MINIMAX_API_KEY`, `OPENAI_API_KEY`, `VYDRA_API_KEY`, `XAI_API_KEY`, `XIAOMI_API_KEY`). Volcengine натомість використовує `appId`/`token`.
...
- `providers.openai.baseUrl`: перевизначає кінцеву точку OpenAI TTS.
  ...
  - Нестандартні значення вважаються OpenAI-сумісними кінцевими точками TTS, тому дозволені власні назви моделей і голосів.
- `providers.elevenlabs.voiceSettings`:
  - `stability`, `similarityBoost`, `style`: `0..1`
  - `useSpeakerBoost`: `true|false`
  - `speed`: `0.5..2.0` (1.0 = нормальна швидкість)
- `providers.elevenlabs.applyTextNormalization`: `auto|on|off`
- `providers.elevenlabs.languageCode`: 2-літерний ISO 639-1 (наприклад, `en`, `de`)
- `providers.elevenlabs.seed`: ціле число `0..4294967295` (детермінізм у межах найкращої спроби)
...
- `providers.minimax.pitch`: ціле зміщення тону `-12..12` (типово 0). Дробові значення усікаються перед викликом MiniMax T2A, оскільки API відхиляє нецілі значення тону.
- `providers.tts-local-cli.command`: локальний виконуваний файл або командний рядок для CLI TTS.
...
- `providers.google.audioProfile`: запит у природній мові зі стилем, який додається перед озвучуваним текстом.
- `providers.google.speakerName`: необов’язкова мітка мовця, яка додається перед озвучуваним текстом, якщо ваш TTS-запит використовує іменованого мовця.
...
- `providers.volcengine.resourceId`: ідентифікатор ресурсу BytePlus Seed Speech (типово
  ...
  ваш проєкт BytePlus має право доступу до TTS 2.0).
...
- `providers.xiaomi.style`: необов’язкова стильова інструкція природною мовою, що надсилається як повідомлення користувача; вона не озвучується.
...
- `providers.microsoft.outputFormat`: формат виводу Microsoft (наприклад, `audio-24khz-48kbitrate-mono-mp3`).
  - Дійсні значення див. у форматах виводу Microsoft Speech; не всі формати підтримуються вбудованим транспортом на основі Edge.
...
- `providers.microsoft.proxy`: URL-адреса проксі для запитів до Microsoft speech.
...
Необов’язковий список дозволів (увімкнути перемикання провайдера, залишаючи інші параметри налаштовуваними):
...
- **Feishu**: коли відповідь у форматі голосової нотатки створюється як MP3/WAV/M4A або інший
  імовірний аудіофайл, Plugin Feishu транскодує її до 48 кГц Ogg/Opus за допомогою
...
- **Xiaomi MiMo**: MP3 за замовчуванням або WAV, якщо налаштовано. Для цілей голосових нотаток, таких як Feishu і Telegram, OpenClaw транскодує вивід Xiaomi у 48 кГц Opus за допомогою `ffmpeg` перед доставкою.
- **Local CLI**: використовує налаштований `outputFormat`. Цілі голосових нотаток
  перетворюються в Ogg/Opus, а вивід для телефонії перетворюється в сирий 16 кГц моно PCM
  за допомогою `ffmpeg`.
...
- **xAI**: MP3 за замовчуванням; `responseFormat` може бути `mp3`, `wav`, `pcm`, `mulaw` або `alaw`. OpenClaw використовує пакетний REST TTS endpoint xAI і повертає завершене аудіовкладення; потоковий TTS WebSocket xAI не використовується цим шляхом провайдера. Нативний формат голосових нотаток Opus цим шляхом не підтримується.
...
Є одна команда: `/tts`.
Докладніше про ввімкнення див. у [Слеш-командах](/uk/tools/slash-commands).

Примітка для Discord: `/tts` — це вбудована команда Discord, тому OpenClaw реєструє
`/voice` як нативну команду там. Текстовий `/tts ...` усе одно працює.
...
- Збої API OpenAI і ElevenLabs тепер включають розібрані деталі помилки провайдера та ідентифікатор запиту (коли провайдер його повертає), які відображаються в помилках/журналах TTS.
...
Feishu може транскодувати вивід TTS, що не є Opus, на цьому шляху, якщо доступний
`ffmpeg`.
