...
x-i18n:
    generated_at: "2026-04-26T03:31:41Z"
    model: gpt-5.4
    provider: openai
    source_hash: ee32af79e2b6c7f5104b6583c3b1ceca23284611572bbb0bbca6e95ce333f488
...
Це працює скрізь, де OpenClaw може надсилати аудіо.
...
- **Gradium** (основний або резервний постачальник; підтримує голосові нотатки та вихід для телефонії)
- **Inworld** (основний або резервний постачальник; використовує потоковий Inworld TTS API)
- **Local CLI** (основний або резервний постачальник; запускає налаштовану локальну команду TTS)
...
- **Vydra** (основний або резервний постачальник; спільний постачальник зображень, відео й мовлення)
- **xAI** (основний або резервний постачальник; використовує xAI TTS API)
- **Xiaomi MiMo** (основний або резервний постачальник; використовує MiMo TTS через Xiaomi chat completions)

### Примітки щодо мовлення Microsoft

Наразі вбудований постачальник мовлення Microsoft використовує онлайн-сервіс
нейронного TTS від Microsoft Edge через бібліотеку `node-edge-tts`. Це
хмарний сервіс (не локальний), він використовує кінцеві точки Microsoft і не
потребує API-ключа.
...
з використанням `edge` досі працюють і нормалізуються до `microsoft`.

Оскільки цей шлях використовує публічний вебсервіс без опублікованих SLA або квот,
вважайте його рішенням із режимом best-effort. Якщо вам потрібні гарантовані ліміти
та підтримка, використовуйте OpenAI або ElevenLabs.
...
- `AZURE_SPEECH_KEY` разом із `AZURE_SPEECH_REGION` (також приймає
...
Якщо налаштовано кілька постачальників, спочатку використовується вибраний постачальник, а інші слугують резервними варіантами.
...
Використовуйте `agents.list[].tts`, якщо один агент має говорити з іншим постачальником,
...
4. вбудовані директиви `[[tts:...]]`, якщо ввімкнено перевизначення моделей
...
`baseUrl` не задано, OpenClaw може повторно використати налаштований хост OAuth
...
кодування, тому не передавайте сирий bearer token і не кодуйте в Base64
його самостійно. Ключ бере значення з резервного `INWORLD_API_KEY` env var. Див.
...
а не OpenAI-сумісний `VOLCANO_ENGINE_API_KEY`, який використовується для постачальників
...
досі працює через `messages.tts.providers.volcengine.appId` / `token` або
`VOLCENGINE_TTS_APPID` / `VOLCENGINE_TTS_TOKEN`. Цілі для голосових нотаток
запитують рідний для постачальника `ogg_opus`; звичайні цілі аудіофайлів запитують `mp3`.
...
xAI TTS використовує той самий шлях `XAI_API_KEY`, що й вбудований постачальник моделі Grok.
...
Цілі для голосових нотаток транскодуються в Ogg/Opus, а вихід для телефонії
транскодується в raw 16 kHz mono PCM за допомогою `ffmpeg`. Застарілий псевдонім постачальника
`cli` досі працює, але в новій конфігурації слід використовувати `tts-local-cli`.
...
### Користувацькі ліміти + шлях до prefs
...
- `summaryModel`: необов’язкова недорога модель для auto-summary; типово використовується `agents.defaults.model.primary`.
...
- `maxTextLength`: жорстка верхня межа для вхідного тексту TTS (символи). Якщо її перевищено, `/tts audio` завершується з помилкою.
- `timeoutMs`: тайм-аут запиту (мс).
- `prefsPath`: перевизначає локальний шлях до JSON-файлу prefs (постачальник/ліміт/summary).
- Значення `apiKey` беруться з env vars, якщо не задані явно (`AZURE_SPEECH_KEY`/`AZURE_SPEECH_API_KEY`/`SPEECH_KEY`, `ELEVENLABS_API_KEY`/`XI_API_KEY`, `GEMINI_API_KEY`/`GOOGLE_API_KEY`, `GRADIUM_API_KEY`, `INWORLD_API_KEY`, `MINIMAX_API_KEY`, `OPENAI_API_KEY`, `VYDRA_API_KEY`, `XAI_API_KEY`, `XIAOMI_API_KEY`). Volcengine натомість використовує `appId`/`token`.
...
- `providers.openai.baseUrl`: перевизначає endpoint OpenAI TTS.
  ...
  - Значення, відмінні від типового, розглядаються як OpenAI-сумісні endpoints TTS, тому допускаються користувацькі назви моделей і голосів.
...
- `providers.minimax.pitch`: ціле зміщення висоти тону `-12..12` (типово 0). Дробові значення обрізаються перед викликом MiniMax T2A, оскільки API відхиляє нецілі значення висоти тону.
...
- `providers.google.audioProfile`: запит стилю природною мовою, який додається перед озвучуваним текстом.
...
- `providers.google.baseUrl`: перевизначає базовий URL Gemini API. Дозволено лише `https://generativelanguage.googleapis.com`.
  - Якщо `messages.tts.providers.google.apiKey` пропущено, TTS може повторно використати `models.providers.google.apiKey` перед переходом до env vars.
...
- `providers.volcengine.speedRatio`: рідне для постачальника співвідношення швидкості.
- `providers.volcengine.emotion`: рідний для постачальника тег емоції.
...
- `providers.xai.speed`: рідне для постачальника перевизначення швидкості.
...
- `providers.openrouter.speed`: рідне для постачальника перевизначення швидкості.
...
надання виразних тегів (сміх, підказки для співу тощо), які мають з’являтися лише в
аудіо.

Потокова доставка блоків прибирає ці директиви з видимого тексту до того,
як канал їх побачить, навіть коли директиву розділено між сусідніми блоками. Режим
...
аналізуються лише цим постачальником. Непідтримувані ключі прибираються з видимого тексту
...
- `pitch` (ціле значення висоти тону MiniMax, від -12 до 12; дробові значення обрізаються перед запитом до MiniMax)
...
Необов’язковий allowlist (увімкнення перемикання постачальника, зберігаючи інші параметри доступними для налаштування):
...
- **Feishu / WhatsApp**: коли відповідь-голосова нотатка створюється у форматі MP3/WebM/WAV/M4A
  або іншого ймовірного аудіофайлу, channel Plugin транскодує її в 48 кГц
  Ogg/Opus за допомогою `ffmpeg` перед надсиланням нативного голосового повідомлення. WhatsApp надсилає
  результат через payload `audio` у Baileys з `ptt: true` і
  `audio/ogg; codecs=opus`. Якщо перетворення не вдається, Feishu отримує початковий
  файл як вкладення; у WhatsApp надсилання завершується помилкою, а не публікує несумісний
  payload PTT.
...
- **Xiaomi MiMo**: MP3 за замовчуванням або WAV, якщо налаштовано. Для цілей голосових нотаток, таких як Feishu, Telegram і WhatsApp, OpenClaw транскодує вихід Xiaomi у 48 кГц Opus за допомогою `ffmpeg` перед доставкою.
- **Local CLI**: використовує налаштований `outputFormat`. Цілі для голосових нотаток
  перетворюються в Ogg/Opus, а вихід для телефонії перетворюється в raw 16 kHz mono PCM
  за допомогою `ffmpeg`.
- **Google Gemini**: Gemini API TTS повертає raw 24 kHz PCM. OpenClaw обгортає його як WAV для аудіовкладень, транскодує в 48 кГц Opus для цілей голосових нотаток і повертає PCM безпосередньо для Talk/телефонії.
- **Gradium**: WAV для аудіовкладень, Opus для цілей голосових нотаток і `ulaw_8000` на 8 кГц для телефонії.
- **Inworld**: MP3 для звичайних аудіовкладень, нативний `OGG_OPUS` для цілей голосових нотаток і raw `PCM` на 22050 Гц для Talk/телефонії.
- **xAI**: типово MP3; `responseFormat` може бути `mp3`, `wav`, `pcm`, `mulaw` або `alaw`. OpenClaw використовує пакетний REST endpoint TTS від xAI і повертає готове аудіовкладення; потоковий TTS WebSocket від xAI не використовується в цьому шляху постачальника. Нативний формат голосових нотаток Opus у цьому шляху не підтримується.
...
- створює підсумок довгих відповідей, коли це ввімкнено, використовуючи `agents.defaults.model.primary` (або `summaryModel`).
- додає згенероване аудіо до відповіді.
- у `mode: "final"` усе одно надсилає TTS лише як аудіо для потокових фінальних відповідей
  після завершення текстового потоку; згенеровані медіа проходять через ту саму
...
- `/tts chat on|off|default` записує перевизначення auto-TTS в межах поточної сесії для поточного чату.
...
- `/tts latest` зчитує останню відповідь асистента з транскрипту поточної сесії й надсилає її як аудіо один раз. На записі сесії зберігається лише хеш цієї відповіді, щоб запобігти дублюванню голосових надсилань.
...
- `/status` показує активний режим TTS разом із налаштованими постачальником, моделлю, голосом
  і очищеними метаданими користувацького endpoint, коли TTS увімкнено.
- Помилки API OpenAI та ElevenLabs тепер включають розібрані деталі помилки постачальника та request id (коли постачальник їх повертає), які відображаються в помилках/логах TTS.
...
Feishu і WhatsApp можуть транскодувати не-Opus вихід TTS у цьому шляху, коли
доступний `ffmpeg`.
