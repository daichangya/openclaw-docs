...
x-i18n:
    generated_at: "2026-04-26T03:13:11Z"
    model: gpt-5.4
    provider: openai
...
Поточний вбудований постачальник Microsoft speech використовує онлайн-сервіс
...
потребує API-ключа. `node-edge-tts` надає параметри конфігурації мовлення та
формати виводу, але не всі параметри підтримуються сервісом. Застаріла
конфігурація та вхідні директиви з `edge` усе ще працюють і нормалізуються до
`microsoft`.

Оскільки цей шлях використовує публічний вебсервіс без опублікованого SLA або
...
Якщо налаштовано кілька постачальників, спочатку використовується вибраний постачальник, а решта виступають як резервні варіанти.
...
```json5
{
  messages: {
    tts: {
      ...
      providers: {
        "azure-speech": {
          // apiKey uses AZURE_SPEECH_KEY as fallback.
          // region uses AZURE_SPEECH_REGION as fallback.
...
визначення такий: `messages.tts.providers.azure-speech.apiKey` ->
...
Google Gemini TTS використовує шлях API-ключа Gemini. Тут також підходить ключ API Google Cloud Console,
...
самостійно. Ключ бере значення з резервного env var `INWORLD_API_KEY`. Див.
...
а не сумісний з OpenAI `VOLCANO_ENGINE_API_KEY`, що використовується для постачальників
моделей Doubao. Порядок визначення такий: `messages.tts.providers.volcengine.apiKey` ->
...
постачальник моделей OpenRouter. Порядок визначення такий:
...
- `enabled`: застарілий перемикач (doctor переносить це в `auto`).
...
- `maxTextLength`: жорстке обмеження для вхідного тексту TTS (символи). `/tts audio` завершується з помилкою, якщо ліміт перевищено.
- `timeoutMs`: тайм-аут запиту (мс).
- `prefsPath`: перевизначає локальний шлях до JSON-файлу prefs (постачальник/ліміт/підсумок).
- Значення `apiKey` використовують резервні env vars (`AZURE_SPEECH_KEY`/`AZURE_SPEECH_API_KEY`/`SPEECH_KEY`, `ELEVENLABS_API_KEY`/`XI_API_KEY`, `GEMINI_API_KEY`/`GOOGLE_API_KEY`, `GRADIUM_API_KEY`, `INWORLD_API_KEY`, `MINIMAX_API_KEY`, `OPENAI_API_KEY`, `VYDRA_API_KEY`, `XAI_API_KEY`, `XIAOMI_API_KEY`). Volcengine натомість використовує `appId`/`token`.
...
- `providers.openai.baseUrl`: перевизначає endpoint OpenAI TTS.
  ...
  - Значення, відмінні від типових, вважаються сумісними з OpenAI TTS endpoint, тому приймаються власні назви моделей і голосів.
...
- `providers.elevenlabs.seed`: ціле число `0..4294967295` (детермінізм best-effort)
...
- `providers.minimax.pitch`: цілочисельний зсув тону `-12..12` (типово 0). Дробові значення відкидаються перед викликом MiniMax T2A, оскільки API відхиляє нецілочисельні значення тону.
...
- `providers.google.baseUrl`: перевизначає базовий URL Gemini API. Приймається лише `https://generativelanguage.googleapis.com`.
  - Якщо `messages.tts.providers.google.apiKey` пропущено, TTS може повторно використати `models.providers.google.apiKey` до переходу до env fallback.
...
- `providers.openrouter.voice`: власний для постачальника ідентифікатор голосу (типово `af_alloy`; також приймається `voiceId`).
...
Потокова доставка блоків видаляє ці директиви з видимого тексту до того, як канал їх побачить, навіть коли директива розділена між сусідніми блоками. Режим final усе одно аналізує накопичену сиру відповідь для синтезу TTS.
...
Необов’язковий список дозволів (увімкнення перемикання постачальника, зберігаючи можливість налаштовувати інші параметри):
...
- **Feishu / WhatsApp**: коли відповідь голосовою нотаткою створюється як MP3/WebM/WAV/M4A
  або інший імовірний аудіофайл, channel Plugin транскодує її в 48kHz
...
- **MiniMax**: MP3 (модель `speech-2.8-hd`, частота дискретизації 32kHz) для звичайних аудіовкладень. Для цілей голосових нотаток, таких як Feishu, Telegram і WhatsApp, OpenClaw транскодує MP3 від MiniMax у 48kHz Opus за допомогою `ffmpeg` перед доставленням.
- **Xiaomi MiMo**: MP3 за замовчуванням або WAV, якщо налаштовано. Для цілей голосових нотаток, таких як Feishu, Telegram і WhatsApp, OpenClaw транскодує вихід Xiaomi у 48kHz Opus за допомогою `ffmpeg` перед доставленням.
- **Local CLI**: використовує налаштований `outputFormat`. Цілі голосових нотаток
  ...
  за допомогою `ffmpeg`.
- **Google Gemini**: Gemini API TTS повертає raw 24kHz PCM. OpenClaw обгортає його у WAV для аудіовкладень, транскодує в 48kHz Opus для цілей голосових нотаток і повертає PCM напряму для Talk/телефонії.
- **Gradium**: WAV для аудіовкладень, Opus для цілей голосових нотаток і `ulaw_8000` на 8 kHz для телефонії.
- **Inworld**: MP3 для звичайних аудіовкладень, нативний `OGG_OPUS` для цілей голосових нотаток і raw `PCM` на 22050 Hz для Talk/телефонії.
- **xAI**: MP3 за замовчуванням; `responseFormat` може бути `mp3`, `wav`, `pcm`, `mulaw` або `alaw`. OpenClaw використовує пакетний REST TTS endpoint xAI і повертає завершене аудіовкладення; потоковий TTS WebSocket xAI не використовується цим шляхом постачальника. Нативний формат голосових нотаток Opus цим шляхом не підтримується.
...
Є одна команда: `/tts`.
Див. [Команди зі слешем](/uk/tools/slash-commands) для деталей щодо ввімкнення.
...
- `/tts chat on|off|default` записує перевизначення авто-TTS з областю дії сесії для поточного чату.
...
- `/tts latest` зчитує останню відповідь асистента з транскрипту поточної сесії та надсилає її як аудіо один раз. Вона зберігає лише хеш цієї відповіді в записі сесії, щоб уникнути дубльованого надсилання голосу.
...
- `/status` показує активний режим TTS разом із налаштованими постачальником, моделлю, голосом
  і очищеними метаданими custom endpoint, коли TTS увімкнено.
- Помилки API OpenAI і ElevenLabs тепер включають розібрані деталі помилки постачальника та request id (коли їх повертає постачальник), які відображаються в помилках/журналах TTS.
...
Feishu і WhatsApp можуть транскодувати вихід TTS не в Opus на цьому шляху, коли
доступний `ffmpeg`.
