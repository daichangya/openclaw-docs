...
x-i18n:
    generated_at: "2026-04-06T03:21:19Z"
    model: gpt-5.4
    provider: openai
...
має або використовувати ці локальні для plugin barrel-файли, або додати вузький загальний контракт SDK,
...
підшляхи існують лише для підтримки й сумісності вбудованих plugins; їх
...
згенерованому списку. Розглядайте їх як деталі реалізації/поверхні сумісності, якщо тільки сторінка документації
...
<AccordionGroup>
  <Accordion title="Підшляхи channel">
    ...
    | `plugin-sdk/account-core` | Допоміжні засоби багатoоблікового запису config/action-gate, допоміжні засоби резервного варіанта облікового запису за замовчуванням |
    ...
    | `plugin-sdk/channel-config-schema` | Типи схеми config для channel |
    ...
    | `plugin-sdk/inbound-reply-dispatch` | Спільні допоміжні засоби запису й диспетчеризації inbound |
    ...
    | `plugin-sdk/outbound-runtime` | Допоміжні засоби outbound identity/send delegate |
    | `plugin-sdk/thread-bindings-runtime` | Допоміжні засоби життєвого циклу прив’язок потоків і адаптерів |
    | `plugin-sdk/agent-media-payload` | Застарілий побудовник payload media агента |
    ...
    | `plugin-sdk/channel-config-primitives` | Вузькі примітиви схеми config для channel |
    | `plugin-sdk/channel-config-writes` | Допоміжні засоби авторизації запису config для channel |
    ...
    | `plugin-sdk/interactive-runtime` | Допоміжні засоби нормалізації/зведення payload інтерактивних відповідей |
  ...
  <Accordion title="Підшляхи provider">
    ...
    | `plugin-sdk/self-hosted-provider-setup` | Спеціалізовані допоміжні засоби налаштування self-hosted provider, сумісних з OpenAI |
    | `plugin-sdk/provider-auth-runtime` | Допоміжні засоби визначення API-key під час виконання для plugins provider |
    | `plugin-sdk/provider-auth-api-key` | Допоміжні засоби онбордингу/запису профілю API-key |
    | `plugin-sdk/provider-auth-result` | Стандартний побудовник OAuth auth-result |
    | `plugin-sdk/provider-auth-login` | Спільні допоміжні засоби інтерактивного входу для plugins provider |
    | `plugin-sdk/provider-env-vars` | Допоміжні засоби пошуку env-var auth для provider |
    ...
    | `plugin-sdk/provider-web-search` | Допоміжні засоби реєстрації/кешування/config provider web-search |
    ...
    | `plugin-sdk/provider-stream` | `ProviderStreamFamily`, `buildProviderStreamFamilyHooks`, `composeProviderStreamWrappers`, типи обгорток stream і спільні допоміжні обгортки для Anthropic/Bedrock/Google/Kilocode/Moonshot/OpenAI/OpenRouter/Z.A.I/MiniMax/Copilot |
    | `plugin-sdk/provider-onboard` | Допоміжні засоби патчу config для онбордингу |
    | `plugin-sdk/global-singleton` | Допоміжні засоби singleton/map/cache локально для процесу |
  </Accordion>
  ...
  <Accordion title="Підшляхи auth і безпеки">
    ...
    | `plugin-sdk/approval-reply-runtime` | Допоміжні засоби payload reply для exec/plugin approval |
    | `plugin-sdk/command-auth-native` | Допоміжні засоби native auth команд + native session-target |
  ...
  <Accordion title="Підшляхи runtime і storage">
    ...
    | `plugin-sdk/telegram-command-config` | Допоміжні засоби нормалізації назви/опису команд Telegram і перевірки дублікатів/конфліктів, навіть коли surface контракту вбудованого Telegram недоступна |
    | `plugin-sdk/approval-runtime` | Допоміжні засоби exec/plugin approval, побудовники можливостей approval, допоміжні засоби auth/profile, native маршрутизації/runtime |
    | `plugin-sdk/reply-runtime` | Спільні допоміжні засоби runtime inbound/reply, chunking, dispatch, heartbeat, планувальник reply |
    | `plugin-sdk/reply-dispatch-runtime` | Вузькі допоміжні засоби dispatch/finalize для reply |
    | `plugin-sdk/reply-history` | Спільні допоміжні засоби коротковіконної history reply, такі як `buildHistoryContext`, `recordPendingHistoryEntry` і `clearHistoryEntriesIfEnabled` |
    ...
    | `plugin-sdk/status-helpers` | Спільні допоміжні засоби підсумку статусу channel/account, значення за замовчуванням для runtime-state і допоміжні засоби метаданих issue |
    | `plugin-sdk/target-resolver-runtime` | Спільні допоміжні засоби визначення цілей |
    | `plugin-sdk/string-normalization-runtime` | Допоміжні засоби нормалізації slug/рядків |
    | `plugin-sdk/request-url` | Витягує рядкові URL з вхідних даних, подібних до fetch/request |
    ...
    | `plugin-sdk/tool-send` | Витягує канонічні поля цілі send з аргументів tool |
    ...
    | `plugin-sdk/file-lock` | Допоміжні засоби повторновхідного file-lock |
    | `plugin-sdk/persistent-dedupe` | Допоміжні засоби dedupe cache із зберіганням на диску |
    | `plugin-sdk/acp-runtime` | Допоміжні засоби runtime/session ACP і reply-dispatch |
    | `plugin-sdk/agent-config-primitives` | Вузькі примітиви схеми config runtime агента |
    ...
    | `plugin-sdk/extension-shared` | Спільні примітиви допоміжних засобів passive-channel і статусу |
    ...
    | `plugin-sdk/retry-runtime` | Допоміжні засоби config retry і виконавця retry |
  ...
  <Accordion title="Підшляхи можливостей і тестування">
    | Subpath | Ключові експорти |
    | --- | --- |
    | `plugin-sdk/media-runtime` | Спільні допоміжні засоби fetch/transform/store для media плюс побудовники payload media |
    | `plugin-sdk/media-generation-runtime` | Спільні допоміжні засоби failover для media-generation, вибір кандидатів і повідомлення про відсутню модель |
    | `plugin-sdk/media-understanding` | Типи provider для media understanding плюс експорти допоміжних засобів зображень/аудіо для provider |
    | `plugin-sdk/text-runtime` | Спільні допоміжні засоби text/markdown/logging, такі як видалення видимого для assistant тексту, допоміжні засоби render/chunking/table для markdown, редагування чутливих даних, допоміжні засоби тегів директив і утиліти безпечного тексту |
    | `plugin-sdk/text-chunking` | Допоміжний засіб chunking для outbound text |
    | `plugin-sdk/speech` | Типи speech provider плюс експорти директив, реєстру й валідації для provider |
    | `plugin-sdk/speech-core` | Спільні типи speech provider, реєстр, директиви та допоміжні засоби нормалізації |
    ...
    | `plugin-sdk/image-generation` | Типи provider для image generation |
    ...
    | `plugin-sdk/music-generation-core` | Спільні типи music-generation, допоміжні засоби failover, пошуку provider і розбору model-ref |
    | `plugin-sdk/video-generation` | Типи provider/request/result для video generation |
    | `plugin-sdk/video-generation-core` | Спільні типи video-generation, допоміжні засоби failover, пошуку provider і розбору model-ref |
  ...
  <Accordion title="Підшляхи memory">
    ...
    | `plugin-sdk/memory-core-host-engine-foundation` | Експорти foundation engine для хоста memory |
    | `plugin-sdk/memory-core-host-engine-embeddings` | Експорти embedding engine для хоста memory |
    | `plugin-sdk/memory-core-host-engine-qmd` | Експорти QMD engine для хоста memory |
    | `plugin-sdk/memory-core-host-engine-storage` | Експорти storage engine для хоста memory |
    | `plugin-sdk/memory-core-host-multimodal` | Допоміжні засоби multimodal для хоста memory |
    | `plugin-sdk/memory-core-host-query` | Допоміжні засоби query для хоста memory |
    | `plugin-sdk/memory-core-host-secret` | Допоміжні засоби secret для хоста memory |
    | `plugin-sdk/memory-core-host-status` | Допоміжні засоби status для хоста memory |
    | `plugin-sdk/memory-core-host-runtime-cli` | Допоміжні засоби CLI runtime для хоста memory |
    | `plugin-sdk/memory-core-host-runtime-core` | Допоміжні засоби core runtime для хоста memory |
    | `plugin-sdk/memory-core-host-runtime-files` | Допоміжні засоби file/runtime для хоста memory |
    | `plugin-sdk/memory-lancedb` | Поверхня допоміжних засобів вбудованого memory-lancedb |
  </Accordion>

  <Accordion title="Зарезервовані допоміжні підшляхи для вбудованих компонентів">
    ...
    | Специфічні для channel helpers | `plugin-sdk/googlechat`, `plugin-sdk/zalouser`, `plugin-sdk/bluebubbles`, `plugin-sdk/bluebubbles-policy`, `plugin-sdk/mattermost`, `plugin-sdk/mattermost-policy`, `plugin-sdk/feishu-conversation`, `plugin-sdk/msteams`, `plugin-sdk/nextcloud-talk`, `plugin-sdk/nostr`, `plugin-sdk/tlon`, `plugin-sdk/twitch` | Шви сумісності/допоміжні шви для вбудованих channels |
    | Специфічні для auth/plugin helpers | `plugin-sdk/github-copilot-login`, `plugin-sdk/github-copilot-token`, `plugin-sdk/diagnostics-otel`, `plugin-sdk/diffs`, `plugin-sdk/llm-task`, `plugin-sdk/thread-ownership`, `plugin-sdk/voice-call` | Допоміжні шви для вбудованих features/plugins; `plugin-sdk/github-copilot-token` наразі експортує `DEFAULT_COPILOT_API_BASE_URL`, `deriveCopilotApiBaseUrlFromToken` і `resolveCopilotApiToken` |
  </Accordion>
</AccordionGroup>
...
| `api.registerRealtimeVoiceProvider(...)`         | Дуплексні сесії voice realtime |
...
методів, що належать plugin, віддавайте перевагу специфічним для plugin префіксам.
...
- `descriptors`: дескриптори команд на етапі розбору, що використовуються для довідки кореневого CLI,
  маршрутизації та лінивої реєстрації plugin CLI
...
Цей шлях сумісності з eager-завантаженням залишається підтримуваним, але він не встановлює
...
- `reply_dispatch`: повернення `{ handled: true, ... }` є термінальним. Щойно будь-який handler перехоплює dispatch, handlers із нижчим пріоритетом і типовий шлях dispatch моделі пропускаються.
...
| `api.config`             | `OpenClawConfig`          | Поточний знімок config (активний внутрішньопам’ятний runtime-знімок, коли доступний)        |
...
Поверхні публічних API вбудованих plugins, завантажені через facade (`api.ts`, `runtime-api.ts`,
`index.ts`, `setup-entry.ts` та подібні публічні entry-файли), тепер віддають перевагу
активному знімку runtime config, коли OpenClaw уже запущено. Якщо знімок runtime
ще не існує, вони повертаються до визначеного файлу config на диску.

Plugins provider також можуть експонувати вузький локальний barrel-файл контракту plugin, коли
допоміжний засіб навмисно є специфічним для provider і ще не належить до загального підшляху SDK.
Поточний вбудований приклад: provider Anthropic зберігає свої допоміжні засоби Claude
stream у власному публічному шві `api.ts` / `contract-api.ts`, замість того щоб
просувати логіку Anthropic beta-header і `service_tier` до загального
контракту `plugin-sdk/*`.

Інші поточні вбудовані приклади:

- `@openclaw/openai-provider`: `api.ts` експортує побудовники provider,
  допоміжні засоби моделей за замовчуванням і побудовники provider realtime
...
<Warning>
  ...
  поверхні, орієнтованої на можливості, замість того щоб жорстко зв’язувати два plugins між собою.
</Warning>
...
- [Налаштування і config](/uk/plugins/sdk-setup) — пакування, маніфести, схеми config
