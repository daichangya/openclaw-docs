...
summary: Огляд постачальника моделей з прикладами конфігурацій + потоками CLI
...
Щоб дізнатися про правила вибору моделей, дивіться [/concepts/models](/uk/concepts/models).
...
- Правила резервного перемикання, перевірки в період охолодження та збереження перевизначень сесії: [Перемикання моделей при збої](/uk/concepts/model-failover).
- Маршрути сімейства OpenAI залежать від префікса: `openai/<model>` використовує постачальника з прямим API-ключем OpenAI у PI, `openai-codex/<model>` використовує OAuth Codex у PI, а `openai/<model>` разом із `agents.defaults.embeddedHarness.runtime: "codex"` використовує нативний app-server harness Codex. Дивіться [OpenAI](/uk/providers/openai) і [Codex harness](/uk/plugins/codex-harness).
- Автоматичне ввімкнення Plugin дотримується цієї ж межі: `openai-codex/<model>` належить Plugin OpenAI, тоді як Plugin Codex вмикається через `embeddedHarness.runtime: "codex"` або застарілі посилання `codex/<model>`.
- Середовища виконання CLI використовують той самий поділ: вибирайте канонічні посилання на моделі, такі як `anthropic/claude-*`, `google/gemini-*` або `openai/gpt-*`, а потім задавайте `agents.defaults.embeddedHarness.runtime` як `claude-cli`, `google-gemini-cli` або `codex-cli`, якщо потрібен локальний бекенд CLI. Застарілі посилання `claude-cli/*`, `google-gemini-cli/*` і `codex-cli/*` мігрують назад до канонічних посилань постачальника, а середовище виконання записується окремо.
- GPT-5.5 наразі доступна через маршрути підписки/OAuth: `openai-codex/gpt-5.5` у PI або `openai/gpt-5.5` з harness app-server Codex. Прямий маршрут з API-ключем для `openai/gpt-5.5` підтримуватиметься, щойно OpenAI увімкне GPT-5.5 у публічному API; до того часу для налаштувань `OPENAI_API_KEY` використовуйте моделі, доступні через API, наприклад `openai/gpt-5.4`.

## Поведінка постачальника, якою володіє Plugin

Більшість логіки, специфічної для постачальника, живе в Plugin постачальників (`registerProvider(...)`), тоді як OpenClaw зберігає загальний цикл інференсу. Plugins відповідають за онбординг, каталоги моделей, зіставлення env vars для автентифікації, нормалізацію транспорту/конфігурації, очищення схем інструментів, класифікацію резервного перемикання при збої, оновлення OAuth, звітність про використання, профілі thinking/reasoning тощо.

Повний список хуків SDK постачальників і прикладів вбудованих Plugins наведено в [Provider plugins](/uk/plugins/sdk-provider-plugins). Постачальник, якому потрібен повністю кастомний виконавець запитів, — це окрема, глибша поверхня розширення.

<Note>
`capabilities` середовища виконання постачальника — це спільні метадані раннера (сімейство постачальника, особливості транскриптів/інструментів, підказки щодо транспорту/кешу). Це не те саме, що [публічна модель можливостей](/uk/plugins/architecture#public-capability-model), яка описує, що реєструє Plugin (текстовий інференс, мовлення тощо).
</Note>

## Ротація API-ключів

- Підтримується загальна ротація ключів постачальника для вибраних постачальників.
...
- Для постачальників Google як резервний варіант також використовується `GOOGLE_API_KEY`.
- Порядок вибору ключів зберігає пріоритет і прибирає дублікати значень.
- Запити повторюються з наступним ключем лише у відповідь на обмеження швидкості (наприклад, `429`, `rate_limit`, `quota`, `resource exhausted`, `Too many
...
- Якщо всі доступні ключі завершуються збоєм, повертається фінальна помилка з останньої спроби.

## Вбудовані постачальники (каталог pi-ai)

OpenClaw постачається з каталогом pi‑ai. Для цих постачальників **не потрібна**
конфігурація `models.providers`; просто задайте автентифікацію та виберіть модель.
...
- Пряма підтримка API для GPT-5.5 тут готова на майбутнє, щойно OpenAI відкриє GPT-5.5 у API
...
- Прогрівання WebSocket OpenAI Responses за замовчуванням увімкнене через `params.openaiWsWarmup` (`true`/`false`)
...
- Приховані заголовки атрибуції OpenClaw (`originator`, `version`,
  `User-Agent`) застосовуються лише до нативного трафіку OpenAI на `api.openai.com`, а не до загальних проксі, сумісних з OpenAI
- Нативні маршрути OpenAI також зберігають `store` Responses, підказки кешу prompt і формування payload для сумісності reasoning OpenAI; проксі-маршрути цього не роблять
...
- Прямі публічні запити Anthropic також підтримують спільний перемикач `/fast` і `params.fastMode`, включно з трафіком, автентифікованим через API-ключ і OAuth, що надсилається на `api.anthropic.com`; OpenClaw зіставляє це з Anthropic `service_tier` (`auto` або `standard_only`)
...
- Токен налаштування Anthropic як і раніше доступний як підтримуваний шлях токена OpenClaw, але тепер OpenClaw надає перевагу повторному використанню Claude CLI та `claude -p`, якщо це можливо.
...
- Межа Plugin: `openai-codex/*` завантажує Plugin OpenAI; нативний Plugin app-server Codex вибирається лише через середовище виконання harness Codex або застарілі посилання `codex/*`.
...
- `openai-codex/gpt-5.5` зберігає нативне `contextWindow = 1000000` і стандартне робоче `contextTokens = 272000`; перевизначте робоче обмеження через `models.providers.openai-codex.models[].contextTokens`
...
- Прямі запуски Gemini також приймають `agents.defaults.models["google/<model>"].params.cachedContent`
  (або застаріле `cached_content`) для передачі нативного дескриптора постачальника
  `cachedContents/...`; влучання в кеш Gemini відображаються в OpenClaw як `cacheRead`
...
- OAuth Gemini CLI постачається як частина вбудованого Plugin `google`.
  ...
  - Примітка: ви **не** вставляєте client id або secret в `openclaw.json`. Потік входу CLI зберігає
    токени в профілях автентифікації на хості Gateway.
  - Якщо після входу запити завершуються збоєм, задайте `GOOGLE_CLOUD_PROJECT` або `GOOGLE_CLOUD_PROJECT_ID` на хості Gateway.
  - JSON-відповіді Gemini CLI розбираються з `response`; використання резервно береться з
    `stats`, а `stats.cached` нормалізується в OpenClaw як `cacheRead`.
...
- Статичний резервний каталог постачається з `kilocode/kilo/auto`; live-виявлення через
  ...
  середовища виконання.
- Точна висхідна маршрутизація за `kilocode/kilo/auto` визначається Kilo Gateway,
  а не жорстко закодована в OpenClaw.

Дивіться [/providers/kilocode](/uk/providers/kilocode) для деталей налаштування.
...
Корисні особливості, які варто знати:

- **OpenRouter** застосовує свої заголовки атрибуції застосунку та маркери Anthropic `cache_control` лише на перевірених маршрутах `openrouter.ai`. Як проксі-шлях у стилі OpenAI, сумісний з OpenAI, він пропускає формування, притаманне лише нативному OpenAI (`serviceTier`, Responses `store`, підказки кешу prompt, сумісність reasoning OpenAI). Посилання, що працюють через Gemini, зберігають лише очищення thought-signature для проксі-Gemini.
- **Kilo Gateway** для посилань, що працюють через Gemini, дотримується того самого шляху очищення проксі-Gemini; `kilocode/kilo/auto` та інші посилання, де проксі-reasoning не підтримується, пропускають інʼєкцію proxy reasoning.
- **MiniMax** під час онбордингу через API-ключ записує явні визначення чат-моделей лише для тексту M2.7; розуміння зображень залишається на медіа-постачальнику `MiniMax-VL-01`, яким володіє Plugin.
...
Багато з наведених нижче вбудованих Plugins постачальників уже публікують каталог за замовчуванням.
...
У пікерах вибору моделі для онбордингу/налаштування варіант автентифікації Volcengine надає перевагу рядкам
...
У пікерах вибору моделі для онбордингу/налаштування варіант автентифікації BytePlus надає перевагу рядкам
...
Дивіться [/providers/minimax](/uk/providers/minimax) для деталей налаштування, варіантів моделей і фрагментів конфігурації.
...
Дивіться [/providers/lmstudio](/uk/providers/lmstudio) для налаштування та усунення проблем.
...
Ollama виявляється локально за адресою `http://127.0.0.1:11434`, якщо ви вмикаєте це через
`OLLAMA_API_KEY`, а вбудований Plugin постачальника додає Ollama безпосередньо в
`openclaw onboard` і пікер моделей. Дивіться [/providers/ollama](/uk/providers/ollama)
...
Дивіться [/providers/vllm](/uk/providers/vllm) для деталей.
...
Дивіться [/providers/sglang](/uk/providers/sglang) для деталей.
...
- Для `api: "openai-completions"` на ненативних кінцевих точках (будь-який непорожній `baseUrl`, де хост не є `api.openai.com`) OpenClaw примусово встановлює `compat.supportsDeveloperRole: false`, щоб уникнути помилок 400 від постачальника для непідтримуваних ролей `developer`.
- Маршрути в стилі проксі, сумісні з OpenAI, також пропускають формування запитів, притаманне лише нативному OpenAI: без `service_tier`, без Responses `store`, без підказок кешу prompt, без формування payload для сумісності reasoning OpenAI і без прихованих заголовків атрибуції OpenClaw.
- Якщо `baseUrl` порожній/не заданий, OpenClaw зберігає стандартну поведінку OpenAI (яка розв’язується в `api.openai.com`).
