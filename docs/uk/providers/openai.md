...
title: OpenAI
x-i18n:
    generated_at: "2026-04-21T01:38:05Z"
    model: gpt-5.4
    provider: openai
    source_hash: 4db510f037ec0dab5c2b419b242f182324543ad7feeaba515f71ed17f05a9486
...
<Tabs>
    ...
    **Найкраще підходить для:** прямого доступу до API та оплати за використання.
    ...
    <Steps>
      <Step title="Отримайте свій API-ключ">
        Створіть або скопіюйте API-ключ у [панелі OpenAI Platform](https://platform.openai.com/api-keys).
      </Step>
  ...
  <Tab title="Підписка Codex">
    **Найкраще підходить для:** використання вашої підписки ChatGPT/Codex замість окремого API-ключа. Codex cloud потребує входу через ChatGPT.
    ...
    Менше обмеження за замовчуванням на практиці забезпечує кращі характеристики затримки та якості. Замініть його через `contextTokens`:
...
<Note>
Дивіться [Генерація зображень](/uk/tools/image-generation), щоб дізнатися про спільні параметри інструмента, вибір провайдера та поведінку failover.
</Note>
...
| Вхідні посилання | 1 зображення або 1 відео                                                          |
| Перевизначення розміру | Підтримується                                                               |
...
<Note>
Дивіться [Генерація відео](/uk/tools/video-generation), щоб дізнатися про спільні параметри інструмента, вибір провайдера та поведінку failover.
</Note>

## Внесок у промпт GPT-5

OpenClaw додає специфічний для OpenAI внесок у промпт GPT-5 для запусків сімейства GPT-5 `openai/*` і `openai-codex/*`. Він міститься у вбудованому Plugin OpenAI, застосовується до ідентифікаторів моделей, таких як `gpt-5`, `gpt-5.2`, `gpt-5.4` і `gpt-5.4-mini`, і не застосовується до старіших моделей GPT-4.x.

Внесок GPT-5 за замовчуванням додає стислий формат виводу, інструкції для викликів інструментів і поведінки з ухилом на виконання. Ці інструкції завжди ввімкнені для відповідних моделей GPT-5. Рівень дружнього стилю взаємодії є окремим і налаштовуваним.
...
<AccordionGroup>
  <Accordion title="Транспорт (WebSocket чи SSE)">
    ...
    - Додає стабільні заголовки ідентифікації сеансу й ходу для повторних спроб і повторних з’єднань
  ...
  <Accordion title="Суворий агентний режим GPT">
    ...
    <Note>
    Застосовується лише до запусків сімейства OpenAI і Codex GPT-5. Інші провайдери та старіші сімейства моделей зберігають типову поведінку.
    </Note>
  ...
  <Accordion title="Нативні маршрути та маршрути, сумісні з OpenAI">
    ...
    **Нативні маршрути** (`openai/*`, `openai-codex/*`, Azure OpenAI):
    ...
    - Зберігають формування запитів, специфічне для OpenAI (`service_tier`, `store`, reasoning-compat, підказки кешу промптів)
