...
title: OpenAI
x-i18n:
    generated_at: "2026-04-23T16:52:29Z"
    model: gpt-5.4
    provider: openai
...
---
  ...
  - **Підписка Codex** — вхід через ChatGPT/Codex із доступом за підпискою (`openai-codex/*` models)
  ...
  | Вебпошук на стороні сервера | Вбудований інструмент OpenAI Responses  | Так, коли вебпошук увімкнено і провайдера не закріплено |
  ...
  | Векторні подання          | провайдер embedding для пам’яті           | Так                                                    |
  ...
  <Tab title="API-ключ (OpenAI Platform)">
    **Найкраще підходить для:** прямого доступу до API та тарифікації за використання.
    ...
    <Warning>
    OpenClaw **не** надає `openai/gpt-5.3-codex-spark` у прямому маршруті API. Живі запити до OpenAI API відхиляють цю модель. Spark доступний лише для Codex.
    </Warning>
    ...
    **Найкраще підходить для:** використання вашої підписки ChatGPT/Codex замість окремого API-ключа. Codex cloud потребує входу через ChatGPT.
    ...
    <Steps>
      <Step title="Запустіть Codex OAuth">
        ...
        Для headless-середовищ або конфігурацій, несумісних із callback, додайте `--device-code`, щоб увійти через потік device-code ChatGPT замість callback браузера на localhost:
    ...
    <Note>
    Онбординг більше не імпортує OAuth-матеріали з `~/.codex`. Увійдіть через OAuth у браузері (типово) або через наведений вище потік device-code — OpenClaw зберігає отримані облікові дані у власному сховищі автентифікації агентів.
    </Note>
...
`gpt-image-2` — типова модель як для генерації зображень з тексту в OpenAI, так і для редагування зображень. `gpt-image-1` усе ще можна використовувати як явне перевизначення моделі, але нові робочі процеси OpenAI для зображень мають використовувати `openai/gpt-image-2`.
...
OpenClaw додає спільний внесок у промпт GPT-5 для запусків сімейства GPT-5 у різних провайдерів. Він застосовується за id моделі, тому `openai/gpt-5.4`, `openai-codex/gpt-5.4`, `openrouter/openai/gpt-5.4`, `opencode/gpt-5.4` та інші сумісні посилання GPT-5 отримують однакове накладання. Старіші моделі GPT-4.x — ні.

Вбудований нативний провайдер Codex harness (`codex/*`) використовує ту саму поведінку GPT-5 і накладання Heartbeat через інструкції розробника Codex app-server, тому сеанси `codex/gpt-5.x` зберігають ту саму послідовність виконання та проактивні вказівки Heartbeat, хоча рештою промпту harness керує Codex.

Внесок GPT-5 додає тегований контракт поведінки для збереження персони, безпеки виконання, дисципліни інструментів, форми виводу, перевірок завершення та верифікації. Поведінка відповіді, специфічна для каналу, і поведінка тихих повідомлень залишаються у спільному системному промпті OpenClaw та політиці вихідної доставки. Вказівки GPT-5 завжди ввімкнені для відповідних моделей. Дружній стиль взаємодії є окремим і налаштовуваним.
...
<Note>
Застаріле `plugins.entries.openai.config.personality` усе ще зчитується як сумісний запасний варіант, якщо спільне налаштування `agents.defaults.promptOverlays.gpt5.personality` не встановлено.
</Note>
...
<AccordionGroup>
  <Accordion title="Синтез мовлення (TTS)">
    ...
    <Note>
    Установіть `OPENAI_TTS_BASE_URL`, щоб перевизначити базовий URL для TTS, не впливаючи на endpoint API чату.
    </Note>
  ...
  <Accordion title="Перетворення мовлення на текст">
    ...
    Підказки щодо мови та prompt передаються до OpenAI, коли їх надано
    через спільну конфігурацію аудіомедіа або запит на транскрибування для
    конкретного виклику.
  ...
  <Accordion title="Транскрибування в реальному часі">
    ...
    <Note>
    Використовує з’єднання WebSocket з `wss://api.openai.com/v1/realtime` з аудіо G.711 u-law (`g711_ulaw` / `audio/pcmu`). Цей потоковий провайдер призначено для шляху транскрибування в реальному часі у Voice Call; голосовий Discord зараз натомість записує короткі сегменти й використовує пакетний шлях транскрибування `tools.media.audio`.
    </Note>
...
зображень OpenClaw виявляє хости Azure у `models.providers.openai.baseUrl` і
...
реальному часі** в розділі [Голос і мовлення](#voice-and-speech) для його
параметрів Azure.
</Note>
...
Для генерації зображень Azure через вбудований провайдер `openai` укажіть у
`models.providers.openai.baseUrl` ваш ресурс Azure і встановіть у `apiKey`
...
потребує OpenClaw 2026.4.22 або новішої версії. У попередніх версіях будь-який
власний `openai.baseUrl` обробляється як публічний endpoint OpenAI і не
працюватиме з deployment-ами зображень Azure.
</Note>
...
deployment-у та підтвердьте, що конкретна модель доступна у вашому регіоні.
...
параметрів, які підтримуються вашим конкретним deployment-ом і версією API в
порталі Azure.

<Note>
Azure OpenAI використовує нативний transport і поведінку сумісності, але не отримує
приховані заголовки attribution OpenClaw — див. акордеон **Нативні маршрути та маршрути, сумісні з OpenAI**
в розділі [Розширена конфігурація](#advanced-configuration).

Для чату або трафіку Responses в Azure (окрім генерації зображень) використовуйте
...
<AccordionGroup>
  <Accordion title="Transport (WebSocket чи SSE)">
    OpenClaw використовує пріоритет WebSocket із резервним переходом на SSE (`"auto"`) як для `openai/*`, так і для `openai-codex/*`.
  ...
  <Accordion title="Пріоритетна обробка (service_tier)">
    ...
    <Warning>
    `serviceTier` передається лише до нативних endpoint-ів OpenAI (`api.openai.com`) і нативних endpoint-ів Codex (`chatgpt.com/backend-api`). Якщо ви маршрутизуєте будь-який із цих провайдерів через проксі, OpenClaw не змінює `service_tier`.
    </Warning>
  ...
  <Accordion title="Суворий агентний режим GPT">
    ...
    <Note>
    Застосовується лише до запусків сімейства GPT-5 OpenAI і Codex. Інші провайдери та старіші сімейства моделей зберігають типову поведінку.
    </Note>

  </Accordion>

  <Accordion title="Нативні маршрути та маршрути, сумісні з OpenAI">
    OpenClaw по-різному обробляє прямі endpoint-и OpenAI, Codex і Azure OpenAI порівняно з типовими проксі `/v1`, сумісними з OpenAI:
    ...
    **Проксі/сумісні маршрути:**
    ...
    Azure OpenAI використовує нативний transport і compat-поведінку, але не отримує приховані заголовки attribution.
