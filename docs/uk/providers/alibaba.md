...
read_when:
    ...
    - Вам потрібно налаштувати API-ключ Model Studio або DashScope для генерації відео
summary: Генерація відео Wan в Alibaba Model Studio в OpenClaw
title: Alibaba Model Studio
x-i18n:
    generated_at: "2026-04-05T22:22:55Z"
    model: gpt-5.4
    provider: openai
...
OpenClaw постачається з вбудованим провайдером генерації відео `alibaba` для моделей Wan в
...
- Режим з еталонним зображенням/відео наразі вимагає **віддалені URL-адреси http(s)**

## Зв’язок із Qwen

Вбудований провайдер `qwen` також використовує розміщені в Alibaba кінцеві точки DashScope для
генерації відео Wan. Використовуйте:

- `qwen/...`, якщо вам потрібна канонічна поверхня провайдера Qwen
- `alibaba/...`, якщо вам потрібна пряма поверхня відео Wan від постачальника

## Пов’язане

- [Генерація відео](/uk/tools/video-generation)
- [Qwen](/uk/providers/qwen)
- [Qwen / Model Studio](/uk/providers/qwen_modelstudio)
