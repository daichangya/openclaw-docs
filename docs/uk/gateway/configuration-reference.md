...
Базовий довідник із конфігурації для `~/.openclaw/openclaw.json`. Огляд, орієнтований на завдання, див. у [Configuration](/uk/gateway/configuration).

Охоплює основні поверхні конфігурації OpenClaw і дає посилання назовні, коли підсистема має власний докладніший довідник. Каталоги команд, що належать каналам і плагінам, а також глибокі параметри пам’яті/QMD винесені на окремі сторінки, а не наведені тут.
...
- сторінки каналів/плагінів-власників для поверхонь команд, специфічних для каналу

Формат конфігурації — **JSON5** (дозволені коментарі та коми в кінці). Усі поля необов’язкові — OpenClaw використовує безпечні значення за замовчуванням, якщо їх не вказано.
...
{
  mcp: {
    // Необов’язково. За замовчуванням: 600000 мс (10 хвилин). Встановіть 0, щоб вимкнути витіснення під час простою.
...
- `mcp.sessionIdleTtlMs`: TTL простою для bundled MCP-середовищ виконання з прив’язкою до сесії.
...
- Зміни в `mcp.*` застосовуються на льоту шляхом звільнення кешованих session MCP-середовищ виконання.
...
- `entries.<skillKey>.apiKey`: зручне поле для skills, що оголошують основну змінну середовища (plaintext рядок або об’єкт SecretRef).
...
- Виявлення підтримує нативні OpenClaw plugins, а також сумісні бандли Codex і Claude, включно з бандлами Claude стандартного макета без manifest.
- **Зміни конфігурації потребують перезапуску gateway.**
- `allow`: необов’язковий список дозволу (завантажуються лише перелічені plugins). `deny` має пріоритет.
- `plugins.entries.<id>.apiKey`: зручне поле API-ключа на рівні плагіна (коли це підтримує плагін).
- `plugins.entries.<id>.env`: карта змінних середовища в області плагіна.
- `plugins.entries.<id>.hooks.allowPromptInjection`: якщо `false`, ядро блокує `before_prompt_build` і ігнорує поля, що змінюють промпт, із застарілого `before_agent_start`, водночас зберігаючи застарілі `modelOverride` і `providerOverride`. Застосовується до нативних hooks плагіна та підтримуваних каталогів hooks, наданих бандлами.
- `plugins.entries.<id>.hooks.allowConversationAccess`: якщо `true`, довірені неbundled plugins можуть читати необроблений вміст розмови з типізованих hooks, таких як `llm_input`, `llm_output`, `before_agent_finalize` і `agent_end`.
...
- `plugins.entries.<id>.subagent.allowedModels`: необов’язковий список дозволу канонічних цілей `provider/model` для довірених перевизначень subagent. Використовуйте `"*"`, лише якщо ви навмисно хочете дозволити будь-яку модель.
- `plugins.entries.<id>.config`: визначений плагіном об’єкт конфігурації (валідується схемою нативного OpenClaw plugin, коли вона доступна).
- Налаштування облікових записів/середовища виконання для channel plugin розміщуються в `channels.<id>` і мають описуватися метаданими `channelConfigs` у manifest плагіна-власника, а не центральним реєстром параметрів OpenClaw.
- `plugins.entries.firecrawl.config.webFetch`: налаштування провайдера веб-отримання Firecrawl.
  - `apiKey`: API-ключ Firecrawl (приймає SecretRef). Використовує `plugins.entries.firecrawl.config.webSearch.apiKey`, застарілий `tools.web.fetch.firecrawl.apiKey` або змінну середовища `FIRECRAWL_API_KEY` як резервне значення.
...
- `plugins.entries.memory-core.config.dreaming`: налаштування Dreaming для пам’яті. Див. [Dreaming](/uk/concepts/dreaming) щодо фаз і порогів.
  ...
  - `frequency`: Cron-ритм для кожного повного проходу Dreaming (за замовчуванням `"0 3 * * *"`).
...
- Увімкнені plugins-бандли Claude також можуть додавати вбудовані значення Pi за замовчуванням із `settings.json`; OpenClaw застосовує їх як санітизовані налаштування агента, а не як сирі патчі конфігурації OpenClaw.
- `plugins.slots.memory`: вибрати ідентифікатор активного memory plugin або `"none"`, щоб вимкнути plugins пам’яті.
- `plugins.slots.contextEngine`: вибрати ідентифікатор активного плагіна рушія контексту; за замовчуванням `"legacy"`, якщо ви не встановите й не виберете інший рушій.
...
- `ssrfPolicy.dangerouslyAllowPrivateNetwork` вимкнено, якщо не задано, тож навігація браузера за замовчуванням залишається суворо обмеженою.
...
```json5
{
  gateway: {
    ...
    controlUi: {
      ...
      // allowExternalEmbedUrls: false, // небезпечно: дозволяє абсолютні зовнішні URL для вбудовування через http(s)
      // allowedOrigins: ["https://control.example.com"], // обов’язково для не-loopback Control UI
      // dangerouslyAllowHostHeaderOriginFallback: false, // небезпечний режим резервного визначення джерела через Host-заголовок
    ...
    nodes: {
      pairing: {
        // Необов’язково. За замовчуванням не задано/вимкнено.
...
- **Примітка для Docker**: bind `loopback` за замовчуванням слухає `127.0.0.1` усередині контейнера. Із bridge-мережею Docker (`-p 18789:18789`) трафік надходить через `eth0`, тому gateway недоступний. Використовуйте `--network host` або встановіть `bind: "lan"` (або `bind: "custom"` з `customBindHost: "0.0.0.0"`), щоб слухати на всіх інтерфейсах.
- **Auth**: за замовчуванням обов’язкова. Для bind не-loopback потрібна auth gateway. На практиці це означає спільний token/password або reverse proxy з урахуванням ідентичності з `gateway.auth.mode: "trusted-proxy"`. Майстер початкового налаштування за замовчуванням генерує token.
- Якщо налаштовано і `gateway.auth.token`, і `gateway.auth.password` (зокрема через SecretRef), явно задайте `gateway.auth.mode` як `token` або `password`. Під час запуску та в потоках встановлення/відновлення сервісу виникає помилка, якщо налаштовано обидва значення, а mode не задано.
- `gateway.auth.mode: "none"`: явний режим без auth. Використовуйте лише для довірених локальних налаштувань local loopback; цей варіант навмисно не пропонується в підказках початкового налаштування.
...
- `gateway.auth.allowTailscale`: якщо `true`, заголовки ідентичності Tailscale Serve можуть задовольняти auth для Control UI/WebSocket (перевіряється через `tailscale whois`). Кінцеві точки HTTP API **не** використовують цю auth через заголовки Tailscale; вони натомість дотримуються звичайного HTTP-режиму auth gateway. Цей потік без token припускає, що хост gateway є довіреним. За замовчуванням `true`, коли `tailscale.mode = "serve"`.
- `gateway.auth.rateLimit`: необов’язковий обмежувач невдалих спроб auth. Застосовується окремо для кожної IP-адреси клієнта та для кожної області auth (спільний секрет і token пристрою відстежуються незалежно). Заблоковані спроби повертають `429` + `Retry-After`.
  - На асинхронному шляху Tailscale Serve Control UI невдалі спроби для того самого `{scope, clientIp}` серіалізуються перед записом невдачі. Тому паралельні неправильні спроби від того самого клієнта можуть спрацювати на обмежувач уже на другому запиті, замість того щоб обидві пройшли як звичайні невідповідності.
  - `gateway.auth.rateLimit.exemptLoopback` за замовчуванням має значення `true`; установіть `false`, якщо ви навмисно хочете застосовувати rate limit і до localhost-трафіку (для тестових налаштувань або суворих розгортань через proxy).
- Спроби WS auth із браузерного походження завжди обмежуються без винятку для loopback (захист у глибину від brute force через браузер до localhost).
- На loopback ці блокування браузерного походження ізолюються за нормалізованим значенням `Origin`,
  тож повторні невдачі з одного localhost-походження не блокують автоматично
  інше походження.
...
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`: аварійне перевизначення
  середовища процесу на боці клієнта, яке дозволяє відкритий `ws://` до довірених IP-адрес
  приватної мережі; за замовчуванням відкритий трафік, як і раніше, дозволений лише для loopback. Еквівалента в
  `openclaw.json` немає, а конфігурація приватної мережі браузера, наприклад
...
- `gateway.push.apns.relay.baseUrl`: базовий HTTPS URL зовнішнього relay APNs, який використовується офіційними/TestFlight збірками iOS після того, як вони публікують до gateway реєстрації з підтримкою relay. Цей URL має збігатися з URL relay, скомпільованим у збірку iOS.
- `gateway.push.apns.relay.timeoutMs`: тайм-аут надсилання gateway-to-relay у мілісекундах. За замовчуванням `10000`.
- Реєстрації з підтримкою relay делегуються конкретній ідентичності gateway. Спарений застосунок iOS отримує `gateway.identity.get`, включає цю ідентичність у реєстрацію relay і пересилає до gateway дозвіл надсилання в межах цієї реєстрації. Інший gateway не може повторно використати цю збережену реєстрацію.
...
- `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true`: аварійний вихід лише для розробки для URL relay loopback HTTP. URL relay у production мають залишатися на HTTPS.
...
- Якщо `gateway.auth.token` / `gateway.auth.password` явно налаштовано через SecretRef і їх не вдається розв’язати, розв’язання завершується закритою відмовою (без маскування через резервний віддалений варіант).
...
- `allowRealIpFallback`: якщо `true`, gateway приймає `X-Real-IP`, якщо `X-Forwarded-For` відсутній. За замовчуванням `false` для поведінки fail-closed.
- `gateway.nodes.pairing.autoApproveCidrs`: необов’язковий список дозволу CIDR/IP для автоматичного схвалення першого pairing пристрою Node без запитаних scopes. Якщо не задано, вимкнено. Це не схвалює автоматично pairing operator/browser/Control UI/WebChat і не схвалює автоматично оновлення ролі, scope, metadata або публічного ключа.
...
- `gateway.tools.allow`: вилучити назви інструментів зі стандартного списку HTTP-заборон.
...
- `deferralTimeoutMs`: необов’язковий максимальний час у мс очікування завершення операцій у процесі перед примусовим перезапуском. Пропустіть або встановіть `0`, щоб чекати безстроково та журналювати періодичні попередження про ще незавершені операції.
...
- `POST /hooks/<name>` → розв’язується через `hooks.mappings`
  - Значення `sessionKey` у mapping, згенеровані шаблоном, розглядаються як зовнішньо задані й також вимагають `hooks.allowRequestSessionKey=true`.

<Accordion title="Деталі mapping">

- `match.path` збігається з підшляхом після `/hooks` (наприклад, `/hooks/gmail` → `gmail`).
- `match.source` збігається з полем payload для загальних шляхів.
...
- `allowRequestSessionKey`: дозволити викликачам `/hooks/agent` і ключам сесії mapping на основі шаблонів задавати `sessionKey` (за замовчуванням: `false`).
...
- Для bind не-loopback: маршрути canvas вимагають auth Gateway (token/password/trusted-proxy), як і інші HTTP-поверхні Gateway.
- Node WebViews зазвичай не надсилають заголовки auth; після pairing і підключення node Gateway рекламує URL можливостей з областю node для доступу до canvas/A2UI.
...
Записує unicast-зону DNS-SD до `~/.openclaw/dns/`. Для виявлення між мережами поєднуйте з DNS-сервером (рекомендується CoreDNS) + Tailscale split DNS.
...
Посилання на секрети є адитивними: plaintext-значення також продовжують працювати.
...
- `id` для `source: "exec"` не повинні містити сегменти шляху, розділені `/`, у вигляді `.` або `..` (наприклад, `a/../b` відхиляється)
...
- Шляхи провайдерів file і exec завершуються закритою відмовою, якщо перевірка Windows ACL недоступна. Установлюйте `allowInsecurePath: true` лише для довірених шляхів, які не можна перевірити.
- Провайдер `exec` вимагає абсолютний шлях `command` і використовує payload протоколу через stdin/stdout.
- За замовчуванням шляхи команд-символічних посилань відхиляються. Установіть `allowSymlinkCommand: true`, щоб дозволити шляхи-символічні посилання, водночас перевіряючи шлях розв’язаного цільового об’єкта.
...
- Посилання на секрети розв’язуються під час активації у snapshot у пам’яті, а шляхи запитів потім читають лише цей snapshot.
...
- Профілі для кожного агента зберігаються у `<agentDir>/auth-profiles.json`.
...
- Статичні облікові дані під час виконання надходять із розв’язаних snapshot у пам’яті; застарілі статичні записи `auth.json` очищаються при виявленні.
...
- `billingBackoffHours`: базовий backoff у годинах, коли профіль завершується помилкою через справжні
  ...
  усе одно може потрапити сюди навіть для відповідей `401`/`403`, але специфічні для провайдера
  зіставлювачі тексту залишаються в межах провайдера, якому вони належать (наприклад для OpenRouter
  `Key limit exceeded`). Повторювані помилки використання `402` usage-window або
  повідомлення про ліміт витрат організації/робочого простору натомість залишаються в гілці `rate_limit`.
...
- `authPermanentBackoffMinutes`: базовий backoff у хвилинах для високовпевнених помилок `auth_permanent` (за замовчуванням: `10`).
...
- `overloadedProfileRotations`: максимальна кількість ротацій auth-profile в межах одного провайдера для помилок перевантаження перед переходом на резервну модель (за замовчуванням: `1`). Сюди потрапляють форми на кшталт `ModelNotReadyException`, коли провайдер зайнятий.
...
- `redactSensitive` / `redactPatterns`: маскування за принципом best-effort для виводу в консолі, файлів журналів, записів журналів OTLP і збереженого тексту транскрипту сесії.
...
- `otel.captureContent`: явне ввімкнення захоплення сирого вмісту для атрибутів span OTEL. За замовчуванням вимкнено. Булеве значення `true` захоплює вміст не системних повідомлень/інструментів; форма об’єкта дає змогу явно вмикати `inputMessages`, `outputMessages`, `toolInputs`, `toolOutputs` і `systemPrompt`.
...
- `OPENCLAW_OTEL_PRELOADED=1`: перемикач середовища для хостів, які вже зареєстрували глобальний OpenTelemetry SDK. Тоді OpenClaw пропускає запуск/завершення SDK, що належить плагіну, зберігаючи активними слухачі діагностики.
- `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT`, `OTEL_EXPORTER_OTLP_METRICS_ENDPOINT` і `OTEL_EXPORTER_OTLP_LOGS_ENDPOINT`: змінні середовища для окремих кінцевих точок сигналів, які використовуються, коли відповідний ключ конфігурації не задано.
...
- `dispatch.enabled`: незалежний gate для dispatch turn сесій ACP (за замовчуванням: `true`). Установіть `false`, щоб залишити команди ACP доступними, але заблокувати виконання.
- `backend`: ідентифікатор стандартного runtime backend ACP (має збігатися із зареєстрованим runtime plugin ACP).
...
- `defaultAgent`: резервний ідентифікатор цільового агента ACP, коли spawn не вказує явну ціль.
...
- `stream.repeatSuppression`: придушувати повторювані рядки статусу/інструментів на turn (за замовчуванням: `true`).
- `stream.deliveryMode`: `"live"` передає потік поступово; `"final_only"` буферизує до terminal event turn.
...
- `stream.maxOutputChars`: максимальна кількість символів виводу помічника, що проєктується на turn ACP.
...
- `runtime.ttlMinutes`: idle TTL у хвилинах для працівників сесій ACP до моменту можливого очищення.
...
Поточні збірки більше не містять TCP bridge. Node підключаються через Gateway WebSocket. Ключі `bridge.*` більше не входять до схеми конфігурації (валідація завершується помилкою, доки їх не видалено; `openclaw doctor --fix` може прибрати невідомі ключі).
...
```json5
{
  cron: {
    ...
    runLog: {
      maxBytes: "2mb", // за замовчуванням 2_000_000 байтів
      keepLines: 2000, // за замовчуванням 2000
    },
  },
}
```

- `sessionRetention`: як довго зберігати завершені ізольовані сесії запуску cron перед очищенням із `sessions.json`. Також керує очищенням архівованих видалених транскриптів cron. За замовчуванням: `24h`; установіть `false`, щоб вимкнути.
- `runLog.maxBytes`: максимальний розмір одного файла журналу запуску (`cron/runs/<jobId>.jsonl`) перед очищенням. За замовчуванням: `2_000_000` байтів.
- `runLog.keepLines`: кількість найновіших рядків, що зберігаються під час очищення журналу запуску. За замовчуванням: `2000`.
- `webhookToken`: bearer token, який використовується для POST-доставки Webhook cron (`delivery.mode = "webhook"`); якщо пропущено, заголовок auth не надсилається.
...
- Стандартне місце призначення для сповіщень про збої cron у всіх завданнях.
...
- Коли не задано ні глобальне, ні на рівні завдання місце призначення для збоїв, завдання, які вже доставляють через `announce`, у разі збою використовують як резервний варіант цю основну announce-ціль.
...
| `{{MaxChars}}`     | Розв’язана максимальна кількість символів для записів CLI |
...
- Шляхи: розв’язуються відносно файла, що включає, але мають залишатися в межах каталогу конфігурації верхнього рівня (`dirname` для `openclaw.json`). Абсолютні форми/`../` дозволені лише тоді, коли після розв’язання вони все одно залишаються в межах цієї межі.
- Записи, що належать OpenClaw і змінюють лише один розділ верхнього рівня, підтриманий включенням одного файла, записуються назад у цей включений файл. Наприклад, `plugins install` оновлює `plugins: { $include: "./plugins.json5" }` у `plugins.json5` і залишає `openclaw.json` без змін.
- Кореневі включення, масиви включень і включення з перевизначеннями сусідніх ключів доступні лише для читання для записів, що належать OpenClaw; такі записи завершуються закритою відмовою замість сплощення конфігурації.
- Помилки: зрозумілі повідомлення для відсутніх файлів, помилок синтаксичного аналізу та циклічних включень.
