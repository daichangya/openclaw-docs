...
x-i18n:
    generated_at: "2026-04-24T03:16:17Z"
    model: gpt-5.4
    provider: openai
    source_hash: 18ec04c94d9702490b723769c67dc4744b0fad118e14fa9455904eeee1ee644b
...
Ця сторінка охоплює основні поверхні конфігурації OpenClaw і містить посилання на окремі довідники, якщо підсистема має власний глибший опис. Вона **не** намагається вбудувати на одній сторінці кожен каталог команд, що належить channel/Plugin, або кожен глибокий параметр memory/QMD.

Джерело істини в коді:

- `openclaw config schema` виводить живу JSON Schema, яка використовується для валідації та Control UI, з об’єднаними метаданими bundled/Plugin/channel, коли вони доступні
- `config.schema.lookup` повертає один вузол схеми в межах шляху для інструментів деталізації
- `pnpm config:docs:check` / `pnpm config:docs:gen` перевіряють хеш базового рівня config-doc щодо поточної поверхні схеми
...
Формат конфігурації — **JSON5** (дозволені коментарі + завершальні коми). Усі поля необов’язкові — OpenClaw використовує безпечні значення за замовчуванням, якщо їх пропущено.

---

## Channels

Кожен channel запускається автоматично, коли існує його секція конфігурації (якщо не встановлено `enabled: false`).
...
| `open`               | Обійти group allowlists (gating за згадкою все ще діє) |
...
Коди pairing спливають через 1 годину. Очікувальні запити на DM pairing обмежені до **3 на channel**.
Якщо блок провайдера відсутній повністю (`channels.<provider>` відсутній), runtime-політика груп повертається до `allowlist` (fail-closed) із попередженням під час запуску.
</Note>

### Перевизначення моделей для channel

Використовуйте `channels.modelByChannel`, щоб закріпити конкретні ID channel за моделлю. Значення приймають `provider/model` або налаштовані псевдоніми моделей. Відображення channel застосовується, коли сесія ще не має перевизначення моделі (наприклад, установленого через `/model`).
...
- `channels.defaults.contextVisibility`: типовий режим видимості додаткового контексту для всіх channels. Значення: `all` (типово, включати весь контекст цитат/гілок/історії), `allowlist` (включати лише контекст від відправників у allowlist), `allowlist_quote` (те саме, що allowlist, але зберігати явний контекст цитати/відповіді). Перевизначення для окремого channel: `channels.<channel>.contextVisibility`.
- `channels.defaults.heartbeat.showOk`: включати стани справних channel у вивід Heartbeat.
...
```json5
{
  channels: {
    telegram: {
      ...
      linkPreview: true,
      streaming: "partial", // off | partial | block | progress (типово: off; явно вмикайте, щоб уникнути обмежень частоти для попереднього перегляду редагувань)
...
- У багатооблікових конфігураціях (2+ ID облікових записів) установіть явний типовий обліковий запис (`channels.telegram.defaultAccount` або `channels.telegram.accounts.default`), щоб уникнути резервної маршрутизації; `openclaw doctor` попереджає, якщо цього бракує або значення невалідне.
- `configWrites: false` блокує запис конфігурації, ініційований через Telegram (міграції ID supergroup, `/config set|unset`).
- Записи верхнього рівня `bindings[]` з `type: "acp"` налаштовують постійні прив’язки ACP для тем форуму (використовуйте канонічне `chatId:topic:topicId` у `match.peer.id`). Семантика полів спільна з [ACP Agents](/uk/tools/acp-agents#channel-specific-settings).
...
- Прямі вихідні виклики, які явно передають Discord `token`, використовують цей токен для виклику; параметри повторних спроб/політики облікового запису все одно беруться з вибраного облікового запису в активному runtime snapshot.
...
- Slug-и guild мають нижній регістр, а пробіли замінюються на `-`; ключі channel використовують slug-ім’я (без `#`). Надавайте перевагу ID guild.
...
- `channels.discord.autoPresence` відображає runtime-стан доступності в присутність бота (healthy => online, degraded => idle, exhausted => dnd) і дозволяє необов’язкові перевизначення тексту статусу.
- `channels.discord.dangerouslyAllowNameMatching` знову вмикає зіставлення за змінними name/tag (аварійний режим сумісності).
- `channels.discord.execApprovals`: власна доставка схвалень exec у Discord і авторизація тих, хто схвалює.
  ...
  - `target`: куди надсилати запити на схвалення. `"dm"` (типово) надсилає в DM тим, хто схвалює, `"channel"` надсилає до вихідного каналу, `"both"` надсилає в обидва місця. Коли target включає `"channel"`, кнопками можуть користуватися лише визначені користувачі, які схвалюють.
  - `cleanupAfterResolve`: якщо `true`, видаляє DM зі схваленнями після схвалення, відхилення або тайм-ауту.
...
- **Режим Socket** вимагає і `botToken`, і `appToken` (`SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN` для резервного env-варіанта типового облікового запису).
...
- snapshots облікових записів Slack показують поля джерела/статусу для кожного облікового даного, як-от
...
- `channels.slack.streaming.mode` — канонічний ключ режиму потоку Slack. `channels.slack.streaming.nativeTransport` керує нативним потоковим транспортом Slack. Застарілі значення `streamMode`, булеві `streaming` і `nativeStreaming` мігруються автоматично.
...
**Ізоляція сесій потоків:** `thread.historyScope` — окремо для потоку (типово) або спільно для каналу. `thread.inheritParent` копіює транскрипт батьківського каналу в нові потоки.

- Нативний потоковий режим Slack разом зі статусом потоку Slack у стилі assistant "is typing..." вимагають ціль відповіді у потоці. Верхньорівневі DM типово залишаються поза потоками, тому вони використовують `typingReaction` або звичайну доставку замість попереднього перегляду у стилі потоку.
...
- `channels.slack.execApprovals`: власна доставка схвалень exec у Slack і авторизація тих, хто схвалює. Та сама схема, що й у Discord: `enabled` (`true`/`false`/`"auto"`), `approvers` (ID користувачів Slack), `agentFilter`, `sessionFilter` і `target` (`"dm"`, `"channel"` або `"both"`).
  ...
  channels: {
    mattermost: {
      ...
      commands: {
        ...
        // Необов’язковий явний URL для розгортань через reverse-proxy/public
...
Режими чату: `oncall` (відповідати на згадку через @, типово), `onmessage` (кожне повідомлення), `onchar` (повідомлення, що починаються з префікса-тригера).
...
- Нативні callback-и slash-команд автентифікуються за допомогою токенів окремих команд, які Mattermost повертає
...
- `channels.signal.account`: закріплює запуск channel за конкретною ідентичністю облікового запису Signal.
- `channels.signal.configWrites`: дозволяє або забороняє записи конфігурації, ініційовані із Signal.
...
- Повну конфігурацію channel BlueBubbles задокументовано в [BlueBubbles](/uk/channels/bluebubbles).
...
- `channels.imessage.configWrites`: дозволяє або забороняє записи конфігурації, ініційовані через iMessage.
...
- `channels.matrix.autoJoin` типово має значення `off`, тому кімнати за запрошенням і нові запрошення в стилі DM ігноруються, доки ви не встановите `autoJoin: "allowlist"` разом із `autoJoinAllowlist` або `autoJoin: "always"`.
- `channels.matrix.execApprovals`: власна доставка схвалень exec у Matrix і авторизація тих, хто схвалює.
  - `enabled`: `true`, `false` або `"auto"` (типово). У режимі auto схвалення exec активуються, коли тих, хто схвалює, можна визначити з `approvers` або `commands.ownerAllowFrom`.
...
- Перевірки стану Matrix і live-пошуки каталогу використовують ту саму політику proxy, що й runtime-трафік.
...
- Повну конфігурацію channel IRC (host/port/TLS/channels/allowlists/gating за згадкою) задокументовано в [IRC](/uk/channels/irc).

### Багатообліковість (усі channels)

Запускайте кілька облікових записів на channel (кожен зі своїм `accountId`):
...
- Якщо ви додаєте не-типовий обліковий запис через `openclaw channels add` (або onboarding channel), перебуваючи ще в конфігурації channel верхнього рівня для одного облікового запису, OpenClaw спочатку переносить значення верхнього рівня для одного облікового запису, що стосуються облікового запису, до map облікових записів channel, щоб вихідний обліковий запис продовжив працювати. Більшість channels переміщують їх до `channels.<channel>.accounts.default`; Matrix натомість може зберегти наявну відповідну іменовану/default-ціль.
- Наявні bindings лише для channel (без `accountId`) і далі збігатимуться з типовим обліковим записом; bindings на рівні облікового запису лишаються необов’язковими.
- `openclaw doctor --fix` також виправляє змішані форми, переміщуючи значення верхнього рівня для одного облікового запису, що стосуються облікового запису, до підвищеного облікового запису, вибраного для цього channel. Більшість channels використовують `accounts.default`; Matrix натомість може зберегти наявну відповідну іменовану/default-ціль.

### Інші Plugin channels

Багато Plugin channels налаштовуються як `channels.<id>` і задокументовані на своїх окремих сторінках channels (наприклад, Feishu, Matrix, LINE, Nostr, Zalo, Nextcloud Talk, Synology Chat і Twitch).
...
- **Текстові шаблони**: безпечні regex-шаблони в `agents.list[].groupChat.mentionPatterns`. Невалідні шаблони та небезпечні вкладені повтори ігноруються.
...
`messages.groupChat.historyLimit` задає глобальне типове значення. Channels можуть перевизначити його через `channels.<channel>.historyLimit` (або на рівні облікового запису). Установіть `0`, щоб вимкнути.
...
- `config: true` вмикає `/config` (читання/запис `openclaw.json`). Для клієнтів gateway `chat.send` постійний запис через `/config set|unset` також вимагає `operator.admin`; доступний лише для читання `/config show` залишається доступним для звичайних операторських клієнтів із дозволом на запис.
- `mcp: true` вмикає `/mcp` для конфігурації серверів MCP, якими керує OpenClaw, у `mcp.servers`.
- `plugins: true` вмикає `/plugins` для пошуку Plugin, встановлення та керування ввімкненням/вимкненням.
- `channels.<provider>.configWrites` керує змінами конфігурації для кожного channel (типово: true).
...
- `ownerAllowFrom` — явний allowlist власника для команд/інструментів лише для власника. Він відокремлений від `allowFrom`.
- `ownerDisplay: "hash"` хешує ID власника в system prompt. Установіть `ownerDisplaySecret`, щоб керувати хешуванням.
- `allowFrom` задається для кожного provider. Якщо його встановлено, це **єдине** джерело авторизації (channel allowlists/pairing і `useAccessGroups` ігноруються).
...
Необов’язковий корінь репозиторію, який показується в рядку Runtime у system prompt. Якщо не встановлено, OpenClaw автоматично визначає його, піднімаючись вгору від workspace.
...
Необов’язковий типовий allowlist Skills для агентів, які не встановлюють
`agents.list[].skills`.
...
- `"continuation-skip"`: безпечні ходи продовження (після завершеної відповіді помічника) пропускають повторне вбудовування bootstrap workspace, зменшуючи розмір prompt. Запуски Heartbeat і повторні спроби після Compaction все одно перебудовують контекст.
...
Максимальна кількість символів для кожного файла bootstrap workspace до обрізання. Типове значення: `12000`.
...
Максимальна загальна кількість символів, що вбудовуються в усі файли bootstrap workspace. Типове значення: `60000`.
...
навмисно розділені між підсистемами замість того, щоб усі проходили через один загальний
перемикач.
...
- `agents.defaults.startupContext.*`:
  одноразова стартова преамбула `/new` і `/reset`, включно з недавніми щоденними
  файлами `memory/*.md`.
- `skills.limits.*`:
...
- `memory.qmd.limits.*`:
  уривок пошуку в індексованій пам’яті та розміри вбудовування.
...
- `memoryGetMaxChars`: типове обмеження уривка `memory_get` до додавання метаданих
  обрізання і повідомлення про продовження.
...
- `imageGenerationModel`: приймає або рядок (`"provider/model"`), або об’єкт (`{ primary, fallbacks }`).
  ...
  - Якщо ви вибираєте provider/model напряму, також налаштуйте відповідну автентифікацію provider (наприклад `GEMINI_API_KEY` або `GOOGLE_API_KEY` для `google/*`, `OPENAI_API_KEY` або OpenAI Codex OAuth для `openai/gpt-image-2`, `FAL_KEY` для `fal/*`).
  - Якщо не вказано, `image_generate` усе одно може вивести типове значення provider на основі автентифікації. Спочатку він пробує поточний типовий provider, а потім решту зареєстрованих provider генерації зображень у порядку ID provider.
- `musicGenerationModel`: приймає або рядок (`"provider/model"`), або об’єкт (`{ primary, fallbacks }`).
  ...
  - Якщо не вказано, `music_generate` усе одно може вивести типове значення provider на основі автентифікації. Спочатку він пробує поточний типовий provider, а потім решту зареєстрованих provider генерації музики у порядку ID provider.
...
- `videoGenerationModel`: приймає або рядок (`"provider/model"`), або об’єкт (`{ primary, fallbacks }`).
  ...
  - Якщо не вказано, `video_generate` усе одно може вивести типове значення provider на основі автентифікації. Спочатку він пробує поточний типовий provider, а потім решту зареєстрованих provider генерації відео у порядку ID provider.
  - Якщо ви вибираєте provider/model напряму, також налаштуйте відповідну автентифікацію provider/API key.
  - Вбудований provider генерації відео Qwen підтримує до 1 вихідного відео, 1 вхідного зображення, 4 вхідних відео, тривалість до 10 секунд і параметри рівня provider `size`, `aspectRatio`, `resolution`, `audio` та `watermark`.
...
- `model.primary`: формат `provider/model` (наприклад `openai/gpt-5.4` для доступу через API key або `openai-codex/gpt-5.5` для Codex OAuth). Якщо ви пропускаєте provider, OpenClaw спочатку пробує псевдонім, потім унікальний збіг налаштованого provider для цього точного ID моделі, і лише після цього повертається до налаштованого типового provider (застаріла поведінка сумісності, тому надавайте перевагу явному `provider/model`). Якщо цей provider більше не надає налаштовану типову модель, OpenClaw повертається до першого налаштованого provider/model замість того, щоб показувати застаріле типове значення від видаленого provider.
- `models`: налаштований каталог моделей і allowlist для `/model`. Кожен запис може містити `alias` (скорочення) і `params` (специфічні для provider, наприклад `temperature`, `maxTokens`, `cacheRetention`, `context1m`, `responsesServerCompaction`, `responsesCompactThreshold`).
...
- `params`: глобальні типові параметри provider, які застосовуються до всіх моделей. Установлюються в `agents.defaults.params` (наприклад `{ cacheRetention: "long" }`).
- Пріоритет злиття `params` (конфігурація): `agents.defaults.params` (глобальна база) перевизначається `agents.defaults.models["provider/model"].params` (для окремої моделі), потім `agents.list[].params` (для відповідного ID агента) перевизначає за ключем. Докладніше див. [Prompt Caching](/uk/reference/prompt-caching).
- `embeddedHarness`: типова політика низькорівневого runtime вбудованого агента. Використовуйте `runtime: "auto"`, щоб зареєстровані harness-и Plugin могли брати підтримувані моделі, `runtime: "pi"`, щоб примусово використовувати вбудований harness PI, або зареєстрований ID harness, як-от `runtime: "codex"`. Установіть `fallback: "none"`, щоб вимкнути автоматичний резервний перехід до PI.
...
- `fallback`: `"pi"` або `"none"`. `"pi"` зберігає вбудований harness PI як резервний варіант сумісності, коли жоден harness Plugin не вибрано. `"none"` призводить до помилки при відсутності або непідтримуваному виборі harness Plugin замість тихого використання PI. Збої вибраного harness Plugin завжди показуються напряму.
...
- Вибір harness закріплюється за ID сесії після першого вбудованого запуску. Зміни конфігурації/env впливають на нові або скинуті сесії, але не на наявний transcript. Застарілі сесії з історією transcript, але без записаного закріплення, вважаються закріпленими за PI. `/status` показує не-PI ID harness, як-от `codex`, поруч із `Fast`.
- Це керує лише harness вбудованого чату. Генерація медіа, vision, PDF, музика, відео й TTS і далі використовують свої налаштування provider/model.
...
Необов’язкові CLI-бекенди для резервних запусків лише з текстом (без викликів інструментів). Корисно як резервний варіант, коли API provider-и недоступні.
...
Замінює весь system prompt, зібраний OpenClaw, фіксованим рядком. Установлюється на типовому рівні (`agents.defaults.systemPromptOverride`) або для окремого агента (`agents.list[].systemPromptOverride`). Значення для окремого агента мають пріоритет; порожнє або таке, що складається лише з пробілів, значення ігнорується. Корисно для контрольованих експериментів із prompt.
...
Незалежні від provider накладки prompt, які застосовуються за сімейством моделей. ID моделей сімейства GPT-5 отримують спільний поведінковий контракт між provider-ами; `personality` керує лише шаром дружнього стилю взаємодії.
...
- `"off"` вимикає лише дружній шар; тегований поведінковий контракт GPT-5 залишається ввімкненим.
- Застаріле `plugins.entries.openai.config.personality` і далі читається, якщо це спільне налаштування не встановлено.
...
- `includeSystemPromptSection`: коли `false`, прибирає секцію Heartbeat із system prompt і пропускає вбудовування `HEARTBEAT.md` у bootstrap-контекст. Типове значення: `true`.
- `suppressToolErrorWarnings`: коли `true`, пригнічує payload-и попереджень про помилки інструментів під час запусків heartbeat.
- `timeoutSeconds`: максимальний час у секундах, дозволений для одного ходу агента heartbeat, після чого його буде перервано. Якщо не встановлено, використовується `agents.defaults.timeoutSeconds`.
...
- `isolatedSession`: коли `true`, кожен heartbeat запускається в новій сесії без попередньої історії розмови. Та сама схема ізоляції, що й у Cron `sessionTarget: "isolated"`. Зменшує витрати токенів на один heartbeat приблизно зі ~100K до ~2-5K токенів.
- Для окремого агента: встановіть `agents.list[].heartbeat`. Коли будь-який агент визначає `heartbeat`, запускаються heartbeat **лише для цих агентів**.
  ...
  agents: {
    defaults: {
      compaction: {
        ...
        provider: "my-provider", // id зареєстрованого Plugin provider-а Compaction (необов’язково)
        timeoutSeconds: 900,
        ...
        notifyUser: true, // надсилати короткі сповіщення користувачу на початку та після завершення Compaction (типово: false)
        memoryFlush: {
          enabled: true,
...
- `provider`: id зареєстрованого Plugin provider-а Compaction. Якщо встановлено, замість вбудованого підсумовування LLM викликається `summarize()` цього provider-а. У разі помилки відбувається повернення до вбудованого варіанта. Встановлення provider-а примусово задає `mode: "safeguard"`. Див. [Compaction](/uk/concepts/compaction).
...
- `postCompactionSections`: необов’язкові назви секцій H2/H3 з AGENTS.md, які треба повторно вбудувати після Compaction. Типово `["Session Startup", "Red Lines"]`; установіть `[]`, щоб вимкнути повторне вбудовування. Якщо значення не встановлено або явно встановлено в цю типову пару, старі заголовки `Every Session`/`Safety` також приймаються як резервний застарілий варіант.
- `model`: необов’язкове перевизначення `provider/model-id` лише для підсумовування Compaction. Використовуйте це, коли основна сесія має залишатися на одній моделі, а підсумки Compaction повинні запускатися на іншій; якщо не встановлено, Compaction використовує основну модель сесії.
- `notifyUser`: коли `true`, надсилає користувачу короткі повідомлення, коли Compaction починається і коли завершується (наприклад, "Compacting context..." і "Compaction complete"). Типово вимкнено, щоб Compaction відбувався безшумно.
- `memoryFlush`: безшумний агентний хід перед auto-compaction для збереження тривалих спогадів. Пропускається, коли workspace лише для читання.
...
- Для каналів, що не є Telegram, потрібне явне `*.blockStreaming: true`, щоб увімкнути блочні відповіді.
    ...
    defaults: {
      sandbox: {
        ...
        ssh: {
          ...
          // Також підтримуються SecretRef-и / вбудований вміст:
...
- `workspaceRoot`: абсолютний віддалений корінь, який використовується для workspace у межах області
...
- Значення `*Data` на основі SecretRef розв’язуються з активного runtime snapshot секретів перед початком сесії sandbox
...
- `none`: workspace sandbox у межах області під `~/.openclaw/sandboxes`
...
- `remote`: один раз ініціалізує віддалений простір під час створення sandbox, далі канонічним залишається віддалений workspace

У режимі `remote` локальні зміни на хості, зроблені поза OpenClaw, після етапу ініціалізації автоматично не синхронізуються до sandbox.
...
Доступ спостерігача через noVNC типово використовує автентифікацію VNC, а OpenClaw видає короткоживучий URL із токеном (замість того, щоб відкривати пароль у спільному URL).
...
- `cdpSourceRange` за бажанням обмежує вхідний CDP-трафік на межі контейнера до діапазону CIDR (наприклад `172.21.0.1/32`).
- `sandbox.browser.binds` монтує додаткові каталоги хоста лише в контейнер браузера sandbox. Якщо встановлено (включно з `[]`), це замінює `docker.binds` для контейнера браузера.
    ...
    list: [
      {
        ...
        params: { cacheRetention: "none" }, // перевизначає за ключами відповідні defaults.models params
...
- `default`: якщо встановлено кілька, перший має пріоритет (записується попередження). Якщо не встановлено жодного, типовим буде перший запис у списку.
- `model`: рядкова форма перевизначає лише `primary`; форма об’єкта `{ primary, fallbacks }` перевизначає обидва (`[]` вимикає глобальні fallback). Cron-завдання, які перевизначають лише `primary`, усе одно успадковують типові fallback, якщо ви не встановите `fallbacks: []`.
...
- `skills`: необов’язковий allowlist Skills для окремого агента. Якщо не вказано, агент успадковує `agents.defaults.skills`, коли його встановлено; явний список замінює типові значення замість злиття, а `[]` означає відсутність Skills.
- `thinkingDefault`: необов’язковий типовий рівень thinking для окремого агента (`off | minimal | low | medium | high | xhigh | adaptive | max`). Перевизначає `agents.defaults.thinkingDefault` для цього агента, якщо не встановлено перевизначення для окремого повідомлення або сесії.
- `reasoningDefault`: необов’язкова типова видимість reasoning для окремого агента (`on | off | stream`). Застосовується, коли не встановлено перевизначення reasoning для окремого повідомлення або сесії.
...
- Захист успадкування sandbox: якщо сесія, що запитує, працює в sandbox, `sessions_spawn` відхиляє цілі, які запускалися б поза sandbox.
...
5. `match.accountId: "*"` (для всього channel)
6. Типовий агент
...
- **`dmScope`**: як групуються DM.
  ...
  - `per-channel-peer`: ізоляція для кожної пари channel + відправник (рекомендовано для вхідних скриньок із багатьма користувачами).
  - `per-account-channel-peer`: ізоляція для кожної пари обліковий запис + channel + відправник (рекомендовано для багатообліковості).
- **`identityLinks`**: мапа канонічних id до peer-ів із префіксом provider для спільного використання сесій між channels.
- **`reset`**: основна політика скидання. `daily` скидає о `atHour` за місцевим часом; `idle` скидає після `idleMinutes`. Якщо налаштовано обидва, спрацьовує той варіант, що спливає раніше.
- **`resetByType`**: перевизначення за типом (`direct`, `group`, `thread`). Застарілий `dm` приймається як псевдонім для `direct`.
- **`parentForkMaxTokens`**: максимальний `totalTokens` батьківської сесії, дозволений під час створення форкнутої сесії потоку (типово `100000`).
...
- **`maintenance`**: засоби очищення сховища session + контролі зберігання.
  ...
  - `maxDiskBytes`: необов’язковий бюджет дискового простору для каталогу sessions. У режимі `warn` записуються попередження; у режимі `enforce` спочатку видаляються найстаріші артефакти/sessions.
...
Порядок визначення (найспецифічніше має пріоритет): обліковий запис → channel → глобальне значення. `""` вимикає й зупиняє каскад. `"auto"` виводить `[{identity.name}]`.
...
Об’єднує швидкі текстові повідомлення від того самого відправника в один хід агента. Media/attachments скидаються негайно. Команди керування обходять debounce.
...
- `modelOverrides` типово ввімкнено; `modelOverrides.allowProvider` типово має значення `false` (потрібен явний opt-in).
...
- Коли `openai.baseUrl` вказує на не-OpenAI endpoint, OpenClaw трактує його як OpenAI-сумісний TTS-сервер і послаблює валідацію моделі/голосу.
...
- `talk.provider` має збігатися з ключем у `talk.providers`, коли налаштовано кілька provider-ів Talk.
- Застарілі пласкі ключі Talk (`talk.voiceId`, `talk.voiceAliases`, `talk.modelId`, `talk.outputFormat`, `talk.apiKey`) призначені лише для сумісності та автоматично мігруються до `talk.providers.<provider>`.
...
- Резервне значення `ELEVENLABS_API_KEY` застосовується лише тоді, коли жоден API key Talk не налаштовано.
- `providers.*.voiceAliases` дозволяє директивам Talk використовувати дружні назви.
- `silenceTimeoutMs` визначає, як довго режим Talk чекає після мовчання користувача перед надсиланням transcript. Якщо не встановлено, зберігається типове для платформи вікно паузи (`700 ms на macOS і Android, 900 ms на iOS`).
...
| `full`     | Без обмежень (те саме, що й не встановлено)                                                                                     |
...
Додатково обмежує інструменти для конкретних provider-ів або моделей. Порядок: базовий профіль → профіль provider-а → allow/deny.
...
- Підвищений `exec` обходить sandbox і використовує налаштований шлях виходу (`gateway` типово, або `node`, коли ціль `exec` — це `node`).
...
```json5
{
  tools: {
    media: {
      concurrency: 2,
      asyncCompletion: {
        directSend: false, // opt-in: надсилати готову асинхронну музику/відео напряму в channel
      },
      audio: {
        enabled: true,
        maxBytes: 20971520,
...
Автентифікація provider-а використовує стандартний порядок: `auth-profiles.json` → env vars → `models.providers.*.apiKey`.

**Поля асинхронного завершення:**

- `asyncCompletion.directSend`: коли `true`, завершені асинхронні завдання `music_generate`
  і `video_generate` спочатку намагаються доставити результат напряму в channel. Типове значення: `false`
...
Керує тим, на які сесії можна націлюватися через інструменти сесій (`sessions_list`, `sessions_history`, `sessions_send`).
...
Прапори експериментальних вбудованих інструментів. Типово вимкнено, якщо не застосовується правило авто-вмикання strict-agentic GPT-5.
...
- Типове значення: `false`, якщо лише `agents.defaults.embeddedPi.executionContract` (або перевизначення для окремого агента) не встановлено в `"strict-agentic"` для запуску OpenAI або OpenAI Codex сімейства GPT-5. Установіть `true`, щоб примусово ввімкнути інструмент поза цією сферою, або `false`, щоб залишити його вимкненим навіть для запусків strict-agentic GPT-5.
- Коли інструмент увімкнено, system prompt також додає інструкції з використання, щоб модель застосовувала його лише для суттєвої роботи й утримувала не більше одного кроку у стані `in_progress`.
...
- `model`: типова модель для породжених sub-agents. Якщо не вказано, sub-agents успадковують модель викликувача.
...
OpenClaw використовує вбудований каталог моделей. Додавайте власні provider-и через `models.providers` у конфігурації або `~/.openclaw/agents/<agentId>/agent/models.json`.
...
- Пріоритет злиття для відповідних ID provider-ів:
  - Непорожні значення `baseUrl` в агентському `models.json` мають пріоритет.
  - Непорожні значення `apiKey` в агенті мають пріоритет лише тоді, коли цей provider не керується через SecretRef у поточному контексті config/auth-profile.
  - Значення `apiKey` для provider-ів, керованих через SecretRef, оновлюються з маркерів джерела (`ENV_VAR_NAME` для env refs, `secretref-managed` для file/exec refs) замість збереження розв’язаних секретів.
  - Значення заголовків provider-а, керованих через SecretRef, оновлюються з маркерів джерела (`secretref-env:ENV_VAR_NAME` для env refs, `secretref-managed` для file/exec refs).
  - Порожні або відсутні `apiKey`/`baseUrl` в агента повертаються до `models.providers` у конфігурації.
  ...
  - Для відповідних `contextTokens` моделі зберігається явне runtime-обмеження, якщо воно є; використовуйте це, щоб обмежити ефективний контекст, не змінюючи нативні метадані моделі.
  - Використовуйте `models.mode: "replace"`, коли хочете, щоб конфігурація повністю переписала `models.json`.
  - Збереження маркерів є авторитетним щодо джерела: маркери записуються з активного snapshot вихідної конфігурації (до розв’язання), а не з розв’язаних runtime-значень секретів.

### Деталі полів provider-а

- `models.mode`: поведінка каталогу provider-ів (`merge` або `replace`).
- `models.providers`: власна map provider-ів із ключем за id provider-а.
...
- `models.providers.*.request`: перевизначення транспорту для HTTP-запитів model-provider.
  - `request.headers`: додаткові заголовки (зливаються з типовими значеннями provider-а). Значення приймають SecretRef.
  - `request.auth`: перевизначення стратегії автентифікації. Режими: `"provider-default"` (використовувати вбудовану автентифікацію provider-а), `"authorization-bearer"` (з `token`), `"header"` (з `headerName`, `value`, необов’язковим `prefix`).
  ...
  - `request.allowPrivateNetwork`: коли `true`, дозволяє HTTPS до `baseUrl`, якщо DNS розв’язується в приватні, CGNAT або подібні діапазони, через захист HTTP fetch provider-а (операторський opt-in для довірених self-hosted OpenAI-сумісних endpoint-ів). WebSocket використовує той самий `request` для заголовків/TLS, але не цей SSRF-захист fetch. Типове значення `false`.
...
- `models.providers.*.models.*.compat.requiresStringContent`: необов’язкова підказка сумісності для OpenAI-сумісних chat-endpoint-ів, що приймають лише рядки. Коли `true`, OpenClaw сплощує масиви `messages[].content`, які містять лише текст, у звичайні рядки перед надсиланням запиту.
...
- `plugins.entries.amazon-bedrock.config.discovery.refreshInterval`: інтервал polling для оновлення виявлення.
...
транспорті `openai-completions`, і OpenClaw орієнтується на можливості endpoint-а,
а не лише на вбудований id provider-а.
...
Див. [Локальні моделі](/uk/gateway/local-models). Коротко: запускайте велику локальну модель через LM Studio Responses API на серйозному обладнанні; зберігайте hosted-моделі об’єднаними для резервного використання.
...
- `install.nodeManager`: пріоритетний node-інсталятор для специфікацій `metadata.openclaw.install`
  (`npm` | `pnpm` | `yarn` | `bun`).
- `entries.<skillKey>.enabled: false` вимикає Skill, навіть якщо він bundled/встановлений.
- `entries.<skillKey>.apiKey`: зручне поле для Skills, що оголошують основну env var (plaintext-рядок або об’єкт SecretRef).
...
- Виявлення приймає нативні Plugins OpenClaw, а також сумісні bundl-и Codex і Claude, включно з bundl-ами Claude зі стандартним макетом без маніфеста.
...
- `plugins.entries.<id>.env`: map env vars у межах Plugin.
- `plugins.entries.<id>.hooks.allowPromptInjection`: коли `false`, ядро блокує `before_prompt_build` і ігнорує поля legacy `before_agent_start`, що змінюють prompt, зберігаючи при цьому legacy `modelOverride` і `providerOverride`. Застосовується до нативних hook-ів Plugin і підтримуваних каталогів hook-ів, наданих bundle-ами.
...
- `plugins.entries.<id>.config`: об’єкт конфігурації, визначений Plugin (перевіряється нативною схемою Plugin OpenClaw, коли вона доступна).
- `plugins.entries.firecrawl.config.webFetch`: налаштування provider-а веб-отримання Firecrawl.
  - `apiKey`: API key Firecrawl (приймає SecretRef). За відсутності використовується `plugins.entries.firecrawl.config.webSearch.apiKey`, застарілий `tools.web.fetch.firecrawl.apiKey` або env var `FIRECRAWL_API_KEY`.
...
- `plugins.entries.memory-core.config.dreaming`: налаштування dreaming пам’яті. Докладніше про фази та пороги див. у [Dreaming](/uk/concepts/dreaming).
  - `enabled`: головний перемикач dreaming (типово `false`).
  - `frequency`: розклад Cron для кожного повного проходу dreaming (типово `"0 3 * * *"`).
...
- Увімкнені Plugins Claude bundle також можуть додавати вбудовані типові значення Pi із `settings.json`; OpenClaw застосовує їх як очищені налаштування агента, а не як сирі патчі конфігурації OpenClaw.
- `plugins.slots.memory`: вибір id активного Plugin пам’яті або `"none"` для вимкнення Plugin-ів пам’яті.
...
- `plugins.installs`: метадані встановлення, якими керує CLI і які використовує `openclaw plugins update`.
  ...
  - Сприймайте `plugins.installs.*` як керований стан; віддавайте перевагу командам CLI замість ручних змін.
...
```json5
{
  browser: {
    ...
    defaultProfile: "user",
    ssrfPolicy: {
      // dangerouslyAllowPrivateNetwork: true, // вмикайте лише свідомо для довіреного доступу до приватної мережі
...
- `ssrfPolicy.dangerouslyAllowPrivateNetwork` вимкнено, якщо не встановлено, тому навігація browser лишається суворою за замовчуванням.
...
- У суворому режимі віддалені endpoint-и профілів CDP (`profiles.*.cdpUrl`) підлягають тому самому блокуванню приватної мережі під час перевірок доступності/виявлення.
...
- `profiles.*.cdpUrl` приймає `http://`, `https://`, `ws://` і `wss://`.
  ...
  коли ваш provider надає прямий URL DevTools WebSocket.
...
- Профілі `existing-session` можуть встановлювати `userDataDir`, щоб націлюватися на конкретний
  профіль браузера на основі Chromium, наприклад Brave або Edge.
- Профілі `existing-session` зберігають поточні обмеження маршруту Chrome MCP:
  дії на основі snapshot/ref замість націлювання через CSS-селектори, hook-и
  завантаження одного файла, відсутність перевизначення тайм-аутів діалогів, відсутність `wait --load networkidle`,
  а також відсутність `responsebody`, експорту PDF, перехоплення завантажень чи пакетних дій.
...
- Сервіс керування: лише local loopback (порт виводиться з `gateway.port`, типово `18791`).
...
- `seamColor`: акцентний колір для chrome нативного UI застосунку (тонування бульбашки Talk Mode тощо).
- `assistant`: перевизначення identity для Control UI. Якщо не встановлено, використовується identity активного агента.
...
```json5
{
  gateway: {
    ...
    controlUi: {
      ...
      // allowExternalEmbedUrls: false, // небезпечно: дозволити абсолютні зовнішні http(s) URL для вбудовування
    ...
    tools: {
      ...
      deny: ["browser"],
      // Прибрати інструменти з типового HTTP deny list
      allow: ["gateway"],
    },
    push: {
      apns: {
...
- **Примітка для Docker**: типовий bind `loopback` слухає `127.0.0.1` усередині контейнера. За мережі Docker bridge (`-p 18789:18789`) трафік надходить через `eth0`, тому gateway недоступний. Використовуйте `--network host` або встановіть `bind: "lan"` (або `bind: "custom"` разом із `customBindHost: "0.0.0.0"`), щоб слухати на всіх інтерфейсах.
- **Auth**: типово обов’язкова. Bind-и не через loopback вимагають автентифікації gateway. На практиці це означає спільний token/password або reverse proxy з підтримкою identity із `gateway.auth.mode: "trusted-proxy"`. Майстер onboarding типово генерує token.
- Якщо налаштовано і `gateway.auth.token`, і `gateway.auth.password` (включно з SecretRef), явно встановіть `gateway.auth.mode` у `token` або `password`. Потоки запуску та встановлення/відновлення сервісу завершуються помилкою, коли налаштовано обидва параметри, а mode не встановлено.
...
- `gateway.auth.allowTailscale`: коли `true`, заголовки identity з Tailscale Serve можуть задовольнити auth для Control UI/WebSocket (перевіряється через `tailscale whois`). HTTP API endpoint-и **не** використовують цю автентифікацію за заголовками Tailscale; вони дотримуються звичайного HTTP auth-режиму gateway. Цей безтокеновий потік передбачає, що хост gateway є довіреним. Типово `true`, коли `tailscale.mode = "serve"`.
- `gateway.auth.rateLimit`: необов’язковий лімітатор невдалих спроб auth. Застосовується для кожної IP-адреси клієнта й для кожної області auth (shared-secret і device-token відстежуються окремо). Заблоковані спроби повертають `429` + `Retry-After`.
  - На асинхронному шляху Tailscale Serve Control UI невдалі спроби для тієї самої пари `{scope, clientIp}` серіалізуються перед записом помилки. Тому одночасні помилкові спроби від того самого клієнта можуть спровокувати спрацювання лімітатора на другому запиті замість того, щоб обидва просто пройшли як звичайні невідповідності.
  - `gateway.auth.rateLimit.exemptLoopback` типово дорівнює `true`; установіть `false`, коли свідомо хочете, щоб локальний трафік localhost також підпадав під rate limiting (для тестових конфігурацій або суворих розгортань proxy).
- Спроби автентифікації WS з browser-origin завжди обмежуються без винятку для loopback (додатковий захист від brute force localhost із браузера).
- На loopback такі блокування за browser-origin ізолюються для кожного нормалізованого значення `Origin`,
...
- `controlUi.allowedOrigins`: явний allowlist browser-origin для WebSocket-підключень до Gateway. Обов’язковий, коли очікуються browser-клієнти з origin, відмінних від loopback.
- `controlUi.dangerouslyAllowHostHeaderOriginFallback`: небезпечний режим, який вмикає резервне визначення origin через Host-header для розгортань, що свідомо покладаються на політику origin через Host-header.
- `remote.transport`: `ssh` (типово) або `direct` (ws/wss). Для `direct` значення `remote.url` має бути `ws://` або `wss://`.
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`: аварійне перевизначення на боці клієнта, яке дозволяє plaintext `ws://` до довірених IP приватної мережі; типове обмеження для plaintext і далі лишається лише для loopback.
- `gateway.remote.token` / `.password` — це поля облікових даних віддаленого клієнта. Вони самі по собі не налаштовують auth gateway.
- `gateway.push.apns.relay.baseUrl`: базовий HTTPS URL зовнішнього APNs relay, який офіційні/TestFlight збірки iOS використовують після публікації relay-backed реєстрацій до gateway. Цей URL має збігатися з URL relay, вбудованим у збірку iOS.
- `gateway.push.apns.relay.timeoutMs`: тайм-аут надсилання gateway-to-relay у мілісекундах. Типове значення: `10000`.
- Реєстрації через relay делегуються конкретній identity gateway. Пов’язаний застосунок iOS отримує `gateway.identity.get`, включає цю identity в реєстрацію relay і передає gateway дозвіл на надсилання в межах реєстрації. Інший gateway не може повторно використати цю збережену реєстрацію.
...
- `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true`: лише для розробки, аварійний дозвіл на loopback HTTP URL relay. У production URL relay мають залишатися на HTTPS.
- `gateway.channelHealthCheckMinutes`: інтервал моніторингу стану channels у хвилинах. Установіть `0`, щоб глобально вимкнути перезапуски health-monitor. Типове значення: `5`.
...
- `gateway.channelMaxRestartsPerHour`: максимальна кількість перезапусків health-monitor на channel/обліковий запис у межах ковзної години. Типове значення: `10`.
- `channels.<provider>.healthMonitor.enabled`: відмова для окремого channel від перезапусків health-monitor, при цьому глобальний монітор лишається ввімкненим.
- `channels.<provider>.accounts.<accountId>.healthMonitor.enabled`: перевизначення для окремого облікового запису в багатооблікових channels. Якщо встановлено, воно має пріоритет над перевизначенням на рівні channel.
...
- Якщо `gateway.auth.token` / `gateway.auth.password` явно налаштовано через SecretRef і вони не розв’язуються, розв’язання завершується fail-closed (без резервного переходу до remote).
- `trustedProxies`: IP reverse proxy, які завершують TLS або вставляють заголовки переадресованого клієнта. Вказуйте лише proxy, які ви контролюєте. Записи loopback і далі валідні для конфігурацій proxy/локального виявлення на тому самому хості (наприклад Tailscale Serve або локальний reverse proxy), але вони **не** роблять loopback-запити придатними для `gateway.auth.mode: "trusted-proxy"`.
...
- `keyPath`: шлях файлової системи до приватного ключа TLS; обмежуйте права доступу.
- `caPath`: необов’язковий шлях до набору CA для перевірки клієнта або власних ланцюгів довіри.
...
- `mode`: керує тим, як зміни конфігурації застосовуються під час runtime.
  ...
  - `"hybrid"` (типово): спочатку пробувати hot reload; за потреби переходити до restart.
- `debounceMs`: вікно debounce в ms перед застосуванням змін конфігурації (невід’ємне ціле число).
- `deferralTimeoutMs`: максимальний час у ms очікування завершення поточних операцій перед примусовим перезапуском (типово: `300000` = 5 хвилин).
...
- `allowRequestSessionKey`: дозволяє викликувачам `/hooks/agent` і session key у mapping, керованих шаблонами, встановлювати `sessionKey` (типово: `false`).
...
- `model` перевизначає LLM для цього запуску hook (має бути дозволений, якщо встановлено каталог моделей).
...
- Якщо ви зберігаєте цю маршрутизацію для окремих повідомлень, установіть `hooks.allowRequestSessionKey: true` і обмежте `hooks.allowedSessionKeyPrefixes`, щоб вони відповідали простору імен Gmail, наприклад `["hook:", "hook:gmail:"]`.
- Якщо вам потрібен `hooks.allowRequestSessionKey: false`, перевизначте preset статичним `sessionKey` замість шаблонізованого типового значення.
...
- Для bind не через loopback: маршрути canvas вимагають auth Gateway (token/password/trusted-proxy), так само як і інші HTTP-поверхні Gateway.
- Node WebViews зазвичай не надсилають auth-заголовки; після pairing і підключення node Gateway рекламує URL можливостей з областю node для доступу до canvas/A2UI.
...
- Вбудовані env vars застосовуються лише тоді, коли в env процесу бракує цього ключа.
...
Посилання на секрети є додатковими: plaintext-значення й далі працюють.
...
- `id` для `source: "exec"` не мають містити slash-розділених сегментів шляху `.` або `..` (наприклад `a/../b` відхиляється)
...
- Посилання в `auth-profiles.json` включаються в runtime-розв’язання й покриття аудиту.
...
- Шляхи provider-ів file і exec завершуються fail-closed, коли перевірка Windows ACL недоступна. Установлюйте `allowInsecurePath: true` лише для довірених шляхів, які не можна перевірити.
- Provider `exec` вимагає абсолютний шлях `command` і використовує payload-и протоколу через stdin/stdout.
- Типово шляхи команд symlink відхиляються. Установіть `allowSymlinkCommand: true`, щоб дозволити шляхи symlink із перевіркою розв’язаного цільового шляху.
...
- Посилання на секрети розв’язуються під час активації в in-memory snapshot, після чого шляхи запитів читають лише цей snapshot.
- Під час активації застосовується фільтрація активної поверхні: нерозв’язані refs на ввімкнених поверхнях спричиняють збій startup/reload, а неактивні поверхні пропускаються з діагностикою.
...
- `auth-profiles.json` підтримує refs на рівні значень (`keyRef` для `api_key`, `tokenRef` для `token`) для режимів статичних облікових даних.
- Профілі режиму OAuth (`auth.profiles.<id>.mode = "oauth"`) не підтримують облікові дані auth-profile на основі SecretRef.
- Статичні runtime-облікові дані беруться з розв’язаних in-memory snapshot-ів; застарілі статичні записи `auth.json` очищаються після виявлення.
...
- `billingBackoffHours`: базовий backoff у годинах, коли профіль зазнає збою через справжні
  помилки billing/недостатнього кредиту (типово: `5`). Явний текст про billing усе одно
  може потрапити сюди навіть у відповідях `401`/`403`, але специфічні для provider-а
  текстові зіставлення залишаються обмеженими provider-ом, якому вони належать (наприклад OpenRouter
  `Key limit exceeded`). Відновлювані повідомлення HTTP `402` про вікно використання або
  ліміт витрат організації/робочого простору натомість лишаються в шляху `rate_limit`.
- `billingBackoffHoursByProvider`: необов’язкові перевизначення годин backoff для billing на рівні provider-а.
...
- `overloadedProfileRotations`: максимальна кількість ротацій auth-profile в межах одного provider-а для помилок перевантаження перед переходом до fallback моделі (типово: `1`). Такі форми зайнятості provider-а, як `ModelNotReadyException`, потрапляють сюди.
- `overloadedBackoffMs`: фіксована затримка перед повторною спробою ротації перевантаженого provider-а/профілю (типово: `0`).
- `rateLimitedProfileRotations`: максимальна кількість ротацій auth-profile в межах одного provider-а для помилок rate-limit перед переходом до fallback моделі (типово: `1`). Цей bucket rate-limit включає характерний для provider-ів текст, як-от `Too many concurrent requests`, `ThrottlingException`, `concurrency limit reached`, `workers_ai ... quota limit exceeded` і `resource exhausted`.
...
- `maxFileBytes`: максимальний розмір файла журналу в байтах, після якого записи припиняються (додатне ціле число; типово: `524288000` = 500 MB). Для production-розгортань використовуйте зовнішню ротацію логів.
...
- `flags`: масив рядків прапорів, які вмикають цільовий вивід журналу (підтримуються wildcard-и на кшталт `"telegram.*"` або `"*"`).
...
- `otel.headers`: додаткові HTTP/gRPC metadata-заголовки, які надсилаються із запитами експорту OTel.
...
- `otel.sampleRate`: частота sampling для trace `0`–`1`.
- `otel.flushIntervalMs`: інтервал періодичного скидання telemetry у ms.
- `cacheTrace.enabled`: журналювати snapshot-и cache trace для вбудованих запусків (типово: `false`).
...
- `auto.enabled`: увімкнути фонове auto-update для package-встановлень (типово: `false`).
...
- `dispatch.enabled`: незалежний gate для dispatch ходів сесії ACP (типово: `true`). Установіть `false`, щоб зберегти доступність команд ACP, але блокувати виконання.
- `backend`: id типового runtime backend-а ACP (має збігатися із зареєстрованим ACP runtime Plugin).
...
- `stream.maxChunkChars`: максимальний розмір chunk перед розбиттям потокової проєкції блоку.
...
- `stream.maxOutputChars`: максимальна кількість символів виводу помічника, спроєктованих за один ACP-хід.
...
- `runtime.ttlMinutes`: idle TTL у хвилинах для worker-ів сесій ACP до можливого очищення.
...
- `cli.banner.taglineMode` керує стилем слогана в banner:
  ...
  - `"off"`: без тексту слогана (назва/версія banner все одно показуються).
...
Поточні збірки більше не містять TCP bridge. Node-и підключаються через Gateway WebSocket. Ключі `bridge.*` більше не входять до схеми конфігурації (валідація завершується помилкою, доки їх не буде видалено; `openclaw doctor --fix` може прибрати невідомі ключі).

<Accordion title="Legacy bridge config (історична довідка)">
...
- `sessionRetention`: як довго зберігати завершені ізольовані сесії запусків Cron перед очищенням із `sessions.json`. Також керує очищенням архівованих видалених transcript-ів Cron. Типове значення: `24h`; установіть `false`, щоб вимкнути.
...
- `runLog.keepLines`: кількість найновіших рядків, що зберігаються, коли запускається очищення run-log. Типове значення: `2000`.
- `webhookToken`: bearer token, який використовується для POST-доставки Webhook Cron (`delivery.mode = "webhook"`); якщо не вказано, auth-заголовок не надсилається.
...
- Коли не встановлено ні глобальну, ні окрему для завдання failure destination, завдання, які вже доставляють через `announce`, у разі збою повертаються до цієї основної announce-цілі.
...
## Include-ви конфігурації (`$include`)
...
- Шляхи: розв’язуються відносно файла, що включає, але мають залишатися в межах каталогу конфігурації верхнього рівня (`dirname` для `openclaw.json`). Абсолютні форми/форми з `../` дозволені лише тоді, коли після розв’язання вони все одно залишаються в межах цього кордону.
- Записи, якими керує OpenClaw і які змінюють лише один верхньорівневий розділ, підкріплений include-ом одного файла, записуються безпосередньо в цей включений файл. Наприклад, `plugins install` оновлює `plugins: { $include: "./plugins.json5" }` у `plugins.json5` і залишає `openclaw.json` без змін.
- Кореневі include-и, масиви include-ів та include-и із сусідніми перевизначеннями є лише для читання для записів, якими керує OpenClaw; такі записи завершуються fail-closed замість сплощення конфігурації.
