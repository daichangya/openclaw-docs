...
<Steps>
  ...
  <Step title="Gerar uma URL de convite e adicionar o bot ao seu servidor">
    ...
    **Text Permissions**
    ...
    Este é o conjunto básico para canais de texto normais. Se você planeja publicar em threads do Discord, incluindo fluxos de canais de fórum ou mídia que criam ou continuam uma thread, ative também **Send Messages in Threads**.
  ...
  <Step title="Definir seu token de bot com segurança (não o envie no chat)">
    O token do seu bot do Discord é um segredo (como uma senha). Defina-o na máquina que está executando o OpenClaw antes de enviar mensagem ao seu agente.
...
```
    ...
    Se o OpenClaw já estiver sendo executado como um serviço em segundo plano, reinicie-o pelo app OpenClaw para Mac ou interrompendo e reiniciando o processo `openclaw gateway run`.
  ...
  <Step title="Configurar o OpenClaw e parear">
    ...
    <Tabs>
      <Tab title="Peça ao seu agente">
        ...
        > "Eu já defini meu token de bot do Discord na config. Por favor, conclua a configuração do Discord com User ID `<user_id>` e Server ID `<server_id>`."
      </Tab>
...
<Note>
A resolução de token considera a conta. Valores de token na config têm prioridade sobre o fallback de env. `DISCORD_BOT_TOKEN` é usado apenas para a conta padrão.
Para chamadas de saída avançadas (ferramenta de mensagem/ações de canal), um `token` explícito por chamada é usado nessa chamada. Isso se aplica a ações de envio e de leitura/sondagem (por exemplo, read/search/fetch/thread/pins/permissions). As configurações de política de conta/repetição ainda vêm da conta selecionada no snapshot ativo do runtime.
</Note>

## Recomendado: configurar um workspace de servidor

Quando as DMs estiverem funcionando, você poderá configurar seu servidor do Discord como um workspace completo, em que cada canal recebe sua própria sessão de agente com seu próprio contexto. Isso é recomendado para servidores privados em que estão apenas você e seu bot.
...
```
  ...
  <Step title="Permitir respostas sem @mention">
    Por padrão, seu agente só responde em canais do servidor quando é mencionado com @. Em um servidor privado, você provavelmente vai querer que ele responda a todas as mensagens.
...
Canais de fórum e mídia do Discord aceitam apenas publicações em thread. O OpenClaw oferece duas formas de criá-las:
...
<Tabs>
  <Tab title="Política de DM">
    ...
    - Contas nomeadas herdam `channels.discord.allowFrom` quando o próprio `allowFrom` delas não estiver definido.
  ...
  <Tab title="Política de servidor">
    ...
    - listas de permissões opcionais de remetente: `users` (IDs estáveis recomendados) e `roles` (somente IDs de função); se qualquer um estiver configurado, remetentes serão permitidos quando corresponderem a `users` OU `roles`
    - correspondência direta por nome/tag vem desativada por padrão; ative `channels.discord.dangerouslyAllowNameMatching: true` apenas como modo de compatibilidade de emergência
    ...
    - se um servidor não tiver bloco `channels`, todos os canais naquele servidor da allowlist serão permitidos
...
### Roteamento de agente com base em função
...
<AccordionGroup>
  ...
  <Accordion title="Copiar IDs">
    Ative o Discord Developer Mode e então copie:
...
- Os comandos ainda podem ser visíveis na interface do Discord para usuários não autorizados; a execução ainda aplica a autenticação do OpenClaw e retorna "não autorizado".

Consulte [Comandos slash](/pt-BR/tools/slash-commands) para o catálogo de comandos e comportamento.
...
<AccordionGroup>
  <Accordion title="Tags de resposta e respostas nativas">
    ...
    `first` sempre anexa a referência implícita de resposta nativa à primeira mensagem de saída do Discord naquele turno.
    ...
    IDs de mensagem são expostos no contexto/histórico para que agentes possam segmentar mensagens específicas.
  ...
  <Accordion title="Prévia de transmissão ao vivo">
    ...
    - `channels.discord.streaming` controla a prévia por streaming (`off` | `partial` | `block` | `progress`, padrão: `off`).
    - O padrão permanece `off` porque edições de prévia no Discord podem atingir rapidamente limites de taxa, especialmente quando vários bots ou gateways compartilham a mesma conta ou tráfego de servidor.
    - `progress` é aceito por consistência entre canais e mapeia para `partial` no Discord.
    ...
    - Mídia, erro e respostas finais explícitas cancelam edições de prévia pendentes sem enviar um rascunho temporário antes da entrega normal.
    - `streaming.preview.toolProgress` controla se atualizações de ferramenta/progresso reutilizam a mesma mensagem de prévia em rascunho (padrão: `true`). Defina `false` para manter mensagens separadas de ferramenta/progresso.
...
```
    ...
    Padrões de fragmentação do modo `block` (limitados a `channels.discord.textChunkLimit`):
...
```
    ...
    A prévia por streaming é somente texto; respostas com mídia recorrem à entrega normal.

    Observação: a prévia por streaming é separada do streaming em blocos. Quando o streaming em blocos é explicitamente
    ativado para Discord, o OpenClaw ignora a prévia por streaming para evitar streaming duplo.
  ...
  <Accordion title="Histórico, contexto e comportamento de thread">
    ...
    - metadados da thread pai podem ser usados para vinculação de sessão pai
    - a config da thread herda a config do canal pai, a menos que exista uma entrada específica para a thread
  ...
  channels: {
    discord: {
      threadBindings: {
        ...
        idleHours: 24,
        maxAgeHours: 0,
        spawnSubagentSessions: false, // opcional, mediante ativação
      },
    },
  },
}
```
    ...
    - Se os vínculos de thread estiverem desativados para uma conta, `/focus` e operações relacionadas de vínculo de thread não ficarão disponíveis.
...
```
    ...
    - `/acp spawn codex --bind here` vincula o canal ou thread atual do Discord no lugar e mantém mensagens futuras roteadas para a mesma sessão ACP.
    ...
    - O Codex ainda pode ser executado em seu próprio `cwd` ou workspace de backend em disco. Esse workspace é estado de runtime, não uma thread do Discord.
    - Mensagens de thread podem herdar o vínculo ACP do canal pai.
    - Em um canal ou thread vinculado, `/new` e `/reset` redefinem a mesma sessão ACP no lugar.
    - Vínculos temporários de thread ainda funcionam e podem sobrescrever a resolução do destino enquanto estiverem ativos.
    - `spawnAcpSessions` só é necessário quando o OpenClaw precisa criar/vincular uma thread filha via `--thread auto|here`. Ele não é necessário para `/acp spawn ... --bind here` no canal atual.
...
```
    ...
    - nomes de exibição de membro são correspondidos por nome/slug somente quando `channels.discord.dangerouslyAllowNameMatching: true`
...
```
  ...
  <Accordion title="Aprovações no Discord">
    ...
    deve incluir um comando manual `/approve` apenas quando o resultado da ferramenta informar
    ...
    - no modo local, `gateway.remote.*` pode ser usado como fallback somente quando `gateway.auth.*` não estiver definido; SecretRefs locais configurados, mas não resolvidos, falham de forma fechada
    - suporte a modo remoto via `gateway.remote.*` quando aplicável
    - sobrescritas de URL são seguras para sobrescrita: sobrescritas por CLI não reutilizam credenciais implícitas, e sobrescritas por env usam apenas credenciais de env
    ...
    - O Discord não faz um salto extra de fallback de exec para plugin aqui; o
...
Use o comando nativo exclusivo do Discord `/vc join|leave|status` para controlar sessões. O comando usa o agente padrão da conta e segue as mesmas regras de allowlist e política de grupo dos outros comandos do Discord.
...
- Turnos de transcrição de voz derivam o status de proprietário de `allowFrom` do Discord (ou `dm.allowFrom`); falantes não proprietários não podem acessar ferramentas exclusivas do proprietário (por exemplo `gateway` e `cron`).
- Voz vem ativada por padrão; defina `channels.discord.voice.enabled=false` para desativá-la.
...
- Se os logs de recebimento mostrarem repetidamente `DecryptionFailed(UnencryptedWhenPassthroughDisabled)`, isso pode ser o bug de recepção upstream de `@discordjs/voice` rastreado em [discord.js #11419](https://github.com/discordjs/discord.js/issues/11419).

## Mensagens de voz

Mensagens de voz do Discord mostram uma prévia de forma de onda e exigem áudio OGG/Opus além de metadados. O OpenClaw gera a forma de onda automaticamente, mas precisa que `ffmpeg` e `ffprobe` estejam disponíveis no host do gateway para inspecionar e converter arquivos de áudio.
...
```
  ...
  <Accordion title="Manipuladores de longa duração expiram ou respostas duplicam">
