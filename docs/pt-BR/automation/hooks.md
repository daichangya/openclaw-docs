...
- **Hooks internos** (esta página): executados dentro do Gateway quando eventos do agente disparam, como `/new`, `/reset`, `/stop` ou eventos do ciclo de vida.
...
```bash
# Lista os hooks disponíveis
openclaw hooks list

# Habilita um hook
openclaw hooks enable session-memory

# Verifica o status do hook
...
| `session:compact:after`  | Depois que a Compaction termina                 |
...
| `message:preprocessed`   | Depois que todo o entendimento de mídia e links termina |
...
| `requires` | `bins`, `anyBins`, `env` ou caminhos `config` necessários |
...
const handler = async (event) => {
  ...
  // Opcionalmente, envie mensagem ao usuário
...
Hooks são descobertos nestes diretórios, em ordem crescente de precedência de sobrescrita:
...
Habilite qualquer hook agrupado:
...
<Note>
O formato legado de configuração de array `hooks.internal.handlers` ainda é compatível por questões de compatibilidade retroativa, mas novos hooks devem usar o sistema baseado em descoberta.
</Note>
...
# Mostra o resumo de elegibilidade
openclaw hooks check
...
- **Mantenha os manipuladores rápidos.** Hooks são executados durante o processamento de comandos. Dispare trabalhos pesados em segundo plano com `void processInBackground(event)`.
...
Verifique bins ausentes (PATH), variáveis de ambiente, valores de configuração ou compatibilidade com o SO.
