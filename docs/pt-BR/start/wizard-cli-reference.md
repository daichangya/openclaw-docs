...
summary: Referência completa do fluxo de configuração da CLI, configuração de autenticação/modelo, saídas e componentes internos
title: Referência de configuração da CLI
x-i18n:
    generated_at: "2026-04-23T05:43:44Z"
    model: gpt-5.4
    provider: openai
...
Para o guia curto, veja [Onboarding (CLI)](/pt-BR/start/wizard).
...
<Steps>
  ...
  <Step title="Workspace">
    ...
    - Semeia arquivos de workspace necessários para o ritual de bootstrap da primeira execução.
  ...
  <Step title="Gateway">
    ...
    - Caminho não interativo para token SecretRef: `--gateway-token-ref-env <ENV_VAR>`.
  ...
  <Step title="Instalação do daemon">
    - macOS: LaunchAgent
      - Exige sessão de usuário logada; para modo headless, use um LaunchDaemon personalizado (não incluído).
    ...
    - Seleção de runtime: Node (recomendado; exigido para WhatsApp e Telegram). Bun não é recomendado.
  </Step>
  <Step title="Verificação de saúde">
    - Inicia o gateway (se necessário) e executa `openclaw health`.
    - `openclaw status --deep` adiciona a sondagem de saúde do gateway ao vivo à saída de status, incluindo sondagens de canal quando compatível.
  </Step>
  <Step title="Skills">
...
<Note>
Se nenhuma GUI for detectada, o assistente imprime instruções de encaminhamento de porta por SSH para a Control UI em vez de abrir um navegador.
...
<AccordionGroup>
  ...
  <Accordion title="Ollama (Cloud e modelos abertos locais)">
    ...
    Os modos respaldados por host solicitam URL base (padrão `http://127.0.0.1:11434`), descobrem modelos disponíveis e sugerem padrões.
  ...
  <Accordion title="Provedor personalizado">
    ...
    - **Usar referência de segredo** (referência de env ou referência de provedor configurado, com validação preliminar)
...
- Escolha o modelo padrão a partir das opções detectadas ou informe manualmente provedor e modelo.
...
- O modo interativo de referência executa uma validação preliminar rápida antes de salvar.
  - Referências de env: valida o nome da variável + valor não vazio no ambiente atual do onboarding.
...
- No modo não interativo, `--secret-input-mode ref` usa apenas env.
...
- `session.dmScope` (o onboarding local usa por padrão `per-channel-peer` quando não definido; valores explícitos existentes são preservados)
