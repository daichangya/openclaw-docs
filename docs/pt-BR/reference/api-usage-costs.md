...
read_when:
    ...
    - Você precisa auditar chaves, custos e a visibilidade de uso
...
recursos do OpenClaw que podem gerar uso de provedores ou chamadas pagas de API.
...
- Se os metadados da sessão em tempo real forem escassos, `/status` pode recuperar
  contadores de tokens/cache e o rótulo do modelo de runtime ativo a partir da entrada
  de uso mais recente da transcrição. Valores em tempo real não nulos existentes ainda têm prioridade, e totais da transcrição no tamanho do prompt podem prevalecer quando os totais armazenados estiverem ausentes ou forem menores.
...
- `/usage tokens` mostra apenas tokens; fluxos de OAuth/token no estilo assinatura e da CLI ocultam o custo em dólares.
- Observação sobre o Gemini CLI: quando a CLI retorna saída JSON, o OpenClaw lê o uso de
...
novamente permitido, então o OpenClaw trata a reutilização do Claude CLI e o uso de `claude -p` como
...
- A saída legível por humanos é normalizada para `X% left` entre os provedores.
...
- A autenticação de uso para essas janelas de cota vem de hooks específicos do provedor quando
  ...
  OAuth/chave de API a partir de perfis de autenticação, ambiente ou configuração.

Consulte [Uso e custos de tokens](/pt-BR/reference/token-use) para detalhes e exemplos.
...
o caminho Claude-login do OpenClaw da Anthropic com **Extra Usage** habilitado.

Consulte [Modelos](/pt-BR/providers/models) para configuração de preços e [Uso e custos de tokens](/pt-BR/reference/token-use) para exibição.
...
Recursos compartilhados de geração também podem consumir chaves de provedores:
...
A busca semântica na memória usa **APIs de embeddings** quando configurada para provedores remotos:
...
- **Ollama Web Search**: sem chave por padrão, mas requer um host Ollama acessível e `ollama signin`; também pode reutilizar a autenticação bearer normal do provedor Ollama quando o host exigir isso
...
Caminhos legados de provedor `tools.web.search.*` ainda são carregados por meio do shim temporário de compatibilidade, mas já não são mais a superfície de configuração recomendada.
...
Essas chamadas normalmente têm baixo volume, mas ainda assim atingem APIs de provedores:
