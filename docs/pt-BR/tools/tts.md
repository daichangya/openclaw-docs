...
x-i18n:
    generated_at: "2026-04-23T05:44:58Z"
    model: gpt-5.4
    provider: openai
...
Funciona em qualquer lugar em que o OpenClaw possa enviar áudio.
...
O provedor de fala da Microsoft empacotado atualmente usa o serviço
online de TTS neural do Microsoft Edge por meio da biblioteca `node-edge-tts`. É um serviço hospedado (não
...
a padrão. `language` aceita uma tag BCP-47 ou `auto`.
...
### Responder com áudio apenas após uma mensagem de voz de entrada
...
- `auto`: modo de TTS automático (`off`, `always`, `inbound`, `tagged`).
  - `inbound` só envia áudio após uma mensagem de voz de entrada.
...
- `modelOverrides`: permite que o modelo emita diretivas de TTS (habilitado por padrão).
  - `allowProvider` usa `false` por padrão (troca de provedor é opt-in).
- `providers.<id>`: configurações do provedor proprietário indexadas pelo id do provedor de fala.
...
- `prefsPath`: substitui o caminho local do JSON de prefs (provedor/limite/resumo).
- Valores `apiKey` recorrem a variáveis de ambiente (`ELEVENLABS_API_KEY`/`XI_API_KEY`, `GEMINI_API_KEY`/`GOOGLE_API_KEY`, `MINIMAX_API_KEY`, `OPENAI_API_KEY`).
- `providers.elevenlabs.baseUrl`: substitui a URL base da API ElevenLabs.
- `providers.openai.baseUrl`: substitui o endpoint de TTS da OpenAI.
  ...
  - Valores diferentes do padrão são tratados como endpoints TTS compatíveis com OpenAI, então nomes personalizados de modelo e voz são aceitos.
...
- `providers.google.model`: modelo TTS Gemini (padrão `gemini-3.1-flash-tts-preview`).
...
- `providers.google.baseUrl`: substitui a URL base da API Gemini. Apenas `https://generativelanguage.googleapis.com` é aceito.
  - Se `messages.tts.providers.google.apiKey` for omitido, o TTS pode reutilizar `models.providers.google.apiKey` antes do fallback por env.
...
- `providers.xai.voiceId`: id da voz xAI (padrão `eve`; vozes live atuais: `ara`, `eve`, `leo`, `rex`, `sal`, `una`).
...
- `providers.xai.speed`: override nativo de velocidade do provedor.
- `providers.microsoft.enabled`: permite uso de fala da Microsoft (padrão `true`; sem chave de API).
- `providers.microsoft.voice`: nome de voz neural da Microsoft (ex.: `en-US-MichelleNeural`).
- `providers.microsoft.lang`: código de idioma (ex.: `en-US`).
- `providers.microsoft.outputFormat`: formato de saída da Microsoft (ex.: `audio-24khz-48kbitrate-mono-mp3`).
  - Consulte os formatos de saída do Microsoft Speech para valores válidos; nem todos os formatos são compatíveis com o transporte empacotado com suporte do Edge.
...
- `providers.microsoft.timeoutMs`: override do tempo limite da solicitação (ms).
- `edge.*`: alias legado para as mesmas configurações da Microsoft.

## Overrides orientados por modelo (habilitados por padrão)
...
Desabilite todos os overrides orientados por modelo:
...
Os comandos de barra gravam overrides locais em `prefsPath` (padrão:
`~/.openclaw/settings/tts.json`, substituível por `OPENCLAW_TTS_PREFS` ou
...
- **Google Gemini**: o TTS da API Gemini retorna PCM bruto de 24kHz. O OpenClaw o encapsula como WAV para anexos de áudio e retorna PCM diretamente para Talk/telefonia. O formato nativo de nota de voz Opus não é compatível com esse caminho.
- **xAI**: MP3 por padrão; `responseFormat` pode ser `mp3`, `wav`, `pcm`, `mulaw` ou `alaw`. O OpenClaw usa o endpoint REST em lote de TTS da xAI e retorna um anexo de áudio completo; o WebSocket de streaming TTS da xAI não é usado por esse caminho de provedor. O formato nativo de nota de voz Opus não é compatível com esse caminho.
- **Microsoft**: usa `microsoft.outputFormat` (padrão `audio-24khz-48kbitrate-mono-mp3`).
  - O transporte empacotado aceita `outputFormat`, mas nem todos os formatos estão disponíveis no serviço.
...
Veja [Comandos de barra](/pt-BR/tools/slash-commands) para detalhes de habilitação.

Observação sobre Discord: `/tts` é um comando interno do Discord, então o OpenClaw registra
...
- Falhas de API da OpenAI e ElevenLabs agora incluem detalhes de erro do provedor analisados e id da solicitação (quando retornado pelo provedor), que são exibidos em erros/logs de TTS.
