...
Referencia principal de configuración para `~/.openclaw/openclaw.json`. Para una visión general orientada a tareas, consulta [Configuration](/es/gateway/configuration).

Esta página cubre las principales superficies de configuración de OpenClaw y enlaza hacia fuera cuando un subsistema tiene su propia referencia más profunda. **No** intenta incluir en línea cada catálogo de comandos propiedad de canal/Plugin ni cada ajuste profundo de memoria/QMD en una sola página.

Fuente de verdad del código:

- `openclaw config schema` imprime el JSON Schema en vivo usado para la validación y la Control UI, con los metadatos integrados de Plugin/canal combinados cuando están disponibles
...
- [Memory configuration reference](/es/reference/memory-config) para `agents.defaults.memorySearch.*`, `memory.qmd.*`, `memory.citations` y la configuración de dreaming bajo `plugins.entries.memory-core.config.dreaming`
...
Si falta por completo un bloque de proveedor (`channels.<provider>` ausente), la política de grupo en tiempo de ejecución recurre a `allowlist` (bloqueo por defecto) con una advertencia al inicio.
</Note>

### Anulaciones de modelo por canal

Usa `channels.modelByChannel` para fijar IDs de canal concretos a un modelo. Los valores aceptan `provider/model` o alias de modelos configurados. La asignación del canal se aplica cuando una sesión aún no tiene una anulación de modelo (por ejemplo, establecida mediante `/model`).
...
```json5
{
  channels: {
    whatsapp: {
      ...
      mediaMaxMb: 50,
      sendReadReceipts: true, // ticks azules (false en modo de chat con uno mismo)
      groups: {
        "*": { requireMention: true },
      },
...
- `channels.whatsapp.defaultAccount` opcional anula esa selección de cuenta predeterminada de reserva cuando coincide con un id de cuenta configurado.
...
```json5
{
  channels: {
    telegram: {
      ...
      linkPreview: true,
      streaming: "partial", // off | partial | block | progress (predeterminado: off; habilítalo explícitamente para evitar límites de frecuencia de edición de vista previa)
...
- Token del bot: `channels.telegram.botToken` o `channels.telegram.tokenFile` (solo archivo normal; se rechazan enlaces simbólicos), con `TELEGRAM_BOT_TOKEN` como reserva para la cuenta predeterminada.
...
- En configuraciones multicuenta (2+ ids de cuenta), establece un valor predeterminado explícito (`channels.telegram.defaultAccount` o `channels.telegram.accounts.default`) para evitar el enrutamiento de reserva; `openclaw doctor` avisa cuando falta o no es válido.
...
- Las entradas `bindings[]` de nivel superior con `type: "acp"` configuran asociaciones persistentes de ACP para temas de foro (usa `chatId:topic:topicId` canónico en `match.peer.id`). La semántica de los campos se comparte en [ACP Agents](/es/tools/acp-agents#channel-specific-settings).
...
- Las llamadas directas salientes que proporcionan un `token` explícito de Discord usan ese token para la llamada; los ajustes de reintento/política de cuenta siguen viniendo de la cuenta seleccionada en la instantánea activa del entorno de ejecución.
...
- Usa `user:<id>` (DM) o `channel:<id>` (canal del servidor) para los destinos de entrega; los IDs numéricos sin prefijo se rechazan.
- Los slugs de servidores están en minúsculas con los espacios sustituidos por `-`; las claves de canal usan el nombre convertido a slug (sin `#`). Prefiere IDs de servidor.
- Los mensajes creados por bots se ignoran de forma predeterminada. `allowBots: true` los habilita; usa `allowBots: "mentions"` para aceptar solo mensajes de bot que mencionen al bot (los mensajes propios siguen filtrándose).
- `channels.discord.guilds.<id>.ignoreOtherMentions` (y las anulaciones por canal) descarta mensajes que mencionan a otro usuario o rol pero no al bot (excepto @everyone/@here).
...
- `channels.discord.threadBindings` controla el enrutamiento vinculado a hilos de Discord:
  - `enabled`: anulación de Discord para las funciones de sesión vinculadas a hilos (`/focus`, `/unfocus`, `/agents`, `/session idle`, `/session max-age` y la entrega/enrutamiento vinculados)
...
- Las entradas `bindings[]` de nivel superior con `type: "acp"` configuran asociaciones persistentes de ACP para canales e hilos (usa el id de canal/hilo en `match.peer.id`). La semántica de los campos se comparte en [ACP Agents](/es/tools/acp-agents#channel-specific-settings).
...
- `channels.discord.voice` habilita conversaciones en canales de voz de Discord y anulaciones opcionales de unión automática + TTS.
...
- `channels.discord.execApprovals`: entrega nativa en Discord para aprobación de ejecución y autorización de aprobadores.
  - `enabled`: `true`, `false` o `"auto"` (predeterminado). En modo auto, las aprobaciones de ejecución se activan cuando los aprobadores pueden resolverse desde `approvers` o `commands.ownerAllowFrom`.
  ...
  - `target`: dónde enviar solicitudes de aprobación. `"dm"` (predeterminado) las envía a los DMs del aprobador, `"channel"` las envía al canal de origen, `"both"` las envía a ambos. Cuando el destino incluye `"channel"`, los botones solo pueden usarlos aprobadores resueltos.
  ...
  channels: {
    slack: {
      ...
      streaming: {
        ...
        nativeTransport: true, // usa la API nativa de streaming de Slack cuando mode=partial
      },
      mediaMaxMb: 20,
      execApprovals: {
        enabled: "auto", // true | false | "auto"
...
- **Modo Socket** requiere tanto `botToken` como `appToken` (`SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN` para la reserva por entorno de la cuenta predeterminada).
...
- El streaming nativo de Slack más el estado de hilo de estilo asistente de Slack "is typing..." requieren un destino de respuesta en hilo. Los DMs de nivel superior permanecen fuera del hilo de forma predeterminada, por lo que usan `typingReaction` o la entrega normal en lugar de la vista previa de estilo hilo.
...
- `channels.slack.execApprovals`: entrega nativa en Slack para aprobación de ejecución y autorización de aprobadores. Mismo esquema que Discord: `enabled` (`true`/`false`/`"auto"`), `approvers` (IDs de usuario de Slack), `agentFilter`, `sessionFilter` y `target` (`"dm"`, `"channel"` o `"both"`).
...
Modos de chat: `oncall` (responde con @mención, predeterminado), `onmessage` (cada mensaje), `onchar` (mensajes que empiezan con un prefijo disparador).
...
- `channels.mattermost.groups.<channelId>.requireMention`: anulación de restricción por mención por canal (`"*"` para el valor predeterminado).
...
```json5
{
  channels: {
    signal: {
      enabled: true,
      account: "+15555550123", // asociación opcional de cuenta
      dmPolicy: "pairing",
...
- Las entradas `bindings[]` de nivel superior con `type: "acp"` pueden vincular conversaciones de BlueBubbles a sesiones persistentes de ACP. Usa un identificador BlueBubbles o una cadena de destino (`chat_id:*`, `chat_guid:*`, `chat_identifier:*`) en `match.peer.id`. Semántica compartida de los campos: [ACP Agents](/es/tools/acp-agents#channel-specific-settings).
...
- `cliPath` puede apuntar a un wrapper SSH; establece `remoteHost` (`host` o `user@host`) para recuperar archivos adjuntos mediante SCP.
...
- Las entradas `bindings[]` de nivel superior con `type: "acp"` pueden vincular conversaciones de iMessage a sesiones persistentes de ACP. Usa un identificador normalizado o un destino de chat explícito (`chat_id:*`, `chat_guid:*`, `chat_identifier:*`) en `match.peer.id`. Semántica compartida de los campos: [ACP Agents](/es/tools/acp-agents#channel-specific-settings).
...
- `channels.matrix.proxy` enruta el tráfico HTTP de Matrix a través de un proxy HTTP(S) explícito. Las cuentas nombradas pueden reemplazarlo con `channels.matrix.accounts.<id>.proxy`.
...
- `channels.matrix.autoJoin` usa `off` de forma predeterminada, por lo que las salas invitadas y las invitaciones nuevas de estilo DM se ignoran hasta que establezcas `autoJoin: "allowlist"` con `autoJoinAllowlist` o `autoJoin: "always"`.
- `channels.matrix.execApprovals`: entrega nativa en Matrix para aprobación de ejecución y autorización de aprobadores.
  - `enabled`: `true`, `false` o `"auto"` (predeterminado). En modo auto, las aprobaciones de ejecución se activan cuando los aprobadores pueden resolverse desde `approvers` o `commands.ownerAllowFrom`.
...
- `channels.matrix.dm.sessionScope` controla cómo los DMs de Matrix se agrupan en sesiones: `per-user` (predeterminado) comparte por par enrutado, mientras que `per-room` aísla cada sala de DM.
...
- Si añades una cuenta no predeterminada mediante `openclaw channels add` (o la incorporación del canal) mientras sigues en una configuración de canal de nivel superior de cuenta única, OpenClaw promociona primero los valores de cuenta única de nivel superior con ámbito de cuenta al mapa de cuentas del canal para que la cuenta original siga funcionando. La mayoría de los canales los mueve a `channels.<channel>.accounts.default`; Matrix puede conservar en su lugar un destino nombrado/predeterminado coincidente existente.
- Las asociaciones existentes solo de canal (sin `accountId`) siguen coincidiendo con la cuenta predeterminada; las asociaciones con ámbito de cuenta siguen siendo opcionales.
- `openclaw doctor --fix` también repara formas mixtas moviendo valores de cuenta única de nivel superior con ámbito de cuenta a la cuenta promocionada elegida para ese canal. La mayoría de los canales usa `accounts.default`; Matrix puede conservar en su lugar un destino nombrado/predeterminado coincidente existente.

### Otros canales Plugin

Muchos canales Plugin se configuran como `channels.<id>` y están documentados en sus páginas dedicadas del canal (por ejemplo Feishu, Matrix, LINE, Nostr, Zalo, Nextcloud Talk y Twitch).
Consulta el índice completo de canales: [Channels](/es/channels).

### Restricción por mención en chats grupales

Los mensajes de grupo usan **requiere mención** de forma predeterminada (mención en metadatos o patrones regex seguros). Se aplica a chats grupales de WhatsApp, Telegram, Discord, Google Chat e iMessage.

**Tipos de mención:**

- **Menciones en metadatos**: @menciones nativas de la plataforma. Se ignoran en el modo de chat con uno mismo de WhatsApp.
...
- Esta página es una **referencia de claves de configuración**, no el catálogo completo de comandos. Los comandos propiedad de canal/Plugin, como `/bot-ping` `/bot-help` `/bot-logs` de QQ Bot, `/card` de LINE, `/pair` de device-pair, `/dreaming` de memory, `/phone` de phone-control y `/voice` de Talk, están documentados en sus páginas de canal/Plugin, además de en [Slash Commands](/es/tools/slash-commands).
...
- `config: true` habilita `/config` (lee/escribe `openclaw.json`). Para clientes `chat.send` del Gateway, las escrituras persistentes de `/config set|unset` también requieren `operator.admin`; `/config show` de solo lectura sigue disponible para clientes operador normales con ámbito de escritura.
...
Raíz opcional del repositorio mostrada en la línea Runtime del prompt del sistema. Si no está definida, OpenClaw la detecta automáticamente subiendo desde el workspace.
...
- Una lista no vacía de `agents.list[].skills` es el conjunto final para ese agente;
...
Máximo total de caracteres inyectados en todos los archivos bootstrap del workspace. Predeterminado: `60000`.
...
- `agents.defaults.startupContext.*`:
  preludio de inicio de un solo uso para ejecuciones `/new` y `/reset`, incluidos archivos
  recientes `memory/*.md` diarios.
- `skills.limits.*`:
  la lista compacta de Skills inyectada en el prompt del sistema.
- `agents.defaults.contextLimits.*`:
  extractos delimitados del entorno de ejecución y bloques inyectados propiedad del entorno de ejecución.
- `memory.qmd.limits.*`:
...
Controla el preludio de inicio del primer turno inyectado en ejecuciones
simples de `/new` y `/reset`.
...
Valores predeterminados compartidos para superficies de contexto delimitadas del entorno de ejecución.
...
- `imageGenerationModel`: acepta una cadena (`"provider/model"`) o un objeto (`{ primary, fallbacks }`).
  ...
  - Si se omite, `image_generate` aún puede inferir un proveedor predeterminado respaldado por autenticación. Primero prueba el proveedor predeterminado actual y luego los demás proveedores registrados de generación de imágenes en orden por id de proveedor.
- `musicGenerationModel`: acepta una cadena (`"provider/model"`) o un objeto (`{ primary, fallbacks }`).
  ...
  - Si se omite, `music_generate` aún puede inferir un proveedor predeterminado respaldado por autenticación. Primero prueba el proveedor predeterminado actual y luego los demás proveedores registrados de generación de música en orden por id de proveedor.
...
- `videoGenerationModel`: acepta una cadena (`"provider/model"`) o un objeto (`{ primary, fallbacks }`).
  ...
  - Si se omite, `video_generate` aún puede inferir un proveedor predeterminado respaldado por autenticación. Primero prueba el proveedor predeterminado actual y luego los demás proveedores registrados de generación de vídeo en orden por id de proveedor.
  - Si seleccionas directamente un proveedor/modelo, configura también la autenticación/clave API correspondiente del proveedor.
  - El proveedor integrado de generación de vídeo Qwen admite hasta 1 vídeo de salida, 1 imagen de entrada, 4 vídeos de entrada, 10 segundos de duración y opciones a nivel de proveedor `size`, `aspectRatio`, `resolution`, `audio` y `watermark`.
...
- `model.primary`: formato `provider/model` (por ejemplo `openai/gpt-5.4`). Si omites el proveedor, OpenClaw intenta primero un alias, luego una coincidencia única de proveedor configurado para ese id exacto de modelo y solo después recurre al proveedor predeterminado configurado (comportamiento de compatibilidad desaprobado, así que prefiere `provider/model` explícito). Si ese proveedor ya no expone el modelo predeterminado configurado, OpenClaw recurre al primer proveedor/modelo configurado en lugar de mostrar un valor predeterminado obsoleto de un proveedor eliminado.
...
- `embeddedHarness`: política predeterminada del entorno de ejecución embebido de bajo nivel del agente. Usa `runtime: "auto"` para permitir que los harness Plugin registrados reclamen modelos compatibles, `runtime: "pi"` para forzar el harness PI integrado o un id de harness registrado como `runtime: "codex"`. Establece `fallback: "none"` para desactivar la reserva automática a PI.
- Los escritores de configuración que mutan estos campos (por ejemplo `/models set`, `/models set-image` y comandos de añadir/eliminar reserva) guardan la forma canónica de objeto y conservan las listas existentes de reserva cuando es posible.
...
`embeddedHarness` controla qué ejecutor de bajo nivel ejecuta los turnos del agente embebido.
La mayoría de las implementaciones deben mantener el valor predeterminado `{ runtime: "auto", fallback: "pi" }`.
Úsalo cuando un Plugin de confianza proporcione un harness nativo, como el
harness integrado del servidor de aplicaciones Codex.
...
- `runtime`: `"auto"`, `"pi"` o un id de harness Plugin registrado. El Plugin integrado Codex registra `codex`.
- `fallback`: `"pi"` o `"none"`. `"pi"` mantiene el harness PI integrado como reserva de compatibilidad. `"none"` hace que la selección faltante o no compatible de un harness Plugin falle en lugar de usar PI silenciosamente.
- Anulaciones por entorno: `OPENCLAW_AGENT_RUNTIME=<id|auto|pi>` reemplaza `runtime`; `OPENCLAW_AGENT_HARNESS_FALLBACK=none` desactiva la reserva a PI para ese proceso.
- Para implementaciones solo con Codex, establece `model: "codex/gpt-5.4"`, `embeddedHarness.runtime: "codex"` y `embeddedHarness.fallback: "none"`.
- Esto solo controla el harness de chat embebido. La generación de medios, visión, PDF, música, vídeo y TTS siguen usando sus propios ajustes de proveedor/modelo.
...
Los modelos GLM-4.x de Z.AI activan automáticamente el modo thinking salvo que establezcas `--thinking off` o definas tú mismo `agents.defaults.models["zai/<model>"].params.thinking`.
Los modelos de Z.AI habilitan `tool_stream` de forma predeterminada para el streaming de llamadas a herramientas. Establece `agents.defaults.models["zai/<model>"].params.tool_stream` en `false` para desactivarlo.
...
Backends CLI opcionales para ejecuciones de reserva solo de texto (sin llamadas a herramientas). Útiles como respaldo cuando fallan los proveedores API.
...
- Los backends CLI son primero texto; las herramientas siempre están desactivadas.
...
Reemplaza todo el prompt del sistema ensamblado por OpenClaw con una cadena fija. Establécelo en el nivel predeterminado (`agents.defaults.systemPromptOverride`) o por agente (`agents.list[].systemPromptOverride`). Los valores por agente tienen prioridad; un valor vacío o solo con espacios se ignora. Útil para experimentos controlados con prompts.
...
- `isolatedSession`: cuando es true, cada Heartbeat se ejecuta en una sesión nueva sin historial previo de conversación. Mismo patrón de aislamiento que `sessionTarget: "isolated"` de Cron. Reduce el coste de tokens por Heartbeat de ~100K a ~2-5K tokens.
...
- `provider`: id de un Plugin de proveedor de Compaction registrado. Cuando está definido, se llama al `summarize()` del proveedor en lugar del resumen LLM integrado. Recurre al integrado en caso de fallo. Establecer un proveedor fuerza `mode: "safeguard"`. Consulta [Compaction](/es/concepts/compaction).
...
- `postCompactionSections`: nombres opcionales de secciones H2/H3 de `AGENTS.md` que se reinyectan después de Compaction. Predeterminado: `["Session Startup", "Red Lines"]`; establece `[]` para desactivar la reinyección. Cuando no está definido o se establece explícitamente en ese par predeterminado, también se aceptan los encabezados antiguos `Every Session`/`Safety` como reserva heredada.
- `model`: anulación opcional `provider/model-id` solo para el resumen de Compaction. Úsalo cuando la sesión principal deba mantener un modelo, pero los resúmenes de Compaction deban ejecutarse con otro; cuando no está definido, Compaction usa el modelo principal de la sesión.
- `notifyUser`: cuando es `true`, envía avisos breves al usuario cuando Compaction comienza y cuando termina (por ejemplo, "Compacting context..." y "Compaction complete"). Está desactivado de forma predeterminada para mantener Compaction en silencio.
- `memoryFlush`: turno agéntico silencioso antes de la Compaction automática para almacenar memorias duraderas. Se omite cuando el workspace es de solo lectura.
...
- La poda primero recorta suavemente los resultados sobredimensionados de herramientas y luego vacía por completo los resultados más antiguos si es necesario.

**Soft-trim** conserva el principio + el final e inserta `...` en el medio.

**Hard-clear** reemplaza el resultado completo de la herramienta por el marcador.
...
Consulta [Session Pruning](/es/concepts/session-pruning) para los detalles de comportamiento.
...
- luego mantiene como canónico el workspace remoto SSH
...
- `mirror`: inicializa remoto desde local antes de `exec`, sincroniza de vuelta después de `exec`; el workspace local sigue siendo canónico
- `remote`: inicializa remoto una vez cuando se crea el sandbox y luego mantiene como canónico el workspace remoto
...
El transporte es SSH hacia el sandbox OpenShell, pero el Plugin es propietario del ciclo de vida del sandbox y de la sincronización mirror opcional.
...
**Los archivos adjuntos entrantes** se almacenan temporalmente en `media/inbound/*` dentro del workspace activo.
...
- `sandbox.browser.binds` monta directorios adicionales del host solo en el contenedor del navegador sandbox. Cuando está definido (incluido `[]`), reemplaza `docker.binds` para el contenedor del navegador.
- Los valores predeterminados de lanzamiento están definidos en `scripts/sandbox-browser-entrypoint.sh` y ajustados para hosts con contenedores:
  ...
  - `OPENCLAW_BROWSER_DISABLE_EXTENSIONS=0` vuelve a habilitar extensiones si tu flujo
    de trabajo depende de ellas.
    ...
    list: [
      {
        ...
        params: { cacheRetention: "none" }, // reemplaza por clave los params coincidentes de defaults.models
        skills: ["docs-search"], // reemplaza agents.defaults.skills cuando está definido
        identity: {
          name: "Samantha",
          theme: "helpful sloth",
          emoji: "🦥",
...
- `default`: cuando hay varios definidos, el primero tiene prioridad (se registra una advertencia). Si no hay ninguno definido, la primera entrada de la lista es la predeterminada.
- `model`: la forma de cadena reemplaza solo `primary`; la forma de objeto `{ primary, fallbacks }` reemplaza ambos (`[]` desactiva las reservas globales). Los trabajos Cron que solo reemplazan `primary` siguen heredando las reservas predeterminadas salvo que establezcas `fallbacks: []`.
- `params`: params de stream por agente combinados sobre la entrada del modelo seleccionado en `agents.defaults.models`. Úsalo para anulaciones específicas del agente como `cacheRetention`, `temperature` o `maxTokens` sin duplicar todo el catálogo de modelos.
...
- `thinkingDefault`: nivel predeterminado opcional de thinking por agente (`off | minimal | low | medium | high | xhigh | adaptive | max`). Reemplaza `agents.defaults.thinkingDefault` para este agente cuando no hay una anulación por mensaje o por sesión.
- `reasoningDefault`: visibilidad predeterminada opcional del razonamiento por agente (`on | off | stream`). Se aplica cuando no hay una anulación de razonamiento por mensaje o por sesión.
- `fastModeDefault`: valor predeterminado opcional por agente para modo rápido (`true | false`). Se aplica cuando no hay una anulación de modo rápido por mensaje o por sesión.
- `embeddedHarness`: anulación opcional por agente de la política de harness de bajo nivel. Usa `{ runtime: "codex", fallback: "none" }` para hacer que un agente sea solo Codex mientras otros agentes mantienen la reserva predeterminada PI.
...
- Protección de herencia del sandbox: si la sesión solicitante está en sandbox, `sessions_spawn` rechaza destinos que se ejecutarían fuera del sandbox.
...
### Campos `match` de asociaciones

- `type` (opcional): `route` para enrutamiento normal (si falta, el valor predeterminado es route), `acp` para asociaciones persistentes de conversaciones ACP.
...
Para entradas `type: "acp"`, OpenClaw resuelve por identidad exacta de conversación (`match.channel` + cuenta + `match.peer.id`) y no usa el orden de niveles de asociaciones route anterior.
...
```json5
{
  session: {
    ...
    parentForkMaxTokens: 100000, // omite la bifurcación del hilo padre por encima de este recuento de tokens (0 desactiva)
    maintenance: {
    ...
    threadBindings: {
      enabled: true,
      idleHours: 24, // pérdida automática de foco predeterminada por inactividad en horas (`0` desactiva)
...
- **`reset`**: política principal de reinicio. `daily` reinicia a `atHour` hora local; `idle` reinicia después de `idleMinutes`. Cuando ambos están configurados, gana el que expire primero.
...
- **`threadBindings`**: valores predeterminados globales para funciones de sesión vinculadas a hilos.
  - `enabled`: interruptor predeterminado maestro (los proveedores pueden reemplazarlo; Discord usa `channels.discord.threadBindings.enabled`)
  - `idleHours`: pérdida automática de foco predeterminada por inactividad en horas (`0` desactiva; los proveedores pueden reemplazarlo)
  - `maxAgeHours`: antigüedad máxima estricta predeterminada en horas (`0` desactiva; los proveedores pueden reemplazarlo)
...
Resolución (la más específica gana): cuenta → canal → global. `""` desactiva y detiene la cascada. `"auto"` deriva `[{identity.name}]`.
...
- Usa por defecto `identity.emoji` del agente activo; si no, `"👀"`. Establece `""` para desactivar.
...
- `messages.statusReactions.enabled`: habilita reacciones de estado del ciclo de vida en Slack, Discord y Telegram.
  En Slack y Discord, si no está definido, las reacciones de estado permanecen habilitadas cuando las reacciones de acuse están activas.
  En Telegram, establécelo explícitamente en `true` para habilitar reacciones de estado del ciclo de vida.

### Debounce entrante

Agrupa mensajes rápidos de solo texto del mismo remitente en un único turno del agente. Los medios/archivos adjuntos vacían inmediatamente. Los comandos de control omiten el debounce.
...
- `openai.baseUrl` reemplaza el endpoint TTS de OpenAI. El orden de resolución es configuración, luego `OPENAI_TTS_BASE_URL`, luego `https://api.openai.com/v1`.
...
- La reserva `ELEVENLABS_API_KEY` solo se aplica cuando no hay ninguna clave API de Talk configurada.
- `providers.*.voiceAliases` permite que las directivas de Talk usen nombres descriptivos.
- `silenceTimeoutMs` controla cuánto tiempo espera el modo Talk después del silencio del usuario antes de enviar la transcripción. Si no está definido, se mantiene la ventana de pausa predeterminada de la plataforma (`700 ms en macOS y Android, 900 ms en iOS`).
...
La incorporación local usa `tools.profile: "coding"` de forma predeterminada en configuraciones locales nuevas cuando no está definido (los perfiles explícitos existentes se conservan).
...
La configuración puede definirse globalmente en `tools.loopDetection` y reemplazarse por agente en `agents.list[].tools.loopDetection`.
...
```json5
{
  tools: {
    media: {
      concurrency: 2,
      asyncCompletion: {
        directSend: false, // activación explícita: enviar música/vídeo asíncronos completados directamente al canal
      },
      audio: {
        enabled: true,
        maxBytes: 20971520,
...
- Los archivos adjuntos solo se admiten para `runtime: "subagent"`. El entorno de ejecución ACP los rechaza.
...
- La limpieza sigue la política `cleanup`: `delete` siempre elimina los archivos adjuntos; `keep` solo los conserva cuando `retainOnSessionKeep: true`.

### `tools.experimental`

Indicadores experimentales de herramientas integradas. Predeterminado desactivado salvo que se aplique una regla estricta de autoactivación agentic de GPT-5.
...
- `allowAgents`: lista de permitidos predeterminada de ids de agente de destino para `sessions_spawn` cuando el agente solicitante no define su propio `subagents.allowAgents` (`["*"]` = cualquiera; predeterminado: solo el mismo agente).
...
- Precedencia de combinación para ids de proveedor coincidentes:
  ...
  - Los `contextTokens` de modelos coincidentes conservan un límite explícito del entorno de ejecución cuando existe; úsalo para limitar el contexto efectivo sin cambiar los metadatos nativos del modelo.
...
- `models.providers.*.request`: anulaciones de transporte para solicitudes HTTP del proveedor de modelos.
  ...
  - `request.allowPrivateNetwork`: cuando es `true`, permite HTTPS a `baseUrl` cuando el DNS resuelve a rangos privados, CGNAT o similares, mediante la protección SSRF de fetch HTTP del proveedor (activación explícita del operador para endpoints OpenAI compatibles autohospedados y de confianza). WebSocket usa el mismo `request` para encabezados/TLS, pero no esa protección SSRF de fetch. Predeterminado `false`.
...
- `models.providers.*.models.*.contextTokens`: límite opcional del contexto del entorno de ejecución. Úsalo cuando quieras un presupuesto efectivo de contexto menor que el `contextWindow` nativo del modelo.
...
- `models.providers.*.models.*.compat.requiresStringContent`: pista opcional de compatibilidad para endpoints de chat OpenAI compatibles que solo aceptan cadenas. Cuando es `true`, OpenClaw aplana los arrays de `messages[].content` de texto puro a cadenas simples antes de enviar la solicitud.
...
- Para el endpoint general, define un proveedor personalizado con la anulación de `baseUrl`.
...
`openai-completions`, y OpenClaw lo determina según las capacidades del endpoint
en lugar de hacerlo solo por el id del proveedor integrado.
...
La base URL debe omitir `/v1` (el cliente de Anthropic la añade). Atajo: `openclaw onboard --auth-choice synthetic-api-key`.
...
El catálogo de modelos usa M2.7 solo de forma predeterminada.
En la ruta de streaming compatible con Anthropic, OpenClaw desactiva el thinking de MiniMax
de forma predeterminada salvo que establezcas explícitamente `thinking` tú mismo. `/fast on` o
...
Consulta [Local Models](/es/gateway/local-models). Resumen: ejecuta un modelo local grande mediante la API Responses de LM Studio en hardware potente; mantén modelos alojados combinados como reserva.
...
- `allowBundled`: lista de permitidos opcional solo para Skills integradas (las Skills gestionadas/del workspace no se ven afectadas).
...
- `install.nodeManager`: preferencia del gestor Node para especificaciones
...
- El descubrimiento acepta plugins nativos de OpenClaw además de paquetes Codex y paquetes Claude compatibles, incluidos paquetes Claude sin manifiesto con diseño predeterminado.
...
- `plugins.entries.memory-core.config.dreaming`: ajustes de dreaming de memory. Consulta [Dreaming](/es/concepts/dreaming) para fases y umbrales.
  - `enabled`: interruptor maestro de dreaming (predeterminado `false`).
  - `frequency`: cadencia de Cron para cada barrido completo de dreaming (predeterminado `"0 3 * * *"`).
...
- Los plugins de paquetes Claude habilitados también pueden aportar valores predeterminados embebidos de Pi desde `settings.json`; OpenClaw los aplica como ajustes saneados del agente, no como parches en bruto de configuración de OpenClaw.
...
- `plugins.installs`: metadatos de instalación gestionados por CLI usados por `openclaw plugins update`.
  ...
  - Trata `plugins.installs.*` como estado gestionado; prefiere comandos CLI frente a ediciones manuales.
...
```json5
{
  browser: {
    ...
    defaultProfile: "user",
    ssrfPolicy: {
      // dangerouslyAllowPrivateNetwork: true, // activación explícita solo para acceso confiable a redes privadas
...
- En modo estricto, los endpoints de perfiles CDP remotos (`profiles.*.cdpUrl`) están sujetos al mismo bloqueo de red privada durante las comprobaciones de disponibilidad/descubrimiento.
...
- Los perfiles remotos son solo de adjuntar (inicio/parada/reinicio desactivados).
...
- Los perfiles `existing-session` pueden definir `userDataDir` para apuntar a un perfil
  concreto de navegador basado en Chromium como Brave o Edge.
...
- `extraArgs` añade flags adicionales de lanzamiento al inicio local de Chromium (por ejemplo
...
- `seamColor`: color de acento para la chrome de la UI de la app nativa (tinte de burbuja de Talk Mode, etc.).
...
```json5
{
  gateway: {
    ...
    controlUi: {
      ...
      // allowExternalEmbedUrls: false, // peligroso: permite URLs absolutas externas http(s) embebidas
      // allowedOrigins: ["https://control.example.com"], // obligatorio para Control UI fuera de loopback
      // dangerouslyAllowHostHeaderOriginFallback: false, // modo peligroso de reserva de origen Host-header
...
- `mode`: `local` (ejecutar Gateway) o `remote` (conectarse a Gateway remoto). El Gateway se niega a iniciarse salvo que sea `local`.
...
- Si tanto `gateway.auth.token` como `gateway.auth.password` están configurados (incluidos SecretRefs), establece `gateway.auth.mode` explícitamente en `token` o `password`. Los flujos de inicio e instalación/reparación del servicio fallan cuando ambos están configurados y `mode` no está definido.
- `gateway.auth.mode: "none"`: modo explícito sin autenticación. Úsalo solo para configuraciones locales confiables de loopback; intencionadamente no se ofrece en las indicaciones de incorporación.
- `gateway.auth.mode: "trusted-proxy"`: delega la autenticación a un proxy inverso con reconocimiento de identidad y confía en los encabezados de identidad de `gateway.trustedProxies` (consulta [Trusted Proxy Auth](/es/gateway/trusted-proxy-auth)). Este modo espera un origen de proxy **que no sea loopback**; los proxies inversos loopback del mismo host no cumplen la autenticación trusted-proxy.
- `gateway.auth.allowTailscale`: cuando es `true`, los encabezados de identidad de Tailscale Serve pueden satisfacer la autenticación de Control UI/WebSocket (verificado mediante `tailscale whois`). Los endpoints de la API HTTP **no** usan esa autenticación por encabezado de Tailscale; siguen el modo de autenticación HTTP normal del Gateway. Este flujo sin token asume que el host del Gateway es de confianza. Usa `true` de forma predeterminada cuando `tailscale.mode = "serve"`.
- `gateway.auth.rateLimit`: limitador opcional de autenticación fallida. Se aplica por IP del cliente y por ámbito de autenticación (secreto compartido y token de dispositivo se rastrean de forma independiente). Los intentos bloqueados devuelven `429` + `Retry-After`.
  - En la ruta asíncrona de Tailscale Serve Control UI, los intentos fallidos para el mismo `{scope, clientIp}` se serializan antes de escribir el fallo. Los intentos incorrectos concurrentes del mismo cliente pueden por tanto disparar el limitador en la segunda solicitud en lugar de dejar pasar ambas como simples discrepancias.
  - `gateway.auth.rateLimit.exemptLoopback` usa `true` de forma predeterminada; establece `false` cuando quieras intencionalmente que el tráfico localhost también tenga limitación de frecuencia (para configuraciones de prueba o despliegues estrictos con proxy).
- Los intentos de autenticación WS con origen de navegador siempre se limitan con la exención de loopback desactivada (defensa en profundidad contra fuerza bruta basada en navegador en localhost).
- En loopback, esos bloqueos de origen de navegador se aíslan por valor
  `Origin` normalizado, por lo que fallos repetidos desde un origen localhost no
  bloquean automáticamente otro origen distinto.
- `tailscale.mode`: `serve` (solo tailnet, bind loopback) o `funnel` (público, requiere autenticación).
- `controlUi.allowedOrigins`: lista explícita de permitidos de origen de navegador para conexiones WebSocket del Gateway. Obligatoria cuando se esperan clientes de navegador desde orígenes que no son loopback.
- `controlUi.dangerouslyAllowHostHeaderOriginFallback`: modo peligroso que habilita la reserva de origen basada en Host-header para despliegues que dependen intencionadamente de la política de origen basada en Host-header.
- `remote.transport`: `ssh` (predeterminado) o `direct` (ws/wss). Para `direct`, `remote.url` debe ser `ws://` o `wss://`.
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`: anulación de emergencia del lado del cliente que permite `ws://` en texto plano a IPs confiables de red privada; el valor predeterminado sigue siendo solo loopback para texto plano.
...
- `gateway.push.apns.relay.baseUrl`: base HTTPS URL del relay externo APNs usado por las compilaciones oficiales/TestFlight de iOS después de publicar registros respaldados por relay en el Gateway. Esta URL debe coincidir con la URL del relay compilada en la build de iOS.
- `gateway.push.apns.relay.timeoutMs`: tiempo de espera en milisegundos para el envío Gateway-a-relay. Predeterminado: `10000`.
- Los registros respaldados por relay se delegan a una identidad específica del Gateway. La app iOS emparejada recupera `gateway.identity.get`, incluye esa identidad en el registro del relay y reenvía una concesión de envío acotada al registro al Gateway. Otro Gateway no puede reutilizar ese registro almacenado.
...
- `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true`: escape solo para desarrollo para URLs loopback HTTP del relay. Las URLs de relay de producción deben seguir usando HTTPS.
...
- `gateway.channelMaxRestartsPerHour`: máximo de reinicios por monitor de salud por canal/cuenta en una hora móvil. Predeterminado: `10`.
...
- `channels.<provider>.accounts.<accountId>.healthMonitor.enabled`: anulación por cuenta para canales multicuenta. Cuando está definida, tiene prioridad sobre la anulación a nivel de canal.
- Las rutas locales de llamadas al Gateway pueden usar `gateway.remote.*` como reserva solo cuando `gateway.auth.*` no está definido.
- Si `gateway.auth.token` / `gateway.auth.password` está configurado explícitamente mediante SecretRef y no se resuelve, la resolución falla en modo cerrado (sin reserva remota que lo enmascare).
- `trustedProxies`: IPs de proxies inversos que terminan TLS o inyectan encabezados de cliente reenviado. Lista solo proxies que controles. Las entradas loopback siguen siendo válidas para configuraciones de detección local/proxy del mismo host (por ejemplo Tailscale Serve o un proxy inverso local), pero **no** hacen que las solicitudes loopback puedan usar `gateway.auth.mode: "trusted-proxy"`.
- `allowRealIpFallback`: cuando es `true`, el Gateway acepta `X-Real-IP` si falta `X-Forwarded-For`. Predeterminado `false` para comportamiento de bloqueo por defecto.
...
- Refuerzo de entradas URL de Responses:
  ...
  - `gateway.http.endpoints.responses.images.urlAllowlist`
    ...
    y/o `gateway.http.endpoints.responses.images.allowUrl=false` para desactivar la recuperación por URL.
- Encabezado opcional de refuerzo de respuestas:
  - `gateway.http.securityHeaders.strictTransportSecurity` (defínelo solo para orígenes HTTPS que controles; consulta [Trusted Proxy Auth](/es/gateway/trusted-proxy-auth#tls-termination-and-hsts))
...
- Si una asignación o preset usa un `sessionKey` con plantillas, establece `hooks.allowedSessionKeyPrefixes` y `hooks.allowRequestSessionKey=true`. Las claves estáticas de asignación no requieren esa activación explícita.
...
- Las plantillas como `{{messages[0].subject}}` leen desde la carga.
...
- `allowRequestSessionKey`: permite que los llamadores de `/hooks/agent` y las claves `sessionKey` de asignación controladas por plantillas establezcan `sessionKey` (predeterminado: `false`).
- `allowedSessionKeyPrefixes`: lista de permitidos opcional de prefijos para valores explícitos de `sessionKey` (solicitud + asignación), p. ej. `["hook:"]`. Se vuelve obligatoria cuando cualquier asignación o preset usa un `sessionKey` con plantillas.
- `deliver: true` envía la respuesta final a un canal; `channel` usa `last` de forma predeterminada.
- `model` reemplaza el LLM para esta ejecución del hook (debe estar permitido si el catálogo de modelos está definido).
...
- Si necesitas `hooks.allowRequestSessionKey: false`, reemplaza el preset con una `sessionKey` estática en lugar de la predeterminada con plantillas.
...
- Las URLs de capacidad están vinculadas a la sesión WS activa del nodo y caducan rápidamente. No se usa reserva basada en IP.
...
Escribe una zona DNS-SD de unidifusión en `~/.openclaw/dns/`. Para descubrimiento entre redes, combínalo con un servidor DNS (se recomienda CoreDNS) + DNS dividido de Tailscale.
...
- `shellEnv`: importa claves esperadas faltantes desde el perfil de tu shell de login.
...
Las refs de secretos son aditivas: los valores de texto plano siguen funcionando.
...
- Las refs de `auth-profiles.json` están incluidas en la resolución en tiempo de ejecución y en la cobertura de auditoría.
...
- Si se configura `trustedDirs`, la comprobación de directorio de confianza se aplica a la ruta del destino resuelto.
- El entorno hijo de `exec` es mínimo de forma predeterminada; pasa las variables requeridas explícitamente con `passEnv`.
- Las refs de secretos se resuelven en el momento de la activación en una instantánea en memoria; luego las rutas de solicitud solo leen esa instantánea.
...
- `auth-profiles.json` admite refs a nivel de valor (`keyRef` para `api_key`, `tokenRef` para `token`) para modos estáticos de credenciales.
- Los perfiles en modo OAuth (`auth.profiles.<id>.mode = "oauth"`) no admiten credenciales de auth-profile respaldadas por SecretRef.
- Las credenciales estáticas del entorno de ejecución provienen de instantáneas resueltas en memoria; las entradas heredadas estáticas de `auth.json` se depuran cuando se descubren.
...
- `billingBackoffHours`: backoff base en horas cuando un perfil falla por errores reales de
  ...
  seguir cayendo aquí incluso en respuestas `401`/`403`, pero los comparadores de texto específicos
  por proveedor siguen acotados al proveedor que los posee (por ejemplo OpenRouter
  `Key limit exceeded`). Los mensajes reintentables `402` de ventana de uso o
  de límite de gasto de organización/workspace siguen yendo por la ruta `rate_limit`
  en su lugar.
...
- `overloadedProfileRotations`: máximo de rotaciones del auth-profile del mismo proveedor por errores de sobrecarga antes de cambiar a la reserva de modelo (predeterminado: `1`). Formas de proveedor ocupado como `ModelNotReadyException` entran aquí.
- `overloadedBackoffMs`: retraso fijo antes de reintentar una rotación de proveedor/perfil sobrecargado (predeterminado: `0`).
- `rateLimitedProfileRotations`: máximo de rotaciones del auth-profile del mismo proveedor por errores de limitación de frecuencia antes de cambiar a la reserva de modelo (predeterminado: `1`). Ese bucket de rate-limit incluye texto con forma de proveedor como `Too many concurrent requests`, `ThrottlingException`, `concurrency limit reached`, `workers_ai ... quota limit exceeded` y `resource exhausted`.
...
- `maxFileBytes`: tamaño máximo del archivo de log en bytes antes de que se supriman escrituras (entero positivo; predeterminado: `524288000` = 500 MB). Usa rotación externa de logs para despliegues de producción.
...
- `flags`: array de cadenas de indicadores que habilita salida de log dirigida (admite comodines como `"telegram.*"` o `"*"`).
...
- `cacheTrace.enabled`: registra instantáneas de rastreo de caché para ejecuciones embebidas (predeterminado: `false`).
...
- `auto.stableJitterHours`: ventana adicional en horas para repartir el despliegue del canal estable (predeterminado: `12`; máximo: `168`).
...
- `backend`: id predeterminado del backend del entorno de ejecución ACP (debe coincidir con un Plugin de entorno ACP registrado).
...
- Para ocultar todo el banner (no solo los lemas), establece la variable de entorno `OPENCLAW_HIDE_BANNER=1`.
...
Las compilaciones actuales ya no incluyen el bridge TCP. Los nodos se conectan mediante el WebSocket del Gateway. Las claves `bridge.*` ya no forman parte del esquema de configuración (la validación falla hasta que se eliminen; `openclaw doctor --fix` puede quitar claves desconocidas).
...
```json5
{
  cron: {
    enabled: true,
    maxConcurrentRuns: 2,
    webhook: "https://example.invalid/legacy", // reserva desaprobada para trabajos almacenados con notify:true
...
- `sessionRetention`: cuánto tiempo conservar sesiones completadas de ejecuciones Cron aisladas antes de podarlas de `sessions.json`. También controla la limpieza de transcripciones archivadas eliminadas de Cron. Predeterminado: `24h`; establece `false` para desactivar.
...
- `runLog.keepLines`: líneas más nuevas que se conservan cuando se activa la poda del log de ejecución. Predeterminado: `2000`.
...
- `webhook`: URL de webhook heredada y desaprobada (http/https) usada solo para trabajos almacenados que aún tienen `notify: true`.
...
- Cuando no se define un destino de fallo global ni por trabajo, los trabajos que ya entregan mediante `announce` recurren a ese destino principal announce al fallar.
...
Consulta [Cron Jobs](/es/automation/cron-jobs). Las ejecuciones aisladas de Cron se rastrean como [background tasks](/es/automation/tasks).
...
- Array de archivos: combinación profunda en orden (lo posterior reemplaza lo anterior).
- Claves hermanas: se combinan después de las inclusiones (reemplazan valores incluidos).
