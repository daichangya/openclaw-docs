...
qué mensajes incluir, cómo resumir el historial antiguo y cómo gestionar el
contexto a través de los límites de subagentes.

OpenClaw incluye un motor `legacy` integrado y lo usa de forma predeterminada; la mayoría de los
usuarios nunca necesitan cambiarlo. Instala y selecciona un motor de Plugin solo cuando
quieras un ensamblaje, Compaction o comportamiento de recuerdo entre sesiones diferente.
...
1. **Ingest**: se llama cuando se añade un nuevo mensaje a la sesión. El motor
...
4. **After turn**: se llama después de que finaliza una ejecución. El motor puede conservar el estado,
   activar Compaction en segundo plano o actualizar índices.

Para el arnés Codex no ACP incluido, OpenClaw aplica el mismo ciclo de vida proyectando el
contexto ensamblado en instrucciones de desarrollador de Codex y en el prompt del turno
actual. Codex sigue siendo responsable de su historial de hilos nativo y de su compactador nativo.

### Ciclo de vida de subagentes (opcional)

OpenClaw llama a dos hooks opcionales del ciclo de vida de subagentes:

- **prepareSubagentSpawn**: prepara el estado de contexto compartido antes de que comience
  una ejecución hija. El hook recibe claves de sesión padre/hija, `contextMode`
  (`isolated` o `fork`), ids/archivos de transcripción disponibles y TTL opcional.
  Si devuelve un controlador de rollback, OpenClaw lo llama cuando la creación falla después
...
la antepone al prompt del sistema de la ejecución. Esto permite que los motores inyecten
guías dinámicas de recuerdo, instrucciones de recuperación o sugerencias
...
- **Ingest**: no-op (el gestor de sesiones se encarga directamente de conservar los mensajes).
- **Assemble**: passthrough (la canalización existente sanitize → validate → limit
  en el runtime se encarga del ensamblaje del contexto).
- **Compact**: delega en la Compaction integrada basada en resumen, que crea
  un único resumen de los mensajes antiguos y mantiene intactos los mensajes recientes.
- **After turn**: no-op.
...
| ----------------- | -------- | -------------------------------------------------------- |
| `info`            | Propiedad | Id del motor, nombre, versión y si controla la Compaction |
...
| `afterTurn(params)`         | Método | Trabajo de ciclo de vida posterior a la ejecución (conservar estado, activar Compaction en segundo plano).       |
...
`ownsCompaction` controla si la auto-Compaction integrada en el intento de Pi permanece
habilitada para la ejecución:

- `true`: el motor controla el comportamiento de Compaction. OpenClaw desactiva la
  auto-Compaction integrada de Pi para esa ejecución, y la implementación `compact()` del motor
  es responsable de `/compact`, la Compaction de recuperación por desbordamiento y cualquier
  Compaction proactiva que quiera hacer en `afterTurn()`. OpenClaw aún puede ejecutar la
  protección previa al prompt frente a desbordamiento; cuando predice que la transcripción completa
  desbordará, la ruta de recuperación llama a `compact()` del motor activo antes de
  enviar otro prompt.
...
- **Modo propietario**: implementa tu propio algoritmo de Compaction y establece
  `ownsCompaction: true`.
- **Modo delegado**: establece `ownsCompaction: false` y haz que `compact()` llame
...
desactiva la ruta normal de Compaction de `/compact` y recuperación por desbordamiento para ese
slot de motor.
...
para una ejecución o una operación de Compaction dada. Otros
Plugins `kind: "context-engine"` habilitados aún pueden cargarse y ejecutar su código de registro;
`plugins.slots.contextEngine` solo selecciona qué id de motor registrado
...
- **Los Plugins de memoria** (`plugins.slots.memory`) son independientes de los motores de contexto.
  Los Plugins de memoria proporcionan búsqueda/recuperación; los motores de contexto controlan lo que
  ve el modelo. Pueden trabajar juntos: un motor de contexto podría usar datos del
  Plugin de memoria durante el ensamblaje. Los motores de Plugin que quieran la ruta activa del
  prompt de memoria deberían preferir `buildMemorySystemPromptAddition(...)` de
  ...
  de más bajo nivel, aún puede extraer líneas sin procesar de
  `openclaw/plugin-sdk/memory-host-core` mediante
  `buildActiveMemoryPromptSection(...)`.
- **La poda de sesiones** (recortar resultados antiguos de herramientas en memoria) sigue ejecutándose
...
- Los errores del motor se registran y se muestran en diagnósticos. Si un motor de Plugin
  no se registra o no se puede resolver el id de motor seleccionado, OpenClaw
  no vuelve automáticamente al valor anterior; las ejecuciones fallan hasta que corrijas el Plugin o
...
- [Contexto](/es/concepts/context): cómo se construye el contexto para turnos de agente
