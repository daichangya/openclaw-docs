...
| `code_execution`                           | Ejecutar análisis remoto aislado en Python                            | [Code Execution](/es/tools/code-execution)                      |
| `browser`                                  | Controlar un navegador Chromium (navegar, hacer clic, captura)        | [Browser](/es/tools/browser)                                    |
...
| `canvas`                                   | Controlar node Canvas (present, eval, snapshot)                       |                                                              |
| `nodes`                                    | Descubrir y dirigirse a dispositivos emparejados                      |                                                              |
| `cron` / `gateway`                         | Gestionar tareas programadas; inspeccionar, parchear, reiniciar o actualizar el gateway |                                                              |
...
| `session_status`                           | Lectura ligera estilo `/status` y override del modelo de sesión       | [Herramientas de sesión](/es/concepts/session-tool)             |
...
opcionalmente establecer un override de modelo por sesión; `model=default` limpia ese
override. Igual que `/status`, puede rellenar contadores dispersos de tokens/caché y la
etiqueta activa del modelo en tiempo de ejecución a partir de la última entrada de uso de la transcripción.
...
- [Lobster](/es/tools/lobster) — tiempo de ejecución de flujos de trabajo tipado con aprobaciones reanudables
...
- [OpenProse](/es/prose) — orquestación de flujos de trabajo orientada primero a markdown
...
registra `query_db`. Si ninguna herramienta integrada, de plugin o MCP empaquetada coincide con
la lista de permitidos, la ejecución se detiene antes de la llamada al modelo en lugar de continuar como
una ejecución solo de texto que podría alucinar resultados de herramientas.
...
etiquetas de thinking, andamiaje `<relevant-memories>`, payloads XML de llamadas a herramientas en texto plano
...
y XML malformado de llamadas a herramientas de MiniMax del texto del asistente; después aplica
