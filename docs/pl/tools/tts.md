...
x-i18n:
    generated_at: "2026-04-23T10:10:25Z"
    model: gpt-5.4
    provider: openai
...
- **ElevenLabs** (provider główny lub awaryjny)
- **Google Gemini** (provider główny lub awaryjny; używa Gemini API TTS)
- **Microsoft** (provider główny lub awaryjny; obecna dołączona implementacja używa `node-edge-tts`)
- **MiniMax** (provider główny lub awaryjny; używa API T2A v2)
- **OpenAI** (provider główny lub awaryjny; używany także do podsumowań)
- **xAI** (provider główny lub awaryjny; używa xAI TTS API)

### Uwagi o mowie Microsoft

Dołączony provider mowy Microsoft obecnie używa internetowej
...
Ponieważ ta ścieżka to publiczna usługa webowa bez opublikowanego SLA ani limitów,
...
Jeśli skonfigurowano wielu providerów, najpierw używany jest wybrany provider, a pozostali są opcjami awaryjnymi.
Auto-summary używa skonfigurowanego `summaryModel` (lub `agents.defaults.model.primary`),
więc ten provider także musi być uwierzytelniony, jeśli włączysz podsumowania.
...
providera mowy według kolejności auto-select rejestru.
...
### OpenAI jako główny z awaryjnym ElevenLabs
...
ograniczony do Gemini API jest tutaj prawidłowy i jest to ten sam typ klucza,
którego używa dołączony provider generowania obrazów Google. Kolejność rozwiązywania to
...
xAI TTS używa tej samej ścieżki `XAI_API_KEY` co dołączony provider modeli Grok.
Kolejność rozwiązywania to `messages.tts.providers.xai.apiKey` -> `XAI_API_KEY`.
Obecne aktywne głosy to `ara`, `eve`, `leo`, `rex`, `sal` i `una`; `eve` jest
domyślne. `language` akceptuje znacznik BCP-47 lub `auto`.
...
- `provider`: id providera mowy, takie jak `"elevenlabs"`, `"google"`, `"microsoft"`, `"minimax"` lub `"openai"` (fallback jest automatyczny).
- Jeśli `provider` **nie jest ustawione**, OpenClaw używa pierwszego skonfigurowanego providera mowy według kolejności auto-select rejestru.
- Starsze `provider: "edge"` nadal działa i jest normalizowane do `microsoft`.
- `summaryModel`: opcjonalny tani model do auto-summary; domyślnie `agents.defaults.model.primary`.
...
- `modelOverrides`: pozwala modelowi emitować dyrektywy TTS (domyślnie włączone).
  - `allowProvider` domyślnie ma wartość `false` (przełączanie providera wymaga opt-in).
- `providers.<id>`: ustawienia należące do providera, kluczowane przez id providera mowy.
- Starsze bezpośrednie bloki providerów (`messages.tts.openai`, `messages.tts.elevenlabs`, `messages.tts.microsoft`, `messages.tts.edge`) są automatycznie migrowane do `messages.tts.providers.<id>` przy wczytywaniu.
...
- Wartości `apiKey` przechodzą awaryjnie do zmiennych env (`ELEVENLABS_API_KEY` / `XI_API_KEY`, `GEMINI_API_KEY` / `GOOGLE_API_KEY`, `MINIMAX_API_KEY`, `OPENAI_API_KEY`).
...
- `providers.google.baseUrl`: nadpisuje bazowy URL Gemini API. Akceptowane jest tylko `https://generativelanguage.googleapis.com`.
  - Jeśli pominięto `messages.tts.providers.google.apiKey`, TTS może ponownie użyć `models.providers.google.apiKey` przed awaryjnym przejściem do env.
...
- `providers.xai.voiceId`: id głosu xAI (domyślnie `eve`; obecne aktywne głosy: `ara`, `eve`, `leo`, `rex`, `sal`, `una`).
...
- `providers.xai.speed`: natywne nadpisanie prędkości providera.
...
dostarczyć ekspresyjne tagi (śmiech, wskazówki śpiewu itd.), które powinny pojawić się
tylko w audio.
...
- `provider` (id zarejestrowanego providera mowy, na przykład `openai`, `elevenlabs`, `google`, `minimax` lub `microsoft`; wymaga `allowProvider: true`)
- `voice` (głos OpenAI), `voiceName` / `voice_name` / `google_voice` (głos Google) lub `voiceId` (ElevenLabs / MiniMax / xAI)
- `model` (model OpenAI TTS, id modelu ElevenLabs lub model MiniMax) lub `google_model` (model Google TTS)
...
Opcjonalna lista dozwolonych (włącza przełączanie providerów przy zachowaniu konfigurowalności innych parametrów):
...
Slash commands zapisują lokalne nadpisania do `prefsPath` (domyślnie:
...
- **MiniMax**: MP3 (model `speech-2.8-hd`, częstotliwość próbkowania 32 kHz). Format notatki głosowej nie jest natywnie obsługiwany; użyj OpenAI lub ElevenLabs dla gwarantowanych wiadomości głosowych Opus.
...
- **xAI**: domyślnie MP3; `responseFormat` może mieć wartość `mp3`, `wav`, `pcm`, `mulaw` lub `alaw`. OpenClaw używa batch REST TTS endpoint xAI i zwraca kompletny załącznik audio; strumieniowy TTS WebSocket xAI nie jest używany przez tę ścieżkę providera. Natywny format notatki głosowej Opus nie jest obsługiwany przez tę ścieżkę.
- **Microsoft**: używa `microsoft.outputFormat` (domyślnie `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - Jeśli skonfigurowany format wyjściowy Microsoft zakończy się niepowodzeniem, OpenClaw ponawia próbę z MP3.
...
## Użycie slash commands

Istnieje jedno polecenie: `/tts`.
Szczegóły włączania znajdziesz w [Slash commands](/pl/tools/slash-commands).

Uwaga dla Discord: `/tts` to wbudowane polecenie Discord, więc OpenClaw rejestruje tam
...
- Używaj konfiguracji, gdy chcesz domyślnych wartości `inbound` lub `tagged`.
...
- Błędy API OpenAI i ElevenLabs zawierają teraz sparsowane szczegóły błędu providera oraz request id (gdy provider je zwróci), co jest ujawniane w błędach / logach TTS.
