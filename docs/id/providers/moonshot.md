---
read_when:
    - Anda menginginkan konfigurasi Moonshot K2 (Moonshot Open Platform) vs Kimi Coding
    - Anda perlu memahami endpoint, key, dan referensi model yang terpisah
    - Anda menginginkan konfigurasi copy/paste untuk salah satu provider
summary: Konfigurasikan Moonshot K2 vs Kimi Coding (provider + key terpisah)
title: Moonshot AI
x-i18n:
    generated_at: "2026-04-21T09:23:25Z"
    model: gpt-5.4
    provider: openai
...
<Warning>
Moonshot dan Kimi Coding adalah **provider terpisah**. Key tidak dapat dipertukarkan, endpoint berbeda, dan referensi model berbeda (`moonshot/...` vs `kimi/...`).
</Warning>
...
Estimasi biaya bawaan untuk model K2 yang saat ini di-host oleh Moonshot menggunakan
...
menggunakan placeholder biaya nol kecuali Anda meng-override-nya di konfigurasi.

## Memulai

Pilih provider Anda dan ikuti langkah konfigurasinya.
...
<Tabs>
    ...
    **Terbaik untuk:** model Kimi K2 melalui Moonshot Open Platform.
    ...
    <Steps>
      ...
      <Step title="Jalankan smoke test live">
        Gunakan state dir terisolasi ketika Anda ingin memverifikasi akses model dan pelacakan biaya
        ...
        yang dinormalisasi plus estimasi biaya di bawah `usage.cost` saat Moonshot mengembalikan
        metadata penggunaan.
      </Step>
    </Steps>
  ...
  <Tab title="Kimi Coding">
    **Terbaik untuk:** tugas yang berfokus pada kode melalui endpoint Kimi Coding.
    ...
    <Note>
    Kimi Coding menggunakan API key dan prefiks provider yang berbeda (`kimi/...`) dibanding Moonshot (`moonshot/...`). Referensi model legacy `kimi/k2p5` tetap diterima sebagai ID kompatibilitas.
    </Note>
...
<AccordionGroup>
  <Accordion title="Mode thinking native">
    ...
    <Warning>
    Saat thinking Moonshot diaktifkan, `tool_choice` harus `auto` atau `none`. OpenClaw menormalkan nilai `tool_choice` yang tidak kompatibel ke `auto` demi kompatibilitas.
    </Warning>

    Kimi K2.6 juga menerima field opsional `thinking.keep` yang mengontrol
    retensi multi-giliran dari `reasoning_content`. Setel ke `"all"` untuk menyimpan
    reasoning lengkap lintas giliran; hilangkan saja (atau biarkan `null`) untuk menggunakan
    strategi default server. OpenClaw hanya meneruskan `thinking.keep` untuk
  ...
  <Accordion title="Kompatibilitas penggunaan streaming">
    ...
    transport bersama `openai-completions`. OpenClaw mengaitkannya ke kapabilitas endpoint,
    ...
    dan cache-read juga dikonversi menjadi estimasi biaya USD lokal untuk `/status`, `/usage full`, `/usage cost`, dan akuntansi sesi berbasis transkrip.

  </Accordion>

  <Accordion title="Referensi endpoint dan model ref">
    | Provider      | Prefiks model ref | Endpoint                    | Variabel env auth   |
    | ------------- | ----------------- | --------------------------- | ------------------- |
...
<CardGroup cols={2}>
  ...
  <Card title="Moonshot Open Platform" href="https://platform.moonshot.ai" icon="globe">
    Manajemen API key dan dokumentasi Moonshot.
  </Card>
</CardGroup>
