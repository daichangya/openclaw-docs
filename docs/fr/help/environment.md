...
read_when:
    ...
    - Vous documentez l’authentification du fournisseur ou les environnements de déploiement
summary: où OpenClaw charge les variables d’environnement et l’ordre de priorité
title: variables d’environnement
x-i18n:
    generated_at: "2026-04-24T07:13:56Z"
    model: gpt-5.4
    provider: openai
...
OpenClaw récupère les variables d’environnement depuis plusieurs sources. La règle est **ne jamais remplacer les valeurs existantes**.
...
2. **`.env` dans le répertoire de travail courant** (comportement par défaut de dotenv ; ne remplace pas).
3. **`.env` global** dans `~/.openclaw/.env` (alias `$OPENCLAW_STATE_DIR/.env` ; ne remplace pas).
...
Sur les installations Ubuntu fraîches qui utilisent le répertoire d’état par défaut, OpenClaw traite aussi `~/.config/openclaw/gateway.env` comme un repli de compatibilité après le `.env` global. Si les deux fichiers existent et divergent, OpenClaw conserve `~/.openclaw/.env` et affiche un avertissement.
...
Vous pouvez référencer directement les variables d’environnement dans les valeurs de chaîne de configuration avec la syntaxe `${VAR_NAME}` :
...
## Références de secret vs chaînes `${ENV}`
...
Les deux se résolvent depuis l’environnement du processus au moment de l’activation. Les détails SecretRef sont documentés dans [Gestion des secrets](/fr/gateway/secrets).

## Variables d’environnement liées aux chemins

| Variable               | Objectif                                                                                                                                                                          |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OPENCLAW_HOME`        | Redéfinit le répertoire personnel utilisé pour toute la résolution de chemins internes (`~/.openclaw/`, répertoires d’agent, sessions, identifiants). Utile lors de l’exécution d’OpenClaw sous un utilisateur de service dédié. |
| `OPENCLAW_STATE_DIR`   | Redéfinit le répertoire d’état (par défaut `~/.openclaw`).                                                                                                                       |
| `OPENCLAW_CONFIG_PATH` | Redéfinit le chemin du fichier de configuration (par défaut `~/.openclaw/openclaw.json`).                                                                                        |

## Journalisation

| Variable             | Objectif                                                                                                                                                                                      |
| -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OPENCLAW_LOG_LEVEL` | Redéfinit le niveau de journalisation pour le fichier et la console (par ex. `debug`, `trace`). Prend le pas sur `logging.level` et `logging.consoleLevel` dans la configuration. Les valeurs invalides sont ignorées avec un avertissement. |

### `OPENCLAW_HOME`

Lorsqu’elle est définie, `OPENCLAW_HOME` remplace le répertoire personnel système (`$HOME` / `os.homedir()`) pour toute la résolution de chemins internes. Cela permet une isolation complète du système de fichiers pour les comptes de service headless.
...
le magasin CA fourni avec nvm, qui peut ne pas contenir des autorités racines modernes (ISRG Root X1/X2 pour Let’s Encrypt,
...
**Correctif manuel (pour les versions plus anciennes ou les lancements directs `node ...`) :**
...
- [FAQ : variables d’environnement et chargement de .env](/fr/help/faq#env-vars-and-env-loading)
