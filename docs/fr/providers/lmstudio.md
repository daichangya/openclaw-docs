...
title: LM Studio
x-i18n:
    generated_at: "2026-04-24T07:27:29Z"
    model: gpt-5.4
    provider: openai
...
LM Studio est une application conviviale mais puissante pour exécuter des modèles à poids ouverts sur votre propre matériel. Elle permet d’exécuter des modèles llama.cpp (GGUF) ou MLX (Apple Silicon). Elle existe en application GUI ou en démon headless (`llmster`). Pour la documentation produit et de configuration, voir [lmstudio.ai](https://lmstudio.ai/).
...
Assurez-vous soit de lancer l’application desktop, soit d’exécuter le démon avec la commande suivante :
...
Pour les détails de configuration de l’authentification LM Studio, voir [Authentification LM Studio](https://lmstudio.ai/docs/developer/core/authentication).
...
Les clés de modèle LM Studio suivent le format `author/model-name` (par ex. `qwen/qwen3.5-9b`). Les références de modèle OpenClaw
...
Utilisez l’onboarding non interactif lorsque vous souhaitez automatiser la configuration (CI, provisioning, bootstrap distant) :
...
`--custom-model-id` prend la clé du modèle telle que renvoyée par LM Studio (par ex. `qwen/qwen3.5-9b`), sans
le préfixe fournisseur `lmstudio/`.
...
### Compatibilité du streaming d’usage

LM Studio est compatible avec le streaming d’usage. Lorsqu’il n’émet pas un objet
`usage` de forme OpenAI, OpenClaw récupère à la place les comptes de jetons à partir des métadonnées de style llama.cpp
...
LM Studio prend en charge le chargement de modèle juste-à-temps (JIT), où les modèles sont chargés à la première requête. Assurez-vous que cela est activé afin d’éviter les erreurs « Model not loaded ».
