...
x-i18n:
    generated_at: "2026-04-24T07:27:48Z"
    model: gpt-5.4
    provider: openai
...
OpenClaw peut utiliser de nombreux fournisseurs LLM. Choisissez-en un, authentifiez-vous, puis définissez le modèle par défaut sous la forme `provider/model`.
...
- `anthropic-vertex` - prise en charge implicite d’Anthropic sur Google Vertex lorsque les identifiants Vertex sont disponibles ; pas de choix d’authentification distinct dans l’onboarding
- `copilot-proxy` - pont local VS Code Copilot Proxy ; utilisez `openclaw onboard --auth-choice copilot-proxy`
- `google-gemini-cli` - flux OAuth non officiel Gemini CLI ; exige une installation locale de `gemini` (`brew install gemini-cli` ou `npm install -g @google/gemini-cli`) ; modèle par défaut `google-gemini-cli/gemini-3-flash-preview` ; utilisez `openclaw onboard --auth-choice google-gemini-cli` ou `openclaw models auth login --provider google-gemini-cli --set-default`
