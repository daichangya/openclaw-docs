...
x-i18n:
    generated_at: "2026-04-24T07:09:38Z"
    model: gpt-5.4
    provider: openai
...
Cette page couvre les principales surfaces de configuration d’OpenClaw et renvoie vers d’autres pages lorsqu’un sous-système possède sa propre référence plus détaillée. Elle ne cherche **pas** à intégrer sur une seule page chaque catalogue de commandes propre aux canaux/plugins ni tous les réglages avancés de memory/QMD.

Source de vérité du code :

- `openclaw config schema` affiche le schéma JSON en direct utilisé pour la validation et l’interface Control UI, avec les métadonnées des plugins/canaux intégrés et des plugins fusionnées lorsque disponibles
- `config.schema.lookup` renvoie un nœud de schéma limité à un chemin pour les outils d’exploration détaillée
- `pnpm config:docs:check` / `pnpm config:docs:gen` valident le hash de référence config-doc par rapport à la surface actuelle du schéma

Références dédiées approfondies :

- [Référence de configuration de memory](/fr/reference/memory-config) pour `agents.defaults.memorySearch.*`, `memory.qmd.*`, `memory.citations` et la configuration Dreaming sous `plugins.entries.memory-core.config.dreaming`
- [Commandes slash](/fr/tools/slash-commands) pour le catalogue actuel des commandes intégrées et des commandes intégrées via Plugin
...
canaux intégrés (authentification, contrôle d’accès, multi-comptes, contrôle par mention).
...
- `agents.defaults.*` (espace de travail, modèle, réflexion, Heartbeat, memory, médias, Skills, sandbox)
- `multiAgent.*` (routage multi-agent et liaisons)
- `session.*` (cycle de vie des sessions, Compaction, élagage)
- `messages.*` (remise des messages, synthèse vocale, rendu Markdown)
- `talk.*` (mode Talk)
...
La politique des outils, les bascules expérimentales, la configuration des outils adossés aux fournisseurs et la configuration des fournisseurs personnalisés / `baseUrl` ont été déplacées vers une page dédiée — voir
...
- `allowBundled` : liste d’autorisation facultative pour les Skills intégrées uniquement (les Skills gérées/de l’espace de travail ne sont pas affectées).
- `load.extraDirs` : racines supplémentaires partagées pour les Skills (priorité la plus basse).
- `install.preferBrew` : lorsque cette valeur est `true`, privilégie les installateurs Homebrew lorsque `brew` est
...
- `install.nodeManager` : préférence de gestionnaire Node pour les spécifications `metadata.openclaw.install`
  (`npm` | `pnpm` | `yarn` | `bun`).
- `entries.<skillKey>.enabled: false` désactive une Skill même si elle est intégrée/installée.
- `entries.<skillKey>.apiKey` : champ pratique pour les Skills déclarant une variable env principale (chaîne en texte brut ou objet SecretRef).
...
- La découverte accepte les plugins OpenClaw natifs ainsi que les bundles Codex compatibles et les bundles Claude compatibles, y compris les bundles Claude sans manifeste avec disposition par défaut.
...
- `plugins.entries.<id>.hooks.allowPromptInjection` : lorsque `false`, le noyau bloque `before_prompt_build` et ignore les champs modifiant l’invite issus de l’ancien `before_agent_start`, tout en conservant l’ancien `modelOverride` et `providerOverride`. S’applique aux hooks de plugin natifs et aux répertoires de hooks fournis par bundle pris en charge.
...
- `plugins.entries.firecrawl.config.webFetch` : paramètres du fournisseur de récupération web Firecrawl.
  - `apiKey` : clé API Firecrawl (accepte SecretRef). Repli vers `plugins.entries.firecrawl.config.webSearch.apiKey`, l’ancien `tools.web.fetch.firecrawl.apiKey`, ou la variable d’environnement `FIRECRAWL_API_KEY`.
...
- `plugins.entries.memory-core.config.dreaming` : paramètres Dreaming de memory. Voir [Dreaming](/fr/concepts/dreaming) pour les phases et les seuils.
...
- La configuration complète de memory se trouve dans [Référence de configuration de memory](/fr/reference/memory-config) :
  - `agents.defaults.memorySearch.*`
  - `memory.backend`
  - `memory.citations`
  - `memory.qmd.*`
  - `plugins.entries.memory-core.config.dreaming`
- Les plugins bundle Claude activés peuvent aussi apporter des valeurs par défaut Pi embarquées depuis `settings.json` ; OpenClaw les applique comme paramètres d’agent nettoyés, pas comme patches bruts de configuration OpenClaw.
- `plugins.slots.memory` : choisir l’ID du plugin memory actif, ou `"none"` pour désactiver les plugins memory.
- `plugins.slots.contextEngine` : choisir l’ID du plugin de moteur de contexte actif ; par défaut vaut `"legacy"` sauf si vous installez et sélectionnez un autre moteur.
...
```json5
{
  browser: {
    ...
    defaultProfile: "user",
    ssrfPolicy: {
      // dangerouslyAllowPrivateNetwork: true, // n’activer que pour l’accès volontaire à un réseau privé de confiance
...
- Définissez `ssrfPolicy.dangerouslyAllowPrivateNetwork: true` uniquement lorsque vous faites volontairement confiance à la navigation browser sur réseau privé.
- En mode strict, les endpoints de profil CDP distants (`profiles.*.cdpUrl`) sont soumis au même blocage de réseau privé pendant les vérifications de disponibilité/découverte.
...
- Les profils distants sont en mode attach-only (démarrage/arrêt/réinitialisation désactivés).
...
- Les profils `existing-session` conservent les limites actuelles de routage Chrome MCP :
  actions basées sur snapshot/ref au lieu du ciblage par sélecteur CSS, hooks de téléversement d’un seul fichier, pas de remplacements de délai pour les boîtes de dialogue, pas de `wait --load networkidle`, et pas de `responsebody`, export PDF, interception de téléchargement ou actions par lot.
...
- `extraArgs` ajoute des indicateurs supplémentaires au démarrage local de Chromium (par exemple
...
- `seamColor` : couleur d’accent pour le chrome de l’interface native (teinte de bulle du mode Talk, etc.).
- `assistant` : remplacement d’identité de l’interface Control UI. Repli vers l’identité de l’agent actif.
...
- **Authentification** : requise par défaut. Les liaisons non-loopback nécessitent une authentification gateway. En pratique, cela signifie un jeton/mot de passe partagé ou un proxy inverse tenant compte de l’identité avec `gateway.auth.mode: "trusted-proxy"`. L’assistant d’initialisation génère un jeton par défaut.
...
- `gateway.auth.mode: "none"` : mode sans authentification explicite. À utiliser uniquement pour des configurations loopback locales de confiance ; ce mode n’est volontairement pas proposé par les invites d’initialisation.
- `gateway.auth.mode: "trusted-proxy"` : déléguer l’authentification à un proxy inverse tenant compte de l’identité et faire confiance aux en-têtes d’identité de `gateway.trustedProxies` (voir [Authentification Trusted Proxy](/fr/gateway/trusted-proxy-auth)). Ce mode attend une source de proxy **non-loopback** ; les proxys inverses loopback sur le même hôte ne satisfont pas l’authentification trusted-proxy.
- `gateway.auth.allowTailscale` : lorsque `true`, les en-têtes d’identité Tailscale Serve peuvent satisfaire l’authentification de Control UI/WebSocket (vérifiée via `tailscale whois`). Les endpoints d’API HTTP n’utilisent **pas** cette authentification par en-tête Tailscale ; ils suivent à la place le mode d’authentification HTTP normal du gateway. Ce flux sans jeton suppose que l’hôte du gateway est de confiance. La valeur par défaut est `true` lorsque `tailscale.mode = "serve"`.
- `gateway.auth.rateLimit` : limite facultative des échecs d’authentification. S’applique par IP cliente et par périmètre d’authentification (secret partagé et jeton d’appareil sont suivis indépendamment). Les tentatives bloquées renvoient `429` + `Retry-After`.
  - Sur le chemin asynchrone Tailscale Serve Control UI, les tentatives échouées pour le même `{scope, clientIp}` sont sérialisées avant l’écriture de l’échec. Des tentatives invalides concurrentes depuis le même client peuvent donc déclencher la limitation sur la deuxième requête au lieu de passer toutes deux en parallèle comme de simples non-correspondances.
...
- Les tentatives d’authentification WS d’origine navigateur sont toujours limitées avec l’exemption loopback désactivée (défense en profondeur contre la force brute localhost depuis le navigateur).
...
- `controlUi.dangerouslyAllowHostHeaderOriginFallback` : mode dangereux qui active le repli d’origine via l’en-tête Host pour les déploiements qui reposent intentionnellement sur cette politique.
- `remote.transport` : `ssh` (par défaut) ou `direct` (ws/wss). Pour `direct`, `remote.url` doit être `ws://` ou `wss://`.
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1` : remplacement de secours, côté client et au niveau de l’environnement du processus,
  qui autorise `ws://` en clair vers des IP de réseau privé de confiance ; par défaut, le texte clair reste limité au loopback. Il n’existe pas d’équivalent dans `openclaw.json`,
  et la configuration de réseau privé du navigateur, telle que
  `browser.ssrfPolicy.dangerouslyAllowPrivateNetwork`, n’affecte pas les clients WebSocket Gateway.
- `gateway.remote.token` / `.password` sont des champs d’identifiants du client distant. Ils ne configurent pas l’authentification gateway à eux seuls.
...
- Les enregistrements adossés au relais sont délégués à une identité gateway spécifique. L’application iOS appairée récupère `gateway.identity.get`, inclut cette identité dans l’enregistrement du relais, et transfère au gateway un droit d’envoi limité à cet enregistrement. Un autre gateway ne peut pas réutiliser cet enregistrement stocké.
...
- `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true` : échappatoire réservée au développement pour des URL de relais HTTP en loopback. Les URL de relais de production doivent rester en HTTPS.
- `gateway.channelHealthCheckMinutes` : intervalle, en minutes, du moniteur de santé des canaux. Définissez `0` pour désactiver globalement les redémarrages du moniteur de santé. Par défaut : `5`.
- `gateway.channelStaleEventThresholdMinutes` : seuil, en minutes, des sockets obsolètes. Gardez cette valeur supérieure ou égale à `gateway.channelHealthCheckMinutes`. Par défaut : `30`.
...
- `channels.<provider>.accounts.<accountId>.healthMonitor.enabled` : remplacement par compte pour les canaux multi-comptes. Lorsqu’il est défini, il prévaut sur le remplacement au niveau du canal.
- Les chemins d’appel du gateway local peuvent utiliser `gateway.remote.*` en repli uniquement lorsque `gateway.auth.*` n’est pas défini.
...
- En-tête facultatif de renforcement des réponses :
  - `gateway.http.securityHeaders.strictTransportSecurity` (définissez-le uniquement pour des origines HTTPS que vous contrôlez ; voir [Authentification Trusted Proxy](/fr/gateway/trusted-proxy-auth#tls-termination-and-hsts))
...
- `autoGenerate` : génère automatiquement une paire certificat/clé auto-signée locale lorsqu’aucun fichier explicite n’est configuré ; à utiliser uniquement en local/dev.
...
- `caPath` : chemin facultatif vers un bundle d’AC pour la vérification client ou des chaînes de confiance personnalisées.
...
- `mode` : contrôle la façon dont les modifications de configuration sont appliquées à l’exécution.
  ...
  - `"hybrid"` (par défaut) : essayer d’abord le hot reload ; revenir au redémarrage si nécessaire.
- `debounceMs` : fenêtre de debounce en ms avant l’application des modifications de configuration (entier non négatif).
- `deferralTimeoutMs` : temps maximal en ms à attendre les opérations en cours avant de forcer un redémarrage (par défaut : `300000` = 5 minutes).
...
- Si un mapping ou un preset utilise un `sessionKey` basé sur un template, définissez `hooks.allowedSessionKeyPrefixes` et `hooks.allowRequestSessionKey=true`. Les clés de mapping statiques ne nécessitent pas cette activation explicite.

**Endpoints :**

- `POST /hooks/wake` → `{ text, mode?: "now"|"next-heartbeat" }`
- `POST /hooks/agent` → `{ message, name?, agentId?, sessionKey?, wakeMode?, deliver?, channel?, to?, model?, thinking?, timeoutSeconds? }`
  - Le `sessionKey` issu de la charge utile de la requête est accepté uniquement lorsque `hooks.allowRequestSessionKey=true` (par défaut : `false`).
- `POST /hooks/<name>` → résolu via `hooks.mappings`
  - Les valeurs `sessionKey` de mapping rendues via template sont traitées comme fournies de l’extérieur et exigent aussi `hooks.allowRequestSessionKey=true`.
...
- `allowedSessionKeyPrefixes` : liste d’autorisation facultative de préfixes pour les valeurs explicites de `sessionKey` (requête + mapping), par ex. `["hook:"]`. Elle devient requise lorsqu’un mapping ou preset utilise un `sessionKey` basé sur un template.
...
- Si vous avez besoin de `hooks.allowRequestSessionKey: false`, remplacez le preset avec un `sessionKey` statique au lieu de la valeur par défaut basée sur template.
...
- Les WebViews de nœud n’envoient généralement pas d’en-têtes d’authentification ; après appairage et connexion d’un nœud, le Gateway annonce des URL de capacité limitées au nœud pour l’accès Canvas/A2UI.
...
- les ID `source: "exec"` ne doivent pas contenir de segments de chemin `.` ou `..` séparés par des slashs (par exemple `a/../b` est refusé)
...
- Le fournisseur `file` prend en charge `mode: "json"` et `mode: "singleValue"` (dans le mode singleValue, `id` doit valoir `"value"`).
- Les chemins des fournisseurs file et exec échouent de manière fermée lorsque la vérification ACL Windows n’est pas disponible. Définissez `allowInsecurePath: true` uniquement pour des chemins de confiance qui ne peuvent pas être vérifiés.
- Le fournisseur `exec` exige un chemin `command` absolu et utilise des charges utiles de protocole sur stdin/stdout.
- Par défaut, les chemins de commande symlink sont refusés. Définissez `allowSymlinkCommand: true` pour autoriser les chemins symlink tout en validant le chemin cible résolu.
...
- Le filtrage de surface active s’applique pendant l’activation : les références non résolues sur les surfaces activées font échouer le démarrage/rechargement, tandis que les surfaces inactives sont ignorées avec diagnostics.
...
- Les identifiants statiques à l’exécution proviennent de snapshots résolus en mémoire ; les anciennes entrées statiques `auth.json` sont nettoyées lorsqu’elles sont découvertes.
...
- `billingBackoffHours` : délai de repli de base, en heures, lorsqu’un profil échoue à cause de véritables erreurs de facturation/crédit insuffisant (par défaut : `5`). Un texte explicite de facturation peut
  toujours aboutir ici même sur des réponses `401`/`403`, mais les matchers de texte spécifiques aux fournisseurs restent limités au fournisseur auquel ils appartiennent (par exemple OpenRouter
  `Key limit exceeded`). Les messages réessayables d’usage en fenêtre `402` ou de limite de dépense d’organisation/espace de travail restent dans le chemin `rate_limit`.
- `billingBackoffHoursByProvider` : remplacements facultatifs, par fournisseur, du délai de repli de facturation en heures.
- `billingMaxHours` : plafond, en heures, de la croissance exponentielle du délai de repli de facturation (par défaut : `24`).
...
- `authPermanentMaxMinutes` : plafond, en minutes, de la croissance du délai de repli `auth_permanent` (par défaut : `60`).
- `failureWindowHours` : fenêtre glissante, en heures, utilisée pour les compteurs de délai de repli (par défaut : `24`).
- `overloadedProfileRotations` : nombre maximal de rotations de profil d’authentification sur le même fournisseur pour les erreurs de surcharge avant de passer au basculement de modèle (par défaut : `1`). Les formes de fournisseur occupé comme `ModelNotReadyException` aboutissent ici.
- `overloadedBackoffMs` : délai fixe avant une nouvelle tentative de rotation de fournisseur/profil surchargé (par défaut : `0`).
- `rateLimitedProfileRotations` : nombre maximal de rotations de profil d’authentification sur le même fournisseur pour les erreurs de limitation de débit avant de passer au basculement de modèle (par défaut : `1`). Ce compartiment de limitation de débit inclut des textes propres aux fournisseurs comme `Too many concurrent requests`, `ThrottlingException`, `concurrency limit reached`, `workers_ai ... quota limit exceeded` et `resource exhausted`.
...
- `maxFileBytes` : taille maximale du fichier journal en octets avant que les écritures ne soient supprimées (entier positif ; par défaut : `524288000` = 500 MB). Utilisez une rotation de journaux externe pour les déploiements en production.
...
- `otel.traces` / `otel.metrics` / `otel.logs` : activer l’export de traces, métriques ou journaux.
...
- `cacheTrace.enabled` : journaliser des snapshots de trace de cache pour les exécutions intégrées (par défaut : `false`).
...
- `auto.stableJitterHours` : fenêtre supplémentaire, en heures, d’étalement du déploiement du canal stable (par défaut : `12` ; max : `168`).
...
- `enabled` : bascule globale de fonctionnalité ACP (par défaut : `false`).
- `dispatch.enabled` : bascule indépendante pour la distribution des tours de session ACP (par défaut : `true`). Définissez `false` pour garder les commandes ACP disponibles tout en bloquant l’exécution.
- `backend` : ID du backend runtime ACP par défaut (doit correspondre à un Plugin de runtime ACP enregistré).
- `defaultAgent` : ID d’agent cible de secours ACP lorsque les lancements ne spécifient pas de cible explicite.
- `allowedAgents` : liste d’autorisation des ID d’agent permis pour les sessions runtime ACP ; vide signifie aucune restriction supplémentaire.
- `maxConcurrentSessions` : nombre maximal de sessions ACP actives simultanément.
- `stream.coalesceIdleMs` : fenêtre de vidage au repos, en ms, pour le texte diffusé en streaming.
- `stream.maxChunkChars` : taille maximale de segment avant division de la projection de bloc diffusé.
- `stream.repeatSuppression` : supprimer les lignes répétées d’état/d’outil par tour (par défaut : `true`).
...
```json5
{
  cron: {
    enabled: true,
    maxConcurrentRuns: 2,
    webhook: "https://example.invalid/legacy", // repli obsolète pour les jobs stockés notify:true
...
- `sessionRetention` : durée de conservation des sessions isolées terminées d’exécution Cron avant leur suppression de `sessions.json`. Contrôle aussi le nettoyage des transcriptions Cron supprimées archivées. Par défaut : `24h` ; définissez `false` pour désactiver.
...
- `webhookToken` : jeton bearer utilisé pour la remise POST du webhook Cron (`delivery.mode = "webhook"`), si omis aucun en-tête d’authentification n’est envoyé.
- `webhook` : URL webhook de repli héritée obsolète (http/https) utilisée uniquement pour les jobs stockés qui ont encore `notify: true`.
...
- `maxAttempts` : nombre maximal de nouvelles tentatives pour les jobs ponctuels en cas d’erreurs transitoires (par défaut : `3` ; plage : `0`–`10`).
- `backoffMs` : tableau des délais de backoff en ms pour chaque tentative de nouvelle exécution (par défaut : `[30000, 60000, 300000]` ; 1 à 10 entrées).
...
S’applique uniquement aux jobs Cron ponctuels. Les jobs récurrents utilisent une gestion d’échec distincte.
...
- `enabled` : activer les alertes d’échec pour les jobs Cron (par défaut : `false`).
- `after` : nombre d’échecs consécutifs avant le déclenchement d’une alerte (entier positif, min : `1`).
- `cooldownMs` : nombre minimal de millisecondes entre des alertes répétées pour le même job (entier non négatif).
- `mode` : mode de remise — `"announce"` envoie via un message de canal ; `"webhook"` publie vers le webhook configuré.
- `accountId` : ID de compte ou de canal facultatif pour limiter la remise des alertes.
...
- Destination par défaut pour les notifications d’échec Cron sur tous les jobs.
- `mode` : `"announce"` ou `"webhook"` ; vaut `"announce"` par défaut lorsque suffisamment de données de cible existent.
...
- `accountId` : remplacement facultatif du compte pour la remise.
- `delivery.failureDestination` par job remplace cette valeur par défaut globale.
- Lorsqu’aucune destination d’échec globale ou par job n’est définie, les jobs qui remettent déjà via `announce` reviennent, en cas d’échec, à cette cible announce primaire.
- `delivery.failureDestination` n’est pris en charge que pour les jobs `sessionTarget="isolated"` sauf si le `delivery.mode` principal du job vaut `"webhook"`.
...
## Variables de template de modèle média
...
- Tableau de fichiers : fusion profonde dans l’ordre (les plus tardifs remplacent les plus anciens).
...
- Chemins : résolus par rapport au fichier incluant, mais doivent rester à l’intérieur du répertoire de configuration de niveau supérieur (`dirname` de `openclaw.json`). Les formes absolues/`../` ne sont autorisées que si elles se résolvent toujours à l’intérieur de cette limite.
- Les écritures gérées par OpenClaw qui ne modifient qu’une seule section de niveau supérieur adossée à un include de fichier unique sont écrites directement dans ce fichier inclus. Par exemple, `plugins install` met à jour `plugins: { $include: "./plugins.json5" }` dans `plugins.json5` et laisse `openclaw.json` intact.
- Les includes racine, les tableaux d’includes et les includes avec remplacements frères sont en lecture seule pour les écritures gérées par OpenClaw ; ces écritures échouent de manière fermée au lieu d’aplatir la configuration.
