...
summary: 'Aperçu de l’appairage : approuver qui peut vous envoyer des messages privés et quels Node peuvent rejoindre'
title: Appairage
x-i18n:
...
Lorsqu’un canal est configuré avec la politique de message privé `pairing`, les expéditeurs inconnus reçoivent un code court et leur message **n’est pas traité** tant que vous ne l’avez pas approuvé.
...
- Les comptes non par défaut lisent/écrivent uniquement leur propre fichier de liste d’autorisation.
- Le compte par défaut utilise le fichier de liste d’autorisation sans portée spécifique, au niveau du canal.

Traitez ces fichiers comme sensibles (ils contrôlent l’accès à votre assistant).

Important : ce stockage concerne l’accès en message privé. L’autorisation de groupe est distincte.
Approuver un code d’appairage de message privé n’autorise pas automatiquement cet expéditeur à exécuter des commandes de groupe ou à contrôler le bot dans des groupes. Pour l’accès aux groupes, configurez les listes d’autorisation explicites du canal pour les groupes (par exemple `groupAllowFrom`, `groups` ou des remplacements par groupe/par sujet selon le canal).

## 2) Appairage d’appareils Node (Node iOS/Android/macOS/sans interface)
...
3. Sur votre téléphone, ouvrez l’app OpenClaw iOS → Réglages → Gateway.
4. Collez le code de configuration et connectez-vous.
5. De retour dans Telegram : `/pair pending` (vérifiez les identifiants de demande, le rôle et les scopes), puis approuvez.
...
- `bootstrapToken` : un jeton bootstrap à durée de vie courte, valable pour un seul appareil, utilisé pour la poignée de main initiale d’appairage
...
- les vérifications de scope bootstrap sont préfixées par rôle, pas regroupées dans un seul pool de scopes plat :
...
se reconnecte en demandant davantage de scopes ou un rôle plus étendu, OpenClaw conserve
l’approbation existante telle quelle et crée une nouvelle demande de mise à niveau en attente. Utilisez
`openclaw devices list` pour comparer l’accès actuellement approuvé avec l’accès
nouvellement demandé avant d’approuver.
...
- L’API héritée `node.pair.*` (CLI : `openclaw nodes pending|approve|reject|rename`) est un
  stockage d’appairage distinct, géré par le gateway. Les Node WS nécessitent toujours l’appairage des appareils.
- L’enregistrement d’appairage est la source de vérité durable pour les rôles approuvés. Les
  jetons d’appareil actifs restent limités à cet ensemble de rôles approuvé ; une entrée de jeton isolée
