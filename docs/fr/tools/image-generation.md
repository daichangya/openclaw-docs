...
summary: Générez et modifiez des images à l’aide des fournisseurs configurés (OpenAI, OpenAI Codex OAuth, Google Gemini, OpenRouter, LiteLLM, fal, MiniMax, ComfyUI, Vydra, xAI)
...
L’outil `image_generate` permet à l’agent de créer et de modifier des images à l’aide de vos fournisseurs configurés. Les images générées sont remises automatiquement comme pièces jointes média dans la réponse de l’agent.

<Note>
L’outil n’apparaît que lorsqu’au moins un fournisseur de génération d’images est disponible. Si vous ne voyez pas `image_generate` dans les outils de votre agent, configurez `agents.defaults.imageGenerationModel`, définissez une clé API de fournisseur, ou connectez-vous avec OpenAI Codex OAuth.
</Note>
...
Une configuration d’image explicite personnalisée `models.providers.openai`, telle qu’une clé API ou
une URL de base personnalisée/Azure, réactive le routage direct vers l’API OpenAI Images.
Pour les points de terminaison LAN compatibles OpenAI tels que LocalAI, conservez
...
modification d’image de référence. Utilisez `image` pour une référence ou `images` pour plusieurs références.
...
| Fournisseur | Modèle par défaut                        | Prise en charge de modification     | Auth                                                  |
| ----------- | ---------------------------------------- | ----------------------------------- | ----------------------------------------------------- |
...
Plusieurs images de référence pour le mode modification (jusqu’à 5).
</ParamField>
...
Délai d’expiration facultatif de la requête fournisseur en millisecondes.
</ParamField>
...
Tous les fournisseurs ne prennent pas en charge tous les paramètres. Lorsqu’un fournisseur de repli prend en charge une option de géométrie proche au lieu de celle demandée exactement, OpenClaw remappe vers la taille, le ratio d’aspect ou la résolution prise en charge la plus proche avant soumission. Les indications de sortie non prises en charge telles que `quality` ou `outputFormat` sont supprimées pour les fournisseurs qui ne déclarent pas de prise en charge et sont signalées dans le résultat de l’outil.

Les résultats d’outil signalent les paramètres appliqués. Lorsque OpenClaw remappe la géométrie pendant le repli fournisseur, les valeurs renvoyées `size`, `aspectRatio` et `resolution` reflètent ce qui a réellement été envoyé, et `details.normalization` capture la traduction entre demande et application.
...
4. **Détection automatique** — utilise uniquement les valeurs par défaut des fournisseurs adossés à l’authentification :
   - fournisseur par défaut actuel en premier
   - fournisseurs restants enregistrés de génération d’images dans l’ordre des identifiants de fournisseur
...
- Un remplacement `model` par appel est exact : OpenClaw essaie uniquement ce fournisseur/modèle
  et ne continue pas vers le principal/repli configuré ni les fournisseurs
...
- Définissez `agents.defaults.imageGenerationModel.timeoutMs` pour les backends d’image lents.
...
OpenAI, OpenRouter, Google, fal, MiniMax, ComfyUI et xAI prennent en charge la modification d’images de référence. Passez un chemin ou une URL d’image de référence :
...
La génération d’images OpenRouter utilise la même clé `OPENROUTER_API_KEY` et passe par l’API d’images de chat completions d’OpenRouter. Sélectionnez les modèles d’image OpenRouter avec le préfixe `openrouter/` :
...
utilisé par les modèles de chat par abonnement Codex et envoie la requête d’image
...
`https://chatgpt.com/backend-api/codex` pour les requêtes d’image. Il ne revient pas
silencieusement à `OPENAI_API_KEY` pour cette requête. Pour forcer le routage direct vers l’API OpenAI
Images, configurez explicitement `models.providers.openai` avec une clé API,
...
de génération et de modification d’images doivent utiliser `gpt-image-2`.

`gpt-image-2` prend en charge à la fois la génération texte-vers-image et la
modification d’image de référence via le même outil `image_generate`. OpenClaw transmet `prompt`,
...
OpenClaw les convertit en une `size` prise en charge, sinon l’outil les signale comme
...
`openai.background` accepte `transparent`, `opaque` ou `auto` ; les sorties transparentes exigent `outputFormat` `png` ou `webp`. `openai.outputCompression` s’applique aux sorties JPEG/WebP.

Générez une image paysage 4K :

```
/tool image_generate action=generate model=openai/gpt-image-2 prompt="A clean editorial poster for OpenClaw image generation" size=3840x2160 count=1
```

Générez deux images carrées :

```
/tool image_generate action=generate model=openai/gpt-image-2 prompt="Two visual directions for a calm productivity app icon" size=1024x1024 count=2
```

Modifiez une image de référence locale :

```
/tool image_generate action=generate model=openai/gpt-image-2 prompt="Keep the subject, replace the background with a bright studio setup" image=/path/to/reference.png size=1024x1536
```

Modifiez avec plusieurs références :

```
/tool image_generate action=generate model=openai/gpt-image-2 prompt="Combine the character identity from the first image with the color palette from the second" images='["/path/to/character.png","/path/to/palette.jpg"]' size=1536x1024
```

Pour router la génération d’images OpenAI via un déploiement Azure OpenAI au lieu
de `api.openai.com`, voir [Points de terminaison Azure OpenAI](/fr/providers/openai#azure-openai-endpoints)
dans la documentation du fournisseur OpenAI.

La génération d’images MiniMax est disponible via les deux chemins d’authentification MiniMax groupés :
...
| Modification/référence | Oui (jusqu’à 5 images) | Oui (jusqu’à 5 images) | Oui (1 image)    | Oui (1 image, référence de sujet) | Oui (1 image, configurée par workflow) | Non     | Oui (jusqu’à 5 images) |
...
avec invite uniquement et `/v1/images/edits` lorsque `image` ou `images` est présent.
...
OpenClaw n’expose intentionnellement pas `quality`, `mask`, `user` natifs à xAI, ni
les ratios d’aspect supplémentaires disponibles uniquement en natif, tant que ces contrôles n’existent pas dans le contrat partagé
