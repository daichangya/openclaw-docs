...
read_when:
    ...
    - Configurer les fournisseurs et modèles de génération d’images
...
x-i18n:
    generated_at: "2026-04-23T14:02:44Z"
    model: gpt-5.4
    provider: openai
...
L’outil `image_generate` permet à l’agent de créer et modifier des images à l’aide de vos fournisseurs configurés. Les images générées sont automatiquement livrées comme pièces jointes média dans la réponse de l’agent.
...
Tous les fournisseurs ne prennent pas en charge tous les paramètres. Lorsqu’un fournisseur de repli prend en charge une option géométrique proche au lieu de la valeur exacte demandée, OpenClaw remappe vers la taille, le ratio d’aspect ou la résolution pris en charge les plus proches avant l’envoi. Les surcharges réellement non prises en charge sont quand même signalées dans le résultat de l’outil.

Les résultats de l’outil signalent les paramètres appliqués. Lorsque OpenClaw remappe la géométrie pendant un repli de fournisseur, les valeurs retournées `size`, `aspectRatio` et `resolution` reflètent ce qui a réellement été envoyé, et `details.normalization` capture la traduction entre valeur demandée et valeur appliquée.
...
4. **Détection automatique** — utilise uniquement les valeurs par défaut des fournisseurs adossées à l’authentification :
...
Si un fournisseur échoue (erreur d’authentification, limitation de débit, etc.), le candidat suivant est essayé automatiquement. Si tous échouent, l’erreur inclut les détails de chaque tentative.
...
- La détection automatique est activée par défaut. Définissez
  `agents.defaults.mediaGenerationAutoProviderFallback: false` si vous voulez que la génération
...
modèle `openai/gpt-image-1` peut encore être sélectionné explicitement, mais les nouvelles demandes OpenAI
...
l’édition d’images de référence via le même outil `image_generate`. OpenClaw transmet `prompt`,
`count`, `size`, et les images de référence à OpenAI. OpenAI ne reçoit pas directement
`aspectRatio` ni `resolution` ; lorsque c’est possible, OpenClaw les mappe vers une
`size` prise en charge, sinon l’outil les signale comme surcharges ignorées.
...
Pour faire passer la génération d’images OpenAI par un déploiement Azure OpenAI
au lieu de `api.openai.com`, consultez [Endpoints Azure OpenAI](/fr/providers/openai#azure-openai-endpoints)
...
avec prompt seul et `/v1/images/edits` lorsque `image` ou `images` est présent.
...
OpenClaw n’expose volontairement pas les options natives xAI `quality`, `mask`, `user`, ni
les ratios d’aspect supplémentaires natifs uniquement tant que ces contrôles n’existent pas dans le contrat
...
- [Vydra](/fr/providers/vydra) — configuration de Vydra pour image, vidéo et parole
- [xAI](/fr/providers/xai) — configuration de Grok pour image, vidéo, recherche, exécution de code et TTS
