...
read_when:
    ...
    - |-
      Vous devez configurer, autoriser ou refuser des outils♀♀♀#+#+#+#+assistant to=functions.read వ్యాఖ్యary  天天购彩票json
      {"path":"/home/runner/work/docs/docs/source/AGENTS.md","offset":1,"limit":20}
...
Les outils sont le moyen par lequel l’agent lit des fichiers, exécute des commandes, navigue sur le web, envoie
...
<Steps>
  <Step title="Les outils sont ce que l’agent appelle">
    ...
    L’agent voit les outils comme des définitions structurées de fonctions envoyées à l’API du modèle.
  ...
  <Step title="Les Skills apprennent à l’agent quand et comment">
    ...
    Les Skills donnent à l’agent du contexte, des contraintes et des guides étape par étape pour
  ...
  <Step title="Les plugins empaquettent tout ensemble">
    Un plugin est un package pouvant enregistrer n’importe quelle combinaison de capacités :
    canaux, fournisseurs de modèles, outils, Skills, parole, transcription temps réel,
    voix temps réel, compréhension des médias, génération d’images, génération vidéo,
    récupération web, recherche web, et plus encore. Certains plugins sont **noyau** (fournis avec
...
| `web_search` / `x_search` / `web_fetch`    | Rechercher sur le web, rechercher des publications X, récupérer du contenu de page | [Web](/fr/tools/web), [Web Fetch](/fr/tools/web-fetch)             |
| `read` / `write` / `edit`                  | E/S de fichiers dans l’espace de travail                             |                                                              |
| `apply_patch`                              | Patches de fichiers multi-hunks                                      | [Apply Patch](/fr/tools/apply-patch)                            |
| `message`                                  | Envoyer des messages sur tous les canaux                             | [Envoi par agent](/fr/tools/agent-send)                         |
| `canvas`                                   | Piloter le Canvas de nœud (présenter, eval, snapshot)                |                                                              |
| `nodes`                                    | Découvrir et cibler les appareils appairés                           |                                                              |
| `cron` / `gateway`                         | Gérer les tâches planifiées ; inspecter, patcher, redémarrer ou mettre à jour le gateway |                                                              |
...
| `tts`                                      | Conversion texte-parole en une fois                                  | [TTS](/fr/tools/tts)                                            |
| `sessions_*` / `subagents` / `agents_list` | Gestion de session, état et orchestration de sous-agents             | [Sous-agents](/fr/tools/subagents)                              |
| `session_status`                           | Relecture légère de type `/status` et remplacement du modèle par session | [Outils de session](/fr/concepts/session-tool)                |

Pour le travail sur les images, utilisez `image` pour l’analyse et `image_generate` pour la génération ou l’édition. Si vous ciblez `openai/*`, `google/*`, `fal/*` ou un autre fournisseur d’image non par défaut, configurez d’abord l’authentification / la clé API de ce fournisseur.
...
Il répond à des questions de type `/status` sur la session actuelle et peut
éventuellement définir un remplacement de modèle par session ; `model=default` efface ce
remplacement. Comme `/status`, il peut compléter des compteurs de tokens/cache partiels et le libellé
du modèle d’exécution actif depuis la dernière entrée d’utilisation de transcription.

`gateway` est l’outil runtime réservé au propriétaire pour les opérations gateway :

- `config.schema.lookup` pour un sous-arbre de configuration limité à un chemin avant modifications
- `config.get` pour le snapshot de configuration actuel + hash
...
- [LLM Task](/fr/tools/llm-task) — étape LLM JSON uniquement pour sortie structurée
- [Lobster](/fr/tools/lobster) — runtime de workflow typé avec approbations reprenables
- [Génération musicale](/fr/tools/music-generation) — outil partagé `music_generate` avec fournisseurs adossés à un workflow
...
configuration. Le refus est toujours prioritaire sur l’autorisation.
...
les balises de thinking, le squelette `<relevant-memories>`, les charges utiles XML d’appel d’outil en texte brut (y compris `<tool_call>...</tool_call>`,
`<function_call>...</function_call>`, `<tool_calls>...</tool_calls>`,
`<function_calls>...</function_calls>` et les blocs d’appel d’outil tronqués),
le squelette dégradé d’appel d’outil, les jetons de contrôle de modèle ASCII/full-width divulgués,
ainsi que le XML malformé d’appel d’outil MiniMax du texte assistant, puis applique
une expurgation/troncature et d’éventuels placeholders de ligne surdimensionnée au lieu d’agir
comme une simple extraction brute de transcription.

### Restrictions spécifiques au fournisseur

Utilisez `tools.byProvider` pour restreindre les outils pour certains fournisseurs sans
changer les valeurs globales par défaut :
