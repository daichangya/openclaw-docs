...
read_when:
    ...
    - Vous voulez un Gateway permanent de niveau production sur votre propre VM
...
title: GCP
x-i18n:
    generated_at: "2026-04-24T07:16:55Z"
    model: gpt-5.4
    provider: openai
...
Exécuter un Gateway OpenClaw persistant sur une VM GCP Compute Engine avec Docker, avec un état durable, des binaires intégrés à l’image et un comportement de redémarrage sûr.

Si vous voulez « OpenClaw 24/7 pour ~$5-12/mois », c’est une configuration fiable sur Google Cloud.
Le tarif varie selon le type de machine et la région ; choisissez la plus petite VM adaptée à votre charge et augmentez-la si vous rencontrez des OOM.
...
- Accéder à l’interface de contrôle depuis votre portable via un tunnel SSH

Cet état monté `~/.openclaw` inclut `openclaw.json`, le fichier par agent
...
6. Créer des répertoires hôte persistants
...
- Un compte GCP (éligible au free tier pour e2-micro)
- La CLI gcloud installée (ou utiliser la Cloud Console)
- Un accès SSH depuis votre portable
...
<Steps>
  ...
  <Step title="Créer un projet GCP">
    ...
    **Console :**
    ...
    4. Allez dans API et services > Activer des API > recherchez « Compute Engine API » > Activer
  ...
  <Step title="Créer la VM">
    **Types de machine :**
    ...
    | Type      | Caractéristiques          | Coût               | Notes                                           |
    | --------- | ------------------------- | ------------------ | ----------------------------------------------- |
    | e2-medium | 2 vCPU, 4GB RAM           | ~$25/mois          | Le plus fiable pour les builds Docker locaux    |
    | e2-small  | 2 vCPU, 2GB RAM           | ~$12/mois          | Minimum recommandé pour un build Docker         |
    | e2-micro  | 2 vCPU (partagés), 1GB RAM | Éligible free tier | Échoue souvent avec OOM lors du build Docker (exit 137) |
  ...
  <Step title="Configurer les variables d’environnement">
    ...
    le gérer via `.env` ; OpenClaw écrit un jeton gateway aléatoire dans
    ...
    **Ne validez pas ce fichier dans git.**

    Ce fichier `.env` est destiné à l’environnement conteneur/runtime, comme `OPENCLAW_GATEWAY_TOKEN`.
    L’authentification OAuth/clé API des fournisseurs stockée vit dans le fichier monté
  ...
  <Step title="Configuration Docker Compose">
    ...
    services:
      openclaw-gateway:
        ...
        ports:
          # Recommandé : garder le Gateway en loopback uniquement sur la VM ; accès via tunnel SSH.
    ...
    `--allow-unconfigured` n’est là que pour faciliter l’initialisation, ce n’est pas un remplacement d’une configuration gateway correcte. Définissez quand même l’authentification (`gateway.auth.token` ou mot de passe) et utilisez des paramètres de bind sûrs pour votre déploiement.
  ...
  <Step title="Notes de lancement spécifiques à GCP">
    Sur GCP, si le build échoue avec `Killed` ou `exit code 137` pendant `pnpm install --frozen-lockfile`, la VM manque de mémoire. Utilisez `e2-small` au minimum, ou `e2-medium` pour des premiers builds plus fiables.

    Lors d’un bind sur LAN (`OPENCLAW_GATEWAY_BIND=lan`), configurez une origine de navigateur de confiance avant de continuer :
  ...
  <Step title="Accès depuis votre portable">
    ...
    Réafficher un lien propre du tableau de bord :

    ```bash
    docker compose run --rm openclaw-cli dashboard --no-open
    ```

    Si l’interface demande une authentification par secret partagé, collez le jeton configuré ou
    le mot de passe dans les paramètres de l’interface de contrôle. Ce flux Docker écrit un jeton par
...
Assurez-vous que votre compte possède les autorisations IAM requises (Compute OS Login ou Compute OS Admin Login).
...
Voir [https://cloud.google.com/iam/docs/understanding-roles](https://cloud.google.com/iam/docs/understanding-roles) pour le détail des rôles IAM.
...
- Jumeler des appareils locaux comme nœuds : [Nodes](/fr/nodes)
