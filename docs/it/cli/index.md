...
`openclaw` è il punto di ingresso principale della CLI. Ogni comando core ha
una pagina di riferimento dedicata oppure è documentato insieme al comando di cui è alias; questo
...
| Scoperta e documentazione   | [`dns`](/it/cli/dns) · [`docs`](/it/cli/docs)                                                                                                                                                                                                      |
...
| `--container <name>`    | Seleziona un container con nome come destinazione per l'esecuzione                                |
...
- I collegamenti ipertestuali OSC-8 vengono visualizzati come link cliccabili dove supportato; altrimenti la
...
I messaggi in chat supportano i comandi `/...`. Vedi [slash commands](/it/tools/slash-commands).
...
di utilizzo del provider e vengono normalizzati in `X% left`. I provider con finestre
di utilizzo correnti sono: Anthropic, GitHub Copilot, Gemini CLI, OpenAI Codex, MiniMax,
Xiaomi e z.ai.

Vedi [Usage tracking](/it/concepts/usage-tracking) per i dettagli.

## Correlati

- [Slash commands](/it/tools/slash-commands)
- [Configuration](/it/gateway/configuration)
- [Environment](/it/help/environment)
