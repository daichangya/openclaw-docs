...
- Consigliato/testato: macOS Sequoia (15). macOS Tahoe (26) funziona; al momento la modifica è non funzionante su Tahoe e gli aggiornamenti dell'icona del gruppo possono segnalare esito positivo ma non sincronizzarsi.
- OpenClaw comunica con esso tramite la sua API REST (`GET /api/v1/ping`, `POST /message/text`, `POST /chat/:id/*`).
- I messaggi in entrata arrivano tramite Webhook; le risposte in uscita, gli indicatori di digitazione, le conferme di lettura e i tapback sono chiamate REST.
...
- L'associazione/lista di consentiti funziona allo stesso modo degli altri canali (`/channels/pairing` ecc.) con `channels.bluebubbles.allowFrom` + codici di associazione.
...
- Funzionalità avanzate: modifica, annullamento invio, thread di risposta, effetti messaggio, gestione gruppi.
...
- L'autenticazione del Webhook è sempre richiesta. OpenClaw rifiuta le richieste Webhook BlueBubbles a meno che non includano una password/guid che corrisponde a `channels.bluebubbles.password` (ad esempio `?password=<password>` o `x-password`), indipendentemente dalla topologia loopback/proxy.
...
In alcune configurazioni macOS VM / always-on, Messages.app può finire in stato “idle” (gli eventi in entrata si interrompono finché l'app non viene aperta/portata in primo piano). Una semplice soluzione alternativa è **sollecitare Messages ogni 5 minuti** usando un AppleScript + LaunchAgent.
...
- **Criterio DM**: associazione, lista consentiti, aperto o disabilitato
- **Lista consentiti**: numeri di telefono, email o destinazioni chat
...
I Webhook di gruppo BlueBubbles spesso includono solo indirizzi grezzi dei partecipanti. Se vuoi che il contesto `GroupMembers` mostri invece i nomi dei contatti locali, puoi attivare facoltativamente l'arricchimento dai Contatti locali su macOS:

- `channels.bluebubbles.enrichGroupParticipantsFromContacts = true` abilita la ricerca. Predefinito: `false`.
- Le ricerche vengono eseguite solo dopo che accesso al gruppo, autorizzazione dei comandi e controllo delle menzioni hanno consentito il passaggio del messaggio.
...
- I comandi di controllo da mittenti autorizzati aggirano il controllo delle menzioni.
...
La chiave corrisponde a qualunque valore BlueBubbles segnali come `chatGuid` / `chatIdentifier` / `chatId` numerico per il gruppo, e una voce jolly `"*"` fornisce un valore predefinito per ogni gruppo senza una corrispondenza esatta (lo stesso schema usato da `requireMention` e dai criteri degli strumenti per gruppo). Le corrispondenze esatte hanno sempre priorità sul carattere jolly. I DM ignorano questo campo; usa invece la personalizzazione del prompt a livello di agente o di account.

#### Esempio pratico: risposte in thread e reazioni tapback (API privata)

Con la API privata BlueBubbles abilitata, i messaggi in entrata arrivano con ID messaggio brevi (ad esempio `[[reply_to:5]]`) e l'agente può chiamare `action=reply` per inserirsi in thread sotto un messaggio specifico oppure `action=react` per aggiungere un tapback. Un `systemPrompt` per gruppo è un modo affidabile per far scegliere all'agente lo strumento corretto:
      ...
      groups: {
        "iMessage;+;chat-family": {
          systemPrompt: [
            ...
            "messageId [[reply_to:N]] dal contesto in modo che la tua risposta sia in thread",
            "sotto il messaggio che ha attivato l'azione. Non inviare mai un nuovo messaggio non collegato.",
            "",
            "Per brevi conferme ('ok', 'ricevuto', 'ci penso io'), usa",
            "action=react con un emoji tapback appropriato (❤️, 👍, 😂, ‼️, ❓)",
...
Le reazioni tapback e le risposte in thread richiedono entrambe la API privata BlueBubbles; vedi [Azioni avanzate](#advanced-actions) e [ID messaggio](#message-ids-short-vs-full) per i meccanismi sottostanti.

## Associazioni conversazione ACP

Le chat BlueBubbles possono essere trasformate in spazi di lavoro ACP persistenti senza modificare il livello di trasporto.
...
`match.peer.id` può usare qualunque formato di destinazione BlueBubbles supportato:
...
- **react**: aggiunge/rimuove reazioni tapback (`messageId`, `emoji`, `remove`). L'insieme nativo di tapback di iMessage è `love`, `like`, `dislike`, `laugh`, `emphasize` e `question`. Quando un agente sceglie un'emoji al di fuori di questo insieme (ad esempio `👀`), lo strumento di reazione ripiega su `love` in modo che il tapback venga comunque visualizzato invece di far fallire l'intera richiesta. Le reazioni di conferma configurate continuano invece a essere validate in modo rigoroso e restituiscono un errore per valori sconosciuti.
...
- Gli ID brevi sono in memoria; possono scadere dopo un riavvio o l'eliminazione della cache.
- Le azioni accettano `messageId` brevi o completi, ma gli ID brevi restituiranno un errore se non sono più disponibili.
...
I due Webhook arrivano a OpenClaw a circa 0,8-2,0 s di distanza nella maggior parte delle configurazioni. Senza coalescenza, l'agente riceve il solo comando al turno 1, risponde (spesso "inviami l'URL") e vede l'URL solo al turno 2 — quando il contesto del comando è già andato perso.

`channels.bluebubbles.coalesceSameSenderDms` consente a un DM di unire Webhook consecutivi dello stesso mittente in un unico turno agente. Le chat di gruppo continuano invece a essere indicizzate per messaggio, così viene preservata la struttura dei turni multiutente.
...
- hai bisogno della latenza minima dei comandi per trigger DM a una sola parola.
...
```json5
{
  messages: {
    inbound: {
      byChannel: {
        // 2500 ms funziona per la maggior parte delle configurazioni; alza a 4000 ms se il tuo Mac è lento
        // o sotto pressione di memoria (in tal caso il divario osservato può superare i 2 s).
        bluebubbles: 2500,
      },
    },
  },
...
- **Latenza aggiuntiva per i comandi di controllo nei DM.** Con il flag attivo, i messaggi di comando di controllo nei DM (come `Dump`, `Save`, ecc.) ora attendono fino alla finestra di debounce prima dell'invio, nel caso stia arrivando un Webhook con payload. I comandi nelle chat di gruppo continuano a essere inviati immediatamente.
- **L'output unito è limitato** — il testo unito è limitato a 4000 caratteri con un indicatore esplicito `…[truncated]`; gli allegati sono limitati a 20; le voci sorgente sono limitate a 10 (oltre questo limite vengono mantenuti il primo e il più recente). Ogni `messageId` sorgente raggiunge comunque l'inbound-dedupe, così un successivo replay MessagePoller di un qualsiasi singolo evento viene riconosciuto come duplicato.
...
| Composizione dell'utente                                            | Consegna di Apple          | Flag disattivato (predefinito)          | Flag attivo + finestra di 2500 ms                                       |
| ------------------------------------------------------------------- | -------------------------- | --------------------------------------- | ----------------------------------------------------------------------- |
...
| Flusso rapido (>10 piccoli DM nella finestra)                       | N Webhook                  | N turni                                 | Un turno, output limitato (primo + ultimo, con limiti su testo/allegati) |

### Risoluzione dei problemi della coalescenza per invii suddivisi
...
3. **I timestamp JSONL della sessione ≠ arrivo del Webhook.** I timestamp degli eventi di sessione (`~/.openclaw/agents/<id>/sessions/*.jsonl`) riflettono il momento in cui il Gateway consegna un messaggio all'agente, **non** quando è arrivato il Webhook. Un secondo messaggio in coda con tag `[Queued messages while agent was busy]` significa che il primo turno era ancora in esecuzione quando è arrivato il secondo Webhook — il bucket di coalescenza era già stato svuotato. Regola la finestra in base al log del server BB, non al log della sessione.

4. **La pressione di memoria rallenta l'invio della risposta.** Su macchine più piccole (8 GB), i turni dell'agente possono durare abbastanza a lungo da far svuotare il bucket di coalescenza prima che la risposta sia completata, e l'URL finisce come secondo turno in coda. Controlla `memory_pressure` e `ps -o rss -p $(pgrep openclaw-gateway)`; se il Gateway supera ~500 MB RSS e il compressore è attivo, chiudi altri processi pesanti o passa a un host più capiente.

5. **Gli invii come citazione di risposta seguono un percorso diverso.** Se l'utente ha toccato `Dump` come **risposta** a un balloon URL esistente (iMessage mostra un badge "1 Reply" sul balloon Dump), l'URL si trova in `replyToBody`, non in un secondo Webhook. La coalescenza non si applica — è una questione di skill/prompt, non del debouncer.

## Block streaming

Controlla se le risposte vengono inviate come singolo messaggio o trasmesse in blocchi:

```json5
{
  channels: {
    bluebubbles: {
      blockStreaming: true, // abilita la trasmissione a blocchi (disattivata per impostazione predefinita)
    },
  },
}
```
...
- Limite dei media tramite `channels.bluebubbles.mediaMaxMb` per media in entrata e in uscita (predefinito: 8 MB).
...
- `channels.bluebubbles.serverUrl`: URL di base della API REST di BlueBubbles.
...
- `channels.bluebubbles.allowFrom`: lista di consentiti DM (handle, email, numeri E.164, `chat_id:*`, `chat_guid:*`).
- `channels.bluebubbles.groupPolicy`: `open | allowlist | disabled` (predefinito: `allowlist`).
- `channels.bluebubbles.groupAllowFrom`: lista di consentiti dei mittenti di gruppo.
...
- `channels.bluebubbles.blockStreaming`: abilita la trasmissione a blocchi (predefinito: `false`; richiesto per le risposte in streaming).
- `channels.bluebubbles.textChunkLimit`: dimensione dei blocchi in uscita in caratteri (predefinito: 4000).
- `channels.bluebubbles.sendTimeoutMs`: timeout per richiesta in ms per gli invii di testo in uscita tramite `/api/v1/message/text` (predefinito: 30000). Aumentalo su configurazioni macOS 26 in cui gli invii iMessage tramite API privata possono bloccarsi per oltre 60 secondi all'interno del framework iMessage; ad esempio `45000` o `60000`. Sonde, ricerche chat, reazioni, modifiche e controlli di integrità mantengono attualmente il valore predefinito più breve di 10 s; l'estensione della copertura a reazioni e modifiche è prevista come passaggio successivo. Override per account: `channels.bluebubbles.accounts.<accountId>.sendTimeoutMs`.
...
- `channels.bluebubbles.mediaLocalRoots`: lista esplicita di consentiti di directory locali assolute permesse per i percorsi dei media locali in uscita. Gli invii di percorsi locali sono negati per impostazione predefinita finché questo non viene configurato. Override per account: `channels.bluebubbles.accounts.<accountId>.mediaLocalRoots`.
- `channels.bluebubbles.coalesceSameSenderDms`: unisce Webhook DM consecutivi dello stesso mittente in un unico turno agente in modo che l'invio suddiviso testo+URL di Apple arrivi come un solo messaggio (predefinito: `false`). Vedi [Coalescenza dei DM con invio suddiviso](#coalescing-split-send-dms-command--url-in-one-composition) per scenari, regolazione della finestra e compromessi. Quando è abilitato senza un `messages.inbound.byChannel.bluebubbles` esplicito, amplia la finestra di debounce in entrata predefinita da 500 ms a 2500 ms.
...
- Handle diretti: `+15555550123`, `user@example.com`
  - Se un handle diretto non ha una chat DM esistente, OpenClaw ne creerà una tramite `POST /api/v1/chat/new`. Questo richiede che la API privata BlueBubbles sia abilitata.

### Instradamento iMessage vs SMS

Quando lo stesso handle ha sia una chat iMessage sia una chat SMS sul Mac (ad esempio un numero di telefono registrato su iMessage ma che ha ricevuto anche fallback con balloon verdi), OpenClaw preferisce la chat iMessage e non esegue mai in modo silenzioso il downgrade a SMS. Per forzare la chat SMS, usa un prefisso di destinazione `sms:` esplicito (ad esempio `sms:+15555550123`). Gli handle senza una chat iMessage corrispondente continuano comunque a inviare tramite qualunque chat BlueBubbles segnali.
...
- Non esiste alcun bypass localhost per l'autenticazione Webhook BlueBubbles. Se usi un proxy per il traffico Webhook, mantieni la password BlueBubbles nella richiesta end-to-end. `gateway.trustedProxies` non sostituisce `channels.bluebubbles.password` in questo caso. Vedi [Sicurezza del Gateway](/it/gateway/security#reverse-proxy-configuration).
...
- Le reazioni richiedono la API privata BlueBubbles (`POST /api/v1/message/react`); assicurati che la versione del server la esponga.
- Modifica/annullamento invio richiedono macOS 13+ e una versione compatibile del server BlueBubbles. Su macOS 26 (Tahoe), la modifica è attualmente non funzionante a causa di cambiamenti nell'API privata.
...
- `coalesceSameSenderDms` è abilitato ma gli invii suddivisi (ad esempio `Dump` + URL) arrivano ancora come due turni: vedi la checklist di [risoluzione dei problemi della coalescenza per invii suddivisi](#split-send-coalescing-troubleshooting) — le cause comuni sono una finestra di debounce troppo stretta, timestamp del log di sessione interpretati erroneamente come arrivo del Webhook, o un invio come citazione di risposta (che usa `replyToBody`, non un secondo Webhook).
- Per informazioni su stato/integrità: `openclaw status --all` o `openclaw status --deep`.

Per il riferimento generale del flusso di lavoro dei canali, vedi [Canali](/it/channels) e la guida [Plugin](/it/tools/plugin).
