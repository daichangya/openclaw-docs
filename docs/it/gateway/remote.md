---
read_when:
    - Esecuzione o risoluzione dei problemi delle configurazioni gateway remote
...
Questo repo supporta il “remoto via SSH” mantenendo un singolo Gateway (il master) in esecuzione su un host dedicato (desktop/server) e collegandovi i client.

- Per gli **operatori (tu / l'app macOS)**: il tunneling SSH è il fallback universale.
- Per i **Node (iOS/Android e dispositivi futuri)**: connettiti al **WebSocket** del Gateway (LAN/tailnet o tunnel SSH secondo necessità).

## L'idea di base

- Il Gateway WebSocket si collega a **loopback** sulla porta configurata (predefinita: 18789).
...
Pensa all'**host Gateway** come al luogo “dove vive l'agente”. Possiede sessioni, profili auth, canali e stato.
...
- **Fallback:** mantieni loopback + tunnel SSH da qualsiasi macchina che abbia bisogno di accesso.
- **Esempi:** [exe.dev](/it/install/exe-dev) (VM semplice) o [Hetzner](/it/install/hetzner) (VPS di produzione).

Questa soluzione è ideale quando il tuo laptop va spesso in stop ma vuoi che l'agente sia sempre attivo.
...
- L'app apre e gestisce il tunnel, quindi WebChat + health check “funzionano e basta”.
...
- **I Node non eseguono il servizio gateway.** Per host dovrebbe essere in esecuzione un solo gateway, a meno che tu non stia eseguendo intenzionalmente profili isolati (vedi [Gateway multipli](/it/gateway/multiple-gateways)).
- La “modalità Node” dell'app macOS è solo un client Node sul Gateway WebSocket.
...
Nota: quando passi `--url`, la CLI non usa credenziali di fallback da configurazione o ambiente.
...
Puoi rendere persistente una destinazione remota in modo che i comandi CLI la usino come predefinita:
...
La risoluzione delle credenziali del Gateway segue un unico contratto condiviso tra percorsi call/probe/status e monitoraggio Discord delle approvazioni exec. L'host Node usa lo stesso contratto di base con un'eccezione in modalità locale (ignora intenzionalmente `gateway.remote.*`):

- Le credenziali esplicite (`--token`, `--password` o lo strumento `gatewayToken`) hanno sempre la precedenza nei percorsi call che accettano autenticazione esplicita.
- Sicurezza dell'override URL:
  - Gli override URL della CLI (`--url`) non riutilizzano mai credenziali implicite da config/env.
  - Gli override URL da ambiente (`OPENCLAW_GATEWAY_URL`) possono usare solo credenziali env (`OPENCLAW_GATEWAY_TOKEN` / `OPENCLAW_GATEWAY_PASSWORD`).
...
- Eccezione host Node in modalità locale: `gateway.remote.token` / `gateway.remote.password` vengono ignorati.
- I controlli del token probe/status remoto sono rigorosi per impostazione predefinita: usano solo `gateway.remote.token` (nessun fallback al token locale) quando puntano alla modalità remota.
- Gli override env del Gateway usano solo `OPENCLAW_GATEWAY_*`.

## UI chat via SSH

WebChat non usa più una porta HTTP separata. L'interfaccia chat SwiftUI si collega direttamente al Gateway WebSocket.
...
- `ws://` in chiaro è consentito per impostazione predefinita solo su loopback. Per reti private fidate, imposta `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1` nel processo client come opzione di emergenza. Non esiste un equivalente in `openclaw.json`; deve essere un ambiente di processo per il client che effettua la connessione WebSocket.
- I **bind non-loopback** (`lan`/`tailnet`/`custom`, o `auto` quando loopback non è disponibile) devono usare l'autenticazione del gateway: token, password o un reverse proxy identity-aware con `gateway.auth.mode: "trusted-proxy"`.
- `gateway.remote.token` / `.password` sono sorgenti di credenziali del client. Non configurano da soli l'autenticazione del server.
- I percorsi call locali possono usare `gateway.remote.*` come fallback solo quando `gateway.auth.*` non è impostato.
- Se `gateway.auth.token` / `gateway.auth.password` è esplicitamente configurato tramite SecretRef e non risolto, la risoluzione fallisce in modalità chiusa (nessun fallback remoto che mascheri il problema).
- `gateway.remote.tlsFingerprint` fissa il certificato TLS remoto quando usi `wss://`.
- **Tailscale Serve** può autenticare il traffico Control UI/WebSocket tramite header di identità quando `gateway.auth.allowTailscale: true`; gli endpoint API HTTP non usano quell'autenticazione tramite header Tailscale e seguono invece la normale modalità di autenticazione HTTP del gateway. Questo flusso senza token presuppone che l'host gateway sia fidato. Impostalo su `false` se vuoi l'autenticazione con segreto condiviso ovunque.
- L'autenticazione **trusted-proxy** è solo per configurazioni non-loopback con proxy identity-aware. I reverse proxy loopback sullo stesso host non soddisfano `gateway.auth.mode: "trusted-proxy"`.
- Tratta il controllo del browser come accesso operatore: solo tailnet + pairing deliberato del Node.

Approfondimento: [Sicurezza](/it/gateway/security).

### macOS: tunnel SSH persistente tramite LaunchAgent

Per i client macOS che si collegano a un gateway remoto, la configurazione persistente più semplice usa una voce `LocalForward` nella configurazione SSH più un LaunchAgent per mantenere attivo il tunnel tra riavvii e crash.
...
#### Passo 3: configura il token gateway
...
Nota: se hai un LaunchAgent `com.openclaw.ssh-tunnel` residuo di una configurazione precedente, scaricalo ed eliminalo.
