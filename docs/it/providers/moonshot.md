...
read_when:
    ...
    - Devi capire endpoint, chiavi e riferimenti ai modelli separati
    - Vuoi una configurazione pronta da copiare e incollare per uno dei due provider
summary: Configura Moonshot K2 vs Kimi Coding (provider + chiavi separati)
title: Moonshot AI
x-i18n:
...
Moonshot fornisce l'API Kimi con endpoint compatibili OpenAI. Configura il
...
Le stime di costo incluse per gli attuali modelli K2 ospitati da Moonshot usano le
...
<Tabs>
  <Tab title="API Moonshot">
    **Ideale per:** modelli Kimi K2 tramite Moonshot Open Platform.
    ...
    <Steps>
      <Step title="Scegli la tua regione endpoint">
      ...
      <Step title="Esegui uno smoke test live">
        Usa una directory di stato isolata quando vuoi verificare accesso al modello e
        tracciamento dei costi senza toccare le tue normali sessioni:
        ...
        l'uso normalizzato dei token più il costo stimato sotto `usage.cost` quando Moonshot restituisce
  ...
  <Tab title="Kimi Coding">
    **Ideale per:** attività focalizzate sul codice tramite l'endpoint Kimi Coding.
    ...
    <Note>
    Kimi Coding usa una chiave API diversa e un prefisso provider diverso (`kimi/...`) rispetto a Moonshot (`moonshot/...`). Il ref modello legacy `kimi/k2p5` continua a essere accettato come id di compatibilità.
    </Note>
...
<Steps>
  <Step title="Esegui il setup interattivo della ricerca web">
    ```bash
...
<AccordionGroup>
  <Accordion title="Modalità thinking nativa">
    ...
    <Warning>
    Quando il thinking Moonshot è abilitato, `tool_choice` deve essere `auto` o `none`. OpenClaw normalizza i valori incompatibili di `tool_choice` in `auto` per compatibilità.
    </Warning>
  ...
  <Accordion title="Sanitizzazione dell'id di chiamata degli strumenti">
  ...
  <Accordion title="Compatibilità dell'utilizzo in streaming">
    ...
    `https://api.moonshot.cn/v1`) pubblicizzano compatibilità dell'utilizzo in streaming sul
    trasporto condiviso `openai-completions`. OpenClaw la ricava dalle capacità degli endpoint,
    quindi id provider personalizzati compatibili che puntano agli stessi host
    Moonshot nativi ereditano lo stesso comportamento di streaming-usage.

    Con i prezzi K2.6 inclusi, l'uso in streaming che include token di input, output
    e cache-read viene anche convertito nel costo USD locale stimato per
    `/status`, `/usage full`, `/usage cost` e la contabilità delle sessioni
    supportata dal transcript.

  </Accordion>

  <Accordion title="Riferimento per endpoint e ref modello">
...
<CardGroup cols={2}>
  ...
  <Card title="Ricerca web" href="/it/tools/web" icon="magnifying-glass">
    Configurazione dei provider di ricerca web inclusa Kimi.
  </Card>
