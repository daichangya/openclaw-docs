...
Gli agenti OpenClaw possono generare video da prompt testuali, immagini di riferimento o video esistenti. Sono supportati quattordici backend provider, ciascuno con diverse opzioni di modello, modalità di input e set di funzionalità. L'agente sceglie automaticamente il provider giusto in base alla tua configurazione e alle chiavi API disponibili.

<Note>
Lo strumento `video_generate` compare solo quando è disponibile almeno un provider di generazione video. Se non lo vedi negli strumenti del tuo agente, imposta una chiave API del provider o configura `agents.defaults.videoGenerationModel`.
</Note>
...
Al di fuori delle esecuzioni agente supportate da sessione (ad esempio, invocazioni dirette dello strumento), lo strumento usa la generazione inline come fallback e restituisce il percorso finale del media nello stesso turno.

I file video generati vengono salvati nell'archiviazione media gestita da OpenClaw quando il
provider restituisce byte. Il limite predefinito di salvataggio per i video generati segue il limite dei media video,
...
invece di far fallire l'attività se la persistenza locale rifiuta un file troppo grande.

### Ciclo di vita dell'attività

Ogni richiesta `video_generate` attraversa quattro stati:
...
Alcuni provider accettano env var di chiave API aggiuntive o alternative. Vedi le singole [pagine provider](#related) per i dettagli.
...
Questo è il contratto esplicito di modalità usato da `video_generate`, dai test di contratto
...
| Google   | Sì         | Sì             | Sì             | `generate`, `imageToVideo`; `videoToVideo` condiviso saltato perché l'attuale sweep Gemini/Veo supportato da buffer non accetta quell'input |
...
| `imageRoles` | string[] | Suggerimenti di ruolo facoltativi per posizione, in parallelo all'elenco combinato delle immagini. Valori canonici: `first_frame`, `last_frame`, `reference_image` |
| `video`      | string   | Singolo video di riferimento (percorso o URL)                                                                                            |
| `videos`     | string[] | Più video di riferimento (fino a 4)                                                                                                      |
| `videoRoles` | string[] | Suggerimenti di ruolo facoltativi per posizione, in parallelo all'elenco combinato dei video. Valore canonico: `reference_video`       |
...
| `audioRoles` | string[] | Suggerimenti di ruolo facoltativi per posizione, in parallelo all'elenco combinato degli audio. Valore canonico: `reference_audio`     |
...
stringhe di ruolo. Gli array `*Roles` non devono avere più voci del
relativo elenco di riferimenti; gli errori off-by-one falliscono con un messaggio chiaro.
Usa una stringa vuota per lasciare una posizione non impostata.
...
| `size`            | string  | Suggerimento sulla dimensione quando il provider lo supporta                          |
...
Seedance lo usa per rilevare automaticamente il rapporto dalle
dimensioni dell'immagine di input). I provider che non lo dichiarano espongono il valore tramite
...
| `model`           | string | Override del provider/modello (ad esempio `runway/gen4.5`)                                                                                                                                                                                                                                                                                     |
...
- Gli input audio di riferimento non cambiano la modalità risolta; si applicano in aggiunta a qualunque modalità selezionata dai riferimenti immagine/video e funzionano solo con provider che dichiarano `maxInputAudios`
...
Alcuni controlli delle capacità vengono applicati al livello di fallback invece che al
confine dello strumento, così una richiesta che supera i limiti del provider primario
può comunque essere eseguita su un fallback compatibile:
...
- Se `maxDurationSeconds` del candidato attivo è inferiore al valore richiesto
  in `durationSeconds` e il candidato non dichiara un elenco
...
- Se la richiesta contiene `providerOptions` e il candidato attivo
  ...
  esplicitamente rinunciare a tutte le provider options dichiarando uno schema vuoto
...
quando il provider primario è stato scartato; i salti successivi vengono registrati a
...
<AccordionGroup>
  ...
  <Accordion title="fal">
    Usa un flusso supportato da coda per job di lunga durata. Solo un'immagine di riferimento.
  </Accordion>
  ...
  <Accordion title="MiniMax">
    Solo un'immagine di riferimento.
  </Accordion>
  ...
  <Accordion title="Runway">
    Supporta file locali tramite URI dati. Video-to-video richiede `runway/gen4_aleph`. Le esecuzioni solo testo espongono i rapporti d'aspetto `16:9` e `9:16`.
  </Accordion>
  ...
  <Accordion title="Vydra">
    Usa direttamente `https://www.vydra.ai/api/v1` per evitare redirect che eliminano l'auth. `veo3` incluso è solo text-to-video; `kling` richiede un URL immagine remoto.
  </Accordion>

  <Accordion title="xAI">
    Supporta flussi text-to-video, image-to-video e di modifica/estensione video remoti.
  </Accordion>
</AccordionGroup>
...
capacità specifiche per modalità invece di soli limiti aggregati flat. Le nuove
...
sufficienti per dichiarare il supporto delle modalità di trasformazione. I provider dovrebbero dichiarare
...
per impostazione predefinita uno smoke sicuro per la release:
...
FAL è opt-in perché la latenza della coda lato provider può dominare il tempo di release:
...
- [Background Tasks](/it/automation/tasks) -- tracciamento delle attività per la generazione video asincrona
