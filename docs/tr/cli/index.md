...
title: CLI Başvurusu
x-i18n:
    generated_at: "2026-04-23T09:00:29Z"
    model: gpt-5.4
    provider: openai
...
`openclaw` ana CLI giriş noktasıdır. Her çekirdek komutun ya ayrı bir
başvuru sayfası vardır ya da takma adı olduğu komutla birlikte belgelenmiştir; bu
...
| Alan                 | Komutlar                                                                                                                                                                                                                  |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Kurulum ve ilk kullanım | [`setup`](/tr/cli/setup) · [`onboard`](/tr/cli/onboard) · [`configure`](/tr/cli/configure) · [`config`](/tr/cli/config) · [`completion`](/tr/cli/completion) · [`doctor`](/tr/cli/doctor) · [`dashboard`](/tr/cli/dashboard)                   |
| Sıfırlama ve kaldırma  | [`backup`](/tr/cli/backup) · [`reset`](/tr/cli/reset) · [`uninstall`](/tr/cli/uninstall) · [`update`](/tr/cli/update)                                                                                                                 |
| Mesajlaşma ve agent'lar | [`message`](/tr/cli/message) · [`agent`](/tr/cli/agent) · [`agents`](/tr/cli/agents) · [`acp`](/tr/cli/acp) · [`mcp`](/tr/cli/mcp)                                                                                                       |
...
| Ağ ve Nodes    | [`directory`](/tr/cli/directory) · [`nodes`](/tr/cli/nodes) · [`devices`](/tr/cli/devices) · [`node`](/tr/cli/node)                                                                                                                   |
| Çalışma zamanı ve sandbox  | [`approvals`](/tr/cli/approvals) · `exec-policy` ([`approvals`](/tr/cli/approvals) bölümüne bakın) · [`sandbox`](/tr/cli/sandbox) · [`tui`](/tr/cli/tui) · `chat`/`terminal` ([`tui --local`](/tr/cli/tui) için takma adlar) · [`browser`](/tr/cli/browser) |
...
| `--update`              | [`openclaw update`](/tr/cli/update) için kısa yazım (yalnızca kaynak kurulumları) |
| `-V`, `--version`, `-v` | Sürümü yazdırır ve çıkar                                                |

## Çıktı modları

- ANSI renkleri ve ilerleme göstergeleri yalnızca TTY oturumlarında işlenir.
- OSC-8 köprüleri desteklenen yerlerde tıklanabilir bağlantılar olarak işlenir; aksi durumda
  CLI düz URL'lere fallback yapar.
- Desteklenen yerlerde `--json` (ve `--plain`) temiz çıktı için biçimlendirmeyi devre dışı bırakır.
...
openclaw [--dev] [--profile <name>] <command>
  ...
  dns
    setup
  tui
  chat (takma ad: tui --local)
  terminal (takma ad: tui --local)
```
...
- `/config` — kalıcı config değişiklikleri.
- `/debug` — yalnızca çalışma zamanı config geçersiz kılmaları (disk değil bellek; `commands.debug: true` gerektirir).
...
Ayrıntılar için [Usage tracking](/tr/concepts/usage-tracking) sayfasına bakın.
