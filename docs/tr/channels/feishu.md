...
> **OpenClaw 2026.4.25 veya üzeri gerekir.** Kontrol etmek için `openclaw --version` komutunu çalıştırın. `openclaw update` ile yükseltin.
...
### Tüm gruplara izin ver, @mention gerektirme
...
```json5
{
  channels: {
    feishu: {
      ...
      groups: {
        oc_xxx: {
          // Kullanıcı open_id'leri şöyle görünür: ou_xxx
...
> Feishu/Lark yerel slash-command menülerini desteklemez, bu yüzden bunları düz metin mesajlar olarak gönderin.
...
Yanıtın tamamını tek mesajda göndermek için `streaming: false` ayarlayın.

### Kota optimizasyonu

İki isteğe bağlı bayrakla Feishu/Lark API çağrılarının sayısını azaltın:

- `typingIndicator` (varsayılan `true`): yazma tepkisi çağrılarını atlamak için `false` olarak ayarlayın
- `resolveSenderNames` (varsayılan `true`): gönderici profil aramalarını atlamak için `false` olarak ayarlayın
...
Feishu/Lark, DM'ler ve grup konu mesajları için ACP'yi destekler. Feishu/Lark ACP, metin komutlarıyla çalışır — yerel slash-command menüleri yoktur, bu yüzden `/acp ...` mesajlarını doğrudan konuşma içinde kullanın.
...
Bir Feishu/Lark DM'sinde veya konu başlığında:

```text
/acp spawn codex --thread here
```

`--thread here`, DM'ler ve Feishu/Lark konu mesajları için çalışır. Bağlı konuşmadaki sonraki mesajlar doğrudan bu ACP oturumuna yönlendirilir.
...
| `channels.feishu.webhookHost`                     | Webhook bağlama sunucusu                   | `127.0.0.1`      |
...
| `channels.feishu.groups.<chat_id>.requireMention` | Grup başına @mention geçersiz kılma        | inherited        |
...
| `channels.feishu.resolveSenderNames`              | Gönderici görünen adlarını çözümle         | `true`           |
...
Yerel Feishu/Lark ses baloncukları, Feishu `audio` mesaj türünü kullanır ve
Ogg/Opus yükleme medyası gerektirir (`file_type: "opus"`). Mevcut `.opus` ve `.ogg` medya
doğrudan yerel ses olarak gönderilir. MP3/WAV/M4A ve diğer olası ses biçimleri,
yalnızca yanıt sesle teslimat istediğinde (`audioAsVoice` / mesaj aracı `asVoice`, TTS sesli not
yanıtları dahil) `ffmpeg` ile 48kHz Ogg/Opus biçimine dönüştürülür.
Normal MP3 ekleri normal dosya olarak kalır. `ffmpeg` eksikse veya
...
Feishu/Lark konu grupları kanonik konu oturum anahtarı olarak `thread_id` (`omt_*`) olayını kullanır.
OpenClaw'ın konuya dönüştürdüğü normal grup yanıtları ise
ilk tur ile takip turunun aynı oturumda kalması için yanıt kök mesaj kimliğini (`om_*`) kullanmaya devam eder.
