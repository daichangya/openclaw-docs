---
read_when:
    - Sie möchten datenschutzorientierte Inferenz in OpenClaw nutzen
...
title: Venice AI
x-i18n:
    generated_at: "2026-04-24T06:56:19Z"
    model: gpt-5.4
    provider: openai
...
Venice AI bietet **datenschutzorientierte AI-Inferenz** mit Unterstützung für unzensierte Modelle und Zugriff auf große proprietäre Modelle über ihren anonymisierten Proxy. Jede Inferenz ist standardmäßig privat — kein Training auf Ihren Daten, kein Logging.
...
- OpenAI-kompatible Endpunkte `/v1`.
...
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| **Private**    | Vollständig privat. Prompts/Antworten werden **nie gespeichert oder protokolliert**. Flüchtig.                                       | Llama, Qwen, DeepSeek, Kimi, MiniMax, Venice Uncensored usw.  |
| **Anonymized** | Über Venice weitergeleitet, wobei Metadaten entfernt werden. Der zugrunde liegende Provider (OpenAI, Anthropic, Google, xAI) sieht anonymisierte Anfragen. | Claude, GPT, Gemini, Grok                                     |

<Warning>
Anonymisierte Modelle sind **nicht** vollständig privat. Venice entfernt Metadaten vor der Weiterleitung, aber der zugrunde liegende Provider (OpenAI, Anthropic, Google, xAI) verarbeitet die Anfrage weiterhin. Wählen Sie **Private**-Modelle, wenn vollständige Privatsphäre erforderlich ist.
</Warning>

## Funktionen

- **Datenschutzorientiert**: Wählen Sie zwischen den Modi „private“ (vollständig privat) und „anonymized“ (weitergeleitet)
...
- **OpenAI-kompatible API**: Standardmäßige Endpunkte `/v1` für einfache Integration
...
<AccordionGroup>
  <Accordion title="Private Modelle (26) — vollständig privat, kein Logging">
...
Der Endpunkt `/models` ist öffentlich (keine Auth für Listen erforderlich), aber Inferenz erfordert einen gültigen API-Schlüssel.
...
| **Vision/Images**    | Modelle mit der Funktion „Vision“                     |
| **JSON mode**        | Unterstützt über `response_format`                    |

## Preise

Venice verwendet ein kreditbasiertes System. Prüfen Sie [venice.ai/pricing](https://venice.ai/pricing) für aktuelle Preise:
...
| **Abrechnung** | Venice-Guthaben             | Abrechnung des Providers |
...
<AccordionGroup>
  ...
  <Accordion title="Modell nicht verfügbar">
    Der Venice-Modellkatalog wird dynamisch aktualisiert. Führen Sie `openclaw models list` aus, um die aktuell verfügbaren Modelle zu sehen. Manche Modelle können vorübergehend offline sein.
  </Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="Venice AI" href="https://venice.ai" icon="globe">
    Venice-AI-Homepage und Konto-Registrierung.
  </Card>
