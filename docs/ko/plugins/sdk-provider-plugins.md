...
read_when:
    ...
    - provider 인증, 카탈로그, 런타임 hook를 이해해야 합니다
...
API 키 인증, 동적 모델 확인을 갖춘 provider를 만들 수 있습니다.

<Info>
  아직 OpenClaw Plugin을 한 번도 빌드해본 적이 없다면, 먼저
  기본 패키지 구조와 매니페스트 설정을 위해 [Getting Started](/ko/plugins/building-plugins)를
  읽어보세요.
</Info>

<Tip>
  Provider Plugin은 OpenClaw의 일반 추론 루프에 모델을 추가합니다. 모델이
  스레드, Compaction 또는 도구 이벤트를 소유하는 기본 에이전트 데몬을 통해 실행되어야 한다면,
...
<Steps>
  ...
  <Step title="패키지 및 매니페스트">
    ...
    매니페스트는 OpenClaw가 Plugin 런타임을 로드하지 않고도
    자격 증명을 감지할 수 있도록 `providerAuthEnvVars`를 선언합니다. provider 변형이 다른 provider ID의 인증을 재사용해야 한다면
    `providerAuthAliases`를 추가하세요. `modelSupport`는
    선택 사항이며 런타임 hook가 존재하기 전에 `acme-large` 같은 축약형
    모델 ID로부터 OpenClaw가 provider Plugin을 자동 로드할 수 있게 합니다. provider를
    ClawHub에 게시하는 경우 `package.json`의 `openclaw.compat`와 `openclaw.build` 필드는
    필수입니다.
  ...
  <Step title="provider 등록">
    ...
    자신의 모델로 `acme-ai/acme-large`를 선택할 수 있습니다.
    ...
    API 키
    인증과 단일 catalog 기반 런타임을 가진 텍스트 provider 하나만 등록하는 번들 provider의 경우,
    더 좁은 `defineSingleProviderPluginEntry(...)`
    helper를 사용하는 편이 좋습니다:
    ...
    provider 인증을 확인할 수 있을 때 사용하는 라이브 카탈로그 경로입니다. provider별 탐색을 수행할 수 있습니다.
    `buildStaticProvider`는 인증이
    구성되기 전에 안전하게 표시할 수 있는 오프라인 행에만 사용하세요. 자격 증명이 필요하거나 네트워크 요청을 해서는 안 됩니다.
    OpenClaw의 `models list --all` 표시는 현재 번들 provider Plugin에 대해서만
    정적 카탈로그를 실행하며, 빈 구성, 빈 환경 변수, 에이전트/워크스페이스 경로 없음 상태에서 실행됩니다.

    인증 흐름이 온보딩 중에 `models.providers.*`,
    aliases, 에이전트 기본 모델도 패치해야 한다면
    `openclaw/plugin-sdk/provider-onboard`의 preset helper를 사용하세요. 가장 좁은 helper는
    `createDefaultModelPresetAppliers(...)`,
    `createDefaultModelsPresetAppliers(...)`,
    `createModelCatalogPresetAppliers(...)`입니다.

    provider의 기본 endpoint가 일반
    `openai-completions` 전송에서 스트리밍 사용량 블록을 지원한다면, provider ID 검사를 하드코딩하는 대신
    `openclaw/plugin-sdk/provider-catalog-shared`의 공유 카탈로그 helper를 사용하는 편이 좋습니다.
    `supportsNativeStreamingUsageCompat(...)`와
    `applyProviderNativeStreamingUsageCompat(...)`는 endpoint capability 맵에서 지원 여부를 감지하므로,
    Plugin이 사용자 지정 provider ID를 사용하더라도 기본 Moonshot/DashScope 스타일 endpoint가
    여전히 선택적으로 활성화됩니다.

  </Step>

  <Step title="동적 모델 확인 추가">
    provider가 임의의 모델 ID를 허용하는 경우(프록시 또는 라우터처럼)
  ...
  <Step title="런타임 hook 추가(필요한 경우)">
    대부분의 provider는 `catalog` + `resolveDynamicModel`만 필요합니다. provider에 필요해질 때마다
    hook를 점진적으로 추가하세요.

    이제 공유 helper builder가 가장 일반적인 replay/tool 호환
    계열을 다루므로, 일반적으로 Plugin이 각 hook를 하나씩 직접 연결할 필요는 없습니다:
    ...
    | Family | 연결되는 항목 |
    | --- | --- |
    | `openai-compatible` | OpenAI 호환 전송을 위한 공유 OpenAI 스타일 replay 정책. 여기에는 tool-call-id 정리, assistant 우선 순서 수정, 전송에서 필요할 때의 일반적인 Gemini 턴 검증이 포함됩니다 |
    | `anthropic-by-model` | `modelId`로 선택되는 Claude 인식 replay 정책. 따라서 Anthropic 메시지 전송은 확인된 모델이 실제로 Claude ID일 때만 Claude 전용 사고 블록 정리를 받습니다 |
    | `google-gemini` | 기본 Gemini replay 정책과 bootstrap replay 정리, 태그된 reasoning 출력 모드 |
    | `passthrough-gemini` | OpenAI 호환 프록시 전송을 통해 실행되는 Gemini 모델용 Gemini thought-signature 정리. 기본 Gemini replay 검증이나 bootstrap 재작성은 활성화하지 않습니다 |
    | `hybrid-anthropic-openai` | 하나의 Plugin에서 Anthropic 메시지 표면과 OpenAI 호환 모델 표면을 함께 제공하는 provider용 하이브리드 정책. 선택적 Claude 전용 사고 블록 제거는 Anthropic 쪽에만 범위가 제한됩니다 |
    ...
    | `kilocode-thinking` | 공유 프록시 스트림 경로에서 Kilo reasoning 래퍼. `kilo/auto` 및 지원되지 않는 프록시 reasoning ID는 삽입된 thinking을 건너뜁니다 |
    | `moonshot-thinking` | 구성 + `/think` 수준에서 Moonshot 이진 native-thinking 페이로드 매핑 |
    | `minimax-fast-mode` | 공유 스트림 경로에서 MiniMax fast-mode 모델 재작성 |
    | `openai-responses-defaults` | 공유 native OpenAI/Codex Responses 래퍼: attribution 헤더, `/fast`/`serviceTier`, 텍스트 상세도, 기본 Codex 웹 검색, reasoning 호환 페이로드 형태 조정, Responses 컨텍스트 관리 |
    | `openrouter-thinking` | 프록시 경로용 OpenRouter reasoning 래퍼. 지원되지 않는 모델/`auto` 건너뛰기는 중앙에서 처리됩니다 |
    | `tool-stream-default-on` | 명시적으로 비활성화하지 않는 한 도구 스트리밍을 원하는 Z.AI 같은 provider용 기본 활성 `tool_stream` 래퍼 |
    ...
    enum과 이러한 계열이 기반으로 하는 공유 helper도 export합니다. 일반적인 공개
    export는 다음과 같습니다:
    ...
    - `sanitizeGoogleGeminiReplayHistory(...)`
      및 `resolveTaggedReasoningOutputMode()` 같은 Gemini replay helper
    - `resolveProviderEndpoint(...)`,
      `normalizeProviderId(...)`, `normalizeGooglePreviewModelId(...)`,
      `normalizeNativeXaiModelId(...)` 같은 endpoint/model helper

    `openclaw/plugin-sdk/provider-stream`은 family builder와
    이 계열이 재사용하는 공개 래퍼 helper를 모두 노출합니다. 일반적인 공개 export는
    다음과 같습니다:
    ...
    일부 스트림 helper는 의도적으로 provider 로컬에 남아 있습니다. 현재 번들
    예시: `@openclaw/anthropic-provider`는
    공개 `api.ts` /
    `contract-api.ts` 표면에서 `wrapAnthropicProviderStream`, `resolveAnthropicBetas`,
    `resolveAnthropicFastMode`, `resolveAnthropicServiceTier`, 그리고 더 낮은 수준의 Anthropic 래퍼 builder를 export합니다.
    이 helper는 Claude OAuth 베타 처리와
    `context1m` 게이팅도 인코딩하므로 Anthropic 전용으로 유지됩니다.

    다른 번들 provider도 동작이 계열 전반에 걸쳐 깔끔하게 공유되지 않을 때
    전송 전용 래퍼를 로컬에 유지합니다. 현재 예시: 번들 xAI Plugin은
    자체 `wrapStreamFn`에 native xAI Responses 형태 조정을 유지합니다.
    ...
    도구 스키마 계열과 공유 스키마/호환 helper를 노출합니다:

    - `ProviderToolCompatFamily`는 현재 공유 계열 인벤토리를 문서화합니다.
    - `buildProviderToolCompatFamilyHooks("gemini")`는 Gemini 안전 도구 스키마가 필요한 provider용 Gemini 스키마
      정리 + 진단을 연결합니다.
    - `normalizeGeminiToolSchemas(...)` 및 `inspectGeminiToolSchemas(...)`는
      기반이 되는 공개 Gemini 스키마 helper입니다.
    - `resolveXaiModelCompatPatch()`는 번들 xAI 호환 패치를 반환합니다:
      `toolSchemaProfile: "xai"`, 지원되지 않는 스키마 키워드, native
      `web_search` 지원, HTML 엔터티 도구 호출 인수 디코딩.
    - `applyXaiModelCompat(model)`은 러너에 도달하기 전에
      확인된 모델에 동일한 xAI 호환 패치를 적용합니다.

    실제 번들 예시: xAI Plugin은 `normalizeResolvedModel`과
    `contributeResolvedModelCompat`를 사용해 해당 호환 메타데이터를 코어에 xAI 규칙을 하드코딩하는 대신
    ...
    <Tabs>
      ...
      <Tab title="기본 전송 ID">
        일반 HTTP 또는 WebSocket 전송에서 기본 요청/세션 헤더 또는 메타데이터가 필요한 provider의 경우:
    ...
    <Accordion title="사용 가능한 모든 provider hook">
      OpenClaw는 다음 순서로 hook를 호출합니다. 대부분의 provider는 2-3개만 사용합니다:

      | # | Hook | 사용 시점 |
      | --- | --- | --- |
      | 1 | `catalog` | 모델 카탈로그 또는 기본 base URL |
      | 2 | `applyConfigDefaults` | 구성 구체화 중 provider 소유 전역 기본값 |
      | 3 | `normalizeModelId` | lookup 전 레거시/preview 모델 ID 별칭 정리 |
      | 4 | `normalizeTransport` | 일반 모델 조립 전 provider family `api` / `baseUrl` 정리 |
      | 5 | `normalizeConfig` | `models.providers.<id>` 구성 정규화 |
      | 6 | `applyNativeStreamingUsageCompat` | 구성 provider용 native 스트리밍 사용량 호환 재작성 |
      | 7 | `resolveConfigApiKey` | provider 소유 env-marker 인증 확인 |
      | 8 | `resolveSyntheticAuth` | 로컬/self-hosted 또는 구성 기반 synthetic 인증 |
      | 9 | `shouldDeferSyntheticProfileAuth` | synthetic 저장 프로필 placeholder를 env/구성 인증 뒤로 내리기 |
    ...
    - `normalizeConfig`는 먼저 일치하는 provider를 확인한 다음,
      실제로 구성을 변경할 때까지 다른
      hook 지원 provider Plugin을 확인합니다.
      어떤 provider hook도 지원되는 Google family 구성 항목을 재작성하지 않으면,
      번들 Google 구성 정규화기가 여전히 적용됩니다.
    - `resolveConfigApiKey`는 노출된 경우 provider hook를 사용합니다. 번들
      `amazon-bedrock` 경로도 여기에서 내장 AWS env-marker 확인기를 갖지만,
      ...
      | 17 | `resolveReasoningOutputMode` | 태그형 vs native reasoning 출력 계약 |
      | 18 | `prepareExtraParams` | 기본 요청 params |
      | 19 | `createStreamFn` | 완전 사용자 지정 StreamFn 전송 |
      | 20 | `wrapStreamFn` | 일반 스트림 경로의 사용자 지정 헤더/본문 래퍼 |
      | 21 | `resolveTransportTurnState` | 기본 턴별 헤더/메타데이터 |
      | 22 | `resolveWebSocketSessionPolicy` | 기본 WS 세션 헤더/쿨다운 |
      ...
      | 43 | `validateReplayTurns` | 내장 러너 전 엄격한 replay 턴 검증 |
      | 44 | `onModelSelected` | 선택 후 콜백(예: telemetry) |

      프롬프트 튜닝 참고:

      - `resolveSystemPromptContribution`을 사용하면 provider가 모델 family용 캐시 인식
        시스템 프롬프트 가이드를 주입할 수 있습니다. 동작이 하나의 provider/모델
        family에 속하고 안정적/동적 캐시 분리를 유지해야 한다면
        `before_prompt_build`보다 이것을 사용하는 편이 좋습니다.
  ...
  <Step title="추가 capability 추가(선택 사항)">
    ...
    OpenClaw는 이를 **hybrid-capability** Plugin으로 분류합니다. 이는
    회사 Plugin(벤더당 하나의 Plugin)에 권장되는 패턴입니다. [Internals: Capability Ownership](/ko/plugins/architecture#capability-ownership-model)을
    참조하세요.

    비디오 생성의 경우 위에 표시된 모드 인식 capability 형태를 사용하는 편이 좋습니다:
    `generate`, `imageToVideo`, `videoToVideo`. `maxInputImages`, `maxInputVideos`, `maxDurationSeconds` 같은
    평면 집계 필드만으로는 변환 모드 지원 또는 비활성화된 모드를
    깔끔하게 광고하기에 충분하지 않습니다.

    음악 생성 provider도 동일한 패턴을 따라야 합니다:
    프롬프트 전용 생성용 `generate`, 참조 이미지 기반
    생성용 `edit`. `maxInputImages`,
    `supportsLyrics`, `supportsFormat` 같은 평면 집계 필드는
    edit 지원을 광고하기에 충분하지 않습니다. 명시적인 `generate` / `edit`
    블록이 기대되는 계약입니다.
  ...
  <Step title="테스트">
    ...
    // index.ts 또는 전용 파일에서 provider 구성 객체를 export하세요
...
provider Plugin은 다른 외부 코드 Plugin과 동일한 방식으로 게시합니다:
...
├── openclaw.plugin.json      # provider 인증 메타데이터가 포함된 매니페스트
