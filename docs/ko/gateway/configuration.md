...
title: 구성
x-i18n:
    generated_at: "2026-04-24T06:13:33Z"
    model: gpt-5.4
    provider: openai
...
외부에 구성을 보관하는 경우 `OPENCLAW_CONFIG_PATH`를 실제 파일로 직접 지정하세요.
...
<Tip>
**구성이 처음이신가요?** 대화형 설정을 위해 `openclaw onboard`로 시작하거나, 완전한 복사-붙여넣기 구성을 보려면 [구성 예시](/ko/gateway/configuration-examples) 가이드를 확인하세요.
</Tip>
...
<Tabs>
  ...
  <Tab title="Control UI">
    ...
    Control UI는 라이브 구성 스키마에서 폼을 렌더링하며, 사용 가능한 경우
    필드 `title` / `description` 문서 메타데이터와 Plugin 및 채널 스키마를 포함하고,
    탈출구로 **Raw JSON** 편집기도 제공합니다. drill-down
    UI 및 기타 도구를 위해 gateway는
    하나의 경로 범위 스키마 노드와 즉시 하위 요약을 가져오는 `config.schema.lookup`도 노출합니다.
  </Tab>
  <Tab title="직접 편집">
...
<Warning>
OpenClaw는 스키마와 완전히 일치하는 구성만 허용합니다. 알 수 없는 키, 잘못된 타입, 유효하지 않은 값이 있으면 Gateway는 **시작을 거부**합니다. 유일한 루트 수준 예외는 `$schema`(문자열)이며, 편집기가 JSON Schema 메타데이터를 연결할 수 있도록 하기 위한 것입니다.
</Warning>

`openclaw config schema`는 Control UI
및 검증에서 사용되는 정식 JSON Schema를 출력합니다. `config.schema.lookup`은 단일 경로 범위 노드와
drill-down 도구용 하위 요약을 가져옵니다. 필드 `title`/`description` 문서 메타데이터는
중첩 객체, 와일드카드(`*`), 배열 항목(`[]`), `anyOf`/
`oneOf`/`allOf` 분기에도 유지됩니다. 런타임 Plugin 및 채널 스키마는
...
Gateway는 각 성공적인 시작 후 신뢰할 수 있는 마지막 정상 복사본을 유지합니다.
이후 `openclaw.json`이 검증에 실패하거나(`gateway.mode`가 사라지거나, 급격히
작아지거나, 앞에 엉뚱한 로그 줄이 추가된 경우 포함) OpenClaw는 손상된 파일을
`.clobbered.*`로 보존하고, 마지막 정상 복사본을 복원한 뒤, 복구
이유를 로그에 남깁니다. 다음 에이전트 턴도 시스템 이벤트 경고를 받아 주 에이전트가
복원된 구성을 무작정 다시 쓰지 않도록 합니다. 후보에 `***` 같은
redacted secret 플레이스홀더가 포함된 경우 마지막 정상 복사본으로의 승격은 건너뜁니다.
...
<AccordionGroup>
  ...
  <Accordion title="모델 선택 및 구성">
    ...
    - 기존 모델을 제거하지 않고 허용 목록 항목을 추가하려면 `openclaw config set agents.defaults.models '<json>' --strict-json --merge`를 사용하세요. 항목을 제거하는 일반 교체는 `--replace`를 전달하지 않으면 거부됩니다.
    - 모델 참조는 `provider/model` 형식을 사용합니다(예: `anthropic/claude-opus-4-6`).
    - `agents.defaults.imageMaxDimensionPx`는 transcript/도구 이미지 다운스케일링을 제어합니다(기본값 `1200`). 값을 낮추면 스크린샷이 많은 실행에서 보통 비전 토큰 사용량이 줄어듭니다.
    - 채팅에서 모델 전환은 [Models CLI](/ko/concepts/models)를, 인증 순환 및 폴백 동작은 [모델 페일오버](/ko/concepts/model-failover)를 참조하세요.
    - 사용자 정의/자체 호스팅 provider는 참조 문서의 [사용자 정의 provider](/ko/gateway/config-tools#custom-providers-and-base-urls)를 참조하세요.
  ...
  <Accordion title="누가 봇에 메시지를 보낼 수 있는지 제어">
    ...
    - `"pairing"` (기본값): 알 수 없는 발신자는 승인할 일회성 페어링 코드를 받음
  ...
  <Accordion title="Gateway 채널 상태 모니터링 조정">
    ...
    - 전역 모니터를 비활성화하지 않고 하나의 채널 또는 계정에 대한 자동 재시작만 끄려면 `channels.<provider>.healthMonitor.enabled` 또는 `channels.<provider>.accounts.<id>.healthMonitor.enabled`를 사용하세요.
    - 운영 디버깅은 [상태 점검](/ko/gateway/health)을, 모든 필드는 [전체 참조](/ko/gateway/configuration-reference#gateway)를 참조하세요.
  ...
  <Accordion title="세션 및 재설정 구성">
    ...
    - 범위, identity link, 전송 정책은 [세션 관리](/ko/concepts/session)를 참조하세요.
  ...
  <Accordion title="공식 iOS 빌드용 relay 기반 push 활성화">
    ...
    - Gateway가 외부 relay를 통해 `push.test`, wake nudge, reconnect wake를 보낼 수 있게 합니다.
    ...
    - 로컬/수동 iOS 빌드는 direct APNs를 유지합니다. relay 기반 전송은 relay를 통해 등록한 공식 배포 빌드에만 적용됩니다.
    - registration 및 전송 트래픽이 동일한 relay 배포로 도달하도록, 공식/TestFlight iOS 빌드에 내장된 relay base URL과 반드시 일치해야 합니다.

    종단 간 흐름:

    1. 동일한 relay base URL로 컴파일된 공식/TestFlight iOS 빌드를 설치합니다.
    2. Gateway에서 `gateway.push.apns.relay.baseUrl`을 구성합니다.
    3. iOS 앱을 Gateway와 페어링하고 노드 세션과 운영자 세션이 모두 연결되도록 합니다.
    4. iOS 앱이 Gateway identity를 가져오고, App Attest와 앱 receipt를 사용해 relay에 등록한 다음, relay 기반 `push.apns.register` 페이로드를 페어링된 Gateway에 게시합니다.
    5. Gateway는 relay handle과 send grant를 저장하고 이를 `push.test`, wake nudge, reconnect wake에 사용합니다.

    운영 참고:

    - iOS 앱을 다른 Gateway로 전환하는 경우, 앱이 해당 Gateway에 바인딩된 새 relay registration을 게시할 수 있도록 앱을 다시 연결하세요.
    ...
    - `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true`는 loopback 전용 개발용 비상 탈출구로 유지됩니다. HTTP relay URL을 구성에 영구 저장하지 마세요.

    종단 간 흐름은 [iOS 앱](/ko/platforms/ios#relay-backed-push-for-official-builds)을, relay 보안 모델은 [인증 및 신뢰 흐름](/ko/platforms/ios#authentication-and-trust-flow)을 참조하세요.
  ...
  <Accordion title="Webhook(Hooks) 설정">
    ...
    - `hooks.path`는 `/`가 될 수 없습니다. Webhook ingress는 `/hooks` 같은 전용 하위 경로에 두세요.
    - 범위가 엄격히 제한된 디버깅이 아닌 이상 안전하지 않은 콘텐츠 우회 플래그(`hooks.gmail.allowUnsafeExternalContent`, `hooks.mappings[].allowUnsafeExternalContent`)는 비활성화 상태로 유지하세요.
    - `hooks.allowRequestSessionKey`를 활성화하는 경우, 호출자가 선택한 세션 키를 제한하기 위해 `hooks.allowedSessionKeyPrefixes`도 설정하세요.
    - hook 기반 에이전트에는 강한 최신 모델 티어와 엄격한 도구 정책(예: 가능한 경우 메시징 전용 + 샌드박싱)을 우선 사용하세요.
  ...
  <Accordion title="멀티 에이전트 라우팅 구성">
    ...
    바인딩 규칙 및 에이전트별 액세스 프로필은 [멀티 에이전트](/ko/concepts/multi-agent) 및 [전체 참조](/ko/gateway/config-agents#multi-agent-routing)를 참조하세요.
  ...
  <Accordion title="구성을 여러 파일로 분할하기($include)">
    ...
    - **파일 배열**: 순서대로 deep-merge됩니다(나중 항목 우선)
    - **형제 키**: include 이후 병합됩니다(포함된 값 재정의)
    - **중첩 include**: 최대 10단계 깊이까지 지원
    - **상대 경로**: 포함하는 파일 기준으로 확인
    ...
    - **미지원 write-through**: 루트 include, include 배열, 형제 재정의가 있는 include는
      OpenClaw 소유 쓰기에서 구성을 평탄화하지 않고 닫힌 실패로 처리됩니다
...
직접 파일 편집은 검증되기 전까지 신뢰할 수 없는 것으로 취급됩니다. watcher는
편집기의 임시 쓰기/이름 변경 변동이 안정될 때까지 기다린 뒤 최종 파일을 읽고,
잘못된 외부 편집은 마지막 정상 구성으로 복원하여 거부합니다. OpenClaw 소유
구성 쓰기도 쓰기 전에 동일한 스키마 게이트를 사용합니다. `gateway.mode`를
삭제하거나 파일 크기를 절반 이하로 줄이는 등 파괴적인 손상은 거부되며
검사용으로 `.rejected.*`로 저장됩니다.

로그에 `Config auto-restored from last-known-good` 또는
`config reload restored last-known-good config`가 표시되면 `openclaw.json`
옆의 일치하는 `.clobbered.*` 파일을 검사하고, 거부된 페이로드를 수정한 다음
`openclaw config validate`를 실행하세요. 복구 점검표는 [Gateway 문제 해결](/ko/gateway/troubleshooting#gateway-restored-last-known-good-config)을 참조하세요.
...
| Gateway 서버 | `gateway.*` (포트, bind, auth, tailscale, TLS, HTTP) | **예** |
| 인프라 | `discovery`, `canvasHost`, `plugins` | **예** |

<Note>
`gateway.reload`와 `gateway.remote`는 예외입니다. 이를 변경해도 **재시작을 트리거하지 않습니다**.
</Note>
...
하나의 최상위 섹션이 자체 포함 파일에 있을 때도
핫 리로드 결정(핫 적용 vs 재시작)을 예측 가능하게 유지할 수 있습니다. 소스 레이아웃이 모호하면 리로드 계획은 닫힌 실패로 처리됩니다.

## 구성 RPC(프로그래밍 방식 업데이트)

Gateway API를 통해 구성을 쓰는 도구에는 다음 흐름을 권장합니다.

- `config.schema.lookup`으로 하나의 하위 트리 검사(얕은 스키마 노드 + 하위
  요약)
- `config.get`으로 현재 스냅샷과 `hash` 가져오기
- 부분 업데이트에는 `config.patch` 사용(JSON merge patch: 객체는 병합, `null`은
  삭제, 배열은 교체)
...
`deviceId+clientIp`당 60초에 3회로 속도 제한됩니다. 재시작
요청은 병합 처리되며, 이후 재시작 주기 사이에 30초 쿨다운을 적용합니다.
</Note>
...
어느 파일도 기존 env var를 재정의하지 않습니다. 구성에 인라인 env var를 설정할 수도 있습니다.
...
동일한 env var: `OPENCLAW_LOAD_SHELL_ENV=1`
</Accordion>

<Accordion title="구성 값의 env var 치환">
  `${VAR_NAME}`으로 모든 구성 문자열 값에서 env var를 참조할 수 있습니다.
...
`env`/`file`/`exec`용 `secrets.providers`를 포함한 SecretRef 세부 정보는 [Secrets 관리](/ko/gateway/secrets)에 있습니다.
