...
<Tip>
**구성이 처음이신가요?** 대화형 설정을 위해 `openclaw onboard`로 시작하거나, 완전한 복사-붙여넣기용 config를 보려면 [구성 예제](/ko/gateway/configuration-examples) 가이드를 확인하세요.
</Tip>
...
<Tabs>
  ...
  <Tab title="Control UI">
    ...
    Control UI는 live config schema에서 폼을 렌더링하며, 사용 가능할 경우 필드
    `title` / `description` 문서 메타데이터와 함께 Plugin 및 채널 schema도 포함하고,
    탈출구로 **Raw JSON** 편집기를 제공합니다. 드릴다운 UI 및 기타 도구를 위해
    gateway는 하나의 경로 범위 schema 노드와 즉시 하위 요약을 가져오는
    `config.schema.lookup`도 노출합니다.
  </Tab>
  <Tab title="직접 편집">
    `~/.openclaw/openclaw.json`을 직접 편집하세요. Gateway가 파일을 감시하고 자동으로 변경 사항을 적용합니다([hot reload](#config-hot-reload) 참조).
  </Tab>
</Tabs>
...
<Warning>
OpenClaw는 schema와 완전히 일치하는 구성만 허용합니다. 알 수 없는 키, 잘못된 타입, 유효하지 않은 값이 있으면 Gateway는 **시작을 거부**합니다. 유일한 루트 수준 예외는 `$schema`(문자열)이며, 편집기가 JSON Schema 메타데이터를 연결할 수 있도록 허용됩니다.
</Warning>

Schema 도구 관련 참고:

- `openclaw config schema`는 Control UI와 config 검증에서 사용하는 동일한 JSON Schema 계열을 출력합니다.
- 해당 schema 출력을 `openclaw.json`의 표준 기계 판독 계약으로 간주하세요. 이 개요와 구성 참조는 이를 요약한 것입니다.
- 필드 `title` 및 `description` 값은 편집기와 폼 도구를 위해 schema 출력에 포함됩니다.
- 중첩 객체, 와일드카드(`*`), 배열 항목(`[]`) 항목은 일치하는 필드 문서가 있는 경우 동일한 문서 메타데이터를 상속합니다.
- `anyOf` / `oneOf` / `allOf` 구성 분기도 동일한 문서 메타데이터를 상속하므로 union/intersection 변형에서도 동일한 필드 도움말이 유지됩니다.
- `config.schema.lookup`은 정규화된 하나의 config 경로와 얕은 schema 노드(`title`, `description`, `type`, `enum`, `const`, 일반적인 경계값 및 유사한 검증 필드), 일치하는 UI 힌트 메타데이터, 드릴다운 도구를 위한 즉시 하위 요약을 반환합니다.
- runtime에서 Gateway가 현재 manifest registry를 로드할 수 있으면 Plugin/채널 schema가 병합됩니다.
- `pnpm config:docs:check`는 문서용 config 기준 아티팩트와 현재 schema 표면 간의 드리프트를 감지합니다.
...
- 수리를 적용하려면 `openclaw doctor --fix`(또는 `--yes`) 실행

또한 Gateway는 성공적으로 시작한 후 신뢰할 수 있는 마지막 정상 사본도 유지합니다.
이후 `openclaw.json`이 OpenClaw 외부에서 변경되어 더 이상 유효하지 않게 되면, 시작
및 hot reload 시 깨진 파일은 타임스탬프가 붙은 `.clobbered.*` 스냅샷으로 보존되고,
마지막 정상 사본이 복원되며, 복구 이유와 함께 강한 경고가 로그에 기록됩니다.
다음 메인 에이전트 턴에도 config가 복원되었으며 무작정 다시 쓰면 안 된다는
시스템 이벤트 경고가 전달됩니다. 마지막 정상 사본 승격은 검증된 시작 후와
수락된 hot reload 후에 갱신되며, 여기에는 저장된 파일 해시가 수락된 쓰기와 여전히
일치하는 OpenClaw 소유 config 쓰기도 포함됩니다. 후보에 `***` 또는 축약된 토큰 값
같은 마스킹된 secret placeholder가 포함되어 있으면 승격은 건너뜁니다.
...
<AccordionGroup>
  ...
  <Accordion title="모델 선택 및 구성">
    ...
    - `agents.defaults.imageMaxDimensionPx`는 transcript/tool 이미지 다운스케일링을 제어합니다(기본값 `1200`). 값이 낮을수록 스크린샷이 많은 실행에서 vision token 사용량이 보통 줄어듭니다.
    - 채팅에서 모델 전환은 [Models CLI](/ko/concepts/models)를, 인증 순환 및 fallback 동작은 [Model Failover](/ko/concepts/model-failover)를 참조하세요.
    - 커스텀/셀프호스팅 provider는 참조 문서의 [Custom providers](/ko/gateway/configuration-reference#custom-providers-and-base-urls)를 확인하세요.
  ...
  <Accordion title="누가 봇에 메시지를 보낼 수 있는지 제어">
    ...
    - `"pairing"` (기본값): 알 수 없는 발신자는 승인할 1회용 페어링 코드를 받음
    - `"allowlist"`: `allowFrom`(또는 페어링된 allow store)에 있는 발신자만 허용
  ...
  <Accordion title="그룹 채팅 멘션 게이팅 설정">
    ...
    - 채널별 재정의 및 self-chat 모드는 [전체 참조](/ko/gateway/configuration-reference#group-chat-mention-gating)를 참조하세요.

  </Accordion>

  <Accordion title="에이전트별 Skills 제한">
    공유 기본값에는 `agents.defaults.skills`를 사용하고, 특정
    ...
    - [Skills](/ko/tools/skills), [Skills config](/ko/tools/skills-config), 그리고
      [구성 참조](/ko/gateway/configuration-reference#agents-defaults-skills)를 참조하세요.

  </Accordion>

  <Accordion title="gateway 채널 상태 모니터링 조정">
    stale 상태로 보이는 채널을 gateway가 얼마나 적극적으로 재시작할지 제어합니다.
    ...
    - `channelStaleEventThresholdMinutes`는 점검 간격보다 크거나 같아야 합니다.
    - 전역 모니터를 끄지 않고 하나의 채널 또는 계정에 대한 자동 재시작만 끄려면 `channels.<provider>.healthMonitor.enabled` 또는 `channels.<provider>.accounts.<id>.healthMonitor.enabled`를 사용하세요.
  ...
  <Accordion title="샌드박싱 활성화">
    격리된 샌드박스 runtime에서 에이전트 세션을 실행합니다.
    ...
    전체 가이드는 [Sandboxing](/ko/gateway/sandboxing)를, 모든 옵션은 [전체 참조](/ko/gateway/configuration-reference#agentsdefaultssandbox)를 참조하세요.
  ...
  <Accordion title="공식 iOS 빌드를 위한 relay 기반 푸시 활성화">
    ...
    - 각 relay 기반 registration을 iOS 앱이 페어링한 gateway identity에 바인딩하므로, 다른 gateway는 저장된 registration을 재사용할 수 없습니다.
    - 로컬/수동 iOS 빌드는 direct APNs를 계속 사용합니다. relay 기반 전송은 relay를 통해 등록한 공식 배포 빌드에만 적용됩니다.
    - 공식/TestFlight iOS 빌드에 내장된 relay base URL과 일치해야 하며, 그래야 registration 및 전송 트래픽이 동일한 relay 배포에 도달합니다.
    ...
    - iOS 앱을 다른 gateway로 전환하면, 앱을 다시 연결하여 해당 gateway에 바인딩된 새 relay registration을 게시할 수 있게 하세요.
    ...
    - `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true`는 local loopback 전용 개발용 예외 수단으로 유지됩니다. HTTP relay URL을 config에 영구 저장하지 마세요.

    종단 간 흐름은 [iOS App](/ko/platforms/ios#relay-backed-push-for-official-builds)를, relay 보안 모델은 [Authentication and trust flow](/ko/platforms/ios#authentication-and-trust-flow)를 참조하세요.
  ...
  <Accordion title="Cron 작업 구성">
    ...
    - `runLog`: `cron/runs/<jobId>.jsonl`을 크기와 유지 라인 수 기준으로 정리합니다.
  ...
  <Accordion title="Webhook 설정(hooks)">
    ...
    보안 참고:
    - 모든 hook/Webhook payload 콘텐츠는 신뢰되지 않은 입력으로 취급하세요.
    ...
    - `hooks.path`는 `/`일 수 없습니다. Webhook ingress는 `/hooks` 같은 전용 하위 경로에 두세요.
    - 강하게 범위를 제한한 디버깅이 아닌 한, 안전하지 않은 콘텐츠 우회 플래그(`hooks.gmail.allowUnsafeExternalContent`, `hooks.mappings[].allowUnsafeExternalContent`)는 비활성화 상태로 유지하세요.
    - `hooks.allowRequestSessionKey`를 활성화하는 경우, 호출자가 선택한 세션 키 범위를 제한하기 위해 `hooks.allowedSessionKeyPrefixes`도 설정하세요.
    - hook 기반 에이전트에는 강력한 최신 모델 계층과 엄격한 도구 정책(예: 가능한 경우 메시징 전용 + 샌드박싱)을 권장합니다.
  ...
  <Accordion title="멀티 에이전트 라우팅 구성">
    별도의 워크스페이스와 세션을 가진 여러 개의 격리된 에이전트를 실행합니다.
    ...
    바인딩 규칙 및 에이전트별 액세스 프로필은 [Multi-Agent](/ko/concepts/multi-agent) 및 [전체 참조](/ko/gateway/configuration-reference#multi-agent-routing)를 참조하세요.
  ...
  <Accordion title="config를 여러 파일로 분리($include)">
    ...
    - **단일 파일**: 포함하는 객체를 대체
    - **파일 배열**: 순서대로 deep-merge됨(나중 항목 우선)
    - **형제 키**: include 이후 병합됨(include된 값을 재정의)
...
직접 파일을 편집한 경우 검증될 때까지는 신뢰되지 않은 것으로 처리됩니다. watcher는
에디터의 임시 쓰기/이름 변경 변경이 안정화되기를 기다린 뒤 최종 파일을 읽고,
유효하지 않은 외부 편집은 마지막 정상 config를 복원해 거부합니다. OpenClaw가 수행한
config 쓰기도 저장 전에 동일한 schema 게이트를 사용하며, `gateway.mode`를 제거하거나
파일 크기를 절반 이상 줄이는 등의 파괴적 clobber는 거부되어 검토용 `.rejected.*`로 저장됩니다.

로그에 `Config auto-restored from last-known-good` 또는
`config reload restored last-known-good config`가 보이면, `openclaw.json`
옆의 해당 `.clobbered.*` 파일을 확인하고 거부된 payload를 수정한 뒤
`openclaw config validate`를 실행하세요. 복구 체크리스트는
...
| **`hot`**              | 안전한 변경만 hot apply합니다. 재시작이 필요하면 경고를 기록하며, 처리는 직접 해야 합니다. |
...
<Note>
control-plane 쓰기 RPC(`config.apply`, `config.patch`, `update.run`)는 `deviceId+clientIp` 기준으로 **60초당 3요청**으로 rate limit됩니다. 제한되면 RPC는 `retryAfterMs`와 함께 `UNAVAILABLE`을 반환합니다.
</Note>

안전한/기본 흐름:

- `config.schema.lookup`: 얕은 schema 노드, 일치하는 힌트 메타데이터, 즉시 하위 요약과 함께 하나의 경로 범위 config 하위 트리를 확인
- `config.get`: 현재 스냅샷 + 해시 가져오기
- `config.patch`: 권장되는 부분 업데이트 경로
- `config.apply`: 전체 config 교체 시에만 사용
- `update.run`: 명시적 self-update + 재시작

전체 config를 교체하는 것이 아니라면 `config.schema.lookup`
다음 `config.patch`를 사용하는 것을 권장합니다.

<AccordionGroup>
  <Accordion title="config.apply (전체 교체)">
    전체 config를 검증 + 기록하고 한 단계에서 Gateway를 재시작합니다.
    ...
    - `raw` (문자열) — 전체 config에 대한 JSON5 payload
    - `baseHash` (선택 사항) — `config.get`의 config 해시(config가 존재하면 필요)
    - `sessionKey` (선택 사항) — 재시작 후 wake-up ping용 세션 키
    - `note` (선택 사항) — restart sentinel용 메모
    - `restartDelayMs` (선택 사항) — 재시작 전 지연 시간(기본값 2000)

    이미 보류 중이거나 진행 중인 재시작이 있을 때는 재시작 요청이 병합되며, 재시작 사이클 사이에는 30초 쿨다운이 적용됩니다.
  ...
  <Accordion title="config.patch (부분 업데이트)">
    부분 업데이트를 기존 config에 병합합니다(JSON merge patch semantics).
    ...
    - `raw` (문자열) — 변경할 키만 포함한 JSON5
    - `baseHash` (필수) — `config.get`에서 가져온 config 해시
    - `sessionKey`, `note`, `restartDelayMs` — `config.apply`와 동일

    재시작 동작은 `config.apply`와 동일합니다. 보류 중인 재시작은 병합되며, 재시작 사이클 사이에는 30초 쿨다운이 적용됩니다.
...
Env var 해당값: `OPENCLAW_LOAD_SHELL_ENV=1`
</Accordion>

<Accordion title="config 값에서 env var 치환">
  모든 config 문자열 값에서 `${VAR_NAME}`으로 env vars를 참조할 수 있습니다.
...
_관련 항목: [구성 예제](/ko/gateway/configuration-examples) · [구성 참조](/ko/gateway/configuration-reference) · [Doctor](/ko/gateway/doctor)_
