---
read_when:
    - 문제 해결 허브에서 더 심층적인 진단을 위해 여기로 안내한 경우
    - 정확한 명령이 포함된 안정적인 증상 기반 실행 가이드 섹션이 필요한 경우
summary: Gateway, 채널, 자동화, Nodes, 브라우저를 위한 심층 문제 해결 실행 가이드
title: 문제 해결
x-i18n:
...
- 긴 세션/1M beta 경로가 필요한 모델 실행에서만 요청이 실패함

해결 방법:

1. 해당 모델에서 `context1m`을 비활성화해 일반 컨텍스트 윈도우로 대체합니다.
2. long-context 요청에 적합한 Anthropic 자격 증명을 사용하거나 Anthropic API 키로 전환합니다.
3. Anthropic long-context 요청이 거부될 때도 실행이 계속되도록 fallback 모델을 구성합니다.
...
## 로컬 OpenAI 호환 백엔드는 직접 프로브는 통과하지만 에이전트 실행은 실패함
...
- `messages[...].content: invalid type: sequence, expected a string` → 백엔드가
  구조화된 Chat Completions 콘텐츠 파트를 거부함. 해결: 
  `models.providers.<provider>.models[].compat.requiresStringContent: true` 설정
- 직접적인 작은 요청은 성공하지만, OpenClaw 에이전트 실행은 백엔드/모델
  충돌(예: 일부 `inferrs` 빌드의 Gemma)과 함께 실패함 → OpenClaw 전송은
  이미 올바를 가능성이 높고, 백엔드가 더 큰 에이전트 런타임
  프롬프트 형태에서 실패하는 것임
- 도구를 비활성화하면 실패가 줄어들지만 완전히 사라지지 않음 → 도구 스키마가
  압박의 일부였지만, 남은 문제는 여전히 업스트림 모델/서버
  용량 또는 백엔드 버그임

해결 방법:

1. 문자열 전용 Chat Completions 백엔드에 `compat.requiresStringContent: true`를 설정합니다.
2. OpenClaw의 도구 스키마 표면을 안정적으로 처리할 수 없는 모델/백엔드에는
   `compat.supportsTools: false`를 설정합니다.
3. 가능한 경우 프롬프트 압박을 줄입니다: 더 작은 워크스페이스 bootstrap, 더 짧은
   세션 기록, 더 가벼운 로컬 모델, 또는 더 강한 long-context
   지원을 가진 백엔드 사용.
4. 작은 직접 요청은 계속 통과하지만 OpenClaw 에이전트 턴은 여전히
   백엔드 내부에서 충돌한다면, 이를 업스트림 서버/모델 제한으로 보고
   허용된 페이로드 형태와 함께 그쪽에 재현 사례를 제출하세요.
...
채널은 올라와 있지만 아무 응답도 없으면, 무엇이든 다시 연결하기 전에 라우팅과 정책을 확인하세요.
...
- 장치 식별이 필요한 곳에서 HTTP 사용

일반적인 징후:

- `device identity required` → non-secure context 또는 장치 인증 누락
- `origin not allowed` → 브라우저 `Origin`이 `gateway.controlUi.allowedOrigins`에 없음
  (또는 명시적 허용 목록 없이 non-loopback 브라우저 origin에서 연결 중)
- `device nonce required` / `device nonce mismatch` → 클라이언트가
  challenge 기반 장치 인증 흐름(`connect.challenge` + `device.nonce`)을 완료하지 않음
- `device signature invalid` / `device signature expired` → 클라이언트가 현재 핸드셰이크에 대해 잘못된
  페이로드(또는 오래된 타임스탬프)에 서명함
- `AUTH_TOKEN_MISMATCH` with `canRetryWithDeviceToken=true` → 클라이언트가 캐시된 device token으로 한 번 신뢰된 재시도를 할 수 있음
- 해당 캐시 token 재시도는 페어링된
  device token과 함께 저장된 캐시된 scope 집합을 재사용합니다. 명시적 `deviceToken` / 명시적 `scopes` 호출자는 요청한 scope 집합을 유지합니다.
- 그 재시도 경로 밖에서는 connect 인증 우선순위가 명시적 공유
  token/password 우선, 다음은 명시적 `deviceToken`, 그다음 저장된 device token,
  마지막으로 bootstrap token입니다.
- 비동기 Tailscale Serve Control UI 경로에서는 동일한
  `{scope, ip}`의 실패 시도가 limiter가 실패를 기록하기 전에 직렬화됩니다. 따라서 같은 클라이언트의 잘못된 동시 재시도 두 번은 두 번째 시도에서 단순 불일치 대신 `retry later`를 보일 수 있습니다.
- 브라우저 origin 기반
  loopback 클라이언트에서 `too many failed authentication attempts (retry later)` → 같은 정규화된 `Origin`의 반복 실패가 일시적으로 잠깁니다. 다른 localhost origin은 별도 버킷을 사용합니다.
- 그 재시도 이후 반복되는 `unauthorized` → 공유 token/device token 드리프트. token config를 새로 고치고 필요 시 device token을 재승인/순환하세요.
...
| 상세 코드                   | 의미                                                                                                                                                                                       | 권장 조치                                                                                                                                                                                                                                                                             |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `AUTH_TOKEN_MISSING`        | 클라이언트가 필요한 공유 token을 보내지 않았습니다.                                                                                                                                         | 클라이언트에 token을 붙여넣거나 설정한 뒤 재시도하세요. dashboard 경로의 경우: `openclaw config get gateway.auth.token`을 실행한 뒤 Control UI 설정에 붙여넣으세요.                                                                                                              |
| `AUTH_TOKEN_MISMATCH`       | 공유 token이 gateway 인증 token과 일치하지 않았습니다.                                                                                                                                      | `canRetryWithDeviceToken=true`이면 한 번의 신뢰된 재시도를 허용하세요. 캐시 token 재시도는 저장된 승인 scope를 재사용하며, 명시적 `deviceToken` / `scopes` 호출자는 요청한 scope를 유지합니다. 여전히 실패하면 [token 드리프트 복구 체크리스트](/ko/cli/devices#token-drift-recovery-checklist)를 실행하세요. |
| `AUTH_DEVICE_TOKEN_MISMATCH` | 캐시된 장치별 token이 오래되었거나 폐기되었습니다.                                                                                                                                          | [devices CLI](/ko/cli/devices)를 사용해 device token을 순환/재승인한 뒤 다시 연결하세요.                                                                                                                                                                                               |
| `PAIRING_REQUIRED`          | 장치 식별에 승인이 필요합니다. `error.details.reason`에서 `not-paired`, `scope-upgrade`, `role-upgrade`, `metadata-upgrade`를 확인하고, 존재하면 `requestId` / `remediationHint`를 사용하세요. | 보류 중인 요청 승인: `openclaw devices list` 후 `openclaw devices approve <requestId>`. 범위/역할 업그레이드도 요청된 액세스를 검토한 뒤 같은 흐름을 사용합니다.                                                                                                                   |
...
2. challenge에 바인딩된 페이로드에 서명함
3. 동일한 challenge nonce와 함께 `connect.params.device.nonce`를 전송함

`openclaw devices rotate` / `revoke` / `remove`가 예상과 다르게 거부되면:

- 페어링된 장치 token 세션은 호출자가
  `operator.admin`도 가지고 있지 않은 한 **자기 자신의** 장치만 관리할 수 있습니다
...
- `Other gateway-like services detected (best effort)` → 오래되었거나 병렬인 launchd/systemd/schtasks unit가 존재함. 대부분의 설정은 머신당 하나의 gateway를 유지해야 하며, 둘 이상이 필요하다면 포트 + config/state/workspace를 분리하세요. [/gateway#multiple-gateways-same-host](/ko/gateway#multiple-gateways-same-host)를 참조하세요.
...
- 거부된 config가 시작 또는 hot reload 중 검증에 실패했습니다.
- OpenClaw는 거부된 페이로드를 `.clobbered.*`로 보존했습니다.
- 활성 config는 마지막으로 검증된 last-known-good 사본에서 복원되었습니다.
...
- `.clobbered.*`가 존재함 → 외부 직접 편집 또는 시작 시 읽기가 복원됨.
- `.rejected.*`가 존재함 → OpenClaw가 소유한 config 쓰기가 커밋 전에 스키마 또는 손상 검사에 실패함.
- `Config write rejected:` → 쓰기 시도가 필수 형태를 제거하거나, 파일 크기를 급격히 줄이거나, 유효하지 않은 config를 저장하려고 했음.
- `missing-meta-vs-last-good`, `gateway-mode-missing-vs-last-good`, 또는 `size-drop-vs-last-good:*` → 현재 파일이 last-known-good 백업과 비교해 필드나 크기를 잃어서 시작 시 손상된 것으로 처리됨.
- `Config last-known-good promotion skipped` → 후보에 `***` 같은 마스킹된 시크릿 placeholder가 포함되어 있었음.
...
- 경고가 SSH 대체, 여러 gateway, 누락된 scope, 확인되지 않은 auth ref 중 무엇에 관한 것인지

일반적인 징후:

- `SSH tunnel failed to start; falling back to direct probes.` → SSH 설정에 실패했지만, 명령은 여전히 직접 구성된/loopback 대상을 시도함.
- `multiple reachable gateways detected` → 둘 이상의 대상이 응답함. 보통 의도적인 multi-gateway 설정이거나 오래되었거나 중복된 리스너를 의미함.
- `Read-probe diagnostics are limited by gateway scopes (missing operator.read)` → 연결은 되었지만 detail RPC가 scope에 의해 제한됨. 장치 식별을 페어링하거나 `operator.read`가 있는 자격 증명을 사용하세요.
- `Capability: pairing-pending` 또는 `gateway closed (1008): pairing required` → gateway는 응답했지만, 이 클라이언트는 정상적인 operator 액세스 전에 여전히 페어링/승인이 필요함.
- 확인되지 않은 `gateway.auth.*` / `gateway.remote.*` SecretRef 경고 텍스트 → 실패한 대상에 대해 이 명령 경로에서 인증 자료를 사용할 수 없었음.
...
채널 상태는 연결됨인데 메시지 흐름이 죽어 있으면, 정책, 권한, 채널별 전달 규칙에 집중하세요.
...
- `pairing` / 보류 중 승인 흔적 → 발신자가 승인되지 않음.
...
- `heartbeat skipped` with `reason=quiet-hours` → 활성 시간 범위 밖.
- `heartbeat skipped` with `reason=empty-heartbeat-file` → `HEARTBEAT.md`가 존재하지만 빈 줄 / markdown 헤더만 포함하므로 OpenClaw가 모델 호출을 건너뜀.
- `heartbeat skipped` with `reason=no-tasks-due` → `HEARTBEAT.md`에 `tasks:` 블록이 있지만, 이 tick에서 실행할 작업이 없음.
- `heartbeat: unknown accountId` → Heartbeat 전달 대상에 대한 잘못된 account ID.
- `heartbeat skipped` with `reason=dm-blocked` → Heartbeat 대상이 DM 스타일 대상으로 확인되었지만 `agents.defaults.heartbeat.directPolicy`(또는 에이전트별 재정의)가 `block`으로 설정됨.
...
Node는 페어링되었지만 도구가 실패하면, 포그라운드, 권한, 승인 상태를 분리해서 확인하세요.
...
- 예상 capability와 함께 Node가 온라인 상태인지
- 카메라/마이크/위치/화면에 대한 OS 권한 부여 여부
- Exec 승인 및 허용 목록 상태
...
- `browser.enabled=true`인데 브라우저 도구가 없거나 사용 불가 → `plugins.allow`가 `browser`를 제외하여 Plugin이 로드되지 않음.
...
- `Could not find DevToolsActivePort for chrome` → Chrome MCP existing-session이 선택된 브라우저 데이터 디렉터리에 아직 연결할 수 없음. 브라우저 inspect 페이지를 열고 원격 디버깅을 활성화하고, 브라우저를 열린 상태로 유지하고, 첫 연결 프롬프트를 승인한 뒤 다시 시도하세요. 로그인 상태가 필요 없다면 managed `openclaw` 프로필을 사용하는 것이 좋습니다.
...
- `Playwright is not available in this gateway build; '<feature>' is unsupported.` → 현재 gateway 설치에 번들 브라우저 Plugin의 `playwright-core` 런타임 의존성이 없음. `openclaw doctor --fix`를 실행한 후 gateway를 재시작하세요. ARIA 스냅샷과 기본 페이지 스크린샷은 여전히 동작할 수 있지만, 탐색, AI 스냅샷, CSS selector 요소 스크린샷, PDF 내보내기는 계속 사용할 수 없습니다.
...
- `existing-session file uploads do not support element selectors; use ref/inputRef.` → Chrome MCP 업로드 훅에는 CSS selector가 아니라 스냅샷 ref가 필요함.
- `existing-session file uploads currently support one file at a time.` → Chrome MCP 프로필에서는 호출당 하나의 업로드만 보내세요.
- `existing-session dialog handling does not support timeoutMs.` → Chrome MCP 프로필의 대화상자 훅은 timeout 재정의를 지원하지 않음.
- `response body is not supported for existing-session profiles yet.` → `responsebody`는 여전히 managed 브라우저 또는 raw CDP 프로필이 필요함.
- attach-only 또는 원격 CDP 프로필에서 오래된 viewport / dark-mode / locale / offline 재정의 → `openclaw browser stop --browser-profile <name>`을 실행해 전체 gateway를 재시작하지 않고도 활성 control 세션을 닫고 Playwright/CDP 에뮬레이션 상태를 해제하세요.
...
업그레이드 후 발생하는 대부분의 문제는 config 드리프트이거나 더 엄격한 기본값이 이제 적용되기 때문입니다.
...
- `gateway.mode=remote`이면, 로컬 서비스는 멀쩡한데 CLI 호출이 원격 대상을 향하고 있을 수 있습니다.
...
- 대시보드/Nodes용 보류 중인 장치 승인
- 정책 또는 식별 변경 후 보류 중인 DM 페어링 승인

일반적인 징후:

- `device identity required` → 장치 인증 요구가 충족되지 않음.
- `pairing required` → 발신자/장치 승인이 필요함.

점검 후에도 서비스 config와 런타임이 여전히 다르면, 같은 프로필/상태 디렉터리에서 서비스 메타데이터를 다시 설치하세요.
