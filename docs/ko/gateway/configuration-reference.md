---
read_when:
    - 정확한 필드 수준의 구성 의미나 기본값이 필요할 때
    - 채널, 모델, gateway 또는 도구 구성 블록을 검증하고 있을 때
summary: 모든 OpenClaw 구성 키, 기본값, 채널 설정에 대한 전체 참조
title: 구성 참조
x-i18n:
    generated_at: "2026-04-07T06:06:20Z"
    model: gpt-5.4
    provider: openai
...
구성 형식은 **JSON5**입니다(주석 + 후행 쉼표 허용). 모든 필드는 선택 사항이며, 생략 시 OpenClaw는 안전한 기본값을 사용합니다.
...
| DM 정책             | 동작                                                            |
| ------------------- | --------------------------------------------------------------- |
| `pairing` (기본값)  | 알 수 없는 발신자는 1회용 페어링 코드를 받고, 소유자가 승인해야 함 |
...
provider 블록이 완전히 누락된 경우(`channels.<provider>` 없음), 런타임 그룹 정책은 시작 시 경고와 함께 `allowlist`(기본 차단)로 대체됩니다.
</Note>

### 채널 모델 재정의

특정 채널 ID를 모델에 고정하려면 `channels.modelByChannel`을 사용하세요. 값은 `provider/model` 또는 구성된 모델 별칭을 허용합니다. 채널 매핑은 세션에 모델 재정의가 이미 없을 때만 적용됩니다(예: `/model`로 설정한 경우).
...
provider 전반에서 공유되는 그룹 정책 및 하트비트 동작에는 `channels.defaults`를 사용하세요.
...
- `channels.defaults.contextVisibility`: 모든 채널의 기본 보조 컨텍스트 표시 모드입니다. 값: `all`(기본값, 모든 인용/스레드/기록 컨텍스트 포함), `allowlist`(허용 목록에 있는 발신자의 컨텍스트만 포함), `allowlist_quote`(allowlist와 동일하지만 명시적 인용/답장 컨텍스트는 유지). 채널별 재정의: `channels.<channel>.contextVisibility`.
...
- 발신 명령은 `default` 계정이 있으면 기본적으로 해당 계정을 사용하고, 없으면 첫 번째로 구성된 계정 ID(정렬 순서)를 사용합니다.
- 선택적 `channels.whatsapp.defaultAccount`는 구성된 계정 ID와 일치할 때 이 기본 계정 선택을 재정의합니다.
- 레거시 단일 계정 Baileys 인증 디렉터리는 `openclaw doctor`가 `whatsapp/default`로 마이그레이션합니다.
...
- 다중 계정 구성(계정 ID 2개 이상)에서는 대체 라우팅을 피하기 위해 명시적 기본값(`channels.telegram.defaultAccount` 또는 `channels.telegram.accounts.default`)을 설정하세요. 누락되었거나 유효하지 않으면 `openclaw doctor`가 경고합니다.
- `configWrites: false`는 Telegram에서 시작된 구성 쓰기(슈퍼그룹 ID 마이그레이션, `/config set|unset`)를 차단합니다.
- 최상위 `bindings[]` 항목에서 `type: "acp"`를 사용하면 포럼 토픽에 대한 영구 ACP 바인딩을 구성합니다(`match.peer.id`에는 표준 `chatId:topic:topicId` 사용). 필드 의미는 [ACP Agents](/ko/tools/acp-agents#channel-specific-settings)에서 공유됩니다.
- Telegram 스트림 미리보기는 `sendMessage` + `editMessageText`를 사용합니다(개인 및 그룹 채팅에서 동작).
- 재시도 정책: [Retry policy](/ko/concepts/retry) 참조.
...
- 명시적 Discord `token`을 제공하는 직접 발신 호출은 해당 토큰으로 호출을 수행하며, 계정 재시도/정책 설정은 활성 런타임 스냅샷에서 선택된 계정에서 계속 가져옵니다.
...
- 봇이 작성한 메시지는 기본적으로 무시됩니다. `allowBots: true`로 활성화할 수 있으며, `allowBots: "mentions"`를 사용하면 봇을 멘션한 봇 메시지만 허용합니다(자체 메시지는 계속 필터링).
- `channels.discord.guilds.<id>.ignoreOtherMentions`(및 채널 재정의)는 봇이 아닌 다른 사용자나 역할을 멘션했지만 봇은 멘션하지 않은 메시지를 삭제합니다(@everyone/@here 제외).
- `maxLinesPerMessage`(기본값 17)는 2000자 미만이어도 세로로 긴 메시지를 분할합니다.
- `channels.discord.threadBindings`는 Discord 스레드 바인딩 라우팅을 제어합니다.
  - `enabled`: 스레드 바인딩 세션 기능에 대한 Discord 재정의(`/focus`, `/unfocus`, `/agents`, `/session idle`, `/session max-age`, 바인딩 전달/라우팅)
  - `idleHours`: 비활성 상태 자동 해제의 Discord 재정의 시간(`0`이면 비활성화)
  - `maxAgeHours`: 하드 최대 수명의 Discord 재정의 시간(`0`이면 비활성화)
  - `spawnSubagentSessions`: `sessions_spawn({ thread: true })` 자동 스레드 생성/바인딩 opt-in 스위치
- 최상위 `bindings[]` 항목에서 `type: "acp"`를 사용하면 채널 및 스레드에 대한 영구 ACP 바인딩을 구성합니다(`match.peer.id`에 채널/스레드 ID 사용). 필드 의미는 [ACP Agents](/ko/tools/acp-agents#channel-specific-settings)에서 공유됩니다.
...
- OpenClaw는 추가로 반복적인 복호화 실패 후 음성 세션을 나갔다가 다시 참가하여 음성 수신 복구를 시도합니다.
- `channels.discord.streaming`은 표준 스트림 모드 키입니다. 레거시 `streamMode` 및 불리언 `streaming` 값은 자동 마이그레이션됩니다.
- `channels.discord.autoPresence`는 런타임 가용성을 봇 상태로 매핑합니다(정상 => online, 저하 => idle, 소진 => dnd) 그리고 선택적 상태 텍스트 재정의를 허용합니다.
...
- `channels.discord.execApprovals`: Discord 네이티브 exec 승인 전달 및 승인자 인증.
  ...
  - `agentFilter`: 선택적 agent ID 허용 목록입니다. 생략하면 모든 agent에 대한 승인을 전달합니다.
  - `sessionFilter`: 선택적 세션 키 패턴(부분 문자열 또는 정규식)입니다.
  - `target`: 승인 프롬프트를 보낼 위치입니다. `"dm"`(기본값)은 승인자 DM으로, `"channel"`은 원래 채널로, `"both"`는 둘 다 보냅니다. target에 `"channel"`이 포함되면 버튼은 확인된 승인자만 사용할 수 있습니다.
  - `cleanupAfterResolve`: `true`이면 승인, 거부 또는 시간 초과 후 승인 DM을 삭제합니다.

**리액션 알림 모드:** `off`(없음), `own`(봇의 메시지, 기본값), `all`(모든 메시지), `allowlist`(`guilds.<id>.users`의 사용자가 모든 메시지에 대해 적용).
...
- **소켓 모드**에는 `botToken`과 `appToken`이 모두 필요합니다(기본 계정 환경 변수 대체값: `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`).
- **HTTP 모드**에는 `botToken`과 `signingSecret`(루트 또는 계정별)이 필요합니다.
- `botToken`, `appToken`, `signingSecret`, `userToken`은 일반 문자열 또는 SecretRef 객체를 허용합니다.
- Slack 계정 스냅샷은 `botTokenSource`, `botTokenStatus`, `appTokenStatus`, HTTP 모드에서는 `signingSecretStatus`와 같은 자격 증명별 소스/상태 필드를 노출합니다. `configured_unavailable`은 계정이 SecretRef를 통해 구성되었지만 현재 명령/런타임 경로에서 비밀 값을 확인할 수 없음을 의미합니다.
- `configWrites: false`는 Slack에서 시작된 구성 쓰기를 차단합니다.
- 선택적 `channels.slack.defaultAccount`는 구성된 계정 ID와 일치할 때 기본 계정 선택을 재정의합니다.
- `channels.slack.streaming`은 표준 스트림 모드 키입니다. 레거시 `streamMode` 및 불리언 `streaming` 값은 자동 마이그레이션됩니다.
...
- `typingReaction`은 답변 실행 중에 수신 Slack 메시지에 임시 리액션을 추가하고 완료 시 제거합니다. `"hourglass_flowing_sand"`와 같은 Slack 이모지 쇼트코드를 사용하세요.
...
- 비공개/tailnet/internal 콜백 호스트의 경우 Mattermost에서 `ServiceSettings.AllowedUntrustedInternalConnections`에 콜백 호스트/도메인을 포함해야 할 수 있습니다.
...
- 최상위 `bindings[]` 항목에서 `type: "acp"`를 사용하면 BlueBubbles 대화를 영구 ACP 세션에 바인딩할 수 있습니다. `match.peer.id`에는 BlueBubbles handle 또는 대상 문자열(`chat_id:*`, `chat_guid:*`, `chat_identifier:*`)을 사용하세요. 공유 필드 의미: [ACP Agents](/ko/tools/acp-agents#channel-specific-settings).
...
- 가능하면 `chat_id:<id>` 대상을 사용하세요. 채팅 목록을 보려면 `imsg chats --limit 20`을 사용하세요.
...
- 최상위 `bindings[]` 항목에서 `type: "acp"`를 사용하면 iMessage 대화를 영구 ACP 세션에 바인딩할 수 있습니다. `match.peer.id`에는 정규화된 handle 또는 명시적 채팅 대상(`chat_id:*`, `chat_guid:*`, `chat_identifier:*`)을 사용하세요. 공유 필드 의미: [ACP Agents](/ko/tools/acp-agents#channel-specific-settings).
...
- `channels.matrix.network.dangerouslyAllowPrivateNetwork`는 비공개/internal homeserver를 허용합니다. `proxy`와 이 네트워크 opt-in은 서로 독립적인 제어입니다.
...
- `channels.matrix.dm.sessionScope`는 Matrix DM이 세션으로 그룹화되는 방식을 제어합니다. `per-user`(기본값)는 라우팅된 peer 기준으로 공유하고, `per-room`은 각 DM 룸을 분리합니다.
...
- `default`는 `accountId`가 생략되면 사용됩니다(CLI + 라우팅).
- 환경 변수 토큰은 **기본** 계정에만 적용됩니다.
- 기본 채널 설정은 계정별로 재정의되지 않는 한 모든 계정에 적용됩니다.
- 각 계정을 다른 agent로 라우팅하려면 `bindings[].match.accountId`를 사용하세요.
- 단일 계정 최상위 채널 구성 상태에서 `openclaw channels add`(또는 채널 온보딩)로 비기본 계정을 추가하면, OpenClaw는 먼저 계정 범위의 최상위 단일 계정 값을 채널 계정 맵으로 승격하여 원래 계정이 계속 동작하도록 합니다. 대부분의 채널은 이를 `channels.<channel>.accounts.default`로 이동하며, Matrix는 기존 일치하는 named/default 대상을 유지할 수 있습니다.
- 기존 채널 전용 바인딩(`accountId` 없음)은 기본 계정과 계속 매칭되며, 계정 범위 바인딩은 계속 선택 사항입니다.
- `openclaw doctor --fix`도 혼합된 구조를 복구하여 계정 범위의 최상위 단일 계정 값을 해당 채널에 대해 선택된 승격 계정으로 이동합니다. 대부분 채널은 `accounts.default`를 사용하며, Matrix는 기존 일치하는 named/default 대상을 유지할 수 있습니다.
...
- 멘션 게이팅은 감지가 가능한 경우(네이티브 멘션이 있거나 패턴이 최소 하나 이상 있는 경우)에만 적용됩니다.
...
- `native: "auto"`는 Discord/Telegram에서 네이티브 명령을 켜고 Slack은 꺼 둡니다.
...
- `bash: true`는 호스트 셸에 대한 `! <cmd>`를 활성화합니다. `tools.elevated.enabled`가 필요하며 발신자가 `tools.elevated.allowFrom.<channel>`에 있어야 합니다.
...
- `allowFrom`은 provider별입니다. 설정되면 이것이 **유일한** 인증 소스가 됩니다(채널 허용 목록/페어링 및 `useAccessGroups`는 무시됨).
- `useAccessGroups: false`는 `allowFrom`이 설정되지 않은 경우 명령이 접근 그룹 정책을 우회하도록 허용합니다.
...
시스템 프롬프트의 Runtime 줄에 표시되는 선택적 리포지토리 루트입니다. 설정하지 않으면 OpenClaw가 workspace에서 상위로 탐색하여 자동 감지합니다.
...
`agents.list[].skills`를 설정하지 않은 agent에 대한 선택적 기본 Skills 허용 목록입니다.
...
- Skills를 없애려면 `agents.list[].skills: []`로 설정하세요.
...
workspace 부트스트랩 파일을 시스템 프롬프트에 주입하는 시점을 제어합니다. 기본값: `"always"`.

- `"continuation-skip"`: 완료된 assistant 응답 이후의 안전한 연속 턴에서는 workspace 부트스트랩 재주입을 건너뛰어 프롬프트 크기를 줄입니다. 하트비트 실행과 압축 후 재시도는 여전히 컨텍스트를 다시 빌드합니다.
...
모든 workspace 부트스트랩 파일 전체에 주입되는 총 최대 문자 수입니다. 기본값: `150000`.
...
부트스트랩 컨텍스트가 잘렸을 때 agent에게 보이는 경고 텍스트를 제어합니다.
기본값: `"once"`.
...
값을 낮추면 보통 비전 토큰 사용량과 요청 페이로드 크기가 줄어듭니다.
...
- `imageModel`: 문자열(`"provider/model"`) 또는 객체(`{ primary, fallbacks }`)를 허용합니다.
  ...
  - 선택된/기본 모델이 이미지 입력을 처리할 수 없을 때 대체 라우팅으로도 사용됩니다.
- `imageGenerationModel`: 문자열(`"provider/model"`) 또는 객체(`{ primary, fallbacks }`)를 허용합니다.
  - 공유 이미지 생성 기능 및 향후 이미지 생성 도구/plugin 표면에서 사용됩니다.
  - 일반적인 값: 기본 Gemini 이미지 생성을 위한 `google/gemini-3.1-flash-image-preview`, fal용 `fal/fal-ai/flux/dev`, OpenAI Images용 `openai/gpt-image-1`.
  - provider/model을 직접 선택하면 해당 provider 인증/API 키도 함께 구성하세요(예: `google/*`에는 `GEMINI_API_KEY` 또는 `GOOGLE_API_KEY`, `openai/*`에는 `OPENAI_API_KEY`, `fal/*`에는 `FAL_KEY`).
  - 생략해도 `image_generate`는 인증이 설정된 provider 기본값을 추론할 수 있습니다. 먼저 현재 기본 provider를 시도한 다음, 등록된 나머지 이미지 생성 provider를 provider ID 순서로 시도합니다.
- `musicGenerationModel`: 문자열(`"provider/model"`) 또는 객체(`{ primary, fallbacks }`)를 허용합니다.
  ...
  - 생략해도 `music_generate`는 인증이 설정된 provider 기본값을 추론할 수 있습니다. 먼저 현재 기본 provider를 시도한 다음, 등록된 나머지 음악 생성 provider를 provider ID 순서로 시도합니다.
  - provider/model을 직접 선택하면 해당 provider 인증/API 키도 함께 구성하세요.
- `videoGenerationModel`: 문자열(`"provider/model"`) 또는 객체(`{ primary, fallbacks }`)를 허용합니다.
  ...
  - 생략해도 `video_generate`는 인증이 설정된 provider 기본값을 추론할 수 있습니다. 먼저 현재 기본 provider를 시도한 다음, 등록된 나머지 비디오 생성 provider를 provider ID 순서로 시도합니다.
  - provider/model을 직접 선택하면 해당 provider 인증/API 키도 함께 구성하세요.
...
- `verboseDefault`: agent의 기본 자세한 출력 수준입니다. 값: `"off"`, `"on"`, `"full"`. 기본값: `"off"`.
- `elevatedDefault`: agent의 기본 상승 출력 수준입니다. 값: `"off"`, `"on"`, `"ask"`, `"full"`. 기본값: `"on"`.
- `model.primary`: 형식은 `provider/model`(예: `openai/gpt-5.4`). provider를 생략하면 OpenClaw는 먼저 별칭을 시도하고, 그다음 정확한 모델 ID에 대해 고유하게 구성된 provider 일치를 시도한 뒤, 마지막으로 구성된 기본 provider로 대체합니다(더 이상 권장되지 않는 호환 동작이므로 명시적인 `provider/model` 사용 권장). 해당 provider가 더 이상 구성된 기본 모델을 노출하지 않으면, OpenClaw는 오래된 제거된 provider 기본값을 보여주는 대신 첫 번째로 구성된 provider/model로 대체합니다.
- `models`: 구성된 모델 카탈로그 및 `/model`용 허용 목록입니다. 각 항목에는 `alias`(바로가기)와 `params`(예: `temperature`, `maxTokens`, `cacheRetention`, `context1m` 같은 provider별 값)가 포함될 수 있습니다.
- `params`: 모든 모델에 적용되는 전역 기본 provider 매개변수입니다. `agents.defaults.params`에 설정합니다(예: `{ cacheRetention: "long" }`).
- `params` 병합 우선순위(구성): `agents.defaults.params`(전역 기반)는 `agents.defaults.models["provider/model"].params`(모델별)로 재정의되며, 이후 `agents.list[].params`(일치하는 agent ID)가 키별로 다시 재정의합니다. 자세한 내용은 [Prompt Caching](/ko/reference/prompt-caching)을 참조하세요.
- 이 필드를 변경하는 구성 작성기(예: `/models set`, `/models set-image`, fallback 추가/제거 명령)는 가능한 경우 표준 객체 형식으로 저장하고 기존 fallback 목록을 보존합니다.
- `maxConcurrent`: 세션 간 최대 병렬 agent 실행 수입니다(각 세션은 여전히 직렬화됨). 기본값: 4.
...
텍스트 전용 대체 실행(도구 호출 없음)을 위한 선택적 CLI 백엔드입니다. API provider 실패 시 백업으로 유용합니다.
...
- `suppressToolErrorWarnings`: true이면 하트비트 실행 중 도구 오류 경고 페이로드를 억제합니다.
- `directPolicy`: 직접/DM 전달 정책입니다. `allow`(기본값)는 직접 대상 전달을 허용합니다. `block`은 직접 대상 전달을 억제하고 `reason=dm-blocked`를 발생시킵니다.
- `lightContext`: true이면 하트비트 실행이 경량 부트스트랩 컨텍스트를 사용하고 workspace 부트스트랩 파일 중 `HEARTBEAT.md`만 유지합니다.
- `isolatedSession`: true이면 각 하트비트 실행이 이전 대화 기록이 없는 새 세션에서 실행됩니다. cron `sessionTarget: "isolated"`와 동일한 격리 패턴입니다. 하트비트당 토큰 비용을 약 100K에서 약 2-5K 토큰으로 줄입니다.
- agent별: `agents.list[].heartbeat`를 설정하세요. 어떤 agent라도 `heartbeat`를 정의하면 **그 agent들만** 하트비트를 실행합니다.
- 하트비트는 전체 agent 턴을 실행하므로 간격이 짧을수록 더 많은 토큰을 소모합니다.
  ...
  agents: {
    defaults: {
      compaction: {
        ...
        model: "openrouter/anthropic/claude-sonnet-4-6", // 선택적 압축 전용 모델 재정의
        notifyUser: true, // 압축 시작 시 사용자에게 짧은 알림 전송(기본값: false)
        memoryFlush: {
          enabled: true,
          softThresholdTokens: 6000,
          systemPrompt: "세션이 곧 압축됩니다. 지금 영구 메모리를 저장하세요.",
          prompt: "지속되어야 할 메모리를 memory/YYYY-MM-DD.md에 작성하세요. 저장할 내용이 없으면 정확한 무응답 토큰 NO_REPLY로 답하세요.",
        },
      },
    },
  },
...
- `mode`: `default` 또는 `safeguard`(긴 기록에 대한 청크 요약). [Compaction](/ko/concepts/compaction) 참조.
- `timeoutSeconds`: OpenClaw가 중단하기 전 단일 압축 작업에 허용되는 최대 시간(초)입니다. 기본값: `900`.
- `identifierPolicy`: `strict`(기본값), `off`, 또는 `custom`. `strict`는 압축 요약 시 내장된 불투명 식별자 보존 지침을 앞에 추가합니다.
- `identifierInstructions`: `identifierPolicy=custom`일 때 사용하는 선택적 사용자 지정 식별자 보존 텍스트입니다.
- `postCompactionSections`: 압축 후 다시 주입할 AGENTS.md의 H2/H3 섹션 이름 선택 목록입니다. 기본값은 `["Session Startup", "Red Lines"]`이며, 비활성화하려면 `[]`로 설정하세요. 설정하지 않았거나 명시적으로 이 기본 쌍으로 설정한 경우 이전의 `Every Session`/`Safety` 제목도 레거시 대체로 허용됩니다.
- `model`: 압축 요약에만 적용되는 선택적 `provider/model-id` 재정의입니다. 메인 세션은 한 모델을 유지하되 압축 요약은 다른 모델로 실행하고 싶을 때 사용하세요. 설정되지 않으면 압축은 세션의 기본 모델을 사용합니다.
- `notifyUser`: `true`이면 압축이 시작될 때 사용자에게 짧은 알림(예: `"Compacting context..."`)을 보냅니다. 기본적으로는 압축을 조용히 유지하기 위해 비활성화됩니다.
- `memoryFlush`: 자동 압축 전 조용한 agent 턴으로 영구 메모리를 저장합니다. workspace가 읽기 전용이면 건너뜁니다.

### `agents.defaults.contextPruning`

LLM에 보내기 전에 메모리 내 컨텍스트에서 **오래된 도구 결과**를 가지치기합니다. 디스크의 세션 기록은 수정하지 않습니다.
  ...
  agents: {
    defaults: {
      contextPruning: {
        ...
        hardClear: { enabled: true, placeholder: "[오래된 도구 결과 내용이 제거됨]" },
...
- 가지치기는 먼저 과도하게 큰 도구 결과를 소프트 트림하고, 필요하면 더 오래된 도구 결과를 하드 클리어합니다.
...
- 이미지 블록은 절대 잘리거나 제거되지 않습니다.
- 비율은 정확한 토큰 수가 아니라 문자 기반의 근사값입니다.
...
동작 및 청킹 세부 사항은 [Streaming](/ko/concepts/streaming)을 참조하세요.
...
내장 agent용 선택적 샌드박싱입니다. 전체 가이드는 [Sandboxing](/ko/gateway/sandboxing)을 참조하세요.
...
- `identityData` / `certificateData` / `knownHostsData`: 런타임에 OpenClaw가 임시 파일로 물질화하는 인라인 내용 또는 SecretRef
...
- SecretRef 기반 `*Data` 값은 샌드박스 세션이 시작되기 전에 활성 비밀 런타임 스냅샷에서 해결됨

**SSH 백엔드 동작:**

- 생성 또는 재생성 후 원격 workspace를 한 번 시드함
- 그 이후 원격 SSH workspace를 기준으로 유지함
...
- `ro`: 샌드박스 workspace는 `/workspace`, agent workspace는 `/agent`에 읽기 전용으로 마운트
...
- `mirror`: exec 전에 로컬에서 원격으로 시드하고 exec 후 다시 동기화; 로컬 workspace가 기준으로 유지됨
- `remote`: 샌드박스 생성 시 한 번만 원격으로 시드한 뒤, 원격 workspace를 기준으로 유지

`remote` 모드에서는 OpenClaw 외부에서 수행한 호스트 로컬 편집은 시드 단계 이후 자동으로 샌드박스로 동기화되지 않습니다.
전송은 OpenShell 샌드박스로의 SSH를 사용하지만, plugin이 샌드박스 수명 주기와 선택적 mirror sync를 소유합니다.

**`setupCommand`**는 컨테이너 생성 후 한 번 실행됩니다(`sh -lc` 경유). 네트워크 송신, 쓰기 가능한 루트, 루트 사용자가 필요합니다.

**컨테이너 기본값은 `network: "none"`**입니다. agent에 외부 접근이 필요하면 `"bridge"`(또는 사용자 지정 브리지 네트워크)로 설정하세요.
...
**`docker.binds`**는 추가 호스트 디렉터리를 마운트하며, 전역 및 agent별 binds는 병합됩니다.

**샌드박스 브라우저**(`sandbox.browser.enabled`): 컨테이너 내 Chromium + CDP. noVNC URL이 시스템 프롬프트에 주입됩니다. `openclaw.json`에서 `browser.enabled`는 필요하지 않습니다.
noVNC 관찰자 접근은 기본적으로 VNC 인증을 사용하며 OpenClaw는 공유 URL에 비밀번호를 노출하는 대신 짧은 수명의 토큰 URL을 발급합니다.

- `allowHostControl: false`(기본값)는 샌드박스 세션이 호스트 브라우저를 대상으로 지정하는 것을 차단합니다.
...
- 실행 기본값은 `scripts/sandbox-browser-entrypoint.sh`에 정의되어 있으며 컨테이너 호스트에 맞게 조정되어 있습니다.
  ...
  - `--disable-extensions`(기본 활성)
  - `--disable-3d-apis`, `--disable-software-rasterizer`, `--disable-gpu`는 기본 활성화되어 있으며, WebGL/3D 사용에 필요하면 `OPENCLAW_BROWSER_DISABLE_GRAPHICS_FLAGS=0`으로 비활성화할 수 있습니다.
  - 워크플로우가 확장 기능에 의존하면 `OPENCLAW_BROWSER_DISABLE_EXTENSIONS=0`으로 다시 활성화할 수 있습니다.
...
브라우저 샌드박싱과 `sandbox.docker.binds`는 현재 Docker에서만 지원됩니다.
...
- `id`: 안정적인 agent ID(필수).
- `default`: 여러 개가 설정되면 첫 번째가 우선합니다(경고 기록). 아무 것도 없으면 목록의 첫 항목이 기본값입니다.
...
- `params`: `agents.defaults.models`의 선택된 모델 항목 위에 병합되는 agent별 스트림 params입니다. 전체 모델 카탈로그를 중복하지 않고 `cacheRetention`, `temperature`, `maxTokens` 같은 agent별 재정의에 사용하세요.
- `skills`: 선택적 agent별 Skills 허용 목록입니다. 생략하면 agent는 설정된 `agents.defaults.skills`를 상속합니다. 명시적인 목록은 병합하지 않고 기본값을 대체하며, `[]`는 Skills 없음입니다.
- `thinkingDefault`: 선택적 agent별 기본 thinking 수준(`off | minimal | low | medium | high | xhigh | adaptive`)입니다. 메시지별 또는 세션 재정의가 없을 때 이 agent에 대해 `agents.defaults.thinkingDefault`를 재정의합니다.
- `reasoningDefault`: 선택적 agent별 기본 reasoning 가시성(`on | off | stream`)입니다. 메시지별 또는 세션 reasoning 재정의가 없을 때 적용됩니다.
- `fastModeDefault`: 선택적 agent별 기본 fast mode(`true | false`)입니다. 메시지별 또는 세션 fast-mode 재정의가 없을 때 적용됩니다.
- `runtime`: 선택적 agent별 런타임 설명자입니다. agent가 기본적으로 ACP harness 세션을 사용해야 하는 경우 `type: "acp"`와 함께 `runtime.acp` 기본값(`agent`, `backend`, `mode`, `cwd`)을 사용하세요.
...
- `subagents.allowAgents`: `sessions_spawn`용 agent ID 허용 목록(`["*"]` = 아무 agent나 허용; 기본값: 동일 agent만).
- 샌드박스 상속 보호: 요청 세션이 샌드박스에 있으면 `sessions_spawn`은 샌드박스 없이 실행될 대상을 거부합니다.
- `subagents.requireAgentId`: true이면 `agentId` 없이 `sessions_spawn` 호출을 차단합니다(명시적 프로필 선택 강제, 기본값: false).

---

## 다중 agent 라우팅

하나의 Gateway 안에서 여러 개의 격리된 agent를 실행합니다. [Multi-Agent](/ko/concepts/multi-agent)를 참조하세요.
...
- `type`(선택 사항): 일반 라우팅에는 `route`(없으면 route 기본값), 영구 ACP 대화 바인딩에는 `acp`
- `match.channel`(필수)
...
`type: "acp"` 항목의 경우 OpenClaw는 정확한 대화 ID(`match.channel` + account + `match.peer.id`)로 해결하며 위의 route 바인딩 계층 순서를 사용하지 않습니다.
...
```json5
{
  session: {
    ...
    parentForkMaxTokens: 100000, // 이 토큰 수를 넘으면 상위 스레드 포크 건너뜀(0이면 비활성화)
    maintenance: {
    ...
    threadBindings: {
      enabled: true,
      idleHours: 24, // 기본 비활성 자동 해제 시간(`0`이면 비활성화)
...
- **`scope`**: 그룹 채팅 컨텍스트의 기본 세션 그룹화 전략.
  ...
  - `global`: 채널 컨텍스트의 모든 참가자가 하나의 세션을 공유함(공유 컨텍스트가 의도된 경우에만 사용).
- **`dmScope`**: DM을 그룹화하는 방식.
  - `main`: 모든 DM이 메인 세션을 공유.
  - `per-peer`: 채널 전반에서 발신자 ID별로 분리.
  - `per-channel-peer`: 채널 + 발신자별로 분리(다중 사용자 받은편지함에 권장).
  - `per-account-channel-peer`: 계정 + 채널 + 발신자별로 분리(다중 계정에 권장).
- **`identityLinks`**: 채널 간 세션 공유를 위한 표준 ID와 provider 접두사 peer의 매핑.
- **`reset`**: 기본 리셋 정책. `daily`는 로컬 시간 `atHour`에 리셋되고, `idle`은 `idleMinutes` 후 리셋됩니다. 둘 다 구성된 경우 먼저 만료되는 것이 우선합니다.
- **`resetByType`**: 유형별 재정의(`direct`, `group`, `thread`). 레거시 `dm`은 `direct`의 별칭으로 허용됩니다.
- **`parentForkMaxTokens`**: 포크된 스레드 세션 생성 시 허용되는 상위 세션의 최대 `totalTokens`입니다(기본값 `100000`).
  - 상위 `totalTokens`가 이 값을 넘으면 OpenClaw는 상위 transcript 기록을 상속하는 대신 새 스레드 세션을 시작합니다.
  - 이 보호 기능을 비활성화하고 항상 상위 포크를 허용하려면 `0`으로 설정하세요.
- **`mainKey`**: 레거시 필드입니다. 런타임은 이제 메인 direct-chat 버킷에 항상 `"main"`을 사용합니다.
- **`agentToAgent.maxPingPongTurns`**: agent 간 교환 중 최대 응답 왕복 턴 수(정수, 범위: `0`–`5`). `0`은 핑퐁 체이닝을 비활성화합니다.
- **`sendPolicy`**: `channel`, `chatType`(`direct|group|channel`, 레거시 `dm` 별칭 포함), `keyPrefix`, `rawKeyPrefix`로 매치합니다. 첫 번째 deny가 우선합니다.
- **`maintenance`**: 세션 저장소 정리 + 보존 제어.
  - `mode`: `warn`은 경고만 출력하고, `enforce`는 실제 정리를 수행합니다.
  ...
  - `resetArchiveRetention`: `*.reset.<timestamp>` transcript 아카이브의 보존 기간. 기본값은 `pruneAfter`이며, 비활성화하려면 `false`로 설정합니다.
...
- **`threadBindings`**: 스레드 바인딩 세션 기능의 전역 기본값.
  ...
  - `idleHours`: 비활성 자동 해제의 기본 시간(`0`이면 비활성화; provider 재정의 가능)
  - `maxAgeHours`: 하드 최대 수명의 기본 시간(`0`이면 비활성화; provider 재정의 가능)
...
해결 순서(가장 구체적인 것 우선): account → channel → global. `""`는 비활성화하고 상위 단계 전파도 중단합니다. `"auto"`는 `[{identity.name}]`을 파생합니다.
...
- `messages.statusReactions.enabled`: Slack, Discord, Telegram에서 수명 주기 상태 리액션을 활성화합니다.
  Slack과 Discord에서는 설정하지 않으면 ack 리액션이 활성일 때 상태 리액션도 활성 상태로 유지합니다.
...
- `openai.baseUrl`이 OpenAI가 아닌 엔드포인트를 가리키면 OpenClaw는 이를 OpenAI 호환 TTS 서버로 간주하고 model/voice 검증을 완화합니다.
...
- `providers.*.apiKey`는 일반 문자열 또는 SecretRef 객체를 허용합니다.
- `ELEVENLABS_API_KEY` 대체는 Talk API 키가 구성되지 않았을 때만 적용됩니다.
- `providers.*.voiceAliases`를 사용하면 Talk 지시문에서 친숙한 이름을 사용할 수 있습니다.
- `silenceTimeoutMs`는 사용자가 말을 멈춘 후 Talk 모드가 transcript를 보내기까지 기다리는 시간을 제어합니다. 설정하지 않으면 플랫폼 기본 일시정지 창을 유지합니다(`macOS와 Android에서는 700 ms, iOS에서는 900 ms`).
...
로컬 온보딩은 설정되지 않은 새 로컬 구성에 기본적으로 `tools.profile: "coding"`을 적용합니다(기존의 명시적 프로필은 유지).
...
| 그룹               | 도구                                                                                                                    |
| ------------------ | ----------------------------------------------------------------------------------------------------------------------- |
| `group:runtime`    | `exec`, `process`, `code_execution` (`bash`는 `exec`의 별칭으로 허용)                                                   |
...
- agent별 재정의(`agents.list[].tools.elevated`)는 추가 제한만 가능합니다.
- `/elevated on|off|ask|full`은 상태를 세션별로 저장하며, 인라인 지시문은 단일 메시지에만 적용됩니다.
- 상승된 `exec`는 샌드박싱을 우회하며 구성된 이스케이프 경로(`gateway`가 기본값, exec 대상이 `node`일 때는 `node`)를 사용합니다.
...
설정은 전역 `tools.loopDetection`에 정의할 수 있고 agent별로 `agents.list[].tools.loopDetection`에서 재정의할 수 있습니다.
...
- `historySize`: 루프 분석을 위해 유지할 최대 도구 호출 기록 수.
- `warningThreshold`: 진행 없는 반복 패턴에 대해 경고를 발생시키는 임계값.
- `criticalThreshold`: 치명적인 루프를 차단하는 더 높은 반복 임계값.
- `globalCircuitBreakerThreshold`: 진행 없는 실행 전체에 대한 하드 중단 임계값.
- `detectors.genericRepeat`: 동일 도구/동일 인자 호출 반복에 대해 경고.
- `detectors.knownPollNoProgress`: 알려진 poll 도구(`process.poll`, `command_status` 등)의 진행 없는 반복에 대해 경고/차단.
- `detectors.pingPong`: 진행 없는 교대 쌍 패턴에 대해 경고/차단.
...
- `asyncCompletion.directSend`: `true`이면 완료된 비동기 `music_generate` 및 `video_generate` 작업이 먼저 채널로 직접 전달을 시도합니다. 기본값: `false`(기존 요청자 세션 wake/model-delivery 경로).
...
- `agent`: 현재 agent ID에 속한 모든 세션(같은 agent ID 아래에서 발신자별 세션을 실행하면 다른 사용자도 포함될 수 있음).
- `all`: 모든 세션. 교차 agent 대상 지정에는 여전히 `tools.agentToAgent`가 필요합니다.
- 샌드박스 클램프: 현재 세션이 샌드박스 환경이고 `agents.defaults.sandbox.sessionToolsVisibility="spawned"`이면 `tools.sessions.visibility="all"`이어도 visibility는 강제로 `tree`가 됩니다.
...
```json5
{
  tools: {
    sessions_spawn: {
      attachments: {
        enabled: false, // opt-in: 인라인 파일 첨부를 허용하려면 true 설정
...
- 파일은 자식 workspace의 `.openclaw/attachments/<uuid>/`에 `.manifest.json`과 함께 물질화됩니다.
- 첨부 내용은 transcript 저장에서 자동으로 redaction됩니다.
...
실험적 내장 도구 플래그입니다. 런타임별 자동 활성화 규칙이 적용되지 않는 한 기본적으로 꺼져 있습니다.
...
- 기본값: OpenAI가 아닌 provider는 `false`. OpenAI 및 OpenAI Codex 실행은 자동으로 활성화합니다.
- 활성화되면 시스템 프롬프트에 사용 지침도 추가되어, 모델이 상당한 작업에만 이를 사용하고 한 번에 최대 한 단계만 `in_progress` 상태로 유지하도록 합니다.
...
- `allowAgents`: 요청 agent가 자체 `subagents.allowAgents`를 설정하지 않았을 때 `sessions_spawn`의 대상 agent ID 기본 허용 목록(`["*"]` = 아무 agent나 허용; 기본값: 동일 agent만).
...
OpenClaw는 내장 모델 카탈로그를 사용합니다. 사용자 지정 provider는 구성 또는 `~/.openclaw/agents/<agentId>/agent/models.json`의 `models.providers`를 통해 추가할 수 있습니다.
...
- agent 구성 루트는 `OPENCLAW_AGENT_DIR`(또는 레거시 환경 변수 별칭 `PI_CODING_AGENT_DIR`)로 재정의할 수 있습니다.
- 일치하는 provider ID에 대한 병합 우선순위:
  ...
  - SecretRef 관리 provider의 `apiKey` 값은 해결된 비밀을 저장하는 대신 소스 마커(`ENV_VAR_NAME`은 env ref, `secretref-managed`는 file/exec ref)에서 새로 고칩니다.
  - SecretRef 관리 provider header 값도 소스 마커(`secretref-env:ENV_VAR_NAME`은 env ref, `secretref-managed`는 file/exec ref)에서 새로 고칩니다.
  - 비어 있거나 없는 agent `apiKey`/`baseUrl`는 config의 `models.providers`로 대체됩니다.
  - 일치하는 모델의 `contextWindow`/`maxTokens`는 명시적 config와 암시적 카탈로그 값 중 더 큰 값을 사용합니다.
  - 일치하는 모델의 `contextTokens`는 명시적 런타임 캡이 있으면 이를 유지합니다. 네이티브 모델 메타데이터를 바꾸지 않고 유효 컨텍스트를 제한하려면 이를 사용하세요.
  - config가 `models.json`을 완전히 다시 쓰게 하려면 `models.mode: "replace"`를 사용하세요.
  - 마커 저장은 소스 권한적입니다. 마커는 해결된 런타임 비밀 값이 아니라 활성 소스 구성 스냅샷(해결 전)에서 기록됩니다.
...
- `models.providers.*.apiKey`: provider 자격 증명(가능하면 SecretRef/env 대체 사용 권장).
...
- `models.providers.*.authHeader`: 필요 시 `Authorization` 헤더를 통한 자격 증명 전송 강제.
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: 선택적 호환성 힌트입니다. `api: "openai-completions"`이고 비어 있지 않은 비네이티브 `baseUrl`(호스트가 `api.openai.com`이 아님)일 경우 OpenClaw는 런타임에 이를 강제로 `false`로 설정합니다. `baseUrl`이 비어 있거나 생략되면 기본 OpenAI 동작을 유지합니다.
- `plugins.entries.amazon-bedrock.config.discovery`: Bedrock 자동 감지 설정 루트.
- `plugins.entries.amazon-bedrock.config.discovery.enabled`: 암시적 감지의 켜기/끄기.
- `plugins.entries.amazon-bedrock.config.discovery.region`: 감지에 사용할 AWS 리전.
- `plugins.entries.amazon-bedrock.config.discovery.providerFilter`: 대상 감지를 위한 선택적 provider-id 필터.
- `plugins.entries.amazon-bedrock.config.discovery.refreshInterval`: 감지 새로 고침의 폴링 간격.
- `plugins.entries.amazon-bedrock.config.discovery.defaultContextWindow`: 감지된 모델의 대체 컨텍스트 창.
- `plugins.entries.amazon-bedrock.config.discovery.defaultMaxTokens`: 감지된 모델의 대체 최대 출력 토큰 수.
...
Cerebras에는 `cerebras/zai-glm-4.7`을, Z.AI 직접 연결에는 `zai/glm-4.7`을 사용하세요.
...
네이티브 Moonshot 엔드포인트는 공유 `openai-completions` 전송에서 스트리밍 사용 호환성을 알리며, OpenClaw는 이제 이를 내장 provider ID만이 아니라 엔드포인트 기능을 기준으로 판단합니다.
...
Base URL에는 `/v1`를 포함하지 마세요(Anthropic 클라이언트가 이를 추가함). 바로가기: `openclaw onboard --auth-choice synthetic-api-key`.
...
이제 모델 카탈로그의 기본값은 M2.7 בלבד입니다.
Anthropic 호환 스트리밍 경로에서 OpenClaw는 사용자가 직접 `thinking`을 설정하지 않는 한 MiniMax thinking을 기본적으로 비활성화합니다. `/fast on` 또는 `params.fastMode: true`는 `MiniMax-M2.7`을 `MiniMax-M2.7-highspeed`로 다시 씁니다.

</Accordion>

<Accordion title="로컬 모델 (LM Studio)">

[Local Models](/ko/gateway/local-models)를 참조하세요. 요약: 충분한 하드웨어에서 LM Studio Responses API를 통해 대형 로컬 모델을 실행하고, fallback을 위해 호스팅 모델은 병합 상태로 유지하세요.
...
- `install.preferBrew`: true이고 `brew`를 사용할 수 있으면 다른 설치 방식으로 대체하기 전에 Homebrew 설치를 우선합니다.
- `install.nodeManager`: `metadata.openclaw.install` 스펙용 Node 설치 관리자 우선순위(`npm` | `pnpm` | `yarn` | `bun`).
- `entries.<skillKey>.enabled: false`는 번들/설치된 skill이라도 비활성화합니다.
- `entries.<skillKey>.apiKey`: 기본 환경 변수를 선언하는 Skills를 위한 편의 필드입니다(일반 문자열 또는 SecretRef 객체).
...
- 감지는 네이티브 OpenClaw plugins와 호환되는 Codex 번들 및 Claude 번들을 허용하며, manifest가 없는 Claude 기본 레이아웃 번들도 포함합니다.
- **구성 변경에는 gateway 재시작이 필요합니다.**
- `allow`: 선택적 허용 목록(목록에 있는 plugins만 로드). `deny`가 우선합니다.
- `plugins.entries.<id>.apiKey`: plugin에서 지원하는 경우 plugin 수준 API 키 편의 필드입니다.
- `plugins.entries.<id>.env`: plugin 범위 환경 변수 맵.
- `plugins.entries.<id>.hooks.allowPromptInjection`: `false`이면 core가 `before_prompt_build`를 차단하고 레거시 `before_agent_start`의 프롬프트 변경 필드는 무시하되, 레거시 `modelOverride`와 `providerOverride`는 유지합니다. 네이티브 plugin hook 및 지원되는 번들 제공 hook 디렉터리에 적용됩니다.
...
- `plugins.entries.<id>.subagent.allowedModels`: 신뢰된 subagent 재정의에 대한 선택적 표준 `provider/model` 대상 허용 목록입니다. 어떤 모델이든 허용하려는 의도가 분명한 경우에만 `"*"`를 사용하세요.
...
- `plugins.entries.firecrawl.config.webFetch`: Firecrawl 웹 가져오기 provider 설정.
  ...
  - `maxAgeMs`: 최대 캐시 사용 기간(밀리초, 기본값: `172800000` / 2일).
...
- `plugins.entries.memory-core.config.dreaming`: 메모리 dreaming(실험적) 설정. 단계와 임계값은 [Dreaming](/ko/concepts/dreaming)을 참조하세요.
  - `enabled`: dreaming 마스터 스위치(기본값 `false`).
  - `frequency`: 각 전체 dreaming 스윕의 cron 주기(기본값 `"0 3 * * *"`).
  - 단계 정책과 임계값은 구현 세부 사항이며 사용자 대상 구성 키가 아닙니다.
- 활성화된 Claude 번들 plugins는 `settings.json`의 내장 Pi 기본값도 제공할 수 있으며, OpenClaw는 이를 원시 OpenClaw 구성 패치가 아니라 정제된 agent 설정으로 적용합니다.
- `plugins.slots.memory`: 활성 메모리 plugin ID를 선택하거나 메모리 plugins를 비활성화하려면 `"none"`으로 설정합니다.
...
- 엄격 모드에서는 원격 CDP 프로필 엔드포인트(`profiles.*.cdpUrl`)도 도달 가능성/감지 검사 중 동일한 비공개 네트워크 차단이 적용됩니다.
- `ssrfPolicy.allowPrivateNetwork`는 레거시 별칭으로 계속 지원됩니다.
- 엄격 모드에서는 명시적 예외를 위해 `ssrfPolicy.hostnameAllowlist` 및 `ssrfPolicy.allowedHostnames`를 사용하세요.
- 원격 프로필은 attach-only입니다(start/stop/reset 비활성).
- `profiles.*.cdpUrl`은 `http://`, `https://`, `ws://`, `wss://`를 허용합니다.
  `/json/version`을 OpenClaw가 발견하게 하려면 HTTP(S)를 사용하고,
...
- `existing-session` 프로필은 현재 Chrome MCP 경로 제한을 유지합니다: CSS 셀렉터 대상 지정 대신 snapshot/ref 기반 작업, 단일 파일 업로드 훅, 대화상자 타임아웃 재정의 없음, `wait --load networkidle` 없음, `responsebody`, PDF 내보내기, 다운로드 가로채기, 배치 작업 없음.
- 로컬에서 관리되는 `openclaw` 프로필은 `cdpPort`와 `cdpUrl`을 자동 할당합니다. 원격 CDP인 경우에만 `cdpUrl`을 명시적으로 설정하세요.
...
- `extraArgs`는 로컬 Chromium 시작에 추가 실행 플래그를 붙입니다(예: `--disable-gpu`, 창 크기 설정, 디버그 플래그).
...
- `assistant`: Control UI identity 재정의입니다. 활성 agent identity로 대체됩니다.
