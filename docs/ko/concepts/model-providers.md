---
read_when:
    - provider별 모델 설정 참고 문서가 필요합니다
    - 모델 provider용 예시 구성이나 CLI 온보딩 명령어가 필요합니다
summary: 모델 provider 개요와 예시 구성 + CLI 흐름
title: 모델 provider
x-i18n:
    generated_at: "2026-04-21T06:01:52Z"
    model: gpt-5.4
    provider: openai
...
이 페이지는 **LLM/모델 provider**를 다룹니다(WhatsApp/Telegram 같은 채팅 채널이 아님).
...
- 폴백 런타임 규칙, cooldown probe, 세션 오버라이드 지속성은
...
- provider manifest는 `providerAuthEnvVars`와
  `providerAuthAliases`를 선언할 수 있으므로, 일반적인 env 기반 인증 probe와 provider variant가
  Plugin 런타임을 로드할 필요가 없습니다. 이제 core의 나머지 env-var 맵은
  Plugin이 아닌/core provider와 Anthropic API-key-first 온보딩 같은 일부 일반 우선순위 사례에만
  사용됩니다.
...
- 참고: provider 런타임 `capabilities`는 공유 runner 메타데이터입니다(provider
  family, transcript/tooling 특성, transport/cache 힌트). 이는
  Plugin이 등록하는 내용을 설명하는 [공개 capability 모델](/ko/plugins/architecture#public-capability-model)과는
  다릅니다(텍스트 추론, 음성 등).
- 번들된 `codex` provider는 번들된 Codex agent harness와 함께 사용됩니다.
  Codex 소유 로그인, 모델 탐색, 기본 thread 재개, 앱 서버 실행을 원하면
  `codex/gpt-*`를 사용하세요. 일반 `openai/gpt-*` ref는 계속 OpenAI provider와
  일반 OpenClaw provider transport를 사용합니다.
  Codex 전용 배포는
  `agents.defaults.embeddedHarness.fallback: "none"`으로 자동 PI 폴백을 비활성화할 수 있습니다. 자세한 내용은
...
- `auth[].run` / `auth[].runNonInteractive`: provider가 `openclaw onboard`, `openclaw models auth`, 헤드리스 설정용 온보딩/로그인 흐름을 소유
- `wizard.setup` / `wizard.modelPicker`: provider가 인증 선택 라벨,
  레거시 alias, 온보딩 허용 목록 힌트, 온보딩/모델 picker의 설정 항목을 소유
- `catalog`: provider가 `models.providers`에 표시됨
- `normalizeModelId`: provider가 조회 또는 정규화 전에
  레거시/preview 모델 ID를 정규화
- `normalizeTransport`: provider가 일반 모델 조립 전에 transport-family `api` / `baseUrl`을 정규화합니다.
  OpenClaw는 먼저 일치하는 provider를 확인한 다음,
  실제로 transport를 변경하는 Plugin이 나올 때까지 다른 hook 지원 provider Plugin을 확인합니다
- `normalizeConfig`: provider가 런타임 사용 전에 `models.providers.<id>` 구성을 정규화합니다.
  OpenClaw는 먼저 일치하는 provider를 확인한 다음,
  실제로 구성을 변경하는 Plugin이 나올 때까지 다른 hook 지원 provider Plugin을 확인합니다.
  어떤 provider hook도 구성을 다시 쓰지 않으면, 번들된 Google 계열 도우미가 계속
  지원되는 Google provider 항목을 정규화합니다.
- `applyNativeStreamingUsageCompat`: provider가 구성된 provider에 대해 엔드포인트 기반 기본 스트리밍 usage 호환성 재작성을 적용
- `resolveConfigApiKey`: provider가 구성된 provider에 대해
  전체 런타임 인증 로딩 없이 env-marker 인증을 확인
  `amazon-bedrock`도 여기에서 내장 AWS env-marker resolver를 가집니다.
  Bedrock 런타임 인증은 AWS SDK 기본 체인을 사용하더라도 그렇습니다.
- `resolveSyntheticAuth`: provider가 평문 비밀을 저장하지 않고도
  local/self-hosted 또는 기타 config 기반 인증 가용성을 노출할 수 있음
- `shouldDeferSyntheticProfileAuth`: provider가 저장된 synthetic profile
  placeholder를 env/config 기반 인증보다 낮은 우선순위로 표시할 수 있음
- `resolveDynamicModel`: provider가 아직 로컬 정적 카탈로그에 없는
  모델 ID를 허용
- `prepareDynamicModel`: provider가 동적 확인을 다시 시도하기 전에
  메타데이터 새로 고침이 필요함
- `normalizeResolvedModel`: provider가 transport 또는 base URL 재작성이 필요함
- `contributeResolvedModelCompat`: provider가
  다른 호환 transport를 통해 들어온 경우에도 자사 vendor 모델에 대한 compat 플래그를 기여
- `capabilities`: provider가 transcript/tooling/provider-family 특성을 게시
- `normalizeToolSchemas`: provider가 내장 runner가 보기 전에
  도구 스키마를 정리
- `inspectToolSchemas`: provider가 정규화 후
  transport별 스키마 경고를 노출
- `resolveReasoningOutputMode`: provider가 기본 또는 태그 기반
  reasoning-output 계약을 선택
- `prepareExtraParams`: provider가 모델별 요청 파라미터를 기본 설정하거나 정규화
- `createStreamFn`: provider가 일반 stream 경로를 완전한
  사용자 정의 transport로 대체
- `wrapStreamFn`: provider가 요청 헤더/본문/모델 compat 래퍼를 적용
- `resolveTransportTurnState`: provider가 턴별 기본 transport
  헤더 또는 메타데이터를 제공
- `resolveWebSocketSessionPolicy`: provider가 기본 WebSocket 세션
  헤더 또는 세션 cool-down 정책을 제공
- `createEmbeddingProvider`: provider가
  core 임베딩 스위치보드가 아니라 provider Plugin에 속하는 경우 메모리 임베딩 동작을 소유
- `formatApiKey`: provider가 저장된 인증 프로필을
  transport가 기대하는 런타임 `apiKey` 문자열로 포맷
- `refreshOAuth`: 공유 `pi-ai`
  refresher로 충분하지 않을 때 provider가 OAuth 갱신을 소유
- `buildAuthDoctorHint`: OAuth 갱신이 실패할 때
  provider가 복구 안내를 추가
...
- `classifyFailoverReason`: provider가 provider별 원시 transport/API
  오류를 rate limit 또는 overload 같은 폴백 사유로 매핑
...
- `suppressBuiltInModel`: provider가 오래된 업스트림 행을 숨기고
  직접 확인 실패 시 vendor 소유 오류를 반환할 수 있음
- `augmentModelCatalog`: provider가 탐색과 구성 병합 후
  synthetic/final 카탈로그 행을 추가
- `isBinaryThinking`: provider가 이진 on/off thinking UX를 소유
- `supportsXHighThinking`: provider가 선택된 모델에 `xhigh`를 적용
- `supportsAdaptiveThinking`: provider가 선택된 모델에 `adaptive`를 적용
- `resolveDefaultThinkingLevel`: provider가 모델 계열의
  기본 `/think` 정책을 소유
- `applyConfigDefaults`: provider가 인증 모드, env, 모델 계열에 따라
  구성 구체화 중 provider별 전역 기본값을 적용
- `isModernModelRef`: provider가 live/smoke 선호 모델 매칭을 소유
- `prepareRuntimeAuth`: provider가 구성된 자격 증명을
  수명이 짧은 런타임 토큰으로 변환
- `resolveUsageAuth`: provider가 `/usage` 및 관련 상태/보고 표면용
  usage/quota 자격 증명을 확인
- `fetchUsageSnapshot`: provider가 usage 엔드포인트 가져오기/파싱을 소유하고,
  core는 계속 요약 셸과 포맷을 소유
- `onModelSelected`: provider가 텔레메트리나 provider 소유 세션 기록 같은
  선택 후 부수 효과를 실행

현재 번들된 예시:

- `anthropic`: Claude 4.6 전방 호환 폴백, 인증 복구 힌트, usage
  엔드포인트 가져오기, cache-TTL/provider-family 메타데이터, 그리고 인증 인식 전역
  구성 기본값
- `amazon-bedrock`: Bedrock 전용 throttle/not-ready 오류에 대한
  provider 소유 컨텍스트 오버플로 매칭 및 폴백 사유 분류, 그리고
  Anthropic 트래픽에서 Claude 전용 replay-policy
  가드를 위한 공유 `anthropic-by-model` 재생 패밀리
- `anthropic-vertex`: Anthropic-message
  트래픽에서 Claude 전용 replay-policy 가드
- `openrouter`: 패스스루 모델 ID, 요청 래퍼, provider capability
  힌트, 프록시 Gemini 트래픽에서 Gemini thought-signature 정리, `openrouter-thinking` 스트림 패밀리를 통한
  프록시 reasoning 주입,
  라우팅 메타데이터 전달, 그리고 cache-TTL 정책
- `github-copilot`: 온보딩/기기 로그인, 전방 호환 모델 폴백,
  Claude-thinking transcript 힌트, 런타임 토큰 교환, 그리고 usage 엔드포인트
  가져오기
- `openai`: GPT-5.4 전방 호환 폴백, 직접 OpenAI transport
  정규화, Codex 인식 누락 인증 힌트, Spark 억제, synthetic
  OpenAI/Codex 카탈로그 행, thinking/live-model 정책, usage-token alias
  정규화(`input` / `output` 및 `prompt` / `completion` 계열), 기본 OpenAI/Codex
  래퍼를 위한 공유 `openai-responses-defaults` 스트림 패밀리,
  provider-family 메타데이터, `gpt-image-1`용 번들 이미지 생성 provider
  등록, 그리고 `sora-2`용 번들 비디오 생성 provider
  등록
- `google` 및 `google-gemini-cli`: Gemini 3.1 전방 호환 폴백,
  기본 Gemini replay 검증, bootstrap replay 정리, 태그 기반
  reasoning-output 모드, 최신 모델 매칭, Gemini image-preview 모델용 번들 이미지 생성
  provider 등록, 그리고 Veo 모델용 번들
  비디오 생성 provider 등록; Gemini CLI OAuth는 또한 인증 프로필 토큰 포맷,
  usage-token 파싱, 그리고 usage 표면용 quota 엔드포인트
  가져오기를 소유함
...
- `kilocode`: 공유 transport, Plugin 소유 요청 헤더, reasoning 페이로드
  ...
  정책
- `zai`: GLM-5 전방 호환 폴백, `tool_stream` 기본값, cache-TTL
  정책, 이진 thinking/live-model 정책, 그리고 usage 인증 + quota 가져오기;
  알 수 없는 `glm-5*` ID는 번들된 `glm-4.7` 템플릿에서 synthetic 생성됨
- `xai`: 기본 Responses transport 정규화, Grok 빠른 variant용
  `/fast` alias 재작성, 기본 `tool_stream`, xAI 전용 tool-schema /
  reasoning 페이로드 정리, 그리고 `grok-imagine-video`용 번들 비디오 생성 provider
  등록
...
- `fal`: 호스팅된 서드파티
  이미지 생성 provider 등록과 FLUX 이미지 모델용 번들 이미지 생성 provider 등록, 그리고 호스팅된 서드파티 비디오 모델용
  번들 비디오 생성 provider 등록
- `cloudflare-ai-gateway`, `huggingface`, `kimi`, `nvidia`, `qianfan`,
  `stepfun`, `synthetic`, `venice`, `vercel-ai-gateway`, 및 `volcengine`:
  Plugin 소유 카탈로그만 해당
- `qwen`: 텍스트 모델용 Plugin 소유 카탈로그와
  멀티모달 표면용 공유 media-understanding 및 비디오 생성 provider 등록;
  Qwen 비디오 생성은 `wan2.6-t2v` 및 `wan2.7-r2v` 같은
  번들 Wan 모델과 함께 Standard DashScope 비디오 엔드포인트를 사용함
- `runway`: `gen4.5` 같은 기본
...
- `minimax`: Plugin 소유 카탈로그, Hailuo 비디오 모델용 번들 비디오 생성 provider
  ...
  선택, 그리고 usage 인증/snapshot 로직
- `together`: Plugin 소유 카탈로그와 Wan 비디오 모델용
  번들 비디오 생성 provider 등록
- `xiaomi`: Plugin 소유 카탈로그와 usage 인증/snapshot 로직

번들된 `openai` Plugin은 이제 두 provider ID를 모두 소유합니다: `openai` 및
`openai-codex`.

여기까지가 OpenClaw의 일반 transport에 여전히 맞는 provider들입니다. 완전히 사용자 정의된 요청 실행기가 필요한 provider는 별도의 더 깊은 확장 표면입니다.

## API 키 순환

- 선택된 provider에 대해 일반적인 provider 순환을 지원합니다.
- 여러 키는 다음을 통해 구성합니다:
  - `OPENCLAW_LIVE_<PROVIDER>_KEY`(단일 라이브 오버라이드, 가장 높은 우선순위)
...
- 모델별 오버라이드는 `agents.defaults.models["openai/<model>"].params.transport`를 사용합니다(`"sse"`, `"websocket"`, 또는 `"auto"`)
- OpenAI Responses WebSocket warm-up은 기본적으로 `params.openaiWsWarmup`으로 활성화됩니다(`true`/`false`)
...
- 숨겨진 OpenClaw attribution 헤더(`originator`, `version`,
  ...
  기본 OpenAI 트래픽에만 적용됩니다
- 기본 OpenAI 경로는 또한 Responses `store`, prompt-cache 힌트, 그리고
  OpenAI reasoning-compat 페이로드 형성을 유지하며,
  프록시 경로는 그렇지 않습니다
...
- 직접 퍼블릭 Anthropic 요청은 공유 `/fast` 토글과 `params.fastMode`도 지원하며, 여기에는 `api.anthropic.com`으로 전송되는 API 키 및 OAuth 인증 트래픽이 포함됩니다. OpenClaw는 이를 Anthropic `service_tier`(`auto` 대 `standard_only`)에 매핑합니다
- Anthropic 참고: Anthropic 직원이 OpenClaw 스타일의 Claude CLI 사용이 다시 허용된다고 알려왔으므로, Anthropic이 새로운 정책을 발표하지 않는 한 OpenClaw는 Claude CLI 재사용과 `claude -p` 사용을 이 통합에 대해 허용된 것으로 취급합니다.
- Anthropic setup-token은 계속 지원되는 OpenClaw 토큰 경로로 제공되지만, OpenClaw는 이제 가능하면 Claude CLI 재사용과 `claude -p`를 우선합니다.
...
- 기본 transport는 `auto`입니다(WebSocket 우선, SSE 폴백)
- 모델별 오버라이드는 `agents.defaults.models["openai-codex/<model>"].params.transport`를 사용합니다(`"sse"`, `"websocket"`, 또는 `"auto"`)
- `params.serviceTier`도 기본 Codex Responses 요청(`chatgpt.com/backend-api`)에서 전달됩니다
- 숨겨진 OpenClaw attribution 헤더(`originator`, `version`,
  `User-Agent`)는 일반 OpenAI 호환 프록시가 아니라
  `chatgpt.com/backend-api`로 가는 기본 Codex 트래픽에만 첨부됩니다
...
- `openai-codex/gpt-5.4`는 기본 `contextWindow = 1050000`과 기본 런타임 `contextTokens = 272000`을 유지합니다. 런타임 상한은 `models.providers.openai-codex.models[].contextTokens`로 오버라이드하세요
...
- Provider: `google`
- 인증: `GEMINI_API_KEY`
- 선택적 순환: `GEMINI_API_KEYS`, `GEMINI_API_KEY_1`, `GEMINI_API_KEY_2`, `GOOGLE_API_KEY` 폴백, 그리고 `OPENCLAW_LIVE_GEMINI_KEY`(단일 오버라이드)
...
- 직접 Gemini 실행은 `agents.defaults.models["google/<model>"].params.cachedContent`
  (또는 레거시 `cached_content`)도 받아들여 provider 기본
...
- 주의: OpenClaw의 Gemini CLI OAuth는 비공식 통합입니다. 일부 사용자는 서드파티 클라이언트를 사용한 뒤 Google 계정 제한을 겪었다고 보고했습니다. 진행하기로 선택했다면 Google 약관을 검토하고 중요하지 않은 계정을 사용하세요.
- Gemini CLI OAuth는 번들된 `google` Plugin의 일부로 제공됩니다.
  ...
  - 참고: `openclaw.json`에 client id나 secret을 **붙여넣지 않습니다**. CLI 로그인 흐름은
    토큰을 Gateway 호스트의 인증 프로필에 저장합니다.
  - 로그인 후 요청이 실패하면 Gateway 호스트에 `GOOGLE_CLOUD_PROJECT` 또는 `GOOGLE_CLOUD_PROJECT_ID`를 설정하세요.
  - Gemini CLI JSON 응답은 `response`에서 파싱되며, usage는 `stats`로 폴백하고,
...
- OpenClaw는 요청 대상이 실제로 `openrouter.ai`일 때만
...
- OpenRouter는 계속 프록시 스타일 OpenAI 호환 경로를 사용하므로, 기본
...
- Gemini 기반 OpenRouter ref는 프록시 Gemini thought-signature 정리만 유지하며,
  기본 Gemini replay 검증과 bootstrap 재작성은 비활성 상태로 유지됩니다
- Kilo Gateway: `kilocode` (`KILOCODE_API_KEY`)
- 예시 모델: `kilocode/kilo/auto`
- Gemini 기반 Kilo ref도 동일한 프록시 Gemini thought-signature
  정리 경로를 유지하며, `kilocode/kilo/auto`와 기타 프록시 reasoning 미지원
...
- MiniMax 온보딩/API 키 설정은
  `input: ["text", "image"]`를 가진 명시적 M2.7 모델 정의를 기록합니다. 번들된 provider 카탈로그는
  해당 provider 구성이 구체화되기 전까지 채팅 ref를
  텍스트 전용으로 유지합니다
...
- xAI: `xai` (`XAI_API_KEY`)
  - 기본 번들 xAI 요청은 xAI Responses 경로를 사용합니다
  - `/fast` 또는 `params.fastMode: true`는 `grok-3`, `grok-3-mini`,
    `grok-4`, 및 `grok-4-0709`를 해당 `*-fast` variant로 재작성합니다
  - `tool_stream`은 기본적으로 켜져 있으며,
    이를 비활성화하려면
...
## `models.providers`를 통한 provider(사용자 정의/기본 URL)

`models.providers`(또는 `models.json`)를 사용해 **사용자 정의** provider 또는
OpenAI/Anthropic 호환 프록시를 추가하세요.

아래 번들된 provider Plugin 중 다수는 이미 기본 카탈로그를 게시합니다.
기본 base URL, 헤더, 모델 목록을 오버라이드하려는 경우에만
명시적인 `models.providers.<id>` 항목을 사용하세요.

### Moonshot AI (Kimi)

Moonshot은 번들된 provider Plugin으로 제공됩니다. 기본적으로는 내장 provider를 사용하고,
base URL 또는 모델 메타데이터를 오버라이드해야 할 때만 명시적인 `models.providers.moonshot` 항목을 추가하세요:
...
레거시 `kimi/k2p5`는 계속 호환성 모델 ID로 허용됩니다.
...
OpenClaw는 비어 있는 provider 범위 picker를 보여주는 대신
...
- 텍스트/채팅 기본값은 `minimax/MiniMax-M2.7`에 유지됨
- 이미지 생성은 `minimax/image-01` 또는 `minimax-portal/image-01`
- 이미지 이해는 두 MiniMax 인증 경로 모두에서 Plugin 소유 `MiniMax-VL-01`
- 웹 검색은 provider ID `minimax`에 유지됨

### LM Studio

LM Studio는 기본 API를 사용하는 번들된 provider Plugin으로 제공됩니다:

- Provider: `lmstudio`
- 인증: `LM_API_TOKEN`
- 기본 추론 base URL: `http://localhost:1234/v1`

그다음 모델을 설정하세요(`http://localhost:1234/api/v1/models`에서 반환된 ID 중 하나로 교체):
...
OpenClaw는 기본적으로 탐색 + 자동 로드를 위해 LM Studio의 기본 `/api/v1/models`와 `/api/v1/models/load`를 사용하고,
추론에는 `/v1/chat/completions`를 사용합니다.
설정 및 문제 해결은 [/providers/lmstudio](/ko/providers/lmstudio)를 참고하세요.

### Ollama

Ollama는 번들된 provider Plugin으로 제공되며 Ollama의 기본 API를 사용합니다:
...
번들된 provider Plugin이 Ollama를 `openclaw onboard`와 모델 picker에 직접 추가합니다.
온보딩, 클라우드/로컬 모드, 사용자 정의 구성은 [/providers/ollama](/ko/providers/ollama)를 참고하세요.

### vLLM

vLLM은 로컬/셀프 호스팅 OpenAI 호환
서버용 번들된 provider Plugin으로 제공됩니다:

- Provider: `vllm`
- 인증: 선택 사항(서버에 따라 다름)
- 기본 base URL: `http://127.0.0.1:8000/v1`

로컬 자동 탐색에 opt in하려면(서버가 인증을 강제하지 않는다면 어떤 값이든 동작):

```bash
export VLLM_API_KEY="vllm-local"
```

그다음 모델을 설정하세요(`/v1/models`에서 반환된 ID 중 하나로 교체):
...
SGLang은 빠른 셀프 호스팅
OpenAI 호환 서버용 번들된 provider Plugin으로 제공됩니다:

- Provider: `sglang`
- 인증: 선택 사항(서버에 따라 다름)
- 기본 base URL: `http://127.0.0.1:30000/v1`

로컬 자동 탐색에 opt in하려면(서버가 인증을
강제하지 않는다면 어떤 값이든 동작):
...
- 사용자 정의 provider의 경우 `reasoning`, `input`, `cost`, `contextWindow`, `maxTokens`는 선택 사항입니다.
...
- 기본 엔드포인트가 아닌 `api: "openai-completions"`의 경우(`api.openai.com`이 아닌 호스트를 가진 비어 있지 않은 `baseUrl`), OpenClaw는 지원되지 않는 `developer` 역할에 대한 provider 400 오류를 방지하기 위해 `compat.supportsDeveloperRole: false`를 강제합니다.
- 프록시 스타일 OpenAI 호환 경로도 기본 OpenAI 전용 요청
  형성을 건너뜁니다: `service_tier` 없음, Responses `store` 없음, prompt-cache 힌트 없음,
  OpenAI reasoning-compat 페이로드 형성 없음, 숨겨진 OpenClaw attribution
  헤더도 없음.
- `baseUrl`이 비어 있거나 생략되면 OpenClaw는 기본 OpenAI 동작을 유지합니다(`api.openai.com`으로 확인됨).
- 안전을 위해, 명시적인 `compat.supportsDeveloperRole: true`도 기본이 아닌 `openai-completions` 엔드포인트에서는 여전히 오버라이드됩니다.
...
추가 참고: 전체 구성 예시는 [/gateway/configuration](/ko/gateway/configuration)을 참고하세요.
