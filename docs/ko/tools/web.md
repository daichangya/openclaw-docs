...
read_when:
    ...
    - 자동 감지와 provider 폴백을 이해하려고 합니다
sidebarTitle: Web Search
summary: web_search, x_search, 그리고 web_fetch -- 웹 검색, X 게시물 검색, 또는 페이지 콘텐츠 가져오기
title: 웹 검색
x-i18n:
    generated_at: "2026-04-21T06:09:31Z"
    model: gpt-5.4
    provider: openai
...
결과를 반환합니다. 결과는 쿼리 기준으로 15분 동안 캐시됩니다(구성 가능).

OpenClaw에는 X(이전 Twitter) 게시물용 `x_search`와
가벼운 URL 가져오기를 위한 `web_fetch`도 포함되어 있습니다. 이 단계에서
`web_fetch`는 로컬에 머무르고, `web_search`와 `x_search`는 내부적으로 xAI Responses를 사용할 수 있습니다.

<Info>
  `web_search`는 브라우저 자동화가 아닌 가벼운 HTTP 도구입니다.
  JS 비중이 큰 사이트나 로그인이 필요한 경우 [Web Browser](/ko/tools/browser)를 사용하세요.
...
<Steps>
  <Step title="provider 선택">
    ...
    키가 필요 없고, 다른 provider는 API 키를 사용합니다. 자세한 내용은 아래 provider 페이지를
    참고하세요.
  </Step>
  <Step title="구성">
    ```bash
    ...
    ```
    이렇게 하면 provider와 필요한 자격 증명이 저장됩니다. API 기반
    provider의 경우 env
    var(예: `BRAVE_API_KEY`)를 설정하고 이 단계를 건너뛸 수도 있습니다.
  </Step>
  <Step title="사용">
...
| [Perplexity](/ko/tools/perplexity-search) | 구조화된 스니펫 | 국가, 언어, 시간, 도메인, 콘텐츠 한도 | `PERPLEXITY_API_KEY` / `OPENROUTER_API_KEY` |
...
Codex 지원 모델은 OpenClaw의 관리형 `web_search` 함수 대신
provider 기본 Responses `web_search` 도구를 선택적으로 사용할 수 있습니다.

- `tools.web.search.openaiCodex` 아래에서 구성합니다
- Codex 지원 모델(`openai-codex/*` 또는 `api: "openai-codex-responses"`를 사용하는 provider)에만 활성화됩니다
...
기본 Codex 검색이 활성화되어 있지만 현재 모델이 Codex 지원이 아니라면,
OpenClaw는 일반 관리형 `web_search` 동작을 유지합니다.

## 웹 검색 설정

문서와 setup 흐름의 provider 목록은 알파벳순입니다. 자동 감지는
별도의 우선순위 순서를 유지합니다.

`provider`가 설정되지 않았다면, OpenClaw는 provider를 다음 순서로 확인하고
준비된 첫 번째 provider를 사용합니다:
...
11. **Ollama Web Search** -- 구성된 Ollama 호스트를 통한 키 없는 폴백. Ollama에 접근 가능해야 하고 `ollama signin`으로 로그인되어 있어야 하며, 호스트에 인증이 필요하면 Ollama provider bearer auth를 재사용할 수 있음 (순서 110)
12. **SearXNG** -- `SEARXNG_BASE_URL` 또는 `plugins.entries.searxng.config.webSearch.baseUrl` (순서 200)

감지된 provider가 없으면 Brave로 폴백합니다(provider를 구성하라는
누락 키 오류가 표시됩니다).

<Note>
  모든 provider 키 필드는 SecretRef 객체를 지원합니다. 자동 감지 모드에서는
  OpenClaw가 선택된 provider 키만 확인하며, 선택되지 않은 SecretRef는
  비활성 상태로 남습니다.
</Note>
...
- 또는 해당 필드를 생략하고 OpenClaw가 사용 가능한 자격 증명에서
  준비된 첫 번째 web-fetch provider를 자동 감지하도록 할 수 있습니다
...
`x_search`의 경우 `plugins.entries.xai.config.xSearch.*`를 구성하세요. 이것은
...
OpenClaw는 같은 키로 선택적 `x_search` setup도 제안할 수 있습니다.
이것은 Grok 경로 안의 별도 후속 단계이며, 별도의 최상위
...
| `max_tokens_per_page` | 페이지별 토큰 한도, 기본값 2048(Perplexity 전용) |

<Warning>
  모든 파라미터가 모든 provider에서 동작하는 것은 아닙니다. Brave `llm-context` 모드는
  ...
  Firecrawl과 Tavily는 `web_search`를 통해 `query`와 `count`만 지원하므로,
  고급 옵션에는 전용 도구를 사용하세요.
</Warning>

## x_search

`x_search`는 xAI를 사용해 X(이전 Twitter) 게시물을 질의하고
...
<Note>
  ...
  답글, 북마크, 조회수 같은 게시물별 참여 통계는
  정확한 게시물 URL 또는 상태 ID에 대한 대상 조회를 우선하세요. 광범위한 키워드 검색은 올바른 게시물을 찾을 수는 있지만
  게시물별 metadata가 덜 완전하게 반환될 수 있습니다. 좋은 패턴은
  먼저 게시물을 찾고, 그다음 해당 정확한 게시물에 집중한 두 번째 `x_search` 쿼리를 실행하는 것입니다.
</Note>
...
```javascript
// 게시물별 통계: 가능하면 정확한 상태 URL 또는 상태 ID를 사용하세요
await x_search({
...
도구 프로필이나 허용 목록을 사용한다면 `web_search`, `x_search`, 또는 `group:web`를 추가하세요:
...
- [Web Browser](/ko/tools/browser) -- JS 비중이 큰 사이트를 위한 전체 브라우저 자동화
