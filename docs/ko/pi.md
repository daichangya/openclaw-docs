...
read_when:
    ...
    - Pi의 에이전트 세션 수명 주기, 도구 또는 provider 연결 수정하기
summary: OpenClaw의 내장 Pi 에이전트 통합 아키텍처 및 세션 수명 주기
title: Pi 통합 아키텍처
x-i18n:
    generated_at: "2026-04-21T06:04:57Z"
    model: gpt-5.4
    provider: openai
...
이 문서는 OpenClaw가 AI 에이전트 기능을 구동하기 위해 [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) 및 그 형제 패키지들(`pi-ai`, `pi-agent-core`, `pi-tui`)과 어떻게 통합되는지 설명합니다.

## 개요

OpenClaw는 pi SDK를 사용해 AI 코딩 에이전트를 메시징 Gateway 아키텍처에 내장합니다. OpenClaw는 pi를 하위 프로세스로 실행하거나 RPC 모드를 사용하는 대신, `createAgentSession()`을 통해 pi의 `AgentSession`을 직접 import하고 인스턴스화합니다. 이 내장 방식은 다음을 제공합니다:

- 세션 수명 주기 및 이벤트 처리에 대한 완전한 제어
- 사용자 지정 도구 주입(메시징, 샌드박스, 채널별 작업)
- 채널/컨텍스트별 시스템 프롬프트 사용자 지정
- 브랜칭/Compaction 지원이 포함된 세션 영속성
- 장애 조치가 가능한 다중 계정 인증 프로필 순환
- provider 비종속 모델 전환
...
│   ├── abort.ts                   # 중단 오류 감지
│   ├── cache-ttl.ts               # 컨텍스트 프루닝용 캐시 TTL 추적
│   ├── compact.ts                 # 수동/자동 Compaction 로직
│   ├── extensions.ts              # 내장 실행용 pi 확장 로드
│   ├── extra-params.ts            # provider별 스트림 매개변수
...
│   ├── session-manager-cache.ts   # SessionManager 인스턴스 캐시
...
├── pi-embedded-block-chunker.ts   # 스트리밍 블록 답장 청킹
...
│   ├── context-pruning.ts         # 캐시 TTL 컨텍스트 프루닝 확장
│   └── context-pruning/
...
2. **사용자 지정 대체**: OpenClaw는 `bash`를 `exec`/`process`로 대체하고, 샌드박스용 `read`/`edit`/`write`를 사용자 지정
...
7. **AbortSignal 래핑**: 도구가 abort signal을 존중하도록 래핑
...
시스템 프롬프트는 `buildAgentSystemPrompt()`(`system-prompt.ts`)에서 구성됩니다. 이 함수는 Tooling, Tool Call Style, Safety guardrails, OpenClaw CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata 섹션에 더해, 활성화된 경우 Memory와 Reactions, 그리고 선택적 컨텍스트 파일 및 추가 시스템 프롬프트 내용을 포함하는 전체 프롬프트를 조합합니다. 하위 에이전트에 사용되는 최소 프롬프트 모드에서는 섹션이 축약됩니다.
...
세션은 트리 구조(id/parentId 링크 포함)를 가진 JSONL 파일입니다. pi의 `SessionManager`가 영속성을 처리합니다:
...
OpenClaw는 provider별 다중 API 키를 갖는 인증 프로필 저장소를 유지합니다:
...
`src/agents/pi-hooks/compaction-safeguard.ts`는 적응형 토큰 예산 편성, 도구 실패 요약, 파일 작업 요약을 포함한 Compaction 안전장치를 추가합니다:
...
`EmbeddedBlockChunker`는 스트리밍 텍스트를 개별 답장 블록으로 관리합니다:
...
const stripBlockTags = (text: string, state: { thinking: boolean; final: boolean }) => {
  // <think>...</think> 내용 제거
  // enforceFinalTag가 설정된 경우 <final>...</final> 내용만 반환
};
```

### 답장 지시어

`[[media:url]]`, `[[voice]]`, `[[reply:id]]` 같은 답장 지시어는 파싱되어 추출됩니다:
...
if (sandboxRoot) {
  // 샌드박스 처리된 read/edit/write 도구 사용
  // Exec는 컨테이너에서 실행
  // 브라우저는 브리지 URL 사용
}
```
...
이로써 pi의 기본 모드와 유사한 대화형 터미널 경험을 제공합니다.
...
2. **세션 관리자 래핑**: `guardSessionManager`는 안전성을 추가하지만 복잡도도 높임
3. **확장 로딩**: pi의 `ResourceLoader`를 더 직접적으로 사용할 수 있음
4. **스트리밍 핸들러 복잡도**: `subscribeEmbeddedPiSession`가 많이 커졌음
...
현재 실행 명령은 [Pi Development Workflow](/ko/pi-dev)를 참고하세요.
