---
read_when:
    - OpenClaw의 Pi SDK 통합 설계 이해하기
    - Pi의 에이전트 세션 수명 주기, 도구 또는 provider wiring 수정하기
...
OpenClaw는 pi SDK를 사용하여 AI 코딩 에이전트를 메시징 gateway 아키텍처에 내장합니다. pi를 subprocess로 실행하거나 RPC 모드를 사용하는 대신, OpenClaw는 `createAgentSession()`을 통해 pi의 `AgentSession`을 직접 import하고 인스턴스화합니다. 이 내장 접근 방식은 다음을 제공합니다.

- 세션 수명 주기 및 이벤트 처리에 대한 완전한 제어
- 커스텀 도구 주입(메시징, sandbox, 채널별 작업)
- 채널/컨텍스트별 시스템 프롬프트 커스터마이징
- 브랜칭/Compaction 지원이 포함된 세션 지속성
- failover를 포함한 멀티 계정 auth profile 순환
- provider 독립적인 모델 전환
...
| `pi-tui`          | 터미널 UI 컴포넌트(OpenClaw의 local TUI 모드에서 사용)                                                 |
...
│   ├── cache-ttl.ts               # 컨텍스트 가지치기를 위한 cache TTL 추적
│   ├── compact.ts                 # 수동/자동 Compaction 로직
│   ├── extensions.ts              # 내장 실행용 pi 확장 로드
│   ├── extra-params.ts            # provider별 stream 매개변수
│   ├── google.ts                  # Google/Gemini 턴 순서 수정
│   ├── history.ts                 # 히스토리 제한(DM 대 그룹)
...
├── pi-embedded-block-chunker.ts   # 스트리밍 블록 응답 청크 분할
...
│   └── context-pruning/
├── model-auth.ts                  # auth profile 확인
├── auth-profiles.ts               # profile 저장소, 쿨다운, failover
...
├── system-prompt-params.ts        # 시스템 프롬프트 매개변수 확인
...
채널별 메시지 작업 runtime은 이제 `src/agents/tools` 아래가 아니라
plugin 소유 extension 디렉터리에 위치합니다. 예를 들면 다음과 같습니다.

- Discord plugin 작업 runtime 파일
- Slack plugin 작업 runtime 파일
- Telegram plugin 작업 runtime 파일
- WhatsApp plugin 작업 runtime 파일
...
`runEmbeddedPiAgent()`에서 호출되는 `runEmbeddedAttempt()` 내부에서는 pi SDK를 사용합니다.
...
SDK가 전체 에이전트 루프를 처리합니다. 즉, LLM으로 전송하고, 도구 호출을 실행하며, 응답을 스트리밍합니다.

이미지 주입은 프롬프트 로컬 방식입니다. OpenClaw는 현재 프롬프트에서 이미지 ref를 로드하고
해당 턴에만 `images`로 전달합니다. 이전 히스토리 턴을 다시 스캔하여 이미지 payload를
재주입하지는 않습니다.
...
2. **커스텀 대체**: OpenClaw는 `bash`를 `exec`/`process`로 대체하고, sandbox용으로 `read`/`edit`/`write`를 커스터마이징
3. **OpenClaw 도구**: 메시징, 브라우저, canvas, 세션, Cron, Gateway 등
4. **채널 도구**: Discord/Telegram/Slack/WhatsApp별 작업 도구
5. **정책 필터링**: profile, provider, 에이전트, 그룹, sandbox 정책에 따라 도구 필터링
6. **Schema 정규화**: Gemini/OpenAI 특이 사항에 맞게 schema 정리
7. **AbortSignal 래핑**: 중단 신호를 따르도록 도구 래핑
...
이렇게 하면 OpenClaw의 정책 필터링, sandbox 통합, 확장된 도구 집합이 provider 전반에서 일관되게 유지됩니다.

## 시스템 프롬프트 구성

시스템 프롬프트는 `buildAgentSystemPrompt()`(`system-prompt.ts`)에서 빌드됩니다. 여기서는 Tooling, Tool Call Style, 안전 가드레일, OpenClaw CLI 참조, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeat, 런타임 메타데이터, 그리고 활성화된 경우 Memory 및 Reactions, 선택적 컨텍스트 파일과 추가 시스템 프롬프트 콘텐츠를 포함하는 전체 프롬프트를 조합합니다. 하위 에이전트에 사용되는 최소 프롬프트 모드에서는 섹션이 축약됩니다.
...
`limitHistoryTurns()`는 채널 유형(DM 대 그룹)에 따라 대화 히스토리를 잘라냅니다.

### Compaction

자동 Compaction은 컨텍스트 오버플로 시 트리거됩니다. 일반적인 오버플로 시그니처에는
...
profile은 쿨다운 추적과 함께 실패 시 순환됩니다.
...
// pi의 ModelRegistry 및 AuthStorage 사용
authStorage.setRuntimeApiKey(model.provider, apiKeyInfo.apiKey);
```

### Failover

구성되어 있으면 `FailoverError`가 모델 fallback을 트리거합니다.
...
OpenClaw는 특수 동작을 위해 커스텀 pi 확장을 로드합니다.

### Compaction Safeguard

`src/agents/pi-hooks/compaction-safeguard.ts`는 adaptive token budgeting과 도구 실패 및 파일 작업 요약을 포함한 Compaction 가드레일을 추가합니다.
...
const stripBlockTags = (text: string, state: { thinking: boolean; final: boolean }) => {
  // <think>...</think> 콘텐츠 제거
  // enforceFinalTag가 설정된 경우 <final>...</final> 콘텐츠만 반환
};
```

### 답장 지시문

`[[media:url]]`, `[[voice]]`, `[[reply:id]]` 같은 답장 지시문은 파싱되어 추출됩니다.
...
isRateLimitAssistantError(...)        // rate limit됨
...
OpenClaw에는 pi-tui 컴포넌트를 직접 사용하는 local TUI 모드도 있습니다.
...
이로써 pi의 네이티브 모드와 유사한 대화형 터미널 경험을 제공합니다.
...
| 인증            | 단일 자격 증명          | 순환을 포함한 멀티 profile                                                                           |
...
2. **세션 관리자 래핑**: `guardSessionManager`가 안전성을 추가하지만 복잡성도 증가시킴
3. **확장 로딩**: pi의 `ResourceLoader`를 더 직접 사용할 수 있음
4. **스트리밍 핸들러 복잡성**: `subscribeEmbeddedPiSession`이 점점 커짐
5. **provider 특이 사항**: pi가 잠재적으로 처리할 수 있는 provider별 코드 경로가 많음
