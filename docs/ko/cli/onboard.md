---
read_when:
    - Gateway, 워크스페이스, 인증, 채널, Skills를 위한 안내형 설정이 필요합니다
...
평문 private-network `ws://` 대상(신뢰할 수 있는 네트워크 전용)의 경우,
...
LM Studio는 비대화형 모드에서 Provider 전용 키 플래그도 지원합니다:
...
`--custom-base-url`의 기본값은 `http://127.0.0.1:11434`입니다. `--custom-model-id`는 선택 사항이며, 생략하면 온보딩은 Ollama의 권장 기본값을 사용합니다. `kimi-k2.5:cloud` 같은 클라우드 모델 ID도 여기서 동작합니다.
...
- 온보딩 프로세스 환경에 Provider 환경 변수를 설정합니다(예: `OPENAI_API_KEY`).
- 해당 환경 변수도 설정되어 있지 않다면 인라인 키 플래그(예: `--openai-api-key`)를 전달하지 마세요.
- 필요한 환경 변수 없이 인라인 키 플래그를 전달하면, 온보딩은 안내와 함께 즉시 실패합니다.
...
- `--install-daemon` 사용 시, 토큰 인증에 토큰이 필요한 경우 SecretRef로 관리되는 Gateway 토큰은 검증되지만 supervisor 서비스 환경 메타데이터에 해석된 평문으로 영속 저장되지는 않습니다.
- `--install-daemon` 사용 시, 토큰 모드에 토큰이 필요한데 구성된 토큰 SecretRef가 해석되지 않으면 온보딩은 수정 안내와 함께 실패 시 닫힘으로 처리됩니다.
...
- 로컬 온보딩은 구성에 `gateway.mode="local"`을 기록합니다. 이후 구성 파일에 `gateway.mode`가 없다면, 이를 유효한 로컬 모드 단축 경로가 아니라 구성 손상 또는 불완전한 수동 편집으로 간주하세요.
- `--allow-unconfigured`는 별도의 Gateway 런타임 이스케이프 해치입니다. 이것이 온보딩에서 `gateway.mode`를 생략해도 된다는 뜻은 아닙니다.
...
- 자동화에서 구성/워크스페이스/bootstrap 기록만 원한다면 `--skip-health`를 사용하세요.
- 기본 Windows에서는 `--install-daemon`이 먼저 Scheduled Tasks를 시도하고, 작업 생성이 거부되면 사용자별 Startup 폴더 로그인 항목으로 폴백합니다.

ref 모드를 사용할 때의 대화형 온보딩 동작:
...
- 온보딩은 ref를 저장하기 전에 빠른 사전 검증을 수행합니다.
  - 검증에 실패하면 온보딩은 오류를 보여주고 다시 시도할 수 있게 합니다.
...
- 인증 선택이 선호 Provider를 암시하는 경우, 온보딩은 기본 모델과 허용 목록 선택기를 해당 Provider로 미리 필터링합니다. Volcengine 및 BytePlus의 경우 이는 coding-plan 변형(`volcengine-plan/*`, `byteplus-plan/*`)에도 적용됩니다.
- 선호 Provider 필터에 아직 로드된 모델이 없으면, 온보딩은 선택기를 비워 두는 대신 필터링되지 않은 카탈로그로 폴백합니다.
- 웹 검색 단계에서 일부 Provider는 Provider 전용 후속 프롬프트를 트리거할 수 있습니다:
...
- 가장 빠른 첫 채팅: `openclaw dashboard` (채널 설정이 필요 없는 Control UI).
- 사용자 지정 Provider: 목록에 없는 호스팅 Provider를 포함하여 모든 OpenAI 또는 Anthropic 호환 엔드포인트를 연결합니다. 자동 감지하려면 Unknown을 사용하세요.
