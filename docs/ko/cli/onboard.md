---
read_when:
    - Gateway, 워크스페이스, 인증, 채널 및 Skills에 대한 안내형 설정이 필요합니다
...
평문 사설 네트워크 `ws://` 대상(신뢰할 수 있는 네트워크에서만 사용)의 경우, 온보딩 프로세스 환경에서 `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`을 설정하세요. 이 클라이언트 측 전송 긴급 우회에는 `openclaw.json`에 해당하는 설정이 없습니다.

비대화형 사용자 정의 provider:
...
`--custom-base-url`의 기본값은 `http://127.0.0.1:11434`입니다. `--custom-model-id`는 선택 사항이며, 생략하면 온보딩이 Ollama의 권장 기본값을 사용합니다. `kimi-k2.5:cloud`와 같은 클라우드 모델 ID도 여기서 사용할 수 있습니다.
...
인증 프로필 기반 provider의 경우 `keyRef` 항목을 기록하고, 사용자 정의 provider의 경우 `models.providers.<id>.apiKey`를 env ref로 기록합니다(예: `{ source: "env", provider: "default", id: "CUSTOM_API_KEY" }`).

비대화형 `ref` 모드 계약:

- 온보딩 프로세스 환경에서 provider env var를 설정합니다(예: `OPENAI_API_KEY`).
- 해당 env var도 설정되어 있지 않다면 인라인 키 플래그(예: `--openai-api-key`)를 전달하지 마세요.
- 필요한 env var 없이 인라인 키 플래그가 전달되면, 온보딩은 안내와 함께 즉시 실패합니다.
...
- `--install-daemon` 사용 시, 토큰 모드에 토큰이 필요한데 설정된 토큰 SecretRef가 해석되지 않으면 온보딩은 수정 안내와 함께 폐쇄적으로 실패합니다.
- `--install-daemon` 사용 시, `gateway.auth.token`과 `gateway.auth.password`가 모두 설정되어 있고 `gateway.auth.mode`가 설정되지 않았다면, 온보딩은 모드를 명시적으로 설정할 때까지 설치를 차단합니다.
- 로컬 온보딩은 config에 `gateway.mode="local"`을 기록합니다. 이후 config 파일에 `gateway.mode`가 없다면, 이를 유효한 로컬 모드 단축 경로가 아니라 config 손상 또는 불완전한 수동 편집으로 간주해야 합니다.
- `--allow-unconfigured`는 별도의 Gateway 런타임 비상 우회입니다. 이것이 온보딩에서 `gateway.mode`를 생략해도 된다는 뜻은 아닙니다.
...
- workspace 파일을 직접 관리한다면 `--skip-bootstrap`을 전달하여 `agents.defaults.skipBootstrap: true`를 설정하고 `AGENTS.md`, `SOUL.md`, `TOOLS.md`, `IDENTITY.md`, `USER.md`, `HEARTBEAT.md`, `BOOTSTRAP.md` 생성을 건너뛰세요.
...
GLM Coding Plan 엔드포인트를 명시적으로 원한다면 `zai-coding-global` 또는 `zai-coding-cn`을 선택하세요.
...
- 인증 선택이 선호 provider를 암시하는 경우, 온보딩은 해당 provider로 기본 모델 및 허용 목록 선택기를 미리 필터링합니다. Volcengine 및 BytePlus의 경우 이는 coding-plan 변형(`volcengine-plan/*`, `byteplus-plan/*`)에도 적용됩니다.
- 선호 provider 필터 결과 아직 로드된 모델이 하나도 없으면, 온보딩은 선택기를 비워 두는 대신 필터링되지 않은 카탈로그로 대체합니다.
- 웹 검색 단계에서는 일부 provider가 provider별 후속 프롬프트를 트리거할 수 있습니다:
  - **Grok**은 동일한 `XAI_API_KEY`와 `x_search` 모델 선택을 사용한 선택적 `x_search` 설정을 제공할 수 있습니다.
...
- 사용자 정의 Provider: 나열되지 않은 호스팅 provider를 포함해 OpenAI 또는 Anthropic 호환 엔드포인트에 연결합니다. 자동 감지를 위해 Unknown을 사용하세요.
