...
title: CLI 참조
x-i18n:
    generated_at: "2026-04-24T06:07:48Z"
    model: gpt-5.4
    provider: openai
...
`openclaw`는 기본 CLI 진입점입니다. 각 핵심 명령에는 전용 참조 페이지가 있거나 해당 명령이 별칭으로 사용하는 명령과 함께 문서화되어 있습니다. 이 색인 페이지에는 명령 목록, 전역 플래그, 그리고 CLI 전체에 적용되는 출력 스타일 규칙이 나와 있습니다.
...
| 플래그                  | 용도                                                                  |
| ----------------------- | --------------------------------------------------------------------- |
| `--dev`                 | 상태를 `~/.openclaw-dev` 아래로 격리하고 기본 포트를 변경             |
| `--profile <name>`      | 상태를 `~/.openclaw-<name>` 아래로 격리                               |
| `--container <name>`    | 실행 대상으로 명명된 컨테이너를 지정                                  |
| `--no-color`            | ANSI 색상 비활성화 (`NO_COLOR=1`도 존중됨)                            |
...
- OSC-8 하이퍼링크는 지원되는 곳에서는 클릭 가능한 링크로 렌더링되며, 그렇지 않으면 CLI는 일반 URL로 폴백합니다.
- 지원되는 경우 `--json`(및 `--plain`)은 깔끔한 출력을 위해 스타일을 비활성화합니다.
- 장시간 실행되는 명령은 진행 표시기(OSC 9;4 지원 시)를 표시합니다.

팔레트 기준 소스: `src/terminal/palette.ts`.
...
채팅 메시지는 `/...` 명령을 지원합니다. 자세한 내용은 [슬래시 명령](/ko/tools/slash-commands)을 참조하세요.

주요 항목:

- `/status` — 빠른 진단.
- `/trace` — 세션 범위 Plugin trace/debug 줄.
- `/config` — 영구 저장되는 구성 변경.
- `/debug` — 런타임 전용 구성 재정의(메모리만, 디스크 아님; `commands.debug: true` 필요).

## 사용량 추적

`openclaw status --usage`와 Control UI는 OAuth/API 자격 증명이 있을 때 제공자 사용량/할당량을 표시합니다. 데이터는 제공자 사용량 엔드포인트에서 직접 가져오며 `X% left`로 정규화됩니다. 현재 사용량 창을 지원하는 제공자: Anthropic, GitHub Copilot, Gemini CLI, OpenAI Codex, MiniMax, Xiaomi, z.ai.
