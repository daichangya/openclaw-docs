---
read_when:
    - 실시간 모델 매트릭스 / CLI 백엔드 / ACP / 미디어 제공자 스모크 실행
    - 실시간 테스트 자격 증명 확인 문제 디버깅
    - 새로운 제공자별 실시간 테스트 추가
...
빠른 시작, QA 실행기, 단위/통합 스위트, Docker 흐름은
[테스트](/ko/help/testing)를 참조하세요. 이 페이지에서는 **실시간**(네트워크에 연결되는) 테스트
스위트, 즉 모델 매트릭스, CLI 백엔드, ACP, 미디어 제공자 실시간 테스트와
자격 증명 처리를 다룹니다.

## 실시간: 로컬 프로필 스모크 명령

임시 실시간 점검 전에 `~/.profile`을 source하여 제공자 키와 로컬 도구
경로가 셸과 일치하도록 하세요:
...
`voicecall smoke`는 `--yes`도 함께 지정되지 않는 한 드라이 런입니다. 실제 알림 전화를 의도적으로 걸고자 할 때만 `--yes`를 사용하세요. Twilio, Telnyx, Plivo의 경우
준비 상태 확인이 성공하려면 공개 Webhook URL이 필요합니다. 로컬 전용
loopback/private 대체 경로는 설계상 거부됩니다.

## 실시간: Android Node 기능 전체 점검
...
실시간 테스트는 장애를 분리할 수 있도록 두 계층으로 나뉩니다:

- “직접 모델”은 주어진 키로 제공자/모델이 실제로 응답할 수 있는지 알려줍니다.
- “Gateway 스모크”는 해당 모델에 대해 전체 gateway+agent 파이프라인이 작동하는지(세션, 기록, 도구, 샌드박스 정책 등) 알려줍니다.
...
- 모델 선택 방법:
  ...
  - 또는 `OPENCLAW_LIVE_MODELS="openai/gpt-5.2,openai-codex/gpt-5.2,anthropic/claude-opus-4-6,..."`(쉼표로 구분된 허용 목록)
  - modern/all 전체 점검은 기본적으로 엄선된 고신호 상한을 사용합니다. exhaustive modern 전체 점검에는 `OPENCLAW_LIVE_MAX_MODELS=0`을, 더 작은 상한에는 양수를 설정하세요.
  - exhaustive 전체 점검은 전체 직접 모델 테스트 타임아웃으로 `OPENCLAW_LIVE_TEST_TIMEOUT_MS`를 사용합니다. 기본값: 60분.
...
- 이것이 존재하는 이유:
  - “제공자 API가 깨짐 / 키가 유효하지 않음”과 “Gateway agent 파이프라인이 깨짐”을 분리
  - 작고 고립된 회귀를 포함(예: OpenAI Responses/Codex Responses 추론 재생 + tool-call 흐름)
...
- 목표:
  ...
  - 키가 있는 모델들을 순회하고 다음을 검증:
    ...
    - OpenAI 회귀 경로(tool-call-only → 후속)가 계속 작동함
- 프로브 세부 정보(장애를 빠르게 설명할 수 있도록):
  - `read` probe: 테스트가 작업 공간에 nonce 파일을 쓰고 agent에게 이를 `read`하여 nonce를 다시 말하도록 요청합니다.
...
- 모델 선택 방법:
  ...
  - modern/all Gateway 전체 점검은 기본적으로 엄선된 고신호 상한을 사용합니다. exhaustive modern 전체 점검에는 `OPENCLAW_LIVE_GATEWAY_MAX_MODELS=0`을, 더 작은 상한에는 양수를 설정하세요.
...
- 도구 + 이미지 프로브는 이 실시간 테스트에서 항상 활성화됩니다:
  - `read` probe + `exec+read` probe(도구 스트레스)
  - 모델이 이미지 입력 지원을 광고하는 경우 image probe 실행
  - 흐름(상위 수준):
    ...
    - 검증: 응답에 `cat` + 해당 코드가 포함됨(OCR 허용 오차: 사소한 실수 허용)
...
- 재정의(선택 사항):
  ...
  - `OPENCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE=1`로 실제 이미지 첨부 전송(경로는 프롬프트에 삽입됨). Docker 레시피는 명시적으로 요청하지 않으면 기본적으로 이를 끕니다.
  - `OPENCLAW_LIVE_CLI_BACKEND_IMAGE_ARG="--image"`로 프롬프트 삽입 대신 이미지 파일 경로를 CLI args로 전달
  - `OPENCLAW_LIVE_CLI_BACKEND_IMAGE_MODE="repeat"`(또는 `"list"`)로 `IMAGE_ARG`가 설정된 경우 이미지 args 전달 방식을 제어
  - `OPENCLAW_LIVE_CLI_BACKEND_RESUME_PROBE=1`로 두 번째 턴을 보내고 재개 흐름 검증
  - `OPENCLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE=1`로 선택한 모델이 전환 대상을 지원할 때 Claude Sonnet -> Opus 동일 세션 연속성 프로브에 옵트인. Docker 레시피는 전체 신뢰성을 위해 기본적으로 이를 끕니다.
  - `OPENCLAW_LIVE_CLI_BACKEND_MCP_PROBE=1`로 MCP/tool loopback probe에 옵트인. Docker 레시피는 명시적으로 요청하지 않으면 기본적으로 이를 끕니다.
...
- 저장소 Docker 이미지 내부에서 비-root `node` 사용자로 실시간 CLI 백엔드 스모크를 실행합니다.
- 해당 확장에서 CLI 스모크 메타데이터를 확인한 다음, 일치하는 Linux CLI 패키지(`@anthropic-ai/claude-code`, `@openai/codex`, 또는 `@google/gemini-cli`)를 `OPENCLAW_DOCKER_CLI_TOOLS_DIR`의 캐시된 쓰기 가능 prefix(기본값: `~/.cache/openclaw/docker-cli-tools`)에 설치합니다.
- `pnpm test:docker:live-cli-backend:claude-subscription`은 `claudeAiOauth.subscriptionType`가 있는 `~/.claude/.credentials.json` 또는 `claude setup-token`의 `CLAUDE_CODE_OAUTH_TOKEN`을 통한 portable Claude Code subscription OAuth가 필요합니다. 먼저 Docker에서 직접 `claude -p`를 검증한 다음, Anthropic API-key env vars를 유지하지 않은 상태로 두 번의 Gateway CLI 백엔드 턴을 실행합니다. 이 subscription 레인은 현재 Claude가 서드파티 앱 사용을 일반 구독 플랜 한도 대신 추가 사용량 청구로 라우팅하므로 Claude MCP/tool 및 image probe를 기본적으로 비활성화합니다.
- 실시간 CLI 백엔드 스모크는 이제 Claude, Codex, Gemini에 대해 동일한 종단 간 흐름을 실행합니다: 텍스트 턴, 이미지 분류 턴, 그다음 gateway CLI를 통해 검증되는 MCP `cron` 도구 호출.
...
- 참고:
  - 이 레인은 Gateway `chat.send` 표면을 admin 전용 합성 originating-route 필드와 함께 사용하므로, 테스트가 외부로 실제 전달하는 척하지 않고도 message-channel 컨텍스트를 연결할 수 있습니다.
  - `OPENCLAW_LIVE_ACP_BIND_AGENT_COMMAND`가 설정되지 않은 경우, 테스트는 선택한 ACP 하니스 agent에 대해 임베디드 `acpx` Plugin의 내장 agent 레지스트리를 사용합니다.
...
- 기본적으로 ACP bind 스모크를 집계된 실시간 CLI agent `claude`, `codex`, `gemini`에 대해 순차적으로 실행합니다.
- 매트릭스를 좁히려면 `OPENCLAW_LIVE_ACP_BIND_AGENTS=claude`, `OPENCLAW_LIVE_ACP_BIND_AGENTS=codex`, `OPENCLAW_LIVE_ACP_BIND_AGENTS=gemini`, 또는 `OPENCLAW_LIVE_ACP_BIND_AGENTS=opencode`를 사용하세요.
- `~/.profile`을 source하고, 일치하는 CLI 인증 자료를 컨테이너에 준비한 다음, 요청된 실시간 CLI(`@anthropic-ai/claude-code`, `@openai/codex`, `@google/gemini-cli`, 또는 `opencode-ai`)가 없으면 설치합니다. ACP 백엔드 자체는 `acpx` Plugin의 번들된 임베디드 `acpx/runtime` 패키지입니다.
- OpenCode Docker 변형은 엄격한 단일 agent 회귀 레인입니다. `~/.profile`을 source한 뒤 `OPENCLAW_LIVE_ACP_BIND_OPENCODE_MODEL`(기본값 `opencode/kimi-k2.6`)에서 임시 `OPENCODE_CONFIG_CONTENT` 기본 모델을 작성하며, `pnpm test:docker:live-acp-bind:opencode`는 일반적인 bind 후 skip을 허용하지 않고 bind된 assistant transcript를 요구합니다.
- 직접 `acpx` CLI 호출은 Gateway 외부에서 동작을 비교하기 위한 수동/우회 경로일 뿐입니다. Docker ACP bind 스모크는 OpenClaw의 임베디드 `acpx` 런타임 백엔드를 실행합니다.

## 실시간: Codex app-server 하니스 스모크

- 목표: 일반 gateway `agent` 메서드를 통해 Plugin이 소유하는 Codex 하니스를 검증:
  - 번들된 `codex` Plugin 로드
  - `OPENCLAW_AGENT_RUNTIME=codex` 선택
  - Codex 하니스를 강제한 상태로 `openai/gpt-5.2`에 첫 번째 gateway agent 턴 전송
  - 같은 OpenClaw 세션에 두 번째 턴을 전송하고 app-server 스레드가 재개될 수 있는지 검증
  - 같은 gateway 명령 경로를 통해 `/codex status` 및 `/codex models` 실행
  - 선택적으로 Guardian 검토가 있는 두 개의 권한 상승 셸 프로브 실행: 승인되어야 하는 무해한 명령 하나와, 거부되어 agent가 다시 물어봐야 하는 가짜 비밀 업로드 하나
...
- 이 스모크는 `OPENCLAW_AGENT_HARNESS_FALLBACK=none`을 설정하므로, 손상된 Codex 하니스가 PI로 조용히 대체되어 통과할 수 없습니다.
- 인증: 로컬 Codex subscription 로그인에서 가져오는 Codex app-server 인증. Docker 스모크는 해당되는 경우 비-Codex 프로브를 위해 `OPENAI_API_KEY`도 제공할 수 있으며, 선택적으로 복사된 `~/.codex/auth.json` 및 `~/.codex/config.toml`도 사용할 수 있습니다.
...
- 마운트된 `~/.profile`을 source하고, `OPENAI_API_KEY`를 전달하며, Codex CLI 인증 파일이 있으면 복사하고, `@openai/codex`를 쓰기 가능한 마운트된 npm prefix에 설치한 뒤, 소스 트리를 준비하고 Codex 하니스 실시간 테스트만 실행합니다.
- Docker는 기본적으로 image, MCP/tool, Guardian probe를 활성화합니다. 더 좁은 디버그 실행이 필요하면 `OPENCLAW_LIVE_CODEX_HARNESS_IMAGE_PROBE=0` 또는 `OPENCLAW_LIVE_CODEX_HARNESS_MCP_PROBE=0` 또는 `OPENCLAW_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=0`을 설정하세요.
- Docker는 또한 `OPENCLAW_AGENT_HARNESS_FALLBACK=none`을 export하며, 이는 실시간 테스트 config와 일치하므로 레거시 별칭이나 PI 대체가 Codex 하니스 회귀를 숨길 수 없습니다.
...
- `google-gemini-cli/...`는 머신의 로컬 Gemini CLI를 사용합니다(별도 인증 + 도구 관련 차이점).
- Gemini API와 Gemini CLI:
  - API: OpenClaw가 Google의 호스팅된 Gemini API를 HTTP로 호출합니다(API 키 / 프로필 인증). 대부분의 사용자가 “Gemini”라고 말할 때 의미하는 것은 이것입니다.
  - CLI: OpenClaw가 로컬 `gemini` 바이너리를 셸로 실행합니다. 자체 인증이 있으며 다르게 동작할 수 있습니다(스트리밍/tool 지원/버전 차이).

## 실시간: 모델 매트릭스(무엇을 다루는가)

고정된 “CI 모델 목록”은 없습니다(실시간은 옵트인). 하지만 키가 있는 개발자 머신에서 정기적으로 다루기를 **권장하는** 모델은 다음과 같습니다.

### modern 스모크 세트(tool calling + image)

이것은 계속 정상 동작하기를 기대하는 “공통 모델” 실행입니다:
...
image probe를 실행하려면 `OPENCLAW_LIVE_GATEWAY_MODELS`에 최소 하나의 image 지원 모델(Claude/Gemini/OpenAI의 vision 지원 변형 등)을 포함하세요.

### 집계자 / 대체 Gateway

키가 활성화되어 있다면 다음을 통한 테스트도 지원합니다:

- OpenRouter: `openrouter/...`(수백 개 모델; 도구+image 지원 후보를 찾으려면 `openclaw models scan` 사용)
- OpenCode: Zen용 `opencode/...`, Go용 `opencode-go/...`(인증은 `OPENCODE_API_KEY` / `OPENCODE_ZEN_API_KEY`)

실시간 매트릭스에 포함할 수 있는 더 많은 제공자(자격 증명/config이 있는 경우):
...
- `models.providers` 경유(커스텀 엔드포인트): `minimax`(클라우드/API), 그리고 모든 OpenAI/Anthropic 호환 프록시(LM Studio, vLLM, LiteLLM 등)

팁: 문서에 “모든 모델”을 하드코딩하려 하지 마세요. 권위 있는 목록은 머신에서 `discoverModels(...)`가 반환하는 것과 사용 가능한 키가 결정합니다.

## 자격 증명(절대 커밋 금지)

실시간 테스트는 CLI와 동일한 방식으로 자격 증명을 찾습니다. 실질적인 의미는 다음과 같습니다:

- CLI가 동작한다면 실시간 테스트도 같은 키를 찾아야 합니다.
- 실시간 테스트가 “자격 증명 없음”이라고 하면 `openclaw models list` / 모델 선택을 디버그하는 것과 같은 방식으로 디버그하세요.

- agent별 인증 프로필: `~/.openclaw/agents/<agentId>/agent/auth-profiles.json`(실시간 테스트에서 “프로필 키”가 의미하는 것)
- Config: `~/.openclaw/openclaw.json`(또는 `OPENCLAW_CONFIG_PATH`)
- 레거시 상태 디렉터리: `~/.openclaw/credentials/`(존재할 경우 준비된 실시간 홈에 복사되지만, 기본 프로필 키 저장소는 아님)
- 실시간 로컬 실행은 기본적으로 활성 config, agent별 `auth-profiles.json` 파일, 레거시 `credentials/`, 지원되는 외부 CLI 인증 디렉터리를 임시 테스트 홈으로 복사합니다. 준비된 실시간 홈은 `workspace/`와 `sandboxes/`를 건너뛰며, `agents.*.workspace` / `agentDir` 경로 재정의도 제거하여 프로브가 실제 호스트 작업 공간에서 벗어나도록 합니다.

env 키(예: `~/.profile`에 export된 값)를 사용하려면 `source ~/.profile` 후 로컬 테스트를 실행하거나, 아래 Docker 실행기를 사용하세요(컨테이너 안에 `~/.profile`을 마운트할 수 있음).

## Deepgram 실시간(audio transcription)
...
- 범위:
  ...
  - `plugins.entries.comfy.config.<capability>`가 구성되지 않으면 각 기능을 건너뜀
  - comfy workflow 제출, 폴링, 다운로드 또는 Plugin 등록을 변경한 뒤 유용함
...
- 범위:
  ...
  - 프로브 실행 전에 로그인 셸(`~/.profile`)에서 누락된 제공자 env var를 로드
  ...
  - 구성된 각 제공자를 공통 이미지 생성 런타임을 통해 실행:
    - `<provider>:generate`
...
- 선택적 인증 동작:
  - `OPENCLAW_LIVE_REQUIRE_PROFILE_KEYS=1`로 프로필 저장소 인증을 강제하고 env 전용 재정의를 무시

출시된 CLI 경로의 경우, 제공자/런타임 실시간
...
이 경로는 CLI 인수 파싱, config/기본 agent 확인, 번들된
Plugin 활성화, 필요 시 번들된 런타임 의존성 복구, 공통
이미지 생성 런타임, 그리고 실시간 제공자 요청을 다룹니다.
...
- 범위:
  - 공통 번들 음악 생성 제공자 경로를 실행
  - 현재 Google과 MiniMax를 다룸
  - 프로브 실행 전에 로그인 셸(`~/.profile`)에서 제공자 env var를 로드
  - 기본적으로 저장된 인증 프로필보다 실시간/env API 키를 우선 사용하므로, `auth-profiles.json`의 오래된 테스트 키가 실제 셸 자격 증명을 가리지 않음
  - 사용 가능한 인증/프로필/모델이 없는 제공자는 건너뜀
  ...
  - 현재 공통 레인 범위:
    ...
    - `minimax`: `generate`
    - `comfy`: 별도 Comfy 실시간 파일, 이 공통 전체 점검은 아님
- 선택적 범위 축소:
...
- 범위:
  - 공통 번들 비디오 생성 제공자 경로를 실행
  - 기본적으로 출시 안전 스모크 경로를 사용: 비-FAL 제공자, 제공자당 하나의 텍스트-투-비디오 요청, 1초 길이의 랍스터 프롬프트, 그리고 `OPENCLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS`(`180000` 기본값)에서 가져오는 제공자별 작업 상한
  - 제공자 측 큐 지연이 출시 시간을 지배할 수 있으므로 기본적으로 FAL을 건너뜀. 명시적으로 실행하려면 `--video-providers fal` 또는 `OPENCLAW_LIVE_VIDEO_GENERATION_PROVIDERS="fal"` 전달
  - 프로브 실행 전에 로그인 셸(`~/.profile`)에서 제공자 env var를 로드
  - 기본적으로 저장된 인증 프로필보다 실시간/env API 키를 우선 사용하므로, `auth-profiles.json`의 오래된 테스트 키가 실제 셸 자격 증명을 가리지 않음
  - 사용 가능한 인증/프로필/모델이 없는 제공자는 건너뜀
  - 기본적으로 `generate`만 실행
  - 사용 가능한 경우 선언된 transform 모드도 실행하려면 `OPENCLAW_LIVE_VIDEO_GENERATION_FULL_MODES=1` 설정:
    - 제공자가 `capabilities.imageToVideo.enabled`를 선언하고, 선택된 제공자/모델이 공통 전체 점검에서 버퍼 기반 로컬 이미지 입력을 허용하는 경우 `imageToVideo`
    - 제공자가 `capabilities.videoToVideo.enabled`를 선언하고, 선택된 제공자/모델이 공통 전체 점검에서 버퍼 기반 로컬 비디오 입력을 허용하는 경우 `videoToVideo`
  - 현재 공통 전체 점검에서 선언되었지만 건너뛰는 `imageToVideo` 제공자:
    - `vydra`: 번들된 `veo3`는 텍스트 전용이고, 번들된 `kling`은 원격 이미지 URL이 필요하기 때문
  - 제공자별 Vydra 범위:
    - `OPENCLAW_LIVE_TEST=1 OPENCLAW_LIVE_VYDRA_VIDEO=1 pnpm test:live -- extensions/vydra/vydra.live.test.ts`
    - 이 파일은 기본적으로 원격 이미지 URL fixture를 사용하는 `kling` 레인과 함께 `veo3` 텍스트-투-비디오를 실행
  - 현재 `videoToVideo` 실시간 범위:
    - 선택된 모델이 `runway/gen4_aleph`일 때만 `runway`
  - 현재 공통 전체 점검에서 선언되었지만 건너뛰는 `videoToVideo` 제공자:
    - `alibaba`, `qwen`, `xai`: 현재 이 경로들이 원격 `http(s)` / MP4 참조 URL을 필요로 하기 때문
    - `google`: 현재 공통 Gemini/Veo 레인이 로컬 버퍼 기반 입력을 사용하며, 이 경로는 공통 전체 점검에서 허용되지 않기 때문
    - `openai`: 현재 공통 레인에 조직별 비디오 인페인트/리믹스 접근 보장이 없기 때문
- 선택적 범위 축소:
  - `OPENCLAW_LIVE_VIDEO_GENERATION_PROVIDERS="google,openai,runway"`
  - `OPENCLAW_LIVE_VIDEO_GENERATION_MODELS="google/veo-3.1-fast-generate-preview,openai/sora-2,runway/gen4_aleph"`
  - 기본 전체 점검에 FAL을 포함한 모든 제공자를 포함하려면 `OPENCLAW_LIVE_VIDEO_GENERATION_SKIP_PROVIDERS=""`
  - 더 공격적인 스모크 실행을 위해 제공자별 작업 상한을 줄이려면 `OPENCLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS=60000`
- 선택적 인증 동작:
  - `OPENCLAW_LIVE_REQUIRE_PROFILE_KEYS=1`로 프로필 저장소 인증을 강제하고 env 전용 재정의를 무시
...
- 목적:
  - 공통 이미지, 음악, 비디오 실시간 스위트를 하나의 저장소 기본 엔트리포인트로 실행
  - `~/.profile`에서 누락된 제공자 env var를 자동 로드
  - 기본적으로 현재 사용 가능한 인증이 있는 제공자로 각 스위트를 자동 축소
