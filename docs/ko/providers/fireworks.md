...
<Steps>
  <Step title="온보딩을 통해 Fireworks 인증 설정">
    ...
    이렇게 하면 Fireworks 키가 OpenClaw config에 저장되고 Fire Pass 시작 모델이 기본값으로 설정됩니다.
...
| ------------------------------------------------------ | --------------------------- | ---------- | ------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `fireworks/accounts/fireworks/models/kimi-k2p6`        | Kimi K2.6                   | text,image | 262,144 | 262,144    | Fireworks의 최신 Kimi 모델. Fireworks K2.6 요청에서는 thinking이 비활성화됩니다. Kimi thinking 출력을 원하면 Moonshot으로 직접 라우팅하세요.          |
...
OpenClaw는 동적 Fireworks 모델 id도 허용합니다. Fireworks에서 표시하는 정확한 모델 또는 router id를 사용하고 앞에 `fireworks/`를 붙이세요.
...
<AccordionGroup>
  <Accordion title="모델 id 접두사 동작 방식">
    OpenClaw의 모든 Fireworks 모델 ref는 `fireworks/`로 시작하고, 그 뒤에 Fireworks 플랫폼의 정확한 id 또는 router 경로가 따라옵니다. 예를 들면 다음과 같습니다.

    - Router 모델: `fireworks/accounts/fireworks/routers/kimi-k2p5-turbo`
    - Direct 모델: `fireworks/accounts/fireworks/models/<model-name>`
  ...
  <Accordion title="환경 참고">
    Gateway가 대화형 셸 밖에서 실행된다면, 해당 프로세스에서도 `FIREWORKS_API_KEY`를 사용할 수 있어야 합니다.

    <Warning>
    키가 `~/.profile`에만 있으면, 그 환경이 가져와지지 않는 한 launchd/systemd 데몬에는 도움이 되지 않습니다. Gateway 프로세스가 읽을 수 있도록 키를 `~/.openclaw/.env` 또는 `env.shellEnv`에 설정하세요.
    </Warning>
