---
read_when:
    - Moonshot K2(Moonshot Open Platform)와 Kimi Coding 설정 방법이 필요함
    - 별도 엔드포인트, 키, 모델 ref를 이해해야 함
    - 각 provider에 대해 바로 복사해 쓸 수 있는 구성이 필요함
summary: Moonshot K2와 Kimi Coding 구성하기(provider와 키 분리)
title: Moonshot AI
x-i18n:
    generated_at: "2026-04-21T06:08:04Z"
    model: gpt-5.4
    provider: openai
...
Moonshot는 OpenAI 호환 엔드포인트를 사용하는 Kimi API를 제공합니다. provider를 구성하고
기본 모델을 `moonshot/kimi-k2.6`으로 설정하거나,
`kimi/kimi-code`로 Kimi Coding을 사용할 수 있습니다.

<Warning>
Moonshot와 Kimi Coding은 **별도의 provider**입니다. 키는 서로 호환되지 않고, 엔드포인트도 다르며, 모델 ref도 다릅니다(`moonshot/...` vs `kimi/...`).
</Warning>
...
현재 Moonshot 호스팅 K2 모델에 대한 번들 비용 추정은 Moonshot가
공개한 종량제 요금을 사용합니다. Kimi K2.6은 캐시 적중 시 $0.16/MTok,
입력 $0.95/MTok, 출력 $4.00/MTok이며, Kimi K2.5는 캐시 적중 시 $0.10/MTok,
입력 $0.60/MTok, 출력 $3.00/MTok입니다. 다른 레거시 카탈로그 항목은
구성에서 override하지 않는 한 비용 0 placeholder를 유지합니다.

## 시작하기

provider를 선택하고 설정 단계를 따르세요.
...
<Tabs>
  <Tab title="Moonshot API">
    **가장 적합한 용도:** Moonshot Open Platform을 통한 Kimi K2 모델
    ...
    <Steps>
      ...
      <Step title="실시간 스모크 테스트 실행">
        일반 세션에 영향을 주지 않고 모델 접근과 비용
        추적을 확인하려면 격리된 상태 디렉터리를 사용하세요:
        ...
        `model: "kimi-k2.6"`이 보고되어야 합니다. assistant transcript 항목은 Moonshot가
        사용량 메타데이터를 반환할 때 정규화된
        토큰 사용량과 추정 비용을 `usage.cost` 아래에 저장합니다.
      </Step>
    </Steps>
  ...
  <Tab title="Kimi Coding">
    **가장 적합한 용도:** Kimi Coding 엔드포인트를 통한 코드 중심 작업
    ...
    <Note>
    Kimi Coding은 Moonshot(`moonshot/...`)과 다른 API 키와 provider 접두사(`kimi/...`)를 사용합니다. 레거시 모델 ref `kimi/k2p5`는 호환성 id로 계속 허용됩니다.
    </Note>
...
OpenClaw는 또한 Moonshot 웹
검색을 기반으로 하는 `web_search` provider로 **Kimi**를 제공합니다.
...
<AccordionGroup>
  <Accordion title="네이티브 thinking 모드">
    ...
    `agents.defaults.models.<provider/model>.params`를 통해 모델별로 구성할 수 있습니다:
    ...
    OpenClaw는 또한 Moonshot에 대해 런타임 `/think` 수준을 매핑합니다:
    ...
    Kimi K2.6은 또한 `reasoning_content`의 다중 턴 보존을 제어하는 선택적 `thinking.keep` 필드를 허용합니다.
    전체 추론을 턴 간에 유지하려면 `"all"`로 설정하고, 서버
    기본 전략을 사용하려면 생략하거나(`null`로 두거나) 하세요. OpenClaw는
    `moonshot/kimi-k2.6`에 대해서만 `thinking.keep`를 전달하고 다른 모델에서는 이를 제거합니다.
  ...
  <Accordion title="스트리밍 사용량 호환성">
    ...
    Moonshot 호스트를 대상으로 하는 호환 custom provider id도 동일한 스트리밍 사용량 동작을 상속합니다.

    번들 K2.6 가격이 있을 때, 입력, 출력,
    그리고 cache-read 토큰을 포함한 스트리밍 사용량은 `/status`, `/usage full`, `/usage cost`, 그리고 transcript 기반 세션
    회계에 사용되는 로컬 추정 USD 비용으로도 변환됩니다.
  ...
  <Accordion title="엔드포인트 및 모델 ref 참조">
    ...
    - 필요하면 `models.providers`에서 가격과 컨텍스트 메타데이터를 override하세요.
    - Moonshot가 모델에 대해 다른 컨텍스트 한계를 게시하면 `contextWindow`를 그에 맞게 조정하세요.
...
<CardGroup cols={2}>
  <Card title="Model selection" href="/ko/concepts/model-providers" icon="layers">
    provider, 모델 ref, 그리고 failover 동작 선택하기
  </Card>
  ...
  <Card title="Configuration reference" href="/ko/gateway/configuration-reference" icon="gear">
    provider, 모델, 그리고 Plugin에 대한 전체 구성 스키마
  </Card>
