---
read_when:
    - OpenClaw에서 privacy-focused Inference를 원합니다
    - Venice AI 설정 가이드가 필요합니다
summary: OpenClaw에서 Venice AI의 privacy-focused 모델 사용하기
title: Venice AI
x-i18n:
    generated_at: "2026-04-24T06:33:02Z"
    model: gpt-5.4
    provider: openai
...
Venice AI는 **privacy-focused AI Inference**를 제공하며, 검열되지 않은 모델과 익명화된 프록시를 통한 주요 proprietary 모델 접근을 지원합니다. 모든 Inference는 기본적으로 비공개입니다 — 사용자 데이터로 학습하지 않고, 로그도 남기지 않습니다.

## OpenClaw에서 Venice를 사용하는 이유

- 오픈소스 모델에 대한 **비공개 Inference**(로그 없음)
- 필요할 때 사용할 수 있는 **검열되지 않은 모델**
- 품질이 중요할 때 Venice의 익명화 프록시를 통한 **proprietary 모델(Opus/GPT/Gemini) 익명 접근**
- OpenAI 호환 `/v1` 엔드포인트

## 개인정보 보호 모드

Venice는 두 가지 개인정보 보호 수준을 제공합니다. 어떤 모델을 선택할지 결정할 때 이것을 이해하는 것이 중요합니다:

| 모드 | 설명 | 모델 |
| -------------- | --------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| **Private** | 완전히 비공개. 프롬프트/응답이 **절대 저장되거나 기록되지 않음**. 일시적. | Llama, Qwen, DeepSeek, Kimi, MiniMax, Venice Uncensored 등 |
| **Anonymized** | 메타데이터를 제거한 뒤 Venice를 통해 프록시됨. 기본 Provider(OpenAI, Anthropic, Google, xAI)는 익명화된 요청을 보게 됨. | Claude, GPT, Gemini, Grok |

<Warning>
Anonymized 모델은 **완전히 비공개가 아닙니다**. Venice가 전달 전에 메타데이터를 제거하지만, 기본 Provider(OpenAI, Anthropic, Google, xAI)는 여전히 요청을 처리합니다. 완전한 개인정보 보호가 필요하면 **Private** 모델을 선택하세요.
</Warning>

## 기능

- **privacy-focused**: "private"(완전 비공개)와 "anonymized"(프록시) 모드 중 선택 가능
- **검열되지 않은 모델**: 콘텐츠 제한 없는 모델 접근
- **주요 모델 접근**: Venice의 익명화 프록시를 통해 Claude, GPT, Gemini, Grok 사용 가능
- **OpenAI 호환 API**: 쉬운 통합을 위한 표준 `/v1` 엔드포인트
- **스트리밍**: 모든 모델에서 지원
- **함수 호출**: 일부 모델에서 지원(모델 기능 확인 필요)
- **비전**: 비전 기능이 있는 모델에서 지원
- **강한 속도 제한 없음**: 극단적 사용 시 fair-use 기반 throttling이 적용될 수 있음
  ...
  <Step title="OpenClaw 구성">
    ...
    <Tabs>
      <Tab title="대화형 (권장)">
        ...
        이 과정은 다음을 수행합니다:
        1. API 키를 요청함(또는 기존 `VENICE_API_KEY` 사용)
...
- **기본 모델**: 강력한 private 추론 + 비전을 위해 `venice/kimi-k2-5`
- **고성능 옵션**: 가장 강력한 anonymized Venice 경로를 위해 `venice/claude-opus-4-6`
- **개인정보 보호**: 완전한 private Inference에는 "private" 모델 선택
...
또는 `openclaw configure`를 실행하고 **Model/auth**를 선택한 뒤 **Venice AI**를 선택할 수도 있습니다.

<Tip>
사용 사례에 맞는 모델을 고르려면 아래 표를 사용하세요.

| 사용 사례 | 권장 모델 | 이유 |
| -------------------------- | -------------------------------- | -------------------------------------------- |
| **일반 채팅 (기본값)** | `kimi-k2-5` | 강력한 private 추론 + 비전 |
| **최고의 전체 품질** | `claude-opus-4-6` | 가장 강력한 anonymized Venice 옵션 |
| **개인정보 보호 + 코딩** | `qwen3-coder-480b-a35b-instruct` | 큰 컨텍스트를 가진 private 코딩 모델 |
| **Private 비전** | `kimi-k2-5` | private 모드를 벗어나지 않고 비전 지원 |
| **빠르고 저렴함** | `qwen3-4b` | 경량 추론 모델 |
| **복잡한 private 작업** | `deepseek-v3.2` | 강력한 추론, 단 Venice 도구 지원 없음 |
...
`/models` 엔드포인트는 공개되어 있으므로(목록 조회에는 인증 불필요), Inference에는 유효한 API 키가 필요합니다.
...
- **Anonymized 모델**: 직접 API 가격 + 소액 Venice 수수료와 유사

### Venice (anonymized) vs direct API

| 측면 | Venice (Anonymized) | 직접 API |
| ------------ | ----------------------------- | ------------------- |
| **개인정보 보호** | 메타데이터 제거, 익명화됨 | 사용자 계정에 연결됨 |
...
# Venice를 통한 Claude Opus 사용 (anonymized)
