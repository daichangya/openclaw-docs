---
read_when:
    - 오디오 첨부파일에 Deepgram 음성-텍스트 변환을 사용하고자 하는 경우
    - Voice Call에 Deepgram 스트리밍 전사를 사용하고자 하는 경우
    - 빠른 Deepgram 구성 예제가 필요한 경우
summary: 수신 음성 노트를 위한 Deepgram 전사
title: Deepgram
x-i18n:
    generated_at: "2026-04-23T06:07:15Z"
    model: gpt-5.4
    provider: openai
...
`tools.media.audio`를 통한 수신 오디오/음성 노트 전사와 Voice Call
`plugins.entries.voice-call.config.streaming`을 통한 스트리밍 STT에 사용됩니다.

배치 전사의 경우 OpenClaw는 전체 오디오 파일을 Deepgram에 업로드하고
전사문을 응답 파이프라인에 주입합니다(`{{Transcript}}` +
`[Audio]` 블록). Voice Call 스트리밍의 경우 OpenClaw는 실시간 G.711
u-law 프레임을 Deepgram의 WebSocket `listen` 엔드포인트로 전달하고,
Deepgram이 반환하는 부분 또는 최종 전사문을 내보냅니다.
...
<Steps>
  ...
  <Step title="음성 노트 보내기">
    연결된 아무 채널에서나 오디오 메시지를 보내세요. OpenClaw가
    Deepgram을 통해 이를 전사하고 전사문을 응답 파이프라인에 주입합니다.
  </Step>
</Steps>
...
<Note>
Voice Call은 8 kHz G.711 u-law 형식의 전화 오디오를 수신합니다. Deepgram
...
<AccordionGroup>
  <Accordion title="인증">
    인증은 표준 provider 인증 순서를 따릅니다. 가장 간단한 방법은 `DEEPGRAM_API_KEY`입니다.
  </Accordion>
