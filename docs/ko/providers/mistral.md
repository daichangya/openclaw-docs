...
summary: Mistral 모델과 Voxtral 전사를 OpenClaw와 함께 사용하기
title: Mistral
x-i18n:
    generated_at: "2026-04-23T06:07:35Z"
    model: gpt-5.4
    provider: openai
...
OpenClaw는 텍스트/이미지 모델 라우팅(`mistral/...`)과 media understanding에서 Voxtral을 통한 오디오 전사를 위해 Mistral을 지원합니다.
...
<Steps>
  <Step title="API 키 받기">
    [Mistral Console](https://console.mistral.ai/)에서 API 키를 만드세요.
  </Step>
  <Step title="온보딩 실행">
    ```bash
...
OpenClaw는 현재 다음 bundled Mistral 카탈로그를 제공합니다.
...
| `mistral/mistral-small-latest` | 텍스트, 이미지 | 128,000 | 16,384 | Mistral Small 4; API `reasoning_effort`로 reasoning 조정 가능 |
...
media understanding 파이프라인을 통해 배치 오디오 전사에 Voxtral을 사용하세요.
...
bundled `mistral` Plugin은 Voice Call 스트리밍 STT provider로 Voxtral Realtime을 등록합니다.
...
<Note>
OpenClaw는 Voice Call이 Twilio 미디어 프레임을 직접 전달할 수 있도록 Mistral 실시간 STT의 기본값을 8 kHz `pcm_mulaw`로 설정합니다. 업스트림 스트림이 이미 원시 PCM인 경우에만 `encoding: "pcm_s16le"`와 일치하는 `sampleRate`를 사용하세요.
</Note>
...
<AccordionGroup>
  <Accordion title="조정 가능한 reasoning (mistral-small-latest)">
    ...
    <Note>
    다른 bundled Mistral 카탈로그 모델은 이 파라미터를 사용하지 않습니다. Mistral의 네이티브 reasoning 우선 동작이 필요하다면 계속 `magistral-*` 모델을 사용하세요.
    </Note>
