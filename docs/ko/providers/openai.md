...
<Tabs>
  ...
  <Tab title="Codex 구독">
    **가장 적합한 경우:** 별도의 API 키 대신 ChatGPT/Codex 구독을 사용하는 경우. Codex cloud는 ChatGPT 로그인이 필요합니다.
    ...
    <Tip>
    온보딩이 기존 Codex CLI 로그인을 재사용하는 경우, 해당 자격 증명은 계속 Codex CLI에서 관리됩니다. 만료 시 OpenClaw는 먼저 외부 Codex 소스를 다시 읽고 새로 고친 자격 증명을 Codex 저장소에 다시 기록합니다.
    </Tip>
    ...
    더 작은 기본 상한이 실제로는 더 나은 지연 시간과 품질 특성을 보입니다. `contextTokens`로 재정의할 수 있습니다:
...
<Note>
공유 도구 매개변수, provider 선택 및 failover 동작은 [Image Generation](/ko/tools/image-generation)을 참고하세요.
</Note>

`gpt-image-2`는 OpenAI 텍스트-투-이미지 생성과 이미지
...
```
/tool image_generate model=openai/gpt-image-2 prompt="물체 형태는 유지하고, 재질을 반투명 유리로 변경" image=/path/to/reference.png size=1024x1536
```
...
<Note>
공유 도구 매개변수, provider 선택 및 failover 동작은 [Video Generation](/ko/tools/video-generation)을 참고하세요.
</Note>

## GPT-5 프롬프트 기여

OpenClaw는 `openai/*` 및 `openai-codex/*` GPT-5 계열 실행에 대해 OpenAI 전용 GPT-5 프롬프트 기여를 추가합니다. 이 기여는 번들 OpenAI plugin에 포함되어 있으며 `gpt-5`, `gpt-5.2`, `gpt-5.4`, `gpt-5.4-mini` 같은 모델 ID에 적용되고, 이전 GPT-4.x 모델에는 적용되지 않습니다.

GPT-5 기여는 페르소나 지속성, 실행 안전성, 도구 규율, 출력 형태, 완료 검사, 검증을 위한 태그된 동작 계약을 추가합니다. 채널별 응답 및 무음 메시지 동작은 공유 OpenClaw 시스템 프롬프트와 아웃바운드 전송 정책에 그대로 유지됩니다. GPT-5 지침은 일치하는 모델에 대해 항상 활성화됩니다. 친화적인 상호작용 스타일 계층은 별도로 존재하며 구성할 수 있습니다.
...
<AccordionGroup>
  <Accordion title="음성 합성 (TTS)">
    ...
    | API 키 | `messages.tts.providers.openai.apiKey` | `OPENAI_API_KEY`로 대체됨 |
  ...
  <Accordion title="실시간 전사">
    ...
    | API 키 | `...openai.apiKey` | `OPENAI_API_KEY`로 대체됨 |
...
<AccordionGroup>
  <Accordion title="전송 (WebSocket vs SSE)">
    OpenClaw는 `openai/*`와 `openai-codex/*` 모두에 대해 WebSocket 우선, SSE 대체(`"auto"`)를 사용합니다.

    `"auto"` 모드에서 OpenClaw는:
    - 초기 WebSocket 실패를 한 번 재시도한 후 SSE로 대체합니다
    - 실패 후 약 60초 동안 WebSocket을 성능 저하 상태로 표시하고 냉각 시간 동안 SSE를 사용합니다
  ...
  <Accordion title="WebSocket 워밍업">
    OpenClaw는 첫 번째 턴 지연 시간을 줄이기 위해 `openai/*`에 대해 기본적으로 WebSocket 워밍업을 활성화합니다.
  ...
  <Accordion title="빠른 모드">
    OpenClaw는 `openai/*`와 `openai-codex/*` 모두에 대해 공유 빠른 모드 토글을 제공합니다:

    - **채팅/UI:** `/fast status|on|off`
    - **구성:** `agents.defaults.models["<provider>/<model>"].params.fastMode`

    활성화되면 OpenClaw는 빠른 모드를 OpenAI 우선 처리(`service_tier = "priority"`)로 매핑합니다. 기존 `service_tier` 값은 유지되며, 빠른 모드는 `reasoning` 또는 `text.verbosity`를 다시 쓰지 않습니다.
  ...
  <Accordion title="우선 처리 (service_tier)">
    ...
    <Warning>
    `serviceTier`는 네이티브 OpenAI 엔드포인트(`api.openai.com`)와 네이티브 Codex 엔드포인트(`chatgpt.com/backend-api`)에만 전달됩니다. 두 provider 중 하나라도 프록시를 통해 라우팅하면 OpenClaw는 `service_tier`를 그대로 둡니다.
    </Warning>
  ...
  <Accordion title="서버 측 Compaction (Responses API)">
    ...
    - `store: true`를 강제함(모델 compat에서 `supportsStore: false`를 설정한 경우 제외)
    - `context_management: [{ type: "compaction", compact_threshold: ... }]`를 주입함
    ...
    <Note>
    `responsesServerCompaction`은 `context_management` 주입만 제어합니다. 직접 OpenAI Responses 모델은 compat에서 `supportsStore: false`를 설정하지 않는 한 계속 `store: true`를 강제합니다.
    </Note>

  </Accordion>

  <Accordion title="strict-agentic GPT 모드">
    `openai/*` 및 `openai-codex/*`의 GPT-5 계열 실행에 대해 OpenClaw는 더 엄격한 임베디드 실행 계약을 사용할 수 있습니다:
    ...
    - 도구 작업을 사용할 수 있을 때 계획만 있는 턴을 더 이상 성공적인 진행으로 간주하지 않음
    - 즉시 실행하도록 유도하는 steer와 함께 해당 턴을 재시도함
    - 상당한 작업에 대해 `update_plan`을 자동으로 활성화함
    - 모델이 계속 계획만 세우고 실행하지 않으면 명시적인 차단 상태를 표시함

    <Note>
    OpenAI 및 Codex GPT-5 계열 실행에만 범위가 제한됩니다. 다른 provider 및 이전 모델 계열은 기본 동작을 유지합니다.
    </Note>

  </Accordion>

  <Accordion title="네이티브 경로 vs OpenAI 호환 경로">
    OpenClaw는 직접 OpenAI, Codex, Azure OpenAI 엔드포인트를 일반적인 OpenAI 호환 `/v1` 프록시와 다르게 처리합니다:

    **네이티브 경로** (`openai/*`, `openai-codex/*`, Azure OpenAI):
    - OpenAI `none` effort를 지원하는 모델에 대해서만 `reasoning: { effort: "none" }`를 유지함
    - `reasoning.effort: "none"`를 거부하는 모델 또는 프록시에는 비활성화된 reasoning을 생략함
    - 도구 스키마를 기본적으로 strict 모드로 설정함
    - 검증된 네이티브 호스트에만 숨겨진 attribution 헤더를 첨부함
    - OpenAI 전용 요청 형태 조정(`service_tier`, `store`, reasoning-compat, prompt-cache 힌트)을 유지함
