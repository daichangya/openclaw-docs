---
read_when:
    - 그룹 채팅 동작 또는 멘션 게이팅을 변경하는 경우
summary: 표면 전반에서의 그룹 채팅 동작(Discord/iMessage/Matrix/Microsoft Teams/Signal/Slack/Telegram/WhatsApp/Zalo)
title: 그룹
x-i18n:
    generated_at: "2026-04-07T05:55:33Z"
    model: gpt-5.4
    provider: openai
...
**사용자 자신이** 그룹에 속해 있다면, OpenClaw는 그 그룹을 볼 수 있고 그곳에서 응답할 수 있습니다.

기본 동작:

- 그룹은 제한됩니다(`groupPolicy: "allowlist"`).
- 명시적으로 멘션 게이팅을 비활성화하지 않는 한, 응답하려면 멘션이 필요합니다.

즉, 허용 목록에 있는 발신자는 OpenClaw를 멘션하여 실행할 수 있습니다.
...
- **트리거 권한 부여**: 누가 에이전트를 실행할 수 있는지(`groupPolicy`, `groups`, `groupAllowFrom`, 채널별 허용 목록)
- **컨텍스트 가시성**: 어떤 보조 컨텍스트가 모델에 주입되는지(답장 텍스트, 인용, 스레드 기록, 전달 메타데이터)

기본적으로 OpenClaw는 일반적인 채팅 동작을 우선하며, 컨텍스트를 대부분 수신된 그대로 유지합니다. 이는 허용 목록이 주로 누가 동작을 실행할 수 있는지를 결정한다는 뜻이지, 인용되었거나 과거의 모든 스니펫에 대한 보편적인 마스킹 경계라는 뜻은 아닙니다.

현재 동작은 채널마다 다릅니다.

- 일부 채널은 특정 경로에서 보조 컨텍스트에 대해 이미 발신자 기반 필터링을 적용합니다(예: Slack 스레드 시딩, Matrix 답장/스레드 조회).
...
이 강화 모델이 채널 전반에서 일관되게 구현되기 전까지는 표면마다 차이가 있을 수 있습니다.
...
| 그룹에서는 사용자만 실행 가능 | `groupPolicy: "allowlist"`, `groupAllowFrom: ["+1555..."]` |
...
이유: 단일 에이전트 모드에서 DM은 일반적으로 **메인** 세션 키(`agent:main:main`)에 들어가고, 그룹은 항상 **비메인** 세션 키(`agent:main:<channel>:group:<id>`)를 사용합니다. `mode: "non-main"`으로 샌드박싱을 활성화하면, 해당 그룹 세션은 Docker에서 실행되고 메인 DM 세션은 호스트에서 유지됩니다.
...
> 작업공간/페르소나를 진정으로 분리해야 한다면(“개인”과 “공개”가 절대 섞이면 안 된다면), 두 번째 에이전트 + 바인딩을 사용하세요. [Multi-Agent Routing](/ko/concepts/multi-agent)을 참고하세요.
...
“호스트 접근 없음” 대신 “그룹은 폴더 X만 볼 수 있음”을 원한다면 `workspaceAccess: "none"`을 유지하고 허용 목록 경로만 샌드박스에 마운트하세요.
...
- WhatsApp/Telegram/Signal/iMessage/Microsoft Teams/Zalo: `groupAllowFrom`을 사용하세요(대체값: 명시적 `allowFrom`).
- DM 페어링 승인(`*-allowFrom` 저장소 항목)은 DM 접근에만 적용되며, 그룹 발신자 권한 부여는 그룹 허용 목록에 대해 명시적으로 유지됩니다.
- Discord: 허용 목록은 `channels.discord.guilds.<id>.channels`를 사용합니다.
- Slack: 허용 목록은 `channels.slack.channels`를 사용합니다.
- Matrix: 허용 목록은 `channels.matrix.groups`를 사용합니다. 룸 ID 또는 별칭 사용을 권장합니다. 참가 중인 룸 이름 조회는 최선형 방식이며, 해석되지 않은 이름은 런타임에 무시됩니다. 발신자를 제한하려면 `channels.matrix.groupAllowFrom`을 사용하세요. 룸별 `users` 허용 목록도 지원됩니다.
...
- 런타임 안전성: provider 블록이 완전히 누락된 경우(`channels.<provider>`가 없음), 그룹 정책은 `channels.defaults.groupPolicy`를 상속하는 대신 실패 시 닫힘 모드(일반적으로 `allowlist`)로 폴백합니다.

빠른 멘털 모델(그룹 메시지 평가 순서):
...
그룹 메시지는 그룹별로 재정의되지 않는 한 멘션이 필요합니다. 기본값은 각 하위 시스템의 `*.groups."*"` 아래에 있습니다.
...
- `mentionPatterns`는 대소문자를 구분하지 않는 안전한 regex 패턴이며, 잘못된 패턴과 안전하지 않은 중첩 반복 형태는 무시됩니다.
- 명시적 멘션을 제공하는 표면은 그대로 통과하며, 패턴은 폴백입니다.
...
- 그룹 기록 컨텍스트는 채널 전반에서 일관되게 래핑되며 **pending-only**입니다(멘션 게이팅 때문에 건너뛴 메시지). 전역 기본값에는 `messages.groupChat.historyLimit`을, 재정의에는 `channels.<channel>.historyLimit`(또는 `channels.<channel>.accounts.*.historyLimit`)을 사용하세요. 비활성화하려면 `0`으로 설정하세요.

## 그룹/채널 도구 제한(선택 사항)

일부 채널 구성은 **특정 그룹/룸/채널 내부에서** 사용할 수 있는 도구를 제한할 수 있습니다.
...
- 그룹/채널 도구 제한은 전역/에이전트 도구 정책에 추가로 적용됩니다(deny가 여전히 우선함).
- 일부 채널은 룸/채널에 대해 다른 중첩 구조를 사용합니다(예: Discord `guilds.*.channels.*`, Slack `channels.*`, Microsoft Teams `teams.*.channels.*`).

## 그룹 허용 목록

`channels.whatsapp.groups`, `channels.telegram.groups`, 또는 `channels.imessage.groups`가 구성되면, 해당 키는 그룹 허용 목록으로 동작합니다. `"*"`를 사용하면 모든 그룹을 허용하면서도 기본 멘션 동작을 계속 설정할 수 있습니다.

자주 발생하는 혼동: DM 페어링 승인은 그룹 권한 부여와 동일하지 않습니다.
DM 페어링을 지원하는 채널의 경우, 페어링 저장소는 DM만 잠금 해제합니다. 그룹 명령은 여전히 `groupAllowFrom` 또는 해당 채널에 문서화된 구성 폴백 같은 구성 허용 목록의 명시적인 그룹 발신자 권한 부여가 필요합니다.
...
소유자는 `channels.whatsapp.allowFrom`으로 결정됩니다(설정되지 않은 경우 봇 자체 E.164). 명령은 독립된 메시지로 보내세요. 다른 표면은 현재 `/activation`을 무시합니다.
...
- BlueBubbles는 `GroupMembers`를 채우기 전에 로컬 Contacts 데이터베이스에서 이름 없는 macOS 그룹 참가자 정보를 선택적으로 보강할 수 있습니다. 이는 기본적으로 꺼져 있으며 일반 그룹 게이팅을 통과한 후에만 실행됩니다.

에이전트 시스템 프롬프트에는 새 그룹 세션의 첫 번째 턴에 그룹 소개가 포함됩니다. 이는 모델에게 사람처럼 응답하고, Markdown 테이블을 피하고, 빈 줄을 최소화하며 일반 채팅 간격을 따르고, 리터럴 `\n` 시퀀스를 입력하지 않도록 상기시킵니다.

## iMessage 세부 사항

- 라우팅 또는 허용 목록에는 `chat_id:<id>` 사용을 권장합니다.
- 채팅 목록 표시: `imsg chats --limit 20`.
- 그룹 답장은 항상 동일한 `chat_id`로 다시 전송됩니다.
