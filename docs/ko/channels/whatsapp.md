---
read_when:
    - WhatsApp/웹 채널 동작 또는 받은편지함 라우팅 작업 중
...
- Dev 채널 + git checkout: 로컬 Plugin 경로를 기본값으로 사용합니다.
- Stable/Beta: npm 패키지 `@openclaw/whatsapp`를 기본값으로 사용합니다.
...
<Note>
OpenClaw는 가능하면 WhatsApp를 별도 번호로 운영할 것을 권장합니다. (채널 메타데이터와 설정 흐름은 해당 구성에 최적화되어 있지만, 개인 번호 구성도 지원됩니다.)
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="개인 번호 대체 구성">
    ...
    런타임에서 self-chat 보호는 연결된 자기 번호와 `allowFrom`을 기준으로 동작합니다.
...
명시적으로 opt-in하지 않으면 WhatsApp는 인바운드 `message_received` hook 페이로드를 Plugin에 브로드캐스트하지 않습니다:
...
인바운드 WhatsApp 메시지 콘텐츠 및 식별자를
받아도 되는 것으로 신뢰하는 Plugin에만 이 기능을 활성화하세요.
...
<Tabs>
  ...
  <Tab title="그룹 정책 + 허용 목록">
    ...
    참고: `channels.whatsapp` 블록이 아예 없으면 런타임 그룹 정책 대체값은 `allowlist`입니다(경고 로그 포함). `channels.defaults.groupPolicy`가 설정되어 있어도 마찬가지입니다.
...
연결된 자기 번호가 `allowFrom`에도 포함되어 있으면 WhatsApp self-chat 보호 장치가 활성화됩니다:

- self-chat 턴에서는 읽음 확인을 건너뜀
- 그렇지 않으면 자신에게 ping을 보낼 수 있는 mention-JID 자동 트리거 동작 무시
- `messages.responsePrefix`가 설정되지 않으면 self-chat 답장은 기본적으로 `[{identity.name}]` 또는 `[openclaw]` 사용

## 메시지 정규화 및 컨텍스트

<AccordionGroup>
  <Accordion title="인바운드 엔벌로프 + 답장 컨텍스트">
    수신된 WhatsApp 메시지는 공유 인바운드 엔벌로프에 래핑됩니다.
  ...
  <Accordion title="미디어 플레이스홀더 및 위치/연락처 추출">
    ...
    위치 본문은 간결한 좌표 텍스트를 사용합니다. 위치 라벨/설명과 연락처/vCard 세부 정보는 인라인 프롬프트 텍스트가 아니라 fenced untrusted metadata로 렌더링됩니다.
  ...
  <Accordion title="읽음 확인">
    읽음 확인은 허용된 인바운드 WhatsApp 메시지에 대해 기본적으로 활성화되어 있습니다.
    ...
    self-chat 턴에서는 전역으로 활성화되어 있어도 읽음 확인을 건너뜁니다.
...
<AccordionGroup>
  <Accordion title="텍스트 청크 분할">
    ...
    - `newline` 모드는 문단 경계(빈 줄)를 우선하며, 이후 길이 안전 청크 분할로 대체됩니다
  </Accordion>

  <Accordion title="아웃바운드 미디어 동작">
    - 이미지, 비디오, 오디오(PTT 음성 노트), 문서 페이로드 지원
    - 답장 페이로드는 `audioAsVoice`를 유지하며, WhatsApp는 오디오 미디어를 Baileys PTT 음성 노트로 전송합니다
    - Microsoft Edge TTS MP3/WebM 출력을 포함한 비 Ogg 오디오는 PTT 전달 전에 Ogg/Opus로 트랜스코딩됩니다
    - 네이티브 Ogg/Opus 오디오는 음성 노트 호환성을 위해 `audio/ogg; codecs=opus`로 전송됩니다
    - 애니메이션 GIF 재생은 비디오 전송 시 `gifPlayback: true`로 지원됩니다
    - 다중 미디어 답장 페이로드를 전송할 때 캡션은 첫 번째 미디어 항목에 적용됩니다
  ...
  <Accordion title="미디어 크기 제한 및 대체 동작">
    ...
    - 이미지는 제한에 맞도록 자동 최적화(리사이즈/품질 스윕)됩니다
    - 미디어 전송 실패 시 첫 번째 항목 대체 동작으로 응답을 조용히 누락하는 대신 텍스트 경고를 보냅니다
  </Accordion>
</AccordionGroup>

## 답장 인용

WhatsApp는 아웃바운드 답장이 인바운드 메시지를 눈에 띄게 인용하는 네이티브 답장 인용을 지원합니다. `channels.whatsapp.replyToMode`로 제어합니다.

| 값          | 동작                                                                   |
| ----------- | ---------------------------------------------------------------------- |
| `"off"`     | 절대 인용하지 않음; 일반 메시지로 전송                                 |
| `"first"`   | 첫 번째 아웃바운드 답장 청크만 인용                                    |
| `"all"`     | 모든 아웃바운드 답장 청크를 인용                                       |
| `"batched"` | 대기열에 들어간 배치 답장만 인용하고 즉시 답장은 인용 없이 유지        |
...
확인 반응은 `reactionLevel`에 의해 게이트되며, `reactionLevel`이 `"off"`이면 억제됩니다.
...
- 실패는 로그에 기록되지만 정상 응답 전달을 차단하지 않음
...
<AccordionGroup>
  <Accordion title="계정 선택 및 기본값">
    ...
    - 기본 계정 선택: `default`가 있으면 그것을 사용하고, 없으면 구성된 첫 번째 계정 ID(정렬됨)를 사용합니다
  ...
  <Accordion title="자격 증명 경로 및 레거시 호환성">
    ...
    - `~/.openclaw/credentials/`의 레거시 기본 인증도 여전히 인식/마이그레이션되며 기본 계정 흐름에 사용됩니다
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="연결되었지만 연결 끊김 / 재연결 루프">
    증상: 연결된 계정에서 연결 끊김 또는 재연결 시도가 반복됨.
  ...
  <Accordion title="그룹 메시지가 예상치 않게 무시됨">
    ...
    - `openclaw.json`의 중복 키(JSON5): 뒤의 항목이 앞의 항목을 덮어쓰므로 범위별로 `groupPolicy`는 하나만 유지하세요
...
1. **그룹별 시스템 프롬프트** (`groups["<groupId>"].systemPrompt`): 특정 그룹 항목이 맵에 존재하고 해당 `systemPrompt` 키가 정의되어 있을 때 사용됩니다. `systemPrompt`가 빈 문자열(`""`)이면 와일드카드가 억제되고 시스템 프롬프트가 적용되지 않습니다.
...
1. **다이렉트별 시스템 프롬프트** (`direct["<peerId>"].systemPrompt`): 특정 피어 항목이 맵에 존재하고 해당 `systemPrompt` 키가 정의되어 있을 때 사용됩니다. `systemPrompt`가 빈 문자열(`""`)이면 와일드카드가 억제되고 시스템 프롬프트가 적용되지 않습니다.
2. **다이렉트 와일드카드 시스템 프롬프트** (`direct["*"].systemPrompt`): 특정 피어 항목이 맵에 아예 없거나, 존재하더라도 `systemPrompt` 키를 정의하지 않았을 때 사용됩니다.

참고: `dms`는 가벼운 DM별 기록 재정의 버킷(`dms.<id>.historyLimit`)으로 유지되며, 프롬프트 재정의는 `direct` 아래에 있습니다.

**Telegram 멀티 계정 동작과의 차이점:** Telegram에서는 봇이 자신이 속하지 않은 그룹의 메시지를 받는 것을 방지하기 위해, 멀티 계정 구성에서 루트 `groups`는 의도적으로 모든 계정에 대해 억제됩니다. 자체 `groups`를 정의하지 않은 계정도 마찬가지입니다. WhatsApp는 이 보호 장치를 적용하지 않습니다. 루트 `groups`와 루트 `direct`는 계정 수준 재정의를 정의하지 않은 계정에 항상 상속되며, 구성된 계정 수와 관계없습니다. 멀티 계정 WhatsApp 구성에서 계정별 그룹 또는 다이렉트 프롬프트를 원한다면, 루트 수준 기본값에 의존하지 말고 각 계정 아래에 전체 맵을 명시적으로 정의하세요.
...
- 그룹 허용과 발신자 권한 부여는 별도의 검사입니다. `groups["*"]`는 그룹 처리에 도달할 수 있는 그룹 집합을 넓히지만, 그 자체로 해당 그룹의 모든 발신자에게 권한을 부여하지는 않습니다. 발신자 액세스는 여전히 `channels.whatsapp.groupPolicy` 및 `channels.whatsapp.groupAllowFrom`에 의해 별도로 제어됩니다.
- `channels.whatsapp.direct`는 DM에 대해 같은 부작용을 가지지 않습니다. `direct["*"]`는 DM이 `dmPolicy`와 `allowFrom` 또는 pairing-store 규칙에 의해 이미 허용된 후에만 기본 다이렉트 채팅 구성을 제공합니다.
      ...
      accounts: {
        work: {
          groups: {
            ...
            "*": { systemPrompt: "업무용 그룹에 대한 기본 프롬프트." },
          },
          direct: {
