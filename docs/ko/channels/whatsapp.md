---
read_when:
    - WhatsApp/web 채널 동작 또는 받은편지함 라우팅 작업 중입니다.
summary: WhatsApp 채널 지원, 액세스 제어, 전송 동작 및 운영
title: WhatsApp
x-i18n:
    generated_at: "2026-04-24T06:05:50Z"
    model: gpt-5.4
    provider: openai
...
- 온보딩(`openclaw onboard`)과 `openclaw channels add --channel whatsapp`는 WhatsApp을 처음 선택할 때 WhatsApp Plugin 설치를 안내합니다.
- `openclaw channels login --channel whatsapp`도 Plugin이 아직 없으면 설치 흐름을 제공합니다.
...
```
  ...
  <Step title="첫 페어링 요청 승인(pairing 모드 사용 시)">
...
```
    ...
    페어링 요청은 1시간 후 만료됩니다. 대기 중 요청은 채널당 최대 3개입니다.

  </Step>
</Steps>

<Note>
가능하면 OpenClaw는 별도 번호로 WhatsApp를 운영하는 것을 권장합니다. (채널 메타데이터와 설정 흐름은 그 구성을 기준으로 최적화되어 있지만, 개인 번호 설정도 지원됩니다.)
</Note>
...
<AccordionGroup>
  <Accordion title="전용 번호(권장)">
    ...
    - 더 명확한 DM allowlist 및 라우팅 경계
    - 자기 자신과의 채팅 혼선 가능성 감소
  ...
  <Accordion title="개인 번호 대체 구성">
    온보딩은 개인 번호 모드를 지원하며 self-chat에 적합한 기본 구성을 기록합니다.

    - `dmPolicy: "allowlist"`
    - `allowFrom`에 내 개인 번호 포함
    - `selfChatMode: true`
...
- 직접 채팅은 DM 세션 규칙을 사용합니다(`session.dmScope`; 기본값 `main`은 DM을 에이전트 메인 세션으로 합칩니다).
- 그룹 세션은 격리됩니다(`agent:<agentId>:whatsapp:group:<jid>`).
- WhatsApp Web 전송은 gateway 호스트의 표준 프록시 환경 변수(`HTTPS_PROXY`, `HTTP_PROXY`, `NO_PROXY` 및 소문자 변형)를 따릅니다. 채널별 WhatsApp 프록시 설정보다 호스트 수준 프록시 구성을 우선 사용하세요.

## 액세스 제어 및 활성화

<Tabs>
  <Tab title="DM 정책">
    `channels.whatsapp.dmPolicy`가 직접 채팅 액세스를 제어합니다.
    ...
    멀티 계정 재정의: `channels.whatsapp.accounts.<id>.dmPolicy`(및 `allowFrom`)가 해당 계정에 대해 채널 수준 기본값보다 우선합니다.

    런타임 동작 세부 정보:

    - 페어링은 채널 allow-store에 영속화되며 구성된 `allowFrom`과 병합됩니다.
    - allowlist가 구성되지 않았으면 연결된 자기 번호가 기본적으로 허용됩니다.
    - OpenClaw는 아웃바운드 `fromMe` DM(연결된 장치에서 자기 자신에게 보낸 메시지)을 자동 페어링하지 않습니다.

  </Tab>

  <Tab title="그룹 정책 + allowlist">
    그룹 액세스는 두 계층으로 구성됩니다.

    1. **그룹 멤버십 allowlist** (`channels.whatsapp.groups`)
       - `groups`가 생략되면 모든 그룹이 대상이 될 수 있습니다.
       - `groups`가 있으면 그룹 allowlist로 동작합니다(`"*"` 허용).
    ...
    - `groupAllowFrom`이 설정되지 않았으면 런타임은 가능할 경우 `allowFrom`으로 대체합니다.
    - 발신자 allowlist는 멘션/답글 활성화보다 먼저 평가됩니다.
  ...
  <Tab title="멘션 + /activation">
    그룹 답글은 기본적으로 멘션이 필요합니다.

    멘션 감지에는 다음이 포함됩니다.

    - 봇 ID에 대한 명시적인 WhatsApp 멘션
    - 구성된 멘션 정규식 패턴(`agents.list[].groupChat.mentionPatterns`, 대체값 `messages.groupChat.mentionPatterns`)
    - 암시적 봇 답글 감지(답글 발신자가 봇 ID와 일치)

    보안 참고:

    - 인용/답글은 멘션 게이팅만 충족하며, 발신자 인증을 부여하지는 않습니다.
    - `groupPolicy: "allowlist"`에서는 allowlist에 없는 발신자가 허용 목록 사용자 메시지에 답글하더라도 계속 차단됩니다.
    ...
    `activation`은 전역 config가 아니라 세션 상태를 업데이트합니다. owner 게이트가 적용됩니다.
...
- self-chat 턴에 대해 읽음 확인 생략
- 그렇지 않으면 자기 자신을 핑하게 되는 mention-JID 자동 트리거 동작 무시
- `messages.responsePrefix`가 설정되지 않은 경우 self-chat 답글 기본값은 `[{identity.name}]` 또는 `[openclaw]`

## 메시지 정규화 및 컨텍스트

<AccordionGroup>
  <Accordion title="인바운드 엔벌로프 + 답글 컨텍스트">
    들어오는 WhatsApp 메시지는 공통 인바운드 엔벌로프로 래핑됩니다.
    ...
    답글 메타데이터 필드도 사용 가능할 때 채워집니다(`ReplyToId`, `ReplyToBody`, `ReplyToSender`, sender JID/E.164).
  ...
  <Accordion title="미디어 플레이스홀더 및 위치/연락처 추출">
    ...
    위치 본문은 간결한 좌표 텍스트를 사용합니다. 위치 레이블/댓글과 연락처/vCard 세부 정보는 인라인 프롬프트 텍스트가 아니라 fenced untrusted metadata로 렌더링됩니다.
...
<AccordionGroup>
  <Accordion title="텍스트 청크 분할">
    ...
    - `newline` 모드는 문단 경계(빈 줄)를 우선하며, 이후 길이 기준 안전 청크 분할로 대체됩니다.
  </Accordion>

  <Accordion title="아웃바운드 미디어 동작">
    - 이미지, 비디오, 오디오(PTT 음성 노트), 문서 페이로드를 지원합니다.
    - `audio/ogg`는 음성 노트 호환성을 위해 `audio/ogg; codecs=opus`로 다시 작성됩니다.
    - 비디오 전송 시 `gifPlayback: true`를 통해 애니메이션 GIF 재생을 지원합니다.
    - 다중 미디어 답글 페이로드 전송 시 캡션은 첫 번째 미디어 항목에 적용됩니다.
    - 미디어 소스는 HTTP(S), `file://`, 로컬 경로를 사용할 수 있습니다.
  </Accordion>
  ...
  <Accordion title="미디어 크기 제한 및 대체 동작">
    ...
    - 미디어 전송 실패 시 첫 번째 항목 대체 동작은 응답을 조용히 버리는 대신 경고 텍스트를 보냅니다.
  </Accordion>
</AccordionGroup>

## 답글 인용

WhatsApp는 아웃바운드 답글이 인바운드 메시지를 눈에 보이게 인용하는 네이티브 답글 인용을 지원합니다. `channels.whatsapp.replyToMode`로 제어합니다.

| Value    | 동작                                                                               |
| -------- | ---------------------------------------------------------------------------------- |
| `"auto"` | 제공자가 지원하면 인바운드 메시지를 인용하고, 그렇지 않으면 인용을 건너뜁니다.    |
| `"on"`   | 항상 인바운드 메시지를 인용하며, 인용이 거부되면 일반 전송으로 대체합니다.         |
...
`channels.whatsapp.reactionLevel`은 에이전트가 WhatsApp에서 이모지 반응을 얼마나 넓게 사용하는지 제어합니다.

| Level         | Ack 반응 | 에이전트 시작 반응        | 설명                                              |
| ------------- | -------- | ------------------------- | ------------------------------------------------- |
| `"off"`       | 아니요   | 아니요                    | 반응을 전혀 사용하지 않음                         |
| `"ack"`       | 예       | 아니요                    | Ack 반응만 사용(답글 전 수신 확인)                |
| `"minimal"`   | 예       | 예(보수적)                | Ack + 보수적 지침에 따른 에이전트 반응            |
| `"extensive"` | 예       | 예(권장)                  | Ack + 권장 지침에 따른 에이전트 반응              |
...
WhatsApp는 `channels.whatsapp.ackReaction`을 통해 인바운드 수신 직후 즉시 ack 반응을 지원합니다.
Ack 반응은 `reactionLevel`의 제어를 받으며, `reactionLevel`이 `"off"`이면 억제됩니다.
...
- 인바운드가 허용된 직후(답글 전) 즉시 전송됩니다.
- 실패는 로그에 기록되지만 일반 답글 전송을 막지는 않습니다.
- 그룹 모드 `mentions`는 멘션으로 트리거된 턴에 반응합니다. 그룹 활성화 `always`는 이 확인을 우회하는 역할을 합니다.
...
<AccordionGroup>
  ...
  <Accordion title="그룹 메시지가 예상치 않게 무시됨">
    다음 순서로 확인하세요.
    ...
    - `openclaw.json`의 중복 키(JSON5): 나중 항목이 이전 항목을 덮어쓰므로 범위별로 `groupPolicy`는 하나만 유지하세요.
...
WhatsApp는 `groups` 및 `direct` 맵을 통해 그룹과 직접 채팅에 대해 Telegram 스타일 시스템 프롬프트를 지원합니다.

그룹 메시지에 대한 확인 계층:

먼저 유효한 `groups` 맵이 결정됩니다. 계정이 자체 `groups`를 정의하면 루트 `groups` 맵을 완전히 대체합니다(딥 머지 없음). 이후 프롬프트 조회는 결과로 나온 단일 맵에서 수행됩니다.

1. **그룹별 시스템 프롬프트** (`groups["<groupId>"].systemPrompt`): 해당 그룹 항목이 `systemPrompt`를 정의하면 사용됩니다.
2. **그룹 와일드카드 시스템 프롬프트** (`groups["*"].systemPrompt`): 해당 그룹 항목이 없거나 `systemPrompt`를 정의하지 않은 경우 사용됩니다.

직접 메시지에 대한 확인 계층:

먼저 유효한 `direct` 맵이 결정됩니다. 계정이 자체 `direct`를 정의하면 루트 `direct` 맵을 완전히 대체합니다(딥 머지 없음). 이후 프롬프트 조회는 결과로 나온 단일 맵에서 수행됩니다.

1. **직접 채팅별 시스템 프롬프트** (`direct["<peerId>"].systemPrompt`): 해당 peer 항목이 `systemPrompt`를 정의하면 사용됩니다.
2. **직접 채팅 와일드카드 시스템 프롬프트** (`direct["*"].systemPrompt`): 해당 peer 항목이 없거나 `systemPrompt`를 정의하지 않은 경우 사용됩니다.

참고: `dms`는 여전히 가벼운 DM별 히스토리 재정의 버킷(`dms.<id>.historyLimit`)이며, 프롬프트 재정의는 `direct` 아래에 있습니다.

**Telegram 멀티 계정 동작과의 차이점:** Telegram에서는 봇이 속하지 않은 그룹의 메시지를 받지 않도록, 멀티 계정 구성에서 루트 `groups`가 모든 계정에 대해 의도적으로 억제됩니다. 자체 `groups`를 정의하지 않은 계정에도 마찬가지입니다. WhatsApp는 이 가드를 적용하지 않습니다. 루트 `groups`와 루트 `direct`는 구성된 계정 수와 관계없이 계정 수준 재정의를 정의하지 않은 계정에 항상 상속됩니다. 멀티 계정 WhatsApp 구성에서 계정별 그룹 또는 직접 채팅 프롬프트가 필요하면 루트 수준 기본값에 의존하지 말고 각 계정 아래에 전체 맵을 명시적으로 정의하세요.

중요한 동작:

- `channels.whatsapp.groups`는 그룹별 config 맵이자 채팅 수준 그룹 allowlist이기도 합니다. 루트 또는 계정 범위 어디에서든 `groups["*"]`는 해당 범위에서 "모든 그룹이 허용됨"을 의미합니다.
- 해당 범위에서 실제로 모든 그룹을 허용하려는 경우에만 와일드카드 그룹 `systemPrompt`를 추가하세요. 여전히 고정된 그룹 ID 집합만 대상으로 두고 싶다면 프롬프트 기본값에 `groups["*"]`를 사용하지 마세요. 대신 명시적으로 allowlist에 넣은 각 그룹 항목에 동일한 프롬프트를 반복하세요.
- 그룹 허용과 발신자 인증은 별도의 검사입니다. `groups["*"]`는 그룹 처리를 통과할 수 있는 그룹 집합을 넓히지만, 그 자체만으로 해당 그룹의 모든 발신자를 인증하지는 않습니다. 발신자 액세스는 여전히 `channels.whatsapp.groupPolicy` 및 `channels.whatsapp.groupAllowFrom`으로 별도 제어됩니다.
- `channels.whatsapp.direct`는 DM에서 같은 부작용을 갖지 않습니다. `direct["*"]`는 DM이 이미 `dmPolicy`와 `allowFrom` 또는 pairing-store 규칙에 의해 허용된 후에만 기본 직접 채팅 config를 제공합니다.
