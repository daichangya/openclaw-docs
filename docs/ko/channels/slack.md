  ...
  <Tab title="Socket Mode (기본값)">
    <Steps>
      <Step title="새 Slack 앱 만들기">
        ...
        - 아래의 [예시 매니페스트](#manifest-and-scope-checklist)를 붙여 넣고 계속 진행해 생성합니다
  ...
  <Tab title="HTTP 요청 URL">
    <Steps>
      <Step title="새 Slack 앱 만들기">
        ...
        - **from a manifest**를 선택하고 앱에 사용할 워크스페이스를 선택합니다
        - [예시 매니페스트](#manifest-and-scope-checklist)를 붙여 넣고 생성 전에 URL을 업데이트합니다
        - 요청 검증을 위해 **Signing Secret**를 저장합니다
        - 앱을 설치하고 표시되는 **Bot Token**(`xoxb-...`)을 복사합니다
...
위 기본값을 확장하는 다양한 기능 표면입니다.

<AccordionGroup>
  <Accordion title="선택적 기본 슬래시 명령어">

    미묘한 차이와 함께 단일 구성 명령어 대신 여러 개의 [기본 슬래시 명령어](#commands-and-slash-behavior)를 사용할 수 있습니다:
...
```
  ...
  <Accordion title="선택적 작성자 범위(write 작업)">
    기본 Slack 앱 ID 대신 활성 에이전트 ID(사용자 지정 사용자 이름 및 아이콘)를 사용해 발신 메시지를 보내려면 `chat:write.customize` bot 스코프를 추가하세요.

    이모지 아이콘을 사용하는 경우 Slack은 `:emoji_name:` 형식을 기대합니다.

  </Accordion>
  <Accordion title="선택적 user-token 스코프(read 작업)">
    `channels.slack.userToken`을 구성하는 경우 일반적인 read 스코프는 다음과 같습니다:
    ...
    - `pins:read`
    - `emoji:read`
    - Slack 검색 읽기에 의존하는 경우 `search:read`
...
<Tip>
작업/디렉터리 읽기의 경우 구성되어 있으면 user token이 우선될 수 있습니다. write의 경우 bot token이 계속 우선되며, user-token write는 `userTokenReadOnly: false`이고 bot token을 사용할 수 없는 경우에만 허용됩니다.
</Tip>
...
<Tabs>
  <Tab title="DM 정책">
    `channels.slack.dmPolicy`가 DM 액세스를 제어합니다(레거시: `channels.slack.dm.policy`):
    ...
    DM의 pairing은 `openclaw pairing approve slack <code>`를 사용합니다.
  ...
  <Tab title="채널 정책">
    ...
    런타임 참고: `channels.slack`이 완전히 없는 경우(환경 변수만 사용하는 설정), 런타임은 `groupPolicy="allowlist"`로 대체되고 경고를 기록합니다(`channels.defaults.groupPolicy`가 설정되어 있어도 마찬가지).
  ...
  <Tab title="멘션 및 채널 사용자">
    ...
    - 봇에게 답장하는 암시적 스레드 동작(`thread.requireExplicitMention`이 `true`이면 비활성화)
...
- 기본 `session.dmScope=main`을 사용하면 Slack DM은 에이전트 기본 세션으로 합쳐집니다.
- 채널 세션: `agent:<agentId>:slack:channel:<channelId>`.
- 해당되는 경우 스레드 답글은 스레드 세션 접미사(`:thread:<threadTs>`)를 만들 수 있습니다.
...
- `channels.slack.thread.requireExplicitMention`(기본값 `false`): `true`이면 암시적 스레드 멘션을 억제하므로, 봇이 이미 해당 스레드에 참여한 경우에도 봇은 스레드 안의 명시적 `@bot` 멘션에만 응답합니다. 이 설정이 없으면 봇이 참여한 스레드의 답글은 `requireMention` 게이트를 우회합니다.
...
참고: `replyToMode="off"`는 명시적 `[[reply_to_*]]` 태그를 포함해 Slack의 **모든** 답글 스레딩을 비활성화합니다. 이는 `"off"` 모드에서도 명시적 태그가 계속 적용되는 Telegram과 다릅니다. 이 차이는 플랫폼의 스레딩 모델을 반영합니다. Slack 스레드는 메시지를 채널에서 숨기지만, Telegram 답글은 기본 채팅 흐름에 계속 표시됩니다.
...
- `progress`: 생성 중에는 진행 상태 텍스트를 표시하고, 이후 최종 텍스트를 전송합니다.
...
- 기본 텍스트 스트리밍 및 Slack assistant 스레드 상태를 표시하려면 답글 스레드를 사용할 수 있어야 합니다. 스레드 선택은 계속 `replyToMode`를 따릅니다.
- 기본 스트리밍을 사용할 수 없는 경우에도 채널 및 그룹 채팅 루트는 일반 초안 미리보기를 계속 사용할 수 있습니다.
- 최상위 Slack DM은 기본적으로 스레드 밖에 유지되므로 스레드 스타일 미리보기가 표시되지 않습니다. 그곳에서 진행 상태를 보이게 하려면 스레드 답글 또는 `typingReaction`을 사용하세요.
- 미디어 및 비텍스트 페이로드는 일반 전송으로 대체됩니다.
- 스트리밍이 답글 도중 실패하면 OpenClaw는 남은 페이로드를 일반 전송으로 대체합니다.
...
`typingReaction`은 OpenClaw가 답글을 처리하는 동안 인바운드 Slack 메시지에 임시 반응을 추가한 뒤, 실행이 끝나면 제거합니다. 이것은 기본 `"is typing..."` 상태 표시기를 사용하는 스레드 답글 외부에서 특히 유용합니다.
...
- 이 반응은 best-effort 방식이며, 답글 또는 실패 경로가 완료된 후 자동 정리를 시도합니다.
...
<AccordionGroup>
  ...
  <Accordion title="아웃바운드 텍스트 및 파일">
    ...
    - 아웃바운드 미디어 상한은 `channels.slack.mediaMaxMb`가 구성된 경우 이를 따르고, 그렇지 않으면 채널 전송은 미디어 파이프라인의 MIME 종류 기본값을 따릅니다
  </Accordion>
...
슬래시 명령어는 Slack에서 하나의 구성된 명령어 또는 여러 기본 명령어로 표시됩니다. 명령어 기본값을 변경하려면 `channels.slack.slashCommand`를 구성하세요:
...
기본 명령어를 사용하려면 Slack 앱에서 [추가 매니페스트 설정](#additional-manifest-settings)이 필요하며, 대신 `channels.slack.commands.native: true` 또는 전역 구성의 `commands.native: true`로 활성화합니다.

- Slack의 기본 명령어 자동 모드는 **비활성화**되어 있으므로 `commands.native: "auto"`는 Slack 기본 명령어를 활성화하지 않습니다.
...
- 최대 5개 옵션: 버튼 블록
- 6-100개 옵션: 정적 선택 메뉴
- 100개 초과 옵션: interactivity 옵션 핸들러를 사용할 수 있으면 비동기 옵션 필터링이 포함된 외부 선택
...
- Plugin 승인은 요청이 이미 Slack에 도착해 있고 승인 ID 종류가 `plugin:`인 경우 동일한 Slack 기본 버튼 표면을 통해 계속 처리될 수 있습니다.
...
이 버튼이 मौजूद하면, 이것이 기본 승인 UX가 되며 OpenClaw는
...
승인자를 재정의하거나 필터를 추가하거나 원본 채팅 전송을 선택하려는 경우에만 명시적 Slack 기본 구성이 필요합니다:
...
공유 `approvals.exec` 전달은 별개입니다. exec 승인 프롬프트를 다른 채팅이나 명시적인 대역 외 대상으로도 라우팅해야 하는 경우에만 이것을 사용하세요. 공유 `approvals.plugin` 전달도 별개입니다. Slack 기본 버튼은 해당 요청이 이미 Slack에 도착한 경우에도 plugin 승인을 계속 처리할 수 있습니다.
...
- 스레드 시작자 및 초기 스레드 히스토리 컨텍스트 시딩은 적용 가능한 경우 구성된 발신자 허용 목록에 따라 필터링됩니다.
- 블록 작업 및 모달 상호작용은 풍부한 페이로드 필드를 포함한 구조화된 `Slack interaction: ...` 시스템 이벤트를 출력합니다:
...
- [구성 참조 - Slack](/ko/gateway/configuration-reference#slack)
  ...
  - 호환성 토글: `dangerouslyAllowNameMatching` (최후 수단, 필요하지 않다면 끄세요)
