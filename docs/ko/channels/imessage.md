...
read_when:
    - iMessage 지원 설정하기
    - iMessage 송수신 디버깅하기
summary: imsg를 통한 레거시 iMessage 지원(`stdio`를 통한 JSON-RPC). 새로 설정하는 경우에는 BlueBubbles를 사용해야 합니다.
title: iMessage
x-i18n:
    generated_at: "2026-04-24T06:03:43Z"
    model: gpt-5.4
    provider: openai
...
상태: 레거시 외부 CLI 통합. Gateway는 `imsg rpc`를 생성하고 stdio에서 JSON-RPC로 통신합니다(별도의 데몬/포트 없음).

<CardGroup cols={3}>
  <Card title="BlueBubbles (권장)" icon="message-circle" href="/ko/channels/bluebubbles">
    새 설정에서 선호되는 iMessage 경로입니다.
  </Card>
...
```
  ...
  <Tab title="SSH를 통한 원격 Mac">
    OpenClaw는 stdio 호환 `cliPath`만 필요하므로, 원격 Mac에 SSH로 연결해 `imsg`를 실행하는 래퍼 스크립트를 `cliPath`로 지정할 수 있습니다.
...
```
    ...
    `remoteHost`가 설정되지 않으면 OpenClaw는 SSH 래퍼 스크립트를 파싱해 자동 감지하려고 시도합니다.
    `remoteHost`는 `host` 또는 `user@host`여야 합니다(공백이나 SSH 옵션 불가).
    OpenClaw는 SCP에 대해 엄격한 호스트 키 검사를 사용하므로, 릴레이 호스트 키가 이미 `~/.ssh/known_hosts`에 있어야 합니다.
...
- Messages.app를 통해 메시지를 보내려면 자동화 권한이 필요합니다.

<Tip>
권한은 프로세스 컨텍스트별로 부여됩니다. Gateway가 헤드리스로 실행되는 경우(LaunchAgent/SSH), 동일한 컨텍스트에서 한 번 대화형 명령을 실행해 프롬프트를 트리거하세요:
...
<Tabs>
  ...
  <Tab title="그룹 정책 + 멘션">
    ...
    런타임 폴백: `groupAllowFrom`이 설정되지 않으면, 사용 가능한 경우 iMessage 그룹 발신자 검사는 `allowFrom`으로 폴백합니다.
    런타임 참고: `channels.imessage`가 완전히 없으면, 런타임은 `groupPolicy="allowlist"`로 폴백하고 경고를 기록합니다(`channels.defaults.groupPolicy`가 설정되어 있더라도).
  ...
  <Tab title="세션 및 결정적 응답">
    ...
    - 기본 `session.dmScope=main`에서는 iMessage DM이 에이전트 기본 세션으로 병합됩니다.
    - 그룹 세션은 격리됩니다(`agent:<agentId>:imessage:group:<chat_id>`).
    - 응답은 원래의 채널/대상 메타데이터를 사용해 iMessage로 다시 라우팅됩니다.
...
레거시 iMessage 채팅은 ACP 세션에도 바인딩할 수 있습니다.
...
<AccordionGroup>
  ...
  <Accordion title="Tailscale을 통한 원격 Mac (예시)">
    ...
    - iMessage + `imsg`는 사용자의 tailnet 내 Mac에서 실행
...
```
    ...
    먼저 호스트 키를 신뢰하도록 설정해야 합니다(예: `ssh bot@mac-mini.tailnet-1234.ts.net`). 그래야 `known_hosts`가 채워집니다.
