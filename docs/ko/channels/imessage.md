...
read_when:
    - iMessage 지원 설정하기
    - iMessage 송수신 디버깅
summary: '`imsg`를 통한 레거시 iMessage 지원(`stdio`를 통한 JSON-RPC)입니다. 새로 설정하는 경우에는 BlueBubbles를 사용해야 합니다.'
title: iMessage
x-i18n:
...
<Warning>
새로운 iMessage 배포에는 <a href="/ko/channels/bluebubbles">BlueBubbles</a>를 사용하세요.

`imsg` 통합은 레거시이며 향후 릴리스에서 제거될 수 있습니다.
</Warning>

상태: 레거시 외부 CLI 통합. Gateway가 `imsg rpc`를 실행하고 stdio의 JSON-RPC를 통해 통신합니다(별도의 데몬/포트 없음).
...
<CardGroup cols={3}>
  ...
  <Card title="구성 참조" icon="settings" href="/ko/gateway/configuration-reference#imessage">
...
<Tip>
권한은 프로세스 컨텍스트별로 부여됩니다. Gateway가 헤드리스(LaunchAgent/SSH)로 실행되는 경우, 동일한 컨텍스트에서 한 번 대화형 명령을 실행해 권한 프롬프트를 트리거하세요:
...
<Tabs>
  <Tab title="DM 정책">
    ...
    허용 목록 항목은 핸들이거나 채팅 대상일 수 있습니다(`chat_id:*`, `chat_guid:*`, `chat_identifier:*`).
  ...
  <Tab title="그룹 정책 + 멘션">
    ...
    런타임 폴백: `groupAllowFrom`이 설정되지 않으면, 가능한 경우 iMessage 그룹 발신자 검사는 `allowFrom`으로 폴백합니다.
    런타임 참고: `channels.imessage`가 완전히 누락된 경우, 런타임은 `groupPolicy="allowlist"`로 폴백하고 경고를 기록합니다(`channels.defaults.groupPolicy`가 설정되어 있더라도 마찬가지).
  ...
  <Tab title="세션 및 결정적 응답">
    ...
    - 기본 `session.dmScope=main`을 사용하면 iMessage DM은 에이전트 메인 세션으로 합쳐집니다.
    - 그룹 세션은 격리됩니다(`agent:<agentId>:imessage:group:<chat_id>`).
    - 응답은 원래의 채널/대상 메타데이터를 사용해 다시 iMessage로 라우팅됩니다.
...
- `/new`와 `/reset`은 같은 바인딩된 ACP 세션을 해당 위치에서 재설정합니다.
- `/acp close`는 ACP 세션을 닫고 바인딩을 제거합니다.

구성된 영구 바인딩은 `type: "acp"` 및 `match.channel: "imessage"`를 가진 최상위 `bindings[]` 항목을 통해 지원됩니다.
...
```
    ...
    SSH와 SCP 모두 비대화형으로 동작하도록 SSH 키를 사용하세요.
    먼저 호스트 키를 신뢰하도록 설정하세요(예: `ssh bot@mac-mini.tailnet-1234.ts.net`). 그러면 `known_hosts`가 채워집니다.
...
```
    ...
    프로브에서 RPC 미지원이 보고되면 `imsg`를 업데이트하세요.
...
```
    ...
    OpenClaw/`imsg`를 실행하는 프로세스 컨텍스트에 전체 디스크 접근 및 자동화 권한이 부여되었는지 확인하세요.
...
- [구성 참조 - iMessage](/ko/gateway/configuration-reference#imessage)
