...
이 Plugin은 Synology Chat 발신 Webhook의 수신 메시지를 받아들이고, Synology Chat 수신 Webhook을 통해 답장을 보냅니다.

## 번들 Plugin

Synology Chat는 현재 OpenClaw 릴리스에서 번들 Plugin으로 제공되므로, 일반적인 패키지 빌드에서는 별도 설치가 필요하지 않습니다.

이전 빌드이거나 Synology Chat가 제외된 커스텀 설치를 사용하는 경우,
수동으로 설치하세요:
...
1. Synology Chat Plugin을 사용할 수 있는지 확인합니다.
   ...
   - 이전/커스텀 설치에서는 위 명령으로 소스 체크아웃에서 수동 추가할 수 있습니다.
...
3. 발신 Webhook URL을 OpenClaw Gateway로 지정합니다:
...
- OpenClaw는 발신 Webhook 토큰을 `body.token`, 그다음
  `?token=...`, 그다음 헤더 순서로 받아들입니다.
- 허용되는 헤더 형식:
  - `x-synology-token`
  - `x-webhook-token`
  - `x-openclaw-token`
  - `Authorization: Bearer <token>`
- 비어 있거나 누락된 토큰은 실패 시 차단됩니다.
...
- `allowlist` 모드에서 `allowedUserIds` 목록이 비어 있으면 잘못된 구성으로 처리되며 Webhook 라우트가 시작되지 않습니다(모두 허용하려면 `dmPolicy: "open"` 사용).
- `dmPolicy: "open"`은 모든 발신자를 허용합니다.
- `dmPolicy: "disabled"`는 DM을 차단합니다.
- 답장 수신자 바인딩은 기본적으로 안정적인 숫자 `user_id`에 유지됩니다. `channels.synology-chat.dangerouslyAllowNameMatching: true`는 변경 가능한 사용자 이름/닉네임 조회를 통한 답장 전달을 다시 활성화하는 비상 호환 모드입니다.
- 페어링 승인에는 다음을 사용할 수 있습니다:
...
발신 파일 URL은 `http` 또는 `https`를 사용해야 하며, 비공개 또는 그 외 차단된 네트워크 대상은 OpenClaw가 URL을 NAS Webhook으로 전달하기 전에 거부됩니다.

## 다중 계정

여러 Synology Chat 계정은 `channels.synology-chat.accounts` 아래에서 지원됩니다.
각 계정은 토큰, 수신 URL, Webhook 경로, DM 정책, 제한을 재정의할 수 있습니다.
직접 메시지 세션은 계정 및 사용자별로 격리되므로, 서로 다른 두 Synology 계정에서 동일한 숫자 `user_id`를 사용해도 기록 상태를 공유하지 않습니다.
활성화된 각 계정에는 고유한 `webhookPath`를 지정하세요. 이제 OpenClaw는 정확히 같은 중복 경로를 거부하며,
다중 계정 설정에서 공유 Webhook 경로만 상속하는 이름 있는 계정의 시작도 거부합니다.
의도적으로 이름 있는 계정에 레거시 상속이 필요한 경우,
해당 계정이나 `channels.synology-chat`에 `dangerouslyAllowInheritedWebhookPath: true`를 설정할 수 있지만,
정확히 같은 중복 경로는 여전히 실패 시 차단됩니다. 명시적인 계정별 경로를 권장합니다.
...
- 수신 Webhook 요청은 토큰 검증과 발신자별 속도 제한이 적용됩니다.
- 잘못된 토큰 검사는 상수 시간 비밀 비교를 사용하며 실패 시 차단됩니다.
- 프로덕션에서는 `dmPolicy: "allowlist"`를 권장합니다.
- 레거시 사용자 이름 기반 답장 전달이 명시적으로 필요한 경우가 아니면 `dangerouslyAllowNameMatching`을 끄세요.
- 다중 계정 설정에서 공유 경로 라우팅 위험을 명시적으로 수용하는 경우가 아니면 `dangerouslyAllowInheritedWebhookPath`를 끄세요.

## 문제 해결

- `Missing required fields (token, user_id, text)`:
  - 발신 Webhook 페이로드에 필요한 필드 중 하나가 누락되었습니다
...
- `Rate limit exceeded`:
  - 동일한 출처에서 너무 많은 잘못된 토큰 시도로 인해 해당 출처가 일시적으로 차단될 수 있습니다
