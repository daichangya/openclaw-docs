...
Mattermost는 자체 호스팅이 가능한 팀 메시징 플랫폼입니다. 제품 세부 정보와 다운로드는
공식 사이트 [mattermost.com](https://mattermost.com)에서 확인하세요.
...
이전 빌드 또는 Mattermost가 제외된 사용자 지정 설치를 사용 중이라면,
수동으로 설치하세요:
...
네이티브 슬래시 명령어는 옵트인 방식입니다. 활성화하면 OpenClaw가 Mattermost API를 통해 `oc_*` 슬래시 명령어를 등록하고 Gateway HTTP 서버에서 콜백 POST를 받습니다.
...
- `callbackUrl`을 생략하면 OpenClaw가 Gateway 호스트/포트와 `callbackPath`를 조합해 자동으로 생성합니다.
- 멀티 계정 구성에서는 `commands`를 최상위 레벨에 두거나
  `channels.mattermost.accounts.<id>.commands` 아래에 둘 수 있습니다(계정 값이 최상위 필드를 덮어씁니다).
- 명령어 콜백은 OpenClaw가 `oc_*` 명령어를 등록할 때 Mattermost가 반환한
  명령어별 토큰으로 검증됩니다.
- 슬래시 콜백은 등록 실패, 부분 시작, 또는
  콜백 토큰이 등록된 명령어와 일치하지 않는 경우 fail closed 방식으로 거부됩니다.
- 연결 가능성 요구사항: 콜백 엔드포인트는 Mattermost 서버에서 도달 가능해야 합니다.
...
- `channels.mattermost.requireMention`은 레거시 구성에서 계속 적용되지만 `chatmode` 사용을 권장합니다.

## 스레드와 세션

`channels.mattermost.replyToMode`를 사용해 채널 및 그룹 응답을
메인 채널에 유지할지, 아니면 트리거한 게시물 아래에 스레드를 시작할지 제어할 수 있습니다.
...
- 런타임 참고: `channels.mattermost`가 완전히 누락되면 런타임은 그룹 확인에 대해 `groupPolicy="allowlist"`로 대체합니다(`channels.defaults.groupPolicy`가 설정되어 있어도 동일).
...
(사용자 ID인지 채널 ID인지 구분 불가).

OpenClaw는 이를 **사용자 우선**으로 확인합니다:

- 해당 ID가 사용자로 존재하면(`GET /api/v4/users/<id>` 성공), OpenClaw는 `/api/v4/channels/direct`를 통해 다이렉트 채널을 확인해 **DM**을 전송합니다.
- 그렇지 않으면 해당 ID를 **채널 ID**로 처리합니다.

결정적인 동작이 필요하면 항상 명시적 접두사(`user:<id>` / `channel:<id>`)를 사용하세요.

## DM 채널 재시도

OpenClaw가 Mattermost DM 대상으로 전송할 때 먼저 다이렉트 채널을 확인해야 하면,
기본적으로 일시적인 다이렉트 채널 생성 실패를 재시도합니다.

Mattermost Plugin 전체에 대해 이 동작을 조정하려면 `channels.mattermost.dmChannelRetry`를,
한 계정에 대해서만 조정하려면 `channels.mattermost.accounts.<id>.dmChannelRetry`를 사용하세요.
...
- 이것은 모든 Mattermost API 호출이 아니라 DM 채널 생성(`/api/v4/channels/direct`)에만 적용됩니다.
- 재시도는 rate limit, 5xx 응답, 네트워크 또는 타임아웃 오류 같은 일시적 실패에 적용됩니다.
- `429`를 제외한 4xx 클라이언트 오류는 영구 실패로 처리되며 재시도하지 않습니다.

## 미리보기 스트리밍

Mattermost는 생각 과정, 도구 활동, 부분 응답 텍스트를 하나의 **초안 미리보기 게시물**로 스트리밍하고, 최종 답변을 안전하게 보낼 수 있을 때 그 자리에서 마무리합니다. 미리보기는 청크별 메시지로 채널을 어지럽히는 대신 같은 게시물 ID에서 업데이트됩니다. 미디어/오류 최종 응답은 대기 중인 미리보기 편집을 취소하고 버려지는 미리보기 게시물을 비우는 대신 일반 전송을 사용합니다.
...
- `partial`이 일반적인 선택입니다. 응답이 늘어남에 따라 편집되는 하나의 미리보기 게시물을 사용한 뒤, 전체 답변으로 마무리합니다.
- `block`은 미리보기 게시물 안에서 추가형 초안 청크를 사용합니다.
- `progress`는 생성 중 상태 미리보기를 표시하고 완료 시 최종 답변만 게시합니다.
- `off`는 미리보기 스트리밍을 비활성화합니다.
- 스트림을 그 자리에서 마무리할 수 없는 경우(예: 스트리밍 중 게시물이 삭제된 경우), OpenClaw는 새 최종 게시물을 전송하는 방식으로 대체하여 응답이 절대 유실되지 않도록 합니다.
- 채널 매핑 행렬은 [Streaming](/ko/concepts/streaming#preview-streaming-modes)을 참고하세요.
...
- 리액션을 제거하려면 `remove=true`(불리언)를 설정하세요.
...
선택 내용을 받고 응답할 수 있습니다.

채널 기능에 `inlineButtons`를 추가해 버튼을 활성화하세요:
...
`buttons` 매개변수와 함께 `message action=send`를 사용하세요. 버튼은 2차원 배열(버튼 행)입니다:
...
- `text`(필수): 표시 라벨.
- `callback_data`(필수): 클릭 시 다시 전송되는 값(작업 ID로 사용).
- `style`(선택 사항): `"default"`, `"primary"`, 또는 `"danger"`.

사용자가 버튼을 클릭하면:

1. 모든 버튼이 확인 줄로 대체됩니다(예: "✓ **Yes** selected by @user").
2. 에이전트가 선택 내용을 인바운드 메시지로 받아 응답합니다.

참고:

- 버튼 콜백은 HMAC-SHA256 검증을 사용합니다(자동, 구성 불필요).
- Mattermost는 API 응답에서 callback data를 제거하므로(보안 기능), 클릭 시 모든 버튼이 제거됩니다. 부분 제거는 불가능합니다.
...
- `channels.mattermost.interactions.callbackBaseUrl`: 버튼 콜백용 선택적 외부 base URL입니다
  (예: `https://gateway.example.com`). Mattermost가 Gateway의 바인드 호스트에 직접
  도달할 수 없을 때 사용하세요.
- 멀티 계정 구성에서는 같은 필드를
...
- `interactions.callbackBaseUrl`을 생략하면 OpenClaw는
  `gateway.customBindHost` + `gateway.port`에서 콜백 URL을 생성한 뒤,
  `http://localhost:<port>`로 대체합니다.
- 연결 가능성 규칙: 버튼 콜백 URL은 Mattermost 서버에서 도달 가능해야 합니다.
  `localhost`는 Mattermost와 OpenClaw가 같은 호스트/네트워크 네임스페이스에서 실행될 때만 동작합니다.
...
직접 버튼을 게시할 수 있습니다. 가능하면 extension의 `buildButtonAttachments()`를 사용하세요.
raw JSON을 게시하는 경우에는 다음 규칙을 따르세요:
...
1. 첨부 파일은 최상위 `attachments`가 아니라 `props.attachments`에 넣어야 합니다(그렇지 않으면 조용히 무시됨).
2. 모든 액션에는 `type: "button"`이 필요합니다. 없으면 클릭이 조용히 삼켜집니다.
3. 모든 액션에는 `id` 필드가 필요합니다. Mattermost는 ID가 없는 액션을 무시합니다.
4. 액션 `id`는 **영숫자만 허용**됩니다(`[a-zA-Z0-9]`). 하이픈과 밑줄은
   Mattermost 서버 측 액션 라우팅을 깨뜨립니다(404 반환). 사용 전에 제거하세요.
5. `context.action_id`는 버튼의 `id`와 일치해야 확인 메시지에 raw ID 대신
   버튼 이름(예: "Approve")이 표시됩니다.
...
3. **정렬된 키**와 **공백 없는 형식**으로 직렬화합니다(Gateway는 정렬된 키와 함께
...
- 항상 **모든** context 필드(`_token` 제외)에 서명하세요. Gateway는 `_token`을 제거한 뒤
  남은 모든 것에 서명합니다. 일부만 서명하면 조용히 검증에 실패합니다.
- `sort_keys=True`를 사용하세요. Gateway는 서명 전에 키를 정렬하며,
  Mattermost는 페이로드 저장 시 context 필드 순서를 바꿀 수 있습니다.
- secret은 무작위 바이트가 아니라 봇 토큰에서 파생하세요(결정적 방식). 버튼을 생성하는 프로세스와 검증하는 Gateway에서
  동일한 secret이어야 합니다.
...
`openclaw message send` 및 cron/Webhook 전송에서 `#channel-name`과 `@username`
대상을 사용할 수 있습니다.
...
- 채널에서 응답이 없음: 봇이 채널에 있는지 확인하고 멘션하세요(oncall). 트리거 접두사를 사용하거나(onchar) `chatmode: "onmessage"`로 설정하세요.
- 인증 오류: 봇 토큰, base URL, 계정 활성화 여부를 확인하세요.
- 멀티 계정 문제: 환경 변수는 `default` 계정에만 적용됩니다.
- 네이티브 슬래시 명령어가 `Unauthorized: invalid command token.`을 반환함: OpenClaw가
  콜백 토큰을 수락하지 않았습니다. 일반적인 원인:
  - 슬래시 명령어 등록이 실패했거나 시작 시 일부만 완료됨
  - 콜백이 잘못된 Gateway/계정에 도달하고 있음
  - Mattermost에 이전 콜백 대상을 가리키는 오래된 명령어가 남아 있음
  - Gateway가 슬래시 명령어를 다시 활성화하지 않은 채 재시작됨
- 네이티브 슬래시 명령어가 작동을 멈췄다면 로그에서
  `mattermost: failed to register slash commands` 또는
  `mattermost: native slash commands enabled but no commands could be registered`
  를 확인하세요.
- `callbackUrl`을 생략했고 로그에 콜백이
  `http://127.0.0.1:18789/...`로 확인되었다는 경고가 나오면, 해당 URL은
  Mattermost가 OpenClaw와 같은 호스트/네트워크 네임스페이스에서 실행될 때만
  도달 가능할 수 있습니다. 대신 외부에서 도달 가능한 명시적
...
- 버튼 클릭 시 404 반환: 버튼 `id`에 하이픈 또는 밑줄이 포함되어 있을 가능성이 높습니다. Mattermost의 액션 라우터는 영숫자가 아닌 ID에서 깨집니다. `[a-zA-Z0-9]`만 사용하세요.
- Gateway 로그에 `invalid _token` 표시: HMAC 불일치입니다. 모든 context 필드(일부가 아닌 전체)에 서명하는지, 키 정렬을 사용하는지, 압축 JSON(공백 없음)을 사용하는지 확인하세요. 위 HMAC 섹션을 참고하세요.
- Gateway 로그에 `missing _token in context` 표시: 버튼 context에 `_token` 필드가 없습니다. integration 페이로드를 만들 때 포함되었는지 확인하세요.
- 확인 메시지에 버튼 이름 대신 raw ID가 표시됨: `context.action_id`가 버튼의 `id`와 일치하지 않습니다. 둘 다 같은 정리된 값으로 설정하세요.
