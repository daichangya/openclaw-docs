...
상태: grammY를 통한 봇 DM + 그룹에 대해 프로덕션 준비 완료. 기본 모드는 롱 폴링이며, Webhook 모드는 선택 사항입니다.
...
```
    ...
    환경 변수 대체값: `TELEGRAM_BOT_TOKEN=...` (기본 계정 전용).
...
<AccordionGroup>
  <Accordion title="개인정보 보호 모드와 그룹 가시성">
    Telegram 봇은 기본적으로 **개인정보 보호 모드**가 활성화되어 있으며, 이 모드는 수신할 수 있는 그룹 메시지를 제한합니다.
...
<Tabs>
  <Tab title="DM 정책">
    ...
    - `pairing` (기본값)
    - `allowlist` (`allowFrom`에 발신자 ID가 최소 1개 있어야 함)
    - `open` (`allowFrom`에 `"*"`가 포함되어 있어야 함)
    - `disabled`

    `channels.telegram.allowFrom`은 숫자형 Telegram 사용자 ID를 받습니다. `telegram:` / `tg:` 접두사는 허용되며 정규화됩니다.
    빈 `allowFrom`으로 `dmPolicy: "allowlist"`를 설정하면 모든 DM이 차단되며 config 유효성 검사에서 거부됩니다.
    설정 시 숫자형 사용자 ID만 요청됩니다.
    업그레이드 후 config에 `@username` allowlist 항목이 포함되어 있다면, `openclaw doctor --fix`를 실행해 이를 해결하세요(최선의 노력 기준이며 Telegram 봇 토큰이 필요합니다).
    ...
    단일 소유자 봇의 경우, 이전 페어링 승인에 의존하는 대신 액세스 정책을 config에 지속적으로 유지하려면 명시적인 숫자형 `allowFrom` ID와 함께 `dmPolicy: "allowlist"`를 사용하는 것이 좋습니다.

    흔한 혼동: DM 페어링 승인이 "이 발신자가 어디서나 인증되었다"는 뜻은 아닙니다.
    페어링은 DM 액세스만 부여합니다. 그룹 발신자 인증은 여전히 명시적 config allowlist에서만 옵니다.
    "한 번 인증하면 DM과 그룹 명령이 모두 동작"하게 하려면, 숫자형 Telegram 사용자 ID를 `channels.telegram.allowFrom`에 넣으세요.
...
```
  ...
  <Tab title="그룹 정책 및 allowlist">
    ...
    1. **허용되는 그룹** (`channels.telegram.groups`)
       - `groups` config 없음:
         - `groupPolicy: "open"`인 경우: 어떤 그룹이든 group-ID 검사 통과 가능
    ...
    보안 경계 (`2026.2.25+`): 그룹 발신자 인증은 DM 페어링 저장소 승인 내역을 상속하지 **않습니다**.
    ...
    단일 소유자 봇을 위한 실용 패턴: 사용자 ID를 `channels.telegram.allowFrom`에 설정하고, `groupAllowFrom`은 비워 둔 채 대상 그룹을 `channels.telegram.groups` 아래에서 허용하세요.
...
```
  ...
  <Tab title="멘션 동작">
    ...
    이는 세션 상태만 업데이트합니다. 영구적으로 유지하려면 config를 사용하세요.
...
- 인바운드 메시지는 응답 메타데이터와 미디어 플레이스홀더를 포함한 공유 채널 엔벌로프로 정규화됩니다.
- 그룹 세션은 그룹 ID별로 격리됩니다. 포럼 토픽은 토픽 격리를 유지하기 위해 `:topic:<threadId>`를 추가합니다.
- DM 메시지는 `message_thread_id`를 포함할 수 있으며, OpenClaw는 이를 스레드 인식 세션 키로 라우팅하고 응답 시 스레드 ID를 유지합니다.
- 롱 폴링은 채팅별/스레드별 시퀀싱과 함께 grammY runner를 사용합니다. 전체 runner sink 동시성은 `agents.defaults.maxConcurrent`를 사용합니다.
- 롱 폴링 watchdog 재시작은 기본적으로 완료된 `getUpdates` 활성 상태가 120초 동안 없을 때 트리거됩니다. 배포 환경에서 긴 작업 중에도 거짓 polling-stall 재시작이 계속 발생하는 경우에만 `channels.telegram.pollingStallThresholdMs`를 늘리세요. 값의 단위는 밀리초이며 `30000`부터 `600000`까지 허용되고, 계정별 재정의도 지원됩니다.
...
<AccordionGroup>
  <Accordion title="실시간 스트림 미리보기(메시지 편집)">
    ...
    - `streaming.preview.toolProgress`는 도구/진행 상황 업데이트가 동일한 편집된 미리보기 메시지를 재사용할지 제어합니다(기본값: `true`). 별도의 도구/진행 메시지를 유지하려면 `false`로 설정하세요.
    - 레거시 `channels.telegram.streamMode` 및 불리언 `streaming` 값은 자동으로 매핑됩니다
    ...
    미리보기 스트리밍은 블록 스트리밍과 별개입니다. Telegram에서 블록 스트리밍이 명시적으로 활성화되면, OpenClaw는 이중 스트리밍을 피하기 위해 미리보기 스트림을 건너뜁니다.

    기본 초안 전송을 사용할 수 없거나 거부되면, OpenClaw는 자동으로 `sendMessage` + `editMessageText`로 대체합니다.
...
```
    ...
    - Telegram 메뉴에 표시되지 않아도 plugin/skill 명령은 직접 입력하면 여전히 동작할 수 있습니다

    기본 명령이 비활성화되면 내장 명령은 제거됩니다. 사용자 지정/plugin 명령은 구성되어 있다면 여전히 등록될 수 있습니다.

    흔한 설정 실패:

    - `setMyCommands failed`와 함께 `BOT_COMMANDS_TOO_MUCH`가 발생하면, 정리 후에도 Telegram 메뉴가 여전히 초과된 것입니다. plugin/skill/사용자 지정 명령 수를 줄이거나 `channels.telegram.commands.native`를 비활성화하세요.
    - `setMyCommands failed`와 함께 네트워크/fetch 오류가 발생하면, 일반적으로 `api.telegram.org`에 대한 아웃바운드 DNS/HTTPS가 차단된 상태입니다.
    ...
    4. 요청을 승인합니다:
       ...
       - 대기 중인 요청이 하나뿐일 때는 `/pair approve`
       - 가장 최근 요청은 `/pair approve latest`

    설정 코드는 수명이 짧은 bootstrap 토큰을 포함합니다. 내장 bootstrap handoff는 기본 node 토큰을 `scopes: []`로 유지합니다. handoff된 operator 토큰은 `operator.approvals`, `operator.read`, `operator.talk.secrets`, `operator.write`로 제한됩니다. Bootstrap 범위 검사는 역할 접두사를 사용하므로, 해당 operator allowlist는 operator 요청에만 충족되며, operator가 아닌 역할은 여전히 자신의 역할 접두사 아래 범위가 필요합니다.

    기기가 인증 세부 정보(예: 역할/범위/공개 키)를 변경한 상태로 재시도하면, 이전 대기 요청은 대체되고 새 요청은 다른 `requestId`를 사용합니다. 승인 전에 `/pair pending`을 다시 실행하세요.
...
```
  ...
  <Accordion title="에이전트 및 자동화를 위한 Telegram 메시지 작업">
    ...
    런타임 전송은 활성 config/보안 정보 스냅샷(시작/리로드 시점)을 사용하므로, 작업 경로는 전송마다 임시로 SecretRef를 다시 확인하지 않습니다.
  ...
  <Accordion title="포럼 토픽 및 스레드 동작">
    ...
    **토픽별 에이전트 라우팅**: 각 토픽은 토픽 config에서 `agentId`를 설정하여 다른 에이전트로 라우팅할 수 있습니다. 이를 통해 각 토픽은 자체적으로 격리된 작업 공간, 메모리, 세션을 갖습니다. 예시:
    ...
    **지속적 ACP 토픽 바인딩**: 포럼 토픽은 최상위 타입 지정 ACP 바인딩을 통해 ACP 하네스 세션을 고정할 수 있습니다.
    ...
    이는 현재 그룹 및 슈퍼그룹의 포럼 토픽에 한정됩니다.

    **채팅에서 시작하는 스레드 바인딩 ACP 스폰**:

    - `/acp spawn <agent> --thread here|auto`는 현재 Telegram 토픽을 새 ACP 세션에 바인딩할 수 있습니다.
    - 이후의 토픽 메시지는 바인딩된 ACP 세션으로 직접 라우팅됩니다(`/acp steer` 불필요).
    - OpenClaw는 바인딩 성공 후 스폰 확인 메시지를 해당 토픽에 고정합니다.
...
```
  ...
  <Accordion title="반응 알림">
    Telegram 반응은 `message_reaction` 업데이트로 도착합니다(메시지 페이로드와는 별도).
    ...
    - `own`은 봇이 보낸 메시지에 대한 사용자 반응만 의미합니다(전송된 메시지 캐시를 통한 best-effort 기준).
    - 반응 이벤트는 여전히 Telegram 액세스 제어(`dmPolicy`, `allowFrom`, `groupPolicy`, `groupAllowFrom`)를 따릅니다. 인증되지 않은 발신자는 폐기됩니다.
    - Telegram은 반응 업데이트에 스레드 ID를 제공하지 않습니다.
      - 포럼이 아닌 그룹은 그룹 채팅 세션으로 라우팅됩니다
      - 포럼 그룹은 정확한 원본 토픽이 아니라 그룹 일반 토픽 세션(`:topic:1`)으로 라우팅됩니다

    폴링/Webhook의 `allowed_updates`에는 자동으로 `message_reaction`이 포함됩니다.
  ...
  <Accordion title="Telegram 이벤트 및 명령으로 인한 config 쓰기">
...
```
  ...
  <Accordion title="롱 폴링과 Webhook">
    ...
    공개 엔드포인트가 다르면, 앞단에 리버스 프록시를 두고 `webhookUrl`을 공개 URL로 지정하세요.
  ...
  <Accordion title="제한, 재시도 및 CLI 대상">
    ...
    - `channels.telegram.pollingStallThresholdMs` 기본값은 `120000`이며, 거짓 양성 polling-stall 재시작에 대해서만 `30000`에서 `600000` 사이로 조정하세요.
    ...
    - Telegram allowlist는 주로 누가 에이전트를 트리거할 수 있는지를 제어하며, 완전한 보조 컨텍스트 리다이렉션 경계는 아닙니다.
    - DM 기록 제어:
...
```
    ...
    - 압축된 사진 또는 애니메이션 미디어 업로드 대신 아웃바운드 이미지 및 GIF를 문서로 전송하는 `--force-document`
  ...
  <Accordion title="Telegram의 exec 승인">
    ...
    승인자는 숫자형 Telegram 사용자 ID여야 합니다. `enabled`가 설정되지 않았거나 `"auto"`이고, `execApprovals.approvers` 또는 계정의 숫자형 소유자 config(`allowFrom` 및 다이렉트 메시지 `defaultTo`)에서 적어도 하나의 승인자를 확인할 수 있으면 Telegram은 기본 exec 승인을 자동 활성화합니다. Telegram을 기본 승인 클라이언트로 명시적으로 비활성화하려면 `enabled: false`를 설정하세요. 그렇지 않으면 승인 요청은 다른 구성된 승인 경로 또는 exec 승인 대체 정책으로 돌아갑니다.

    Telegram은 다른 채팅 채널에서 사용하는 공유 승인 버튼도 렌더링합니다. 기본 Telegram 어댑터는 주로 승인자 DM 라우팅, 채널/토픽 fanout, 그리고 전송 전 타이핑 힌트를 추가합니다.
    이러한 버튼이 존재할 때는 이것이 기본 승인 UX이며, OpenClaw는 도구 결과에서 채팅 승인을 사용할 수 없거나 수동 승인이 유일한 경로라고 명시할 때만 수동 `/approve` 명령을 포함해야 합니다.
    ...
    - `plugin:` 접두사가 있는 ID는 항상 plugin 승인으로 확인됩니다.
    - 그 외 승인 ID는 먼저 `exec.approval.resolve`를 시도합니다.
    - Telegram도 plugin 승인에 대해 권한이 있고 Gateway가 exec 승인이 알 수 없거나 만료되었다고 응답하면, Telegram은 `plugin.approval.resolve`를 통해 한 번 재시도합니다.
    - 실제 exec 승인 거부/오류는 조용히 plugin 승인 확인으로 넘어가지 않습니다.

    채널 전송은 채팅에 명령 텍스트를 표시하므로, 신뢰할 수 있는 그룹/토픽에서만 `channel` 또는 `both`를 활성화하세요. 프롬프트가 포럼 토픽에 도착하면 OpenClaw는 승인 프롬프트와 승인 후 후속 작업 모두에서 해당 토픽을 유지합니다. exec 승인은 기본적으로 30분 후 만료됩니다.

    인라인 승인 버튼도 `channels.telegram.capabilities.inlineButtons`가 대상 표면(`dm`, `group`, 또는 `all`)을 허용해야 합니다.
...
에이전트가 전송 또는 provider 오류를 만났을 때, Telegram은 오류 텍스트로 응답하거나 이를 억제할 수 있습니다. 이 동작은 두 개의 config 키로 제어됩니다.

| Key                                 | Values            | Default | 설명                                                                                  |
| ----------------------------------- | ----------------- | ------- | ------------------------------------------------------------------------------------- |
| `channels.telegram.errorPolicy`     | `reply`, `silent` | `reply` | `reply`는 채팅에 친화적인 오류 메시지를 전송합니다. `silent`는 오류 응답을 완전히 억제합니다. |
| `channels.telegram.errorCooldownMs` | number (ms)       | `60000` | 동일한 채팅에 대한 오류 응답 간 최소 시간입니다. 장애 중 오류 스팸을 방지합니다.              |
...
<AccordionGroup>
  <Accordion title="봇이 멘션 없는 그룹 메시지에 응답하지 않음">
    ...
    - `openclaw channels status`는 config가 멘션 없는 그룹 메시지를 예상할 때 경고합니다.
    - `openclaw channels status --probe`는 명시적인 숫자형 그룹 ID를 확인할 수 있습니다. 와일드카드 `"*"`는 멤버십 프로브를 할 수 없습니다.
  ...
  <Accordion title="봇이 그룹 메시지를 전혀 보지 못함">
    ...
    - 그룹 내 봇 멤버십 확인
    - 로그 검토: 건너뛴 이유는 `openclaw logs --follow`

  </Accordion>

  <Accordion title="명령이 부분적으로만 동작하거나 전혀 동작하지 않음">

    - 발신자 identity를 인증하세요(페어링 및/또는 숫자형 `allowFrom`)
    - 그룹 정책이 `open`이더라도 명령 인증은 여전히 적용됩니다
    - `setMyCommands failed`와 함께 `BOT_COMMANDS_TOO_MUCH`가 발생하면 기본 메뉴 항목이 너무 많다는 뜻입니다. plugin/skill/사용자 지정 명령 수를 줄이거나 기본 메뉴를 비활성화하세요
    - `setMyCommands failed`와 함께 네트워크/fetch 오류가 발생하면 일반적으로 `api.telegram.org`에 대한 DNS/HTTPS 도달성 문제를 의미합니다

  </Accordion>

  <Accordion title="폴링 또는 네트워크 불안정성">

    - Node 22+ + 사용자 지정 fetch/proxy는 AbortSignal 타입 불일치 시 즉시 중단 동작을 유발할 수 있습니다.
    - 일부 호스트는 `api.telegram.org`를 먼저 IPv6로 확인하며, IPv6 아웃바운드가 깨져 있으면 간헐적인 Telegram API 실패를 유발할 수 있습니다.
    - 로그에 `TypeError: fetch failed` 또는 `Network request for 'getUpdates' failed!`가 포함되면, OpenClaw는 이제 이를 복구 가능한 네트워크 오류로 재시도합니다.
    - 로그에 `Polling stall detected`가 포함되면, OpenClaw는 기본적으로 완료된 롱 폴링 활성 상태가 120초 동안 없을 경우 폴링을 재시작하고 Telegram 전송 계층을 다시 빌드합니다.
    - 긴 `getUpdates` 호출이 정상인데도 호스트에서 거짓 polling-stall 재시작이 계속 보고될 때만 `channels.telegram.pollingStallThresholdMs`를 늘리세요. 지속적인 stall은 일반적으로 호스트와 `api.telegram.org` 사이의 프록시, DNS, IPv6 또는 TLS 아웃바운드 문제를 가리킵니다.
...
```
    ...
    - RFC 2544 벤치마크 범위 응답(`198.18.0.0/15`)은 기본적으로 Telegram 미디어 다운로드에 이미 허용되어 있습니다. 신뢰할 수 있는 가짜 IP 또는 투명 프록시가 미디어 다운로드 중 `api.telegram.org`를 다른 private/internal/special-use 주소로 다시 쓴다면, Telegram 전용 우회 기능을 선택적으로 활성화할 수 있습니다.
...
```
    ...
    - 동일한 선택 기능은 계정별로 `channels.telegram.accounts.<accountId>.network.dangerouslyAllowPrivateNetwork`에서도 사용할 수 있습니다.
    - 프록시가 Telegram 미디어 호스트를 `198.18.x.x`로 확인한다면, 먼저 dangerous 플래그를 끈 상태로 두세요. Telegram 미디어는 기본적으로 RFC 2544 벤치마크 범위를 이미 허용합니다.

    <Warning>
      `channels.telegram.network.dangerouslyAllowPrivateNetwork`는 Telegram 미디어 SSRF 보호를 약화시킵니다. Clash, Mihomo, 또는 Surge fake-IP 라우팅처럼 RFC 2544 벤치마크 범위 밖의 private 또는 special-use 응답을 합성하는, 신뢰할 수 있는 운영자 제어 프록시 환경에서만 사용하세요. 일반적인 공용 인터넷 Telegram 액세스에서는 비활성화된 상태로 두세요.
    </Warning>
...
- `channels.telegram.allowFrom`: DM allowlist(숫자형 Telegram 사용자 ID). `allowlist`는 최소 하나의 발신자 ID가 필요합니다. `open`은 `"*"`가 필요합니다. `openclaw doctor --fix`는 레거시 `@username` 항목을 ID로 확인할 수 있으며, allowlist 마이그레이션 흐름에서 pairing-store 파일의 allowlist 항목을 복구할 수 있습니다.
- `channels.telegram.actions.poll`: Telegram 폴 생성 활성화 또는 비활성화(기본값: 활성화, 여전히 `sendMessage` 필요).
- `channels.telegram.defaultTo`: 명시적인 `--reply-to`가 제공되지 않았을 때 CLI `--deliver`에서 사용하는 기본 Telegram 대상.
- `channels.telegram.groupPolicy`: `open | allowlist | disabled` (기본값: allowlist).
- `channels.telegram.groupAllowFrom`: 그룹 발신자 allowlist(숫자형 Telegram 사용자 ID). `openclaw doctor --fix`는 레거시 `@username` 항목을 ID로 확인할 수 있습니다. 숫자가 아닌 항목은 인증 시 무시됩니다. 그룹 인증은 DM pairing-store 대체값을 사용하지 않습니다(`2026.2.25+`).
- 다중 계정 우선순위:
  - 두 개 이상의 계정 ID가 구성된 경우, 기본 라우팅을 명시적으로 하기 위해 `channels.telegram.defaultAccount`를 설정하세요(또는 `channels.telegram.accounts.default` 포함).
  - 둘 다 설정되지 않으면, OpenClaw는 첫 번째 정규화된 계정 ID로 대체하며 `openclaw doctor`가 경고합니다.
...
- `channels.telegram.groups`: 그룹별 기본값 + allowlist(전역 기본값에는 `"*"` 사용).
  ...
  - `channels.telegram.groups.<id>.requireMention`: 기본 멘션 게이트.
  - `channels.telegram.groups.<id>.skills`: Skills 필터(생략 = 모든 Skills, 비어 있음 = 없음).
...
- `channels.telegram.groups.<id>.topics.<threadId>.requireMention`: 토픽별 멘션 게이트 재정의.
- `match.peer.id`에 `type: "acp"` 및 정식 토픽 ID `chatId:topic:topicId`를 사용하는 최상위 `bindings[]`: 지속적 ACP 토픽 바인딩 필드([ACP Agents](/ko/tools/acp-agents#channel-specific-settings) 참조).
- `channels.telegram.direct.<id>.topics.<threadId>.agentId`: DM 토픽을 특정 에이전트로 라우팅합니다(포럼 토픽과 동일한 동작).
- `channels.telegram.execApprovals.enabled`: 이 계정에 대해 Telegram을 채팅 기반 exec 승인 클라이언트로 활성화합니다.
...
- `channels.telegram.execApprovals.target`: `dm | channel | both` (기본값: `dm`). `channel` 및 `both`는 존재할 경우 원래 Telegram 토픽을 유지합니다.
- `channels.telegram.execApprovals.agentFilter`: 전달된 승인 프롬프트용 선택적 에이전트 ID 필터.
- `channels.telegram.execApprovals.sessionFilter`: 전달된 승인 프롬프트용 선택적 세션 키 필터(부분 문자열 또는 regex).
...
- `channels.telegram.chunkMode`: `length`(기본값) 또는 길이 청크 분할 전에 빈 줄(문단 경계)에서 분할하는 `newline`.
- `channels.telegram.linkPreview`: 아웃바운드 메시지의 링크 미리보기 토글(기본값: true).
- `channels.telegram.streaming`: `off | partial | block | progress` (실시간 스트림 미리보기, 기본값: `partial`, `progress`는 `partial`로 매핑, `block`은 레거시 미리보기 모드 호환성). Telegram 미리보기 스트리밍은 한 개의 미리보기 메시지를 제자리에서 편집하는 방식으로 사용합니다.
- `channels.telegram.streaming.preview.toolProgress`: 미리보기 스트리밍이 활성화되어 있을 때 도구/진행 상황 업데이트에 실시간 미리보기 메시지를 재사용합니다(기본값: `true`). 도구/진행 메시지를 별도로 유지하려면 `false`로 설정하세요.
- `channels.telegram.mediaMaxMb`: 인바운드/아웃바운드 Telegram 미디어 상한(MB, 기본값: 100).
- `channels.telegram.retry`: 복구 가능한 아웃바운드 API 오류 발생 시 Telegram 전송 헬퍼(CLI/tools/actions)의 재시도 정책(attempts, minDelayMs, maxDelayMs, jitter).
- `channels.telegram.network.autoSelectFamily`: Node autoSelectFamily 재정의(true=활성화, false=비활성화). Node 22+에서는 기본적으로 활성화되며, WSL2에서는 기본적으로 비활성화됩니다.
- `channels.telegram.network.dnsResultOrder`: DNS 결과 순서 재정의(`ipv4first` 또는 `verbatim`). Node 22+에서는 기본적으로 `ipv4first`입니다.
- `channels.telegram.network.dangerouslyAllowPrivateNetwork`: Telegram 미디어 다운로드가 기본 RFC 2544 벤치마크 범위 허용 외부의 private/internal/special-use 주소로 `api.telegram.org`를 확인하는, 신뢰할 수 있는 fake-IP 또는 투명 프록시 환경을 위한 위험한 opt-in.
...
- `channels.telegram.reactionNotifications`: `off | own | all` — 어떤 반응이 시스템 이벤트를 트리거할지 제어합니다(설정되지 않은 경우 기본값: `own`).
- `channels.telegram.reactionLevel`: `off | ack | minimal | extensive` — 에이전트의 반응 기능을 제어합니다(설정되지 않은 경우 기본값: `minimal`).
- `channels.telegram.errorPolicy`: `reply | silent` — 오류 응답 동작을 제어합니다(기본값: `reply`). 계정별/그룹별/토픽별 재정의 지원.
...
- 서식 지정/전송: `textChunkLimit`, `chunkMode`, `linkPreview`, `responsePrefix`
