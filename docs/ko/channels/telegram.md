...
title: Telegram
x-i18n:
    generated_at: "2026-04-24T06:05:14Z"
    model: gpt-5.4
    provider: openai
...
봇 DM 및 그룹에 대해 grammY를 통해 프로덕션 준비가 완료되었습니다. 기본 모드는 롱 폴링이며, Webhook 모드는 선택 사항입니다.
...
<Steps>
  <Step title="BotFather에서 봇 토큰 생성">
    ...
    `/newbot`을 실행하고, 프롬프트를 따른 뒤, 토큰을 저장하세요.
...
<Note>
토큰 해석 순서는 계정을 인식합니다. 실제로는 구성 값이 환경 변수 폴백보다 우선하며, `TELEGRAM_BOT_TOKEN`은 기본 계정에만 적용됩니다.
</Note>
...
<Tabs>
  <Tab title="DM 정책">
    ...
    설정에서는 숫자형 사용자 ID만 요청합니다.
    업그레이드 후 구성에 `@username` 허용 목록 항목이 있다면, `openclaw doctor --fix`를 실행해 이를 해결하세요(최선의 노력 기준이며 Telegram 봇 토큰이 필요함).
    이전에 페어링 저장소 허용 목록 파일에 의존했다면, `openclaw doctor --fix`가 허용 목록 흐름에서 항목을 `channels.telegram.allowFrom`으로 복구할 수 있습니다(예: `dmPolicy: "allowlist"`에 아직 명시적 ID가 없을 때).

    단일 소유자 봇의 경우, 이전 페어링 승인에 의존하는 대신 접근 정책이 구성에 지속되도록, 명시적인 숫자형 `allowFrom` ID와 함께 `dmPolicy: "allowlist"`를 사용하는 것을 권장합니다.

    흔한 혼동: DM 페어링 승인이라고 해서 "이 발신자가 모든 곳에서 권한이 있다"는 뜻은 아닙니다.
    페어링은 DM 접근만 부여합니다. 그룹 발신자 권한은 여전히 명시적 구성 허용 목록에서만 옵니다.
    "한 번 권한을 받으면 DM과 그룹 명령이 모두 동작하게" 하려면 숫자형 Telegram 사용자 ID를 `channels.telegram.allowFrom`에 넣으세요.
...
```
  ...
  <Tab title="그룹 정책 및 허용 목록">
    ...
    1. **어떤 그룹이 허용되는지** (`channels.telegram.groups`)
       ...
       - `groups`가 구성된 경우: 허용 목록으로 동작(명시적 ID 또는 `"*"`)
    ...
    `groupAllowFrom`이 설정되지 않으면 Telegram은 페어링 저장소가 아니라 구성의 `allowFrom`으로 폴백합니다.
    단일 소유자 봇을 위한 실용적 패턴: 사용자 ID를 `channels.telegram.allowFrom`에 설정하고, `groupAllowFrom`은 설정하지 않은 채, 대상 그룹을 `channels.telegram.groups`에서 허용하세요.
    런타임 참고: `channels.telegram`이 완전히 누락되면, `channels.defaults.groupPolicy`가 명시적으로 설정되지 않은 한 런타임 기본값은 실패 시 닫힘(fail-closed) `groupPolicy="allowlist"`입니다.
...
```
  ...
  <Tab title="멘션 동작">
    ...
    이는 세션 상태만 업데이트합니다. 지속성을 원하면 구성을 사용하세요.
...
- 인바운드 메시지는 응답 메타데이터와 미디어 플레이스홀더가 포함된 공유 채널 엔벌로프로 정규화됩니다.
- 그룹 세션은 그룹 ID별로 격리됩니다. 포럼 토픽은 토픽 격리를 위해 `:topic:<threadId>`를 추가합니다.
- DM 메시지는 `message_thread_id`를 포함할 수 있습니다. OpenClaw는 이를 스레드 인식 세션 키로 라우팅하고 응답을 위해 스레드 ID를 보존합니다.
- 롱 폴링은 채팅별/스레드별 시퀀싱과 함께 grammY runner를 사용합니다. 전체 runner sink 동시성은 `agents.defaults.maxConcurrent`를 사용합니다.
- 롱 폴링 워치독 재시작은 기본적으로 완료된 `getUpdates` 활성 상태가 120초 동안 없을 때 트리거됩니다. 장시간 실행 작업 중에도 배포 환경에서 거짓 폴링 정지 재시작이 계속 보인다면 `channels.telegram.pollingStallThresholdMs`를 늘리세요. 값 단위는 밀리초이며 `30000`에서 `600000`까지 허용됩니다. 계정별 재정의도 지원됩니다.
...
<AccordionGroup>
  <Accordion title="실시간 스트림 미리보기(메시지 수정)">
    ...
    - `progress`는 Telegram에서 `partial`로 매핑됩니다(교차 채널 명명 호환성)
    - `streaming.preview.toolProgress`는 도구/진행률 업데이트가 동일한 수정된 미리보기 메시지를 재사용할지 제어합니다(기본값: `true`). 별도의 도구/진행률 메시지를 유지하려면 `false`로 설정하세요.
    - 레거시 `channels.telegram.streamMode` 및 불리언 `streaming` 값은 자동 매핑됩니다

    텍스트 전용 응답의 경우:

    - DM: OpenClaw는 동일한 미리보기 메시지를 유지하고 최종 수정도 제자리에서 수행합니다(두 번째 메시지 없음)
    - 그룹/토픽: OpenClaw는 동일한 미리보기 메시지를 유지하고 최종 수정도 제자리에서 수행합니다(두 번째 메시지 없음)

    복잡한 응답(예: 미디어 페이로드)의 경우, OpenClaw는 일반 최종 전달로 폴백한 뒤 미리보기 메시지를 정리합니다.
    ...
    - `/reasoning stream`은 생성 중 추론을 실시간 미리보기에 보냄
  ...
  <Accordion title="서식 및 HTML 폴백">
    ...
    - Telegram 파싱 실패를 줄이기 위해 원시 모델 HTML은 이스케이프 처리됩니다.
...
```
    ...
    네이티브 명령이 비활성화되면 내장 명령은 제거됩니다. 사용자 지정/Plugin 명령은 구성되어 있으면 여전히 등록될 수 있습니다.

    흔한 설정 실패:

    - `setMyCommands failed`와 함께 `BOT_COMMANDS_TOO_MUCH`가 발생하면, 트리밍 후에도 Telegram 메뉴가 여전히 넘쳤다는 뜻입니다. Plugin/Skills/사용자 지정 명령을 줄이거나 `channels.telegram.commands.native`를 비활성화하세요.
    - `setMyCommands failed`와 함께 네트워크/fetch 오류가 발생하면, 일반적으로 `api.telegram.org`로의 아웃바운드 DNS/HTTPS가 차단되어 있다는 뜻입니다.
    ...
    4. 요청을 승인합니다:
       ...
       - 대기 중인 요청이 하나뿐일 때 `/pair approve`
       - 가장 최근 요청에 대한 `/pair approve latest`

    설정 코드는 수명이 짧은 bootstrap 토큰을 담고 있습니다. 내장 bootstrap handoff는 기본 Node 토큰을 `scopes: []`로 유지합니다. 전달된 operator 토큰은 `operator.approvals`, `operator.read`, `operator.talk.secrets`, `operator.write`로 제한됩니다. Bootstrap 범위 검사는 역할 접두사를 사용하므로, 해당 operator 허용 목록은 operator 요청만 충족하며, non-operator 역할은 여전히 자체 역할 접두사 아래 범위가 필요합니다.

    기기가 변경된 인증 세부 정보(예: 역할/범위/공개 키)로 재시도하면 이전 대기 요청은 대체되며 새 요청은 다른 `requestId`를 사용합니다. 승인 전에 `/pair pending`을 다시 실행하세요.
...
```
  ...
  <Accordion title="에이전트 및 자동화를 위한 Telegram 메시지 작업">
    ...
    런타임 전송은 활성 구성/시크릿 스냅샷(시작/리로드 시점)을 사용하므로, 작업 경로는 전송마다 임시로 SecretRef를 다시 해석하지 않습니다.
  ...
  <Accordion title="포럼 토픽 및 스레드 동작">
    ...
    **토픽별 에이전트 라우팅**: 각 토픽은 토픽 구성에서 `agentId`를 설정하여 서로 다른 에이전트로 라우팅할 수 있습니다. 이렇게 하면 각 토픽이 자체적으로 격리된 워크스페이스, 메모리, 세션을 갖게 됩니다. 예시:
    ...
    **영구 ACP 토픽 바인딩**: 포럼 토픽은 최상위 타입 지정 ACP 바인딩(`type: "acp"`와 `match.channel: "telegram"`, `peer.kind: "group"`, 그리고 `-1001234567890:topic:42` 같은 토픽 한정 ID를 포함하는 `bindings[]`)을 통해 ACP harness 세션을 고정할 수 있습니다. 현재는 그룹/슈퍼그룹의 포럼 토픽으로 범위가 제한됩니다. [ACP 에이전트](/ko/tools/acp-agents)를 참고하세요.

    **채팅에서의 스레드 바운드 ACP 생성**: `/acp spawn <agent> --thread here|auto`는 현재 토픽을 새 ACP 세션에 바인딩합니다. 이후 후속 메시지는 직접 그곳으로 라우팅됩니다. OpenClaw는 생성 확인 메시지를 토픽 내에 고정합니다. `channels.telegram.threadBindings.spawnAcpSessions=true`가 필요합니다.
  ...
  <Accordion title="오디오, 비디오 및 스티커">
    ...
    - 기본값: 오디오 파일 동작
    - 에이전트 응답에 `[[audio_as_voice]]` 태그를 넣어 음성 노트 전송을 강제
...
```
  ...
  <Accordion title="반응 알림">
    ...
    - 반응 이벤트는 여전히 Telegram 접근 제어(`dmPolicy`, `allowFrom`, `groupPolicy`, `groupAllowFrom`)를 따르며, 권한 없는 발신자는 버려집니다.
  ...
  <Accordion title="Telegram 이벤트 및 명령에서의 구성 쓰기">
...
```
  ...
  <Accordion title="롱 폴링 vs Webhook">
    기본값은 롱 폴링입니다. Webhook 모드를 사용하려면 `channels.telegram.webhookUrl` 및 `channels.telegram.webhookSecret`를 설정하세요. 선택적으로 `webhookPath`, `webhookHost`, `webhookPort`를 설정할 수 있습니다(기본값 `/telegram-webhook`, `127.0.0.1`, `8787`).

    로컬 리스너는 `127.0.0.1:8787`에 바인딩됩니다. 공개 인그레스를 위해서는 로컬 포트 앞에 리버스 프록시를 두거나, 의도적으로 `webhookHost: "0.0.0.0"`를 설정하세요.
  ...
  <Accordion title="제한, 재시도 및 CLI 대상">
    ...
    - `channels.telegram.pollingStallThresholdMs` 기본값은 `120000`입니다. 거짓 양성 폴링 정지 재시작에 대해서만 `30000`~`600000` 범위에서 조정하세요.
    ...
    - Telegram 허용 목록은 주로 누가 에이전트를 트리거할 수 있는지를 제한하며, 완전한 보조 컨텍스트 비식별화 경계는 아닙니다.
    - DM 기록 제어:
      - `channels.telegram.dmHistoryLimit`
      - `channels.telegram.dms["<user_id>"].historyLimit`
    - `channels.telegram.retry` 구성은 복구 가능한 아웃바운드 API 오류에 대해 Telegram 전송 헬퍼(CLI/도구/작업)에 적용됩니다.
...
```
    ...
    - `channels.telegram.actions.poll=false`는 일반 전송은 유지한 채 Telegram 폴 생성만 비활성화합니다

  </Accordion>

  <Accordion title="Telegram의 exec 승인">
    Telegram은 승인자 DM에서 exec 승인을 지원하며, 선택적으로 원래 채팅이나 토픽에 프롬프트를 게시할 수도 있습니다. 승인자는 숫자형 Telegram 사용자 ID여야 합니다.
    ...
    채널 전달은 명령 텍스트를 채팅에 표시하므로, 신뢰할 수 있는 그룹/토픽에서만 `channel` 또는 `both`를 활성화하세요. 프롬프트가 포럼 토픽에 도착하면, OpenClaw는 승인 프롬프트와 후속 처리 모두에 대해 해당 토픽을 유지합니다. exec 승인은 기본적으로 30분 후 만료됩니다.

    인라인 승인 버튼도 대상 표면(`dm`, `group`, 또는 `all`)을 허용하도록 `channels.telegram.capabilities.inlineButtons`가 설정되어 있어야 합니다. `plugin:` 접두사가 붙은 승인 ID는 Plugin 승인을 통해 해석되며, 그 외는 먼저 exec 승인을 통해 해석됩니다.
...
에이전트가 전달 또는 Provider 오류를 만났을 때, Telegram은 오류 텍스트로 응답하거나 이를 억제할 수 있습니다. 이 동작은 두 개의 구성 키로 제어됩니다:

| 키 | 값 | 기본값 | 설명 |
| ----------------------------------- | ----------------- | ------- | ----------------------------------------------------------------------------------------------- |
| `channels.telegram.errorPolicy`     | `reply`, `silent` | `reply` | `reply`는 친화적인 오류 메시지를 채팅에 보냅니다. `silent`는 오류 응답을 완전히 억제합니다. |
| `channels.telegram.errorCooldownMs` | number (ms)       | `60000` | 동일한 채팅에 오류 응답을 보내는 최소 간격입니다. 장애 중 오류 스팸을 방지합니다. |

계정별, 그룹별, 토픽별 재정의가 지원됩니다(다른 Telegram 구성 키와 동일한 상속 방식).
...
<AccordionGroup>
  <Accordion title="멘션이 없는 그룹 메시지에 봇이 응답하지 않음">
  ...
  <Accordion title="명령이 부분적으로만 동작하거나 전혀 동작하지 않음">
    ...
    - `setMyCommands failed`와 함께 네트워크/fetch 오류가 발생하면 보통 `api.telegram.org`에 대한 DNS/HTTPS 연결 문제를 의미합니다

  </Accordion>

  <Accordion title="폴링 또는 네트워크 불안정">

    - Node 22+와 사용자 지정 fetch/proxy 조합은 AbortSignal 타입이 맞지 않으면 즉시 중단 동작을 유발할 수 있습니다.
    - 일부 호스트는 `api.telegram.org`를 먼저 IPv6로 해석합니다. 깨진 IPv6 아웃바운드는 간헐적인 Telegram API 실패를 유발할 수 있습니다.
    - 로그에 `TypeError: fetch failed` 또는 `Network request for 'getUpdates' failed!`가 포함되면, OpenClaw는 이제 이를 복구 가능한 네트워크 오류로 재시도합니다.
    - 로그에 `Polling stall detected`가 포함되면, OpenClaw는 기본적으로 완료된 롱 폴링 활성 상태가 120초 동안 없을 때 폴링을 재시작하고 Telegram 전송을 다시 빌드합니다.
    - 장시간 실행 `getUpdates` 호출이 정상인데도 호스트에서 거짓 폴링 정지 재시작이 계속 보고될 때만 `channels.telegram.pollingStallThresholdMs`를 늘리세요. 지속적인 정지는 보통 호스트와 `api.telegram.org` 사이의 프록시, DNS, IPv6 또는 TLS 아웃바운드 문제를 가리킵니다.
...
```
    ...
    - RFC 2544 벤치마크 범위 응답(`198.18.0.0/15`)은 이미 기본적으로 Telegram 미디어 다운로드에 허용됩니다. 신뢰할 수 있는 가짜 IP 또는 투명 프록시가 미디어 다운로드 중 `api.telegram.org`를 다른 private/internal/special-use 주소로 재작성한다면, Telegram 전용 우회를 선택적으로 활성화할 수 있습니다:
...
```
    ...
    - 동일한 선택 설정은 계정별로 `channels.telegram.accounts.<accountId>.network.dangerouslyAllowPrivateNetwork`에서도 사용할 수 있습니다.
    - 프록시가 Telegram 미디어 호스트를 `198.18.x.x`로 해석하는 경우, 먼저 위험한 플래그를 끈 상태로 두세요. Telegram 미디어는 이미 기본적으로 RFC 2544 벤치마크 범위를 허용합니다.

    <Warning>
      `channels.telegram.network.dangerouslyAllowPrivateNetwork`는 Telegram
      미디어 SSRF 보호를 약화시킵니다. Clash, Mihomo, Surge의 fake-IP 라우팅처럼 RFC 2544 벤치마크 범위를 벗어나는 private 또는 special-use 응답을 합성하는, 신뢰할 수 있고 운영자가 제어하는 프록시 환경에서만 사용하세요. 일반적인 공용 인터넷 Telegram 접근에서는 비활성화 상태로 두세요.
    </Warning>
...
<Accordion title="신호성이 높은 Telegram 필드">
...
<Note>
다중 계정 우선순위: 두 개 이상의 계정 ID가 구성된 경우 기본 라우팅을 명시적으로 만들기 위해 `channels.telegram.defaultAccount`를 설정하세요(또는 `channels.telegram.accounts.default` 포함). 그렇지 않으면 OpenClaw는 첫 번째 정규화된 계정 ID로 폴백하며 `openclaw doctor`가 경고합니다. 이름 있는 계정은 `channels.telegram.allowFrom` / `groupAllowFrom`을 상속하지만 `accounts.default.*` 값은 상속하지 않습니다.
</Note>
