...
title: Telegram
x-i18n:
    generated_at: "2026-04-21T06:00:50Z"
    model: gpt-5.4
    provider: openai
...
```
    ...
    환경 변수 대체값: `TELEGRAM_BOT_TOKEN=...` (기본 계정만).
...
<AccordionGroup>
  <Accordion title="프라이버시 모드와 그룹 가시성">
    Telegram 봇은 기본적으로 **프라이버시 모드**가 활성화되어 있으며, 이로 인해 수신할 수 있는 그룹 메시지가 제한됩니다.

    봇이 모든 그룹 메시지를 볼 수 있어야 한다면 다음 중 하나를 수행하세요.

    - `/setprivacy`로 프라이버시 모드 비활성화
    - 봇을 그룹 관리자로 지정

    프라이버시 모드를 전환한 경우, Telegram이 변경 사항을 적용하도록 각 그룹에서 봇을 제거한 뒤 다시 추가하세요.
...
<Tabs>
  <Tab title="DM 정책">
    ...
    - `pairing` (기본값)
    - `allowlist` (`allowFrom`에 최소 1개의 발신자 ID 필요)
    - `open` (`allowFrom`에 `"*"` 포함 필요)
    - `disabled`

    `channels.telegram.allowFrom`은 숫자형 Telegram 사용자 ID를 허용합니다. `telegram:` / `tg:` 접두사는 허용되며 정규화됩니다.
    `dmPolicy: "allowlist"`에서 `allowFrom`이 비어 있으면 모든 DM이 차단되며 config 검증에서 거부됩니다.
    설정 시에는 숫자형 사용자 ID만 요청합니다.
    업그레이드 후 config에 `@username` allowlist 항목이 포함되어 있다면 `openclaw doctor --fix`를 실행해 해결하세요(최선 노력 방식이며 Telegram 봇 토큰이 필요함).
    이전에 페어링 저장소 allowlist 파일에 의존했다면, `openclaw doctor --fix`는 allowlist 흐름에서 항목을 `channels.telegram.allowFrom`으로 복구할 수 있습니다(예: `dmPolicy: "allowlist"`에 아직 명시적 ID가 없는 경우).

    단일 소유자 봇의 경우, 이전 페어링 승인에 의존하는 대신 액세스 정책을 config에 지속적으로 유지하기 위해 명시적인 숫자형 `allowFrom` ID와 함께 `dmPolicy: "allowlist"`를 사용하는 것을 권장합니다.
...
```
    ...
    서드파티 방법(프라이버시가 더 낮음): `@userinfobot` 또는 `@getidsbot`.
  ...
  <Tab title="그룹 정책 및 allowlist">
    ...
    1. **허용되는 그룹** (`channels.telegram.groups`)
       - `groups` config 없음:
         - `groupPolicy: "open"`인 경우: 어떤 그룹이든 그룹 ID 확인을 통과할 수 있음
    ...
    `groupAllowFrom`에는 Telegram 그룹 또는 슈퍼그룹 채팅 ID를 넣지 마세요. 음수 채팅 ID는 `channels.telegram.groups` 아래에 있어야 합니다.
    ...
    단일 소유자 봇을 위한 실용적인 패턴: 사용자 ID를 `channels.telegram.allowFrom`에 설정하고, `groupAllowFrom`은 비워 두고, 대상 그룹은 `channels.telegram.groups`에서 허용하세요.
    런타임 참고: `channels.telegram`이 완전히 누락된 경우, `channels.defaults.groupPolicy`가 명시적으로 설정되지 않았다면 런타임 기본값은 실패 시 닫힘 방식의 `groupPolicy="allowlist"`입니다.
...
- 라우팅은 결정적입니다. Telegram 인바운드 응답은 Telegram으로 다시 회신됩니다(모델이 채널을 선택하지 않음).
- 인바운드 메시지는 답장 메타데이터와 미디어 플레이스홀더를 포함한 공유 채널 envelope으로 정규화됩니다.
- 그룹 세션은 그룹 ID별로 격리됩니다. 포럼 토픽은 토픽 격리를 위해 `:topic:<threadId>`를 덧붙입니다.
- DM 메시지는 `message_thread_id`를 포함할 수 있으며, OpenClaw는 이를 스레드 인식 세션 키로 라우팅하고 답장을 위해 thread ID를 보존합니다.
- 롱 폴링은 채팅별/스레드별 시퀀싱과 함께 grammY runner를 사용합니다. 전체 runner sink 동시성은 `agents.defaults.maxConcurrent`를 사용합니다.
- 롱 폴링 감시 재시작은 기본적으로 완료된 `getUpdates` 라이브니스가 120초 동안 없으면 트리거됩니다. 긴 작업 중에도 배포 환경에서 잘못된 폴링 정지 재시작이 계속 발생하는 경우에만 `channels.telegram.pollingStallThresholdMs`를 늘리세요. 값 단위는 밀리초이며 `30000`에서 `600000`까지 허용됩니다. 계정별 재정의도 지원됩니다.
...
<AccordionGroup>
  <Accordion title="라이브 스트림 미리보기 (메시지 편집)">
    ...
    복합 응답(예: 미디어 페이로드)의 경우 OpenClaw는 일반 최종 전달로 대체한 다음 미리보기 메시지를 정리합니다.

    미리보기 스트리밍은 블록 스트리밍과 별개입니다. Telegram에서 블록 스트리밍이 명시적으로 활성화된 경우, OpenClaw는 이중 스트리밍을 피하기 위해 미리보기 스트림을 건너뜁니다.

    기본 초안 전송을 사용할 수 없거나 거부되면, OpenClaw는 자동으로 `sendMessage` + `editMessageText`로 대체합니다.

    Telegram 전용 추론 스트림:

    - `/reasoning stream`은 생성 중 추론을 라이브 미리보기로 전송합니다
...
```
    ...
    - plugin/skill 명령은 Telegram 메뉴에 표시되지 않더라도 직접 입력하면 여전히 동작할 수 있습니다

    기본 명령이 비활성화되면 내장 명령은 제거됩니다. 사용자 지정/plugin 명령은 구성된 경우 여전히 등록될 수 있습니다.

    일반적인 설정 실패:

    - `BOT_COMMANDS_TOO_MUCH`와 함께 `setMyCommands failed`가 발생하면, 잘라낸 뒤에도 Telegram 메뉴가 여전히 넘친다는 뜻입니다. plugin/skill/custom 명령을 줄이거나 `channels.telegram.commands.native`를 비활성화하세요.
    - 네트워크/fetch 오류와 함께 `setMyCommands failed`가 발생하면, 일반적으로 `api.telegram.org`에 대한 아웃바운드 DNS/HTTPS가 차단된 상태입니다.
    ...
    4. 요청을 승인합니다:
       - 명시적으로 승인하려면 `/pair approve <requestId>`
       - 대기 중인 요청이 하나뿐이면 `/pair approve`
       - 가장 최근 요청이면 `/pair approve latest`

    설정 코드는 수명이 짧은 부트스트랩 토큰을 포함합니다. 내장 부트스트랩 핸드오프는 기본 node 토큰을 `scopes: []`로 유지합니다. 핸드오프된 operator 토큰은 `operator.approvals`, `operator.read`, `operator.talk.secrets`, `operator.write` 범위로만 제한됩니다. 부트스트랩 범위 검사는 역할 접두사 기반이므로, 그 operator allowlist는 operator 요청만 충족하며, operator가 아닌 역할은 여전히 각자의 역할 접두사 아래 범위가 필요합니다.

    디바이스가 변경된 인증 세부정보(예: 역할/범위/공개 키)로 재시도하면 이전 대기 요청은 대체되며 새 요청은 다른 `requestId`를 사용합니다. 승인 전에 `/pair pending`을 다시 실행하세요.
...
```
  ...
  <Accordion title="에이전트 및 자동화를 위한 Telegram 메시지 액션">
    ...
    채널 메시지 액션은 사용하기 쉬운 별칭을 제공합니다 (`send`, `react`, `delete`, `edit`, `sticker`, `sticker-search`, `topic-create`).
    ...
    런타임 전송은 활성 config/secrets 스냅샷(시작/리로드 시점)을 사용하므로, 액션 경로는 전송마다 임시 SecretRef 재해결을 수행하지 않습니다.
  ...
  <Accordion title="포럼 토픽 및 스레드 동작">
    ...
    - 타이핑 액션에는 여전히 `message_thread_id`가 포함됩니다
    ...
    **토픽별 에이전트 라우팅**: 각 토픽은 토픽 config에서 `agentId`를 설정하여 다른 에이전트로 라우팅할 수 있습니다. 이를 통해 각 토픽은 고유하게 격리된 작업공간, 메모리 및 세션을 가집니다. 예시:
    ...
    그러면 각 토픽은 고유한 세션 키를 가집니다: `agent:zu:telegram:group:-1001234567890:topic:3`

    **영구 ACP 토픽 바인딩**: 포럼 토픽은 최상위 typed ACP 바인딩을 통해 ACP harness 세션을 고정할 수 있습니다:
    ...
    **채팅에서 시작하는 스레드 바운드 ACP 생성**:
    ...
    - OpenClaw는 바인딩에 성공하면 생성 확인 메시지를 토픽 내에 고정합니다.
    ...
    - `message_thread_id`가 있는 비공개 채팅은 DM 라우팅을 유지하지만 스레드 인식 세션 키/답장 대상으로 처리됩니다.
...
```
    ...
    반복적인 비전 호출을 줄이기 위해 스티커는 가능한 경우 한 번만 설명되고 캐시됩니다.
...
```
  ...
  <Accordion title="반응 알림">
    Telegram 반응은 `message_reaction` 업데이트로 들어옵니다(메시지 페이로드와 별도).
    ...
    - `own`은 봇이 보낸 메시지에 대한 사용자 반응만 의미합니다(전송 메시지 캐시를 통한 best-effort).
    - 반응 이벤트도 여전히 Telegram 액세스 제어(`dmPolicy`, `allowFrom`, `groupPolicy`, `groupAllowFrom`)를 따릅니다. 권한 없는 발신자는 폐기됩니다.
    - Telegram은 반응 업데이트에 스레드 ID를 제공하지 않습니다.
      - 비포럼 그룹은 그룹 채팅 세션으로 라우팅
      - 포럼 그룹은 정확한 원본 토픽이 아니라 그룹 일반 토픽 세션(`:topic:1`)으로 라우팅
...
```
  ...
  <Accordion title="제한, 재시도 및 CLI 대상">
    ...
    - `channels.telegram.pollingStallThresholdMs`의 기본값은 `120000`이며, 잘못된 폴링 정지 재시작이 발생할 때만 `30000`에서 `600000` 사이로 조정하세요.
    - 그룹 컨텍스트 기록은 `channels.telegram.historyLimit` 또는 `messages.groupChat.historyLimit`를 사용합니다(기본값 50). `0`이면 비활성화됩니다.
    - 답장/인용/전달의 보조 컨텍스트는 현재 수신된 그대로 전달됩니다.
    - Telegram allowlist는 주로 누가 에이전트를 트리거할 수 있는지를 제어하며, 완전한 보조 컨텍스트 마스킹 경계는 아닙니다.
    - DM 기록 제어:
...
```
    ...
    - `channels.telegram.actions.poll=false`는 일반 전송은 유지하면서 Telegram 투표 생성을 비활성화합니다
  ...
  <Accordion title="Telegram의 exec 승인">
    ...
    - `channels.telegram.execApprovals.approvers` (선택 사항. 가능할 경우 `allowFrom`과 직접 `defaultTo`에서 추론한 숫자형 소유자 ID로 대체)
    - `channels.telegram.execApprovals.target` (`dm` | `channel` | `both`, 기본값: `dm`)
    - `agentFilter`, `sessionFilter`

    승인자는 숫자형 Telegram 사용자 ID여야 합니다. Telegram은 `enabled`가 설정되지 않았거나 `"auto"`이고, `execApprovals.approvers` 또는 계정의 숫자형 소유자 config(`allowFrom` 및 direct-message `defaultTo`)에서 최소 한 명의 승인자를 확인할 수 있으면 기본 exec 승인을 자동 활성화합니다. Telegram을 기본 승인 클라이언트로 명시적으로 비활성화하려면 `enabled: false`를 설정하세요. 그렇지 않으면 승인 요청은 다른 구성된 승인 경로 또는 exec 승인 대체 정책으로 폴백됩니다.

    Telegram은 다른 채팅 채널에서 사용하는 공유 승인 버튼도 렌더링합니다. 기본 Telegram 어댑터는 주로 승인자 DM 라우팅, 채널/토픽 팬아웃, 전송 전 타이핑 힌트를 추가합니다.
    이러한 버튼이 있을 때는 그것이 기본 승인 UX이며, OpenClaw는 도구 결과에서 채팅 승인을 사용할 수 없거나 수동 승인이 유일한 경로라고 표시된 경우에만 수동 `/approve` 명령을 포함해야 합니다.

    전송 규칙:

    - `target: "dm"`은 확인된 승인자 DM으로만 승인 프롬프트를 전송합니다
    ...
    - Telegram도 plugin 승인에 대해 승인되어 있고 Gateway가 exec 승인이 알 수 없거나 만료되었다고 응답하면, Telegram은 `plugin.approval.resolve`를 통해 한 번 더 재시도합니다.
    - 실제 exec 승인 거부/오류는 조용히 plugin 승인 확인으로 넘어가지 않습니다.

    채널 전송은 채팅에 명령 텍스트를 표시하므로, `channel` 또는 `both`는 신뢰된 그룹/토픽에서만 활성화하세요. 프롬프트가 포럼 토픽에 도착하면 OpenClaw는 승인 프롬프트와 승인 후 후속 메시지 모두에서 해당 토픽을 유지합니다. exec 승인은 기본적으로 30분 후 만료됩니다.

    인라인 승인 버튼도 대상 표면(`dm`, `group`, 또는 `all`)을 `channels.telegram.capabilities.inlineButtons`가 허용해야 합니다.
...
에이전트가 전송 또는 provider 오류를 만나면, Telegram은 오류 텍스트로 응답하거나 이를 억제할 수 있습니다. 이 동작은 두 개의 config 키로 제어됩니다:
...
| `channels.telegram.errorCooldownMs` | number (ms)       | `60000` | 같은 채팅에 오류 응답을 보내는 최소 간격입니다. 장애 중 오류 스팸을 방지합니다.              |

계정별, 그룹별, 토픽별 재정의가 지원됩니다(다른 Telegram config 키와 동일한 상속 규칙).
...
<AccordionGroup>
  ...
  <Accordion title="명령이 부분적으로만 동작하거나 전혀 동작하지 않음">
    ...
    - 발신자 identity를 승인하세요(페어링 및/또는 숫자형 `allowFrom`)
    - 그룹 정책이 `open`이더라도 명령 승인은 여전히 적용됩니다
    - `BOT_COMMANDS_TOO_MUCH`와 함께 `setMyCommands failed`가 발생하면 기본 메뉴 항목이 너무 많다는 뜻입니다. plugin/skill/custom 명령을 줄이거나 기본 메뉴를 비활성화하세요
  ...
  <Accordion title="폴링 또는 네트워크 불안정">
    ...
    - Node 22+ + 사용자 지정 fetch/proxy 조합에서는 AbortSignal 타입 불일치 시 즉시 중단 동작이 발생할 수 있습니다.
    - 일부 호스트는 `api.telegram.org`를 먼저 IPv6로 확인하므로, 손상된 IPv6 이그레스는 간헐적인 Telegram API 실패를 유발할 수 있습니다.
    - 로그에 `TypeError: fetch failed` 또는 `Network request for 'getUpdates' failed!`가 포함되면, OpenClaw는 이제 이를 복구 가능한 네트워크 오류로 재시도합니다.
    - 로그에 `Polling stall detected`가 포함되면, OpenClaw는 기본적으로 120초 동안 완료된 롱 폴 liveness가 없을 때 폴링을 재시작하고 Telegram 전송 계층을 다시 빌드합니다.
    - 장시간 실행되는 `getUpdates` 호출이 정상인데도 호스트에서 잘못된 폴링 정지 재시작이 계속 보고되는 경우에만 `channels.telegram.pollingStallThresholdMs`를 늘리세요. 지속적인 정지는 일반적으로 호스트와 `api.telegram.org` 사이의 프록시, DNS, IPv6 또는 TLS 이그레스 문제를 가리킵니다.
...
```
    ...
    - RFC 2544 벤치마크 범위 응답(`198.18.0.0/15`)은 기본적으로 Telegram 미디어 다운로드에 대해 이미 허용됩니다. 신뢰된 가짜 IP 또는 투명 프록시가 미디어 다운로드 중 `api.telegram.org`를 다른 사설/내부/특수 용도 주소로 재작성하는 경우, Telegram 전용 우회를 선택적으로 활성화할 수 있습니다:
...
```
    ...
    - 동일한 선택적 설정은 계정별로도 `channels.telegram.accounts.<accountId>.network.dangerouslyAllowPrivateNetwork`에서 사용할 수 있습니다.
    - 프록시가 Telegram 미디어 호스트를 `198.18.x.x`로 확인한다면, 먼저 위험 플래그를 끈 상태로 두세요. Telegram 미디어는 기본적으로 RFC 2544 벤치마크 범위를 이미 허용합니다.

    <Warning>
      `channels.telegram.network.dangerouslyAllowPrivateNetwork`는 Telegram 미디어 SSRF 보호를 약화시킵니다. Clash, Mihomo 또는 Surge fake-IP 라우팅처럼 운영자가 제어하는 신뢰된 프록시 환경에서, RFC 2544 벤치마크 범위를 벗어나는 사설 또는 특수 용도 응답을 합성하는 경우에만 사용하세요. 일반적인 공용 인터넷 Telegram 액세스에서는 비활성화 상태로 두세요.
    </Warning>
...
- `channels.telegram.allowFrom`: DM allowlist(숫자형 Telegram 사용자 ID). `allowlist`는 최소 1개의 발신자 ID가 필요합니다. `open`은 `"*"`가 필요합니다. `openclaw doctor --fix`는 레거시 `@username` 항목을 ID로 확인할 수 있으며 allowlist 마이그레이션 흐름에서 pairing-store 파일의 allowlist 항목을 복구할 수 있습니다.
...
- `channels.telegram.groupAllowFrom`: 그룹 발신자 allowlist(숫자형 Telegram 사용자 ID). `openclaw doctor --fix`는 레거시 `@username` 항목을 ID로 확인할 수 있습니다. 숫자가 아닌 항목은 인증 시 무시됩니다. 그룹 인증은 DM pairing-store 대체 경로를 사용하지 않습니다(`2026.2.25+`).
- 다중 계정 우선순위:
  - 둘 이상의 account ID가 구성된 경우, 기본 라우팅을 명시적으로 만들기 위해 `channels.telegram.defaultAccount`를 설정하거나(`channels.telegram.accounts.default` 포함) 하세요.
  - 둘 다 설정되지 않으면 OpenClaw는 정규화된 첫 번째 account ID로 대체되며 `openclaw doctor`가 경고를 표시합니다.
  - `channels.telegram.accounts.default.allowFrom` 및 `channels.telegram.accounts.default.groupAllowFrom`은 `default` 계정에만 적용됩니다.
  - 명명된 계정은 계정 수준 값이 설정되지 않은 경우 `channels.telegram.allowFrom` 및 `channels.telegram.groupAllowFrom`을 상속합니다.
...
- `bindings[]` 최상위 항목에서 `type: "acp"` 및 `match.peer.id`의 정규 토픽 ID `chatId:topic:topicId`: 영구 ACP 토픽 바인딩 필드([ACP Agents](/ko/tools/acp-agents#channel-specific-settings) 참조).
...
- `channels.telegram.execApprovals.approvers`: exec 요청을 승인 또는 거부할 수 있는 Telegram 사용자 ID. `channels.telegram.allowFrom` 또는 직접적인 `channels.telegram.defaultTo`가 이미 소유자를 식별하는 경우 선택 사항입니다.
...
- `channels.telegram.chunkMode`: `length`(기본값) 또는 `newline`. 길이 기준 청킹 전에 빈 줄(문단 경계)에서 분할합니다.
- `channels.telegram.linkPreview`: 아웃바운드 메시지의 링크 미리보기 토글(기본값: true).
- `channels.telegram.streaming`: `off | partial | block | progress` (라이브 스트림 미리보기, 기본값: `partial`, `progress`는 `partial`로 매핑, `block`은 레거시 미리보기 모드 호환성). Telegram 미리보기 스트리밍은 제자리 편집되는 단일 미리보기 메시지를 사용합니다.
- `channels.telegram.mediaMaxMb`: 인바운드/아웃바운드 Telegram 미디어 상한(MB, 기본값: 100).
- `channels.telegram.retry`: 복구 가능한 아웃바운드 API 오류 발생 시 Telegram 전송 헬퍼(CLI/tools/actions)에 대한 재시도 정책(`attempts`, `minDelayMs`, `maxDelayMs`, `jitter`).
- `channels.telegram.network.autoSelectFamily`: Node autoSelectFamily 재정의(true=활성화, false=비활성화). 기본적으로 Node 22+에서 활성화되며, WSL2에서는 기본적으로 비활성화됩니다.
- `channels.telegram.network.dnsResultOrder`: DNS 결과 순서 재정의(`ipv4first` 또는 `verbatim`). 기본적으로 Node 22+에서 `ipv4first`입니다.
- `channels.telegram.network.dangerouslyAllowPrivateNetwork`: Telegram 미디어 다운로드 시 `api.telegram.org`가 기본 RFC 2544 벤치마크 범위 허용을 벗어난 사설/내부/특수 용도 주소로 확인되는, 신뢰된 fake-IP 또는 투명 프록시 환경을 위한 위험한 선택적 활성화.
...
- `channels.telegram.errorCooldownMs`: 같은 채팅에 오류 응답을 보내는 최소 ms 간격(기본값: `60000`). 장애 중 오류 스팸을 방지합니다.
