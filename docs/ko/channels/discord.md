...
새 애플리케이션과 봇을 만들고, 봇을 서버에 추가한 뒤, OpenClaw에 페어링해야 합니다. 봇은 본인만 사용하는 비공개 서버에 추가하는 것을 권장합니다. 아직 없다면 먼저 [서버를 만드세요](https://support.discord.com/hc/en-us/articles/204849977-How-do-I-create-a-server) (**Create My Own > For me and my friends** 선택).
...
<Steps>
  ...
  <Step title="봇 토큰 복사">
    다시 **Bot** 페이지 상단으로 올라가 **Reset Token**을 클릭하세요.

    <Note>
    이름과 달리, 이것은 첫 번째 토큰을 생성하는 것이며 실제로 무엇인가를 "재설정"하는 것은 아닙니다.
    </Note>
  ...
  <Step title="초대 URL을 생성하고 봇을 서버에 추가">
  ...
  <Step title="개발자 모드를 활성화하고 ID 수집">
    다시 Discord 앱으로 돌아가 내부 ID를 복사할 수 있도록 개발자 모드를 활성화해야 합니다.
    ...
    **Server ID**와 **User ID**를 Bot Token과 함께 저장하세요. 다음 단계에서 이 세 가지를 모두 OpenClaw에 전달합니다.

  </Step>

  <Step title="서버 멤버의 DM 허용">
    페어링이 작동하려면 Discord에서 봇이 사용자에게 DM을 보낼 수 있어야 합니다. **server icon**을 우클릭 → **Privacy Settings** → **Direct Messages** 활성화.

    이렇게 하면 서버 멤버(봇 포함)가 사용자에게 DM을 보낼 수 있습니다. OpenClaw와 Discord DM을 사용하려면 이 설정을 켜 두세요. 길드 채널만 사용할 계획이라면 페어링 후 DM을 비활성화해도 됩니다.

  </Step>

  <Step title="봇 토큰을 안전하게 설정하기(채팅으로 보내지 마세요)">
    Discord 봇 토큰은 비밀 정보입니다(비밀번호와 유사). 에이전트에 메시지를 보내기 전에 OpenClaw가 실행 중인 머신에서 설정하세요.
...
```
  ...
  <Step title="OpenClaw 구성 및 페어링">
    ...
    <Tabs>
      <Tab title="에이전트에게 요청">
        기존 채널(예: Telegram)에서 OpenClaw 에이전트와 대화하면서 요청하세요. Discord가 첫 번째 채널이라면 CLI / config 탭을 사용하세요.
...
```
        ...
        평문 `token` 값도 지원됩니다. `channels.discord.token`에는 env/file/exec provider 전반에서 SecretRef 값도 지원됩니다. 자세한 내용은 [Secrets Management](/ko/gateway/secrets)를 참고하세요.
...
<Note>
토큰 해석은 계정 인식 방식으로 동작합니다. config의 토큰 값이 env 대체값보다 우선합니다. `DISCORD_BOT_TOKEN`은 기본 계정에만 사용됩니다.
고급 outbound 호출(message tool/channel action)의 경우, 호출별로 명시적인 `token`이 해당 호출에 사용됩니다. 이는 전송뿐 아니라 읽기/프로브 계열 작업(예: read/search/fetch/thread/pins/permissions)에도 적용됩니다. 계정 정책 및 재시도 설정은 여전히 활성 런타임 스냅샷에서 선택된 계정 기준으로 적용됩니다.
</Note>

## 권장: 길드 워크스페이스 설정

DM이 작동하면 Discord 서버를 전체 워크스페이스로 설정할 수 있습니다. 각 채널은 자체 컨텍스트를 가진 독립적인 에이전트 세션을 갖게 됩니다. 이 방식은 사용자와 봇만 있는 비공개 서버에 권장됩니다.
...
```
  ...
  <Step title="@mention 없이 응답 허용">
    기본적으로 에이전트는 길드 채널에서 @mention된 경우에만 응답합니다. 비공개 서버라면 모든 메시지에 응답하도록 설정하는 것이 더 적합할 수 있습니다.

    <Tabs>
      <Tab title="에이전트에게 요청">
        > "내 에이전트가 이 서버에서 @mention 없이도 응답하도록 해 주세요"
      </Tab>
      <Tab title="Config">
...
```
  ...
  <Step title="길드 채널의 메모리 계획">
    ...
    <Tabs>
      ...
      <Tab title="수동">
        모든 채널에서 공유 컨텍스트가 필요하면 안정적인 지침을 `AGENTS.md` 또는 `USER.md`에 넣으세요(이 파일들은 모든 세션에 주입됩니다). 장기 노트는 `MEMORY.md`에 보관하고 필요할 때 memory tool로 접근하세요.
      </Tab>
    </Tabs>
...
이제 Discord 서버에 몇 개의 채널을 만들고 대화를 시작하세요. 에이전트는 채널 이름을 볼 수 있으며, 각 채널은 자체적으로 격리된 세션을 가집니다. 따라서 `#coding`, `#home`, `#research`처럼 워크플로에 맞는 채널을 구성할 수 있습니다.
...
- 네이티브 슬래시 명령어는 격리된 명령 세션(`agent:<agentId>:discord:slash:<userId>`)에서 실행되며, 동시에 라우팅된 대화 세션에 `CommandTargetSessionKey`를 전달합니다.
...
OpenClaw는 에이전트 메시지에 대해 Discord components v2 컨테이너를 지원합니다. `components` 페이로드와 함께 message tool을 사용하세요. 상호작용 결과는 일반 인바운드 메시지처럼 에이전트로 다시 라우팅되며, 기존 Discord `replyToMode` 설정을 따릅니다.

지원되는 블록:

- `text`, `section`, `separator`, `actions`, `media-gallery`, `file`
- 액션 행은 최대 5개의 버튼 또는 단일 select menu를 허용합니다.
- Select 유형: `string`, `user`, `role`, `mentionable`, `channel`

기본적으로 컴포넌트는 1회용입니다. 버튼, select, form을 만료 전까지 여러 번 사용할 수 있게 하려면 `components.reusable=true`를 설정하세요.

버튼을 누를 수 있는 사용자를 제한하려면 해당 버튼에 `allowedUsers`를 설정하세요(Discord 사용자 ID, 태그 또는 `*`). 설정된 경우 일치하지 않는 사용자는 ephemeral 거부 메시지를 받습니다.

`/model` 및 `/models` 슬래시 명령어는 provider 및 model 드롭다운과 Submit 단계가 포함된 인터랙티브 모델 선택기를 엽니다. 선택기 응답은 ephemeral이며, 명령을 실행한 사용자만 사용할 수 있습니다.

파일 첨부:

- `file` 블록은 첨부 참조(`attachment://<filename>`)를 가리켜야 합니다.
- 첨부는 `media`/`path`/`filePath`(단일 파일)로 제공하세요. 여러 파일은 `media-gallery`를 사용하세요.
- 업로드 이름이 첨부 참조와 일치해야 할 경우 `filename`으로 업로드 이름을 재정의하세요.
...
<Tabs>
  ...
  <Tab title="길드 정책">
    ...
    - 선택적 발신자 허용 목록: `users`(안정적인 ID 권장) 및 `roles`(역할 ID만 허용); 둘 중 하나라도 구성되면 발신자는 `users` 또는 `roles` 중 하나와 일치할 때 허용됩니다
...
```
    ...
    `DISCORD_BOT_TOKEN`만 설정하고 `channels.discord` 블록을 만들지 않으면, `channels.defaults.groupPolicy`가 `open`이더라도 런타임 대체값은 `groupPolicy="allowlist"`가 됩니다(로그에 경고가 표시됨).
...
`bindings[].match.roles`를 사용해 역할 ID 기준으로 Discord 길드 멤버를 서로 다른 에이전트로 라우팅할 수 있습니다. 역할 기반 바인딩은 역할 ID만 허용하며, peer 또는 parent-peer 바인딩 다음이면서 guild-only 바인딩 이전에 평가됩니다. 바인딩이 다른 match 필드도 설정하는 경우(예: `peer` + `guildId` + `roles`), 구성된 모든 필드가 일치해야 합니다.
...
<AccordionGroup>
  ...
  <Accordion title="권한이 필요한 인텐트">
    **Bot -> Privileged Gateway Intents**에서 다음을 활성화하세요.
    ...
    Presence intent는 선택 사항이며 상태 업데이트를 수신하려는 경우에만 필요합니다. 봇 상태 설정(`setPresence`)은 멤버의 상태 업데이트 활성화를 요구하지 않습니다.
  ...
  <Accordion title="OAuth 범위 및 기본 권한">
    ...
    명시적으로 필요하지 않다면 `Administrator`는 피하세요.
  ...
  <Accordion title="ID 복사">
    ...
    신뢰할 수 있는 감사 및 프로브를 위해 OpenClaw config에서는 숫자 ID를 사용하는 것이 좋습니다.
...
- Discord UI에서는 권한이 없는 사용자에게도 명령어가 보일 수 있지만, 실행 시에는 여전히 OpenClaw 인증이 적용되며 "not authorized"가 반환됩니다.

명령어 카탈로그 및 동작은 [슬래시 명령어](/ko/tools/slash-commands)를 참고하세요.
...
<AccordionGroup>
  <Accordion title="답글 태그 및 네이티브 답글">
    ...
    `first`는 해당 턴의 첫 번째 outbound Discord 메시지에 항상 암시적 네이티브 답글 참조를 첨부합니다.
    `batched`는 인바운드 턴이 여러 메시지의 디바운스된 배치일 때만 Discord의 암시적 네이티브 답글 참조를 첨부합니다. 이는 모든 단일 메시지 턴이 아니라, 주로 모호하고 급격히 몰리는 채팅에서만 네이티브 답글을 사용하고 싶을 때 유용합니다.

    메시지 ID는 컨텍스트/기록에 표시되므로 에이전트가 특정 메시지를 대상으로 지정할 수 있습니다.
  ...
  <Accordion title="라이브 스트림 미리보기">
    ...
    - 기본값이 `off`인 이유는 Discord 미리보기 수정이 특히 여러 봇 또는 게이트웨이가 동일한 계정이나 길드 트래픽을 공유할 때 레이트 리밋에 빠르게 도달할 수 있기 때문입니다.
    ...
    - `streaming.preview.toolProgress`는 tool/progress 업데이트가 동일한 초안 미리보기 메시지를 재사용할지 여부를 제어합니다(기본값: `true`). 별도의 tool/progress 메시지를 유지하려면 `false`로 설정하세요.
...
```
    ...
    `block` 모드 청크 기본값(`channels.discord.textChunkLimit` 범위 내로 제한됨):
...
```
    ...
    미리보기 스트리밍은 텍스트 전용이며, 미디어 답글은 일반 전달로 대체됩니다.

    참고: 미리보기 스트리밍은 block streaming과 별개입니다. Discord에서 block streaming이 명시적으로 활성화되면 OpenClaw는 중복 스트리밍을 피하기 위해 미리보기 스트림을 건너뜁니다.
  ...
  <Accordion title="기록, 컨텍스트 및 스레드 동작">
    ...
    채널 주제는 **신뢰되지 않은** 컨텍스트로 주입됩니다(시스템 프롬프트로는 주입되지 않음).
    답글 및 인용 메시지 컨텍스트는 현재 수신된 상태 그대로 유지됩니다.
    Discord 허용 목록은 주로 누가 에이전트를 트리거할 수 있는지를 제어하며, 전체 보조 컨텍스트 가림 경계는 아닙니다.
  ...
  <Accordion title="하위 에이전트를 위한 스레드 바인딩 세션">
    ...
    - `/session max-age <duration|off>` 포커스된 바인딩의 최대 수명 확인/업데이트
...
```
    ...
    - `/acp spawn codex --bind here`는 현재 Discord 채널 또는 스레드를 그 자리에서 바인딩하고, 이후 메시지도 계속 같은 ACP 세션으로 라우팅되도록 유지합니다.
    - 이는 여전히 "새 Codex ACP 세션 시작"을 의미할 수 있지만, 자체적으로 새 Discord 스레드를 생성하지는 않습니다. 기존 채널이 그대로 채팅 표면으로 유지됩니다.
    - Codex는 여전히 디스크상의 자체 `cwd` 또는 backend 워크스페이스에서 실행될 수 있습니다. 해당 워크스페이스는 Discord 스레드가 아니라 런타임 상태입니다.
    - 스레드 메시지는 부모 채널 ACP 바인딩을 상속할 수 있습니다.
    - 바인딩된 채널 또는 스레드에서는 `/new` 및 `/reset`이 같은 ACP 세션을 그 자리에서 재설정합니다.
    - 임시 스레드 바인딩도 계속 작동하며, 활성 상태에서는 대상 해석을 재정의할 수 있습니다.
    - `spawnAcpSessions`는 OpenClaw가 `--thread auto|here`를 통해 자식 스레드를 생성/바인딩해야 할 때만 필요합니다. 현재 채널에서 `/acp spawn ... --bind here`를 사용하는 경우에는 필요하지 않습니다.
...
```
    ...
    - 조회에 실패하면 프록시된 메시지는 봇 메시지로 처리되어 `allowBots=true`가 아니면 드롭됩니다
...
```
    ...
    - 2: 청취 중
    - 3: 시청 중
    - 4: 커스텀 (활동 텍스트를 상태 값으로 사용, 이모지는 선택 사항)
    - 5: 경쟁 중
...
```
  ...
  <Accordion title="Discord에서의 승인">
    Discord는 DM에서 버튼 기반 승인 처리를 지원하며, 선택적으로 승인 프롬프트를 원본 채널에 게시할 수 있습니다.
    ...
    `enabled`가 설정되지 않았거나 `"auto"`이고, `execApprovals.approvers` 또는 `commands.ownerAllowFrom`에서 최소 한 명 이상의 approver를 해석할 수 있으면 Discord는 네이티브 exec 승인을 자동으로 활성화합니다. Discord는 채널 `allowFrom`, 레거시 `dm.allowFrom`, 또는 direct-message `defaultTo`에서 exec approver를 추론하지 않습니다. Discord를 네이티브 승인 클라이언트로 명시적으로 비활성화하려면 `enabled: false`를 설정하세요.

    `target`이 `channel` 또는 `both`이면 승인 프롬프트가 채널에 표시됩니다. 해석된 approver만 버튼을 사용할 수 있으며, 다른 사용자는 ephemeral 거부 메시지를 받습니다. 승인 프롬프트에는 명령 텍스트가 포함되므로 신뢰할 수 있는 채널에서만 채널 전달을 활성화하세요. 세션 키에서 채널 ID를 유도할 수 없으면 OpenClaw는 DM 전달로 대체합니다.

    Discord는 다른 채팅 채널에서 사용하는 공유 승인 버튼도 렌더링합니다. 네이티브 Discord adapter는 주로 approver DM 라우팅과 채널 팬아웃을 추가합니다.
    이러한 버튼이 있는 경우, 그것이 기본 승인 UX입니다. OpenClaw는 tool 결과에서 채팅 승인을 사용할 수 없다고 나오거나 수동 승인이 유일한 경로일 때만 수동 `/approve` 명령을 포함해야 합니다.

    이 핸들러의 Gateway auth는 다른 Gateway 클라이언트와 동일한 공유 자격 증명 해석 계약을 사용합니다.

    - env 우선 로컬 auth (`OPENCLAW_GATEWAY_TOKEN` / `OPENCLAW_GATEWAY_PASSWORD`, 그다음 `gateway.auth.*`)
    - 로컬 모드에서는 `gateway.auth.*`가 설정되지 않은 경우에만 `gateway.remote.*`를 대체값으로 사용할 수 있습니다. 구성되었지만 해석되지 않은 로컬 SecretRef는 fail closed 됩니다
    - 적용 가능한 경우 `gateway.remote.*`를 통한 remote-mode 지원
    - URL 재정의는 재정의 안전성을 가집니다. CLI 재정의는 암시적 자격 증명을 재사용하지 않으며, env 재정의는 env 자격 증명만 사용합니다

    승인 해석 동작:

    - `plugin:` 접두사가 있는 ID는 `plugin.approval.resolve`를 통해 해석됩니다.
    - 그 외 ID는 `exec.approval.resolve`를 통해 해석됩니다.
    - 여기서 Discord는 추가적인 exec-to-plugin 대체 단계를 수행하지 않습니다. ID 접두사가 호출할 gateway 메서드를 결정합니다.

    Exec 승인은 기본적으로 30분 후 만료됩니다. 승인 실패 시 알 수 없는 승인 ID가 표시되면 approver 해석, 기능 활성화 여부, 그리고 전달된 승인 ID 종류가 대기 중인 요청과 일치하는지 확인하세요.
...
`event-create` 작업은 예약 이벤트 커버 이미지를 설정하기 위한 선택적 `image` 매개변수(URL 또는 로컬 파일 경로)를 허용합니다.
...
OpenClaw는 exec 승인 및 교차 컨텍스트 마커에 Discord components v2를 사용합니다. Discord 메시지 작업도 커스텀 UI를 위해 `components`를 받을 수 있습니다(고급 기능, discord tool을 통해 컴포넌트 페이로드를 구성해야 함). 레거시 `embeds`도 계속 사용할 수 있지만 권장되지는 않습니다.
...
세션 제어에는 Discord 전용 네이티브 명령 `/vc join|leave|status`를 사용하세요. 이 명령은 계정 기본 에이전트를 사용하며, 다른 Discord 명령과 동일한 allowlist 및 그룹 정책 규칙을 따릅니다.
...
- 음성 전사 턴은 Discord `allowFrom`(또는 `dm.allowFrom`)에서 owner 상태를 파생합니다. owner가 아닌 화자는 owner 전용 tool(예: `gateway`, `cron`)에 접근할 수 없습니다.
...
- OpenClaw는 수신 복호화 실패도 감시하며, 짧은 시간 창 내에 반복 실패가 발생하면 음성 채널에서 나갔다가 다시 참여하는 방식으로 자동 복구합니다.
- 수신 로그에 `DecryptionFailed(UnencryptedWhenPassthroughDisabled)`가 반복적으로 표시되면, 이는 [discord.js #11419](https://github.com/discordjs/discord.js/issues/11419)에 추적된 업스트림 `@discordjs/voice` 수신 버그일 수 있습니다.

## 음성 메시지

Discord 음성 메시지는 파형 미리보기를 표시하며 OGG/Opus 오디오와 메타데이터가 필요합니다. OpenClaw는 파형을 자동 생성하지만, 오디오 파일을 검사하고 변환하려면 Gateway 호스트에 `ffmpeg`와 `ffprobe`가 있어야 합니다.

요구 사항 및 제약 사항:

- **로컬 파일 경로**를 제공해야 합니다(URL은 거부됨).
- 텍스트 콘텐츠는 생략해야 합니다(Discord는 동일한 페이로드에 텍스트와 음성 메시지를 함께 허용하지 않음).
...
<AccordionGroup>
  ...
  <Accordion title="길드 메시지가 예상치 못하게 차단됨">
    ...
    - `groupPolicy` 확인
    - `channels.discord.guilds` 아래 길드 allowlist 확인
...
```
    ...
    느린 리스너 설정에는 `eventQueue.listenerTimeout`을 사용하고, 대기 중인 에이전트 턴에 대해 별도의 안전 장치가 필요할 때만 `inboundWorker.runTimeoutMs`를 사용하세요.
  ...
  <Accordion title="봇 간 루프">
    ...
    봇을 멘션한 봇 메시지만 수락하려면 `channels.discord.allowBots="mentions"`를 권장합니다.

  </Accordion>

  <Accordion title="Voice STT가 DecryptionFailed(...)와 함께 누락됨">
...
- 미디어/재시도: `mediaMaxMb`, `retry`
  - `mediaMaxMb`는 outbound Discord 업로드 상한을 설정합니다(기본값: `100MB`)
- 작업: `actions.*`
