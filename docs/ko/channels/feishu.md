---
read_when:
    - Feishu/Lark 봇을 연결하고 싶으신 것입니다
    - Feishu 채널을 구성하고 있습니다
summary: Feishu 봇 개요, 기능 및 구성
title: Feishu
x-i18n:
    generated_at: "2026-04-24T06:03:10Z"
    model: gpt-5.4
    provider: openai
...
> **OpenClaw 2026.4.24 이상이 필요합니다.** 확인하려면 `openclaw --version`을 실행하세요. 업그레이드하려면 `openclaw update`를 사용하세요.
...
<Steps>
  ...
  <Step title="설정이 완료된 후 변경 사항을 적용하려면 게이트웨이를 재시작하세요">
  ```bash
...
누가 봇에 DM을 보낼 수 있는지 제어하려면 `dmPolicy`를 구성하세요.
...
게이트웨이를 시작하고 봇에 DM을 보낸 다음 로그를 확인하세요.
...
5. 게이트웨이가 실행 중인지 확인하세요: `openclaw gateway status`
...
2. 구성에서 값을 업데이트하세요
3. 게이트웨이를 재시작하세요: `openclaw gateway restart`
...
`streaming: false`로 설정하면 전체 응답을 하나의 메시지로 보냅니다.

### 할당량 최적화

선택적 플래그 두 개로 Feishu/Lark API 호출 수를 줄일 수 있습니다.
...
Feishu/Lark는 DM 및 그룹 스레드 메시지에 대해 ACP를 지원합니다. Feishu/Lark ACP는 텍스트 명령 기반이며, 기본 슬래시 명령 메뉴는 없으므로 대화에서 `/acp ...` 메시지를 직접 사용하세요.
...
Feishu/Lark DM 또는 스레드에서 다음을 입력하세요.
...
`bindings`를 사용하여 Feishu/Lark DM 또는 그룹을 서로 다른 에이전트로 라우팅하세요.
