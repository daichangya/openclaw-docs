...
title: Feishu
x-i18n:
    generated_at: "2026-04-25T12:23:25Z"
    model: gpt-5.4
    provider: openai
...
> **OpenClaw 2026.4.25 이상이 필요합니다.** 확인하려면 `openclaw --version`을 실행하세요. 업그레이드하려면 `openclaw update`를 사용하세요.
...
`defaultAccount`는 아웃바운드 API에서 `accountId`를 지정하지 않을 때 어떤 계정을 사용할지 제어합니다.
...
Feishu/Lark는 DM과 그룹 스레드 메시지에 대해 ACP를 지원합니다. Feishu/Lark ACP는 텍스트 명령 기반이며, 기본 슬래시 명령 메뉴가 없으므로 대화에서 `/acp ...` 메시지를 직접 사용하세요.
...
| `channels.feishu.groups.<chat_id>.requireMention` | 그룹별 @멘션 재정의                        | 상속됨           |
...
- ✅ 인라인 답글
- ✅ 스레드 답글
- ✅ 스레드 메시지에 답글할 때 미디어 답글도 스레드 인식을 유지합니다
