...
summary: 선택적인 Docker 기반 OpenClaw 설정 및 온보딩
title: Docker
x-i18n:
    generated_at: "2026-04-21T06:04:49Z"
    model: gpt-5.4
    provider: openai
...
- **예**: 격리되고 일회성으로 사용할 Gateway 환경이 필요하거나 로컬 설치 없이 호스트에서 OpenClaw를 실행하려는 경우
- **아니요**: 자신의 머신에서 실행 중이고 가장 빠른 개발 루프만 원할 경우. 대신 일반 설치 흐름을 사용하세요.
- **샌드박싱 참고**: 기본 샌드박스 백엔드는 샌드박싱이 활성화되면 Docker를 사용하지만, 샌드박싱은 기본적으로 꺼져 있으며 전체 Gateway를 Docker에서 실행할 **필요는 없습니다**. SSH 및 OpenShell 샌드박스 백엔드도 사용할 수 있습니다. 자세한 내용은 [샌드박싱](/ko/gateway/sandboxing)을 참고하세요.
...
<Steps>
  <Step title="이미지 빌드">
    ...
    이렇게 하면 Gateway 이미지를 로컬에서 빌드합니다. 대신 사전 빌드된 이미지를 사용하려면:
  ...
  <Step title="온보딩 완료">
    설정 스크립트는 자동으로 온보딩을 실행합니다. 수행되는 작업:

    - provider API 키 입력 요청
    - Gateway 토큰 생성 후 `.env`에 기록
    - Docker Compose를 통해 Gateway 시작

    설정 중에는 시작 전 온보딩 및 config 쓰기가 직접
    `openclaw-gateway`를 통해 실행됩니다. `openclaw-cli`는
    Gateway 컨테이너가 이미 존재한 뒤에 실행하는 명령용입니다.

  </Step>

  <Step title="Control UI 열기">
    브라우저에서 `http://127.0.0.1:18789/`를 열고 Settings에 구성된
    공유 비밀을 붙여넣으세요. 설정 스크립트는 기본적으로 토큰을 `.env`에 기록합니다.
    컨테이너 구성을 비밀번호 인증으로 바꿨다면 해당 비밀번호를 대신 사용하세요.
...
또는 `OPENCLAW_HOME_VOLUME`을 활성화했다면, 설정 스크립트가 `docker-compose.extra.yml`을 기록합니다.
이 경우 `-f docker-compose.yml -f docker-compose.extra.yml`과 함께 포함하세요.
</Note>
...
Docker 이미지에는 `/healthz`를 ping하는 내장 `HEALTHCHECK`가 포함됩니다.
...
Docker 포트 게시와 함께 호스트에서 `http://127.0.0.1:18789`로 접근할 수 있습니다.
...
<Note>
`0.0.0.0` 또는 `127.0.0.1` 같은 호스트 alias가 아니라
`gateway.bind`의 bind 모드 값(`lan` / `loopback` / `custom` /
...
이 경로는 컨테이너 교체 후에도 유지됩니다.

이 마운트된 config 디렉터리는 OpenClaw가 다음을 보관하는 위치입니다:
...
[Docker VM Runtime - 무엇이 어디에 유지되는가](/ko/install/docker-vm-runtime#what-persists-where)를 참고하세요.

**디스크 증가 주요 지점:** `media/`, 세션 JSONL 파일, `cron/runs/*.jsonl`,
...
이전 `scripts/shell-helpers/clawdock-helpers.sh` 원시 경로에서 ClawDock을 설치했다면,
위 설치 명령을 다시 실행해서 로컬 도우미 파일이 새 위치를 따라가도록 하세요.

그다음 `clawdock-start`, `clawdock-stop`, `clawdock-dashboard` 등을 사용하세요.
모든 명령은 `clawdock-help`를 실행해 확인할 수 있습니다.
전체 도우미 가이드는 [ClawDock](/ko/install/clawdock)을 참고하세요.

<AccordionGroup>
  <Accordion title="Docker Gateway용 agent 샌드박스 활성화">
    ```bash
    ...
    스크립트는 샌드박스 사전 요구 사항이 통과한 뒤에만 `docker.sock`을 마운트합니다.
  ...
  <Accordion title="자동화 / CI(비대화형)">
    Compose 가상 TTY 할당을 `-T`로 비활성화하세요:
  ...
  <Accordion title="공유 네트워크 보안 참고">
    ...
    신뢰 경계로 취급하세요. compose 구성은 `openclaw-cli`에서 `NET_RAW`/`NET_ADMIN`을 제거하고
    `no-new-privileges`를 활성화합니다.
  </Accordion>

  <Accordion title="권한 및 EACCES">
    이미지는 `node`(uid 1000) 사용자로 실행됩니다. `/home/node/.openclaw`에서
    권한 오류가 보인다면, 호스트 바인드 마운트가 uid 1000 소유인지 확인하세요:
  ...
  <Accordion title="고급 사용자용 컨테이너 옵션">
    기본 이미지는 보안 우선이며 루트가 아닌 `node` 사용자로 실행됩니다. 더
    기능이 많은 컨테이너를 원한다면:
  ...
  <Accordion title="OpenAI Codex OAuth(헤드리스 Docker)">
    ...
    Docker 또는 헤드리스 환경에서는 최종적으로 이동한 전체 리디렉션 URL을 복사해서
    인증을 완료하기 위해 마법사에 다시 붙여넣으세요.
  </Accordion>

  <Accordion title="기본 이미지 메타데이터">
    메인 Docker 이미지는 `node:24-bookworm`을 사용하며
    `org.opencontainers.image.base.name`,
    `org.opencontainers.image.source` 등을 포함한 OCI 기본 이미지
    annotation을 게시합니다. 자세한 내용은
    [OCI image annotations](https://github.com/opencontainers/image-spec/blob/main/annotations.md)를 참고하세요.
  </Accordion>
</AccordionGroup>

### VPS에서 실행 중인가요?

공유 VM 배포 단계(바이너리 내장, 영속성, 업데이트 포함)는
[Hetzner (Docker VPS)](/ko/install/hetzner) 및
[Docker VM Runtime](/ko/install/docker-vm-runtime)을 참고하세요.

## Agent 샌드박스

Docker 백엔드와 함께 `agents.defaults.sandbox`가 활성화되면, Gateway 자체는 호스트에 남아 있는 동안
Gateway는 agent 도구 실행(셸, 파일 읽기/쓰기 등)을 격리된 Docker
컨테이너 내부에서 실행합니다. 이를 통해 신뢰할 수 없거나 멀티테넌트 agent 세션 주위에
전체 Gateway를 컨테이너화하지 않고도 강한 격리 경계를 제공할 수 있습니다.

샌드박스 범위는 agent별(기본값), 세션별 또는 공유로 설정할 수 있습니다. 각 범위는
`/workspace`에 마운트된 자체 워크스페이스를 가집니다. 또한
도구 허용/차단 정책, 네트워크 격리, 리소스 제한, 브라우저
컨테이너도 구성할 수 있습니다.

전체 구성, 이미지, 보안 참고 사항, 멀티 agent 프로필은 다음을 참고하세요:

- [샌드박싱](/ko/gateway/sandboxing) -- 전체 샌드박스 참고 문서
- [OpenShell](/ko/gateway/openshell) -- 샌드박스 컨테이너에 대한 대화형 셸 액세스
- [Multi-Agent Sandbox and Tools](/ko/tools/multi-agent-sandbox-tools) -- agent별 오버라이드
...
<AccordionGroup>
  <Accordion title="이미지가 없거나 샌드박스 컨테이너가 시작되지 않음">
    ...
    샌드박스 이미지를 빌드하거나
    `agents.defaults.sandbox.docker.image`를 사용자 정의 이미지로 설정하세요.
  ...
  <Accordion title="사용자 정의 도구를 샌드박스에서 찾을 수 없음">
    OpenClaw는 `sh -lc`(로그인 셸)로 명령을 실행하므로
    `/etc/profile`을 읽고 PATH를 재설정할 수 있습니다. 사용자 정의
    도구 경로를 앞에 추가하려면 `docker.env.PATH`를 설정하거나, Dockerfile에서 `/etc/profile.d/` 아래에
    스크립트를 추가하세요.
  </Accordion>
  ...
  <Accordion title="Control UI에서 Unauthorized 또는 pairing required가 표시됨">
    새 dashboard 링크를 가져오고 브라우저 기기를 승인하세요:
  ...
  <Accordion title="Gateway 대상이 Docker CLI에서 ws://172.x.x.x로 표시되거나 pairing 오류가 발생함">
