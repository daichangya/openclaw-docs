...
title: CLI 设置参考
x-i18n:
    generated_at: "2026-04-23T19:52:13Z"
    model: gpt-5.4
    provider: openai
...
简要指南请参见 [设置向导（CLI）](/zh-CN/start/wizard)。
...
- 守护进程安装（LaunchAgent、systemd 用户单元，或原生 Windows Scheduled Task，并带有 Startup 文件夹回退方案）
- 健康检查
- Skills 设置
...
<Steps>
  <Step title="现有配置检测">
    ...
    - CLI `--reset` 默认使用 `config+creds+sessions`；使用 `--reset-scope full` 可额外移除工作区。
  ...
  <Step title="Gateway 网关">
    ...
    - 推荐：即使是 loopback，也保持 token 凭证启用，这样本地 WS 客户端仍必须进行身份验证。
    - 在 token 模式下，交互式设置提供：
      - **生成/存储明文 token**（默认）
      - **使用 SecretRef**（可选）
  ...
  <Step title="渠道">
    ...
    - [Mattermost](/zh-CN/channels/mattermost)：bot token + 基础 URL
  ...
  <Step title="守护进程安装">
    ...
    - Linux 和通过 WSL2 的 Windows：systemd 用户单元
      - 向导会尝试执行 `loginctl enable-linger <user>`，以便 Gateway 网关 在注销后继续运行。
    ...
    - 原生 Windows：优先使用 Scheduled Task
      ...
      - 仍然更推荐 Scheduled Task，因为它们能提供更好的 supervisor 状态。
    - 运行时选择：Node（推荐；WhatsApp 和 Telegram 必需）。不推荐使用 Bun。
  </Step>
  <Step title="健康检查">
    - 启动 Gateway 网关（如有需要）并运行 `openclaw health`。
    - `openclaw status --deep` 会将实时 Gateway 网关 健康探测添加到状态输出中，包括受支持时的渠道探测。
  </Step>
  <Step title="Skills">
    - 读取可用的 Skills 并检查要求。
    - 让你选择 node manager：npm、pnpm 或 bun。
    - 安装可选依赖（其中一些在 macOS 上使用 Homebrew）。
  </Step>
  <Step title="完成">
    - 提供摘要和后续步骤，包括 iOS、Android 和 macOS 应用选项。
  </Step>
</Steps>
...
- 如果远程 Gateway 网关 需要凭证，则提供 token（推荐）

<Note>
- 如果 Gateway 网关 仅限 loopback，请使用 SSH 隧道或 tailnet。
- 设备发现提示：
...
<AccordionGroup>
  <Accordion title="Anthropic API key">
    如果存在，则使用 `ANTHROPIC_API_KEY`；否则提示输入一个 key，然后保存以供守护进程使用。
  </Accordion>
  <Accordion title="OpenAI Code 订阅（OAuth）">
    浏览器流程；粘贴 `code#state`。

    当模型未设置或已属于 OpenAI 系列时，将 `agents.defaults.model` 设置为 `openai/gpt-5.5`。

  </Accordion>
  <Accordion title="OpenAI Code 订阅（设备配对）">
    使用短期有效 device code 的浏览器配对流程。

    当模型未设置或已属于 OpenAI 系列时，将 `agents.defaults.model` 设置为 `openai/gpt-5.5`。

  </Accordion>
  <Accordion title="OpenAI API key">
    如果存在，则使用 `OPENAI_API_KEY`；否则提示输入一个 key，然后将凭证存储到 auth profiles 中。
  ...
  <Accordion title="OpenCode">
    提示输入 `OPENCODE_API_KEY`（或 `OPENCODE_ZEN_API_KEY`），并让你选择 Zen 或 Go catalog。
  ...
  <Accordion title="Ollama（Cloud 和本地开放模型）">
    ...
    `Cloud + Local` 还会检查该 Ollama 主机是否已登录以访问 cloud。
    更多细节见：[Ollama](/zh-CN/providers/ollama)。
  </Accordion>
  <Accordion title="Moonshot 和 Kimi Coding">
    Moonshot（Kimi K2）和 Kimi Coding 配置会自动写入。
    更多细节见：[Moonshot AI（Kimi + Kimi Coding）](/zh-CN/providers/moonshot)。
  </Accordion>
  <Accordion title="自定义提供商">
    ...
    - **立即粘贴 API key**（明文）
    - **使用 secret reference**（env ref 或已配置 provider ref，带预检验证）
...
- 当新手引导从某个提供商凭证选项开始时，模型选择器会自动优先该提供商。对于 Volcengine 和 BytePlus（国际版），这种相同的优先逻辑也会匹配它们的 coding-plan 变体（`volcengine-plan/*`、
  `byteplus-plan/*`）。
- 如果该首选提供商筛选结果为空，选择器会回退到完整 catalog，而不是显示无模型。
- 向导会运行模型检查，并在所配置模型未知或缺少凭证时发出警告。
...
- `--secret-input-mode ref` 会启用引用模式，而不是明文 key 存储。
  ...
  - 已配置的 provider ref（`file` 或 `exec`），带 provider alias + id
- 交互式引用模式会在保存前执行快速预检验证。
  - Env refs：验证变量名以及当前新手引导环境中的非空值。
  - Provider refs：验证 provider 配置并解析请求的 id。
  - 如果预检失败，新手引导会显示错误并允许你重试。
- 在非交互模式下，`--secret-input-mode ref` 仅支持基于 env。
  - 在新手引导进程环境中设置 provider env var。
  - 内联 key 标志（例如 `--openai-api-key`）要求该 env var 已设置；否则新手引导会快速失败。
...
- `tools.profile`（本地新手引导在未设置时默认使用 `"coding"`；已有的显式值会被保留）
- `gateway.*`（模式、绑定、凭证、Tailscale）
- `session.dmScope`（本地新手引导在未设置时默认将其设为 `per-channel-peer`；已有的显式值会被保留）
- `channels.telegram.botToken`、`channels.discord.token`、`channels.matrix.*`、`channels.signal.*`、`channels.imessage.*`
- 当你在提示中选择启用时的渠道 allowlist（Slack、Discord、Matrix、Microsoft Teams）（如有可能，名称会解析为 ID）
- `skills.install.nodeManager`
  - `setup --node-manager` 标志接受 `npm`、`pnpm` 或 `bun`。
  - 手动配置稍后仍可将 `skills.install.nodeManager: "yarn"` 设为 `"yarn"`。
- `wizard.lastRunAt`
...
<Note>
某些渠道以 plugin 形式提供。在设置期间选择这些渠道时，向导会先提示安装该 plugin（npm 或本地路径），然后再进行渠道配置。
</Note>
