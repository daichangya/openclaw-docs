...
summary: 模型提供商概览，附带示例配置和 CLI 流程
title: 模型提供商
x-i18n:
    generated_at: "2026-04-23T22:56:55Z"
    model: gpt-5.4
    provider: openai
...
- GPT-5.5 当前可通过订阅/OAuth 路由使用：在 PI 中使用 `openai-codex/gpt-5.5`，或在使用 Codex app-server harness 时使用 `openai/gpt-5.5`。一旦 OpenAI 在公共 API 上启用 GPT-5.5，`openai/gpt-5.5` 的直连 API 密钥路由也会受支持；在此之前，对于 `OPENAI_API_KEY` 设置，请使用诸如 `openai/gpt-5.4` 这类已启用 API 的模型。

## 由插件拥有的 provider 行为

大多数 provider 特定逻辑都位于 provider 插件（`registerProvider(...)`）中，而 OpenClaw 保留通用推理循环。插件负责新手引导、模型目录、认证环境变量映射、传输/配置规范化、工具模式清理、故障切换分类、OAuth 刷新、用量报告、thinking/reasoning 配置文件等。

完整的 provider SDK 钩子列表及内置插件示例请参见[提供商插件](/zh-CN/plugins/sdk-provider-plugins)。如果某个 provider 需要完全自定义的请求执行器，那属于另一种更深层的扩展接口。

<Note>
Provider 运行时 `capabilities` 是共享的运行器元数据（provider 家族、转录/工具怪癖、传输/缓存提示）。它不同于[公共能力模型](/zh-CN/plugins/architecture#public-capability-model)，后者描述的是插件注册了什么（文本推理、语音等）。
</Note>

## API 密钥轮换

- 支持为选定的 provider 进行通用 provider 轮换。
- 可通过以下方式配置多个密钥：
  - `OPENCLAW_LIVE_<PROVIDER>_KEY`（单个实时覆盖，最高优先级）
  - `<PROVIDER>_API_KEYS`（逗号或分号分隔列表）
  - `<PROVIDER>_API_KEY`（主密钥）
  - `<PROVIDER>_API_KEY_*`（编号列表，例如 `<PROVIDER>_API_KEY_1`）
- 对于 Google providers，还会将 `GOOGLE_API_KEY` 作为回退项。
- 密钥选择顺序会保留优先级并去重值。
- 仅当响应为限流时，才会用下一个密钥重试请求（例如 `429`、`rate_limit`、`quota`、`resource exhausted`、`Too many concurrent requests`、`ThrottlingException`、`concurrency limit reached`、`workers_ai ... quota limit exceeded` 或周期性使用限制消息）。
- 非限流失败会立即失败；不会尝试密钥轮换。
- 当所有候选密钥都失败时，将返回最后一次尝试的最终错误。

## 内置 providers（pi-ai 目录）

OpenClaw 内置了 pi‑ai 目录。这些 providers **不需要** `models.providers` 配置；只需设置认证信息并选择一个模型。
...
- 如果你希望显式指定层级，而不是使用共享的 `/fast` 开关，请使用 `params.serviceTier`
...
- `openai/gpt-5.3-codex-spark` 在 OpenClaw 中被有意隐藏，因为实时 OpenAI API 请求会拒绝它，而当前 Codex 目录也未暴露它
...
- Anthropic 说明：Anthropic 员工告诉我们，OpenClaw 风格的 Claude CLI 用法再次被允许，因此除非 Anthropic 发布新的政策，否则 OpenClaw 会将 Claude CLI 复用和 `claude -p` 用法视为该集成的受认可方式。
- Anthropic setup-token 仍然作为受支持的 OpenClaw token 路径提供，但现在 OpenClaw 在可用时更优先选择 Claude CLI 复用和 `claude -p`。
...
- 与直连 `openai/*` 共享相同的 `/fast` 开关和 `params.fastMode` 配置；OpenClaw 会将其映射到 `service_tier=priority`
...
- 政策说明：OpenAI Codex OAuth 明确支持用于像 OpenClaw 这样的外部工具/工作流。
- 当前的 GPT-5.5 访问依赖此 OAuth/订阅路由，直到 OpenAI 在公共 API 中启用 GPT-5.5。
...
- 直连 Gemini 运行还接受 `agents.defaults.models["google/<model>"].params.cachedContent`（或旧版 `cached_content`），用于透传 provider 原生的 `cachedContents/...` 句柄；Gemini 缓存命中会显示为 OpenClaw `cacheRead`

### Google Vertex 和 Gemini CLI

- Providers：`google-vertex`、`google-gemini-cli`
- 认证：Vertex 使用 gcloud ADC；Gemini CLI 使用其 OAuth 流程
- 注意：OpenClaw 中的 Gemini CLI OAuth 属于非官方集成。有用户报告称，使用第三方客户端后 Google 账号受到了限制。如果你选择继续，请先阅读 Google 条款，并使用非关键账号。
- Gemini CLI OAuth 作为内置 `google` 插件的一部分提供。
  ...
  - 注意：你**不需要**将 client id 或 secret 粘贴到 `openclaw.json` 中。CLI 登录流程会将 token 存储在 Gateway 网关宿主机上的认证配置文件中。
...
- 静态回退目录内置了 `kilocode/kilo/auto`；实时 `https://api.kilo.ai/api/gateway/models` 发现可进一步扩展运行时目录。
- `kilocode/kilo/auto` 背后的确切上游路由由 Kilo Gateway 决定，而不是由 OpenClaw 硬编码。
...
- **OpenRouter** 仅在已验证的 `openrouter.ai` 路由上应用其应用归因请求头和 Anthropic `cache_control` 标记。作为代理式 OpenAI 兼容路径，它会跳过仅适用于原生 OpenAI 的整形（`serviceTier`、Responses `store`、提示缓存提示、OpenAI 推理兼容性）。基于 Gemini 的引用仅保留代理 Gemini 的 thought-signature 清理。
- **Kilo Gateway** 中基于 Gemini 的引用遵循相同的代理 Gemini 清理路径；`kilocode/kilo/auto` 以及其他不支持代理推理的引用会跳过代理推理注入。
- **MiniMax** 的 API 密钥新手引导会写入显式的 M2.7 模型定义，并设置 `input: ["text", "image"]`；而内置目录会在该配置实体化之前，将聊天引用保留为仅文本。
...
- **Cerebras** 的 GLM 模型使用 `zai-glm-4.7` / `zai-glm-4.6`；OpenAI 兼容基础 URL 为 `https://api.cerebras.ai/v1`。

## 通过 `models.providers` 配置 providers（自定义/基础 URL）

使用 `models.providers`（或 `models.json`）可添加**自定义** providers 或
OpenAI/Anthropic 兼容代理。

下面的许多内置 provider 插件已经发布了默认目录。
只有当你想覆盖默认基础 URL、请求头或模型列表时，才需要使用显式的
`models.providers.<id>` 条目。

### Moonshot AI（Kimi）

Moonshot 作为内置 provider 插件提供。默认情况下请使用内置 provider，只有在你需要覆盖基础 URL 或模型元数据时，才添加显式的 `models.providers.moonshot` 条目：
...
- Provider：`volcengine`（编码方案：`volcengine-plan`）
...
OpenClaw 会回退到未过滤的目录，而不是显示一个空的按 provider 范围筛选的选择器。
...
- Provider：`byteplus`（编码方案：`byteplus-plan`）
- 认证：`BYTEPLUS_API_KEY`
...
在 MiniMax 的 Anthropic 兼容流式传输路径上，除非你显式设置，否则 OpenClaw 默认会禁用 thinking，并且 `/fast on` 会将
...
- Web 搜索仍保留在 provider id `minimax`
...
OpenClaw 使用 LM Studio 原生的 `/api/v1/models` 和 `/api/v1/models/load` 实现设备发现 + 传输协议和自动加载，默认使用 `/v1/chat/completions` 进行推理。
...
当你通过 `OLLAMA_API_KEY` 选择启用时，系统会在本地 `http://127.0.0.1:11434` 检测 Ollama，并且内置 provider 插件会将 Ollama 直接加入 `openclaw onboard` 和模型选择器。有关新手引导、云端/本地模式以及自定义配置，请参见 [/providers/ollama](/zh-CN/providers/ollama)。
...
SGLang 作为内置 provider 插件提供，用于快速的自托管
OpenAI 兼容服务器：
...
- 对于自定义 providers，`reasoning`、`input`、`cost`、`contextWindow` 和 `maxTokens` 都是可选的。
  省略时，OpenClaw 默认值为：
  - `reasoning: false`
  - `input: ["text"]`
  - `cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }`
...
- 对于非原生端点上的 `api: "openai-completions"`（即任何主机不是 `api.openai.com` 的非空 `baseUrl`），OpenClaw 会强制 `compat.supportsDeveloperRole: false`，以避免 provider 因不支持 `developer` 角色而返回 400 错误。
- 代理式 OpenAI 兼容路由也会跳过仅适用于原生 OpenAI 的请求整形：没有 `service_tier`、没有 Responses `store`、没有提示缓存提示、没有 OpenAI 推理兼容负载整形，也没有隐藏的 OpenClaw 归因请求头。
...
- [提供商](/zh-CN/providers) — 按 provider 划分的设置指南
