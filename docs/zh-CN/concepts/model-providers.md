---
read_when:
    - 你需要一份按提供商逐一整理的模型设置参考文档
    - 你想要模型提供商的示例配置或 CLI 新手引导命令
summary: 模型提供商概览，包含示例配置 + CLI 流程
title: 模型提供商
x-i18n:
    generated_at: "2026-04-23T19:51:30Z"
    model: gpt-5.4
    provider: openai
...
- 回退规则、冷却探测以及会话覆盖持久化：请参见 [模型故障切换](/zh-CN/concepts/model-failover)。
- OpenAI GPT 模型引用的规范形式是 `openai/<model>`。旧版的 `openai-codex/<model>` 和 `codex/<model>` 引用仍保留为兼容别名，用于较早的配置和测试。对于原生 Codex app-server 执行，请将模型引用保持为 `openai/gpt-*`，并强制设置 `agents.defaults.embeddedHarness.runtime: "codex"` —— 请参见 [Codex harness](/zh-CN/plugins/codex-harness)。

## 由插件负责的提供商行为

大多数特定于提供商的逻辑都位于提供商插件（`registerProvider(...)`）中，而 OpenClaw 负责保持通用的推理循环。插件负责新手引导、模型目录、认证环境变量映射、传输/配置规范化、工具 schema 清理、故障切换分类、OAuth 刷新、使用情况上报、thinking/reasoning 配置文件等。

完整的 provider-SDK 钩子列表和内置插件示例，请参见 [提供商插件](/zh-CN/plugins/sdk-provider-plugins)。如果某个提供商需要完全自定义的请求执行器，则属于另一个更深层的扩展接口。

<Note>
提供商运行时 `capabilities` 是共享运行器元数据（提供商家族、转录/工具链特殊行为、传输/缓存提示）。它与[公共能力模型](/zh-CN/plugins/architecture#public-capability-model)不同，后者描述的是一个插件注册了什么能力（文本推理、语音等）。
</Note>

## API 密钥轮换

- 支持对选定提供商进行通用的提供商密钥轮换。
- 通过以下方式配置多个密钥：
  - `OPENCLAW_LIVE_<PROVIDER>_KEY`（单个实时覆盖，优先级最高）
  - `<PROVIDER>_API_KEYS`（逗号或分号分隔列表）
  - `<PROVIDER>_API_KEY`（主密钥）
  - `<PROVIDER>_API_KEY_*`（编号列表，例如 `<PROVIDER>_API_KEY_1`）
- 对于 Google 提供商，还会将 `GOOGLE_API_KEY` 作为回退项纳入。
- 密钥选择顺序会保留优先级并对值去重。
- 仅在收到限流响应时，才会使用下一个密钥重试请求（例如 `429`、`rate_limit`、`quota`、`resource exhausted`、`Too many concurrent requests`、`ThrottlingException`、`concurrency limit reached`、`workers_ai ... quota limit exceeded`，或周期性的使用量限制消息）。
- 非限流失败会立即失败；不会尝试密钥轮换。
...
- 如果你想显式指定层级，而不是使用共享的 `/fast` 开关，请使用 `params.serviceTier`
- 隐藏的 OpenClaw 归属头（`originator`、`version`、
  `User-Agent`）仅适用于发往 `api.openai.com` 的原生 OpenAI 流量，不适用于通用的 OpenAI 兼容代理
- 原生 OpenAI 路由还会保留 Responses `store`、提示缓存提示以及
  OpenAI reasoning 兼容负载整形；代理路由不会
...
- 直接发送到 Anthropic 公共接口的请求支持共享的 `/fast` 开关和 `params.fastMode`，包括发送到 `api.anthropic.com` 的 API 密钥认证和 OAuth 认证流量；OpenClaw 会将其映射为 Anthropic `service_tier`（`auto` 与 `standard_only`）
...
- Anthropic setup-token 仍作为受支持的 OpenClaw 令牌路径可用，但 OpenClaw 现在会在可用时优先使用 Claude CLI 复用和 `claude -p`。
...
- 隐藏的 OpenClaw 归属头（`originator`、`version`、
  `User-Agent`）仅附加在发往
  `chatgpt.com/backend-api` 的原生 Codex 流量上，不适用于通用的 OpenAI 兼容代理
- 与直接的 `openai/*` 共用相同的 `/fast` 开关和 `params.fastMode` 配置；OpenClaw 会将其映射为 `service_tier=priority`
- 当目录暴露该模型时，`openai/gpt-5.3-codex-spark` 仍可通过 Codex OAuth 使用；是否可用取决于权限
- `openai/gpt-5.5` 保持原生 `contextWindow = 1000000` 和默认运行时 `contextTokens = 272000`；可通过 `models.providers.openai-codex.models[].contextTokens` 覆盖运行时上限
...
- 注意：OpenClaw 中的 Gemini CLI OAuth 是非官方集成。有些用户报告在使用第三方客户端后遇到 Google 账号限制。如果你选择继续，请查看 Google 条款，并使用非关键账号。
- Gemini CLI OAuth 作为内置 `google` 插件的一部分提供。
  ...
  - Gemini CLI JSON 回复会从 `response` 中解析；使用情况会回退到
...
- **OpenRouter** 仅在已验证的 `openrouter.ai` 路由上应用其应用归属头和 Anthropic `cache_control` 标记。作为代理式的 OpenAI 兼容路径，它会跳过仅限原生 OpenAI 的整形逻辑（`serviceTier`、Responses `store`、提示缓存提示、OpenAI reasoning 兼容性处理）。以 Gemini 为后端的引用仅保留代理 Gemini 的 thought-signature 清理。
- **Kilo Gateway** 以 Gemini 为后端的引用遵循相同的代理 Gemini 清理路径；`kilocode/kilo/auto` 和其他不支持代理 reasoning 的引用会跳过代理 reasoning 注入。
- **MiniMax** API 密钥新手引导会写入显式的 M2.7 模型定义，并设置 `input: ["text", "image"]`；在该配置实际生成之前，内置目录会将聊天引用保留为仅文本。
- **xAI** 使用 xAI Responses 路径。`/fast` 或 `params.fastMode: true` 会将 `grok-3`、`grok-3-mini`、`grok-4` 和 `grok-4-0709` 改写为它们对应的 `*-fast` 变体。`tool_stream` 默认开启；可通过 `agents.defaults.models["xai/<model>"].params.tool_stream=false` 禁用。
...
在新手引导/配置模型选择器中，Volcengine 认证选项会优先展示
...
在新手引导/配置模型选择器中，BytePlus（国际版）认证选项会优先展示
...
`openclaw onboard` 和模型选择器中。有关新手引导、云端/本地模式以及自定义配置，请参见 [/providers/ollama](/zh-CN/providers/ollama)。
...
- 代理式 OpenAI 兼容路由也会跳过仅限原生 OpenAI 的请求
  整形逻辑：不发送 `service_tier`，不发送 Responses `store`，不发送提示缓存提示，不做
  OpenAI reasoning 兼容负载整形，也不附加隐藏的 OpenClaw 归属
  请求头。
