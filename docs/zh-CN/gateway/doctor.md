...
title: Doctor
x-i18n:
    generated_at: "2026-04-08T22:27:21Z"
    model: gpt-5.4
    provider: openai
...
接受默认值且不提示（包括在适用时的重启/服务/沙箱修复步骤）。

```bash
openclaw doctor --repair
```

不提示，直接应用建议的修复（在安全情况下执行修复 + 重启）。

```bash
openclaw doctor --repair --force
```

也应用激进修复（会覆盖自定义的 supervisor 配置）。

```bash
openclaw doctor --non-interactive
```

在无提示模式下运行，并且只应用安全迁移（配置规范化 + 磁盘上的状态移动）。跳过需要人工确认的重启/服务/沙箱操作。
检测到旧版状态迁移时会自动运行。
...
- 模型认证健康状况：检查 OAuth 过期状态，可刷新即将过期的令牌，并报告认证配置的冷却/禁用状态。
- 额外工作区目录检测（`~/openclaw`）。
- 启用沙箱隔离时的沙箱镜像修复。
...
- 针对开放私信策略的安全警告。
- 本地令牌模式的 Gateway 网关认证检查（当没有令牌来源时提供令牌生成功能；不会覆盖使用 SecretRef 配置的令牌）。
...
- **Backfill** 会扫描当前工作区中的历史 `memory/YYYY-MM-DD.md` 文件，
  运行 grounded REM diary 处理，并把可逆的回填条目写入
  `DREAMS.md`。
- **Reset** 只会从 `DREAMS.md` 中移除那些带标记的回填日记条目。
- **Clear Grounded** 只会移除来自历史回放、且尚未积累实时回忆或每日
  支持的 staged grounded-only 短期条目。

它们**不会**自行执行的事情：

- 它们不会编辑 `MEMORY.md`
- 它们不会运行完整的 Doctor 迁移
- 除非你先显式运行 staged CLI 路径，否则它们不会自动把 grounded 候选项
  放入实时短期提升存储中
...
这会把 grounded 的持久候选项放入短期 dreaming 存储，
同时保持 `DREAMS.md` 作为审查界面。
...
如果配置中包含旧版值结构（例如 `messages.ackReaction`
...
- 对于配置了命名 `accounts`、但仍残留单账户顶层渠道值的渠道，将这些账户作用域的值移动到该渠道选定提升的账户中（大多数渠道使用 `accounts.default`；Matrix 可保留现有匹配的命名/默认目标）
...
这可能会把模型强制路由到错误的 API，或者把费用清零。Doctor 会发出警告，
以便你移除该覆盖并恢复按模型进行的 API 路由 + 费用。

### 2c) 浏览器迁移和 Chrome MCP 就绪状态

如果你的浏览器配置仍指向已移除的 Chrome 扩展路径，Doctor
会将其规范化为当前的 host-local Chrome MCP 附加模型：

- `browser.profiles.*.driver: "extension"` 变为 `"existing-session"`
- 移除 `browser.relayBindHost`

当你使用 `defaultProfile:
"user"` 或已配置的 `existing-session` 配置时，Doctor 还会审计 host-local Chrome MCP 路径：

- 检查默认自动连接配置在同一主机上是否已安装 Google Chrome
...
Doctor 无法替你启用 Chrome 端设置。host-local Chrome MCP
仍然需要：
...
这里的就绪状态仅与本地附加前提条件有关。Existing-session 会保留
...
授权端点，以验证本地 Node/OpenSSL TLS 栈是否能够验证证书链。如果
探测因证书错误而失败（例如 `UNABLE_TO_GET_ISSUER_CERT_LOCALLY`、证书已过期或自签名证书），
Doctor 会打印特定于平台的修复指导。在 macOS 上，如果使用 Homebrew Node，
修复方法通常是 `brew postinstall ca-certificates`。使用 `--deep` 时，
即使 Gateway 网关健康，此探测也会运行。

### 2c) Codex OAuth 提供商覆盖

如果你之前在
`models.providers.openai-codex` 下添加过旧版 OpenAI 传输设置，它们可能会遮蔽
较新版本自动使用的内置 Codex OAuth
提供商路径。Doctor 会在看到这些旧的传输设置与 Codex OAuth
并存时发出警告，以便你删除或重写过期的传输覆盖，
并恢复内置的路由/回退行为。
...
这些迁移会尽力执行且具备幂等性；当它保留任何旧版文件夹作为备份时，
Doctor 会发出警告。Gateway 网关/CLI 在启动时也会自动迁移
旧版会话 + 智能体目录，因此历史记录/认证/模型会落到每个智能体的路径下，
无需手动运行 Doctor。WhatsApp 认证有意只通过 `openclaw doctor` 迁移。
Talk provider/provider-map 规范化现在会按结构相等性比较，因此仅键顺序不同的差异
不再触发重复的无操作 `doctor --fix` 变更。
...
或者在覆盖时使用 `cron.store`），查找调度器当前仍为兼容性而接受的
旧任务结构。
...
Doctor 只会在确认不会改变行为的情况下自动迁移 `notify: true` 任务。
如果某个任务把旧版 notify 回退与现有
非 webhook delivery 模式组合在一起，Doctor 会发出警告，并将该任务留给人工审查。
...
模式下，它会自动移除过期锁文件；否则会打印说明并
指示你使用 `--fix` 重新运行。

### 4) 状态完整性检查（会话持久化、路由和安全性）

状态目录是运行系统的中枢。若它消失，你将失去
...
- **macOS 云同步状态目录**：当状态解析到 iCloud Drive
  ...
  `~/Library/CloudStorage/...` 下时发出警告，因为同步支持路径会导致更慢的 I/O
  以及锁/同步竞争。
...
- **转录不匹配**：当最近的会话条目缺少
  转录文件时发出警告。
- **主会话“单行 JSONL”**：当主转录只有一行时
  标记出来（历史没有持续累积）。
- **多个状态目录**：当不同主目录下存在多个 `~/.openclaw` 文件夹，
  或 `OPENCLAW_STATE_DIR` 指向其他位置时发出警告（历史可能会在不同安装之间拆分）。
- **远程模式提醒**：如果 `gateway.mode=remote`，Doctor 会提醒你在
  远程主机上运行它（状态存放在那里）。
- **配置文件权限**：如果 `~/.openclaw/openclaw.json` 对
  组/全局可读，Doctor 会发出警告，并提供收紧到 `600` 的选项。

### 5) 模型认证健康状况（OAuth 过期）

Doctor 会检查认证存储中的 OAuth 配置，在令牌
即将过期/已过期时发出警告，并在安全情况下刷新它们。如果 Anthropic
...
`invalid_grant`，或提供商告知你需要重新登录），Doctor 会报告
需要重新认证，并打印出确切的 `openclaw models auth login --provider ...`
命令供你运行。

Doctor 还会报告因以下原因暂时不可用的认证配置：
...
目录和 allowlist 校验模型引用，并在它无法解析或不被允许时发出警告。
...
并提供移除它们并使用当前 Gateway 网关端口安装 OpenClaw 服务的选项。
它还可以扫描额外的类似 Gateway 网关的服务并打印清理提示。
带 profile 名称的 OpenClaw Gateway 网关服务被视为一等公民，不会
被标记为“额外”。
...
- **旧版工作区目录**：当 `~/openclaw` 或其他旧版工作区目录
  与当前工作区同时存在时发出警告。
- **插件状态**：统计已加载/已禁用/出错的插件；列出出错插件的插件 ID；
  报告 bundle 插件能力。
- **插件兼容性警告**：标记与
  当前运行时存在兼容性问题的插件。
...
限制时，Doctor 会打印用于调整 `agents.defaults.bootstrapMaxChars`
...
- 如果 Telegram bot 令牌通过 SecretRef 配置，但在当前命令路径中不可用，Doctor 会报告该凭证已配置但不可用，并跳过自动解析，而不是崩溃或错误地把令牌报告为缺失。
...
Doctor 会检查默认智能体已配置的 Memory 搜索嵌入提供商是否
已就绪。行为取决于已配置的后端和提供商：

- **QMD 后端**：探测 `qmd` 二进制是否可用且可启动。
  如果不可用，会打印修复指导，包括 npm 包和手动二进制路径选项。
- **显式本地提供商**：检查本地模型文件或已识别的
  远程/可下载模型 URL。若缺失，则建议切换到远程提供商。
- **显式远程提供商**（`openai`、`voyage` 等）：验证 API 密钥
  是否存在于环境变量或认证存储中。若缺失，会打印可执行的修复提示。
- **自动提供商**：先检查本地模型可用性，再按自动选择顺序尝试每个远程
  提供商。

当 Gateway 网关探测结果可用时（检查期间 Gateway 网关处于健康状态），
Doctor 会将其结果与 CLI 可见配置交叉比对，并指出
任何差异。
...
缺失或过期的默认项（例如 systemd 的 `network-online` 依赖和
...
- 如果令牌认证需要令牌且 `gateway.auth.token` 由 SecretRef 管理，Doctor 的服务安装/修复会验证 SecretRef，但不会把已解析的明文令牌值持久化到 supervisor 服务环境元数据中。
- 如果令牌认证需要令牌且配置的令牌 SecretRef 无法解析，Doctor 会阻止安装/修复路径，并提供可执行的指导。
...
Gateway 网关端口（默认 `18789`）上的端口冲突，并报告可能原因（Gateway 网关已在运行、
SSH 隧道）。

### 17) Gateway 网关运行时最佳实践

当 Gateway 网关服务运行在 Bun 上，或运行于版本管理的 Node 路径
...
当工作区缺少 Memory 系统时，Doctor 会给出建议；如果工作区尚未纳入 git，
它还会打印备份提示。
