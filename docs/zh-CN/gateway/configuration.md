...
如果文件不存在，OpenClaw 会使用安全的默认值。添加配置的常见原因包括：
...
本页用于提供面向任务的指引，而[配置参考](/zh-CN/gateway/configuration-reference)则提供更全面的
字段映射和默认值。
...
<Tabs>
  ...
  <Tab title="Control UI">
    ...
    另有 **原始 JSON** 编辑器作为兜底入口。对于逐层下钻的
    UI 和其他工具，Gateway 网关 还会公开 `config.schema.lookup`，用于获取
...
每次成功启动后，Gateway 网关 都会保留一份可信的“最后已知可用”副本。
如果之后 `openclaw.json` 校验失败（或缺少 `gateway.mode`、内容
显著缩小，或前面意外插入了一行日志），OpenClaw 会将损坏的文件
...
<AccordionGroup>
  ...
  <Accordion title="调整 gateway 渠道健康监控">
    ...
    - 使用 `channels.<provider>.healthMonitor.enabled` 或 `channels.<provider>.accounts.<id>.healthMonitor.enabled`，可只为某一个渠道或账户禁用自动重启，而不必关闭全局监控。
  ...
  <Accordion title="为官方 iOS 构建启用基于 relay 的推送">
    ...
    3. 将 iOS 应用与 gateway 配对，并让节点会话和操作员会话都连接上。
    ...
    - 如果你将 iOS 应用切换到另一个 gateway，请重新连接应用，以便它发布一个绑定到该 gateway 的新 relay 注册。
  ...
  <Accordion title="将配置拆分到多个文件中（$include）">
    ...
    - **不支持透传写入**：根级 include、include 数组，以及带有同级覆盖的 include，
...
会被拒绝，并另存为 `.rejected.*` 以供检查。

插件局部校验失败是一个例外：如果所有问题都位于
`plugins.entries.<id>...` 下，重载会保持当前配置，并报告插件问题，
而不是恢复 `.last-good`。
...
- 对于部分更新，使用 `config.patch`（JSON merge patch：对象合并，`null`
  删除，数组替换）
...
<Accordion title="SecretRef 引用（env、file、exec）">
