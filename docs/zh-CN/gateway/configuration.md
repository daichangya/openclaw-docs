...
生效的配置路径必须是一个常规文件。对于 OpenClaw 拥有的写入操作，不支持使用符号链接的 `openclaw.json`
布局；原子写入可能会替换该路径，而不是保留符号链接。如果你将配置保存在默认状态目录之外，
...
智能体和自动化在编辑配置之前，应使用 `config.schema.lookup` 获取精确到字段级别的
文档。此页面用于面向任务的指导，而[配置参考](/zh-CN/gateway/configuration-reference)
则提供更全面的字段映射和默认值。

<Tip>
**刚接触配置？** 从 `openclaw onboard` 开始进行交互式设置，或者查看[配置示例](/zh-CN/gateway/configuration-examples)指南，获取完整的可复制粘贴配置。
</Tip>
...
<Tabs>
  ...
  <Tab title="Control UI">
    ...
    同时提供 **原始 JSON** 编辑器作为应急出口。对于下钻式 UI
    和其他工具，Gateway 网关 还会暴露 `config.schema.lookup`，用于
    获取一个限定在某个路径范围内的 schema 节点及其直接子项摘要。
  </Tab>
  <Tab title="直接编辑">
...
<Warning>
OpenClaw 只接受完全匹配 schema 的配置。未知键名、格式错误的类型或无效值都会导致 Gateway 网关 **拒绝启动**。根级别唯一的例外是 `$schema`（字符串），这样编辑器就可以附加 JSON Schema 元数据。
</Warning>

`openclaw config schema` 会输出 Control UI
和校验所使用的规范 JSON Schema。`config.schema.lookup` 会获取单个限定路径范围的节点以及
子项摘要，供下钻式工具使用。字段 `title`/`description` 文档元数据
会延续到嵌套对象、通配符（`*`）、数组项（`[]`）以及 `anyOf`/
`oneOf`/`allOf` 分支。加载 manifest 注册表后，运行时插件和渠道 schema 也会合并进来。

当校验失败时：

- Gateway 网关 不会启动
- 只有诊断命令可用（`openclaw doctor`, `openclaw logs`, `openclaw health`, `openclaw status`）
- 运行 `openclaw doctor` 以查看精确问题
- 运行 `openclaw doctor --fix`（或 `--yes`）以应用修复

Gateway 网关 每次成功启动后，都会保留一个可信的最后已知正常副本。
如果之后 `openclaw.json` 校验失败（或丢失 `gateway.mode`、体积
明显缩小，或前面被意外加上一行日志），OpenClaw 会将损坏的文件保留为 `.clobbered.*`，
恢复最后已知正常副本，并记录恢复原因。下一次智能体轮次也会收到一个系统事件警告，
这样主智能体就不会盲目重写已恢复的配置。当候选配置包含经过脱敏的密钥占位符
（例如 `***`）时，将跳过提升为最后已知正常副本。如果所有校验问题都限定在 `plugins.entries.<id>...` 范围内，OpenClaw
不会执行整文件恢复。它会保持当前配置处于激活状态，并呈现插件本地失败，
...
<AccordionGroup>
  ...
  <Accordion title="选择并配置模型">
    ...
    - 使用 `openclaw config set agents.defaults.models '<json>' --strict-json --merge` 添加 allowlist 条目，而不会删除现有模型。会删除条目的普通替换会被拒绝，除非你传入 `--replace`。
    - 模型引用使用 `provider/model` 格式（例如 `anthropic/claude-opus-4-6`）。
    - `agents.defaults.imageMaxDimensionPx` 控制转录/工具图像的缩放下限（默认 `1200`）；在大量使用截图的运行中，较低的值通常会减少视觉 token 使用量。
  ...
  <Accordion title="控制谁可以向机器人发送消息">
    ...
    有关每个渠道的详细信息，请参阅[完整参考](/zh-CN/gateway/config-channels#dm-and-group-access)。
  ...
  <Accordion title="设置群聊提及门控">
    ...
    - 每个渠道的覆盖设置和自聊模式，请参阅[完整参考](/zh-CN/gateway/config-channels#group-chat-mention-gating)。
  ...
  <Accordion title="按智能体限制 Skills">
    ...
    - 将 `agents.list[].skills: []` 设为空，则不启用任何 Skills。
  ...
  <Accordion title="调整 Gateway 网关 渠道健康监控">
    ...
    - 将 `gateway.channelHealthCheckMinutes: 0` 设为 0 以全局禁用健康监控重启。
  ...
  <Accordion title="启用沙箱隔离">
    ...
    有关完整指南，请参阅[沙箱隔离](/zh-CN/gateway/sandboxing)；有关所有选项，请参阅[完整参考](/zh-CN/gateway/config-agents#agentsdefaultssandbox)。
  ...
  <Accordion title="为官方 iOS 构建启用基于 relay 的推送">
    ...
    - 使用由已配对 iOS 应用转发的、限定在注册范围内的发送授权。Gateway 网关 不需要部署范围的 relay token。
    - 将每个基于 relay 的注册绑定到 iOS 应用所配对的 Gateway 网关 身份，因此其他 Gateway 网关 无法复用已存储的注册信息。
    - 让本地/手动构建的 iOS 版本继续直接使用 APNs。基于 relay 的发送仅适用于通过 relay 注册的官方分发版本。
    ...
    有关端到端流程，请参阅[iOS App](/zh-CN/platforms/ios#relay-backed-push-for-official-builds)；有关 relay 安全模型，请参阅[认证与信任流程](/zh-CN/platforms/ios#authentication-and-trust-flow)。
  ...
  <Accordion title="设置 webhook（hooks）">
    ...
    - 对于由 hook 驱动的智能体，优先使用强大的现代模型层级和严格的工具策略（例如尽量仅允许消息传递并启用沙箱隔离）。
  ...
  <Accordion title="配置多智能体路由">
    ...
    绑定规则和每个智能体的访问配置文件请参阅 [Multi-Agent](/zh-CN/concepts/multi-agent) 和[完整参考](/zh-CN/gateway/config-agents#multi-agent-routing)。
  ...
  <Accordion title="将配置拆分为多个文件（$include）">
    ...
    - **单文件**：替换所在对象
    - **文件数组**：按顺序进行深度合并（后者优先）
    - **同级键**：在 include 之后再合并（覆盖 include 的值）
    - **嵌套 include**：最多支持 10 层嵌套
    - **相对路径**：相对于包含它的文件解析
    - **OpenClaw 拥有的写入**：当一次写入只更改一个顶级部分，
      且该部分由单文件 include 支撑，例如 `plugins: { $include: "./plugins.json5" }`，
      OpenClaw 会更新该被包含文件，并保持 `openclaw.json` 不变
    - **不支持的透传写入**：根 include、include 数组以及带有同级覆盖的 includes
      在 OpenClaw 拥有的写入中会以失败关闭的方式处理，而不是
      将配置展平
...
直接文件编辑在通过校验之前都会被视为不可信。监视器会等待
编辑器临时写入/重命名的抖动结束，读取最终文件，并通过恢复最后已知正常配置来拒绝
无效的外部编辑。OpenClaw 拥有的
配置写入在写入前也会通过相同的 schema 闸门；具有破坏性的覆盖，
例如删除 `gateway.mode` 或将文件缩小到原来的一半以下，都会被拒绝，
并保存为 `.rejected.*` 供检查。

插件本地校验失败是个例外：如果所有问题都位于
`plugins.entries.<id>...` 下，重载会保留当前配置并报告该插件问题，
而不是恢复 `.last-good`。
...
大多数字段都能在不停机的情况下热应用。在 `hybrid` 模式下，需要重启的更改会被自动处理。
...
当你编辑一个通过 `$include` 引用的源文件时，OpenClaw 会根据源作者编写的布局来
规划重载，而不是根据展平后的内存视图。
这样即使某个
顶级部分单独位于自己的被包含文件中，例如
`plugins: { $include: "./plugins.json5" }`，热重载决策（热应用还是重启）也仍然可预测。如果
源布局存在歧义，重载规划会以失败关闭的方式处理。
...
- `config.patch` 用于局部更新（JSON merge patch：对象合并，`null`
  表示删除，数组表示替换）
- 仅当你打算替换整个配置时才使用 `config.apply`
- `update.run` 用于显式自更新并重启
- `update.status` 用于检查最近一次更新重启哨兵，并在重启后验证当前运行版本

智能体应将 `config.schema.lookup` 视为获取精确
字段级文档和约束的首选入口。需要更广泛的配置映射、默认值或专用
...
`update.status` 是只读的，但仍属于管理员作用域，因为重启哨兵可能会
包含更新步骤摘要和命令输出尾部。
</Note>
...
<Accordion title="在配置值中替换环境变量">
  可以在任何配置字符串值中使用 `${VAR_NAME}` 引用环境变量：
