...
title: 配置
x-i18n:
    generated_at: "2026-04-24T03:15:37Z"
    model: gpt-5.4
    provider: openai
...
<Tip>
**刚开始接触配置？** 可先使用 `openclaw onboard` 进行交互式设置，或查看[配置示例](/zh-CN/gateway/configuration-examples)指南，获取完整的可复制粘贴配置。
</Tip>
...
<Tabs>
  ...
  <Tab title="Control UI">
    ...
    UI 和其他工具，Gateway 网关还暴露了 `config.schema.lookup`，用于
    获取一个路径作用域的 schema 节点及其直接子项摘要。
  </Tab>
  <Tab title="直接编辑">
...
<Warning>
OpenClaw 仅接受与 schema 完全匹配的配置。未知键、格式错误的类型或无效值都会导致 Gateway 网关**拒绝启动**。唯一的根级例外是 `$schema`（字符串），这样编辑器就可以附加 JSON Schema 元数据。
</Warning>

`openclaw config schema` 会打印 Control UI
和校验所使用的规范 JSON Schema。`config.schema.lookup` 会获取单个路径作用域节点及其
子项摘要，供逐层深入工具使用。字段 `title`/`description` 文档元数据
会沿着嵌套对象、通配符（`*`）、数组项（`[]`）以及 `anyOf`/
`oneOf`/`allOf` 分支继续传递。当清单注册表加载后，运行时插件和渠道 schema
也会被合并进来。
...
Gateway 网关会在每次成功启动后保留一份可信的“最近一次已知正常”副本。
如果之后 `openclaw.json` 校验失败（或缺失 `gateway.mode`、内容大幅缩短，
或前面意外插入了一行日志），OpenClaw 会将损坏文件保留为 `.clobbered.*`，
恢复“最近一次已知正常”副本，并记录恢复原因。
下一次智能体轮次还会收到一条 system-event 警告，这样主智能体就不会盲目改写已恢复的配置。
当候选配置包含诸如 `***` 之类已脱敏的 secret 占位符时，
不会将其提升为“最近一次已知正常”版本。
...
<AccordionGroup>
  <Accordion title="设置渠道（WhatsApp、Telegram、Discord 等）">
    每个渠道在 `channels.<provider>` 下都有各自的配置部分。设置步骤请参见对应的专门渠道页面：
  ...
  <Accordion title="选择并配置模型">
    ...
    - 使用 `openclaw config set agents.defaults.models '<json>' --strict-json --merge` 添加允许列表条目，而不会移除现有模型。若普通替换会移除条目，则除非传入 `--replace`，否则会被拒绝。
    - 模型引用使用 `provider/model` 格式（例如 `anthropic/claude-opus-4-6`）。
    - `agents.defaults.imageMaxDimensionPx` 控制转录/工具图像的缩放上限（默认 `1200`）；在以截图为主的运行中，较低的值通常可减少视觉 token 使用量。
    - 在聊天中切换模型请参见 [Models CLI](/zh-CN/concepts/models)，认证轮换和回退行为请参见 [Model Failover](/zh-CN/concepts/model-failover)。
  ...
  <Accordion title="控制谁可以向 bot 发送消息">
    ...
    各渠道的详细信息请参见[完整参考](/zh-CN/gateway/configuration-reference#dm-and-group-access)。
  ...
  <Accordion title="调整 Gateway 网关渠道健康监控">
    ...
    - 使用 `channels.<provider>.healthMonitor.enabled` 或 `channels.<provider>.accounts.<id>.healthMonitor.enabled`，可仅对单个渠道或账户禁用自动重启，而不禁用全局监控。
  ...
  <Accordion title="配置会话和重置">
    ...
    - 作用域、身份关联和发送策略请参见[会话管理](/zh-CN/concepts/session)。
  ...
  <Accordion title="为官方 iOS 构建启用基于 relay 的推送">
    ...
    - 允许 Gateway 网关通过外部 relay 发送 `push.test`、唤醒 nudges 和重连唤醒。
    - 使用由已配对 iOS 应用转发的注册作用域发送授权。Gateway 网关不需要部署范围的 relay token。
    - 将每个基于 relay 的注册绑定到 iOS 应用配对时所使用的 Gateway 网关身份，因此其他 Gateway 网关无法复用已存储的注册信息。
    - 本地/手动 iOS 构建仍使用直连 APNs。基于 relay 的发送仅适用于通过 relay 注册的官方分发构建。
    ...
    5. Gateway 网关存储 relay 句柄和发送授权，然后将它们用于 `push.test`、唤醒 nudges 和重连唤醒。

    运行注意事项：

    - 如果你将 iOS 应用切换到另一个 Gateway 网关，请重新连接应用，以便它能发布绑定到该 Gateway 网关的新 relay 注册。
  ...
  <Accordion title="设置 heartbeat（定期报到）">
    ...
    - `directPolicy`：`allow`（默认）或 `block`，用于私信风格的 heartbeat 目标
  ...
  <Accordion title="设置 webhook（hooks）">
    ...
    - 除非进行严格限定范围的调试，否则请保持不安全内容绕过 flag 处于禁用状态（`hooks.gmail.allowUnsafeExternalContent`、`hooks.mappings[].allowUnsafeExternalContent`）。
    - 如果你启用了 `hooks.allowRequestSessionKey`，也请设置 `hooks.allowedSessionKeyPrefixes`，以限制调用方可选择的会话键。
    - 对于由 hook 驱动的智能体，优先使用强力的现代模型层级和严格的工具策略（例如尽可能仅允许消息传递并启用沙箱隔离）。
  ...
  <Accordion title="配置多智能体路由">
    ...
    绑定规则和每个智能体的访问配置请参见 [Multi-Agent](/zh-CN/concepts/multi-agent) 和[完整参考](/zh-CN/gateway/configuration-reference#multi-agent-routing)。
  ...
  <Accordion title="将配置拆分为多个文件（`$include`）">
    ...
    - **相对路径**：相对于包含它的文件解析
    - **OpenClaw 自有写入**：当一次写入只变更一个由单文件 include 支持的顶层部分时，
      例如 `plugins: { $include: "./plugins.json5" }`，
      OpenClaw 会更新该被 include 的文件，并保持 `openclaw.json` 不变
    - **不支持的透写**：对于根 include、include 数组以及带有同级覆盖的 include，
      OpenClaw 自有写入会以失败关闭的方式处理，而不是
      将配置拍平
    - **错误处理**：对于缺失文件、解析错误和循环 include，会提供清晰错误信息
...
直接文件编辑在通过校验前会被视为不可信。监视器会等待
编辑器的临时写入/重命名抖动稳定下来，读取最终文件，并通过恢复
“最近一次已知正常”配置来拒绝无效的外部编辑。OpenClaw 自有的
配置写入在写入前也会经过相同的 schema 门控；诸如删除 `gateway.mode`
或将文件缩小超过一半之类的破坏性覆盖会被拒绝，
并保存为 `.rejected.*` 以供检查。
...
<Note>
`gateway.reload` 和 `gateway.remote` 属于例外——更改它们**不会**触发重启。
</Note>

### 重载规划

当你编辑一个通过 `$include` 引用的源文件时，OpenClaw 会根据源作者编写的布局来规划
重载，而不是根据拍平后的内存视图。
这样即使某个顶层部分单独存在于自己的 include 文件中，
例如 `plugins: { $include: "./plugins.json5" }`，
热重载决策（热应用还是重启）也能保持可预测。
...
- `config.patch`：执行部分更新（JSON merge patch：对象合并、`null` 删除、数组替换）
- `config.apply`：仅当你打算替换整个配置时使用
- `update.run`：用于显式自更新并重启

<Note>
控制平面写入（`config.apply`、`config.patch`、`update.run`）会按每个 `deviceId+clientIp`
进行速率限制：每 60 秒最多 3 个请求。
...
这两个文件都不会覆盖现有环境变量。你也可以在配置中内联设置环境变量：
...
<Accordion title="导入 shell 环境变量（可选）">
  如果启用，且预期键名未设置，OpenClaw 会运行你的登录 shell，并仅导入缺失的键：
...
SecretRef 详情（包括用于 `env`/`file`/`exec` 的 `secrets.providers`）请参见[Secrets Management](/zh-CN/gateway/secrets)。
支持的凭证路径列在 [SecretRef Credential Surface](/zh-CN/reference/secretref-credential-surface) 中。
</Accordion>

完整优先级和来源请参见[环境变量](/zh-CN/help/environment)。

## 完整参考

如需完整的逐字段参考，请参见 **[Configuration Reference](/zh-CN/gateway/configuration-reference)**。

---

_相关内容：[配置示例](/zh-CN/gateway/configuration-examples) · [Configuration Reference](/zh-CN/gateway/configuration-reference) · [Doctor](/zh-CN/gateway/doctor)_
