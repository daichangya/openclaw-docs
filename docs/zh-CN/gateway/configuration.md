...
<Tabs>
  ...
  <Tab title="控制 UI">
    ...
    控制 UI 会根据实时配置 schema 渲染一个表单，其中包括字段
    `title` / `description` 文档元数据，以及可用时的插件和渠道 schema，
    同时还提供一个 **Raw JSON** 编辑器作为兜底入口。对于下钻式
    UI 和其他工具，Gateway 网关 还会公开 `config.schema.lookup`，
...
- `openclaw config schema` 会输出与控制 UI 和配置校验使用的相同 JSON Schema 系列。
- 将该 schema 输出视为 `openclaw.json` 的规范机器可读契约；本概览和配置参考只是对其进行总结。
- 字段 `title` 和 `description` 的值会被带入 schema 输出，以供编辑器和表单工具使用。
- 嵌套对象、通配符（`*`）和数组项（`[]`）条目在存在匹配字段文档时，会继承相同的文档元数据。
- `anyOf` / `oneOf` / `allOf` 组合分支也会继承相同的文档元数据，因此联合 / 交叉变体会保留相同的字段帮助信息。
- `config.schema.lookup` 会返回一个标准化配置路径，以及一个浅层 schema 节点（`title`、`description`、`type`、`enum`、`const`、常见边界值和类似校验字段）、匹配的 UI 提示元数据，以及供下钻工具使用的直接子项摘要。
- 当 Gateway 网关 能加载当前清单注册表时，会合并运行时插件 / 渠道 schema。
...
Gateway 网关 在成功启动后还会保留一份受信任的“最近一次正常配置”副本。如果
`openclaw.json` 之后在 OpenClaw 外部被修改且不再通过校验，启动
和热重载会将损坏的文件保留为带时间戳的 `.clobbered.*` 快照，
恢复最近一次正常配置副本，并记录一条醒目的警告，说明恢复原因。
下一个主智能体回合也会收到一条系统事件警告，告知它
配置已被恢复，不能盲目重写。最近一次正常配置的提升
会在校验通过的启动后，以及接受的热重载后更新，其中也包括
由 OpenClaw 自身执行的配置写入，只要持久化文件的哈希仍与已接受的
写入匹配即可。当候选配置包含已脱敏的密钥
占位符（如 `***`）或缩短后的令牌值时，将跳过提升。
...
<AccordionGroup>
  ...
  <Accordion title="选择并配置模型">
    ...
    - `agents.defaults.imageMaxDimensionPx` 控制转录 / 工具图像的下采样（默认值为 `1200`）；在截图较多的运行中，较低的值通常可以减少视觉 token 使用量。
  ...
  <Accordion title="控制谁可以给机器人发消息">
    ...
    - `"pairing"`（默认）：未知发送者会收到一个一次性配对码用于批准
    ...
    参见[完整参考](/zh-CN/gateway/configuration-reference#dm-and-group-access)了解各渠道的详细信息。
  ...
  <Accordion title="设置群聊提及门控">
    ...
    - 参见[完整参考](/zh-CN/gateway/configuration-reference#group-chat-mention-gating)了解每个渠道的覆盖项和自聊模式。
  ...
  <Accordion title="调整 Gateway 网关 渠道健康监控">
    控制 Gateway 网关 以多激进的方式重启看起来已经停滞的渠道：
  ...
  <Accordion title="启用沙箱隔离">
    ...
    参见[沙箱隔离](/zh-CN/gateway/sandboxing)了解完整指南，参见[完整参考](/zh-CN/gateway/configuration-reference#agentsdefaultssandbox)了解所有选项。
  ...
  <Accordion title="为官方 iOS 构建启用基于 relay 的推送">
    ...
    - 将每个基于 relay 的注册绑定到 iOS 应用配对的 Gateway 网关 身份，因此其他 Gateway 网关 无法复用已存储的注册信息。
    ...
    4. iOS 应用获取 Gateway 网关 身份，使用 App Attest 加上应用收据向 relay 注册，然后将基于 relay 的 `push.apns.register` 负载发布到已配对的 Gateway 网关。
    ...
    - `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true` 仍然是仅限 loopback 的开发逃生开关；不要在配置中持久化 HTTP relay URL。

    参见 [iOS App](/zh-CN/platforms/ios#relay-backed-push-for-official-builds) 了解端到端流程，参见[认证与信任流程](/zh-CN/platforms/ios#authentication-and-trust-flow)了解 relay 安全模型。
  ...
  <Accordion title="设置 heartbeat（周期性签到）">
    ...
    - 参见 [Heartbeat](/zh-CN/gateway/heartbeat) 了解完整指南。
  ...
  <Accordion title="配置 cron 作业">
    ...
    - 参见[cron 作业](/zh-CN/automation/cron-jobs)了解功能概览和 CLI 示例。
  ...
  <Accordion title="配置多智能体路由">
    ...
    参见[多智能体](/zh-CN/concepts/multi-agent)和[完整参考](/zh-CN/gateway/configuration-reference#multi-agent-routing)了解绑定规则和每个智能体的访问配置文件。
...
最近一次正常配置来拒绝无效的外部编辑。由 OpenClaw 自身执行的
配置写入在写入前也会经过同样的 schema 关卡；像删除 `gateway.mode`
或将文件缩小超过一半这样的破坏性覆盖会被拒绝，并保存为 `.rejected.*`
以供检查。
...
<Note>
控制平面写入 RPC（`config.apply`、`config.patch`、`update.run`）按每个 `deviceId+clientIp` 进行速率限制：**每 60 秒 3 个请求**。触发限制时，RPC 会返回带有 `retryAfterMs` 的 `UNAVAILABLE`。
</Note>

安全 / 默认流程：

- `config.schema.lookup`：检查一个按路径限定的配置子树，包含一个浅层
...
如果你不是替换整份配置，优先使用 `config.schema.lookup`
然后再用 `config.patch`。

<AccordionGroup>
  <Accordion title="config.apply（完整替换）">
    在一步中校验 + 写入完整配置，并重启 Gateway 网关。
    ...
    - `baseHash`（可选）—— 来自 `config.get` 的配置 hash（当配置存在时为必填）
    - `sessionKey`（可选）—— 重启后唤醒 ping 使用的会话键
    - `note`（可选）—— 重启哨兵的备注
    - `restartDelayMs`（可选）—— 重启前延迟（默认 2000）

    当某个重启已处于待处理 / 进行中状态时，重启请求会被合并，并且两次重启周期之间会有 30 秒冷却时间。
  ...
  <Accordion title="config.patch（部分更新）">
    ...
    - `raw`（string）—— 仅包含需要变更键名的 JSON5
    - `baseHash`（必填）—— 来自 `config.get` 的配置 hash
    - `sessionKey`、`note`、`restartDelayMs` —— 与 `config.apply` 相同

    重启行为与 `config.apply` 一致：会合并待处理的重启请求，并在两次重启周期之间施加 30 秒冷却时间。
...
SecretRef 的详细信息（包括用于 `env` / `file` / `exec` 的 `secrets.providers`）请参见[密钥管理](/zh-CN/gateway/secrets)。
支持的凭证路径列在 [SecretRef 凭证表面](/zh-CN/reference/secretref-credential-surface)。
</Accordion>
