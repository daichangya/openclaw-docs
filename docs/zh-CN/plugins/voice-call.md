...
title: Voice call 插件
x-i18n:
    generated_at: "2026-04-24T09:19:16Z"
    model: gpt-5.4
    provider: openai
...
- `streaming.maxPendingConnections` 限制未认证、启动前套接字的总数。
- `streaming.maxPendingConnectionsPerIp` 限制每个源 IP 的未认证、启动前套接字数量。
- `streaming.maxConnections` 限制打开的媒体流套接字总数（等待中 + 活跃中）。
- 运行时回退当前仍接受这些旧的 voice-call 键，但重写路径是 `openclaw doctor --fix`，兼容垫片只是临时方案。
...
- 内置的实时语音提供商包括 Google Gemini Live（`google`）和 OpenAI（`openai`），由各自的提供商插件注册。
...
请参阅 [Google provider](/zh-CN/providers/google) 和 [OpenAI provider](/zh-CN/providers/openai)，了解提供商特定的实时语音选项。
...
- 如果 `streaming.provider` 指向未注册的提供商，或者根本没有注册任何实时转录提供商，Voice Call 会记录一条警告，并跳过媒体流式传输，而不是让整个插件失败。
...
当前端有代理或 tunnel 位于 Gateway 网关之前时，插件会重建
用于签名校验的公开 URL。这些选项用于控制哪些转发头
会被信任。

`webhookSecurity.allowedHosts` 会对转发头中的主机进行白名单限制。

`webhookSecurity.trustForwardingHeaders` 会在没有白名单的情况下信任转发头。

`webhookSecurity.trustedProxyIPs` 仅在请求的远端 IP
与列表匹配时，才信任转发头。
...
流式语音。你可以在插件配置下用**相同的结构**
对其进行覆盖——它会与 `messages.tts` 深度合并。
...
- 插件配置中的旧版 `tts.<provider>` 键（`openai`、`elevenlabs`、`microsoft`、`edge`）会在加载时自动迁移到 `tts.providers.<provider>`。在提交的配置中，优先使用 `providers` 结构。
- **Microsoft speech 会被语音通话忽略**（电话音频需要 PCM；当前的 Microsoft 传输层不暴露电话用 PCM 输出）。
- 启用 Twilio 媒体流时，会使用核心 TTS；否则通话会回退到提供商原生语音。
- 如果 Twilio 媒体流已经处于活动状态，Voice Call 不会回退到 TwiML `<Say>`。如果在该状态下电话 TTS 不可用，则播放请求会失败，而不是混用两条播放路径。
- 当电话 TTS 回退到次级提供商时，Voice Call 会记录一条带有提供商链（`from`、`to`、`attempts`）的警告日志，便于调试。
...
```json5
{
  ...
  inboundGreeting: "你好！我可以怎么帮你？",
}
```

`inboundPolicy: "allowlist"` 是一种低保障的来电号码筛选机制。插件会
对提供商提供的 `From` 值进行规范化，并将其与 `allowFrom` 进行比较。
Webhook 校验可以验证提供商投递和负载完整性，但
无法证明 PSTN / VoIP 来电号码的归属权。请将 `allowFrom` 视为
来电显示过滤，而不是强来电身份认证。
...
对于自动响应，Voice Call 会向 system prompt 追加一个严格的语音输出约定：
...
- 如果初始播放失败，通话会返回到 `listening`，初始消息会保留在队列中等待重试。
- 对于 Twilio 流式传输，初始播放会在流连接时立即开始，不会有额外延迟。
...
- 如果宽限期结束后仍没有重新注册流，则会结束通话，以防止通话卡在活动状态。
...
`--file <path>` 可指向其他日志，使用 `--last <n>` 可将分析限制为
...
此仓库提供了对应的 skill 文档，位于 `skills/voice-call/SKILL.md`。
