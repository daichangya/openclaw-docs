...
title: 插件 SDK 迁移
x-i18n:
    generated_at: "2026-04-23T20:57:35Z"
    model: gpt-5.4
    provider: openai
...
架构，后者使用聚焦且有文档说明的导入路径。如果你的插件是在新架构之前构建的，
本指南可以帮助你完成迁移。

## 正在发生什么变化

旧插件系统提供了两个开放度很高的表面，让插件可以从单一入口点
导入所需的任意内容：

- **`openclaw/plugin-sdk/compat`** —— 一个单一导入路径，会重新导出几十个
  辅助工具。它最初是为了在新插件架构构建期间，让较旧的基于 hook 的插件继续工作而引入的。
- **`openclaw/extension-api`** —— 一个桥接层，让插件可以直接访问
  宿主机侧辅助工具，例如嵌入式智能体运行器。

这两个表面现在都已**弃用**。它们在运行时仍然可用，但新
插件不得再使用它们，而现有插件应在下一个
major 版本移除它们之前完成迁移。

<Warning>
  这个向后兼容层将在未来的 major 版本中被移除。
  到那时仍然从这些表面导入的插件将会中断。
</Warning>
...
- **启动慢** —— 导入一个辅助工具会连带加载几十个无关模块
- **循环依赖** —— 宽泛的重新导出使得创建导入循环变得很容易
- **API 表面不清晰** —— 无法判断哪些导出是稳定的，哪些只是内部实现

现代插件 SDK 解决了这些问题：每个导入路径（`openclaw/plugin-sdk/\<subpath\>`）
都是一个小型、独立且职责明确的模块，并带有文档化契约。

面向内置渠道的旧版 provider 便利接缝也已移除。诸如
`openclaw/plugin-sdk/slack`、`openclaw/plugin-sdk/discord`、
`openclaw/plugin-sdk/signal`、`openclaw/plugin-sdk/whatsapp`、
带有渠道品牌的 helper 接缝，以及
`openclaw/plugin-sdk/telegram-core` 之类的导入，都是私有 monorepo 快捷路径，
而不是稳定的插件契约。请改用狭窄的通用 SDK 子路径。在内置
插件工作区内部，请将 provider 自有辅助工具保留在该插件自己的
`api.ts` 或 `runtime-api.ts` 中。

当前内置 provider 示例：

- Anthropic 将 Claude 特有的流式辅助工具保留在自己的 `api.ts` /
  `contract-api.ts` 接缝中
- OpenAI 将 provider 构建器、默认模型辅助工具和实时 provider
  构建器保留在自己的 `api.ts` 中
- OpenRouter 将 provider 构建器以及新手引导/配置辅助工具保留在自己的
  `api.ts` 中

## 如何迁移

<Steps>
  <Step title="将 approval-native handlers 迁移为能力事实">
    支持 approval 的渠道插件现在通过
    `approvalCapability.nativeRuntime` 以及共享运行时上下文注册表
    来暴露原生 approval 行为。
    ...
    - 将 approval 特定的 auth/delivery 从旧版 `plugin.auth` /
      `plugin.approvals` 绑定迁移到 `approvalCapability`
    - `ChannelPlugin.approvals` 已从公共 channel-plugin
      契约中移除；请将 delivery/native/render 字段迁移到 `approvalCapability`
    - `plugin.auth` 仍保留用于渠道 login/logout 流程；核心不再读取其中的 approval auth
      hooks
    - 通过 `openclaw/plugin-sdk/channel-runtime-context`
      注册渠道自有的运行时对象，例如 clients、tokens 或 Bolt
      apps
    - 不要从原生 approval handlers 发送插件自有的 reroute 通知；
      核心现在会根据真实投递结果来负责 routed-elsewhere 通知
    - 当将 `channelRuntime` 传入 `createChannelManager(...)` 时，请提供一个真实的
      `createPluginRuntime().channel` 表面。部分 stub 会被拒绝。

    当前 approval capability
    布局请参见 `/plugins/sdk-channel-plugins`。

  </Step>

  <Step title="审计 Windows wrapper 回退行为">
    如果你的插件使用 `openclaw/plugin-sdk/windows-spawn`，
    在 Windows 上，未解析的 `.cmd`/`.bat` wrapper 现在会以失败关闭方式处理，除非你显式传入
  ...
  <Step title="替换为聚焦导入">
    ...
    对于宿主机侧辅助工具，请使用注入的插件运行时，而不是直接导入：
...
## 导入路径参考
  ...
  | `plugin-sdk/core` | 面向渠道入口定义/构建器的旧版聚合重导出 | `defineChannelPluginEntry`, `createChatChannelPlugin` |
  ...
  | `plugin-sdk/account-helpers` | 精简账户辅助工具 | 账户列表/账户动作辅助工具 |
  ...
  | `plugin-sdk/channel-lifecycle` | 账户状态和草稿流生命周期辅助工具 | `createAccountStatusSink`、草稿预览收尾辅助工具 |
  ...
  | `plugin-sdk/approval-auth-runtime` | Approval 认证辅助工具 | approver 解析、同聊天动作认证 |
  | `plugin-sdk/approval-client-runtime` | Approval 客户端辅助工具 | 原生 exec approval profile/filter 辅助工具 |
  | `plugin-sdk/approval-delivery-runtime` | Approval 投递辅助工具 | 原生 approval capability/delivery 适配器 |
  | `plugin-sdk/approval-gateway-runtime` | Approval Gateway 网关辅助工具 | 共享 approval gateway 解析辅助工具 |
  | `plugin-sdk/approval-handler-adapter-runtime` | Approval 适配器辅助工具 | 面向高热渠道入口点的轻量级原生 approval 适配器加载辅助工具 |
  | `plugin-sdk/approval-handler-runtime` | Approval handler 辅助工具 | 更宽泛的 approval handler 运行时辅助工具；如果更精简的 adapter/gateway 接缝已足够，应优先使用后者 |
  ...
  | `plugin-sdk/reply-dispatch-runtime` | 精简回复分发辅助工具 | finalize + provider 分发辅助工具 |
  ...
  | `plugin-sdk/provider-setup` | 面向本地/自托管 provider 的精选设置辅助工具 | 自托管 provider 发现/配置辅助工具 |
  ...
  | `plugin-sdk/provider-model-shared` | 共享 provider 模型/重放辅助工具 | `ProviderReplayFamily`, `buildProviderReplayFamilyHooks`, `normalizeModelCompat`、共享重放策略构建器、provider 端点辅助工具以及模型 id 标准化辅助工具 |
...
| `plugin-sdk/provider-web-search-contract` | provider web 搜索契约辅助工具 | 精简 web 搜索配置/凭证契约辅助工具，例如 `createWebSearchProviderContractFields`、`enablePluginInConfig`、`resolveProviderWebSearchPluginConfig` 以及按作用域划分的凭证 setter/getter |
...
| `plugin-sdk/media-runtime` | 共享媒体辅助工具 | 媒体抓取/转换/存储辅助工具以及媒体负载构建器 |
...
| `plugin-sdk/channel-config-primitives` | 渠道配置基础原语 | 精简渠道配置 schema 原语 |
...
| `plugin-sdk/direct-dm` | 私信直达辅助工具 | 共享私信直达认证/守卫辅助工具 |
| `plugin-sdk/extension-shared` | 共享扩展辅助工具 | 被动渠道/状态与环境代理辅助原语 |
...
| `plugin-sdk/zod` | Zod 重导出 | 面向插件 SDK 使用者的 `zod` 重导出 |
...
| `plugin-sdk/memory-core-host-engine-embeddings` | Memory 宿主嵌入引擎 | Memory 嵌入契约、注册表访问、本地 provider 以及通用批处理/远程辅助工具；具体远程 providers 位于各自所属插件中 |
...
| `plugin-sdk/memory-host-search` | 主动 Memory 搜索门面 | 惰性主动 Memory 搜索管理器运行时门面 |
...
用于内置插件维护和兼容性，但它们被有意地
排除在常见迁移表之外，也不是新插件代码的推荐目标。
...
- IRC：`plugin-sdk/irc*`
- 内置 helper/plugin 表面，例如 `plugin-sdk/googlechat`、
...
`plugin-sdk/github-copilot-token` 当前暴露的是精简 token 辅助工具表面：
...
| ---------------------- | ----------------------------------------------------------------------- |
| **现在** | 已弃用表面会发出运行时警告 |
| **下一个 major 版本** | 已弃用表面将被移除；仍在使用它们的插件将会失败 |
