...
- 避免在 Skills 正文中重新记录整个 infer 的完整功能面
...
- 将模型、图像、音频转录、TTS、视频、网页和嵌入工作流统一放在一个命令树下。
- 为脚本、自动化和智能体驱动的工作流使用稳定的 `--json` 输出结构。
- 当任务本质上是“运行推理”时，优先使用 OpenClaw 的第一方入口。
- 对于大多数 infer 命令，使用常规本地路径而无需依赖 Gateway 网关。

对于端到端的提供商检查，在较底层的提供商测试通过后，优先使用 `openclaw infer ...`。它会在发出提供商请求之前，覆盖已发布的 CLI、配置加载、默认智能体解析、内置插件激活、运行时依赖修复，以及共享能力运行时。
...
- 对于 `image describe`，显式传入 `--model` 会直接运行该提供商/模型。该模型必须在模型目录或提供商配置中支持图像能力。`codex/<model>` 会运行一个受限的 Codex harness app-server 图像理解轮次；`openai-codex/<model>` 使用 OpenAI Codex OAuth 提供商路径。
- 无状态执行命令默认使用本地。
- Gateway 网关管理状态的命令默认使用 gateway。
- 常规本地路径不要求 Gateway 网关处于运行状态。
- `model run` 是一次性执行。通过该命令中的智能体运行时打开的 MCP 服务器会在回复后被回收，无论是本地执行还是 `--gateway` 执行，因此重复脚本调用不会让 stdio MCP 子进程持续存活。
...
- 因为 `model run` 面向无头自动化，所以在命令结束后，它不会保留该命令会话的内置 MCP 运行时。
...
- 从已有输入文件开始时，使用 `image edit`。
- 使用 `image providers --json` 来验证哪些内置图像提供商可被发现、已配置、已选中，以及每个提供商暴露了哪些生成/编辑能力。
- 使用 `image generate --model <provider/model> --json` 作为图像生成变更最窄范围的在线 CLI 冒烟检查。示例：
...
- `tts status` 默认使用 gateway，因为它反映的是 Gateway 网关管理的 TTS 状态。
...
- `video describe` 的 `--model` 必须是 `<provider/model>`。
...
对于生成媒体的命令，`outputs` 包含 OpenClaw 写入的文件。对于自动化，请使用该数组中的 `path`、`mimeType`、`size` 以及任何媒体特有尺寸信息，而不要解析人类可读的 stdout。
