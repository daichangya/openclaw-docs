...
read_when:
    - 添加或修改 CLI 命令或选项
    - 为新的命令界面编写文档
summary: OpenClaw CLI 关于 `openclaw` 命令、子命令和选项的参考文档
title: CLI 参考文档
x-i18n:
...
- [`daemon`](/zh-CN/cli/daemon)（Gateway 网关服务命令的旧版别名）
- [`clawbot`](/zh-CN/cli/clawbot)（旧版别名命名空间）
- [`voicecall`](/zh-CN/cli/voicecall)（插件；如果已安装）

## 全局标志

- `--dev`：将状态隔离在 `~/.openclaw-dev` 下，并调整默认端口。
- `--profile <name>`：将状态隔离在 `~/.openclaw-<name>` 下。
- `--container <name>`：为执行目标指定一个命名容器。
- `--no-color`：禁用 ANSI 颜色。
...
- `--json`（以及在受支持时的 `--plain`）会禁用样式，以获得干净的输出。
...
配色的唯一事实来源：`src/terminal/palette.ts`（“lobster 配色方案”）。
...
- `openclaw security audit` — 审核配置和本地状态中常见的安全隐患。
...
- `reload` 是一个 Gateway 网关 RPC，在解析失败时会保留最后一个已知正常的运行时快照。
- `audit --check` 在发现问题时会返回非零值；未解析的引用会使用更高优先级的非零退出码。
- 默认会跳过 dry-run exec 检查；使用 `--allow-exec` 选择启用。
...
大多数插件更改都需要重启 Gateway 网关。参见 [/plugin](/zh-CN/tools/plugin)。

## Memory

对 `MEMORY.md` + `memory/*.md` 执行向量搜索：

- `openclaw memory status` — 显示索引统计信息；使用 `--deep` 执行向量和嵌入就绪性检查，或使用 `--fix` 修复过期的 recall/promotion 工件。
- `openclaw memory index` — 重新索引 memory 文件。
- `openclaw memory search "<query>"`（或 `--query "<query>"`）— 对 memory 执行语义搜索。
- `openclaw memory promote` — 对短期 recalls 进行排序，并可选择将前几项追加到 `MEMORY.md`。
...
- `sandbox recreate` 会移除现有运行时，以便下次使用时根据当前配置重新初始化。
...
- `/status` 用于快速诊断。
- `/trace` 用于显示会话范围内的插件 trace/debug 行。
- `/config` 用于持久化配置更改。
- `/debug` 用于仅运行时的配置覆盖（存储在内存中，而不是磁盘；需要 `commands.debug: true`）。
...
- 如果不使用 `--install` 或 `--write-state`，`completion` 会将脚本输出到 stdout。
- `--install` 会将一个 `OpenClaw Completion` 代码块写入你的 shell 配置文件，并使其指向 OpenClaw 状态目录下缓存的脚本。
...
存在任何新手引导标志时（`--non-interactive`、`--mode`、`--remote-url`、`--remote-token`），都会自动运行新手引导。
...
- `--reset-scope <config|config+creds+sessions|full>`（默认值为 `config+creds+sessions`；使用 `full` 还会移除工作区）
- `--non-interactive`
- `--mode <local|remote>`
- `--flow <quickstart|advanced|manual>`（`manual` 是 `advanced` 的别名）
- `--auth-choice <choice>`，其中 `<choice>` 必须是以下之一：
...
- Qwen 说明：`qwen-*` 是规范的 `auth-choice` 系列。`modelstudio-*`
  id 仍然会被接受，但仅作为向后兼容的旧版别名。
- `--secret-input-mode <plaintext|ref>`（默认值为 `plaintext`；使用 `ref` 可将提供商默认环境变量引用存储为 SecretRef，而不是明文密钥）
...
- `config get <path>`：打印一个配置值（点路径/方括号路径）。
- `config set`：支持四种赋值模式：
...
- `config set --allow-exec --dry-run`：选择启用 exec SecretRef dry-run 检查（可能会执行 provider 命令）。
- `config set --dry-run --json`：输出机器可读的 dry-run 结果（检查 + 完整性信号、操作、已检查/已跳过的 refs、错误）。
- `config set --strict-json`：要求对路径/值输入执行 JSON5 解析。在 dry-run 输出模式之外，`--json` 仍是严格解析的旧版别名。
- `config unset <path>`：移除一个值。
- `config file`：打印当前激活的配置文件路径。
- `config schema`：打印为 `openclaw.json` 生成的 JSON schema，其中包括在嵌套对象、通配符、数组项和组合分支间传播的字段 `title` / `description` 文档元数据，以及尽力提供的实时插件/渠道 schema 元数据。
...
- `--force`：即使并非严格需要，也强制执行修复。
- `--generate-gateway-token`：生成新的 Gateway 网关认证令牌。

### `dashboard`

使用你当前的令牌打开 Control UI。
...
- `channels status`：检查 Gateway 网关可达性和渠道健康状态（`--probe` 会在 Gateway 网关可达时按账号执行实时 probe/audit 检查；如果不可达，则回退为仅基于配置的渠道摘要。更广泛的 Gateway 网关健康探测请使用 `openclaw health` 或 `openclaw status --deep`）。
- 提示：当 `channels status` 能检测到常见配置错误时，会打印带有建议修复方式的警告（然后引导你使用 `openclaw doctor`）。
- `channels logs`：显示 Gateway 网关日志文件中的最近渠道日志。
- `channels add`：未传入任何标志时使用向导式设置；传入标志后切换为非交互模式。
  - 当向仍在使用单账号顶层配置的渠道中添加非默认账号时，OpenClaw 会先将账号范围的值提升到该渠道的账号映射中，然后再写入新账号。大多数渠道使用 `accounts.default`；Matrix 则可以保留现有匹配的命名/默认目标。
  - 非交互式 `channels add` 不会自动创建/升级绑定；仅渠道级绑定将继续匹配默认账号。
- `channels remove`：默认仅禁用；传入 `--delete` 可在不提示的情况下删除配置项。
- `channels login`：交互式渠道登录（仅限 WhatsApp Web）。
- `channels logout`：退出渠道会话登录（如果支持）。
...
- `--no-usage`：跳过模型提供商使用量/配额快照（仅限基于 OAuth/API 的渠道）。
...
- `channels status --probe` 可根据渠道支持情况显示传输状态，以及 `works`、`probe failed`、`audit ok` 或 `audit failed` 等 probe/audit 结果。
...
查找提供目录界面的渠道中的自身、对等方和群组 ID。参见 [`openclaw directory`](/zh-CN/cli/directory)。
...
提示：对于由 ClawHub 提供支持的 Skills，请使用 `openclaw skills search`、`openclaw skills install` 和 `openclaw skills update`。
...
- `list` 和 `approve` 都支持在多账号渠道中使用 `--account <id>`。
...
- `devices approve` 在铸造令牌前需要显式的请求 ID；省略 `requestId` 或传入 `--latest` 时，只会预览最新的待处理请求。
- 已存储令牌的重连会重用该令牌缓存的已批准 scopes；显式执行
  `devices rotate --scope ...` 会更新这组已存储的 scopes，以供将来
  使用缓存令牌重连时复用。
...
- `--token` 和 `--password` 互斥。
- 设置代码携带的是短时有效的 bootstrap 令牌，而不是共享的 Gateway 网关令牌/密码。
- 内置的 bootstrap 交接会将主节点令牌保持为 `scopes: []`。
- 任何已交接的 operator bootstrap 令牌都会被限制在 `operator.approvals`、`operator.read`、`operator.talk.secrets` 和 `operator.write` 范围内。
- Bootstrap scope 检查带有角色前缀，因此该 operator allowlist 只满足 operator 请求；非 operator 角色仍需要其自身角色前缀下的 scopes。
...
旧版别名命名空间。目前支持 `openclaw clawbot qr`，它映射到 [`openclaw qr`](/zh-CN/cli/qr)。
...
- `hooks install` 和 `hooks update` 仍可作为兼容性别名使用，但会打印弃用警告，并转发到插件命令。

### `webhooks`

Webhook 辅助命令。当前内置界面为 Gmail Pub/Sub 设置 + 运行器：
...
- `webhooks gmail run`（对同一组标志提供运行时覆盖）

说明：

- `setup` 会配置 Gmail watch 以及面向 OpenClaw 的 push 路径。
- `run` 会启动本地 Gmail watcher/renew 循环，并可选择使用运行时覆盖。

### `dns`

广域发现 DNS 辅助命令（CoreDNS + Tailscale）。当前内置界面：
...
- 不使用 `--apply` 时，这是一个规划辅助命令，会打印推荐的 OpenClaw + Tailscale DNS 配置。
- `--apply` 目前仅支持在 macOS 上配合 Homebrew CoreDNS 使用。
...
通过 Gateway 网关运行一次智能体轮次（或使用 `--local` 嵌入式运行）。
...
- Gateway 网关模式会在 Gateway 网关请求失败时回退到嵌入式智能体。
- `--local` 仍会预加载插件注册表，因此在嵌入式运行期间，插件提供的 providers、工具和渠道仍然可用。
...
绑定规范使用 `channel[:accountId]`。省略 `accountId` 时，OpenClaw 可能会通过渠道默认值/插件 hooks 解析账号范围；否则它将成为不带显式账号范围的渠道绑定。
传入任何显式的 add 标志都会使该命令切换到非交互路径。`main` 是保留字，不能用作新智能体 id。
...
- 如果未提供任何显式身份字段，该命令会读取 `IDENTITY.md`。
...
- `--json`
- `--all`（完整诊断；只读，可直接粘贴）
- `--deep`（请求 Gateway 网关执行实时健康探测，在支持时包括渠道探测）
- `--usage`（显示模型提供商使用量/配额）
- `--timeout <ms>`
- `--verbose`
- `--debug`（`--verbose` 的别名）
...
- 概览会在可用时包含 Gateway 网关 + 节点主机服务状态。
- `--usage` 会将标准化后的提供商使用窗口打印为 `X% left`。

### 使用量跟踪

当 OAuth/API 凭证可用时，OpenClaw 可以显示提供商的使用量/配额。

显示位置：

- `/status`（在可用时添加一行简短的提供商使用量信息）
- `openclaw status --usage`（打印完整的提供商细分）
- macOS 菜单栏（Context 下的 Usage 部分）

说明：

- 数据直接来自提供商使用量端点（不是估算值）。
- 面向人的输出会统一标准化为 `X% left`。
- 当前具有使用窗口的提供商包括：Anthropic、GitHub Copilot、Gemini CLI、OpenAI Codex、MiniMax、Xiaomi 和 z.ai。
- MiniMax 说明：原始 `usage_percent` / `usagePercent` 表示剩余配额，因此 OpenClaw 会在显示前对其取反；如果存在基于计数的字段，则仍优先使用。对于 `model_remains` 响应，会优先选择 chat-model 条目，在需要时根据时间戳推导窗口标签，并在 plan 标签中包含模型名称。
- 使用量认证会在可用时来自提供商专用 hooks；否则 OpenClaw 会回退为从认证配置文件、环境变量或配置中匹配 OAuth/API-key 凭证。如果都无法解析，则隐藏使用量。
...
- 默认的 `health` 可以返回一个新的已缓存 Gateway 网关快照。
- `health --verbose` 会强制执行实时探测，并在面向人的输出中展开所有已配置账号和智能体的详情。
...
- `sessions cleanup` 也支持 `--fix-missing`，用于清理其转录文件已丢失的条目。
...
- `--non-interactive` 需要 `--yes` 以及显式范围（或 `--all`）。
...
- `tasks maintenance [--apply] [--json]` — 预览或应用 tasks 和 TaskFlow 清理/对账（ACP/subagent 子会话、活动 cron 作业、实时 CLI 运行）
- `tasks flow list` — 列出活动中和最近的 Task Flow flows
- `tasks flow show <lookup>` — 按 id 或查找键检查一个 flow
...
- `gateway status` 支持 `--no-probe`、`--deep`、`--require-rpc` 和 `--json` 以便脚本调用。
- `gateway status` 还会在能够检测到时显示旧版或额外的 Gateway 网关服务（`--deep` 会增加系统级扫描）。使用 profile 命名的 OpenClaw 服务会被视为一等公民，不会被标记为“额外”。
...
- `gateway install|uninstall|start|stop|restart` 支持 `--json` 以便脚本调用（默认输出仍保持为便于人类阅读的格式）。
- `gateway install` 默认使用 Node 运行时；**不推荐**使用 bun（存在 WhatsApp/Telegram bug）。
- `gateway install` 选项：`--port`、`--runtime`、`--token`、`--force`、`--json`。

### `daemon`

Gateway 网关服务管理命令的旧版别名。参见 [/cli/daemon](/zh-CN/cli/daemon)。
...
- 如果你传入 `--url`，CLI 不会自动应用配置或环境凭证。
- local loopback 配对失败时会回退到已配置的本地日志文件；显式 `--url` 目标不会这样做。

### `gateway <subcommand>`

Gateway 网关 CLI 辅助命令（对 RPC 子命令使用 `--url`、`--token`、`--password`、`--timeout`、`--expect-final`）。
...
- `gateway status --deep` 会增加系统级服务扫描。更深入的运行时探测详情，请使用 `gateway probe`、
  `health --verbose` 或顶级 `status --deep`。

常见 RPC：

- `config.schema.lookup`（检查一个配置子树，返回浅层 schema 节点、匹配的 hint 元数据以及直接子项摘要）
- `config.get`（读取当前配置快照 + hash）
- `config.set`（验证并写入完整配置；使用 `baseHash` 实现乐观并发控制）
- `config.apply`（验证并写入配置 + 重启 + 唤醒）
...
提示：这些配置写入 RPC 会预检提交的配置负载中 refs 的活动 SecretRef 解析状态；如果提交的某个实际生效 ref 未解析，则会拒绝写入。
提示：仅限所有者的 `gateway` 运行时工具仍然拒绝重写 `tools.exec.ask` 或 `tools.exec.security`；旧版 `tools.bash.*` 别名会规范化到相同的受保护 exec 路径。
...
再次被允许，因此除非 Anthropic 发布新政策，否则 OpenClaw 将 Claude CLI 复用和
`claude -p` 用法视为此集成的许可方式。对于生产环境，建议优先使用 Anthropic API key
或其他受支持的订阅式提供商，例如 OpenAI Codex、Alibaba Cloud Model Studio
Coding Plan、MiniMax Coding Plan 或 Z.AI / GLM Coding Plan。

Anthropic setup-token 仍然作为受支持的令牌认证路径可用，但 OpenClaw 现在在可用时更倾向于 Claude CLI 复用和 `claude -p`。
...
`--all` 会在配置认证之前包含由内置 provider 拥有的静态目录条目。
这些条目在存在匹配的 provider 凭证之前仍不可用。
...
始终包含认证概览以及 auth store 中配置文件的 OAuth 过期状态。
`--probe` 会运行实时请求（可能消耗令牌并触发速率限制）。
Probe 行可能来自认证配置文件、环境凭证或 `models.json`。
你可能会看到诸如 `ok`、`auth`、`rate_limit`、`billing`、`timeout`、
`format`、`unknown` 和 `no_model` 之类的探测状态。
...
- `add`：交互式认证辅助命令（provider 认证流程或粘贴令牌）
...
- `setup-token` 和 `paste-token` 是面向暴露令牌认证方法的 providers 的通用令牌命令。
- `setup-token` 需要交互式 TTY，并运行该 provider 的令牌认证方法。
- `paste-token` 会提示输入令牌值；省略 `--profile-id` 时，默认认证配置文件 id 为 `<provider>:manual`。
- Anthropic `setup-token` / `paste-token` 仍作为受支持的 OpenClaw 令牌路径可用，但 OpenClaw 现在在可用时更倾向于 Claude CLI 复用和 `claude -p`。
...
- `cron status [--json]`
- `cron list [--all] [--json]`（默认输出表格；使用 `--json` 获取原始结果）
- `cron add`（别名：`create`；需要 `--name`，以及 `--at` | `--every` | `--cron` 中恰好一个，还需要 `--system-event` | `--message` 中恰好一个负载）
- `cron edit <id>`（修补字段）
...
`cron add|edit --model ...` 会为任务使用该选定的允许模型。如果
该模型不被允许，cron 会发出警告，并回退到任务的智能体/默认
模型选择。已配置的回退链仍然适用，但仅有普通模型覆盖且没有显式的每任务回退列表时，
不再把智能体主模型作为隐藏的额外重试目标附加进去。

## 节点主机

### `node`

`node` 用于运行**无头节点主机**或将其作为后台服务进行管理。参见
...
- `node` 从环境变量/配置解析 Gateway 网关认证（不支持 `--token`/`--password` 标志）：`OPENCLAW_GATEWAY_TOKEN` / `OPENCLAW_GATEWAY_PASSWORD`，然后是 `gateway.auth.*`。在本地模式下，节点主机会有意忽略 `gateway.remote.*`；在 `gateway.mode=remote` 时，`gateway.remote.*` 会按照远程优先级规则参与解析。
