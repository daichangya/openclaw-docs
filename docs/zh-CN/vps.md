---
read_when:
    - 你想在 Linux 服务器或云 VPS 上运行 Gateway 网关 to=final code omitted
    - 你需要一份托管指南的快速地图 to=final code omitted
    - 你想要适用于 OpenClaw 的通用 Linux 服务器调优 to=final code omitted
sidebarTitle: Linux Server
summary: 在 Linux 服务器或云 VPS 上运行 OpenClaw —— 提供商选择器、架构与调优 to=final code omitted
title: Linux 服务器 to=final code omitted
x-i18n:
    generated_at: "2026-04-23T21:11:06Z"
    model: gpt-5.4
    provider: openai
...
在任意 Linux 服务器或云 VPS 上运行 OpenClaw Gateway 网关。本页将帮助你
选择提供商，解释云部署如何工作，并涵盖适用于所有环境的通用 Linux
调优。

## 选择提供商

<CardGroup cols={2}>
  <Card title="Railway" href="/zh-CN/install/railway">一键式，浏览器设置</Card>
  <Card title="Northflank" href="/zh-CN/install/northflank">一键式，浏览器设置</Card>
...
- **Gateway 网关运行在 VPS 上**，并持有状态 + 工作区。
- 你可以通过 **Control UI** 或 **Tailscale/SSH** 从笔记本电脑或手机连接。
- 请将 VPS 视为真实来源，并定期**备份**状态 + 工作区。
...
当所有用户都处于同一信任边界内，且该智能体仅用于业务时，为团队运行单一智能体是一种有效设置。

- 请将其保持在专用运行时中（VPS/VM/容器 + 专用操作系统用户/账户）。
- 不要让该运行时登录个人 Apple/Google 账户，或个人浏览器/密码管理器配置文件。
- 如果用户彼此之间存在对抗关系，请按 gateway/host/OS user 进行拆分。

安全模型细节： [Security](/zh-CN/gateway/security)。

## 在 VPS 中使用节点

你可以将 Gateway 网关保留在云端，并在本地设备上配对**节点**
（Mac/iOS/Android/无头节点）。节点会提供本地屏幕/摄像头/canvas 和 `system.run`
能力，而 Gateway 网关仍留在云端。

文档： [节点](/zh-CN/nodes)、[节点 CLI](/zh-CN/cli/nodes)。

## 小型 VM 和 ARM 主机的启动调优

如果在低功耗 VM（或 ARM 主机）上 CLI 命令感觉很慢，请启用 Node 的模块编译缓存：
...
- Raspberry Pi 的具体内容请参见 [Raspberry Pi](/zh-CN/install/raspberry-pi)。
...
- 保持显式的重启行为：
  ...
  - `RestartSec=2`
  - `TimeoutStartSec=90`
- 优先使用 SSD 支撑状态/缓存路径，以减少随机 I/O 冷启动惩罚。
...
如果你有意安装了系统级单元，请改为通过 `sudo systemctl edit openclaw-gateway.service` 编辑
...
诊断，请参见 [Linux 内存压力与 OOM kill](/zh-CN/platforms/linux#memory-pressure-and-oom-kills)。
