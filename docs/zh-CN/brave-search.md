...
title: Brave 搜索（旧版路径）
x-i18n:
    generated_at: "2026-04-23T20:40:52Z"
    model: gpt-5.4
    provider: openai
...
旧版 `tools.web.search.apiKey` 仍可通过兼容性垫片加载，但它不再是规范配置路径。
...
- OpenClaw 使用 Brave 的 **Search** 套餐。如果你使用的是旧版订阅（例如最初每月 2,000 次查询的免费套餐），它仍然有效，但不包含较新的功能，例如 LLM Context 或更高的速率限制。
- 每个 Brave 套餐都包含**每月 5 美元的免费额度**（按月续期）。Search 套餐的费用是每 1,000 次请求 5 美元，因此该额度可覆盖每月 1,000 次查询。请在 Brave 控制台中设置你的使用上限，以避免产生意外费用。当前套餐信息请参见 [Brave API portal](https://brave.com/search/api/)。
- Search 套餐包含 LLM Context 端点和 AI 推理权限。若要存储结果以训练或微调模型，则需要包含明确存储权限的套餐。请参见 Brave 的 [Terms of Service](https://api-dashboard.search.brave.com/terms-of-service)。
- `llm-context` 模式返回的是带溯源依据的来源条目，而不是常规网页搜索摘要格式。
- `llm-context` 模式不支持 `ui_lang`、`freshness`、`date_after` 或 `date_before`。
- `ui_lang` 必须包含区域子标签，例如 `en-US`。
- 结果默认缓存 15 分钟（可通过 `cacheTtlMinutes` 配置）。

完整的 `web_search` 配置请参见 [Web tools](/zh-CN/tools/web)。
