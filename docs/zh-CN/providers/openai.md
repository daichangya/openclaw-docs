...
OpenAI 为 GPT 模型提供开发者 API。OpenClaw 支持三种 OpenAI 家族路径。模型前缀决定所走路径：

- **API key** —— 直接访问 OpenAI Platform，并按使用量计费（`openai/*` 模型）
- **通过 PI 使用 Codex 订阅** —— 使用 ChatGPT/Codex 登录并通过订阅访问（`openai-codex/*` 模型）
- **Codex app-server harness** —— 原生 Codex app-server 执行（`openai/*` 模型 + `agents.defaults.embeddedHarness.runtime: "codex"`）

OpenAI 明确支持在 OpenClaw 这类外部工具和工作流中使用订阅 OAuth。

<Note>
GPT-5.5 当前可在 OpenClaw 中通过订阅/OAuth 路径使用：
使用 PI 运行器时为 `openai-codex/gpt-5.5`，使用
Codex app-server harness 时为 `openai/gpt-5.5`。一旦 OpenAI 在公开 API 上启用 GPT-5.5，
就会支持通过 API key 直接访问 `openai/gpt-5.5`；在此之前，
`OPENAI_API_KEY` 设置请使用已启用 API 的模型，例如 `openai/gpt-5.4`。
</Note>

## OpenClaw 功能覆盖范围

| OpenAI 能力 | OpenClaw 能力面 | 状态 |
| ------------------------- | ------------------------------------------------------ | ------------------------------------------------------ |
    ...
    **最适合：** 直接 API 访问和按使用量计费。
    ...
    <Steps>
      <Step title="获取你的 API key">
        从 [OpenAI Platform dashboard](https://platform.openai.com/api-keys) 创建或复制一个 API key。
      </Step>
    ...
    | `openai/gpt-5.5` | 一旦 OpenAI 在 API 上启用 GPT-5.5 后的未来直连 API 路径 | `OPENAI_API_KEY` |

    <Note>
    除非你显式强制使用 Codex app-server harness，否则 `openai/*` 就是直连 OpenAI API key 路径。GPT-5.5 本身当前仅支持订阅/OAuth；若要通过默认 PI 运行器使用 Codex OAuth，请使用 `openai-codex/*`。
    </Note>
    ...
    <Warning>
    OpenClaw **不**暴露 `openai/gpt-5.3-codex-spark`。实时 OpenAI API 请求会拒绝该模型，而当前 Codex 目录也未暴露它。
    </Warning>

  </Tab>

  <Tab title="Codex 订阅">
    **最适合：** 使用你的 ChatGPT/Codex 订阅，而不是单独使用 API key。Codex cloud 需要 ChatGPT 登录。
    ...
    <Steps>
      <Step title="运行 Codex OAuth">
        ...
        对于无头环境或不适合回调的设置，可添加 `--device-code`，使用 ChatGPT 设备码流程登录，而不是通过 localhost 浏览器回调：
    ...
    | `openai/gpt-5.5` + `embeddedHarness.runtime: "codex"` | Codex app-server harness | Codex app-server 凭证 |

    <Note>
    对于凭证/配置文件命令，请继续使用 `openai-codex` 提供商 ID。
    ...
    <Note>
    onboarding 不再从 `~/.codex` 导入 OAuth 材料。请使用浏览器 OAuth（默认）或上面的设备码流程登录 —— OpenClaw 会在自己的智能体凭证存储中管理生成的凭证。
    </Note>

    ### 状态指示器

    聊天中的 `/status` 会显示当前会话正在使用哪个嵌入式 harness。默认的 PI harness 显示为 `Runner: pi (embedded)`，不会添加单独的徽标。当选择内置 Codex app-server harness 时，`/status` 会在 `Fast` 旁附加非 PI harness ID，例如 `Fast · codex`。现有会话会保留其已记录的 harness ID，因此如果你在更改 `embeddedHarness` 后希望 `/status` 反映新的 PI/Codex 选择，请使用 `/new` 或 `/reset`。
    ...
    实践中，较小的默认上限在延迟和质量方面表现更好。可通过 `contextTokens` 覆盖：
...
它同时支持使用 OpenAI API key 进行图像生成，以及通过 Codex OAuth 使用相同的 `openai/gpt-image-2` 模型引用进行图像生成。
...
| 宽高比 / 分辨率 | 不会透传到 OpenAI Images API | 在安全情况下映射为受支持尺寸 |
...
<Note>
共享工具参数、提供商选择和故障转移行为请参见 [图像生成](/zh-CN/tools/image-generation)。
</Note>

`gpt-image-2` 是 OpenAI 文生图生成和图像编辑的默认模型。
`gpt-image-1` 仍可作为显式模型覆盖使用，但新的 OpenAI 图像工作流应使用 `openai/gpt-image-2`。

对于 Codex OAuth 安装，请保持使用相同的 `openai/gpt-image-2` 引用。当配置了
`openai-codex` OAuth 配置文件时，OpenClaw 会解析该已存储的 OAuth
访问 token，并通过 Codex Responses 后端发送图像请求。它不会先尝试 `OPENAI_API_KEY`，也不会在该请求上静默回退到 API key。如果你希望走直连 OpenAI Images API 路径，请显式配置 `models.providers.openai`，提供 API key、自定义 base URL 或 Azure endpoint。
...
OpenClaw 会为跨提供商的 GPT-5 系列运行添加一个共享的 GPT-5 提示贡献。它按模型 ID 生效，因此 `openai-codex/gpt-5.5`、`openai/gpt-5.4`、`openrouter/openai/gpt-5.5`、`opencode/gpt-5.5` 以及其他兼容的 GPT-5 引用都会收到相同的覆盖层。较旧的 GPT-4.x 模型则不会。

内置的原生 Codex harness 也会通过 Codex app-server 开发者指令使用相同的 GPT-5 行为和心跳覆盖层，因此即使 Codex 持有了其余 harness 提示，被强制通过 `embeddedHarness.runtime: "codex"` 运行的 `openai/gpt-5.x` 会话仍会保留相同的贯彻执行和主动心跳指导。

GPT-5 贡献会添加一个带标签的行为契约，用于约束角色持续性、执行安全、工具纪律、输出形状、完成检查和验证。渠道特定的回复和静默消息行为仍保留在共享的 OpenClaw 系统提示和出站投递策略中。对于匹配的模型，GPT-5 指导始终启用。友好的交互风格层是独立的，并且可配置。
...
<AccordionGroup>
  <Accordion title="语音合成（TTS）">
    内置的 `openai` 插件会为 `messages.tts` 能力面注册语音合成。
  ...
  <Accordion title="语音转文本">
    ...
    当共享音频媒体配置或按调用的转录请求提供语言和提示信息时，它们会被转发给 OpenAI。
  ...
  <Accordion title="实时转录">
    ...
    <Note>
    使用 WebSocket 连接到 `wss://api.openai.com/v1/realtime`，并采用 G.711 u-law（`g711_ulaw` / `audio/pcmu`）音频。该流式提供商用于 Voice Call 的实时转录路径；Discord 语音当前仍记录短片段，并改用批量 `tools.media.audio` 转录路径。
    </Note>
...
内置的 `openai` 提供商可以通过覆盖 base URL，将图像生成功能指向 Azure OpenAI 资源。在图像生成路径上，OpenClaw 会检测 `models.providers.openai.baseUrl` 中的 Azure 主机名，并自动切换到 Azure 的请求形状。

<Note>
实时语音使用单独的配置路径
（`plugins.entries.voice-call.config.realtime.providers.openai.azureEndpoint`），不会受 `models.providers.openai.baseUrl` 影响。有关其 Azure 设置，请参见[语音与语音处理](#voice-and-speech)下的 **实时语音** 折叠面板。
</Note>
...
Azure OpenAI key（而非 OpenAI Platform key）：
...
其他 base URL（公共 OpenAI、兼容 OpenAI 的代理）会保留标准的
OpenAI 图像请求形状。

<Note>
`openai` 提供商图像生成路径的 Azure 路由需要
OpenClaw 2026.4.22 或更高版本。更早版本会将任何自定义
`openai.baseUrl` 当作公共 OpenAI 端点处理，并会在 Azure
图像 deployment 上失败。
</Note>
...
`uaenorth`）。请在创建 deployment 之前先检查 Microsoft 当前的区域列表，并确认你的区域中提供了特定模型。

### 参数差异

Azure OpenAI 与公共 OpenAI 并不总是接受相同的图像参数。
Azure 可能会拒绝公共 OpenAI 允许的选项（例如 `gpt-image-2` 上某些
`background` 值），或仅在特定模型版本上提供这些选项。这些差异来自 Azure 和底层模型，而不是 OpenClaw。如果 Azure 请求因验证错误而失败，请在 Azure 门户中检查你的具体 deployment 和 API 版本所支持的参数集。

<Note>
Azure OpenAI 使用原生传输和兼容行为，但不会接收
OpenClaw 的隐藏归因头 —— 请参见[高级配置](#advanced-configuration)下的 **原生 vs 兼容 OpenAI 的路由** 折叠面板。

对于 Azure 上的聊天或 Responses 流量（不仅限于图像生成），请使用
onboarding 流程或专用的 Azure 提供商配置 —— 仅设置
`openai.baseUrl` 并不会启用 Azure 的 API/凭证请求形状。另有单独的
...
<AccordionGroup>
  ...
  <Accordion title="快速模式">
    ...
    <Note>
    会话覆盖优先于配置。在 Sessions UI 中清除会话覆盖后，该会话会恢复为配置中的默认值。
    </Note>
  ...
  <Accordion title="服务端压缩（Responses API）">
    对于直连 OpenAI Responses 模型（位于 `api.openai.com` 上的 `openai/*`），OpenClaw 会自动启用服务端压缩：
    ...
    <Tabs>
      <Tab title="显式启用">
        对于 Azure OpenAI Responses 这类兼容端点很有用：
  ...
  <Accordion title="严格智能体 GPT 模式">
    ...
    - 当有可用工具操作时，不再将仅规划轮次视为成功进展
    - 使用“立即行动”的引导重试该轮次
    - 对于较大工作自动启用 `update_plan`
    - 如果模型持续只做规划而不行动，则显式暴露阻塞状态

    <Note>
    仅限于 OpenAI 和 Codex 的 GPT-5 系列运行。其他提供商和较旧模型家族保持默认行为。
    </Note>

  </Accordion>

  <Accordion title="原生 vs 兼容 OpenAI 的路由">
    OpenClaw 会将直连 OpenAI、Codex 和 Azure OpenAI 端点，与通用的兼容 OpenAI 的 `/v1` 代理区别对待：

    **原生路由**（`openai/*`、Azure OpenAI）：
    - 仅对支持 OpenAI `none` effort 的模型保留 `reasoning: { effort: "none" }`
    - 对于会拒绝 `reasoning.effort: "none"` 的模型或代理，省略已禁用的 reasoning
    - 默认将工具 schema 设为严格模式
    - 仅在已验证的原生主机上附加隐藏归因头
    - 保留 OpenAI 专属请求形状（`service_tier`、`store`、reasoning 兼容性、提示缓存提示）
