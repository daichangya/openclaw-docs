...
title: OpenAI
x-i18n:
    generated_at: "2026-04-23T19:52:02Z"
    model: gpt-5.4
    provider: openai
...
---
  ...
  OpenAI 为 GPT 模型提供开发者 API。OpenClaw 在同一组规范 OpenAI 模型引用后支持两种凭证方式：

  - **API 密钥** — 直接访问 OpenAI Platform，并按使用量计费（`openai/*` 模型）
  - **Codex 订阅** — 使用 ChatGPT/Codex 登录并通过订阅访问。内部凭证/提供商 ID 为 `openai-codex`，但新的模型引用仍应使用 `openai/*`。
  ...
  | Codex 订阅模型 | 带有 `openai-codex` 凭证的 `openai/<model>` | 是 |
  ...
  <Tab title="API 密钥（OpenAI Platform）">
    **最适合：** 直接 API 访问和按使用量计费。
    ...
    <Note>
    `openai-codex/*` 仍可作为旧版兼容别名使用，但新配置应使用 `openai/*`。
    </Note>
    ...
    <Warning>
    OpenClaw **不会**在直接 API 路径上暴露 `openai/gpt-5.3-codex-spark`。实时 OpenAI API 请求会拒绝该模型。Spark 仅适用于 Codex。
    </Warning>
  ...
  <Tab title="Codex 订阅">
    **最适合：** 使用你的 ChatGPT/Codex 订阅，而不是单独的 API 密钥。Codex cloud 需要 ChatGPT 登录。
    ...
    | `openai/gpt-5.3-codex-spark` | ChatGPT/Codex OAuth | Codex 登录（取决于权益） |

    <Note>
    `openai-codex/*` 和 `codex/*` 模型引用是旧版兼容别名。对于凭证/配置文件命令，继续使用 `openai-codex` 提供商 ID。
    </Note>
    ...
    <Note>
    新手引导不再从 `~/.codex` 导入 OAuth 材料。请使用浏览器 OAuth（默认）或上面的设备码流程登录——OpenClaw 会在它自己的智能体凭证存储中管理生成的凭证。
    </Note>
...
<Note>
有关共享工具参数、提供商选择和回退行为，请参见 [Image Generation](/zh-CN/tools/image-generation)。
</Note>
...
<Note>
有关共享工具参数、提供商选择和回退行为，请参见 [Video Generation](/zh-CN/tools/video-generation)。
</Note>

## GPT-5 提示词贡献

OpenClaw 会为跨提供商的 GPT-5 系列运行添加一个共享的 GPT-5 提示词贡献。它按模型 ID 应用，因此 `openai/gpt-5.5`、`openrouter/openai/gpt-5.5`、`opencode/gpt-5.5` 以及其他兼容的 GPT-5 引用都会收到相同的叠加层。较旧的 GPT-4.x 模型则不会。

内置的原生 Codex harness 通过 Codex app-server 开发者指令使用相同的 GPT-5 行为和心跳叠加层，因此即使在 `embeddedHarness.runtime: "codex"` 强制启用的 `openai/gpt-5.x` 会话中，其余 harness 提示词由 Codex 控制，仍会保留相同的后续执行和主动心跳指导。

GPT-5 贡献增加了一个带标签的行为契约，用于规范 persona 持续性、执行安全性、工具约束、输出形态、完成检查和验证。渠道特定的回复和静默消息行为仍保留在共享的 OpenClaw 系统提示词和出站传递策略中。GPT-5 指导对于匹配的模型始终启用。友好的交互风格层是独立的，并且可配置。
...
<Note>
旧版 `plugins.entries.openai.config.personality` 仍会作为兼容性回退项读取，前提是未设置共享的 `agents.defaults.promptOverlays.gpt5.personality`。
</Note>
...
<AccordionGroup>
  <Accordion title="语音合成（TTS）">
    ...
    <Note>
将 `OPENAI_TTS_BASE_URL` 设置为覆盖 TTS 的 base URL，而不影响聊天 API 端点。
</Note>
  ...
  <Accordion title="语音转文本">
    ...
    当通过共享音频媒体配置或每次调用的转写请求提供语言和提示词提示时，
    这些值会转发给 OpenAI。
  ...
  <Accordion title="实时转写">
    ...
    <Note>
    使用 WebSocket 连接到 `wss://api.openai.com/v1/realtime`，并采用 G.711 u-law（`g711_ulaw` / `audio/pcmu`）音频。这个流式提供商用于 Voice Call 的实时转写路径；Discord 语音当前仍记录短片段，并改用批量 `tools.media.audio` 转写路径。
    </Note>
...
内置的 `openai` 提供商可以通过覆盖 base URL，将目标指向 Azure OpenAI 资源以进行图像生成。在图像生成路径上，OpenClaw 会检测 `models.providers.openai.baseUrl` 上的 Azure 主机名，并自动切换到 Azure 的请求格式。

<Note>
实时语音使用单独的配置路径
（`plugins.entries.voice-call.config.realtime.providers.openai.azureEndpoint`），不受 `models.providers.openai.baseUrl` 影响。有关其实 Azure 设置，请参见 [语音和语音处理](#voice-and-speech) 下的 **实时语音** 折叠面板。
</Note>
...
如要通过内置的 `openai` 提供商使用 Azure 图像生成，请将
...
`uaenorth`）。在创建部署之前，请先检查 Microsoft 当前的区域列表，并确认特定模型在你的区域内可用。

### 参数差异

Azure OpenAI 与公共 OpenAI 并不总是接受相同的图像参数。
Azure 可能会拒绝公共 OpenAI 允许的选项（例如在 `gpt-image-2` 上某些
`background` 值），或者仅在特定模型版本中公开这些选项。这些差异来自 Azure 和底层模型，而不是 OpenClaw。如果 Azure 请求因验证错误而失败，请在 Azure 门户中检查你的具体部署和 API 版本所支持的参数集。

<Note>
Azure OpenAI 使用原生传输和兼容行为，但不会接收
OpenClaw 的隐藏归因请求头——请参见 [高级配置](#advanced-configuration) 下的 **Native vs OpenAI-compatible routes** 折叠面板。

对于 Azure 上的聊天或 Responses 流量（除图像生成外），请使用新手引导流程或专用的 Azure 提供商配置——单独设置 `openai.baseUrl` 并不会采用 Azure 的 API/凭证格式。另有一个独立的
`azure-openai-responses/*` 提供商；请参见下方的“服务端压缩”折叠面板。
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="WebSocket 预热">
    OpenClaw 默认对 `openai/*` 启用 WebSocket 预热，以减少首次轮次延迟。
  ...
  <Accordion title="快速模式">
    ...
    启用后，OpenClaw 会将快速模式映射到 OpenAI 的优先级处理（`service_tier = "priority"`）。现有的 `service_tier` 值会被保留，快速模式不会重写 `reasoning` 或 `text.verbosity`。
  ...
  <Accordion title="优先级处理（service_tier）">
    OpenAI 的 API 通过 `service_tier` 暴露优先级处理能力。在 OpenClaw 中按模型设置它：
  ...
  <Accordion title="服务端压缩（Responses API）">
    ...
    <Note>
`responsesServerCompaction` 仅控制 `context_management` 注入。直接 OpenAI Responses 模型仍会强制 `store: true`，除非兼容性将 `supportsStore` 设为 `false`。
</Note>
