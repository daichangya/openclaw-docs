...
title: OpenAI
x-i18n:
    generated_at: "2026-04-21T04:00:25Z"
    model: gpt-5.4
    provider: openai
...
OpenAI 提供用于 GPT 模型的开发者 API。OpenClaw 支持两种身份验证方式：
...
<Tabs>
  ...
  <Tab title="Codex 订阅">
    **最适合：** 使用你的 ChatGPT/Codex 订阅，而不是单独的 API 密钥。Codex cloud 需要使用 ChatGPT 登录。
    ...
    <Note>
    这一路径刻意与 `openai/gpt-5.4` 分离。直接 Platform 访问请使用带 API 密钥的 `openai/*`，Codex 订阅访问请使用 `openai-codex/*`。
    </Note>
    ...
    <Tip>
    如果新手引导复用了现有的 Codex CLI 登录，这些凭证将继续由 Codex CLI 管理。凭证过期时，OpenClaw 会先重新读取外部 Codex 来源，再将刷新的凭证写回 Codex 存储。
    </Tip>
...
GPT-5 提示词贡献默认会添加一个带标签的行为契约，涵盖输出形态、工具持续性、依赖检查、并行查找、完成检查、验证和自主性。这些指导对于匹配的 GPT-5 模型始终启用。友好交互风格层则是独立的，并且可配置。
...
<AccordionGroup>
  <Accordion title="语音合成（TTS）">
    ...
    <Note>
    设置 `OPENAI_TTS_BASE_URL` 可以覆盖 TTS 的基础 URL，而不影响聊天 API 端点。
    </Note>
...
<AccordionGroup>
  <Accordion title="传输方式（WebSocket 与 SSE）">
    对于 `openai/*` 和 `openai-codex/*`，OpenClaw 都默认采用 WebSocket 优先并在失败时回退到 SSE（`"auto"`）。
  ...
  <Accordion title="WebSocket 预热">
    对于 `openai/*`，OpenClaw 默认启用 WebSocket 预热，以降低首次轮次延迟。
  ...
  <Accordion title="快速模式">
    ...
    <Note>
    会话级覆盖优先于配置。在会话 UI 中清除会话级覆盖后，会话将恢复为配置中的默认值。
    </Note>
