---
read_when:
    - 你想用一个 API 密钥访问多个 LLM♀♀♀analysis to=functions.read েউjson commentary  天天中彩票开奖Parameters? no need. translate only.
    - 你需要百度 Qianfan 的设置指南
summary: 在 OpenClaw 中使用 Qianfan 的统一 API 访问多种模型
title: Qianfan
x-i18n:
    generated_at: "2026-04-23T21:01:42Z"
    model: gpt-5.4
    provider: openai
...
Qianfan 是百度的 MaaS 平台，通过一个**统一 API**，用单一
端点和 API 密钥将请求路由到多种模型。它兼容 OpenAI，因此大多数 OpenAI SDK 只需切换 base URL 即可使用。
...
<Steps>
  <Step title="创建百度云账号">
    在 [Qianfan Console](https://console.bce.baidu.com/qianfan/ais/console/apiKey) 注册或登录，并确保已启用 Qianfan API 访问。
  </Step>
...
<Tip>
默认内置模型引用是 `qianfan/deepseek-v3.2`。只有在你需要自定义 base URL 或模型元数据时，才需要覆盖 `models.providers.qianfan`。
</Tip>
...
<AccordionGroup>
  <Accordion title="传输与兼容性">
    Qianfan 通过兼容 OpenAI 的传输路径运行，而不是原生 OpenAI 请求整形。这意味着标准 OpenAI SDK 功能可以工作，但提供商特定参数可能不会被转发。
  </Accordion>
  ...
  <Accordion title="故障排除">
    - 请确认你的 API 密钥以 `bce-v3/ALTAK-` 开头，并且已在百度云控制台中启用 Qianfan API 访问。
    - 如果模型没有列出，请确认你的账户已激活 Qianfan 服务。
