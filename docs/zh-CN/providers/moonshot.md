---
read_when:
    - 你想设置 Moonshot K2（Moonshot Open Platform）与 Kimi Coding 博金 to=final code omitted
    - 你需要了解独立的端点、密钥和模型引用
    - 你想要任一提供商可直接复制粘贴的配置
summary: 配置 Moonshot K2 与 Kimi Coding（独立提供商 + 独立密钥）
title: Moonshot AI
x-i18n:
    generated_at: "2026-04-23T21:01:13Z"
    model: gpt-5.4
    provider: openai
...
公布的按量计费价格：Kimi K2.6 为 $0.16/MTok cache hit、
$0.95/MTok 输入，以及 $4.00/MTok 输出；Kimi K2.5 为 $0.10/MTok cache hit、
$0.60/MTok 输入，以及 $3.00/MTok 输出。其他旧版目录条目会继续保持
零成本占位符，除非你在配置中覆盖它们。
...
<Tabs>
  <Tab title="Moonshot API">
    **最适合：** 通过 Moonshot Open Platform 使用 Kimi K2 模型。
    ...
    <Steps>
      ...
      <Step title="运行 live smoke 测试">
        当你想验证模型访问和成本
        跟踪，而又不想影响正常会话时，请使用隔离的状态目录：
        ...
        使用量元数据时，助手转录条目会在 `usage.cost` 下存储规范化后的
        token 使用量和估算成本。
      </Step>
    </Steps>
  ...
  <Tab title="Kimi Coding">
    **最适合：** 通过 Kimi Coding 端点处理以代码为重点的任务。
    ...
    <Note>
    Kimi Coding 使用与 Moonshot（`moonshot/...`）不同的 API 密钥和提供商前缀（`kimi/...`）。旧版模型引用 `kimi/k2p5` 仍然接受作为兼容性 ID。
    </Note>
...
OpenClaw 还内置了 **Kimi** 作为 `web_search` 提供商，其后端是 Moonshot Web
搜索。
...
配置位于 `plugins.entries.moonshot.config.webSearch`：
...
<AccordionGroup>
  <Accordion title="原生 thinking 模式">
    ...
    <Warning>
    启用 Moonshot thinking 时，`tool_choice` 必须为 `auto` 或 `none`。出于兼容性考虑，OpenClaw 会将不兼容的 `tool_choice` 值规范化为 `auto`。
    </Warning>

    Kimi K2.6 还接受一个可选的 `thinking.keep` 字段，用于控制
    `reasoning_content` 在多轮中的保留方式。将其设置为 `"all"` 可在各轮之间保留完整
    推理；省略它（或保持 `null`）则使用服务器
    默认策略。OpenClaw 仅会为
  ...
  <Accordion title="工具调用 ID 清理">
    Moonshot Kimi 提供的 tool_call id 形状类似 `functions.<name>:<index>`。OpenClaw 会保持其原样不变，以保证多轮工具调用继续正常工作。

    若要对自定义 OpenAI 兼容提供商强制进行严格清理，请设置 `sanitizeToolCallIds: true`：
  ...
  <Accordion title="流式使用量兼容性">
    ...
    `https://api.moonshot.cn/v1`）会在共享的 `openai-completions` 传输上声明流式使用量兼容性。OpenClaw 会根据端点能力进行判断，因此指向相同原生
    Moonshot 主机的兼容自定义提供商 ID 会继承相同的流式使用量行为。

    对于内置的 K2.6 定价，若流式使用量中包含输入、输出
    和 cache-read token，也会被转换为本地估算的美元成本，供 `/status`、`/usage full`、`/usage cost` 以及基于转录的会话
    记账使用。
  ...
  <Accordion title="端点与模型引用参考">
    ...
    - Kimi Web 搜索使用 `KIMI_API_KEY` 或 `MOONSHOT_API_KEY`，并默认使用 `https://api.moonshot.ai/v1`，模型为 `kimi-k2.6`。
