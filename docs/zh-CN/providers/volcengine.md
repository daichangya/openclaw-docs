---
read_when:
    - 你想将 Volcano Engine 或 Doubao 模型与 OpenClaw 配合使用
    - 你需要 Volcengine API 密钥设置指引
summary: 火山引擎设置（Doubao 模型、通用端点 + 编码端点）
title: Volcengine（Doubao）
x-i18n:
    generated_at: "2026-04-23T21:02:38Z"
    model: gpt-5.4
    provider: openai
...
Volcengine 提供商可访问 Doubao 模型以及
托管在火山引擎上的第三方模型，并为通用与编码
工作负载提供独立端点。
...
<Steps>
  <Step title="设置 API 密钥">
    ...
    这会使用单个 API 密钥同时注册通用（`volcengine`）和编码（`volcengine-plan`）提供商。
...
<Note>
这两个提供商都使用同一个 API 密钥进行配置。设置时会自动同时注册。
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="模型选择器回退行为">
    在新手引导/配置模型选择期间，Volcengine 认证选项会优先选择
    `volcengine/*` 和 `volcengine-plan/*` 行。如果这些模型
    尚未加载，OpenClaw 会回退到未过滤目录，而不是显示一个
    空的按提供商划分的选择器。
  </Accordion>

  <Accordion title="守护进程的环境变量">
    如果 Gateway 网关以守护进程运行（launchd/systemd），请确保
...
<Warning>
当 OpenClaw 作为后台服务运行时，在交互式 shell 中设置的环境变量
