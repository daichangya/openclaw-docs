...
title: Ollama
x-i18n:
    generated_at: "2026-04-27T01:36:47Z"
    model: gpt-5.4
    provider: openai
...
OpenClaw 通过 Ollama 的原生 API（`/api/chat`）集成托管云模型和本地/自托管 Ollama 服务器。你可以通过三种模式使用 Ollama：通过可访问的 Ollama 主机实现的 `Cloud + Local`、连接 `https://ollama.com` 的 `Cloud only`，或连接可访问 Ollama 主机的 `Local only`。

<Warning>
**远程 Ollama 用户**：不要在 OpenClaw 中使用 `/v1` OpenAI 兼容 URL（`http://host:11434/v1`）。这会破坏工具调用，模型还可能将原始工具 JSON 作为纯文本输出。请改用原生 Ollama API URL：`baseUrl: "http://host:11434"`（不要加 `/v1`）。
</Warning>
  ...
  <Tab title="新手引导（推荐）">
    **最适合：** 以最快路径完成可用的 Ollama 云端或本地设置。
    ...
    <Steps>
      ...
      <Step title="选择一个模型">
        `Cloud only` 会提示输入 `OLLAMA_API_KEY`，并推荐托管云默认模型。`Cloud + Local` 和 `Local only` 会要求输入 Ollama 基础 URL、发现可用模型，并在所选本地模型尚不可用时自动拉取它。`Cloud + Local` 还会检查该 Ollama 主机是否已登录以启用云访问。
      </Step>
      <Step title="验证模型可用">
        ```bash
...
<Tabs>
  <Tab title="Cloud + Local">
    ...
    在设置期间使用 **Cloud + Local**。OpenClaw 会提示输入 Ollama 基础 URL，从该主机发现本地模型，并检查该主机是否已通过 `ollama signin` 登录以启用云访问。当主机已登录时，OpenClaw 还会推荐托管云默认模型，例如 `kimi-k2.5:cloud`、`minimax-m2.7:cloud` 和 `glm-5.1:cloud`。
  ...
  <Tab title="Cloud only">
    `Cloud only` 连接 Ollama 的托管 API：`https://ollama.com`。
    ...
    在 `openclaw onboard` 期间显示的云模型列表会从 `https://ollama.com/api/tags` 实时获取，最多 500 条，因此选择器反映的是当前托管目录，而不是静态预置列表。如果 `ollama.com` 在设置时不可访问或未返回任何模型，OpenClaw 会回退到之前硬编码的推荐值，以便新手引导仍能完成。
...
| 视觉模型 | 当 `/api/show` 报告 `vision` 能力时，这些模型会被标记为支持图像（`input: ["text", "image"]`），因此 OpenClaw 会自动将图像注入提示中 |
...
<Note>
如果你显式设置了 `models.providers.ollama`，自动发现会被跳过，你必须手动定义模型。请参阅下面的显式配置部分。
</Note>
...
对于未标记为支持图像的模型，OpenClaw 会拒绝图像描述请求。使用隐式发现时，如果 `/api/show` 报告了 vision 能力，OpenClaw 会从 Ollama 读取该信息。
...
<Tabs>
  <Tab title="基础（隐式发现）">
    ...
    <Tip>
    如果设置了 `OLLAMA_API_KEY`，你可以在 provider 条目中省略 `apiKey`，OpenClaw 会自动填充它以进行可用性检查。
    </Tip>

  </Tab>

  <Tab title="显式（手动模型）">
    当你希望使用托管云设置、Ollama 运行在其他主机/端口、想强制指定特定上下文窗口或模型列表，或者想完全手动定义模型时，请使用显式配置。
  ...
  <Tab title="自定义基础 URL">
    如果 Ollama 运行在不同的主机或端口上（显式配置会禁用自动发现，因此你必须手动定义模型）：
...
也支持自定义 Ollama provider id。当模型引用使用当前活动的 provider 前缀时，例如 `ollama-spark/qwen3:32b`，OpenClaw 在调用 Ollama 前只会去掉该前缀，因此服务器接收到的是 `qwen3:32b`。
...
| 认证 | 已登录的本地 Ollama 主机无需密钥；对于直接连接 `https://ollama.com` 的搜索或受认证保护的主机，使用 `OLLAMA_API_KEY` 或已配置的 provider 认证 |
| 要求 | 本地/自托管主机必须正在运行并已通过 `ollama signin` 登录；直接使用托管搜索需要 `baseUrl: "https://ollama.com"` 并配合真实的 Ollama API key |
...
<AccordionGroup>
  <Accordion title="旧版 OpenAI 兼容模式">
    <Warning>
    **在 OpenAI 兼容模式下，工具调用并不可靠。** 仅当你需要为代理使用 OpenAI 格式且不依赖原生工具调用行为时，才应使用此模式。
    </Warning>
    ...
    当 Ollama 配合 `api: "openai-completions"` 使用时，OpenClaw 默认会注入 `options.num_ctx`，这样 Ollama 就不会静默回退到 4096 的上下文窗口。如果你的代理/上游会拒绝未知的 `options` 字段，请禁用此行为：
  ...
  <Accordion title="上下文窗口">
    对于自动发现的模型，OpenClaw 会在可用时使用 Ollama 报告的上下文窗口，包括自定义 Modelfiles 中更大的 `PARAMETER num_ctx` 值。否则，它会回退到 OpenClaw 使用的默认 Ollama 上下文窗口。
  ...
  <Accordion title="推理模型">
    OpenClaw 默认将名称中包含 `deepseek-r1`、`reasoning` 或 `think` 的模型视为支持推理。
  ...
  <Accordion title="Memory Wiki 嵌入">
    内置的 Ollama 插件会为
    [memory search](/zh-CN/concepts/memory) 注册一个 Memory Wiki 嵌入 provider。它使用已配置的 Ollama 基础 URL
    和 API key，调用 Ollama 当前的 `/api/embed` 端点，并在可能时将多个
    memory 分块批量合并为一个 `input` 请求。
  ...
  <Accordion title="流式传输配置">
    ...
    对于原生 `/api/chat` 请求，OpenClaw 还会将思考控制直接转发给 Ollama：`/think off` 和 `openclaw agent --thinking off` 会发送顶层 `think: false`，而 `/think low|medium|high` 会发送对应的顶层 `think` 努力度字符串。`/think max` 会映射到 Ollama 最高的原生努力级别，即 `think: "high"`。
