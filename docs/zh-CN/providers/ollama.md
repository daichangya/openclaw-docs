...
title: Ollama
x-i18n:
    generated_at: "2026-04-27T02:31:49Z"
    model: gpt-5.4
    provider: openai
  ...
  <Tab title="新手引导（推荐）">
    **最适合：** 以最快方式完成可用的 Ollama 云端或本地设置。
    ...
    <Steps>
      ...
      <Step title="选择模型">
        `Cloud only` 会提示输入 `OLLAMA_API_KEY`，并建议使用托管云默认模型。`Cloud + Local` 和 `Local only` 会要求提供 Ollama 基础 URL、发现可用模型，并在所选本地模型尚不可用时自动拉取该模型。`Cloud + Local` 还会检查该 Ollama 主机是否已登录以启用云访问。
      </Step>
      <Step title="验证模型可用">
        ```bash
...
<Tabs>
  <Tab title="Cloud + Local">
    ...
    设置时使用 **Cloud + Local**。OpenClaw 会提示输入 Ollama 基础 URL，从该主机发现本地模型，并检查该主机是否已通过 `ollama signin` 登录以启用云访问。当主机已登录时，OpenClaw 还会建议托管云默认模型，例如 `kimi-k2.5:cloud`、`minimax-m2.7:cloud` 和 `glm-5.1:cloud`。
  ...
  <Tab title="Cloud only">
    ...
    `openclaw onboard` 期间显示的云模型列表会通过 `https://ollama.com/api/tags` 实时填充，最多 500 个条目，因此选择器反映的是当前托管目录，而不是静态预设。如果设置时 `ollama.com` 无法访问或未返回任何模型，OpenClaw 会回退到之前的硬编码建议，以便新手引导仍能完成。
...
当你设置了 `OLLAMA_API_KEY`（或身份验证配置文件），并且**没有**定义 `models.providers.ollama` 时，OpenClaw 会从位于 `http://127.0.0.1:11434` 的本地 Ollama 实例发现模型。

| 行为 | 详情 |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 目录查询 | 查询 `/api/tags` |
| 能力检测 | 使用尽力而为的 `/api/show` 查询来读取 `contextWindow`、展开的 `num_ctx` Modelfile 参数，以及包括 vision/tools 在内的能力 |
| 视觉模型 | 当 `/api/show` 报告 `vision` 能力时，带有该能力的模型会被标记为支持图像（`input: ["text", "image"]`），因此 OpenClaw 会自动将图像注入提示中 |
| 推理检测 | 使用模型名称启发式规则（`r1`、`reasoning`、`think`）标记 `reasoning` |
| 令牌限制 | 将 `maxTokens` 设置为 OpenClaw 使用的默认 Ollama 最大令牌上限 |
| 成本 | 将所有成本设置为 `0` |

这样可以避免手动添加模型条目，同时使目录与本地 Ollama 实例保持一致。
...
内置的 Ollama 插件会将 Ollama 注册为支持图像的媒体理解提供商。这使 OpenClaw 能够通过本地或托管的 Ollama 视觉模型，路由显式图像描述请求和已配置的图像模型默认值。
...
OpenClaw 会拒绝未标记为支持图像的模型所发出的图像描述请求。使用隐式发现时，当 `/api/show` 报告 vision 能力，OpenClaw 会从 Ollama 读取该信息。
...
<Tabs>
  ...
  <Tab title="自定义基础 URL">
    ...
    <Warning>
    不要在 URL 中添加 `/v1`。`/v1` 路径会使用 OpenAI 兼容模式，而该模式下工具调用不可靠。请使用不带路径后缀的 Ollama 基础 URL。
    </Warning>
...
| 认证 | 对已登录的本地 Ollama 主机无需密钥；对于直接 `https://ollama.com` 搜索或受保护的主机，使用 `OLLAMA_API_KEY` 或已配置的提供商认证 |
| 要求 | 本地/自托管主机必须正在运行，并已通过 `ollama signin` 登录；直接托管搜索需要 `baseUrl: "https://ollama.com"`，外加真实的 Ollama API 密钥 |
...
<AccordionGroup>
  ...
  <Accordion title="上下文窗口">
    ...
    你可以在显式提供商配置中覆盖 `contextWindow` 和 `maxTokens`。若要在不重建 Modelfile 的情况下限制 Ollama 每次请求的运行时上下文，请设置 `params.num_ctx`；OpenClaw 会将其作为 `options.num_ctx` 发送给原生 Ollama 和 OpenAI 兼容的 Ollama 适配器。无效、零、负数和非有限值会被忽略，并回退到 `contextWindow`。

    原生 Ollama 模型条目也接受 `params` 下常见的 Ollama 运行时选项，包括 `temperature`、`top_p`、`top_k`、`min_p`、`num_predict`、`stop`、`repeat_penalty`、`num_batch`、`num_thread` 和 `use_mmap`。OpenClaw 只会转发 Ollama 请求键，因此 OpenClaw 运行时参数（如 `streaming`）不会泄露给 Ollama。使用 `params.think` 或 `params.thinking` 可发送顶层 Ollama `think`；对于 Qwen 风格的思考模型，`false` 会禁用 API 级别的 thinking。
    ...
    每个模型的 `agents.defaults.models["ollama/<model>"].params.num_ctx` 也可以使用。如果两者都已配置，则显式提供商模型条目优先于智能体默认值。
  ...
  <Accordion title="Memory 嵌入">
    内置的 Ollama 插件为 [memory search](/zh-CN/concepts/memory) 注册了一个记忆嵌入提供商。它使用已配置的 Ollama 基础 URL 和 API 密钥，调用 Ollama 当前的 `/api/embed` 端点，并在可能时将多个记忆片段批处理到一次 `input` 请求中。
    ...
    要将 Ollama 选为 memory search 嵌入提供商：
  ...
  <Accordion title="流式传输配置">
    ...
    对于原生 `/api/chat` 请求，OpenClaw 也会将思考控制直接转发给 Ollama：`/think off` 和 `openclaw agent --thinking off` 会发送顶层 `think: false`，而 `/think low|medium|high` 会发送对应的顶层 `think` 强度字符串。`/think max` 会映射到 Ollama 最高的原生强度，即 `think: "high"`。
...
<AccordionGroup>
  <Accordion title="未检测到 Ollama">
    请确保 Ollama 正在运行，并且你已设置 `OLLAMA_API_KEY`（或身份验证配置文件），同时**没有**定义显式的 `models.providers.ollama` 条目：
