...
read_when:
    - 设置 iMessage 支持
    - 调试 iMessage 的发送/接收
summary: 通过 imsg 提供的旧版 iMessage 支持（基于 stdio 的 JSON-RPC）。新配置应使用 BlueBubbles。
title: iMessage
x-i18n:
...
`imsg` 集成属于旧版方案，未来版本中可能会移除。
</Warning>

状态：旧版外部 CLI 集成。Gateway 网关会启动 `imsg rpc`，并通过 stdio 上的 JSON-RPC 进行通信（不需要单独的守护进程或端口）。
...
```
  ...
  <Tab title="通过 SSH 连接远程 Mac">
    OpenClaw 只要求 `cliPath` 兼容 stdio，因此你可以将 `cliPath` 指向一个包装脚本，由它通过 SSH 连接到远程 Mac 并运行 `imsg`。
...
```
    ...
    `remoteHost` 必须是 `host` 或 `user@host` 格式（不能包含空格或 SSH 选项）。
    OpenClaw 对 SCP 使用严格主机密钥检查，因此中继主机的密钥必须已存在于 `~/.ssh/known_hosts` 中。
...
<Tip>
权限是按进程上下文授予的。如果 gateway 以无界面方式运行（LaunchAgent/SSH），请在相同上下文中运行一次交互式命令以触发权限提示：
...
<Tabs>
  ...
  <Tab title="群组策略 + 提及">
    ...
    运行时说明：如果 `channels.imessage` 完全缺失，运行时会回退为 `groupPolicy="allowlist"` 并记录警告（即使已设置 `channels.defaults.groupPolicy` 也是如此）。
...
- 之后在同一 iMessage 会话中的消息会路由到新建的 ACP 会话。
- `/new` 和 `/reset` 会在原位置重置同一个已绑定的 ACP 会话。
- `/acp close` 会关闭 ACP 会话并移除绑定。

也支持通过顶层 `bindings[]` 条目配置持久绑定，其中使用 `type: "acp"` 和 `match.channel: "imessage"`。

`match.peer.id` 可以使用：

- 规范化后的私信 handle，例如 `+15555550123` 或 `user@example.com`
...
<AccordionGroup>
  <Accordion title="专用机器人 macOS 用户（独立的 iMessage 身份）">
    使用专用 Apple ID 和 macOS 用户，以便将机器人流量与你个人的 Messages 配置文件隔离开来。
    ...
    5. 将 `channels.imessage.accounts.<id>.cliPath` 和 `.dbPath` 指向该用户的配置文件。
...
```
  ...
  <Accordion title="私信被忽略">
    ...
    - 配对批准情况（`openclaw pairing list imessage`）
  ...
  <Accordion title="远程附件失败">
    ...
    - gateway 主机上的 `~/.ssh/known_hosts` 中是否存在主机密钥
...
- [配置参考 - iMessage](/zh-CN/gateway/configuration-reference#imessage)
