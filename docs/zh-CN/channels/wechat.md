...
OpenClaw 通过腾讯提供的外部 `@tencent-weixin/openclaw-weixin` 渠道插件连接到 微信。

状态：外部插件。支持私聊和媒体。不通过当前插件能力元数据宣称支持群聊。

## 命名

- **微信** 是本文档中面向用户的名称。
- **微信** 是腾讯包名以及插件 id 中使用的名称。
...
微信代码不在 OpenClaw 核心仓库中。OpenClaw 提供通用的渠道插件契约，而外部插件提供微信专用的运行时：
...
6. 当 Gateway 网关启动时，插件会为每个已配置账户启动其微信监控器。
7. 入站微信消息会通过渠道契约被标准化，路由到所选的 OpenClaw 智能体，并通过插件的出站路径发回。

这种分离很重要：OpenClaw 核心应保持与渠道无关。微信登录、腾讯 iLink API 调用、媒体上传/下载、上下文令牌和账户监控都由外部插件负责。
...
使用手机上的微信扫描二维码并确认登录。扫描成功后，插件会将账户令牌保存在本地。

要添加另一个微信账户，再次运行相同的登录命令即可。对于多个账户，请按账户、渠道和发送者隔离私信会话：
...
私信使用 OpenClaw 针对渠道插件的常规配对和 allowlist 模型。
...
如果插件提示你的 OpenClaw 版本过旧，请更新 OpenClaw，或者安装旧版插件系列：
...
微信插件在监控腾讯 iLink API 时，可以在 Gateway 网关旁边运行辅助工作进程。在 issue #68451 中，这条辅助路径暴露了 OpenClaw 通用过期 Gateway 网关清理逻辑中的一个 bug：子进程可能会尝试清理父 Gateway 网关进程，从而在 systemd 等进程管理器下导致重启循环。
