...
x-i18n:
    generated_at: "2026-04-23T20:41:40Z"
    model: gpt-5.4
    provider: openai
...
支持文本和私信附件；频道和群组文件发送需要 `sharePointSiteId` + Graph 权限（参见[在群组聊天中发送文件](#sending-files-in-group-chats)）。投票通过 Adaptive Cards 发送。消息操作公开了显式的 `upload-file`，用于以文件为优先的发送。

## 内置插件

Microsoft Teams 在当前 OpenClaw 版本中作为内置插件提供，因此在常规打包构建中
无需单独安装。
...
3. 使用这些凭证配置 OpenClaw。
4. 通过公共 URL 或 tunnel 暴露 `/api/messages`（默认端口为 3978）。
...
对于生产部署，建议考虑使用[联合身份验证](#federated-authentication)（证书或托管身份）来替代 client secret。

注意：群组聊天默认被阻止（`channels.msteams.groupPolicy: "allowlist"`）。若要允许群组回复，请设置 `channels.msteams.groupAllowFrom`（或使用 `groupPolicy: "open"` 以允许任何成员，但默认仍有提及门控）。
...
- UPN/显示名称是可变的；默认禁用直接匹配，只有在 `channels.msteams.dangerouslyAllowNameMatching: true` 时才启用。
...
- 设置 `groupPolicy: "open"` 可允许任意成员（默认仍有提及门控）。
- 若要**完全禁用频道**，请设置 `channels.msteams.groupPolicy: "disabled"`。
...
- 通过在 `channels.msteams.teams` 下列出 teams 和 channels，限定群组/频道回复范围。
- 键应使用稳定的 team ID 和 channel conversation ID。
- 当 `groupPolicy="allowlist"` 且存在 teams allowlist 时，仅接受已列出的 teams/channels（有提及门控）。
- 配置向导接受 `Team/Channel` 条目，并会为你存储它们。
- 启动时，OpenClaw 会将 team/channel 和用户 allowlist 名称解析为 ID（在 Graph 权限允许时）
  并记录映射；默认情况下，无法解析的 team/channel 名称会按原样保留，但会被路由忽略，除非启用 `channels.msteams.dangerouslyAllowNameMatching: true`。
...
<Steps>
  <Step title="创建 Azure Bot">
    前往 [Create Azure Bot](https://portal.azure.com/#create/Microsoft.AzureBot)，并填写 **Basics** 标签页：
    ...
    <Note>
    新的多租户机器人在 2025-07-31 之后已被弃用。新机器人请使用 **Single Tenant**。
    </Note>
  ...
  <Step title="记录凭证">
    ...
    - **Manage Password** → **Certificates & secrets** → **New client secret** → 复制其值 → `appPassword`
  ...
  <Step title="配置消息端点">
    ...
    - 本地开发：使用 tunnel（参见[本地开发](#local-development-tunneling)）
...
对于生产部署，OpenClaw 支持 **联合身份验证**，作为比 client secret 更安全的替代方案。提供两种方式：
...
使用 Azure Managed Identity 实现无密码身份验证。这非常适合部署在 Azure 基础设施上的场景（AKS、App Service、Azure VM），即存在托管身份的环境。
...
- `MSTEAMS_MANAGED_IDENTITY_CLIENT_ID=<client-id>`（仅用户分配时需要）
...
3. 使用应用 client ID **注解 Kubernetes service account**：
...
5. **确保网络可访问** IMDS（`169.254.169.254`）——如果使用 NetworkPolicy，请添加一个出口规则，允许访问 `169.254.169.254/32` 的 80 端口流量。

### 身份验证类型对比

| 方法                 | 配置                                           | 优点                               | 缺点                                  |
| -------------------- | ---------------------------------------------- | ---------------------------------- | ------------------------------------- |
| **Client secret**    | `appPassword`                                  | 设置简单                           | 需要轮换 secret，安全性较低           |
| **Certificate**      | `authType: "federated"` + `certificatePath`    | 无需通过网络共享 secret            | 证书管理有额外开销                    |
| **Managed Identity** | `authType: "federated"` + `useManagedIdentity` | 无密码，无需管理 secret            | 需要 Azure 基础设施                   |

**默认行为：** 当未设置 `authType` 时，OpenClaw 默认使用 client secret 身份验证。现有配置无需更改即可继续工作。

## 本地开发（tunneling）

Teams 无法访问 `localhost`。本地开发请使用 tunnel：
...
2. 发送一条消息 —— 你应当能看到回复
3. 这可确认你的 webhook 端点在 Teams 设置前已正常工作
...
- `MSTEAMS_USE_MANAGED_IDENTITY`、`MSTEAMS_MANAGED_IDENTITY_CLIENT_ID`（联合身份验证 + 托管身份；client ID 仅用于用户分配）

</Accordion>

## 成员信息操作

OpenClaw 为 Microsoft Teams 提供了基于 Graph 的 `member-info` 操作，因此智能体和自动化可以直接通过 Microsoft Graph 解析渠道成员详细信息（显示名称、电子邮件、角色）。

要求：

- `Member.Read.Group` RSC 权限（已包含在推荐的 manifest 中）
- 对于跨 team 查询：带管理员同意的 `User.Read.All` Graph Application 权限

该操作受 `channels.msteams.actions.memberInfo` 控制（默认：当 Graph 凭证可用时启用）。

## 历史上下文

- `channels.msteams.historyLimit` 控制有多少最近的频道/群组消息会被包装进提示中。
- 回退到 `messages.groupChat.historyLimit`。设置为 `0` 可禁用（默认 50）。
- 获取的线程历史会按发送者 allowlist（`allowFrom` / `groupAllowFrom`）进行过滤，因此线程上下文预填充只会包含来自允许发送者的消息。
- 引用的附件上下文（由 Teams 回复 HTML 派生出的 `ReplyTo*`）当前会按接收到的原样传递。
- 换言之，allowlist 用于控制谁可以触发智能体；目前只有特定的补充上下文路径会被过滤。
- 私信历史可通过 `channels.msteams.dmHistoryLimit`（用户轮次）限制。按用户覆盖：`channels.msteams.dms["<user_id>"].historyLimit`。

## 当前 Teams RSC 权限

这些是我们 Teams 应用 manifest 中**现有的 resourceSpecific 权限**。它们仅在应用安装所在的 team/chat 内生效。

**用于 channels（team 作用域）：**
...
- `bots[].supportsFiles: true` 是个人作用域下文件处理所必需的。
...
5. **对于 team channels：** 在每个 team 中重新安装该应用，以使新权限生效
...
- 频道/群组中的**图片或文件内容**（负载中仅包含 HTML 占位内容）。
...
| **历史消息**            | 否                   | 是（可查询历史）                    |
| **设置复杂度**          | 仅需应用 manifest    | 需要管理员同意 + 令牌流程           |
| **离线可用**            | 否（必须运行中）     | 是（可随时查询）                    |

**结论：** RSC 用于实时监听；Graph API 用于历史访问。若要在离线期间补拉错过的消息，你需要带有 `ChannelMessage.Read.All` 的 Graph API（需要管理员同意）。

## 启用 Graph 的媒体 + 历史（channels 必需）

如果你需要 **channels** 中的图片/文件，或希望获取**消息历史**，则必须启用 Microsoft Graph 权限并授予管理员同意。
...
2. 为租户**授予管理员同意**。
3. 提升 Teams 应用 **manifest version**，重新上传，并在 Teams 中**重新安装应用**。
4. **完全退出并重新启动 Teams**，以清除缓存的应用元数据。

**用户提及的附加权限：** 对于当前会话中的用户，用户 @mentions 开箱即用。但如果你希望动态搜索并提及**不在当前会话中**的用户，请添加 `User.Read.All`（Application）权限并授予管理员同意。
...
OpenClaw 通过快速返回并主动发送回复来处理这一问题，但非常慢的响应仍可能引发问题。
...
按设置分组（共享渠道模式参见 `/gateway/configuration`）。
...
<AccordionGroup>
  ...
  <Accordion title="访问控制">
    ...
    - `allowFrom`：私信 allowlist，优先使用 AAD 对象 ID；当 Graph 访问可用时，向导会解析名称
    - `dangerouslyAllowNameMatching`：用于可变 UPN/显示名称以及 team/channel 名称路由的紧急兼容模式
    - `requireMention`：在频道/群组中要求 @mention（默认 `true`）
  </Accordion>

  <Accordion title="Team 和 channel 覆盖">
    以下所有项都会覆盖顶层默认值：
  ...
  <Accordion title="投递、媒体和操作">
    ...
    - `chunkMode`：`length`（默认）或 `newline`（在按长度分割前，先按段落边界分割）
    - `mediaAllowHosts`：入站附件主机 allowlist（默认是 Microsoft/Teams 域名）
    - `mediaAuthAllowHosts`：重试时允许接收 Authorization 头的主机（默认是 Graph + Bot Framework）
    - `replyStyle`：`thread | top-level`（参见[回复样式](#reply-style-threads-vs-posts)）
    - `actions.memberInfo`：切换基于 Graph 的成员信息操作（默认在 Graph 可用时开启）
    - `sharePointSiteId`：群聊/channels 中文件上传所必需（参见[在群组聊天中发送文件](#sending-files-in-group-chats)）
  </Accordion>
</AccordionGroup>
...
- 在 Threads 风格的频道中使用 `thread` → 回复会以不自然的嵌套方式显示
- 在 Posts 风格的频道中使用 `top-level` → 回复会显示为单独的顶层帖子，而不是线程内回复

**解决方案：** 根据频道的设置，按频道配置 `replyStyle`：
...
- **频道/群组：** 附件存储在 M365 存储（SharePoint/OneDrive）中。webhook 负载仅包含 HTML 占位内容，不包含实际文件字节。**下载频道附件需要 Graph API 权限**。
- 对于显式的文件优先发送，请使用 `action=upload-file`，并配合 `media` / `filePath` / `path`；可选的 `message` 会作为附带文本/评论，`filename` 会覆盖上传名称。

如果没有 Graph 权限，带图片的频道消息将仅以文本形式接收（机器人无法访问图片内容）。
默认情况下，OpenClaw 只会从 Microsoft/Teams 主机名下载媒体。可通过 `channels.msteams.mediaAllowHosts` 覆盖（使用 `["*"]` 以允许任意主机）。
Authorization 头只会附加到 `channels.msteams.mediaAuthAllowHosts` 中的主机（默认为 Graph + Bot Framework 主机）。请保持此列表严格（避免多租户后缀）。

## 在群组聊天中发送文件

机器人可以使用 FileConsentCard 流程在私信中发送文件（内置支持）。但是，**在群组聊天/channels 中发送文件**需要额外设置：

| 上下文                   | 文件发送方式                           | 所需设置                                        |
| ------------------------ | -------------------------------------- | ----------------------------------------------- |
| **私信**                 | FileConsentCard → 用户接受 → 机器人上传 | 开箱即用                                        |
| **群组聊天/channels**    | 上传到 SharePoint → 分享链接           | 需要 `sharePointSiteId` + Graph 权限            |
| **图片（任意上下文）**   | Base64 编码内联                        | 开箱即用                                        |

### 为什么群组聊天需要 SharePoint

机器人没有个人 OneDrive 驱动器（Graph API 的 `/me/drive` 端点不适用于应用身份）。要在群组聊天/channels 中发送文件，机器人需要上传到一个 **SharePoint site** 并创建共享链接。
...
3. **获取你的 SharePoint site ID：**
   ...
   ```bash
   # 通过 Graph Explorer 或携带有效令牌的 curl：
...
| 权限                                    | 共享行为                                              |
| --------------------------------------- | ----------------------------------------------------- |
| `Sites.ReadWrite.All` 仅此一项          | 组织范围共享链接（组织中的任何人都可访问）            |
| `Sites.ReadWrite.All` + `Chat.Read.All` | 按用户共享链接（仅聊天成员可访问）                    |

按用户共享更安全，因为只有聊天参与者可以访问该文件。如果缺少 `Chat.Read.All` 权限，机器人会回退为组织范围共享。
...
上传的文件会存储在所配置 SharePoint site 的默认文档库中的 `/OpenClawShared/` 文件夹下。
...
使用 `message` 工具或 CLI，将语义化展示负载发送给 Teams 用户或会话。OpenClaw 会根据通用展示协议将其渲染为 Teams Adaptive Cards。

`presentation` 参数接受语义块。当提供 `presentation` 时，消息文本可选。
...
注意：如果没有 `user:` 前缀，名称默认会按群组/team 解析。按显示名称向个人发送时，请始终使用 `user:`。

## 主动消息

- 只有在用户已发生交互之后，才可以发送**主动消息**，因为我们会在那时存储会话引用。
...
- Team ID = `/team/` 后面的路径段（URL 解码后，例如 `19:Bk4j...@thread.tacv2`）
- Channel ID = `/channel/` 后面的路径段（URL 解码后）
- **忽略** `groupId` 查询参数
...
- **频道中图片不显示：** 缺少 Graph 权限或管理员同意。请重新安装 Teams 应用，并完全退出/重新打开 Teams。
- **频道中无响应：** 默认要求提及；请设置 `channels.msteams.requireMention=false`，或按 team/channel 配置。
- **版本不匹配（Teams 仍显示旧 manifest）：** 删除并重新添加该应用，并完全退出 Teams 以刷新。
- **Webhook 返回 401 Unauthorized：** 在没有 Azure JWT 的情况下手动测试时，这是预期行为 —— 说明端点可达，但身份验证失败。请使用 Azure Web Chat 正确测试。

### Manifest 上传错误

- **“Icon file cannot be empty”：** manifest 引用的图标文件为 0 字节。请创建有效的 PNG 图标（`outline.png` 为 32x32，`color.png` 为 192x192）。
- **“webApplicationInfo.Id already in use”：** 该应用仍安装在另一个 team/chat 中。请先找到并卸载它，或等待 5 - 10 分钟完成传播。
- **上传时出现 “Something went wrong”：** 请改为通过 [https://admin.teams.microsoft.com](https://admin.teams.microsoft.com) 上传，打开浏览器 DevTools（F12）→ Network 标签页，并检查响应体中的实际错误信息。
...
4. 确认你使用了正确的作用域：teams 使用 `ChannelMessage.Read.Group`，群组聊天使用 `ChatMessage.Read.Chat`
