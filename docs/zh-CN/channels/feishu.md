...
**Status：** 适用于机器人私信 + 群聊的生产就绪。WebSocket 是默认模式；webhook 模式为可选。
...
在 Feishu/Lark 中打开该群组，点击右上角的菜单图标，然后进入**设置**。群组 ID (`chat_id`) 会列在设置页面中。
...
| `channels.feishu.requireMention`                  | 在群组中需要 @提及                   | `true`           |
...
对于 `groupSessionScope: "group_topic"` 和 `"group_topic_sender"`，原生 Feishu/Lark 话题群组使用事件 `thread_id`（`omt_*`）作为规范的话题会话键。OpenClaw 转换为线程的普通群组回复则继续使用回复根消息 ID（`om_*`），这样首轮消息和后续消息会保留在同一个会话中。
