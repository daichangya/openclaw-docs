...
即使某个频道已被允许（通过 `groupPolicy` + `groups`），且发送者也已被允许，OpenClaw 在群组上下文中默认仍采用**提及门控**。
...
- `toolsBySender` 的键应对 IRC 发送者身份值使用 `id:`：
  `id:eigen`，或使用更强匹配的 `id:eigen!~eigen@174.127.248.171`。
- 旧版未加前缀的键仍然受支持，但只会按 `id:` 进行匹配。
- 第一个匹配到的发送者策略优先生效；`"*"` 是通配回退项。

有关群组访问与提及门控（以及它们如何交互）的更多内容，请参见：[/channels/groups](/zh-CN/channels/groups)。
