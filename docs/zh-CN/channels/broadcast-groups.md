...
title: 广播组
x-i18n:
    generated_at: "2026-04-23T22:55:51Z"
    model: gpt-5.4
    provider: openai
...
广播组允许多个智能体同时处理并响应同一条消息。这样你就可以创建协同工作的专用智能体团队，并让它们在同一个 WhatsApp 群组或私信中一起工作——全部使用同一个电话号码。

当前范围：**仅支持 WhatsApp**（web 渠道）。

广播组会在渠道允许列表和群组激活规则之后进行评估。在 WhatsApp 群组中，这意味着广播会在 OpenClaw 原本会回复时触发（例如：根据你的群组设置，在被提及时）。
...
添加一个顶层 `broadcast` 部分（与 `bindings` 同级）。键为 WhatsApp 对等方 ID：
...
**结果：** 当 OpenClaw 在此聊天中原本会回复时，它将运行这三个智能体。
...
注意：广播组不会绕过渠道允许列表或群组激活规则（提及 / 命令 / 等）。它们只会改变消息符合处理条件时 _哪些智能体会运行_。
...
- **群组上下文缓冲区**（用于上下文的最近群消息）按对等方共享，因此所有广播智能体在触发时都会看到相同上下文
...
**原因：** 对等方 ID 可能存在于 `bindings` 中，但不在 `broadcast` 中。
