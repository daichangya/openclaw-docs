...
OpenClaw 可以使用 ElevenLabs、Google Gemini、Gradium、Inworld、Local CLI、Microsoft、MiniMax、OpenAI、Volcengine、Vydra、xAI 或 Xiaomi MiMo，将外发回复转换为音频。
...
如果配置了多个提供商，则会优先使用已选提供商，其他提供商作为回退选项。
...
不会。自动 TTS 默认**关闭**。可通过配置中的 `messages.tts.auto` 启用，或在本地使用 `/tts on` 启用。
...
MiniMax TTS 的认证解析顺序为 `messages.tts.providers.minimax.apiKey`，然后是已存储的 `minimax-portal` OAuth/token 配置文件，再然后是 Token Plan 环境变量键（`MINIMAX_OAUTH_TOKEN`、`MINIMAX_CODE_PLAN_KEY`、`MINIMAX_CODING_API_KEY`），最后是 `MINIMAX_API_KEY`。当未显式设置 TTS `baseUrl` 时，OpenClaw 可以为 Token Plan 语音复用已配置的 `minimax-portal` OAuth 主机。
...
`apiKey` 的值必须是从 Inworld 控制台逐字复制的 Base64 编码凭证字符串（Workspace > API Keys）。提供商会将其作为 `Authorization: Basic <apiKey>` 发送，不会进行任何额外编码，因此不要传入原始 bearer token，也不要自行再次进行 Base64 编码。该密钥会回退到 `INWORLD_API_KEY` 环境变量。完整设置请参见 [Inworld 提供商](/zh-CN/providers/inworld)。
...
Local CLI TTS 会在 Gateway 网关主机上运行已配置的命令。`args` 中的 `{{Text}}`、`{{OutputPath}}`、`{{OutputDir}}` 和 `{{OutputBase}}` 占位符会被展开；如果不存在 `{{Text}}` 占位符，OpenClaw 会将要朗读的文本写入 stdin。`outputFormat` 接受 `mp3`、`opus` 或 `wav`。语音便笺目标会被转码为 Ogg/Opus，电话输出会通过 `ffmpeg` 转码为原始 16 kHz 单声道 PCM。旧版提供商别名 `cli` 仍然有效，但新配置应使用 `tts-local-cli`。
...
- `providers.google.speakerName`：可选的说话人标签；当你的 TTS 提示使用具名说话人时，会在朗读文本前附加。
...
- **Feishu**：当语音便笺回复以 MP3/WAV/M4A 或其他
  可能的音频文件格式生成时，Feishu 插件会在发送原生 `audio` 气泡之前使用
...
- **Google Gemini**：Gemini API TTS 返回原始 24 kHz PCM。OpenClaw 会将其封装为 WAV 以用于音频附件，为语音便笺目标转码为 48 kHz Opus，并为 Talk/电话直接返回 PCM。
...
它接受可选的 `channel` 和 `timeoutMs` 字段；`timeoutMs` 是按调用设置的提供商请求超时时间（毫秒）。
