...
当 `messages.tts.provider` 未设置时，OpenClaw 会按注册表自动选择顺序选择第一个已配置的 speech 提供商。
...
- `auto`：自动 TTS 模式（`off`、`always`、`inbound`、`tagged`）。
  ...
  - `tagged` 仅当回复中包含 `[[tts:key=value]]` 指令或 `[[tts:text]]...[[/tts:text]]` 块时发送音频。
- `enabled`：旧版开关（Doctor 会将其迁移为 `auto`）。
- `mode`：`"final"`（默认）或 `"all"`（包括工具/分块回复）。
- `provider`：speech 提供商 id，例如 `"elevenlabs"`、`"google"`、`"microsoft"`、`"minimax"` 或 `"openai"`（后备自动处理）。
- 如果 `provider` **未设置**，OpenClaw 会按注册表自动选择顺序使用第一个已配置的 speech 提供商。
- 旧版 `provider: "edge"` 仍可使用，并会被标准化为 `microsoft`。
...
- `providers.openai.baseUrl`：覆盖 OpenAI TTS 端点。
  ...
  - 非默认值会被视为 OpenAI 兼容 TTS 端点，因此可接受自定义模型名和语音名。
...
- `providers.microsoft.enabled`：允许使用 Microsoft speech（默认 `true`；无 API key）。
...
- `provider`（已注册的 speech 提供商 id，例如 `openai`、`elevenlabs`、`google`、`minimax` 或 `microsoft`；需要 `allowProvider: true`）
...
- **Microsoft**：使用 `microsoft.outputFormat`（默认 `audio-24khz-48kbitrate-mono-mp3`）。
  ...
  - 如果已配置的 Microsoft 输出格式失败，OpenClaw 会回退到 MP3 重试。
...
如果回复超过 `maxLength` 且摘要已关闭（或摘要模型没有 API key），则会跳过音频，
仅发送普通文本回复。
...
Discord 说明：`/tts` 是 Discord 的内置命令，因此 OpenClaw 会
