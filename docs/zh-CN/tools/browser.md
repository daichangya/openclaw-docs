...
x-i18n:
    generated_at: "2026-04-22T20:50:07Z"
    model: gpt-5.4
    provider: openai
...
这个浏览器**不是**你日常使用的主浏览器。它是一个安全、隔离的界面，用于
智能体自动化和验证。
...
默认的 `browser` 工具现在是一个随附的内置插件，且默认启用。
...
Gateway 网关方法（`browser.request`）、智能体工具以及默认浏览器控制
服务都会一并消失。你的 `browser.*` 配置会保持不变，以便替换插件复用。

内置浏览器插件现在也负责浏览器运行时实现。
核心仅保留共享的插件 SDK 辅助工具，以及用于旧内部导入路径的兼容性重新导出。
实际上，移除或替换浏览器插件包时，浏览器功能集会一起消失，而不会再留下第二套由
核心拥有的运行时实现。

浏览器配置变更仍然需要重启 Gateway 网关，这样内置插件才能使用新设置重新注册其
浏览器服务。

## 缺少浏览器命令或工具

如果升级后 `openclaw browser` 突然变成未知命令，或者智能体报告浏览器工具缺失，
最常见的原因是一个限制性的 `plugins.allow` 列表，其中没有包含 `browser`。
...
- 设置了 `plugins.allow` 时，仅有 `browser.enabled=true` 本身并不够。
- 设置了 `plugins.allow` 时，仅有 `plugins.entries.browser.enabled=true` 本身也不够。
- `tools.alsoAllow: ["browser"]` **不会**加载内置浏览器插件。它只会在插件已经加载后调整工具策略。
- 如果你不需要限制性的插件允许列表，移除 `plugins.allow` 也会恢复默认的内置浏览器行为。
...
- `user`：内置的 Chrome MCP 连接配置文件，用于你**真实的、已登录的 Chrome**
  会话。

对于智能体浏览器工具调用：

- 默认：使用隔离的 `openclaw` 浏览器。
- 当现有登录会话很重要，并且用户在电脑前可以点击/批准任何连接提示时，优先使用 `profile="user"`。
...
```json5
{
  browser: {
    ...
    ssrfPolicy: {
      // dangerouslyAllowPrivateNetwork: true, // 仅在你信任私有网络访问时选择启用
...
- 浏览器导航/打开标签页在导航前会受到 SSRF 防护，并会在导航后对最终的 `http(s)` URL 尽力再次检查。
- 在严格 SSRF 模式下，远程 CDP 端点发现/探测（`cdpUrl`，包括 `/json/version` 查找）也会被检查。
- `browser.ssrfPolicy.dangerouslyAllowPrivateNetwork` 默认禁用。只有在你明确信任私有网络浏览器访问时，才将其设置为 `true`。
- `browser.ssrfPolicy.allowPrivateNetwork` 仍作为旧别名保留，以保持兼容性。
- `attachOnly: true` 表示“绝不启动本地浏览器；仅在其已运行时进行连接”。
- `color` + 各配置文件的 `color` 会为浏览器 UI 着色，这样你可以看出当前哪个配置文件处于活动状态。
- 默认配置文件是 `openclaw`（由 OpenClaw 管理的独立浏览器）。使用 `defaultProfile: "user"` 可选择使用用户已登录的浏览器。
- 自动检测顺序：系统默认浏览器（如果它基于 Chromium）；否则为 Chrome → Brave → Edge → Chromium → Chrome Canary。
- 本地 `openclaw` 配置文件会自动分配 `cdpPort`/`cdpUrl` —— 只有远程 CDP 才需要设置这些。
- `driver: "existing-session"` 使用 Chrome DevTools MCP，而不是原始 CDP。请勿为该驱动设置 `cdpUrl`。
- 当 existing-session 配置文件需要连接到非默认的 Chromium 用户配置文件（例如 Brave 或 Edge）时，请设置 `browser.profiles.<name>.userDataDir`。
...
都会保留认证信息。对于令牌，优先使用环境变量或密钥管理器，而不是将它们提交到配置文件中。
...
- `nodeHost.browserProxy.allowProfiles` 是可选的。将其留空可保持旧版/默认行为：所有已配置的配置文件都仍可通过代理访问，包括配置文件创建/删除路由。
- 如果你设置了 `nodeHost.browserProxy.allowProfiles`，OpenClaw 会将其视为最小权限边界：只有在允许列表中的配置文件可以作为目标，并且持久配置文件创建/删除路由会在代理层被阻止。
- 如果你不想使用它，可以禁用：
...
[Browserless](https://browserless.io) 是一个托管的 Chromium 服务，
通过 HTTPS 和 WebSocket 暴露 CDP 连接 URL。OpenClaw 可以使用这两种形式，但
...
- 如果 Browserless 提供给你的是 HTTPS 基础 URL，你可以将其转换为
  `wss://` 以建立直接 CDP 连接，或者保留 HTTPS URL 并让 OpenClaw
  自动发现 `/json/version`。
...
- **裸 WebSocket 根路径** — `ws://host[:port]` 或 `wss://host[:port]`，且不带
  ...
  会回退为在裸根路径上直接进行 WebSocket 握手。这样可以同时覆盖
  Chrome 风格的远程调试端口和仅支持 WebSocket 的提供商。

不带 `/devtools/...` 路径的普通 `ws://host:port` / `wss://host:port`，
如果指向本地 Chrome 实例，也可通过先发现后回退的方式得到支持——
Chrome 只接受对 `/json/version` 返回的特定每浏览器
或每目标路径的 WebSocket 升级，因此单独对裸根路径进行握手
会失败。

### Browserbase

[Browserbase](https://www.browserbase.com) 是一个云平台，用于运行
headless 浏览器，内置 CAPTCHA 求解、隐身模式和住宅代理。
...
- 完整的 API
  参考、SDK 指南和集成示例，请参阅 [Browserbase 文档](https://docs.browserbase.com)。
...
- 如果启用了浏览器控制，且未配置任何共享密钥认证，OpenClaw
...
- **由 openclaw 管理**：一个专用的、基于 Chromium 的浏览器实例，具有自己的用户数据目录 + CDP 端口
- **远程**：一个显式的 CDP URL（基于 Chromium 的浏览器运行在其他地方）
- **现有会话**：通过 Chrome DevTools MCP 自动连接到你现有的 Chrome 配置文件

默认值：

- 如果缺失，会自动创建 `openclaw` 配置文件。
- `user` 配置文件是内置的，用于 Chrome MCP 的 existing-session 连接。
...
OpenClaw 还可以通过官方的 Chrome DevTools MCP 服务器连接到一个正在运行的
基于 Chromium 的浏览器配置文件。这样可以复用该浏览器配置文件中
已经打开的标签页和登录状态。
...
- 如果你使用自定义 existing-session 配置文件，请传入那个明确的配置文件名。
- 仅当用户在电脑前可以批准连接提示时，才选择此模式。
...
- 这条路径比隔离的 `openclaw` 配置文件风险更高，因为它可以
  在你已登录的浏览器会话中执行操作。
- 对于该驱动，OpenClaw 不会启动浏览器；它只会连接到现有会话。
- OpenClaw 在这里使用官方的 Chrome DevTools MCP `--autoConnect` 流程。如果
  设置了 `userDataDir`，OpenClaw 会将其一并传递，以显式定位到对应的
  Chromium 用户数据目录。
- Existing-session 截图支持页面截图以及来自快照的 `--ref` 元素
  截图，但不支持 CSS `--element` 选择器。
- Existing-session 页面截图无需 Playwright，也可通过 Chrome MCP 工作。
  基于 ref 的元素截图（`--ref`）在那里也可用，但 `--full-page`
  不能与 `--ref` 或 `--element` 组合使用。
- Existing-session 操作仍然比受管理浏览器
  路径更受限制：
  ...
  - `hover`、`scrollIntoView`、`drag`、`select`、`fill` 和 `evaluate` 不
    支持按调用覆盖超时设置
  - `select` 当前仅支持单个值
- 与其他浏览器驱动一样，Existing-session `wait --url` 支持精确、子串和 glob 模式。
  目前尚不支持 `wait --load networkidle`。
- Existing-session 上传钩子要求使用 `ref` 或 `inputRef`，一次支持一个文件，
  且不支持 CSS `element` 目标定位。
- Existing-session 对话框钩子不支持超时覆盖。
- 某些功能仍然需要受管理浏览器路径，包括批量
  操作、PDF 导出、下载拦截以及 `responsebody`。
- Existing-session 可以连接到所选主机上，或通过已连接的
  浏览器节点进行连接。如果 Chrome 在别处运行且没有连接浏览器节点，请改用
  远程 CDP 或节点主机。
...
在没有 Playwright 的情况下，以下功能仍可用：

- ARIA 快照
- 受管理的 `openclaw` 浏览器的页面截图，前提是存在每标签页的 CDP
  WebSocket
...
`/home/node/.cache/ms-playwright`），并确保通过 `OPENCLAW_HOME_VOLUME` 或 bind mount 持久化 `/home/node`。
...
- 对于高级操作（点击/输入/快照/PDF），它在
  CDP 之上使用 **Playwright**。
- 缺少 Playwright 时，只有不依赖 Playwright 的操作可用。

这种设计让智能体始终面向一个稳定、可预测的接口，同时允许
你切换本地/远程浏览器和配置文件。
...
- 对于仅连接和远程 CDP 配置文件，测试结束后
  `openclaw browser stop` 仍然是正确的清理命令。它会关闭当前活动的控制会话，并清除临时仿真覆盖项，而不是杀掉底层
  浏览器。
...
- `snapshot`：
  - `--format ai`（安装了 Playwright 时的默认值）：返回带有数字 refs 的 AI 快照（`aria-ref="<n>"`）。
  - `--format aria`：返回无障碍树（无 refs；仅用于检查）。
  - `--efficient`（或 `--mode efficient`）：紧凑 role 快照预设（interactive + compact + depth + 更低的 maxChars）。
  - 配置默认值（仅工具/CLI）：设置 `browser.snapshotDefaults.mode: "efficient"`，可在调用方未传入 mode 时使用高效快照（参见 [Gateway 网关配置](/zh-CN/gateway/configuration-reference#browser)）。
  - Role 快照选项（`--interactive`、`--compact`、`--depth`、`--selector`）会强制使用基于 role 的快照，并生成类似 `ref=e12` 的 refs。
  - `--frame "<iframe selector>"` 会将 role 快照限定在某个 iframe 内（与 `e12` 这类 role refs 配合使用）。
  - `--interactive` 会输出一个扁平、易于选择的交互元素列表（最适合驱动操作）。
  - `--labels` 会额外附加一张仅视口截图，并叠加 ref 标签（输出 `MEDIA:<path>`）。
- `click`/`type`/等操作要求使用 `snapshot` 返回的 `ref`（数字 `12` 或 role ref `e12`）。
  出于设计考虑，操作不支持 CSS 选择器。
...
- refs **不会在导航之间保持稳定**；如果有操作失败，请重新运行 `snapshot` 并使用新的 ref。
- 如果 role 快照是用 `--frame` 获取的，那么这些 role refs 会限定在该 iframe 内，直到下一次 role 快照。
...
3. 如果仍然失败：运行 `openclaw browser highlight <ref>`，查看 Playwright 实际定位到了什么
4. 如果页面行为异常：
   - `openclaw browser errors --clear`
   - `openclaw browser requests --filter api --clear`
5. 如需深度调试：录制 trace：
...
JSON 中的 role 快照包含 `refs` 以及一个小型 `stats` 块（lines/chars/refs/interactive），便于工具推断负载大小和密度。

## 状态和环境控制项

这些在“让网站表现得像 X”这类工作流中很有用：

- Cookies：`cookies`、`cookies set`、`cookies clear`
- Storage：`storage local|session get|set|clear`
- 离线：`set offline on|off`
- Headers：`set headers --headers-json '{"X-Debug":"1"}'`（旧用法 `set headers --json '{"X-Debug":"1"}'` 仍然受支持）
...
- `browser act kind=evaluate` / `openclaw browser evaluate` 和 `wait --fn`
  会在页面上下文中执行任意 JavaScript。提示注入可能引导
  这一过程。如果你不需要它，可用 `browser.evaluateEnabled=false` 禁用。
...
有关 WSL2 Gateway 网关 + Windows Chrome 分离主机设置，参见
...
- 导航 SSRF 阻止：
  - 当 `start` 和 `tabs` 仍然可用时，`open`、`navigate`、snapshot 或标签页打开流程因浏览器/网络策略错误而失败
...
- 即使你没有配置 `browser.ssrfPolicy`，浏览器配置默认也会采用故障关闭的 SSRF 策略对象。
- 对于本地 loopback 的 `openclaw` 受管理配置文件，CDP 健康检查会刻意跳过浏览器 SSRF 可达性强制检查，以便 OpenClaw 自身的本地控制平面可以工作。
- 导航保护是独立的。`start` 或 `tabs` 成功，并不意味着之后的 `open` 或 `navigate` 目标一定被允许。

安全建议：

- **不要**默认放宽浏览器 SSRF 策略。
- 与宽泛的私有网络访问相比，更推荐使用狭义的主机例外，例如 `hostnameAllowlist` 或 `allowedHostnames`。
...
这指向浏览器启动或 CDP 可达性问题，而不是页面 URL 允许列表问题。
