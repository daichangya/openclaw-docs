...
在 Ubuntu（以及许多 Linux 发行版）上，默认的 Chromium 安装是一个 **snap 包**。Snap 的 AppArmor 限制会干扰 OpenClaw 启动和监控浏览器进程的方式。
...
安装官方的 Google Chrome `.deb` 包，它不会像 snap 那样受沙箱限制：
...
### 解决方案 2：将 Snap Chromium 与 Attach-Only 模式配合使用
...
| `browser.executablePath` | 基于 Chromium 的浏览器二进制路径（Chrome/Brave/Edge/Chromium） | 自动检测（若默认浏览器基于 Chromium，则优先使用） |
...
你正在使用 `existing-session` / Chrome MCP 配置档案。OpenClaw 可以看到本地 Chrome，
但没有可附加的已打开标签页。
...
2. **使用 Chrome MCP：** 确保本地 Chrome 正在运行，并至少打开了一个标签页，然后使用 `--browser-profile user` 重试。

说明：

- `user` 仅适用于宿主机。对于 Linux 服务器、容器或远程主机，请优先使用 CDP 配置档案。
- `user` / 其他 `existing-session` 配置档案仍保留当前 Chrome MCP 限制：
  基于 ref 的操作、单文件上传钩子、不支持对话框超时覆盖、不支持
  `wait --load networkidle`，且不支持 `responsebody`、PDF 导出、下载
  拦截或批量操作。
- 本地 `openclaw` 配置档案会自动分配 `cdpPort`/`cdpUrl`；只有远程 CDP 才需要手动设置它们。
- 远程 CDP 配置档案接受 `http://`、`https://`、`ws://` 和 `wss://`。
  当需要通过 `/json/version` 发现时，请使用 HTTP(S)；当你的浏览器
