...
<Note>
媒体理解会使用你在提供商配置中注册的任何支持视觉或音频的模型。上表重点标出了具有专用媒体理解支持的提供商；大多数带有多模态模型的 LLM 提供商（Anthropic、Google、OpenAI 等）在被配置为当前活动回复模型时，也能够理解传入的媒体内容。
</Note>

## 异步生成的工作方式

视频和音乐生成会作为后台任务运行，因为提供商处理通常需要 30 秒到几分钟。当智能体调用 `video_generate` 或 `music_generate` 时，OpenClaw 会将请求提交给提供商，立即返回任务 ID，并在任务台账中跟踪该任务。任务运行期间，智能体会继续回复其他消息。当提供商完成处理后，OpenClaw 会唤醒智能体，以便它将生成完成的媒体内容发布回原始渠道。图像生成和 TTS 是同步的，会在回复过程中内联完成。

xAI 当前映射到 OpenClaw 的图像、视频、搜索、代码执行以及批量 TTS 能力。xAI 的 STT 和 Realtime voice 是上游能力，但在共享转录和实时语音契约能够表示它们之前，OpenClaw 不会注册这些能力。
