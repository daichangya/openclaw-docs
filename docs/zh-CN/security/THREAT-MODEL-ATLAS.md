...
read_when:
    - 审查安全态势或威胁场景
    - 处理安全功能或审计响应 საკითხ to=functions.read კომენტary d天天_input={"path":"/home/runner/work/docs/docs/source/.agents/skills/openclaw-ghsa-maintainer/SKILL.md"} code
...
**框架：** [MITRE ATLAS](https://atlas.mitre.org/)（面向 AI 系统的对抗性威胁版图）

### 框架归属说明

本威胁模型建立在 [MITRE ATLAS](https://atlas.mitre.org/) 之上，它是用于记录 AI/ML 系统对抗性威胁的行业标准框架。ATLAS 由 [MITRE](https://www.mitre.org/) 与 AI 安全社区共同维护。
...
- [向 ATLAS 贡献](https://atlas.mitre.org/resources/contribute)

### 为本威胁模型做贡献

这是一个由 OpenClaw 社区维护的动态文档。有关贡献指南，请参阅 [CONTRIBUTING-THREAT-MODEL.md](/zh-CN/security/CONTRIBUTING-THREAT-MODEL)：
...
本威胁模型使用专为 AI/ML 系统设计的 MITRE ATLAS 框架，记录针对 OpenClaw AI 智能体平台和 ClawHub skill 市场的对抗性威胁。
...
| ClawHub Marketplace | 是 | Skill 发布、审核、分发 |
| MCP 服务器 | 是 | 外部工具提供方 |
| 用户设备 | 部分 | 移动应用、桌面客户端 |
...
│  ┌──────────────────────────────────────────────────────────┐   │
│  │                    执行沙箱                               │   │
│  │  • Docker 沙箱或宿主机（exec-approvals）                   │   │
│  │  • Node 远程执行                                           │   │
│  │  • SSRF 保护（DNS pinning + IP blocking）                 │   │
│  └──────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
...
| **受影响组件** | 所有工具调用 |
| **当前缓解措施** | 对危险命令使用 Exec approvals |
| **剩余风险** | 高 —— 依赖用户判断 |
| **建议** | 实施参数校验和参数化工具调用 |

#### T-EXEC-004：Exec Approval 绕过

| 属性 | 值 |
| ----------------------- | ---------------------------------------------------------- |
| **ATLAS ID** | AML.T0043 - 构造对抗性数据 |
| **描述** | 攻击者构造能绕过批准 allowlist 的命令 |
| **攻击向量** | 命令混淆、别名利用、路径操纵 |
| **受影响组件** | `exec-approvals.ts`、命令 allowlist |
| **当前缓解措施** | Allowlist + ask 模式 |
| **剩余风险** | 高 —— 没有命令规范化 |
...
| **当前缓解措施** | GitHub 账户年龄校验、基于模式的审核标记 |
| **剩余风险** | 严重 —— 无沙箱隔离，审查有限 |
...
| **描述** | 攻击者通过指示智能体将数据发送到外部 URL 来实现数据外泄 |
| **攻击向量** | 提示词注入导致智能体向攻击者服务器 POST 数据 |
| **受影响组件** | `web_fetch` 工具 |
| **当前缓解措施** | 对内部网络的 SSRF 阻断 |
| **剩余风险** | 高 —— 外部 URL 仍被允许 |
| **建议** | 实现 URL allowlist、增强数据分类感知能力 |
...
| **描述** | 攻击者在用户系统上执行任意命令 |
| **攻击向量** | 提示词注入结合 exec approval 绕过 |
| **受影响组件** | Bash 工具、命令执行 |
| **当前缓解措施** | Exec approvals、Docker 沙箱选项 |
| **剩余风险** | 严重 —— 无沙箱的宿主机执行 |
| **建议** | 默认启用沙箱，改进 approval UX |
...
| 路径清洗 | `sanitizePath()` | 高 —— 防止路径穿越 |
| 文件类型校验 | `isTextFile()` | 中 —— 仅限文本文件，但仍可能包含恶意内容 |
| 大小限制 | 总 bundle 限制为 50MB | 高 —— 防止资源耗尽 |
| 必需的 SKILL.md | 强制要求 readme | 安全价值低 —— 仅用于信息说明 |
| 模式审核 | `moderation.ts` 中的 `FLAG_RULES` | 低 —— 很容易被绕过 |
| 审核状态 | `moderationStatus` 字段 | 中 —— 支持人工审查 |

### 4.2 审核标记模式

`moderation.ts` 中当前的模式如下：
...
| 社区举报 | 部分完成（`skillReports` 表已存在） | 中 |
| 审计日志 | 部分完成（`auditLogs` 表已存在） | 中 |
...
| R-006 | 改进 exec approval UX 和校验 | T-EXEC-004 |
| R-007 | 为 `web_fetch` 实现 URL allowlist | T-EXFIL-001 |

### 6.3 中期（P2）

| ID | 建议 | 应对对象 |
| ----- | ----------------------------------------------------- | ------------- |
| R-008 | 在可能情况下增加加密级渠道校验 | T-ACCESS-002 |
...
| `src/agents/sandbox/tool-policy.ts` | 工具策略强制 | **严重** |
...
| 术语 | 定义 |
| -------------------- | --------------------------------------------------------- |
| **ATLAS** | MITRE 的面向 AI 系统的对抗性威胁版图 |
| **ClawHub** | OpenClaw 的 Skill 市场 |
| **Gateway 网关** | OpenClaw 的消息路由与认证层 |
| **MCP** | Model Context Protocol —— 工具提供方接口 |
| **Prompt Injection** | 一种将恶意指令嵌入输入中的攻击 |
| **Skill** | 可下载的 OpenClaw 智能体扩展 |
