...
read_when:
    ...
    - 你需要共享的二进制文件预置、持久化和更新流程
summary: 适用于长期运行 OpenClaw Gateway 网关主机的共享 Docker VM 运行时步骤
title: Docker VM 运行时
x-i18n:
    generated_at: "2026-04-23T20:51:53Z"
    model: gpt-5.4
    provider: openai
...
适用于基于 VM 的 Docker 安装（例如 GCP、Hetzner 和类似 VPS 提供商）的共享运行时步骤。
...
Skills 所需的所有外部二进制文件都必须在镜像构建阶段安装。
...
这些只是示例，不是完整列表。
你可以使用相同模式安装任意多个所需二进制文件。

如果你之后添加了依赖额外二进制文件的新 Skills，则必须：
...
如果在 `pnpm install --frozen-lockfile` 期间构建因 `Killed` 或 `exit code 137` 失败，说明 VM 内存不足。
请先使用更大的机器规格，再重试。
...
所有长期状态都必须能够在重启、重建和重启机器后继续保留。

| 组件 | 位置 | 持久化机制 | 说明 |
| ------------------- | --------------------------------- | ---------------------- | ------------------------------------------------------------- |
| Gateway 网关配置 | `/home/node/.openclaw/` | 主机卷挂载 | 包含 `openclaw.json`、`.env` |
| 模型认证配置 | `/home/node/.openclaw/agents/` | 主机卷挂载 | `agents/<agentId>/agent/auth-profiles.json`（OAuth、API keys） |
| Skill 配置 | `/home/node/.openclaw/skills/` | 主机卷挂载 | Skills 级状态 |
| 智能体工作区 | `/home/node/.openclaw/workspace/` | 主机卷挂载 | 代码和智能体构件 |
