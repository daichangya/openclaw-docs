...
`--no-deps --entrypoint node` 的 `openclaw-gateway`
运行新手引导和安装时配置写入。
</Note>
...
`scripts/docker/setup.sh` 默认将 `OPENCLAW_GATEWAY_BIND=lan`，以便宿主机能够通过
...
`tailnet` / `auto`），而不是宿主机别名，如 `0.0.0.0` 或 `127.0.0.1`。
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="共享网络安全说明">
    ...
    信任边界。compose 配置对 `openclaw-cli` 去除了 `NET_RAW`/`NET_ADMIN`，并启用了
    `no-new-privileges`。
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="在沙箱中找不到自定义工具">
    ...
    `/etc/profile`，并且可能会重置 PATH。请设置 `docker.env.PATH` 以预先添加你的
    自定义工具路径，或在你的 Dockerfile 中将脚本添加到 `/etc/profile.d/` 下。
  </Accordion>
  ...
  <Accordion title="Gateway 网关 目标显示为 ws://172.x.x.x，或从 Docker CLI 出现配对错误">
    重置 Gateway 网关 模式和绑定：
